var View = gc.media.View,
	mediaTrash = gc.media.view.settings.mediaTrash,
	l10n = gc.media.view.l10n,
	$ = jQuery,
	AttachmentsBrowser,
	infiniteScrolling = gc.media.view.settings.infiniteScrolling,
	__ = gc.i18n.__,
	sprintf = gc.i18n.sprintf;

/**
 * gc.media.view.AttachmentsBrowser
 *
 ...
 */
AttachmentsBrowser = View.extend(/** @lends gc.media.view.AttachmentsBrowser.prototype */{
	tagName:   'div',
	...
	initialize: function() {
		_.defaults( this.options, {
			...
			display: false,
			sidebar: true,
			AttachmentView: gc.media.view.Attachment.Library
		});
	...
	/**
	 * Updates the `gc.a11y.speak()` ARIA live region with a message to communicate
	...
	announceSearchResults: _.debounce( function() {
		var count,
			...
			mediaFoundHasMoreResultsMessage = __( '显示的媒体项目数量：%d。点击加载更多结果。' );

		if ( infiniteScrolling ) {
			/* translators: Accessibility text. %d: Number of attachments found in a search. */
			mediaFoundHasMoreResultsMessage = __( '显示的媒体数量：%d。向下滚动页面来获取更多结果。' );
		}
		...
		if ( this.collection.hasOwnProperty('mirroring') && this.collection.mirroring.args.s ) {
			...
			if ( 0 === count ) {
				gc.a11y.speak( l10n.noMediaTryNewSearch );
				return;
			}

			if ( this.collection.hasMore() ) {
				gc.a11y.speak( mediaFoundHasMoreResultsMessage.replace( '%d', count ) );
				return;
			}

			gc.a11y.speak( l10n.mediaFound.replace( '%d', count ) );
		}
	}, 200 ),
	...
	/**
	 * @return {gc.media.view.AttachmentsBrowser} Returns itself to allow chaining.
	 */
	dispose: function() {
	...
	createToolbar: function() {
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			toolbarOptions.className = 'media-toolbar gc-filter';
		}

		/**
		* @member {gc.media.view.Toolbar}
		*/
		this.toolbar = new gc.media.view.Toolbar( toolbarOptions );

		this.views.add( this.toolbar );

		this.toolbar.set( 'spinner', new gc.media.view.Spinner({
			priority: -20
		}) );
		...
		if ( showFilterByType || this.options.date ) {
			 ...
			 */
			this.toolbar.set( 'filters-heading', new gc.media.view.Heading( {
				priority:   -100,
		...
		if ( showFilterByType ) {
			...
			this.toolbar.set( 'filtersLabel', new gc.media.view.Label({
			...
			if ( 'uploaded' === this.options.filters ) {
				this.toolbar.set( 'filters', new gc.media.view.AttachmentFilters.Uploaded({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				}).render() );
			} else {
				Filters = new gc.media.view.AttachmentFilters.All({
					controller: this.controller,
					model:      this.collection.props,
					priority:   -80
				});
		...
		if ( this.controller.isModeActive( 'grid' ) ) {
			LibraryViewSwitcher = View.extend({
				...
				template: gc.template( 'media-library-view-switcher')
			});
			...
			this.toolbar.set( 'dateFilterLabel', new gc.media.view.Label({
			...
			this.toolbar.set( 'dateFilter', new gc.media.view.DateFilter({
				controller: this.controller,
				model:      this.collection.props,
				priority: -75
			}).render() );
			...
			this.toolbar.set( 'selectModeToggleButton', new gc.media.view.SelectModeToggleButton({
				text: l10n.bulkSelect,
				controller: this.controller,
				priority: -70
			}).render() );

			this.toolbar.set( 'deleteSelectedButton', new gc.media.view.DeleteSelectedButton({
				filters: Filters,
				style: 'primary',
				disabled: true,
			...
			if ( mediaTrash ) {
				this.toolbar.set( 'deleteSelectedPermanentlyButton', new gc.media.view.DeleteSelectedPermanentlyButton({
					filters: Filters,
		...
		if ( this.options.search ) {
			...
			this.toolbar.set( 'searchLabel', new gc.media.view.Label({
			...
			this.toolbar.set( 'search', new gc.media.view.Search({
				controller: this.controller,
				model:      this.collection.props,
				priority:   60
			}).render() );
	...
	createUploader: function() {
		this.uploader = new gc.media.view.UploaderInline({
			controller: this.controller,
			status:     false,
	...
	createAttachmentsWrapperView: function() {
		this.attachmentsWrapper = new gc.media.View( {
	...
	createAttachments: function() {
		this.attachments = new gc.media.view.Attachments({
			controller:           this.controller,
	...
	createLoadMoreView: function() {
		...
		this.loadMoreButton = new gc.media.view.Button( {
			text: __( '加载更多' ),
		...
		this.loadMoreSpinner = new gc.media.view.Spinner();

		this.loadMoreJumpToFirst = new gc.media.view.Button( {
			text: __( '跳至首个加载的项目' ),
	...
	createAttachmentsHeading: function() {
		this.attachmentsHeading = new gc.media.view.Heading( {
	...
	createSidebar: function() {
		var options = this.options,
			...
			sidebar = this.sidebar = new gc.media.view.Sidebar({
				controller: this.controller
			});
		...
		if ( this.controller.uploader ) {
			sidebar.set( 'uploads', new gc.media.view.UploaderStatus({
				controller: this.controller,
				priority:   40
			}) );
		}
	...
	createSingle: function() {
		...
		sidebar.set( 'details', new gc.media.view.Attachment.Details({
			controller: this.controller,
			model:      single,
			priority:   80
		}) );

		sidebar.set( 'compat', new gc.media.view.AttachmentCompat({
			controller: this.controller,
			model:      single,
			priority:   120
		}) );
		...
		if ( this.options.display ) {
			sidebar.set( 'display', new gc.media.view.Settings.AttachmentDisplay({
				controller:   this.controller,
