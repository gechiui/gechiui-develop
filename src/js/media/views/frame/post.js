 ...
 */
Post = Select.extend(/** @lends gc.media.view.MediaFrame.Post.prototype */{
	initialize: function() {
		this.counts = {
			audio: {
				count: gc.media.view.settings.attachmentCounts.audio,
				state: 'playlist'
			},
			video: {
				count: gc.media.view.settings.attachmentCounts.video,
	...
	createStates: function() {
		...
		this.states.add([
			...
			// Embed states.
			new gc.media.controller.Embed( { metadata: options.metadata } ),
			...
			// Playlist states.
			new gc.media.controller.CollectionEdit({
				type: 'audio',
				collectionType: 'playlist',
				title:          l10n.editPlaylistTitle,
				SettingsView:   gc.media.view.Settings.Playlist,
				library:        options.selection,
				editing:        options.editing,
				menu:           'playlist',
			...
			new gc.media.controller.CollectionAdd({
				type: 'audio',
				collectionType: 'playlist',
		...
		if ( gc.media.view.settings.post.featuredImageId ) {
			this.states.add( new gc.media.controller.FeaturedImage() );
		}
	},
	...
	bindHandlers: function() {
		...
		if ( typeof checkCounts !== 'undefined' ) {
			this.listenTo( gc.media.model.Attachments.all, 'change:type', this.mediaTypeCounts );
		}
	...
	// Menus.
	/**
	 * @param {gc.Backbone.View} view
	 */
	mainMenu: function( view ) {
		view.set({
			'library-separator': new gc.media.View({
				className:  'separator',
				priority:   100,
				attributes: {
					role: 'presentation'
	...
	galleryMenu: function( view ) {
		...
		view.set({
			...
			separateCancel: new gc.media.View({
				className: 'separator',
				priority: 40
			})
		});
	...
	embedContent: function() {
		var view = new gc.media.view.Embed({
			controller: this,
			model:      this.state()
		}).render();
	...
	editSelectionContent: function() {
		...
		view = new gc.media.view.AttachmentsBrowser({
			controller: this,
			...
			AttachmentView: gc.media.view.Attachments.EditSelection
		}).render();
	...
	selectionStatusToolbar: function( view ) {
		...
		view.set( 'selection', new gc.media.view.Selection({
			controller: this,
	...
	mainInsertToolbar: function( view ) {
		...
		view.set( 'insert', {
			 ...
			 * @ignore
			 *
			 * @fires gc.media.controller.State#insert
			 */
			click: function() {
	...
	mainGalleryToolbar: function( view ) {
		...
		view.set( 'gallery', {
			...
			click: function() {
				...
				edit.set( 'library', new gc.media.model.Selection( models, {
	...
	mainEmbedToolbar: function( toolbar ) {
		toolbar.view = new gc.media.view.Toolbar.Embed({
			controller: this
		});
	},
		...
		this.toolbar.set( new gc.media.view.Toolbar({
			...
			items: {
				insert: {
					...
					/**
					 * @fires gc.media.controller.State#reset
					 */
					click: function() {
						var controller = this.controller,
							state = controller.state(),
