...
/**
 * gc.media.view.MediaFrame.Manage
 *
 ...
 */
Manage = MediaFrame.extend(/** @lends gc.media.view.MediaFrame.Manage.prototype */{
	/**
	 * @constructs
	 */
	initialize: function() {
		...
		this.$window = $( window );
		this.$adminBar = $( '#gcadminbar' );
		// Store the Add New button for later reuse in gc.media.view.UploaderInline.
		...
		this.gridRouter = new gc.media.view.MediaFrame.Manage.Router();
	...
	openEditAttachmentModal: function( model ) {
		...
		if ( gc.media.frames.edit ) {
			gc.media.frames.edit.open().trigger( 'refresh', model );
		} else {
			gc.media.frames.edit = gc.media( {
	...
	browseContent: function( contentRegion ) {
		...
		this.browserView = contentRegion.view = new gc.media.view.AttachmentsBrowser({
			controller: this,
		...
		this.errors = gc.Uploader.errors;
	...
	startHistory: function() {
		...
		if ( window.history && window.history.pushState ) {
			...
			Backbone.history.start( {
				root: window._gcMediaGridSettings.adminUrl,
				pushState: true
			} );
		}
	}
