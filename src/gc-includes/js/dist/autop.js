...
/******/ 	return __webpack_require__(__webpack_require__.s = "./node_modules/@gechiui/autop/build-module/index.js");
/******/ })
/************************************************************************/
/******/ ({

/***/ "./node_modules/@gechiui/autop/build-module/index.js":
/*!***********************************************************!*\
  !*** ./node_modules/@gechiui/autop/build-module/index.js ***!
  \***********************************************************/
...
function autop(text) {
  ...
  if (text.indexOf('<pre') !== -1) {
    ...
    for (let i = 0; i < textParts.length; i++) {
      ...
      const name = '<pre gc-pre-tag-' + i + '></pre>';
  ...
  if (-1 !== text.indexOf('<!-- gcnl -->')) {
    text = text.replace(/\s?<!-- gcnl -->\s?/g, '\n');
  }
...
function removep(html) {
  ...
  if (html.indexOf('[caption') !== -1) {
    ...
    html = html.replace(/\[caption[\s\S]+?\[\/caption\]/g, a => {
      return a.replace(/<br([^>]*)>/g, '<gc-temp-br$1>').replace(/[\r\n\t]+/, '');
    });
  ...
  if (preserveLinebreaks) {
    html = html.replace(/<gc-line-break>/g, '\n');
  }

  if (preserveBr) {
    html = html.replace(/<gc-temp-br([^>]*)>/g, '<br$1>');
  } // Restore preserved tags.


  if (preserve.length) {
    html = html.replace(/<gc-preserve>/g, () => {
      return (
        /** @type {string} */
        preserve.shift()
      );
