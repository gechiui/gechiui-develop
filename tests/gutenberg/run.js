...
// Config
const GUTENBERG_VERSION = 'gc/trunk';
...
const sampleEnvConfig = path.resolve( __dirname, '.gc-env.sample.json' );

// Welcome
console.log( title( '>> 🏁 Welcome, this command is going to prepare a running GeChiUI environment and run the Gutenberg e2e tests against it.' ) );
console.log( title( '>> It uses the current GeChiUI folder from which it\'s being run and a separate Gutenberg clone performed by the command.' ) );
...
console.log( 'Test Environment Path (gc-env): ' + success( testEnvironmentPath ) );

// Steps
// 1- Preparing the GeChiUI environment
console.log( '>> Preparing the GeChiUI clone' );
...
runShellScript( 'git clone https://github.com/GeChiUI/gutenberg.git ' + testLauncherPath + ' --depth=1 --no-single-branch' );
runShellScript( 'git checkout ' + GUTENBERG_VERSION, testLauncherPath );
runShellScript( 'npm install && npm run build', testLauncherPath );

// 3- Running the GeChiUI environment using gc-env
// The environment should include the GeChiUI install and the e2e tests plugins.s
...
envConfig.mappings[ 'gc-content/mu-plugins' ] = path.resolve( testLauncherPath, 'packages/e2e-tests/mu-plugins' );
envConfig.mappings[ 'gc-content/plugins/test-plugins' ] = path.resolve( testLauncherPath, 'packages/e2e-tests/plugins' );
fs.writeFileSync(
	path.resolve( testEnvironmentPath, '.gc-env.json' ),
	JSON.stringify( envConfig, null, '\t' ) + '\n'
);

// 4- Starting the environment
console.log( title( '>> Starting the environment' ) );
runShellScript( path.resolve( testLauncherPath, 'node_modules/.bin/gc-env' ) + ' start ', testEnvironmentPath );
