...
jQuery( 'body' ).on( 'load', function() {
	...
	QUnit.test( 'it should parse _gcCustomizeMenusSettings.defaults into itself', function( assert ) {
		assert.deepEqual( window._gcCustomizeNavMenusSettings, api.Menus.data );
	} );

	QUnit.test( 'empty menus should have no Menu Item Controls', function( assert ) {
		assert.ok( 0 === gc.customize.Menus.getMenuControl( socialMenuId ).getMenuItemControls().length, 'empty menus' );
	} );

	QUnit.test( 'populated menus should have no Menu Item Controls', function( assert ) {
		assert.ok( 0 !== gc.customize.Menus.getMenuControl( primaryMenuId ).getMenuItemControls().length, 'non-empty menus' );
	} );
