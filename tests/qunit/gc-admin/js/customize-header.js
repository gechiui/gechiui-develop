...
jQuery( function() {
	QUnit.module('Custom Header: ChoiceList', {
		beforeEach: function() {
			gc.customize.HeaderTool.currentHeader = new gc.customize.HeaderTool.ImageModel();
			this.apiStub = sinon.stub(gc.customize, 'get').returns('foo');
			this.choiceList = new gc.customize.HeaderTool.ChoiceList();
		},
		afterEach: function() {
	...
	QUnit.test('should parse _gcCustomizeHeader.uploads into itself', function( assert ) {
	...
	QUnit.module('Custom Header: DefaultsList', {
		beforeEach: function() {
			gc.customize.HeaderTool.currentHeader = new gc.customize.HeaderTool.ImageModel();
			this.apiStub = sinon.stub(gc.customize, 'get').returns('foo');
			this.choiceList = new gc.customize.HeaderTool.DefaultsList();
		},
		afterEach: function() {
			this.apiStub.restore();
		}
	...
	QUnit.test('it should parse _gcCustomizeHeader.defaults into itself', function( assert ) {
		assert.equal(this.choiceList.length, 4);
	});
	...
	QUnit.module('Custom Header: HeaderImage shouldBeCropped()', {
		beforeEach: function() {
			gc.customize.HeaderTool.currentHeader = new gc.customize.HeaderTool.ImageModel();
			this.model = new gc.customize.HeaderTool.ImageModel();
			this.model.set({
				themeWidth: 1000,
				themeHeight: 200
			});
