...
jQuery( function( $ ) {
	...
	FooSuperClass = gc.customize.Class.extend(
		{
	...
	QUnit.test( 'FooSuperClass instance foo extended Class', function( assert ) {
		assert.equal( foo.extended( gc.customize.Class ), true );
	});
	...
	ConstructorTestClass = gc.customize.Class.extend(
		{
	...
	QUnit.test( 'ConstructorTestClass instance constructorTest extended Class', function( assert ) {
		assert.equal( constructorTest.extended( gc.customize.Class ), true );
	});
	...
	firstValueInstance = new gc.customize.Value( firstInitialValue );
	...
	valuesInstance = new gc.customize.Values();
	...
	QUnit.test( 'Correct events are triggered when adding to or removing from Values collection', function( assert ) {
		var hasFooOnAdd = false,
			...
			fooValue = new gc.customize.Value( 'foo' );
	...
	QUnit.test( 'Notification object exists and has expected properties', function ( assert ) {
		var notification = new gc.customize.Notification( 'mycode', {
			'message': 'Hello World',
			'type': 'update',
			'setting': 'blogname',
			'fromServer': true,
		...
		notification = new gc.customize.Notification( 'mycode2', {
			'message': 'Hello Space'
		} );
	...
	QUnit.test( 'gc.customize.utils.parseQueryString works', function( assert ) {
		var queryParams;
		queryParams = gc.customize.utils.parseQueryString( 'a=1&b=2' );
		assert.ok( _.isEqual( queryParams, { a: '1', b: '2' } ) );

		queryParams = gc.customize.utils.parseQueryString( 'a+b=1&b=Hello%20World' );
		assert.ok( _.isEqual( queryParams, { 'a_b': '1', b: 'Hello World' } ) );

		queryParams = gc.customize.utils.parseQueryString( 'a%20b=1&b=Hello+World' );
		assert.ok( _.isEqual( queryParams, { 'a_b': '1', b: 'Hello World' } ) );

		queryParams = gc.customize.utils.parseQueryString( 'a=1&b' );
