/* global passwordStrength, gc, jQuery */
jQuery( function() {
	...
	QUnit.test( 'user input disallowed list array should contain expected words', function( assert ) {
		var disallowedList = gc.passwordStrength.userInputDisallowedList();

		assert.ok( Array.isArray( disallowedList ), 'disallowed list is an array' );
		assert.ok( jQuery.inArray( 'GeChiUI', disallowedList ) > -1, 'disallowed list contains "GeChiUI" from page title' );
