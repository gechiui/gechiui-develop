...
( function() {
	...
	QUnit.test( 'image widget control', function( assert ) {
		...
		testImageUrl = 'http://s.w.org/style/images/gc-header-logo.png';
		assert.equal( typeof gc.mediaWidgets.controlConstructors.media_image, 'function', 'gc.mediaWidgets.controlConstructors.media_image is a function' );
		ImageWidgetControl = gc.mediaWidgets.controlConstructors.media_image;
		assert.ok( ImageWidgetControl.prototype instanceof gc.mediaWidgets.MediaWidgetControl, 'gc.mediaWidgets.controlConstructors.media_image subclasses gc.mediaWidgets.MediaWidgetControl' );

		imageWidgetModelInstance = new gc.mediaWidgets.modelConstructors.media_image();
		...
		templateProps = imageWidgetControlInstance.mapModelToPreviewTemplateProps();
		assert.equal( templateProps.currentFilename, 'gc-header-logo.png', 'mapModelToPreviewTemplateProps should set currentFilename based off of url' );
		...
		mappedProps = imageWidgetControlInstance.mapMediaToModelProps( { link: 'post', postUrl: 'https://www.gechiui.com/image-2/' } );
		assert.equal( mappedProps.link_url, 'https://www.gechiui.com/image-2/', 'mapMediaToModelProps should set file link_url according to mediaFrameProps.link' );
		mappedProps = imageWidgetControlInstance.mapMediaToModelProps( { link: 'custom', linkUrl: 'https://www.gechiui.com' } );
		assert.equal( mappedProps.link_url, 'https://www.gechiui.com', 'mapMediaToModelProps should set custom link_url according to mediaFrameProps.linkUrl' );

		// Test mapModelToMediaFrameProps().
		imageWidgetControlInstance.model.set({ error: false, url: testImageUrl, 'link_type': 'custom', 'link_url': 'https://www.gechiui.com', 'size': 'custom', 'width': 100, 'height': 150, 'title': 'widget title', 'image_title': 'title of image' });
		mappedProps = imageWidgetControlInstance.mapModelToMediaFrameProps( imageWidgetControlInstance.model.toJSON() );
		assert.equal( mappedProps.linkUrl, 'https://www.gechiui.com', 'mapModelToMediaFrameProps should set linkUrl from model.link_url' );
	...
	QUnit.test( 'image widget control renderPreview', function( assert ) {
		...
		imageWidgetModelInstance = new gc.mediaWidgets.modelConstructors.media_image();
		imageWidgetControlInstance = new gc.mediaWidgets.controlConstructors.media_image({
			el: jQuery( '<div></div>' ),
			syncContainer: jQuery( '<div></div>' ),
			model: imageWidgetModelInstance
		});
		assert.equal( imageWidgetControlInstance.$el.find( 'img' ).length, 0, 'No images should be rendered' );
		imageWidgetControlInstance.model.set({ error: false, url: 'http://s.w.org/style/images/gc-header-logo.png' });

		// Due to renderPreview being deferred.
		setTimeout( function() {
			assert.equal( imageWidgetControlInstance.$el.find( 'img[src="http://s.w.org/style/images/gc-header-logo.png"]' ).length, 1, 'One image should be rendered' );
			done();
		}, 50 );
	...
	QUnit.test( 'image media model', function( assert ) {
		...
		assert.equal( typeof gc.mediaWidgets.modelConstructors.media_image, 'function', 'gc.mediaWidgets.modelConstructors.media_image is a function' );
		ImageWidgetModel = gc.mediaWidgets.modelConstructors.media_image;
		assert.ok( ImageWidgetModel.prototype instanceof gc.mediaWidgets.MediaWidgetModel, 'gc.mediaWidgets.modelConstructors.media_image subclasses gc.mediaWidgets.MediaWidgetModel' );
