...
gc.customize.settingConstructor.abbreviation = gc.customize.Setting.extend({
...
jQuery( 'body' ).on( 'load', function() {
	...
	testCustomizerModel = function( model, expectedValues ) {
		if ( ! expectedValues.type || ! gc.customize[ expectedValues.type ] ) {
		...
		QUnit.test( 'Model extends proper type', function( assert ) {
			assert.ok( model.extended( gc.customize[ type ] ) );
		} );
	...
	QUnit.test( 'Notifications collection exists', function( assert ) {
		assert.ok( gc.customize.notifications );
		assert.equal( gc.customize.notifications.defaultConstructor, gc.customize.Notification );
	} );
	...
	QUnit.test( 'Notification objects are rendered as part of notifications collection', function( assert ) {
		...
		collection = new gc.customize.Notifications({
			container: container
		});
		collection.add( 'mycode-1', new gc.customize.Notification( 'mycode-1', { message: 'My message 1' } ) );
		collection.render();
		items = collection.container.find( 'li' );
		assert.equal( items.length, 1 );
		assert.equal( items.first().data( 'code' ), 'mycode-1' );

		collection.add( 'mycode-2', new gc.customize.Notification( 'mycode-2', {
	...
	QUnit.test( 'Previewed device defaults to desktop.', function( assert ) {
		assert.equal( gc.customize.previewedDevice.get(), 'desktop' );
	} );

	QUnit.module( 'Customizer Setting in Fixture' );
	QUnit.test( 'Setting has fixture value', function( assert ) {
		assert.equal( gc.customize( 'fixture-setting' )(), 'Lorem Ipsum' );
	} );
	QUnit.test( 'Setting has notifications', function( assert ) {
		var setting = gc.customize( 'fixture-setting' );
		assert.ok( setting.notifications.extended( gc.customize.Values ) );
		assert.equal( gc.customize.Notification, setting.notifications.prototype.constructor.defaultConstructor );
	} );
	QUnit.test( 'Setting has findControls method', function( assert ) {
		var controls, setting = gc.customize( 'fixture-setting' );
	...
	QUnit.test( 'Setting constructor object exists', function( assert ) {
		assert.ok( _.isObject( gc.customize.settingConstructor ) );
	} );
	QUnit.test( 'Custom setting constructor is used', function( assert ) {
		var setting = gc.customize( 'fixture-setting-abbr' );
		assert.ok( setting.extended( gc.customize.settingConstructor.abbreviation ) );
		setting.set( 'usa' );
	...
	QUnit.test( 'Control exists', function( assert ) {
		assert.ok( gc.customize.control.has( 'fixture-control' ) );
	} );
	QUnit.test( 'Control has the fixture setting', function( assert ) {
		var control = gc.customize.control( 'fixture-control' );
	...
	QUnit.test( 'Control has notifications', function ( assert ) {
		var control = gc.customize.control( 'fixture-control' ), settingNotification, controlOnlyNotification, doneEmbedded;
		assert.ok( control.notifications.extended( gc.customize.Values ) );
		assert.equal( gc.customize.Notification, control.notifications.prototype.constructor.defaultConstructor );
		...
		control.deferred.embedded.done( function() {
			...
			settingNotification = new gc.customize.Notification( 'setting_invalidity', { message: 'Invalid setting' } );
			controlOnlyNotification = new gc.customize.Notification( 'control_invalidity', { message: 'Invalid control' } );
	...
	QUnit.test( 'Control can be created without settings', function( assert ) {
		var control = new gc.customize.Control( 'settingless', {
			params: {
				content: jQuery( '<li class="settingless">Hello World</li>' ),
				section: 'fixture-section'
			}
		} );
		gc.customize.control.add( control.id, control );
	...
	QUnit.test( 'Fixture section exists', function( assert ) {
		assert.ok( gc.customize.section.has( 'fixture-section' ) );
	} );
	QUnit.test( 'Fixture section has control among controls()', function( assert ) {
		var section = gc.customize.section( 'fixture-section' );
	...
	QUnit.test( 'Fixture section exists', function( assert ) {
		assert.ok( gc.customize.section.has( 'fixture-section-titleless-templated' ) );
	} );
	QUnit.test( 'Fixture section has expected content', function( assert ) {
	...
	QUnit.test( 'Fixture panel exists', function( assert ) {
		assert.ok( gc.customize.panel.has( 'fixture-panel' ) );
	} );
	QUnit.test( 'Fixture panel has content', function( assert ) {
		var panel = gc.customize.panel( 'fixture-panel' );
	...
	QUnit.test( 'Focusing on a control will expand the panel and section', function( assert ) {
		...
		panel = gc.customize.panel( 'fixture-panel' );
		section = gc.customize.section( 'fixture-section' );
		control = gc.customize.control( 'fixture-control' );
		assert.ok( ! panel.expanded() );
	...
	QUnit.test( 'Fixture panel exists', function( assert ) {
		assert.ok( gc.customize.panel.has( 'fixture-panel-titleless-templated' ) );
	} );
	QUnit.test( 'Fixture panel has expected content', function( assert ) {
	...
	QUnit.test( 'Create a new setting', function( assert ) {
		mockSetting = gc.customize.create(
			settingId,
			settingId,
			settingValue,
			{
				transport: 'refresh',
				previewer: gc.customize.previewer
			}
		);
	...
	mockSection = new gc.customize.Section( sectionId, { params: sectionData } );
	...
	gc.customize.section.add( sectionId, mockSection );

	QUnit.test( 'Section instance added to the gc.customize.section object', function( assert ) {
		assert.ok( gc.customize.section.has( sectionId ) );
	});

	sectionInstance = gc.customize.section( sectionId );

	QUnit.test( 'Section instance has right content when accessed from gc.customize.section()', function( assert ) {
	...
	mockControl = new gc.customize.Control( controlId, {
		params: controlData,
		previewer: gc.customize.previewer
	});
	...
	gc.customize.control.add( controlId, mockControl );

	QUnit.test( 'Control instance was added to the control class.', function( assert ) {
		assert.ok( gc.customize.control.has( controlId ) );
	});

	mockControlInstance = gc.customize.control( controlId );
	...
	mockPanel = new gc.customize.Panel( panelId, { params: panelData } );
	...
	QUnit.module( 'Test gc.customize.findControlsForSettings' );
	QUnit.test( 'findControlsForSettings(blogname)', function( assert ) {
		...
		assert.ok( gc.customize.control.has( controlId ) );
		assert.ok( gc.customize.has( settingId ) );
		controlsForSettings = gc.customize.findControlsForSettings( [ settingId ] );
		...
		assert.equal( gc.customize.control( controlId ), controlsForSettings['fixture-setting'][0] );
	} );

	QUnit.module( 'Customize Controls gc.customize.dirtyValues' );
	QUnit.test( 'dirtyValues() returns expected values', function( assert ) {
		gc.customize.state( 'changesetStatus' ).set( 'auto-draft' );
		gc.customize.each( function( setting ) {
			setting._dirty = false;
		} );
		assert.ok( _.isEmpty( gc.customize.dirtyValues() ) );
		assert.ok( _.isEmpty( gc.customize.dirtyValues( { unsaved: false } ) ) );

		gc.customize( 'fixture-setting' )._dirty = true;
		assert.ok( ! _.isEmpty( gc.customize.dirtyValues() ) );
		assert.ok( _.isEmpty( gc.customize.dirtyValues( { unsaved: true } ) ) );

		gc.customize( 'fixture-setting' ).set( 'Modified' );
		assert.ok( ! _.isEmpty( gc.customize.dirtyValues() ) );
		assert.ok( ! _.isEmpty( gc.customize.dirtyValues( { unsaved: true } ) ) );
		assert.equal( 'Modified', gc.customize.dirtyValues()['fixture-setting'] );

		// When the changeset does not exist, all dirty settings are necessarily unsaved.
		gc.customize.state( 'changesetStatus' ).set( '' );
		gc.customize( 'fixture-setting' )._dirty = true;
		assert.ok( ! _.isEmpty( gc.customize.dirtyValues() ) );
		assert.ok( ! _.isEmpty( gc.customize.dirtyValues( { unsaved: true } ) ) );
	} );

	QUnit.module( 'Customize Controls: gc.customize.requestChangesetUpdate()' );
	QUnit.test( 'requestChangesetUpdate makes request and returns promise', function( assert ) {
		...
		jQuery.ajaxSetup( {
			beforeSend: function( e, data ) {
				...
				queryParams = gc.customize.utils.parseQueryString( data.data );
				...
				assert.equal( gc.customize.settings.changeset.uuid, queryParams.customize_changeset_uuid );
				assert.equal( 'on', queryParams.gc_customize );
		...
		request = gc.customize.requestChangesetUpdate();
		assert.equal( 'resolved', request.state());
		request.done( function( data ) {
			assert.ok( _.isEqual( {}, data ) );
		} );

		gc.customize( 'fixture-setting' ).set( 'requestChangesetUpdate' );
	...
	QUnit.module( 'Customize Utils: gc.customize.utils.getRemainingTime()' );
	QUnit.test( 'utils.getRemainingTime calculates time correctly', function( assert ) {
		...
		timeRemaining = gc.customize.utils.getRemainingTime( datetime );
		timeRemainingWithDateInstance = gc.customize.utils.getRemainingTime( new Date( datetime.replace( /-/g, '/' ) ) );
		timeRemaingingWithTimestamp = gc.customize.utils.getRemainingTime( ( new Date( datetime.replace( /-/g, '/' ) ) ).getTime() );
	...
	QUnit.module( 'Customize Utils: gc.customize.utils.getCurrentTimestamp()' );
	QUnit.test( 'utils.getCurrentTimestamp returns timestamp', function( assert ) {
		var currentTimeStamp;
		currentTimeStamp = gc.customize.utils.getCurrentTimestamp();
		assert.equal( typeof currentTimeStamp, 'number' );
	});

	QUnit.module( 'Customize Controls: gc.customize.DateTimeControl' );
	QUnit.test( 'Test DateTimeControl creation and its methods', function( assert ) {
		...
		section = gc.customize.section( sectionId );

		control = new gc.customize.DateTimeControl( controlId, {
			params: {
				section: section.id,
				type: 'date_time',
				setting: new gc.customize.Value( datetime ),
				includeTime: true,
				content: '<li id="customize-control-' + controlId + '" class="customize-control"></li>'
			}
		} );

		gc.customize.control.add( controlId, control );
		...
		// Tear down.
		gc.customize.control.remove( controlId );
	});

	QUnit.module( 'Customize Sections: gc.customize.OuterSection' );
	QUnit.test( 'Test OuterSection', function( assert ) {
		...
		defaultSection = gc.customize.section( defaultSectionId );

		section = new gc.customize.OuterSection( sectionId, {
			params: {
		...
		gc.customize.section.add( sectionId, section );
		gc.customize.section.add( defaultSectionId, section );
		...
		// Tear down.
		gc.customize.section.remove( sectionId );
	});
		...
		section.deferred.embedded.done( function() {
			_.each( section.controls(), function( control ) {
				if ( 'changeset_preview_link' === control.id ) {
					...
					// Test control.ready().
					newLink = 'http://example.org?' + gc.customize.settings.changeset.uuid;
					...
					assert.equal( control.previewElements.url.element.parent().attr( 'target' ), gc.customize.settings.changeset.uuid );
					...
					assert.equal( control.setting.get(), gc.customize.previewer.getFrontendPreviewUrl() );
				}
			} );
		} );
	});
