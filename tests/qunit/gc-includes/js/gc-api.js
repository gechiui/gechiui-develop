...
( function( QUnit ) {
	...
	QUnit.test( 'API Loaded correctly', function( assert ) {
		...
		assert.ok( gc.api.loadPromise );

		gc.api.loadPromise.done( function() {
			assert.ok( gc.api.models );
			done();
		} );
	...
	_.each( collectionClassNames, function( className ) {
		QUnit.test( 'Testing ' + className + ' collection.', function( assert ) {
			...
			gc.api.loadPromise.done( function() {
				var theCollection = new gc.api.collections[ className ]();
				assert.ok(
					theCollection,
					'We can instantiate gc.api.collections.' + className
				);
					...
					assert.equal(
						...
						if ( ! _.isUndefined( collectionHelperTest ) ) {
							...
							assert.equal(
								...
								firstModel.get( 'type' ),
								'The gc.api.collections.' + className + ' is of type ' + collectionHelperTest.returnsModelType
							);
	...
	_.each( modelsWithIdsClassNames, function( className ) {
		...
		QUnit.test( 'Checking ' + className + ' model.', function( assert ) {
			...
			gc.api.loadPromise.done( function() {
				var theModel = new gc.api.models[ className ]();
				assert.ok( theModel, 'We can instantiate gc.api.models.' + className );
				theModel.fetch().done( function(  ) {
					var theModel2 = new gc.api.models[ className ]();
	...
	_.each( modelsToFetchByRoute, function( model ) {
		QUnit.test( 'Test fetching ' + model + ' by route.', function( assert ) {
			...
			gc.api.loadPromise.done( function() {
				...
				var theModel = gc.api.models[ model ];
				var route = theModel.prototype.route.index;

				assert.equal(
					gc.api.getModelByRoute( route ),
					theModel,
					'gc.api.models.' + model + ' found at route ' + route
				);
	...
	_.each( collectionsToFetchByRoute, function( collection ) {
		QUnit.test( 'Test fetching ' + collection + ' by route.', function( assert ) {
			...
			gc.api.loadPromise.done( function() {
				...
				var theCollection = gc.api.collections[ collection ];
				var route = theCollection.prototype.route.index;

				assert.equal(
					gc.api.getCollectionByRoute( route ),
					theCollection,
					'gc.api.collections.' + collection + ' found at ' + route
				);
		...
		_.each( customClasses, function( className ) {
			QUnit.test( 'Checking ' + className + ' class name.', function( assert ) {
				...
				gc.api.loadPromise.done( function() {
					...
					assert.ok( theModel, 'We can instantiate gc.api.models.' + className );
					var theCollection = new gc.api.collections[ className ]();
					assert.ok( theCollection, 'We can instantiate gc.api.collections.' + className );
					// Trigger Qunit async completion.
					done();
				} );
			} );
	...
	gc.api.loadPromise.done( function() {
		QUnit.test( 'Checking connecting to a remote url.', function( assert ) {
			...
			gc.api.init({
				'apiRoot': 'http://remotehost/gc-json/'
			} ).done( function(){
				var lastEndpoint = gc.api.endpoints.last(),
					models = lastEndpoint.get( 'models' ),
					post = new models.Post();

				assert.equal( 'http://remotehost/gc-json/gc/v2/posts', post.url(), 'The remote API objects should have their own URLs' );

				gc.api.init({
					'apiRoot': 'http://localhost/gc-json/'
				} ).done( function(){
					var lastEndpoint = gc.api.endpoints.first(),
						models = lastEndpoint.get( 'models' ),
						post = new models.Post();

					assert.equal( 'http://localhost/gc-json/gc/v2/posts', post.url(), 'The local API objects should have their own URLs' );
	...
	_.each( modelsThatNeedrequireForceForDelete, function( model ) {
		QUnit.test( 'Test requireForceForDelete is correct for ' + model.name, function( assert ) {
			...
			gc.api.loadPromise.done( function() {
				...
				var theModel = new gc.api.models[ model.name ]();

				// Verify the model's requireForceForDelete is set as expected.
				assert.equal(
					theModel.requireForceForDelete,
					model.expect,
					'gc.api.models.' + model.name + '.requireForceForDelete should be ' + model.expect + '.'
				);
	...
	_.each( theModelTypesWithMeta, function( modelType ) {
		...
		gc.api.loadPromise.done( function() {
			QUnit.test( 'Check meta support for ' + modelType + '.', function( assert ) {
				var theModels = new gc.api.collections[ modelType ]();
