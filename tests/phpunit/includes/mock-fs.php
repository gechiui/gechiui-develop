<?php
class GC_Filesystem_MockFS extends GC_Filesystem_Base {
	private $cwd;
	...
	public $verbose = false; // Enable to debug GC_Filesystem_Base::find_folder() / etc.
	public $errors  = array();
	...
	public function abspath( $path = false ) {
		...
		// Perhaps the FTP folder is rooted at the GeChiUI installation.
		// Check for gc-includes folder in root, could have some false positives, but rare.
		if ( ! $folder && $this->is_dir( '/gc-includes' ) ) {
			$folder = '/';
		}
		return $folder;
	}
	...
	// Here starteth the GC_Filesystem functions.
