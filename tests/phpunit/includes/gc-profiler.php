...
/*
 * A simple manually-instrumented profiler for GeChiUI.
 *
...
class GCProfiler {
	...
	public function start( $name ) {
		...
		// Reset the gcdb queries log, storing it on the profile stack if necessary.
		global $gcdb;
		if ( $this->stack ) {
			$this->stack[ count( $this->stack ) - 1 ]['queries'] = $gcdb->queries;
		}
		$gcdb->queries = array();
		...
		$this->stack[] = array(
			'start'               => $time,
			'name'                => $name,
			'cache_cold_hits'     => $gc_object_cache->cold_cache_hits,
			'cache_warm_hits'     => $gc_object_cache->warm_cache_hits,
			'cache_misses'        => $gc_object_cache->cache_misses,
			'cache_dirty_objects' => $this->_dirty_objects_count( $gc_object_cache->dirty_objects ),
			'actions'             => array(),
			'filters'             => array(),
			'queries'             => array(),
		);
	...
	public function stop() {
		...
		global $gcdb;
		$item['queries'] = $gcdb->queries;
		global $gc_object_cache;

		$cache_dirty_count = $this->_dirty_objects_count( $gc_object_cache->dirty_objects );
		...
		if ( isset( $this->profile[ $name ] ) ) {
			...
			$this->profile[ $name ]['cache_cold_hits']    += ( $gc_object_cache->cold_cache_hits - $item['cache_cold_hits'] );
			$this->profile[ $name ]['cache_warm_hits']    += ( $gc_object_cache->warm_cache_hits - $item['cache_warm_hits'] );
			$this->profile[ $name ]['cache_misses']       += ( $gc_object_cache->cache_misses - $item['cache_misses'] );
			...
			$this->profile[ $name ] = array(
				'time'                        => $time,
				'calls'                       => 1,
				'cache_cold_hits'             => ( $gc_object_cache->cold_cache_hits - $item['cache_cold_hits'] ),
				'cache_warm_hits'             => ( $gc_object_cache->warm_cache_hits - $item['cache_warm_hits'] ),
				'cache_misses'                => ( $gc_object_cache->cache_misses - $item['cache_misses'] ),
	...
	public function _current_action() {
		global $gc_actions;
		return $gc_actions[ count( $gc_actions ) - 1 ];
	}
	...
	public function _query_count( $queries ) {
		// This requires the SAVEQUERIES patch at https://core.trac.gechiui.com/ticket/5218
		$out = array();
		foreach ( $queries as $q ) {
			if ( empty( $q[2] ) ) {
				$out['unknown']++;
...
function gcpf_start( $name ) {
	$GLOBALS['gcpf']->start( $name );
}
...
function gcpf_results() {
	return $GLOBALS['gcpf']->results();
}

function gcpf_print_summary() {
	$GLOBALS['gcpf']->print_summary();
}
