 ...
 * @method GC_Term create_and_get( $args = array(), $generation_definitions = null )
 * @method int[] create_many( $count, $args = array(), $generation_definitions = null )
 */
class GC_UnitTest_Factory_For_Term extends GC_UnitTest_Factory_For_Thing {
	...
	public function __construct( $factory = null, $taxonomy = null ) {
		...
		$this->default_generation_definitions = array(
			'name'        => new GC_UnitTest_Generator_Sequence( 'Term %s' ),
			'taxonomy'    => $this->taxonomy,
			'description' => new GC_UnitTest_Generator_Sequence( 'Term description %s' ),
		);
	}
	...
	public function create_object( $args ) {
		...
		$term_id_pair = gc_insert_term( $args['name'], $args['taxonomy'], $args );
		if ( is_gc_error( $term_id_pair ) ) {
			return $term_id_pair;
		}
	...
	public function update_object( $term, $fields ) {
		...
		$term_id_pair = gc_update_term( $term, $taxonomy, $fields );
		return $term_id_pair['term_id'];
	}
	...
	public function add_post_terms( $post_id, $terms, $taxonomy, $append = true ) {
		return gc_set_post_terms( $post_id, $terms, $taxonomy, $append );
	}
	 ...
	 *
	 * @return GC_Term|GC_Error|null GC_Term on success. GC_Error if taxonomy does not exist. Null for miscellaneous failure.
	 */
	public function create_and_get( $args = array(), $generation_definitions = null ) {
		$term_id = $this->create( $args, $generation_definitions );

		if ( is_gc_error( $term_id ) ) {
			return $term_id;
		}
