 ...
 * @method GC_Site create_and_get( $args = array(), $generation_definitions = null )
 * @method int[] create_many( $count, $args = array(), $generation_definitions = null )
 */
class GC_UnitTest_Factory_For_Blog extends GC_UnitTest_Factory_For_Thing {
	...
	public function __construct( $factory = null ) {
		...
		$this->default_generation_definitions = array(
			...
			'path'       => new GC_UnitTest_Generator_Sequence( $base . 'testpath%s' ),
			'title'      => new GC_UnitTest_Generator_Sequence( 'Site %s' ),
	 ...
	 *
	 * @return int|GC_Error The site ID on success, GC_Error object on failure.
	 */
	public function create_object( $args ) {
		global $gcdb;

		// Map some arguments for backward compatibility with `gcmu_create_blog()` previously used here.
		...
		$blog = gc_insert_site( $args );

		$gcdb->suppress_errors( $suppress );

		// Tell GC we're done installing.
		gc_installing( false );
	 ...
	 *
	 * @return GC_Site|null The site object on success, null on failure.
	 */
