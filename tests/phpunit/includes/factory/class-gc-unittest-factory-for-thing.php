...
/**
 * An abstract class that serves as a basis for all GeChiUI object-type factory classes.
 */
abstract class GC_UnitTest_Factory_For_Thing {
	 ...
	 * can be generators -- an object with next() method. There are some default generators: {@link GC_UnitTest_Generator_Sequence},
	 * {@link GC_UnitTest_Generator_Locale_Name}, {@link GC_UnitTest_Factory_Callback_After_Create}.
	 */
	...
	public function create( $args = array(), $generation_definitions = null ) {
		...
		if ( ! $created || is_gc_error( $created ) ) {
			return $created;
		}
		...
		if ( $callbacks ) {
			...
			if ( ! $save_result || is_gc_error( $save_result ) ) {
				return $save_result;
			}
		}
		return $created;
	 ...
	 *
	 * @return array|GC_Error Combined array on success. GC_Error when default value is incorrent.
	 */
	public function generate_args( $args = array(), $generation_definitions = null, &$callbacks = null ) {
		...
		$gen = new GC_UnitTest_Generator_Sequence();
		...
		foreach ( array_keys( $generation_definitions ) as $field_name ) {
			if ( ! isset( $args[ $field_name ] ) ) {
				...
				if ( is_scalar( $generator ) ) {
					...
					return new GC_Error( 'invalid_argument', 'Factory default value should be either a scalar or an generator object.' );
				}
			}
		}
	 ...
	 *
	 * @param GC_UnitTest_Factory_Callback_After_Create[] $callbacks Array with callback functions.
	 ...
	 *
	 * @return GC_UnitTest_Factory_Callback_After_Create
	 */
	public function callback( $function ) {
		return new GC_UnitTest_Factory_Callback_After_Create( $function );
	}
