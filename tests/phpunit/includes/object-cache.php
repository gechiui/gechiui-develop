 ...
 */
function gc_cache_add( $key, $value, $group = '', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->add( $key, $value, $group, $expiration );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_add_by_key( $server_key, $key, $value, $group = '', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->addByKey( $server_key, $key, $value, $group, $expiration );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_add_server( $host, $port, $weight = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->addServer( $host, $port, $weight );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_add_servers( $servers ) {
	global $gc_object_cache;
	return $gc_object_cache->addServers( $servers );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_append( $key, $value, $group = '' ) {
	global $gc_object_cache;
	return $gc_object_cache->append( $key, $value, $group );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_append_by_key( $server_key, $key, $value, $group = '' ) {
	global $gc_object_cache;
	return $gc_object_cache->appendByKey( $server_key, $key, $value, $group );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_cas( $cas_token, $key, $value, $group = '', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->cas( $cas_token, $key, $value, $group, $expiration );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_cas_by_key( $cas_token, $server_key, $key, $value, $group = '', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->casByKey( $cas_token, $server_key, $key, $value, $group, $expiration );
}

/**
 * Closes the cache.
 *
 * This function has ceased to do anything since GeChiUI 2.5.
 ...
 */
function gc_cache_decrement( $key, $offset = 1, $group = '' ) {
	global $gc_object_cache;
	return $gc_object_cache->decrement( $key, $offset, $group );
}

/**
 * Decrements a numeric item's value.
 *
 * This is the same as gc_cache_decrement(), but kept for backward compatibility.
 * The original GeChiUI caching backends use gc_cache_decr().
 *
 * @link https://www.php.net/manual/en/memcached.decrement.php
 *
 * @param string $key    The key under which to store the value.
...
function gc_cache_decr( $key, $offset = 1, $group = '' ) {
	return gc_cache_decrement( $key, $offset, $group );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_delete( $key, $group = '', $time = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->delete( $key, $group, $time );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_delete_by_key( $server_key, $key, $group = '', $time = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->deleteByKey( $server_key, $key, $group, $time );
}
 ...
 */
function gc_cache_fetch() {
	global $gc_object_cache;
	return $gc_object_cache->fetch();
}
 ...
 */
function gc_cache_fetch_all() {
	global $gc_object_cache;
	return $gc_object_cache->fetchAll();
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_flush( $delay = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->flush( $delay );
}
 ...
 * with the native gc_cache_get() function.
 *
 ...
 */
function gc_cache_get( $key, $group = '', $force = false, &$found = null, $cache_cb = null, &$cas_token = null ) {
	global $gc_object_cache;

	if ( func_num_args() > 4 ) {
		return $gc_object_cache->get( $key, $group, $force, $found, '', false, $cache_cb, $cas_token );
	} else {
 ...
 * @return bool|mixed Cached object value.
 */
function gc_cache_get_by_key( $server_key, $key, $group = '', $force = false, &$found = null, $cache_cb = null, &$cas_token = null ) {
	global $gc_object_cache;

	if ( func_num_args() > 5 ) {
		return $gc_object_cache->getByKey( $server_key, $key, $group, $force, $found, $cache_cb, $cas_token );
	} else {
		return $gc_object_cache->getByKey( $server_key, $key, $group, $force, $found );
	}
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_get_delayed( $keys, $groups = '', $with_cas = false, $value_cb = null ) {
	global $gc_object_cache;
	return $gc_object_cache->getDelayed( $keys, $groups, $with_cas, $value_cb );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_get_delayed_by_key( $server_key, $keys, $groups = '', $with_cas = false, $value_cb = null ) {
	global $gc_object_cache;
	return $gc_object_cache->getDelayedByKey( $server_key, $keys, $groups, $with_cas, $value_cb );
}
 ...
 */
function gc_cache_get_multi( $keys, $groups = '', &$cas_tokens = null, $flags = null ) {
	global $gc_object_cache;

	if ( func_num_args() > 2 ) {
		return $gc_object_cache->getMulti( $keys, $groups, '', $cas_tokens, $flags );
	} else {
		return $gc_object_cache->getMulti( $keys, $groups );
	}
}
 ...
 * @return bool|array The array of found items on success, false on failure.
 */
function gc_cache_get_multi_by_key( $server_key, $keys, $groups = '', &$cas_tokens = null, $flags = null ) {
	global $gc_object_cache;

	if ( func_num_args() > 3 ) {
		return $gc_object_cache->getMultiByKey( $server_key, $keys, $groups, $cas_tokens, $flags );
	} else {
		return $gc_object_cache->getMultiByKey( $server_key, $keys, $groups );
	}
}
 ...
 */
function gc_cache_get_option( $option ) {
	global $gc_object_cache;
	return $gc_object_cache->getOption( $option );
}
 ...
 */
function gc_cache_get_result_code() {
	global $gc_object_cache;
	return $gc_object_cache->getResultCode();
}
 ...
 */
function gc_cache_get_result_message() {
	global $gc_object_cache;
	return $gc_object_cache->getResultMessage();
}
 ...
 */
function gc_cache_get_server_by_key( $server_key ) {
	global $gc_object_cache;
	return $gc_object_cache->getServerByKey( $server_key );
}
 ...
 */
function gc_cache_get_server_list() {
	global $gc_object_cache;
	return $gc_object_cache->getServerList();
}
 ...
 */
function gc_cache_get_stats() {
	global $gc_object_cache;
	return $gc_object_cache->getStats();
}
 ...
 */
function gc_cache_get_version() {
	global $gc_object_cache;
	return $gc_object_cache->getVersion();
}
 ...
 * @return int|bool Item's new value on success, false on failure.
 */
function gc_cache_increment( $key, $offset = 1, $group = '' ) {
	global $gc_object_cache;
	return $gc_object_cache->increment( $key, $offset, $group );
}

/**
 * Increments a numeric item's value.
 *
 * This is the same as gc_cache_increment(), but kept for backward compatibility.
 * The original GeChiUI caching backends use gc_cache_incr().
 *
 * @link https://www.php.net/manual/en/memcached.increment.php
 *
 * @param string $key    The key under which to store the value.
...
function gc_cache_incr( $key, $offset = 1, $group = '' ) {
	return gc_cache_increment( $key, $offset, $group );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_prepend( $key, $value, $group = '' ) {
	global $gc_object_cache;
	return $gc_object_cache->prepend( $key, $value, $group );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_prepend_by_key( $server_key, $key, $value, $group = '' ) {
	global $gc_object_cache;
	return $gc_object_cache->prependByKey( $server_key, $key, $value, $group );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_replace( $key, $value, $group = '', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->replace( $key, $value, $group, $expiration );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_replace_by_key( $server_key, $key, $value, $group = '', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->replaceByKey( $server_key, $key, $value, $group, $expiration );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_set( $key, $value, $group = '', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->set( $key, $value, $group, $expiration );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_set_by_key( $server_key, $key, $value, $group = '', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->setByKey( $server_key, $key, $value, $group, $expiration );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_set_multi( $items, $groups = '', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->setMulti( $items, $groups, $expiration );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_set_multi_by_key( $server_key, $items, $groups = 'default', $expiration = 0 ) {
	global $gc_object_cache;
	return $gc_object_cache->setMultiByKey( $server_key, $items, $groups, $expiration );
}
 ...
 * @return bool True on success, false on failure.
 */
function gc_cache_set_option( $option, $value ) {
	global $gc_object_cache;
	return $gc_object_cache->setOption( $option, $value );
}
...
function gc_cache_switch_to_blog( $blog_id ) {
	global $gc_object_cache;
	return $gc_object_cache->switch_to_blog( $blog_id );
}
 ...
 *
 * @global GC_Object_Cache $gc_object_cache GeChiUI Object Cache
 * @return void
 */
function gc_cache_init() {
	global $gc_object_cache;
	$gc_object_cache = new GC_Object_Cache();
}
...
function gc_cache_add_non_persistent_groups( $groups ) {
	global $gc_object_cache;
	$gc_object_cache->add_non_persistent_groups( $groups );
}

// phpcs:disable GeChiUI.NamingConventions.ValidFunctionName.MethodNameInvalid
class GC_Object_Cache {
	...
	public function __construct( $persistent_id = null ) {
		 ...
		 * and for multi single GC installations on the same server.
		 */
		if ( ! defined( 'GC_CACHE_KEY_SALT' ) ) {
			define( 'GC_CACHE_KEY_SALT', '' );
		}
	...
	public function add( $key, $value, $group = 'default', $expiration = 0, $server_key = '', $by_key = false ) {
		/*
		 * Ensuring that gc_suspend_cache_addition is defined before calling, because sometimes an advanced-cache.php
		 * file will load object-cache.php before gc-includes/functions.php is loaded. In those cases, if gc_cache_add
		 * is called in advanced-cache.php before any more of GeChiUI is loaded, we get a fatal error because
		 * gc_suspend_cache_addition will not be defined until gc-includes/functions.php is loaded.
		 */
		if ( function_exists( 'gc_suspend_cache_addition' ) && gc_suspend_cache_addition() ) {
			return false;
		}
	 ...
	 * Alias for $this->incr().
	 *
	 * Certain plugins expect an "incr" method on the $gc_object_cache object (e.g., Batcache).
	 ...
	 *
	 * This function is inspired by the original GC Memcached Object cache.
	 *
	 * @author Ryan Boren
	 * @link http://www.gechiui.com/extend/plugins/memcached/
	 *
	 * @param string $key   The key under which to store the value.
	 * @param string $group The group value appended to the $key.
	 * @return string
	...
	public function buildKey( $key, $group = 'default' ) {
		...
		return preg_replace( '/\s+/', '', GC_CACHE_KEY_SALT . "$prefix$group:$key" );
	}
	 ...
	 * Adds global groups.
	 *
	 * This function comes straight from the original GC Memcached Object cache.
	 *
	 * @author Ryan Boren
	 * @link http://www.gechiui.com/extend/plugins/memcached/
	 *
