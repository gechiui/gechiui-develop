 ...
 */
function tests_reset__SERVER() { // phpcs:ignore GeChiUI.NamingConventions.ValidFunctionName.FunctionNameInvalid
	$_SERVER['HTTP_HOST']       = GC_TESTS_DOMAIN;
	$_SERVER['REMOTE_ADDR']     = '127.0.0.1';
	$_SERVER['REQUEST_METHOD']  = 'GET';
	$_SERVER['REQUEST_URI']     = '';
	$_SERVER['SERVER_NAME']     = GC_TESTS_DOMAIN;
...
function tests_add_filter( $tag, $function_to_add, $priority = 10, $accepted_args = 1 ) {
	...
	if ( function_exists( 'add_filter' ) ) {
		...
		$gc_filter[ $tag ][ $priority ][ $idx ] = array(
 ...
 *
 * @see _gc_filter_build_unique_id()
 *
...
function _delete_all_data() {
	...
	foreach ( array(
		...
		//phpcs:ignore GeChiUI.DB.PreparedSQL.InterpolatedNotPrepared
		$gcdb->query( "DELETE FROM {$table} WHERE term_id != 1" );
	}

	$gcdb->query( "UPDATE {$gcdb->term_taxonomy} SET count = 0" );

	$gcdb->query( "DELETE FROM {$gcdb->users} WHERE ID != 1" );
	$gcdb->query( "DELETE FROM {$gcdb->usermeta} WHERE user_id != 1" );
}
...
function _delete_all_posts() {
	...
	$all_posts = $gcdb->get_results( "SELECT ID, post_type from {$gcdb->posts}", ARRAY_A );
	if ( ! $all_posts ) {
		return;
	}
	...
	foreach ( $all_posts as $data ) {
		if ( 'attachment' === $data['post_type'] ) {
			gc_delete_attachment( $data['ID'], true );
		} else {
			gc_delete_post( $data['ID'], true );
		}
	}
}

/**
 * Handles the GC die handler by outputting the given values as text.
 *
 * @param string $message The message.
 * @param string $title   The title.
 * @param array  $args    Array with arguments.
 */
function _gc_die_handler( $message, $title = '', $args = array() ) {
	if ( ! $GLOBALS['_gc_die_disabled'] ) {
		_gc_die_handler_txt( $message, $title, $args );
	} else {
		// Ignore at our peril.
	}
}
...
/**
 * Disables the GC die handler.
 */
function _disable_gc_die() {
	$GLOBALS['_gc_die_disabled'] = true;
}

/**
 * Enables the GC die handler.
 */
function _enable_gc_die() {
 ...
 */
function _gc_die_handler_filter() {
	return '_gc_die_handler';
}
 ...
 * @return string The die handler.
 */
function _gc_die_handler_filter_exit() {
	return '_gc_die_handler_exit';
}
 ...
 * @param array  $args    Array with arguments.
 */
function _gc_die_handler_txt( $message, $title, $args ) {
	echo "\ngc_die called\n";
 ...
 * @param array  $args    Array with arguments.
 */
function _gc_die_handler_exit( $message, $title, $args ) {
	echo "\ngc_die called\n";
	echo "Message: $message\n";
 ...
 * `gc_install()`, so that GC doesn't attempt to do a time-consuming remote request.
 *
 * @since 4.2.0
 */
 ...
 */
function _gc_rest_server_class_filter() {
