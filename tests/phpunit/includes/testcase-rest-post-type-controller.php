...
abstract class GC_Test_REST_Post_Type_Controller_Testcase extends GC_Test_REST_Controller_Testcase {
	...
	protected function check_post_data( $post, $data, $context, $links ) {
		...
		$taxonomies = gc_list_filter( get_object_taxonomies( $post->post_type, 'objects' ), array( 'show_in_rest' => true ) );
		...
		if ( $links ) {
			...
			$this->assertSame( $links['self'][0]['href'], rest_url( 'gc/v2/' . $post_type->rest_base . '/' . $data['id'] ) );
			$this->assertSame( $links['collection'][0]['href'], rest_url( 'gc/v2/' . $post_type->rest_base ) );
			$this->assertSame( $links['about'][0]['href'], rest_url( 'gc/v2/types/' . $data['type'] ) );

			if ( post_type_supports( $post->post_type, 'author' ) && $data['author'] ) {
				$this->assertSame( $links['author'][0]['href'], rest_url( 'gc/v2/users/' . $data['author'] ) );
			}

			if ( post_type_supports( $post->post_type, 'comments' ) ) {
				$this->assertSame( $links['replies'][0]['href'], add_query_arg( 'post', $data['id'], rest_url( 'gc/v2/comments' ) ) );
			}

			if ( post_type_supports( $post->post_type, 'revisions' ) ) {
				$this->assertSame( $links['version-history'][0]['href'], rest_url( 'gc/v2/' . $post_type->rest_base . '/' . $data['id'] . '/revisions' ) );
			}

			if ( $post_type->hierarchical && ! empty( $data['parent'] ) ) {
				$this->assertSame( $links['up'][0]['href'], rest_url( 'gc/v2/' . $post_type->rest_base . '/' . $data['parent'] ) );
			}

			if ( ! in_array( $data['type'], array( 'attachment', 'nav_menu_item', 'revision' ), true ) ) {
				$this->assertSame( $links['https://api.w.org/attachment'][0]['href'], add_query_arg( 'parent', $data['id'], rest_url( 'gc/v2/media' ) ) );
			}

			if ( ! empty( $data['featured_media'] ) ) {
				$this->assertSame( $links['https://api.w.org/featuredmedia'][0]['href'], rest_url( 'gc/v2/media/' . $data['featured_media'] ) );
			}
