...
/**
 * Drops all tables from the GeChiUI database
 */
function drop_tables() {
	global $gcdb;
	$tables = $gcdb->get_col( 'SHOW TABLES;' );
	foreach ( $tables as $table ) {
		// phpcs:ignore GeChiUI.DB.PreparedSQL.InterpolatedNotPrepared
		$gcdb->query( "DROP TABLE IF EXISTS {$table}" );
	}
}
...
function mask_input_value( $in, $name = '_gcnonce' ) {
...
function _unregister_post_status( $status ) {
	unset( $GLOBALS['gc_post_statuses'][ $status ] );
}

function _cleanup_query_vars() {
	// Clean out globals to stop them polluting gc and gc_query.
	foreach ( $GLOBALS['gc']->public_query_vars as $v ) {
		unset( $GLOBALS[ $v ] );
	}

	foreach ( $GLOBALS['gc']->private_query_vars as $v ) {
		unset( $GLOBALS[ $v ] );
	}

	foreach ( get_taxonomies( array(), 'objects' ) as $t ) {
		if ( $t->publicly_queryable && ! empty( $t->query_var ) ) {
			$GLOBALS['gc']->add_query_var( $t->query_var );
		}
	}
...
/**
 * Special class for exposing protected gcdb methods we need to access
 */
class GcdbExposedMethodsForTesting extends gcdb {
	public function __construct() {
		global $gcdb;
		$this->dbh         = $gcdb->dbh;
		$this->use_mysqli  = $gcdb->use_mysqli;
		$this->is_mysql    = $gcdb->is_mysql;
		$this->ready       = true;
		$this->field_types = $gcdb->field_types;
		$this->charset     = $gcdb->charset;

		$this->dbuser     = $gcdb->dbuser;
		$this->dbpassword = $gcdb->dbpassword;
		$this->dbname     = $gcdb->dbname;
		$this->dbhost     = $gcdb->dbhost;
	}
...
function test_rest_expand_compact_links( $links ) {
	...
	foreach ( $links as $rel => $links_array ) {
		...
		$curie              = gc_list_filter( $links['curies'], array( 'name' => $name[0] ) );
