 ...
 * variable GC_Test_Stream::$data['bucket']['/foo'].
 *
 * Creating a directory at "protocol://bucket/foo" will store the string
 * 'DIRECTORY' to the static variable GC_Test_Stream::$data['bucket']['/foo/']
...
class GC_Test_Stream {
	...
	private function open( $url ) {
		...
		if ( ! isset( GC_Test_Stream::$data[ $this->bucket ] ) ) {
			GC_Test_Stream::$data[ $this->bucket ] = array();
		}

		$this->data_ref =& GC_Test_Stream::$data[ $this->bucket ][ $this->file ];
	...
	public function stream_write( $data ) {
		...
		GC_Test_Stream::$data[ $this->bucket ][ $this->file ] = $left . $data . $right;
	...
	public function mkdir( $path, $mode, $options ) {
		...
		if ( isset( GC_Test_Stream::$data[ $this->bucket ][ $file ] ) ) {
			return false;
		}
	...
	public function stream_stat() {
		...
		if ( substr( $this->file, -1 ) === '/' || isset( $dir_ref ) ) {
			return $this->make_stat(
				array(
					'mode' => GC_Test_Stream::DIRECTORY_MODE,
				)
			);
		}
	...
	public function unlink( $path ) {
		...
		unset( GC_Test_Stream::$data[ $this->bucket ][ $this->file ] );
		return true;
	}
	...
	private function &get_directory_ref() {
		return GC_Test_Stream::$data[ $this->bucket ][ rtrim( $this->file, '/' ) . '/' ];
	}
}
