 ...
 *
 * Resets the state of the GeChiUI installation before and after every test.
 *
 * Includes utility functions and assertions useful for testing GeChiUI.
 *
 * All GeChiUI unit tests should inherit from this class.
 */
abstract class GC_UnitTestCase_Base extends PHPUnit_Adapter_TestCase {
	...
	/**
	 * Fetches the factory object for generating GeChiUI fixtures.
	 *
	 * @return GC_UnitTest_Factory The fixture factory.
	 */
	protected static function factory() {
		static $factory = null;
		if ( ! $factory ) {
			$factory = new GC_UnitTest_Factory();
		}
		return $factory;
	}
	...
	public static function set_up_before_class() {
		...
		if ( method_exists( $class, 'gcSetUpBeforeClass' ) ) {
			call_user_func( array( $class, 'gcSetUpBeforeClass' ), self::factory() );
		}
	...
	public static function tear_down_after_class() {
		...
		if ( method_exists( $class, 'gcTearDownAfterClass' ) ) {
			call_user_func( array( $class, 'gcTearDownAfterClass' ) );
		}
	...
	public function set_up() {
		...
		if ( defined( 'GC_RUN_CORE_TESTS' ) && GC_RUN_CORE_TESTS ) {
			...
			if ( $gc_rewrite->permalink_structure ) {
		...
		add_filter( 'gc_die_handler', array( $this, 'get_gc_die_handler' ) );
	}

	/**
	 * After a test method runs, resets any state in GeChiUI the test method might have changed.
	 */
	public function tear_down() {
		global $gcdb, $gc_query, $gc;
		$gcdb->query( 'ROLLBACK' );
		if ( is_multisite() ) {
		 ...
		 *
		 * The globals are from `GC_Screen::set_current_screen()`.
		 *
		 * Why not invoke `set_current_screen( 'front' )`?
		 * Performance (faster test runs with less memory usage). How so? For each test,
		 * it saves creating an instance of GC_Screen, making two method calls,
		...
		/*
		 * Reset $gc_sitemap global so that sitemap-related dynamic $gc->public_query_vars
		 * are added when the next test runs.
		 */
		$GLOBALS['gc_sitemaps'] = null;

		$this->unregister_all_meta_keys();
		remove_theme_support( 'html5' );
		remove_filter( 'query', array( $this, '_create_temporary_tables' ) );
		remove_filter( 'query', array( $this, '_drop_temporary_tables' ) );
		remove_filter( 'gc_die_handler', array( $this, 'get_gc_die_handler' ) );
		$this->_restore_hooks();
	 ...
	 *
	 * Stores $gc_actions, $gc_current_filter, and $gc_filter on a class variable
	 * so they can be restored on tearDown() using _restore_hooks().
	 *
	 * @global array $gc_actions
	 * @global array $gc_current_filter
	 * @global array $gc_filter
	 */
	protected function _backup_hooks() {
		$globals = array( 'gc_actions', 'gc_current_filter' );
		foreach ( $globals as $key ) {
			self::$hooks_saved[ $key ] = $GLOBALS[ $key ];
		}
		self::$hooks_saved['gc_filter'] = array();
		foreach ( $GLOBALS['gc_filter'] as $hook_name => $hook_object ) {
			self::$hooks_saved['gc_filter'][ $hook_name ] = clone $hook_object;
		}
	}
	...
	protected function _restore_hooks() {
		...
		if ( isset( self::$hooks_saved['gc_filter'] ) ) {
			$GLOBALS['gc_filter'] = array();
			foreach ( self::$hooks_saved['gc_filter'] as $hook_name => $hook_object ) {
				$GLOBALS['gc_filter'][ $hook_name ] = clone $hook_object;
			}
		}
	}

	/**
	 * Flushes the GeChiUI object cache.
	 */
	public static function flush_cache() {
		global $gc_object_cache;
		$gc_object_cache->group_ops      = array();
		$gc_object_cache->stats          = array();
		$gc_object_cache->memcache_debug = array();
		$gc_object_cache->cache          = array();
		if ( method_exists( $gc_object_cache, '__remoteset' ) ) {
			$gc_object_cache->__remoteset();
		}
		gc_cache_flush();
		...
		gc_cache_add_non_persistent_groups( array( 'comment', 'counts', 'plugins' ) );
	}
	...
	public function unregister_all_meta_keys() {
		...
		foreach ( $gc_meta_keys as $object_type => $type_keys ) {
	...
	public function start_transaction() {
		global $gcdb;
		$gcdb->query( 'SET autocommit = 0;' );
		$gcdb->query( 'START TRANSACTION;' );
	...
	/**
	 * Retrieves the `gc_die()` handler.
	 *
	 * @param callable $handler The current die handler.
	 * @return callable The test die handler.
	 */
	public function get_gc_die_handler( $handler ) {
		return array( $this, 'gc_die_handler' );
	}
	 ...
	 *
	 * @throws GCDieException Exception containing the message and the response code.
	 *
	 * @param string|GC_Error $message The `gc_die()` message or GC_Error object.
	 * @param string          $title   The `gc_die()` title.
	 * @param string|array    $args    The `gc_die()` arguments.
	 */
	public function gc_die_handler( $message, $title, $args ) {
		if ( is_gc_error( $message ) ) {
		...
		throw new GCDieException( $message, $code );
	}
	...
	/**
	 * Asserts that the given value is an instance of GC_Error.
	 *
	 * @param mixed  $actual  The value to check.
	 * @param string $message Optional. Message to display when the assertion fails.
	 */
	public function assertWPError( $actual, $message = '' ) {
		$this->assertInstanceOf( 'GC_Error', $actual, $message );
	}

	/**
	 * Asserts that the given value is not an instance of GC_Error.
	 *
	 * @param mixed  $actual  The value to check.
	 * @param string $message Optional. Message to display when the assertion fails.
	 */
	public function assertNotWPError( $actual, $message = '' ) {
		if ( '' === $message && is_gc_error( $actual ) ) {
			$message = $actual->get_error_message();
		}
		$this->assertNotInstanceOf( 'GC_Error', $actual, $message );
	}
	...
	public function go_to( $url ) {
		/*
		 * Note: the GC and GC_Query classes like to silently fetch parameters
		...
		self::flush_cache();
		unset( $GLOBALS['gc_query'], $GLOBALS['gc_the_query'] );
		$GLOBALS['gc_the_query'] = new GC_Query();
		$GLOBALS['gc_query']     = $GLOBALS['gc_the_query'];

		$public_query_vars  = $GLOBALS['gc']->public_query_vars;
		$private_query_vars = $GLOBALS['gc']->private_query_vars;

		$GLOBALS['gc']                     = new GC();
		$GLOBALS['gc']->public_query_vars  = $public_query_vars;
		$GLOBALS['gc']->private_query_vars = $private_query_vars;

		_cleanup_query_vars();

		$GLOBALS['gc']->main( $parts['query'] );
	}
	...
	public function knownWPBug( $ticket_id ) {
		if ( GC_TESTS_FORCE_KNOWN_BUGS || in_array( $ticket_id, self::$forced_tickets, true ) ) {
			return;
		}
		if ( ! TracTickets::isTracTicketClosed( 'https://core.trac.gechiui.com', $ticket_id ) ) {
			$this->markTestSkipped( sprintf( 'GeChiUI Ticket #%d is not fixed', $ticket_id ) );
		}
	}
	...
	public function knownPluginBug( $ticket_id ) {
		if ( GC_TESTS_FORCE_KNOWN_BUGS || in_array( 'Plugin' . $ticket_id, self::$forced_tickets, true ) ) {
			return;
		}
		if ( ! TracTickets::isTracTicketClosed( 'https://plugins.trac.gechiui.com', $ticket_id ) ) {
			$this->markTestSkipped( sprintf( 'GeChiUI Plugin Ticket #%d is not fixed', $ticket_id ) );
		}
	}
	 ...
	 * includes included files. This does not work with GeChiUI, as the included files define the constants.
	 *
	...
	public function prepareTemplate( Text_Template $template ) {
		...
		$template->setVar( array( 'gc_constants' => PHPUnit_Util_GlobalState::getConstantsAsString() ) );
	...
	public function temp_filename() {
		...
		return tempnam( $tmp_dir, 'gcunit' );
	}

	/**
	 * Checks each of the GC_Query is_* functions/properties against expected boolean value.
	 *
	 ...
	 *
	 * @since 2.5.0
	 * @since 3.8.0 Moved from `Tests_Query_Conditionals` to `GC_UnitTestCase`.
	 * @since 5.3.0 Formalized the existing `...$prop` parameter by adding it
	 *              to the function signature.
	 *
	 * @param string ...$prop Any number of GC_Query properties that are expected to be true for the current request.
	 */
	public function assertQueryTrue( ...$prop ) {
		...
		foreach ( $all as $query_thing ) {
			$result = is_callable( $query_thing ) ? call_user_func( $query_thing ) : $gc_query->$query_thing;
	...
	public static function delete_user( $user_id ) {
		if ( is_multisite() ) {
			return gcmu_delete_user( $user_id );
		}

		return gc_delete_user( $user_id );
	}
	...
	public function set_permalink_structure( $structure = '' ) {
		...
		$gc_rewrite->init();
		$gc_rewrite->set_permalink_structure( $structure );
	 ...
	 * @since 4.4.0
	 *
	 * @param array $upload         Array of information about the uploaded file, provided by gc_upload_bits().
	...
	public function _make_attachment( $upload, $parent_post_id = 0 ) {
		...
		$id = gc_insert_attachment( $attachment, $upload['file'], $parent_post_id );
