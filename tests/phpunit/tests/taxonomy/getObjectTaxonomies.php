 ...
 * @group taxonomy
 */
class Tests_Taxonomy_GetObjectTaxonomies extends GC_UnitTestCase {
	...
	public function test_object_should_accept_string() {
		$found    = get_object_taxonomies( 'gctests_pt' );
	...
	public function test_object_should_accept_array_of_post_type_names() {
		$found    = get_object_taxonomies( array( 'gctests_pt' ) );
		$expected = array( 'gctests_tax' );
	...
	public function test_object_should_accept_post_object() {
		$p        = self::factory()->post->create_and_get( array( 'post_type' => 'gctests_pt' ) );
	...
	public function test_should_respect_output_names() {
		$found = get_object_taxonomies( 'gctests_pt', 'objects' );

		$this->assertSame( array( 'gctests_tax' ), array_keys( $found ) );
		$this->assertIsObject( $found['gctests_tax'] );
		$this->assertSame( 'gctests_tax', $found['gctests_tax']->name );
	}

	public function test_any_value_of_output_other_than_names_should_return_objects() {
		$found    = get_object_taxonomies( 'gctests_pt', 'foo' );
		$expected = get_object_taxonomies( 'gctests_pt', 'objects' );
	...
	public function test_should_return_all_attachment_taxonomies_for_attachment_object_type() {
		...
		$this->assertSame( array( 'gctests_tax2' ), $found );
	}
