 ...
 * @group query
 */
class Tests_Canonical_CustomRules extends GC_Canonical_UnitTestCase {

	public function set_up() {
		parent::set_up();
		global $gc_rewrite;
		// Add a custom Rewrite rule to test category redirections.
		$gc_rewrite->add_rule( 'ccr/(.+?)/sort/(asc|desc)', 'index.php?category_name=$matches[1]&order=$matches[2]', 'top' ); // ccr = Custom_Cat_Rule.
