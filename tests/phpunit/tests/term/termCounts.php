 ...
 * @group taxonomy
 */
class Tests_Term_termCount extends GC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$attachment_term = $factory->term->create( array( 'taxonomy' => 'gc_test_tax_counts' ) );
		self::$user_term       = $factory->term->create( array( 'taxonomy' => 'gc_test_user_tax_counts' ) );
	...
	public static function register_taxonomies() {
		register_taxonomy( 'gc_test_tax_counts', array( 'post', 'attachment' ) );
		register_taxonomy( 'gc_test_user_tax_counts', 'user' );
	}

	/**
	 * Term counts are incremented when post created.
	 *
	 * @covers ::gc_update_term_count
	...
	public function test_term_count_changes_for_update_post_term_count_statuses_filter( $post_status, $change ) {
		...
		$post_id = $this->factory()->post->create( array( 'post_status' => $post_status ) );
		gc_add_object_terms( $post_id, self::$attachment_term, 'gc_test_tax_counts' );
		...
		gc_add_object_terms( $attachment_id, self::$attachment_term, 'gc_test_tax_counts' );
	...
	public function test_term_counts_user_adding_term() {
		...
		gc_add_object_terms( self::$user_id, self::$user_term, 'gc_test_user_tax_counts' );
	...
	public function test_term_counts_user_removing_term() {
		...
		gc_remove_object_terms( self::$user_id, self::$user_term, 'gc_test_user_tax_counts' );
		$expected = $term_count - 1;
		$this->assertSame( $expected, get_term( self::$user_term )->count );
	}
}
