 ...
 * @group taxonomy
 */
class Tests_Term_GetTerm extends GC_UnitTestCase {
	...
	/**
	 * Utility function for generating two shared terms, in the 'gctests_tax' and 'gctests_tax_2' taxonomies.
	 *
	...
	public function test_passing_term_object_should_skip_database_query_when_filter_property_is_empty() {
		...
		$term = self::factory()->term->create_and_get( array( 'taxonomy' => 'gctests_tax' ) );
		clean_term_cache( $term->term_id, 'gctests_tax' );

		$num_queries = $gcdb->num_queries;

		unset( $term->filter );
		$term_a = get_term( $term, 'gctests_tax' );
	...
	public function test_passing_term_string_that_casts_to_int_0_should_return_null() {
		$this->assertNull( get_term( 'abc', 'gctests_tax' ) );
	}

	public function test_should_return_null_for_invalid_term_id() {
		$this->assertNull( get_term( 99999999, 'gctests_tax' ) );
	}
	...
	public function test_cache_should_be_populated_by_successful_fetch() {
		...
		clean_term_cache( $t, 'gctests_tax' );

		// Prime cache.
		$term_a      = get_term( $t, 'gctests_tax' );
		$num_queries = $gcdb->num_queries;

		// Second call shouldn't require a database query.
		$term_b = get_term( $t, 'gctests_tax' );
		$this->assertSame( $num_queries, $gcdb->num_queries );
	...
	public function test_output_object() {
		$t = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax' ) );
		$this->assertIsObject( get_term( $t, 'gctests_tax', OBJECT ) );
	}

	public function test_output_array_a() {
		$t    = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax' ) );
		$term = get_term( $t, 'gctests_tax', ARRAY_A );
	...
	public function test_output_array_n() {
		$t    = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax' ) );
		$term = get_term( $t, 'gctests_tax', ARRAY_N );
		$this->assertIsArray( $term );
	...
	public function test_output_should_fall_back_to_object_for_invalid_input() {
		$t = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax' ) );
		$this->assertIsObject( get_term( $t, 'gctests_tax', 'foo' ) );
	}
	...
	public function test_numeric_properties_should_be_cast_to_ints() {
		...
		$term_data = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->terms t JOIN $gcdb->term_taxonomy tt ON ( t.term_id = tt.term_id ) WHERE t.term_id = %d", $t ) );
		...
		foreach ( $contexts as $context ) {
			...
			$this->assertInstanceOf( 'GC_Term', $found );
	 ...
	 * @ticket 34533
	 */
	public function test_should_return_gc_error_when_term_is_shared_and_no_taxonomy_is_specified() {
	...
	public function test_should_return_term_when_term_is_shared_and_correct_taxonomy_is_specified() {
		...
		$found = get_term( $terms[0]['term_id'], 'gctests_tax' );
	...
	public function test_shared_term_in_cache_should_be_ignored_when_specifying_a_different_taxonomy() {
		...
		// Prime cache for 'gctests_tax'.
		get_term( $terms[0]['term_id'], 'gctests_tax' );
		$num_queries = $gcdb->num_queries;

		// Database should be hit again.
		$found = get_term( $terms[1]['term_id'], 'gctests_tax_2' );
		$num_queries++;

		$this->assertSame( $num_queries, $gcdb->num_queries );
		$this->assertInstanceOf( 'GC_Term', $found );
		$this->assertSame( 'gctests_tax_2', $found->taxonomy );
	}
