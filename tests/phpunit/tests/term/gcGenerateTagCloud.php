 ...
 * @group taxonomy
 */
class Tests_GC_Generate_Tag_Cloud extends GC_UnitTestCase {
	 ...
	 *
	 * @param $expected Expected output from `gc_generate_tag_cloud()`.
	 * @param $args     Options for `gc_generate_tag_cloud()`.
	 */
	public function test_empty_tags_passed( $expected, $args ) {
		$empty_tags = array();
		$this->assertSame( $expected, gc_generate_tag_cloud( $empty_tags, $args ) );
	}
	...
	public function test_empty_tags_list_returned( $expected, $args ) {
		...
		$this->assertSame( $expected, gc_generate_tag_cloud( $tags, $args ) );
	}
	...
	public function test_hide_empty_false_format_list() {
		...
		$this->assertMatchesRegularExpression( "|^<ul class='gc-tag-cloud' role='list'>|", $found );
	...
	public function test_topic_count_text() {
		...
		$term_ids    = self::factory()->term->create_many( 2, array( 'taxonomy' => 'gctests_tax' ) );
		$this->terms = array();
		foreach ( $term_ids as $term_id ) {
			$this->terms[] = get_term( $term_id, 'post_tag' );
		}
		...
		gc_set_post_terms( $posts[0], $term_ids, 'gctests_tax' );
		gc_set_post_terms( $posts[1], array( $term_ids[1] ), 'gctests_tax' );
	...
	public function test_should_include_tag_link_position_class() {
		...
		register_taxonomy( 'gctests_tax', 'post' );
		$term_ids = self::factory()->term->create_many( 3, array( 'taxonomy' => 'gctests_tax' ) );

		$p = self::factory()->post->create();
		gc_set_post_terms( $p, $term_ids, 'gctests_tax' );
		...
		$cloud = gc_generate_tag_cloud( $term_objects );
