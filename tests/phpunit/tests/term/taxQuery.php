 ...
 * @group taxonomy
 */
class Tests_Term_Tax_Query extends GC_UnitTestCase {
	protected $q;
	...
	public function test_transform_query_resulting_field_sanitized() {
		...
		gc_set_post_categories( $p, $t1 );

		$tq1 = new GC_Tax_Query(
			array(
				array(
					'terms' => array( 'foo' ),
					'field' => 'slug',
	...
	public function test_transform_query_field_slug() {
		$t1     = self::factory()->category->create( array( 'slug' => 'foo' ) );
		$p      = self::factory()->post->create();
		$tt_ids = gc_set_post_categories( $p, $t1 );
	...
	public function test_get_sql_operator_not_in_empty_terms() {
		...
		$this->assertSame( $expected, $tq->get_sql( $gcdb->posts, 'ID' ) );
