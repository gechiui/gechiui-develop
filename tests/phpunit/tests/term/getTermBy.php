 ...
 * @group taxonomy
 */
class Tests_Term_GetTermBy extends GC_UnitTestCase {

	public function test_get_term_by_slug() {
		$term1 = gc_insert_term( 'Foo', 'category', array( 'slug' => 'foo' ) );
	...
	public function test_get_term_by_unknown() {
		gc_insert_term( 'Foo', 'category', array( 'slug' => 'foo' ) );
	...
	public function test_get_term_by_slug_should_match_nonaccented_equivalents() {
		...
		$found = get_term_by( 'slug', 'nas', 'gctests_tax' );
	...
	public function test_should_prime_term_cache() {
		...
		$found       = get_term_by( 'slug', 'foo', 'gctests_tax' );
		$num_queries++;
	...
	public function test_should_unslash_name() {
		register_taxonomy( 'gctests_tax', 'post' );
		$term_name         = 'Foo " \o/';
		$term_name_slashed = gc_slash( $term_name );
		$t                 = self::factory()->term->create(
			array(
				'taxonomy' => 'gctests_tax',
		...
		$found = get_term_by( 'name', $term_name_slashed, 'gctests_tax' );
	...
	public function test_should_sanitize_slug() {
		...
		$found1 = get_term_by( 'slug', 'foo foo', 'gctests_tax' );

		$this->assertInstanceOf( 'GC_Term', $found1 );
		...
		$found2 = get_term_by( 'slug', '仪表盘', 'gctests_tax' );

		$this->assertInstanceOf( 'GC_Term', $found2 );
	...
	public function test_query_should_not_contain_order_by_clause() {
		...
		$this->assertStringNotContainsString( 'ORDER BY', $gcdb->last_query );
	}
	...
	public function test_query_should_contain_limit_clause() {
		...
		$found   = get_term_by( 'name', 'burrito', 'post_tag' );
		$this->assertSame( $term_id, $found->term_id );
		$this->assertStringContainsString( 'LIMIT 1', $gcdb->last_query );
	}
	...
	public function test_get_term_by_name_with_string_0() {
		...
		$found = get_term_by( 'name', '0', 'gctests_tax' );
		$this->assertSame( $term_id, $found->term_id );
	}
	...
	public function test_get_term_by_slug_with_string_0() {
		...
		$found = get_term_by( 'slug', '0', 'gctests_tax' );
		$this->assertSame( $term_id, $found->term_id );
	}
	...
	public function test_get_term_by_with_empty_string() {
		...
		$found_by_slug = get_term_by( 'slug', '', 'gctests_tax' );
		$found_by_name = get_term_by( 'name', '', 'gctests_tax' );
