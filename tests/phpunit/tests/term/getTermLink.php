 ...
 */
class Tests_Term_GetTermLink extends GC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		$taxonomies = array( 'category', 'post_tag', 'gctests_tax' );
	...
	public function test_integer_should_be_interpreted_as_term_id() {
		...
		$actual = get_term_link( $term, 'gctests_tax' );
		$this->assertStringContainsString( 'gctests_tax=foo', $actual );
	}
	...
	public function test_numeric_string_should_be_interpreted_as_term_slug() {
		...
		$actual = get_term_link( $term, 'gctests_tax' );
		$this->assertStringContainsString( 'gctests_tax=' . $term, $actual );
	}

	public function test_invalid_term_should_return_gc_error() {
		$actual = get_term_link( 'foo', 'gctests_tax' );
	...
	public function test_taxonomy_with_query_var_should_use_that_query_var_with_term_slug() {
		...
		$actual = get_term_link( $t, 'gctests_tax2' );
	...
	public function test_taxonomy_without_query_var_should_use_taxonomy_query_var_and_term_query_var_with_term_slug() {
		...
		$actual = get_term_link( $t, 'gctests_tax2' );
		$this->assertStringContainsString( 'taxonomy=gctests_tax2', $actual );
	...
	public function test_taxonomy_with_rewrite_false_and_custom_permalink_structure() {
		...
		add_permastruct( 'gctests_tax2', 'foo/%gctests_tax2%' );
		...
		remove_permastruct( 'gctests_tax2' );
	...
	public function test_taxonomy_permastruct_with_hierarchical_rewrite_should_put_term_ancestors_in_link() {
		...
		$actual = get_term_link( $t2, 'gctests_tax2' );
	 ...
	 * @param string $taxonomy Taxonomy being tested (used for index of term keys).
	 * @param bool   $use_id   When true, pass term ID. Else, pass term object.
	 * @return GC_Term|int If $use_id is true, term ID is returned; else instance of GC_Term.
	 */
