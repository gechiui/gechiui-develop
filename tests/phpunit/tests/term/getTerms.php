 ...
 * @group taxonomy
 */
class Tests_Term_getTerms extends GC_UnitTestCase {
	...
	public function test_meta_query_args_only() {
		...
		$term1 = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax' ) );
		$term2 = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax' ) );

		$post = self::factory()->post->create( array( 'post_type' => 'post' ) );

		update_term_meta( $term1, 'somekey', 'thevalue' );

		gc_set_post_terms( $post, array( $term1, $term2 ), 'gctests_tax' );
		...
		$this->assertSameSets( array( $term1 ), gc_list_pluck( $found, 'term_id' ) );
	}
	...
	public function test_should_accept_an_args_array_containing_taxonomy_for_first_parameter() {
		...
		$term = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax' ) );
	...
	public function test_legacy_params_as_query_string_should_be_properly_parsed() {
		...
		$found = get_terms( 'gctests_tax', 'hide_empty=0&fields=ids&update_term_meta_cache=0' );
	...
	public function test_new_params_as_query_string_should_be_properly_parsed() {
		...
		$found = get_terms( 'taxonomy=gctests_tax&hide_empty=0&fields=ids&update_term_meta_cache=0' );
	...
	public function test_get_terms_cache() {
		...
		$time1 = gc_cache_get( 'last_changed', 'terms' );
		...
		$terms = get_terms( 'post_tag', array( 'update_term_meta_cache' => false ) );
		$this->assertCount( 3, $terms );
		$this->assertSame( $time1, gc_cache_get( 'last_changed', 'terms' ) );
	 ...
	 * @ticket 23326
	 */
	public function test_gc_delete_term_should_invalidate_cache() {
		global $gcdb;
		...
		gc_delete_term( $terms[0]->term_id, 'post_tag' );

		$num_queries = $gcdb->num_queries;
		$time2       = gc_cache_get( 'last_changed', 'terms' );
		$this->assertNotEquals( $time1, $time2 );

		// last_changed and num_queries should bump after a term is deleted.
		$terms = get_terms( 'post_tag' );
		$this->assertCount( 2, $terms );
		$this->assertSame( $time2, gc_cache_get( 'last_changed', 'terms' ) );
		$this->assertSame( $num_queries + 1, $gcdb->num_queries );
	...
	public function test_get_terms_include_exclude() {
		...
		$this->assertSame( array( $term_id1, $term_id2 ), gc_list_pluck( $inc_terms, 'term_id' ) );
		...
		$this->assertSame( array(), gc_list_pluck( $exc_terms, 'term_id' ) );
	...
	public function test_exclude_with_hierarchical_true_for_non_hierarchical_taxonomy() {
		...
		$this->assertSame( array( $terms[1] ), gc_list_pluck( $found, 'term_id' ) );
	...
	public function test_get_terms_parent_zero() {
		...
		foreach ( $post_ids as $id ) {
			gc_set_post_terms( $id, $cheddar, $tax );
		}
		...
		gc_set_post_terms( $post_id, $brie, $tax );
		...
		foreach ( $post_ids as $id ) {
			gc_set_post_terms( $id, $butter, $tax );
		}
		...
		$post_ids   = self::factory()->post->create_many( 1 );
		foreach ( $post_ids as $id ) {
			gc_set_post_terms( $id, $multigrain, $tax );
		}
		...
		$this->assertCount( 2, $terms );
		$this->assertSame( gc_list_pluck( $terms, 'name' ), array( 'Cheese', 'Crackers' ) );
	}
	...
	public function test_get_terms_grandparent_zero() {
		...
		$post_id = self::factory()->post->create();
		gc_set_post_terms( $post_id, $spread, $tax );
		...
		$this->assertSame( array( 'Cheese' ), gc_list_pluck( $terms, 'name' ) );
	...
	public function test_get_terms_seven_levels_deep() {
		...
		$post_id = self::factory()->post->create();
		gc_set_post_terms( $post_id, $t[7], $tax );
		...
		$this->assertCount( 1, $terms );
		$this->assertSame( array( 'term1' ), gc_list_pluck( $terms, 'name' ) );
	...
	public function test_child_of_should_respect_multiple_taxonomies() {
		register_taxonomy( 'gctests_tax1', 'post', array( 'hierarchical' => true ) );
		register_taxonomy( 'gctests_tax2', 'post', array( 'hierarchical' => true ) );
		...
		$found = get_terms(
			array( 'gctests_tax1', 'gctests_tax2' ),
			array(
				'fields'     => 'ids',
				'hide_empty' => false,
				'child_of'   => $t2,
	...
	public function test_get_term_children_recursion() {
		...
		// See: https://core.trac.gechiui.com/changeset/15806
		remove_filter( 'gc_update_term_parent', 'gc_check_term_hierarchy_for_loops', 10 );

		$term = gc_insert_term( 'Test', 'category' );
		$term = gc_update_term( $term['term_id'], 'category', array( 'parent' => $term['term_id'] ) );
		...
		add_filter( 'gc_update_term_parent', 'gc_check_term_hierarchy_for_loops', 10, 3 );
	}
	...
	public function test__get_term_children_handles_cycles() {
		...
		gc_update_term( $c1, 'category', array( 'parent' => $c3 ) );
	...
	public function test__get_term_children_handles_cycles_when_terms_argument_contains_objects() {
		...
		gc_update_term( $c1->term_id, 'category', array( 'parent' => $c3->term_id ) );
	...
	public function test_hierarchical_should_recurse_properly_for_all_taxonomies() {
		...
		$t4 = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax2' ) );
		...
		gc_set_object_terms( $p, $t3, 'gctests_tax1' );
		gc_set_object_terms( $p, $t6, 'gctests_tax2' );
	...
	public function test_hierarchical_true_with_parent() {
		...
		$actual   = gc_list_pluck( $terms, 'term_id' );
	...
	public function test_pad_counts() {
		register_taxonomy( 'gctests_tax_1', 'post', array( 'hierarchical' => true ) );
		...
		gc_set_object_terms( $posts[0], array( $t1 ), 'gctests_tax_1' );
		gc_set_object_terms( $posts[1], array( $t2 ), 'gctests_tax_1' );
		gc_set_object_terms( $posts[2], array( $t3 ), 'gctests_tax_1' );
		...
		$this->assertSameSets( array( $t1, $t2, $t3 ), gc_list_pluck( $found, 'term_id' ) );
	...
	public function test_pad_counts_should_not_recurse_infinitely_when_term_hierarchy_has_a_loop() {
		...
		$posts = self::factory()->post->create_many( 3 );
		gc_set_post_terms( $posts[0], $c1, 'category' );
		gc_set_post_terms( $posts[1], $c2, 'category' );
		gc_set_post_terms( $posts[2], $c3, 'category' );
		...
		$this->assertSameSets( array( $c1, $c2, $c3 ), gc_list_pluck( $terms, 'term_id' ) );
	...
	public function test_pad_counts_should_work_when_first_taxonomy_is_nonhierarchical_and_second_taxonomy_is_hierarchical() {
		register_taxonomy( 'gctests_tax1', 'post', array( 'hierarchical' => false ) );
		register_taxonomy( 'gctests_tax2', 'post', array( 'hierarchical' => true ) );
		...
		$posts = self::factory()->post->create_many( 3 );
		gc_set_object_terms( $posts[0], $t1, 'gctests_tax1' );
		gc_set_object_terms( $posts[1], $t2, 'gctests_tax2' );
		gc_set_object_terms( $posts[2], $t3, 'gctests_tax2' );
	...
	public function test_meta_query_should_not_return_duplicates() {
		register_taxonomy( 'gctests_tax', 'post' );
		$terms = self::factory()->term->create_many( 1, array( 'taxonomy' => 'gctests_tax' ) );
		add_term_meta( $terms[0], 'foo', 'bar' );
	 ...
	 * @ticket 14162
	 */
	public function test_should_return_gc_term_objects() {
		register_taxonomy( 'gctests_tax', 'post' );
	 ...
	 * @ticket 34282
	 */
	public function test_should_return_gc_term_objects_when_pulling_from_the_cache() {
		global $gcdb;
	...
	public function test_with_term_slug_equal_to_string_0() {
		...
		$this->assertSameSets( array( $term_id ), gc_list_pluck( $found, 'term_id' ) );
	}
	...
	public function test_with_multiple_term_slugs_one_equal_to_string_0() {
		...
		$this->assertSameSets( array( $term_id1, $term_id2 ), gc_list_pluck( $found, 'term_id' ) );
	}
	...
	protected function create_hierarchical_terms_and_posts() {
		...
		$post_id = self::factory()->post->create();
		gc_set_post_terms( $post_id, $terms['parent2'], 'hierarchical_fields', true );
		gc_set_post_terms( $post_id, $terms['child1'], 'hierarchical_fields', true );
	...
	protected function create_hierarchical_terms() {
		 ...
		 *   - Seven
		 */
		$one_term   = gc_insert_term(
			'One',
			'category'
		);
		$two_term   = gc_insert_term(
			'Two',
			'category',
			array(
		...
		gc_set_post_terms( $first_post_id, array( $three_term['term_id'] ), 'category' );
		gc_set_post_terms( $second_post_id, array( $six_term['term_id'] ), 'category' );
	...
	protected function set_up_three_posts_and_tags() {
		...
		foreach ( $posts as $i => $post ) {
			gc_set_object_terms( $post, "term_{$i}", 'post_tag' );
		}
