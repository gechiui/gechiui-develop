 ...
 * @ticket 10142
 */
class Tests_Term_Meta extends GC_UnitTestCase {
	...
	public function test_term_meta_should_be_lazy_loaded_for_all_terms_in_gc_query_loop() {
		global $gcdb;

		// Clear any previous term IDs from the queue.
		gc_metadata_lazyloader()->reset_queue( 'term' );
		...
		$orphan_term = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax' ) );
	...
	public function test_term_meta_should_be_deleted_when_term_is_deleted() {
		...
		$this->assertTrue( gc_delete_term( $t, 'gctests_tax' ) );
	...
	public function data_register_term_meta() {
		return array(
			array( 'gctests_tax', 'registered_key1', array( 'single' => true ) ),
	...
	public function data_unregister_term_meta() {
		return array(
			array( 'gctests_tax', 'registered_key1' ),
	...
	public function test_add_metadata_sets_terms_last_changed() {
		...
		$this->assertNotFalse( gc_cache_get_last_changed( 'terms' ) );
	}
