 ...
 * @group taxonomy
 */
class Tests_Term_GcSetObjectTerms extends GC_UnitTestCase {
	...
	public function test_set_object_terms_by_id() {
		...
		for ( $i = 0; $i < 3; $i++ ) {
			$term   = "term_{$i}";
			$result = gc_insert_term( $term, $this->taxonomy );
			$this->assertIsArray( $result );
			$term_id[ $term ] = $result['term_id'];
		}

		foreach ( $ids as $id ) {
			$tt = gc_set_object_terms( $id, array_values( $term_id ), $this->taxonomy );
	...
	public function test_set_object_terms_by_name() {
		...
		foreach ( $ids as $id ) {
			$tt = gc_set_object_terms( $id, $terms, $this->taxonomy );
			// Should return three term taxonomy IDs.
			$this->assertCount( 3, $tt );
	...
	public function test_set_object_terms_invalid() {
		// Bogus taxonomy.
		$result = gc_set_object_terms( self::$post_ids[0], array( 'foo' ), 'invalid-taxonomy' );
		$this->assertWPError( $result );
	}

	public function test_gc_set_object_terms_append_true() {
		...
		$added1 = gc_set_object_terms( $p, array( $t1 ), 'gctests_tax' );
		$this->assertNotEmpty( $added1 );
		$this->assertSameSets( array( $t1 ), gc_get_object_terms( $p, 'gctests_tax', array( 'fields' => 'ids' ) ) );

		$added2 = gc_set_object_terms( $p, array( $t2 ), 'gctests_tax', true );
		$this->assertNotEmpty( $added2 );
		$this->assertSameSets( array( $t1, $t2 ), gc_get_object_terms( $p, 'gctests_tax', array( 'fields' => 'ids' ) ) );

		_unregister_taxonomy( 'gctests_tax' );
	}

	public function test_gc_set_object_terms_append_false() {
		register_taxonomy( 'gctests_tax', 'post' );
		$p  = self::$post_ids[0];
		$t1 = self::factory()->term->create(
			array(
		...
		$added2 = gc_set_object_terms( $p, array( $t2 ), 'gctests_tax', false );
		$this->assertNotEmpty( $added2 );
		$this->assertSameSets( array( $t2 ), gc_get_object_terms( $p, 'gctests_tax', array( 'fields' => 'ids' ) ) );

		_unregister_taxonomy( 'gctests_tax' );
	}

	public function test_gc_set_object_terms_append_default_to_false() {
		register_taxonomy( 'gctests_tax', 'post' );
		$p  = self::$post_ids[0];
		$t1 = self::factory()->term->create(
			array(
		...
		$added2 = gc_set_object_terms( $p, array( $t2 ), 'gctests_tax' );
		$this->assertNotEmpty( $added2 );
		$this->assertSameSets( array( $t2 ), gc_get_object_terms( $p, 'gctests_tax', array( 'fields' => 'ids' ) ) );
	...
	public function test_should_create_term_that_does_not_exist() {
		...
		$this->assertFalse( get_term_by( 'slug', 'foo', 'gctests_tax' ) );

		$tt_ids = gc_set_object_terms( self::$post_ids[0], 'foo', 'gctests_tax' );
	...
	public function test_should_find_existing_term_by_name_match() {
		...
		$tt_ids = gc_set_object_terms( self::$post_ids[0], 'Bar', 'gctests_tax' );
	...
	public function test_non_existent_integers_should_be_ignored() {
		...
		$tt_ids = gc_set_object_terms( self::$post_ids[0], 12345, 'gctests_tax' );
