 ...
 * @group taxonomy
 */
class Tests_Term_SplitSharedTerm extends GC_UnitTestCase {
	protected $terms = array();

	/**
	 * Sets up a number of split terms for testing. Terms are as follows.
	 *
	 * - `$this->terms['t1']` is an array of the 'term_id' and 'term_taxonomy_id' of a term in the 'gctests_tax'
	 *   taxonomy. Pre-split, the term_id of t1 (`$this->terms['t1']['term_id']`) was shared by t1, t2, and t3.
	 * - `$this->terms['t2']` is an array of the 'term_id' and 'term_taxonomy_id' of a term in the 'gctests_tax_2'
	 *   taxonomy. Pre-split, the term_id of t2 was `$this->terms['t1']['term_id']`.
	 * - `$this->terms['t3']` is an array of the 'term_id' and 'term_taxonomy_id' of a term in the 'gctests_tax_3'
	 *   taxonomy. Pre-split, the term_id of t2 was `$this->terms['t1']['term_id']`.
	 * - `$this->terms['t2_child']` is an array of the 'term_id' and 'term_taxonomy_id' of a term in the
	 *   'gctests_tax_2' taxonomy. This term is a child of t2, and is used to test parent/child relationships
	...
	public function set_up() {
		...
		$t3 = gc_insert_term( 'Foo', 'gctests_tax_3' );
	...
	public function test_should_create_new_term_ids() {
		$t1_term = get_term_by( 'term_taxonomy_id', $this->terms['t1']['term_taxonomy_id'], 'gctests_tax' );
		$t2_term = get_term_by( 'term_taxonomy_id', $this->terms['t2']['term_taxonomy_id'], 'gctests_tax_2' );
		$t3_term = get_term_by( 'term_taxonomy_id', $this->terms['t3']['term_taxonomy_id'], 'gctests_tax_3' );
	...
	public function test_should_rebuild_split_term_taxonomy_hierarchy() {
		...
		$t1 = gc_insert_term( 'Foo1', 'gctests_tax_3' );
		$t2 = gc_insert_term( 'Foo1 Parent', 'gctests_tax_4' );
		$t3 = gc_insert_term(
			'Foo1',
			'gctests_tax_4',
			array(
		...
		$th = _get_term_hierarchy( 'gctests_tax_4' );

		$new_term_id = _split_shared_term( $t1['term_id'], $t3['term_taxonomy_id'] );

		$t2_children = get_term_children( $t2['term_id'], 'gctests_tax_4' );
	...
	public function test_should_update_default_category_on_term_split() {
		global $gcdb;
		$t1 = gc_insert_term( 'Foo Default', 'category' );

		update_option( 'default_category', $t1['term_id'] );

		register_taxonomy( 'gctests_tax_5', 'post' );
		$t2 = gc_insert_term( 'Foo Default', 'gctests_tax_5' );
	...
	public function test_should_update_menus_on_term_split() {
		...
		$t1 = gc_insert_term( 'Foo Menu', 'category' );

		register_taxonomy( 'gctests_tax_6', 'post' );
		$t2 = gc_insert_term( 'Foo Menu', 'gctests_tax_6' );
		...
		$menu_id       = gc_create_nav_menu( 'Nav Menu Bar' );
		$cat_menu_item = gc_update_nav_menu_item(
			$menu_id,
			0,
			array(
	...
	public function test_nav_menu_locations_should_be_updated_on_split() {
		...
		$cat_term       = gc_insert_term( 'Foo Menu', 'category' );
		$shared_term_id = $cat_term['term_id'];

		$nav_term_id = gc_create_nav_menu( 'Foo Menu' );
	...
	public function test_nav_menu_term_should_retain_menu_items_on_split() {
		...
		$t1            = gc_insert_term( 'Random term', 'category' );
		$cat_menu_item = gc_update_nav_menu_item(
			$shared_term_id,
			0,
			array(
		...
		$new_nav_menu_id = gc_update_nav_menu_object(
			$shared_term_id,
			array(
		...
		$menu = gc_get_nav_menu_object( $new_nav_menu_id );
		$this->assertSame( 'Updated Foo Menu', $menu->name );
		$this->assertSame( 'Updated Foo Menu', $menu->description );

		$menu_items = gc_get_nav_menu_items( $new_nav_menu_id );
		$this->assertSame( array( $cat_menu_item ), gc_list_pluck( $menu_items, 'ID' ) );
	}

	public function test_gc_get_split_terms() {
		$found = gc_get_split_terms( $this->terms['t1']['term_id'] );

		$expected = array(
			'gctests_tax_2' => $this->terms['t2']['term_id'],
			'gctests_tax_3' => $this->terms['t3']['term_id'],
		);

		$this->assertSameSets( $expected, $found );
	}

	public function test_gc_get_split_term() {
		$found = gc_get_split_term( $this->terms['t1']['term_id'], 'gctests_tax_3' );
