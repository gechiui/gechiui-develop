 ...
 * @ticket 21767
 */
class Tests_Term_Slashes extends GC_Ajax_UnitTestCase {
	 ...
	 */
	public function test_gc_insert_term() {
		$taxonomies = array(
			'category',
			'post_tag',
		);
		foreach ( $taxonomies as $taxonomy ) {
			$insert = gc_insert_term(
				$this->slash_1,
				$taxonomy,
				array(
			...
			$this->assertSame( gc_unslash( $this->slash_1 ), $term->name );
			$this->assertSame( gc_unslash( $this->slash_3 ), $term->description );
			...
			$term   = get_term( $insert['term_id'], $taxonomy );
			$this->assertSame( gc_unslash( $this->slash_3 ), $term->name );
			$this->assertSame( gc_unslash( $this->slash_5 ), $term->description );
			...
			$term   = get_term( $insert['term_id'], $taxonomy );
			$this->assertSame( gc_unslash( $this->slash_2 ), $term->name );
			$this->assertSame( gc_unslash( $this->slash_4 ), $term->description );
		}
	}
	 ...
	 * Tests the model function that expects slashed data.
	 */
	public function test_gc_update_term() {
		$taxonomies = array(
			'category',
			'post_tag',
		);
