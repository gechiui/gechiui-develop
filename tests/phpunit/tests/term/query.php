 ...
 * @group taxonomy
 */
class Tests_Term_Query extends GC_UnitTestCase {
	/**
	 * @ticket 37545
	 */
	public function test_taxonomy_should_accept_single_taxonomy_as_string() {
		register_taxonomy( 'gctests_tax_1', 'post' );
		register_taxonomy( 'gctests_tax_2', 'post' );

		$term_1 = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax_1' ) );
		$term_2 = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax_2' ) );
	...
	public function test_terms_is_set() {
		...
		self::factory()->term->create( array( 'taxonomy' => 'gctests_tax_1' ) );

		$q1 = new GC_Term_Query(
			array(
	...
	public function test_orderby_include_with_comma_separated_list() {
		...
		$t1 = self::factory()->term->create_and_get( array( 'taxonomy' => 'gctests_tax_1' ) );
		$t2 = self::factory()->term->create_and_get( array( 'taxonomy' => 'gctests_tax_1' ) );
	...
	public function test_object_ids_single() {
		...
		$t = self::factory()->term->create( array( 'taxonomy' => 'gctests_tax_1' ) );

		gc_set_object_terms( $p, array( $t ), 'gctests_tax_1' );
	...
	public function test_object_ids_cache_should_be_invalidated_by_term_relationship_change() {
		...
		$p     = self::factory()->post->create();
		$terms = self::factory()->term->create_many( 2, array( 'taxonomy' => 'gctests_tax_1' ) );

		gc_set_object_terms( $p, array( $terms[0] ), 'gctests_tax_1' );
		...
		gc_set_object_terms( $p, array( $terms[1] ), 'gctests_tax_1' );
	...
	public function test_count_query_cache_should_be_invalidated_with_incrementor_bump() {
		...
		gc_delete_term( $terms[0], 'gctests_tax_1' );
	...
	public function test_get_the_terms_should_respect_taxonomy_orderby() {
		...
		gc_set_object_terms( $post_id, array( $term_ids[0], $term_ids[1] ), 'gctests_tax' );
		$terms = get_the_terms( $post_id, 'gctests_tax' );
		$this->assertSame( array( $term_ids[0], $term_ids[1] ), gc_list_pluck( $terms, 'term_id' ) );
		// Flip the order.
		gc_set_object_terms( $post_id, array( $term_ids[1], $term_ids[0] ), 'gctests_tax' );
		$terms = get_the_terms( $post_id, 'gctests_tax' );
		$this->assertSame( array( $term_ids[1], $term_ids[0] ), gc_list_pluck( $terms, 'term_id' ) );
	}

	/**
	 * @ticket 40496
	 */
	public function test_gc_get_object_terms_should_respect_taxonomy_orderby() {
		register_taxonomy(
			'gctests_tax',
			'post',
			array(
		...
		$post_id  = self::factory()->post->create();
		gc_set_object_terms( $post_id, array( $term_ids[0], $term_ids[1] ), 'gctests_tax' );
		$terms = gc_get_object_terms( $post_id, array( 'category', 'gctests_tax' ) );
		$this->assertSame( array( $term_ids[0], $term_ids[1], 1 ), gc_list_pluck( $terms, 'term_id' ) );
		// Flip the order.
		gc_set_object_terms( $post_id, array( $term_ids[1], $term_ids[0] ), 'gctests_tax' );
		$terms = gc_get_object_terms( $post_id, array( 'category', 'gctests_tax' ) );
		$this->assertSame( array( $term_ids[1], $term_ids[0], 1 ), gc_list_pluck( $terms, 'term_id' ) );
	}
	...
	public function test_should_allow_same_args_with_the_get_terms() {
		...
		register_taxonomy( 'gctests_tax', 'gctests_pt' );
		...
		gc_set_object_terms( $p, array( $t1, $t2 ), 'gctests_tax' );

		$expected = gc_get_post_terms(
			$p,
			'gctests_tax',
			array(
				'fields' => 'ids',
	...
	public function test_query_should_return_zero_for_field_count_and_parent_set() {
		...
		gc_set_object_terms( $post_id, array( $term_id ), 'gctests_tax' );
	...
	public function test_null_term_object_should_be_discarded() {
		...
		$this->assertSameSets( $expected, gc_list_pluck( $found, 'term_id' ) );
	}
	...
	public function test_error_term_object_should_be_discarded() {
		...
		add_filter( 'get_term', array( $this, 'filter_term_to_gc_error' ) );
		$found = get_terms(
			array(
				'taxonomy'   => 'gctests_tax',
				'hide_empty' => false,
		...
		remove_filter( 'get_term', array( $this, 'filter_term_to_gc_error' ) );
	...
	public function filter_term_to_gc_error( $term ) {
		if ( $this->term_id === $term->term_id ) {
			return new GC_Error( 'foo' );
		}
	...
	public function test_hide_empty_should_include_empty_parents_of_nonempty_children() {
		...
		gc_set_object_terms( $p, $t2, 'gctests_tax' );
