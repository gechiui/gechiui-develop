 ...
 * @group security-153
 */
class Tests_DB_Charset extends GC_UnitTestCase {
	...
	public static function set_up_before_class() {
		...
		self::$server_info = self::$_gcdb->db_server_info();
	}
	...
	public function test_strip_invalid_text( $data, $expected, $message ) {
		...
		if ( 'utf8mb4' === $new_charset && ! self::$_gcdb->has_cap( 'utf8mb4' ) ) {
		...
		if ( 'big5' === $new_charset && 'byte' === $data[0]['length']['type'] && false !== strpos( self::$server_info, 'MariaDB' ) ) {
			$this->markTestSkipped( "MariaDB doesn't support this data set. See https://core.trac.gechiui.com/ticket/33171." );
		}

		self::$_gcdb->charset = $new_charset;
		self::$_gcdb->set_charset( self::$_gcdb->dbh, $new_charset );

		$actual = self::$_gcdb->strip_invalid_text( $data );

		self::$_gcdb->charset = $charset;
		self::$_gcdb->set_charset( self::$_gcdb->dbh, $charset );
	...
	public function test_process_fields_failure() {
		...
		$charset = $gcdb->get_col_charset( $gcdb->posts, 'post_content' );
	...
	public function data_process_field_charsets() {
		if ( $GLOBALS['gcdb']->charset ) {
			$charset = $GLOBALS['gcdb']->charset;
		} else {
			$charset = $GLOBALS['gcdb']->get_col_charset( $GLOBALS['gcdb']->posts, 'post_content' );
		}
	 ...
	 * @ticket 21212
	 *
	 * @covers gcdb::process_field_charsets
	 */
	public function test_process_field_charsets( $data, $expected, $message ) {
		$actual = self::$_gcdb->process_field_charsets( $data, $GLOBALS['gcdb']->posts );
		$this->assertSame( $expected, $actual, $message );
	}
	...
	public function test_process_field_charsets_on_nonexistent_table() {
		...
		$this->assertFalse( self::$_gcdb->process_field_charsets( $data, 'nonexistent_table' ) );
	 ...
	 * @ticket 21212
	 *
	 * @covers gcdb::check_ascii
	 */
	public function test_check_ascii() {
		$ascii = "\0\t\n\r '" . '!"#$%&()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}~';
		$this->assertTrue( self::$_gcdb->check_ascii( $ascii ) );
	}
	...
	public function test_check_ascii_false() {
		$this->assertFalse( self::$_gcdb->check_ascii( 'ABCDEFGHIJKLMNOPQRSTUVWXYZ¡©«' ) );
	}

	/**
	 * @ticket 21212
	 *
	 * @covers gcdb::strip_invalid_text_for_column
	 */
	public function test_strip_invalid_text_for_column() {
		...
		$expected = 'H€lloWorld¢';
		$actual   = $gcdb->strip_invalid_text_for_column( $gcdb->posts, 'post_content', $value );
		$this->assertSame( $expected, $actual );
	}

	/**
	 * Set of table definitions for testing gcdb::get_table_charset and gcdb::get_column_charset
	 *
	 * @var array
	 */
	 ...
	 * @ticket 21212
	 *
	 * @covers gcdb::get_table_charset
	 */
	public function test_get_table_charset( $drop, $create, $table, $expected_charset ) {
		self::$_gcdb->query( $drop );

		if ( ! self::$_gcdb->has_cap( 'utf8mb4' ) && preg_match( '/utf8mb[34]/i', $create ) ) {
			$this->markTestSkipped( "This version of MySQL doesn't support utf8mb4." );
		}

		self::$_gcdb->query( $create );

		$charset = self::$_gcdb->get_table_charset( $table );
		$this->assertSame( $expected_charset, $charset );

		$charset = self::$_gcdb->get_table_charset( strtoupper( $table ) );
		$this->assertSame( $expected_charset, $charset );
	...
	public function test_get_column_charset( $drop, $create, $table, $expected_charset ) {
		...
		foreach ( $expected_charset as $column => $charset ) {
			$this->assertSame( $charset, self::$_gcdb->get_col_charset( $table, $column ) );
			$this->assertSame( $charset, self::$_gcdb->get_col_charset( strtoupper( $table ), strtoupper( $column ) ) );
		}
	...
	public function test_get_column_charset_non_mysql( $drop, $create, $table, $columns ) {
		...
		self::$_gcdb->is_mysql = false;

		self::$_gcdb->query( $create );

		$columns = array_keys( $columns );
		foreach ( $columns as $column => $charset ) {
			$this->assertFalse( self::$_gcdb->get_col_charset( $table, $column ) );
		}

		self::$_gcdb->query( $drop );

		self::$_gcdb->is_mysql = true;
	}
	 ...
	 * @ticket 21212
	 *
	 * @covers gcdb::strip_invalid_text_from_query
	 */
	public function test_strip_invalid_text_from_query( $create, $query, $expected, $drop ) {
		...
		$return = self::$_gcdb->strip_invalid_text_from_query( $query );
	...
	public function test_invalid_characters_in_query() {
		...
		$this->assertFalse( $gcdb->query( "INSERT INTO {$gcdb->posts} (post_content) VALUES ('foo\xf0\xff\xff\xffbar')" ) );
	}
	 ...
	 * @ticket 21212
	 *
	 * @covers gcdb::check_safe_collation
	 */
	public function test_table_collation_check( $create, $expected, $query, $drop, $always_true ) {
		...
		$return = self::$_gcdb->check_safe_collation( $query );
		$this->assertSame( $expected, $return );

		foreach ( $always_true as $true_query ) {
			$return = self::$_gcdb->check_safe_collation( $true_query );
	...
	public function test_strip_invalid_text_for_column_bails_if_ascii_input_too_long() {
		...
		// TEXT column.
		$stripped = $gcdb->strip_invalid_text_for_column( $gcdb->comments, 'comment_content', str_repeat( 'A', 65536 ) );
		$this->assertSame( 65535, strlen( $stripped ) );

		// VARCHAR column.
		$stripped = $gcdb->strip_invalid_text_for_column( $gcdb->comments, 'comment_agent', str_repeat( 'A', 256 ) );
	...
	public function test_strip_invalid_text_from_query_cp1251_is_safe() {
		...
		if ( ! self::$_gcdb->query( "CREATE TABLE $tablename ( a VARCHAR(50) ) DEFAULT CHARSET 'cp1251'" ) ) {
			$this->markTestSkipped( "Test requires the 'cp1251' charset." );
		}

		$safe_query     = "INSERT INTO $tablename( `a` ) VALUES( 'safe data' )";
		$stripped_query = self::$_gcdb->strip_invalid_text_from_query( $safe_query );

		self::$_gcdb->query( "DROP TABLE $tablename" );
	 ...
	 * @ticket 36649
	 *
	 * @covers gcdb::set_charset
	 */
	public function test_set_charset_changes_the_connection_collation() {
		self::$_gcdb->set_charset( self::$_gcdb->dbh, 'utf8', 'utf8_general_ci' );
		$results = self::$_gcdb->get_results( "SHOW VARIABLES WHERE Variable_name='collation_connection'" );
		$this->assertSame( 'utf8_general_ci', $results[0]->Value );

		self::$_gcdb->set_charset( self::$_gcdb->dbh, 'utf8mb4', 'utf8mb4_unicode_ci' );
		$results = self::$_gcdb->get_results( "SHOW VARIABLES WHERE Variable_name='collation_connection'" );
		$this->assertSame( 'utf8mb4_unicode_ci', $results[0]->Value );

		self::$_gcdb->set_charset( self::$_gcdb->dbh );
	}
}
