 ...
 * @group admin
 */
class Tests_Admin_IncludesPost extends GC_UnitTestCase {
	...
	public function test__gc_translate_postdata_cap_checks_contributor() {
		...
		$_results = _gc_translate_postdata( false, $_post_data );
		...
		$_results = _gc_translate_postdata( false, $_post_data );
		$this->assertInstanceOf( 'GC_Error', $_results );
		...
		$_results = _gc_translate_postdata( true, $_post_data );
		$this->assertInstanceOf( 'GC_Error', $_results );
		$this->assertSame( 'edit_others_posts', $_results->get_error_code() );
		$this->assertSame( 'Sorry, you are not allowed to edit posts as this user.', $_results->get_error_message() );
	}

	public function test__gc_translate_postdata_cap_checks_editor() {
	...
	public function test_edit_post_should_parse_tax_input_by_name_rather_than_slug_for_nonhierarchical_taxonomies() {
		...
		register_taxonomy( 'gctests_tax', array( 'post' ) );
		...
		$found = gc_get_post_terms( self::$post_id, 'gctests_tax' );

		// Should contain the term with the name 'foo', not the slug.
		$this->assertContains( $t1, gc_list_pluck( $found, 'term_id' ) );

		// The 'baz' tag should have been created.
		$this->assertContains( 'baz', gc_list_pluck( $found, 'name' ) );
	}
	 ...
	 * @ticket 41396
	 */
	public function test_bulk_edit_posts_should_set_post_format_before_gc_update_post_runs() {
		gc_set_current_user( self::$admin_id );
	...
	public function test_post_exists_should_not_match_invalid_post_type_and_status_combined() {
		...
		$this->assertSame( 0, post_exists( $title, null, null, 'gc_tests', $post_status ) );
	}
}
