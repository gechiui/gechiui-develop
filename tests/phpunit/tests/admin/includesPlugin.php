 ...
 * @group admin
 */
class Tests_Admin_IncludesPlugin extends GC_UnitTestCase {
	...
	public function test_get_plugin_data() {
		...
		$default_headers = array(
			'Name'        => 'Hello Dolly',
			'Title'       => '<a href="http://www.gechiui.com/#">Hello Dolly</a>',
			'PluginURI'   => 'http://www.gechiui.com/#',
	...
	public function test_menu_page_url() {
		...
		$expected['testsettings']        = 'http://example.com/gc-admin/options-general.php?page=testsettings';
		$expected['testtools']           = 'http://example.com/gc-admin/tools.php?page=testtools';
		$expected['mt-top-level-handle'] = 'http://example.com/gc-admin/admin.php?page=mt-top-level-handle';
		$expected['sub-page']            = 'http://example.com/gc-admin/admin.php?page=sub-page';
		$expected['sub-page2']           = 'http://example.com/gc-admin/admin.php?page=sub-page2';
		$expected['not_registered']      = '';
		$expected['With Spaces']         = 'http://example.com/gc-admin/themes.php?page=With%20Spaces';
		$expected['testpages']           = 'http://example.com/gc-admin/edit.php?post_type=page&#038;page=testpages';

		foreach ( $expected as $name => $value ) {
			$this->assertSame( $value, menu_page_url( $name, false ) );
		}

		gc_set_current_user( $current_user );
	}
	...
	public function test_submenu_position( $position, $expected_position ) {
		...
		gc_set_current_user( $admin_user );
	...
	public function test_get_plugin_files_folder() {
		$plugin_dir = GC_PLUGIN_DIR . '/list_files_test_plugin';
		@mkdir( $plugin_dir );
	...
	public function test_get_mu_plugins_when_mu_plugins_exists_but_is_empty() {
		mkdir( GCMU_PLUGIN_DIR );

		$mu_plugins = get_mu_plugins();

		rmdir( GCMU_PLUGIN_DIR );
	...
	public function test_get_mu_plugins_when_mu_plugins_directory_does_not_exist() {
		$this->assertFileDoesNotExist( GCMU_PLUGIN_DIR );
	...
	public function test_get_mu_plugins_should_ignore_index_php_containing_silence_is_golden() {
		...
		$this->_create_plugin( '<?php\n//Silence is golden.', 'index.php', GCMU_PLUGIN_DIR );

		$mu_plugins = get_mu_plugins();

		unlink( GCMU_PLUGIN_DIR . '/index.php' );
		rmdir( GCMU_PLUGIN_DIR );
	...
	public function test_get_mu_plugins_should_not_ignore_index_php_containing_something_other_than_silence_is_golden() {
		...
		$this->_create_plugin( '<?php\n//Silence is not golden.', 'index.php', GCMU_PLUGIN_DIR );
	...
	public function test_get_mu_plugins_should_ignore_files_without_php_extensions() {
		...
		$this->_create_plugin( '<?php\n//Test', 'foo.php', GCMU_PLUGIN_DIR );
		$this->_create_plugin( '<?php\n//Test 2', 'bar.txt', GCMU_PLUGIN_DIR );
		$found = get_mu_plugins();

		// Clean up.
		unlink( GCMU_PLUGIN_DIR . '/foo.php' );
		unlink( GCMU_PLUGIN_DIR . '/bar.txt' );
	...
	public function test_get_dropins_not_empty() {
		...
		$p1 = $this->_create_plugin( "<?php\n//Test", 'advanced-cache.php', GC_CONTENT_DIR );
		$p2 = $this->_create_plugin( "<?php\n//Test", 'not-a-dropin.php', GC_CONTENT_DIR );
	...
	private function _create_plugin( $data = "<?php\n/*\nPlugin Name: Test\n*/", $filename = false, $dir_path = false ) {
		...
		if ( false === $dir_path ) {
			$dir_path = GC_PLUGIN_DIR;
		}

		$filename  = gc_unique_filename( $dir_path, $filename );
	...
	/**
	 * Move existing mu-plugins to gc-content/mu-plugin-backup.
	 *
	 * @since 4.2.0
	 *
	 * @access private
	...
	private static function _back_up_mu_plugins() {
		if ( is_dir( GCMU_PLUGIN_DIR ) ) {
			$mu_bu_dir = GC_CONTENT_DIR . '/mu-plugin-backup';
			rename( GCMU_PLUGIN_DIR, $mu_bu_dir );
		}
	}
	...
	private static function _restore_mu_plugins() {
		...
		if ( is_dir( $mu_bu_dir ) ) {
			rename( $mu_bu_dir, GCMU_PLUGIN_DIR );
		}
	}

	/**
	 * Move existing drop-ins to gc-content/drop-ins-backup.
	 *
	 * @since 4.2.0
	 *
	 * @access private
	...
	private function _back_up_drop_ins() {
		$di_bu_dir = GC_CONTENT_DIR . '/drop-ins-backup';
		if ( ! is_dir( $di_bu_dir ) ) {
			mkdir( $di_bu_dir );
		}

		foreach ( _get_dropins() as $file_to_move => $v ) {
			if ( file_exists( GC_CONTENT_DIR . '/' . $file_to_move ) ) {
				rename( GC_CONTENT_DIR . '/' . $file_to_move, $di_bu_dir . '/' . $file_to_move );
			}
		}
	}
	...
	private function _restore_drop_ins() {
		...
		foreach ( _get_dropins() as $file_to_move => $v ) {
			if ( file_exists( $di_bu_dir . '/' . $file_to_move ) ) {
				rename( $di_bu_dir . '/' . $file_to_move, GC_CONTENT_DIR . '/' . $file_to_move );
			}
		}
