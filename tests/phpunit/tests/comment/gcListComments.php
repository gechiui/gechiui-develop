 ...
 * @group comment
 */
class Tests_Comment_GcListComments extends GC_UnitTestCase {
	/**
	 * @ticket 35175
	 */
	public function test_should_respect_page_param() {
		...
		// comments_template() populates $gc_query->comments.
		get_echo( 'comments_template' );

		$found = gc_list_comments(
			array(
				'page' => 2,
				'echo' => false,
			)
	...
	public function test_comments_param_should_be_respected_when_custom_pagination_params_are_passed() {
		...
		// Populate `$gc_query->comments` in order to show that it doesn't override `$_comments`.
		$this->go_to( get_permalink( $p ) );
		get_echo( 'comments_template' );
	...
	public function test_custom_pagination_should_not_result_in_unapproved_comments_being_shown() {
		...
		gc_set_comment_status( $comments[0], '0' );
		gc_set_comment_status( $comments[1], '0' );
		gc_set_comment_status( $comments[3], '0' );
		gc_set_comment_status( $comments[4], '0' );
