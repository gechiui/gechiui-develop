 ...
 * @group comment
 */
class Tests_Comment_Query extends GC_UnitTestCase {
	...
	public function test_orderby_meta() {
		...
		$this->assertEquals( array( $comment_id3, $comment_id ), gc_list_pluck( $comments, 'comment_ID' ) );
	...
	public function test_search_int_0_should_not_be_ignored() {
		...
		$this->assertStringContainsString( "comment_author LIKE '%0%'", $gcdb->remove_placeholder_escape( $q->request ) );
	}
	...
	public function test_orderby_default() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->comments.comment_date_gmt", $q->request );
	}
	...
	public function test_orderby_single() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->comments.comment_agent", $q->request );
	}
	...
	public function test_orderby_space_separated() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->comments.comment_agent DESC, $gcdb->comments.comment_approved DESC", $q->request );
	}
	...
	public function test_orderby_array() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->comments.comment_agent DESC, $gcdb->comments.comment_date_gmt ASC, $gcdb->comments.comment_ID DESC", $q->request );
	}
	...
	public function test_orderby_array_should_discard_invalid_columns() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->comments.comment_agent DESC, $gcdb->comments.comment_ID DESC", $q->request );
	}
	...
	public function test_orderby_array_should_convert_invalid_order_to_DESC() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->comments.comment_agent DESC, $gcdb->comments.comment_date_gmt DESC, $gcdb->comments.comment_ID DESC", $q->request );
	}
	...
	public function test_orderby_array_should_sort_by_comment_ID_as_fallback_and_should_inherit_order_from_comment_date_gmt() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->comments.comment_agent DESC, $gcdb->comments.comment_date_gmt ASC, $gcdb->comments.comment_ID ASC", $q->request );
	}
	...
	public function test_orderby_array_should_sort_by_comment_ID_as_fallback_and_should_inherit_order_from_comment_date() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->comments.comment_agent DESC, $gcdb->comments.comment_date ASC, $gcdb->comments.comment_ID ASC", $q->request );
	}
	...
	public function test_orderby_array_should_sort_by_comment_ID_DESC_as_fallback_when_not_sorted_by_date() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->comments.comment_agent ASC, $gcdb->comments.comment_ID DESC", $q->request );
	}
	...
	public function test_orderby_date_modified_gmt_should_order_by_comment_ID_in_case_of_tie_ASC() {
		...
		$this->assertEquals( $comments, gc_list_pluck( $found, 'comment_ID' ) );
	}
	...
	public function test_comment_query_object() {
		...
		$query2 = new GC_Comment_Query( array( 'status' => 'all' ) );
	...
	public function test_cache_should_be_sensitive_to_parent__in() {
		...
		$this->assertNotEquals( $num_queries, $gcdb->num_queries );
	}
	...
	public function modify_meta_query( $q ) {
		$q->meta_query = new GC_Meta_Query(
			array(
				array(
					'key'   => 'foo',
					'value' => 'bar',
	...
	public function test_hierarchical_should_not_include_child_comments_in_number() {
		...
		$this->assertEqualSets( array( $top_level_0, $child_of_0, $top_level_comments[0] ), gc_list_pluck( $q->comments, 'comment_ID' ) );
	}
	...
	public function test_hierarchical_threaded() {
		...
		$q = new GC_Comment_Query( $query_args );

		// Top-level comments.
		$this->assertEqualSets( array( $c1, $c5 ), array_values( gc_list_pluck( $q->comments, 'comment_ID' ) ) );

		// Direct descendants of $c1.
		$this->assertEqualSets( array( $c2, $c4 ), array_values( gc_list_pluck( $q->comments[ $c1 ]->get_children( $args ), 'comment_ID' ) ) );

		// Direct descendants of $c2.
		$this->assertEqualSets( array( $c3 ), array_values( gc_list_pluck( $q->comments[ $c1 ]->get_child( $c2 )->get_children( $args ), 'comment_ID' ) ) );

		// Direct descendants of $c5.
		$this->assertEqualSets( array( $c6 ), array_values( gc_list_pluck( $q->comments[ $c5 ]->get_children( $args ), 'comment_ID' ) ) );
	}
	...
	public function test_hierarchical_threaded_approved() {
		...
		// Direct descendants of $c2.
		$this->assertEqualSets( array(), array_values( gc_list_pluck( $q->comments[ $c1 ]->get_child( $c2 )->get_children( $args ), 'comment_ID' ) ) );
	}
	...
	public function test_comment_clauses_prepend_callback_should_be_respected_when_filling_descendants() {
		...
		$this->assertEqualSets( array( $top_level_0, $child1_of_0, $top_level_comments[0], $top_level_comments[2] ), gc_list_pluck( $q->comments, 'comment_ID' ) );
	}

	public function prepend_exclusions( $clauses ) {
		global $gcdb;
		$clauses['where'] = $gcdb->prepare( 'comment_ID != %d AND comment_ID != %d AND ', $this->to_exclude[0], $this->to_exclude[1] ) . $clauses['where'];
		return $clauses;
	}
	...
	public function append_exclusions( $clauses ) {
		global $gcdb;
		$clauses['where'] .= $gcdb->prepare( ' AND comment_ID != %d AND comment_ID != %d', $this->to_exclude[0], $this->to_exclude[1] );
		return $clauses;
	}
	...
	public function test_cache_should_be_hit_when_querying_descendants() {
		...
		$q1_ids = gc_list_pluck( $q1->comments, 'comment_ID' );
		...
		$q2_ids      = gc_list_pluck( $q2->comments, 'comment_ID' );
	...
	public function test_hierarchy_should_be_filled_when_cache_is_incomplete() {
		...
		$last_changed = gc_cache_get( 'last_changed', 'comment' );
		$key          = md5( serialize( gc_array_slice_assoc( $q1->query_vars, array_keys( $q1->query_var_defaults ) ) ) );
		$cache_key    = "get_comment_child_ids:$comment_2:$key:$last_changed";
		gc_cache_delete( $cache_key, 'comment' );
	...
	public function test_comment_objects_should_be_filled_from_cache() {
		...
		$found = gc_list_pluck( $q->comments, 'comment_ID' );
	...
	public function test_comment_objects_should_be_fetched_from_database_when_suspend_cache_addition() {
		$suspend = gc_suspend_cache_addition();
		gc_suspend_cache_addition( true );
		...
		gc_suspend_cache_addition( $suspend );
