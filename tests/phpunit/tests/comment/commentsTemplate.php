 ...
 */
class Tests_Comment_CommentsTemplate extends GC_UnitTestCase {
	...
	public function test_query_offset_should_include_unapproved_comments() {
		...
		add_filter( 'gc_get_current_commenter', array( $this, 'fake_current_commenter' ) );
		$this->go_to( get_permalink( $p ) );
		$found = get_echo( 'comments_template' );
		remove_filter( 'gc_get_current_commenter', array( $this, 'fake_current_commenter' ) );
		...
		$this->go_to(
			add_query_arg(
				array(
					...
					'moderation-hash' => gc_hash( $comment->comment_date_gmt ),
				),
