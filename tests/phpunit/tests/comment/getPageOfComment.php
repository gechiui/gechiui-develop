 ...
 */
class Tests_Comment_GetPageOfComment extends GC_UnitTestCase {
	...
	public function test_cache_should_be_invalidated_when_comment_is_approved() {
		...
		// Approve comment.
		gc_set_comment_status( $c, 'approve' );

		$this->assertFalse( gc_cache_get( $c, 'comment_pages' ) );
	}
	...
	public function test_page_number_when_unapproved_comments_are_included_for_current_commenter() {
		...
		add_filter( 'gc_get_current_commenter', array( $this, 'get_current_commenter' ) );
		...
		remove_filter( 'gc_get_current_commenter', array( $this, 'get_current_commenter' ) );

		$this->assertContains( (string) $new_unapproved, gc_list_pluck( $comments, 'comment_ID' ) );
	}
