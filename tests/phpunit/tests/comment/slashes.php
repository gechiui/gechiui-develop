 ...
 * @ticket 21767
 */
class Tests_Comment_Slashes extends GC_UnitTestCase {
	 ...
	 */
	public function test_gc_new_comment() {
		...
		$comment_id = gc_new_comment( $data );

		$comment = get_comment( $comment_id );

		$this->assertSame( gc_unslash( $this->slash_1 ), $comment->comment_author );
		$this->assertSame( gc_unslash( $this->slash_7 ), $comment->comment_content );
		...
		$this->assertSame( gc_unslash( $this->slash_2 ), $comment->comment_author );
		$this->assertSame( gc_unslash( $this->slash_4 ), $comment->comment_content );
	}
	 ...
	 */
	public function test_gc_insert_comment() {
		$post_id = self::$post_id;

		$comment_id = gc_insert_comment(
			array(
				'comment_post_ID' => $post_id,
				'comment_author'  => $this->slash_1,
				'comment_content' => $this->slash_7,
