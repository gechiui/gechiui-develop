 ...
 * @group mail
 */
class Tests_Mail extends GC_UnitTestCase {
	 ...
	 */
	public function test_gc_mail_break_it() {
		$content = str_repeat( 'A', 1000 );
		$this->assertTrue( gc_mail( GC_TESTS_EMAIL, 'Looong line testing', $content ) );
	}

	public function test_gc_mail_custom_boundaries() {
		$to       = 'user@example.com';
		...
		gc_mail( $to, $subject, $body, $headers );
	 ...
	 * @ticket 17305
	 */
	public function test_gc_mail_rfc2822_addresses() {
		...
		// GeChiUI 3.2 and later correctly split the address into the two parts and send them separately to PHPMailer.
	 ...
	 * @ticket 17305
	 */
	public function test_gc_mail_multiple_rfc2822_to_addresses() {
		$to      = 'Name <address@tld.com>, Another Name <another_address@different-tld.com>';
		$subject = 'RFC2822 Testing';
		$message = 'My RFC822 Test Message';

		gc_mail( $to, $subject, $message );
	...
	public function test_gc_mail_multiple_to_addresses() {
	 ...
	 * @ticket 18463
	 */
	public function test_gc_mail_to_address_no_name() {
		$to      = '<address@tld.com>';
		$subject = 'RFC2822 Testing';
		$message = 'My RFC822 Test Message';
	 ...
	 * @ticket 23642
	 */
	public function test_gc_mail_return_value() {
		// No errors.
		$this->assertTrue( gc_mail( 'valid@address.com', 'subject', 'body' ) );

		// Non-fatal errors.
		$this->assertTrue( gc_mail( 'valid@address.com', 'subject', 'body', "Cc: invalid-address\nBcc: @invalid.address", ABSPATH . '/non-existent-file.html' ) );

		// Fatal errors.
		$this->assertFalse( gc_mail( 'invalid.address', 'subject', 'body', '', array() ) );
	}

	/**
	 * @ticket 30266
	 */
	public function test_gc_mail_with_valid_from_header() {
		$to       = 'address@tld.com';
		$subject  = 'Testing';
	 ...
	 * @ticket 30266
	 */
	public function test_gc_mail_with_empty_from_header() {
		$to       = 'address@tld.com';
		$subject  = 'Testing';
		$message  = 'Test Message';
		$headers  = 'From: ';
		$expected = 'From: GeChiUI <gechiui@' . GC_TESTS_DOMAIN . '>';
	 ...
	 * @ticket 30266
	 */
	public function test_gc_mail_with_empty_from_name_for_the_from_header() {
		$to       = 'address@tld.com';
		$subject  = 'Testing';
		$message  = 'Test Message';
		$headers  = 'From: <gechiui@example.com>';
		$expected = 'From: GeChiUI <gechiui@example.com>';
	 ...
	 * @ticket 30266
	 */
	public function test_gc_mail_with_valid_content_type_header() {
		$to       = 'address@tld.com';
		$subject  = 'Testing';
		$message  = 'Test Message';
	 ...
	 * @ticket 30266
	 */
	public function test_gc_mail_with_empty_content_type_header() {
		$to       = 'address@tld.com';
		$subject  = 'Testing';
		$message  = 'Test Message';
	 ...
	 * @ticket 30266
	 */
	public function test_gc_mail_with_empty_charset_for_the_content_type_header() {
		$to       = 'address@tld.com';
		$subject  = 'Testing';
		$message  = 'Test Message';
	 ...
	 * @ticket 43542
	 */
	public function test_gc_mail_does_not_duplicate_mime_version_header() {
		$to       = 'user@example.com';
	...
	public function gc_mail_quoted_printable( $mailer ) {
		$mailer->Encoding = 'quoted-printable';
	}

	public function gc_mail_set_text_message( $mailer ) {
	 ...
	 * @ticket 28039
	 */
	public function test_gc_mail_content_transfer_encoding_in_quoted_printable_multipart() {
		add_action( 'phpmailer_init', array( $this, 'gc_mail_quoted_printable' ) );
		add_action( 'phpmailer_init', array( $this, 'gc_mail_set_text_message' ) );
	 ...
	 * @ticket 21659
	 */
	public function test_gc_mail_addresses_arent_encoded() {
		...
		gc_mail( $to, $subject, $message, array_values( $headers ) );
	 ...
	 * @ticket 37736
	 */
	public function test_gc_mail_sender_not_set() {
		gc_mail( 'user@example.org', 'Testing the Sender field', 'The Sender field should not have been set.' );
	...
	public function test_phpmailer_exception_thrown() {
		...
		$ma = new MockAction();
		add_action( 'gc_mail_failed', array( &$ma, 'action' ) );
		...
		// Retrieve the arguments passed to the 'gc_mail_failed' hook callbacks.
		$all_args  = $ma->get_args();
		$call_args = array_pop( $all_args );

		$this->assertSame( 'gc_mail_failed', $call_args[0]->get_error_code() );
	...
	/**
	 * Test for short-circuiting gc_mail().
	 *
	 * @ticket 35069
	 */
	public function test_gc_mail_can_be_shortcircuited() {
		$result1 = gc_mail( GC_TESTS_EMAIL, 'Foo', 'Bar' );

		add_filter( 'pre_gc_mail', '__return_false' );
		$result2 = gc_mail( GC_TESTS_EMAIL, 'Foo', 'Bar' );
		remove_filter( 'pre_gc_mail', '__return_false' );
