 ...
 * @ticket 5305
 */
class Tests_Rewrite_NumericSlugs extends GC_UnitTestCase {
	...
	public function set_up() {
		...
		// Override the post/archive slug collision prevention in `gc_unique_post_slug()`.
		add_filter( 'gc_unique_post_slug', array( $this, 'filter_unique_post_slug' ), 10, 6 );
	}
	...
	public function test_go_to_year_segment_collision_without_title() {
		...
		$gcdb->update(
			...
			array(
				'ID'   => '2015',
				'guid' => 'http://' . GC_TESTS_DOMAIN . '/?p=2015',
			),
			array( 'ID' => $id )
		);
