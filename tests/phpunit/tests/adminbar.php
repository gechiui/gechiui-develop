 ...
 * @group admin
 */
class Tests_AdminBar extends GC_UnitTestCase {
	...
	public function test_content_post_type() {
		...
		$admin_bar = new GC_Admin_Bar;

		gc_admin_bar_new_content_menu( $admin_bar );
	...
	public function test_admin_bar_contains_correct_links_for_users_with_no_role() {
		...
		gc_set_current_user( self::$no_role_id );

		$gc_admin_bar = $this->get_standard_admin_bar();

		$node_site_name    = $gc_admin_bar->get_node( 'site-name' );
		$node_my_account   = $gc_admin_bar->get_node( 'my-account' );
		$node_user_info    = $gc_admin_bar->get_node( 'user-info' );
		$node_edit_profile = $gc_admin_bar->get_node( 'edit-profile' );
	...
	protected function get_standard_admin_bar() {
		...
		$this->assertInstanceOf( 'GC_Admin_Bar', $gc_admin_bar );
	 ...
	 *
	 * @param array  $node_data     The data for a node, passed to `GC_Admin_Bar::add_node()`.
	 ...
	 * @return array {
	 *     @type array {
	 *         @type array  $node_data     The data for a node, passed to `GC_Admin_Bar::add_node()`.
	...
	public function test_admin_bar_has_edit_link_for_existing_posts() {
		...
		global $gc_the_query;
		$gc_the_query->queried_object = (object) array(
			'ID'        => $id,
			'post_type' => 'post',
		);
		...
		$node_edit = $gc_admin_bar->get_node( 'edit' );
	...
	public function test_admin_bar_has_no_archives_link_if_no_static_front_page() {
		...
		$gc_admin_bar = $this->get_standard_admin_bar();
		$node         = $gc_admin_bar->get_node( 'archive' );
	...
	public function test_admin_bar_contains_correct_about_link_for_users_with_role() {
		...
		$gc_admin_bar = $this->get_standard_admin_bar();
		$gc_logo_node = $gc_admin_bar->get_node( 'gc-logo' );
		$about_node   = $gc_admin_bar->get_node( 'about' );

		$this->assertNotNull( $gc_logo_node );
		$this->assertSame( admin_url( 'about.php' ), $gc_logo_node->href );
		$this->assertArrayNotHasKey( 'tabindex', $gc_logo_node->meta );
	...
	public function test_admin_bar_contains_correct_about_link_for_users_with_no_role() {
		...
		$this->assertNotNull( $gc_logo_node );
		$this->assertFalse( $gc_logo_node->href );
		$this->assertArrayHasKey( 'tabindex', $gc_logo_node->meta );
		$this->assertSame( 0, $gc_logo_node->meta['tabindex'] );
	...
	public function test_admin_bar_contains_correct_about_link_for_users_with_no_role_in_multisite() {
		...
		$this->assertNotNull( $gc_logo_node );
		$this->assertSame( user_admin_url( 'about.php' ), $gc_logo_node->href );
		$this->assertArrayNotHasKey( 'tabindex', $gc_logo_node->meta );
		$this->assertNotNull( $about_node );
	}
	...
	public function test_new_user_link_exists_for_user_with_create_users() {
		...
		$gc_admin_bar = $this->get_standard_admin_bar();
		$node         = $gc_admin_bar->get_node( 'new-user' );
	...
	/**
	 * @ticket 30937
	 * @covers ::gc_admin_bar_customize_menu
	 */
	public function test_customize_link() {
		...
		$gc_admin_bar = $this->get_standard_admin_bar();
		$node         = $gc_admin_bar->get_node( 'customize' );
		$this->assertNotEmpty( $node );

		$parsed_url   = gc_parse_url( $node->href );
		$query_params = array();
		gc_parse_str( $parsed_url['query'], $query_params );
