 ...
 */
class Tests_Functions_getPostGalleries extends GC_UnitTestCase {
	...
	public function test_returns_no_srcs_with_block_v2_in_post_with_no_attached_images() {
		...
		$image_url = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/test.jpg';

		$blob = <<< BLOB
<!-- gc:gallery {"linkTo":"none","className":"columns-2"} -->
<figure
	class="gc-block-gallery has-nested-images columns-default is-cropped columns-2"
>
	<!-- gc:image {"id":$image_id,"sizeSlug":"large","linkDestination":"none"} -->
	<figure class="gc-block-image size-large">
		<img
			src="$image_url"
			alt="Image gallery image"
			class="gc-image-$image_id"
		/>
	</figure>
	<!-- /gc:image -->
</figure>
...
BLOB;
	...
	public function test_returns_html_with_shortcode_gallery() {
		...
		$expected  = 'src="http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/test.jpg"';
...
BLOB;
	...
	public function test_respects_post_id_with_shortcode_gallery() {
		...
		$expected_srcs = array(
			'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/test.jpg',
		);
...
BLOB;
	...
	public function test_respects_post_id_with_block_gallery_v2() {
		...
		$url            = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . 'image1.jpg';
		$global_post_id = $this->factory->post->create(
			array(
				'post_content' => 'Global Post',
			)
...
<figure
	class="gc-block-gallery has-nested-images columns-default is-cropped columns-2"
>
	<!-- gc:image {"id":$attachment_id,"sizeSlug":"large","linkDestination":"none"} -->
	<figure class="gc-block-image size-large">
		<img
			src="$url"
			alt="Image gallery image"
			class="gc-image-$attachment_id"
		/>
	</figure>
	<!-- /gc:image -->
</figure>
...
<!-- gc:gallery {"ids":[$ids2_joined],"columns":3,"imageCrop":false,"linkTo":"media"} -->
<!-- /gc:gallery -->
BLOB;
	...
	public function test_returns_srcs_from_html_with_block_with_no_json_blob() {
		...
		$blob = <<< BLOB
<!-- gc:gallery -->
<ul class="gc-block-gallery columns-2 is-cropped"><li class="blocks-gallery-item">
<figure>
