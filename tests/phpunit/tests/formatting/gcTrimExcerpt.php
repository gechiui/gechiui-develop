...
/**
 * @group formatting
 * @covers ::gc_trim_excerpt
 */
class Tests_Formatting_gcTrimExcerpt extends GC_UnitTestCase {
	/**
	 * @ticket 25349
	 */
	public function test_secondary_loop_respect_more() {
		...
		if ( $q->have_posts() ) {
			while ( $q->have_posts() ) {
				$q->the_post();
				$this->assertSame( 'Post 2 Page 1', gc_trim_excerpt() );
			}
		}
	}
	...
	public function test_should_generate_excerpt_for_empty_values() {
		...
		$this->assertSame( 'Post content', gc_trim_excerpt( '', $post ) );
		$this->assertSame( 'Post content', gc_trim_excerpt( null, $post ) );
		$this->assertSame( 'Post content', gc_trim_excerpt( false, $post ) );
	}
}
