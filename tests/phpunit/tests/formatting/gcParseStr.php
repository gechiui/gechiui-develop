 ...
 * @covers ::gc_parse_str
 */
class Tests_Formatting_gcParseStr extends GC_UnitTestCase {
	 ...
	 *
	 * @dataProvider data_gc_parse_str
	 *
	 * @param mixed $input    Value to parse.
	 * @param array $expected Expected function output.
	 */
	public function test_gc_parse_str( $input, $expected ) {
		gc_parse_str( $input, $output );
	 ...
	 * @return array
	 */
	public function data_gc_parse_str() {
		return array(
			'null'              => array(
				'input'    => null,
				'expected' => array(),
			...
			'stringable object' => array(
				'input'    => new Fixture_Formatting_gcParseStr(),
				'expected' => array(
					'foobar' => '',
				),
			),
	 ...
	 *
	 * @dataProvider data_gc_parse_str_result_array_is_always_overwritten
	 *
	 * @param array|null $output   Value for the `$output` parameter.
	 * @param array      $expected Expected function output.
	 */
	public function test_gc_parse_str_result_array_is_always_overwritten( $output, $expected ) {
		gc_parse_str( 'key=25&thing=text', $output );
		$this->assertSame( $expected, $output );
	}
	 ...
	 * @return array
	 */
	public function data_gc_parse_str_result_array_is_always_overwritten() {
 ...
 */
class Fixture_Formatting_gcParseStr {
