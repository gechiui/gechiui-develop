 ...
 * @group formatting
 */
class Tests_Formatting_gcTexturize extends GC_UnitTestCase {
	public function test_dashes() {
		$this->assertSame( 'Hey &#8212; boo?', gctexturize( 'Hey -- boo?' ) );
		$this->assertSame( '<a href="http://xx--xx">Hey &#8212; boo?</a>', gctexturize( '<a href="http://xx--xx">Hey -- boo?</a>' ) );
	}

	public function test_disable() {
		$this->assertSame( '<pre>---&</pre>', gctexturize( '<pre>---&</pre>' ) );
		$this->assertSame( '<pre><code></code>--&</pre>', gctexturize( '<pre><code></code>--&</pre>' ) );

		$this->assertSame( '<code>---&</code>', gctexturize( '<code>---&</code>' ) );
		$this->assertSame( '<kbd>---&</kbd>', gctexturize( '<kbd>---&</kbd>' ) );
		$this->assertSame( '<style>---&</style>', gctexturize( '<style>---&</style>' ) );
		$this->assertSame( '<script>---&</script>', gctexturize( '<script>---&</script>' ) );
		$this->assertSame( '<tt>---&</tt>', gctexturize( '<tt>---&</tt>' ) );

		$this->assertSame( '<code>href="baba"</code> &#8220;baba&#8221;', gctexturize( '<code>href="baba"</code> "baba"' ) );

		$enabled_tags_inside_code = '<code>curl -s <a href="http://x/">baba</a> | grep sfive | cut -d "\"" -f 10 &gt; topmp3.txt</code>';
		$this->assertSame( $enabled_tags_inside_code, gctexturize( $enabled_tags_inside_code ) );

		$double_nest = '<pre>"baba"<code>"baba"<pre></pre></code>"baba"</pre>';
		$this->assertSame( $double_nest, gctexturize( $double_nest ) );

		$invalid_nest = '<pre></code>"baba"</pre>';
		$this->assertSame( $invalid_nest, gctexturize( $invalid_nest ) );
	...
	public function test_bracketed_quotes_1418() {
		$this->assertSame( '(&#8220;test&#8221;)', gctexturize( '("test")' ) );
		$this->assertSame( '(&#8216;test&#8217;)', gctexturize( "('test')" ) );
		$this->assertSame( '(&#8217;twas)', gctexturize( "('twas)" ) );
	}
	...
	public function test_bracketed_quotes_3810() {
		$this->assertSame( 'A dog (&#8220;Hubertus&#8221;) was sent out.', gctexturize( 'A dog ("Hubertus") was sent out.' ) );
	}
	...
	public function test_basic_quotes() {
		$this->assertSame( 'test&#8217;s', gctexturize( 'test\'s' ) );

		$this->assertSame( '&#8216;quoted&#8217;', gctexturize( '\'quoted\'' ) );
		$this->assertSame( '&#8220;quoted&#8221;', gctexturize( '"quoted"' ) );

		$this->assertSame( 'space before &#8216;quoted&#8217; space after', gctexturize( 'space before \'quoted\' space after' ) );
		$this->assertSame( 'space before &#8220;quoted&#8221; space after', gctexturize( 'space before "quoted" space after' ) );

		$this->assertSame( '(&#8216;quoted&#8217;)', gctexturize( '(\'quoted\')' ) );
		$this->assertSame( '{&#8220;quoted&#8221;}', gctexturize( '{"quoted"}' ) );

		$this->assertSame( '&#8216;qu(ot)ed&#8217;', gctexturize( '\'qu(ot)ed\'' ) );
		$this->assertSame( '&#8220;qu{ot}ed&#8221;', gctexturize( '"qu{ot}ed"' ) );

		$this->assertSame( ' &#8216;test&#8217;s quoted&#8217; ', gctexturize( ' \'test\'s quoted\' ' ) );
		$this->assertSame( ' &#8220;test&#8217;s quoted&#8221; ', gctexturize( ' "test\'s quoted" ' ) );
	}
	...
	public function test_full_sentences_with_unmatched_single_quotes() {
		$this->assertSame(
			...
			gctexturize( "That means every moment you're working on something without it being in the public it's actually dying." )
		);
	}
	...
	public function test_quotes() {
		$this->assertSame( '&#8220;Quoted String&#8221;', gctexturize( '"Quoted String"' ) );
		// $this->assertSame( 'Here is &#8220;<a href="http://example.com">a test with a link</a>&#8221;', gctexturize( 'Here is "<a href="http://example.com">a test with a link</a>"' ) );
		// $this->assertSame( 'Here is &#8220;<a href="http://example.com">a test with a link and a period</a>&#8221;.', gctexturize( 'Here is "<a href="http://example.com">a test with a link and a period</a>".' ) );
		$this->assertSame( 'Here is &#8220;<a href="http://example.com">a test with a link</a>&#8221; and a space.', gctexturize( 'Here is "<a href="http://example.com">a test with a link</a>" and a space.' ) );
		$this->assertSame( 'Here is &#8220;<a href="http://example.com">a test with a link</a> and some text quoted&#8221;', gctexturize( 'Here is "<a href="http://example.com">a test with a link</a> and some text quoted"' ) );
		// $this->assertSame( 'Here is &#8220;<a href="http://example.com">a test with a link</a>&#8221;, and a comma.', gctexturize( 'Here is "<a href="http://example.com">a test with a link</a>", and a comma.' ) );
		// $this->assertSame( 'Here is &#8220;<a href="http://example.com">a test with a link</a>&#8221;; and a semi-colon.', gctexturize( 'Here is "<a href="http://example.com">a test with a link</a>"; and a semi-colon.' ) );
		// $this->assertSame( 'Here is &#8220;<a href="http://example.com">a test with a link</a>&#8221;- and a dash.', gctexturize( 'Here is "<a href="http://example.com">a test with a link</a>"- and a dash.' ) );
		// $this->assertSame( 'Here is &#8220;<a href="http://example.com">a test with a link</a>&#8221;&#8230; and ellipses.', gctexturize( 'Here is "<a href="http://example.com">a test with a link</a>"... and ellipses.' ) );
		// $this->assertSame( 'Here is &#8220;a test <a href="http://example.com">with a link</a>&#8221;.', gctexturize( 'Here is "a test <a href="http://example.com">with a link</a>".' ) );
		// $this->assertSame( 'Here is &#8220;<a href="http://example.com">a test with a link</a>&#8221;and a work stuck to the end.', gctexturize( 'Here is "<a href="http://example.com">a test with a link</a>"and a work stuck to the end.' ) );
		$this->assertSame( 'A test with a finishing number, &#8220;like 23&#8221;.', gctexturize( 'A test with a finishing number, "like 23".' ) );
		$this->assertSame( 'A test with a number, &#8220;like 62&#8221;, is nice to have.', gctexturize( 'A test with a number, "like 62", is nice to have.' ) );
	}
	...
	public function test_quotes_before_s() {
		...
		$this->assertSame( '&#8216;test&#8217;s', gctexturize( "'test's" ) );
		$this->assertSame( '&#8216;test&#8217;s&#8217;', gctexturize( "'test's'" ) );
		$this->assertSame( '&#8216;string&#8217;', gctexturize( "'string'" ) );
		$this->assertSame( '&#8216;string&#8217;s&#8217;', gctexturize( "'string's'" ) );
	}
	...
	public function test_quotes_before_numbers() {
		$this->assertSame( 'Class of &#8217;99', gctexturize( "Class of '99" ) );
		$this->assertSame( 'Class of &#8217;99&#8217;s', gctexturize( "Class of '99's" ) );
		$this->assertSame( '&#8216;Class of &#8217;99&#8217;', gctexturize( "'Class of '99'" ) );
		$this->assertSame( '&#8216;Class of &#8217;99&#8217; ', gctexturize( "'Class of '99' " ) );
		$this->assertSame( '&#8216;Class of &#8217;99&#8217;.', gctexturize( "'Class of '99'." ) );
		$this->assertSame( '&#8216;Class of &#8217;99&#8217;, she said', gctexturize( "'Class of '99', she said" ) );
		$this->assertSame( '&#8216;Class of &#8217;99&#8217;:', gctexturize( "'Class of '99':" ) );
		$this->assertSame( '&#8216;Class of &#8217;99&#8217;;', gctexturize( "'Class of '99';" ) );
		$this->assertSame( '&#8216;Class of &#8217;99&#8217;!', gctexturize( "'Class of '99'!" ) );
		$this->assertSame( '&#8216;Class of &#8217;99&#8217;?', gctexturize( "'Class of '99'?" ) );
	...
	public function test_other_html() {
		$this->assertSame( '&#8216;<strong>', gctexturize( "'<strong>" ) );
		// $this->assertSame( '&#8216;<strong>Quoted Text</strong>&#8217;,', gctexturize( "'<strong>Quoted Text</strong>'," ) );
		// $this->assertSame( '&#8220;<strong>Quoted Text</strong>&#8221;,', gctexturize( '"<strong>Quoted Text</strong>",' ) );
	}

	public function test_x() {
		$this->assertSame( '14&#215;24', gctexturize( '14x24' ) );
	}

	public function test_minutes_seconds() {
		$this->assertSame( '9&#8242;', gctexturize( '9\'' ) );
		$this->assertSame( '9&#8243;', gctexturize( '9"' ) );

		$this->assertSame( 'a 9&#8242; b', gctexturize( 'a 9\' b' ) );
		$this->assertSame( 'a 9&#8243; b', gctexturize( 'a 9" b' ) );
	 ...
	 * @ticket 8775
	 */
	public function test_gctexturize_quotes_around_numbers() {
		$this->assertSame( '&#8220;12345&#8221;', gctexturize( '"12345"' ) );
		$this->assertSame( '&#8216;12345&#8217;', gctexturize( '\'12345\'' ) );
		$this->assertSame( '&#8220;a 9&#8242; plus a &#8216;9&#8217;, maybe a 9&#8242; &#8216;9&#8217;&#8221;', gctexturize( '"a 9\' plus a \'9\', maybe a 9\' \'9\'"' ) );
		$this->assertSame( '<p>&#8217;99<br />&#8216;123&#8217;<br />&#8217;tis<br />&#8216;s&#8217;</p>', gctexturize( '<p>\'99<br />\'123\'<br />\'tis<br />\'s\'</p>' ) );
	}

	/**
	 * @ticket 8912
	 */
	public function test_gctexturize_html_comments() {
		$this->assertSame( '<!--[if !IE]>--><!--<![endif]-->', gctexturize( '<!--[if !IE]>--><!--<![endif]-->' ) );
		$this->assertSame( '<!--[if !IE]>"a 9\' plus a \'9\', maybe a 9\' \'9\' "<![endif]-->', gctexturize( '<!--[if !IE]>"a 9\' plus a \'9\', maybe a 9\' \'9\' "<![endif]-->' ) );
		$this->assertSame( '<ul><li>Hello.</li><!--<li>Goodbye.</li>--></ul>', gctexturize( '<ul><li>Hello.</li><!--<li>Goodbye.</li>--></ul>' ) );
	}
	...
	public function test_entity_quote_cuddling() {
		$this->assertSame( '&nbsp;&#8220;Testing&#8221;', gctexturize( '&nbsp;"Testing"' ) );
		// $this->assertSame( '&#38;&#8220;Testing&#8221;', gctexturize( '&#38;"Testing"' ) );
	}
	...
	public function test_apostrophes_before_primes() {
		$this->assertSame( 'GeChiUI 3.5&#8217;s release date', gctexturize( "GeChiUI 3.5's release date" ) );
	}
	...
	public function test_spaces_around_hyphens() {
		...
		$this->assertSame( ' &#8211; ', gctexturize( ' - ' ) );
		$this->assertSame( '&nbsp;&#8211;&nbsp;', gctexturize( '&nbsp;-&nbsp;' ) );
		$this->assertSame( ' &#8211;&nbsp;', gctexturize( ' -&nbsp;' ) );
		$this->assertSame( '&nbsp;&#8211; ', gctexturize( '&nbsp;- ' ) );
		$this->assertSame( "$nbsp&#8211;$nbsp", gctexturize( "$nbsp-$nbsp" ) );
		$this->assertSame( " &#8211;$nbsp", gctexturize( " -$nbsp" ) );
		$this->assertSame( "$nbsp&#8211; ", gctexturize( "$nbsp- " ) );
	...
	public function test_spaces_around_quotes_never() {
		...
		$this->assertNotEquals( $problem_output, gctexturize( $problem_input ) );
	}
	...
	public function test_spaces_around_quotes( $input, $output ) {
		return $this->assertSame( $output, gctexturize( $input ) );
	}
	 ...
	 *
	 * Checks all baseline patterns. If anything ever changes in gctexturize(), these tests may fail.
	 *
	 * @ticket 22692
	...
	public function data_tag_avoidance() {
		return array(
			...
			array(
				'word <img src="http://example.com/gc-content/uploads/2014/06/image-300x216.gif" /> word', // Ensure we are not corrupting image URLs.
				'word <img src="http://example.com/gc-content/uploads/2014/06/image-300x216.gif" /> word',
			),
			array(
	...
	/**
	 * Extra sanity checks for _gctexturize_pushpop_element()
	 *
	 * @ticket 28483
	...
	public function test_unregistered_shortcodes( $input, $output ) {
		...
		$output = $this->assertSame( $output, gctexturize( $input ) );
...
String with a number followed by a single quote !q1!Expendables 3!q1! vestibulum in arcu mi.',
	...
	public function test_pcre_performance( $input ) {
		...
		$regex  = _get_gctexturize_split_regex();
		$result = benchmark_pcre_backtracking( $regex, $input, 'split' );
		$this->assertLessThan( 200, $result );

		// With shortcodes enabled.
		$shortcode_regex = _get_gctexturize_shortcode_regex( array_keys( $shortcode_tags ) );
		$regex           = _get_gctexturize_split_regex( $shortcode_regex );
		$result          = benchmark_pcre_backtracking( $regex, $input, 'split' );
	...
	public function test_trailing_less_than() {
		$this->assertSame( 'F&#8211;oo<', gctexturize( 'F--oo<', true ) );
	}
