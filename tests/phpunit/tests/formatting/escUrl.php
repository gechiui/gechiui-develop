 ...
 * @group formatting
 */
class Tests_Formatting_EscUrl extends GC_UnitTestCase {

	/**
	 * @ticket 23605
	 */
	public function test_spaces() {
		$this->assertSame( 'http://example.com/Mr%20GeChiUI', esc_url( 'http://example.com/Mr GeChiUI' ) );
		$this->assertSame( 'http://example.com/Mr%20GeChiUI', esc_url( 'http://example.com/Mr%20GeChiUI' ) );
		$this->assertSame( 'http://example.com/Mr%20%20GeChiUI', esc_url( 'http://example.com/Mr%20%20GeChiUI' ) );
		$this->assertSame( 'http://example.com/Mr+GeChiUI', esc_url( 'http://example.com/Mr+GeChiUI' ) );
		$this->assertSame( 'http://example.com/Mr+GeChiUI', esc_url( ' http://example.com/Mr+GeChiUI' ) );
	...
	public function test_protocol() {
		...
		foreach ( gc_allowed_protocols() as $scheme ) {
		...
		$this->assertNotContains( 'data', gc_allowed_protocols() );
		$this->assertSame( '', esc_url( 'data:text/plain;base64,SGVsbG8sIFdvcmxkIQ%3D%3D' ) );

		$this->assertNotContains( 'foo', gc_allowed_protocols() );
		$this->assertSame(
			'foo://example.com',
			esc_url(
				'foo://example.com',
	...
	public function test_feed() {
		...
		$this->assertSame( 'feed:http://www.gechiui.com/feed/', esc_url( 'feed:http://www.gechiui.com/feed/' ) );
	}
