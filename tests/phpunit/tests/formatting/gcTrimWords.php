 ...
 * @group formatting
 */
class Tests_Formatting_gcTrimWords extends GC_UnitTestCase {
	...
	public function test_strips_script_and_style_content() {
		...
		$this->assertSame( $trimmed, gc_trim_words( $text ) );
	...
	public function test_doesnt_trim_short_text() {
		...
		$this->assertSame( $text, gc_trim_words( $text ) );
	}
	...
	public function test_trims_to_20_counted_by_chars() {
		...
		$actual   = gc_trim_words( $this->long_text, 20 );
	...
	public function test_works_with_non_numeric_num_words() {
		$this->assertSame( '', gc_trim_words( $this->long_text, '', '' ) );
		$this->assertSame( '', gc_trim_words( $this->long_text, 'abc', '' ) );
		$this->assertSame( '', gc_trim_words( $this->long_text, null, '' ) );
		$this->assertSame( 'Lorem ipsum dolor', gc_trim_words( $this->long_text, '3', '' ) );
	}
}
