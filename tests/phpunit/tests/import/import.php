 ...
 * @group import
 */
class Tests_Import_Import extends GC_Import_UnitTestCase {
	public function set_up() {
		...
		foreach ( array( 'posts', 'postmeta', 'comments', 'terms', 'term_taxonomy', 'term_relationships', 'users', 'usermeta' ) as $table ) {
			...
			$gcdb->query( "DELETE FROM {$gcdb->$table}" );
		}
	}
	...
	public function test_small_import() {
		...
		$this->_import_gc( DIR_TESTDATA . '/export/small-export.xml', $authors );
		...
		$this->assertEquals( 30, gc_count_terms( array( 'taxonomy' => 'category' ) ) );
		$this->assertEquals( 3, gc_count_terms( array( 'taxonomy' => 'post_tag' ) ) );
		...
		$post_count = gc_count_posts( 'post' );
		...
		$comment_count = gc_count_comments();
		...
		$cats = gc_get_post_categories( $post->ID );
		...
		$this->assertSame( 'publish', $post->post_status );
		$this->assertSame( 0, $post->post_parent );
		$cats = gc_get_post_categories( $post->ID, array( 'fields' => 'all' ) );
		$this->assertCount( 1, $cats );
		...
		$this->assertSame( 'publish', $post->post_status );
		$this->assertSame( 0, $post->post_parent );
		$this->assertSame( 'onecolumn-page.php', get_post_meta( $post->ID, '_gc_page_template', true ) );
		...
		$this->assertSame( 0, $post->post_parent );
		$this->assertSame( 'default', get_post_meta( $post->ID, '_gc_page_template', true ) );
	...
	public function test_ordering_of_importers() {
		global $gc_importers;
		$_gc_importers = $gc_importers; // Preserve global state.
		$gc_importers  = array(
			'xyz1' => array( 'xyz1' ),
			'XYZ2' => array( 'XYZ2' ),
			'abc2' => array( 'abc2' ),
		...
		$gc_importers = $_gc_importers; // Restore global state.
	}
	...
	public function test_slashes_should_not_be_stripped() {
		...
		$this->_import_gc( DIR_TESTDATA . '/export/slashes.xml', $authors );
