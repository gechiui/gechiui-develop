 ...
 * @group blocks
 */
class Tests_Blocks_Editor extends GC_UnitTestCase {
	...
	public function test_block_editor_context_no_settings() {
		$context = new GC_Block_Editor_Context();
	...
	public function test_block_editor_context_post() {
		$context = new GC_Block_Editor_Context( array( 'post' => get_post() ) );
	...
	public function test_get_block_categories_deprecated_filter_post_editor() {
		...
		$post_editor_context = new GC_Block_Editor_Context( array( 'post' => get_post() ) );
	...
	public function test_get_block_editor_settings_overrides_default_settings_all_editors() {
		...
		$my_editor_context = new GC_Block_Editor_Context();
	...
	public function test_block_editor_rest_api_preload_no_paths() {
		$editor_context = new GC_Block_Editor_Context();
		block_editor_rest_api_preload( array(), $editor_context );

		$after = implode( '', gc_scripts()->registered['gc-api-fetch']->extra['after'] );
		$this->assertStringNotContainsString( 'gc.apiFetch.createPreloadingMiddleware', $after );
	}
	...
	public function test_block_editor_rest_api_preload_filter_all() {
		function filter_add_preload_paths( $preload_paths, GC_Block_Editor_Context $context ) {
			if ( empty( $context->post ) ) {
				array_push( $preload_paths, array( '/gc/v2/types', 'OPTIONS' ) );
			}
		...
		$after = implode( '', gc_scripts()->registered['gc-api-fetch']->extra['after'] );
		$this->assertStringContainsString( 'gc.apiFetch.createPreloadingMiddleware', $after );
		$this->assertStringContainsString( '"\/gc\/v2\/blocks"', $after );
		$this->assertStringContainsString( '"\/gc\/v2\/types"', $after );
	}
	...
	public function test_block_editor_rest_api_preload_adds_missing_leading_slash( array $preload_paths, $expected ) {
		block_editor_rest_api_preload( $preload_paths, new GC_Block_Editor_Context() );
		$haystack = implode( '', gc_scripts()->registered['gc-api-fetch']->extra['after'] );
	...
	public function data_block_editor_rest_api_preload_adds_missing_leading_slash() {
		return array(
			'a string without a slash'               => array(
				'preload_paths' => array( 'gc/v2/blocks' ),
				'expected'      => '\/gc\/v2\/blocks',
			),
			...
			'an array with a string without a slash' => array(
				'preload_paths' => array( array( 'gc/v2/blocks', 'OPTIONS' ) ),
				'expected'      => '\/gc\/v2\/blocks',
			),
