 ...
 * @group blocks
 */
class Tests_Blocks_Render extends GC_UnitTestCase {
	/**
	...
	public function test_the_content() {
		...
		$classic_content = "Foo\n\n[someshortcode]\n\nBar\n\n[/someshortcode]\n\nBaz";
		$block_content   = "<!-- gc:core/paragraph -->\n<p>Foo</p>\n<!-- /gc:core/paragraph -->\n\n<!-- gc:core/shortcode -->[someshortcode]\n\nBar\n\n[/someshortcode]<!-- /gc:core/shortcode -->\n\n<!-- gc:core/paragraph -->\n<p>Baz</p>\n<!-- /gc:core/paragraph -->";
	...
	public function test_nested_calls_to_the_content() {
		...
		$the_content = apply_filters( 'the_content', '<!-- gc:core/test -->' . $content . '<!-- /gc:core/test -->' );
	...
	public function test_can_nest_at_least_so_deep() {
		...
		for ( $i = 0; $i < $minimum_depth; $i++ ) {
			$content = '<!-- gc:core/test -->' . $content . '<!-- /gc:core/test -->';
		}
	...
	public function test_blocks_arent_autopeed() {
		...
		$test_content     = "<!-- gc:fake/block -->\n$expected_content\n<!-- /gc:fake/block -->";

		$current_priority = has_action( 'the_content', 'gcautop' );

		$filtered_content = trim( apply_filters( 'the_content', $test_content ) );

		$this->assertSame( $expected_content, $filtered_content );

		// Check that gcautop() is still defined in the same place.
		$this->assertSame( $current_priority, has_action( 'the_content', 'gcautop' ) );
		// ... and that the restore function has removed itself.
		$this->assertFalse( has_action( 'the_content', '_restore_gcautop_hook' ) );
	...
	public function test_dynamic_block_rendering() {
		...
		$post_content =
			'before' .
			'<!-- gc:core/test {"value":"b1"} --><!-- /gc:core/test -->' .
			'<!-- gc:core/test {"value":"b1"} --><!-- /gc:core/test -->' .
			'between' .
			'<!-- gc:core/test {"value":"b2"} /-->' .
			'<!-- gc:core/test {"value":"b2"} /-->' .
			'after';
	...
	public function test_render_latest_comments_on_password_protected_post() {
		...
		$comment_text = gc_generate_password( 10, false );
		...
		$comments = do_blocks( '<!-- gc:latest-comments {"commentsToShow":1,"displayExcerpt":true} /-->' );
	...
	public function test_dynamic_block_renders_string() {
		...
		register_block_type( 'core/test', $settings );
		$block_type = new GC_Block_Type( 'core/test', $settings );
	...
	public function test_dynamic_block_gets_inner_html() {
		...
		$output = do_blocks( '<!-- gc:dynamic -->inner<!-- /gc:dynamic -->' );
	...
	public function test_dynamic_block_gets_rendered_inner_blocks() {
		...
		$output = do_blocks( '<!-- gc:dynamic -->before<!-- gc:test /-->after<!-- /gc:dynamic -->' );
	...
	public function test_dynamic_block_gets_rendered_inner_dynamic_blocks() {
		...
		$output = do_blocks( '<!-- gc:dynamic -->before<!-- gc:dynamic -->deep inner<!-- /gc:dynamic -->after<!-- /gc:dynamic -->' );
	...
	/**
	 * Test block rendering function, creating a new GC_Query instance.
	 *
	 * @since 5.0.0
	 *
	 * @return string Block output.
	 */
	public function render_test_block_gc_query() {
		$content = '';
		$recent  = new GC_Query(
			array(
				'numberposts'      => 10,
