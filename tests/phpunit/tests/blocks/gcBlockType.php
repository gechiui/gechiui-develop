...
/**
 * Tests for GC_Block_Type.
 *
 * @since 5.0.0
 *
 * @group blocks
 */
class Tests_Blocks_gcBlockType extends GC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$post_with_blocks = $factory->post->create(
			array(
				'post_title'   => 'Example',
				'post_content' => "<!-- gc:core/text {\"dropCap\":true} -->\n<p class=\"has-drop-cap\">Tester</p>\n<!-- /gc:core/text -->",
			)
		);
	...
	public function test_set_props() {
		...
		$block_type = new GC_Block_Type( $name, $args );
	...
	public function test_render_for_static_block() {
		$block_type = new GC_Block_Type( 'core/fake', array() );
	...
	public function test_prepare_attributes_none_defined() {
		...
		$block_type = new GC_Block_Type( 'core/dummy', array() );
	...
	public function test_has_block_with_mixed_content() {
		...
		'<!-- gc:core/fake --><!-- /gc:core/fake -->' .
		'<!-- gc:core/fake_atts {"value":"b1"} --><!-- /gc:core/fake_atts -->' .
		'<!-- gc:core/fake-child -->
		<p>testing the test</p>
		<!-- /gc:core/fake-child -->' .
		'between' .
		'<!-- gc:core/self-close-fake /-->' .
		'<!-- gc:custom/fake {"value":"b2"} /-->' .
		'after';
	...
	public function test_has_block_with_invalid_content() {
		...
		'<!- - gc:core/weird-space --><!-- /gc:core/weird-space -->' .
		'<!--gc:core/untrimmed-left --><!-- /gc:core/untrimmed -->' .
		'<!-- gc:core/fake --><!-- /gc:core/fake -->' .
		'<!-- gc:core/untrimmed-right--><!-- /gc:core/untrimmed2 -->' .
		'after';
	...
	public function test_post_has_block_serialized_name() {
		$content = '<!-- gc:serialized /--><!-- gc:core/normalized /--><!-- gc:plugin/third-party /-->';
	...
	public function data_block_version() {
		return array(
			...
			array( '<hr class="gc-block-separator" />', 0 ),
			// Post content with a block.
			array( '<!-- gc:core/separator -->', 1 ),
			// Post content with a fake block.
			array( '<!-- gc:core/fake --><!-- /gc:core/fake -->', 1 ),
			// Post content with an invalid block.
			array( '<!- - gc:core/separator -->', 0 ),
		);
	}
}
