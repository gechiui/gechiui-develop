 ...
 * @group blocks
 */
class Tests_Blocks_gcBlock extends GC_UnitTestCase {
	...
	public function set_up() {
		...
		$this->registry = new GC_Block_Type_Registry();
	}
	...
	public function test_constructor_assigns_properties_from_parsed_block() {
		...
		$parsed_blocks = parse_blocks( '<!-- gc:example {"ok":true} -->a<!-- gc:example /-->b<!-- /gc:example -->' );
		$parsed_block  = $parsed_blocks[0];
		$context       = array();
		$block         = new GC_Block( $parsed_block, $context, $this->registry );
	...
	public function test_constructor_assigns_block_type_from_registry() {
		...
		$this->assertInstanceOf( GC_Block_Type::class, $block->block_type );
		$this->assertSame(
	...
	public function test_constructor_maps_inner_blocks() {
		...
		$this->assertInstanceOf( GC_Block::class, $block->inner_blocks[0] );
	...
	public function test_constructor_prepares_context_for_inner_blocks() {
		...
		$parsed_blocks = parse_blocks( '<!-- gc:outer {"recordId":10} --><!-- gc:inner /--><!-- /gc:outer -->' );
		$parsed_block  = $parsed_blocks[0];
	...
	public function test_constructor_assigns_merged_context() {
		...
		$parsed_blocks = parse_blocks(
			'<!-- gc:example {"value":"merged"} -->' .
			'<!-- gc:example {"value":null} -->' .
			'<!-- gc:example /-->' .
			'<!-- /gc:example -->' .
			'<!-- /gc:example -->'
		);
	...
	public function test_render_static_block_type_returns_own_content() {
		...
		$parsed_blocks = parse_blocks( '<!-- gc:static -->a<!-- gc:dynamic /-->c<!-- /gc:static -->' );
		$parsed_block  = $parsed_blocks[0];
		$context       = array();
		$block         = new GC_Block( $parsed_block, $context, $this->registry );
	...
	public function test_render_passes_block_for_render_callback() {
		...
		$parsed_blocks = parse_blocks( '<!-- gc:greeting /-->' );
		$parsed_block  = $parsed_blocks[0];
		$context       = array();
		$block         = new GC_Block( $parsed_block, $context, $this->registry );
	...
	public function test_render_applies_render_block_filter() {
		...
		$parsed_blocks = parse_blocks( '<!-- gc:example -->Static<!-- gc:example -->Inner<!-- /gc:example --><!-- /gc:example -->' );
		$parsed_block  = $parsed_blocks[0];
		$context       = array();
		$block         = new GC_Block( $parsed_block, $context, $this->registry );
	...
	public function test_passes_attributes_to_render_callback() {
		...
		$parsed_blocks = parse_blocks( '<!-- gc:greeting {"toWhom":"world"} /-->' );
		$parsed_block  = $parsed_blocks[0];
		$context       = array();
		$block         = new GC_Block( $parsed_block, $context, $this->registry );
	...
	public function test_passes_content_to_render_callback() {
		...
		$parsed_blocks = parse_blocks( '<!-- gc:outer -->a<!-- gc:inner /-->c<!-- /gc:outer -->' );
		$parsed_block  = $parsed_blocks[0];
		$context       = array();
		$block         = new GC_Block( $parsed_block, $context, $this->registry );
	...
	public function test_build_query_vars_from_query_block_no_context() {
		...
		$parsed_blocks    = parse_blocks( '<!-- gc:example {"ok":true} -->a<!-- gc:example /-->b<!-- /gc:example -->' );
		$parsed_block     = $parsed_blocks[0];
		$block_no_context = new GC_Block( $parsed_block, array(), $this->registry );
