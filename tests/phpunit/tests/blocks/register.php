 ...
 * @group blocks
 */
class Tests_Blocks_Register extends GC_UnitTestCase {
	...
	public function test_success_register_block_style_handle() {
		...
		$this->assertSame( 'replace', gc_styles()->get_data( 'unit-tests-test-block-style', 'rtl' ) );

		// @ticket 50328
		$this->assertSame(
			gc_normalize_path( realpath( DIR_TESTDATA . '/blocks/notice/block.css' ) ),
			gc_normalize_path( gc_styles()->get_data( 'unit-tests-test-block-style', 'path' ) )
		);
	}
	 ...
	 * @ticket 50263
	 */
	public function test_metadata_not_found_in_gechiui_core() {
	...
	public function test_block_registers_with_metadata_fixture() {
		...
		$this->assertInstanceOf( 'GC_Block_Type', $result );
	...
	public function test_block_registers_with_metadata_i18n_support() {
		...
		load_textdomain( 'notice', GC_LANG_DIR . '/plugins/notice-pl_PL.mo' );
	...
	public function test_has_blocks() {
		...
		// Test with passing GC_Post object.
		...
		$query = new GC_Query( array( 'post__in' => array( self::$post_id ) ) );
		$query->the_post();
	 ...
	 *
	 * @expectedIncorrectUsage GC_Block_Styles_Registry::register
