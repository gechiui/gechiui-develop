 ...
 * @group hooks
 */
class Tests_Actions extends GC_UnitTestCase {
	 ...
	 *
	 * @see https://core.trac.gechiui.com/ticket/17817#comment:72
	 * @ticket 17817
	 *
	 * @covers ::do_action
	 */
	 ...
	 * https://core.trac.gechiui.com/ticket/17817#comment:52
	 *
	...
	public function test_remove_anonymous_callback() {
		...
		$hook = $GLOBALS['gc_filter'][ $tag ];

		// From http://gechiui.stackexchange.com/a/57088/6445
	...
	/**
	 * Test the ArrayAccess methods of GC_Hook
	 *
	 * @ticket 17817
	 *
	 * @covers GC_Hook::offsetGet
	 * @covers GC_Hook::offsetSet
	 * @covers GC_Hook::offsetUnset
	 */
	public function test_array_access_of_gc_filter_global() {
		global $gc_filter;
		$tag = __FUNCTION__;

		add_action( $tag, '__return_null', 11, 1 );

		$this->assertArrayHasKey( 11, $gc_filter[ $tag ] );
		$this->assertArrayHasKey( '__return_null', $gc_filter[ $tag ][11] );

		unset( $gc_filter[ $tag ][11] );
		$this->assertFalse( has_action( $tag, '__return_null' ) );

		$gc_filter[ $tag ][11] = array(
			'__return_null' => array(
	...
	public function test_current_action() {
		...
		$gc_current_filter[] = 'first';
		$gc_current_filter[] = 'second'; // Let's say a second action was invoked.
	...
	public function test_doing_filter() {
		global $gc_current_filter;
		$gc_current_filter = array(); // Set to an empty array first.
	...
	public function test_do_action_deprecated() {
		$p = new GC_Post( (object) array( 'post_title' => 'Foo' ) );
	...
	public function test_do_action_deprecated_with_multiple_params() {
		$p1 = new GC_Post( (object) array( 'post_title' => 'Foo1' ) );
		$p2 = new GC_Post( (object) array( 'post_title' => 'Foo2' ) );
