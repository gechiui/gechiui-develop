<?php
/**
 * Unit tests covering GC_Widget_Text functionality.
 *
 * @package    GeChiUI
 * @subpackage widgets
 */
...
/**
 * Test gc-includes/widgets/class-gc-widget-text.php
 *
 * @group widgets
 */
class Tests_Widgets_gcWidgetText extends GC_UnitTestCase {
	/**
	 ...
	 * @global GC_Styles            $gc_style
	 * @global GC_Customize_Manager $gc_customize
	 */
	public function clean_up_global_scope() {
		global $gc_scripts, $gc_styles, $gc_customize;
	 ...
	 *
	 * @covers GC_Widget_Text::__construct
	 */
	 ...
	 *
	 * @covers GC_Widget_Text::_register
	 */
	public function test__register() {
		...
		$this->assertSame( 10, has_action( 'admin_footer-widgets.php', array( 'GC_Widget_Text', 'render_control_template_scripts' ) ) );
		$this->assertContains( 'gc.textWidgets.idBases.push( "text" );', gc_scripts()->registered['text-widgets']->extra['after'] );
		$this->assertFalse( has_action( 'gc_enqueue_scripts', array( $widget, 'enqueue_preview_scripts' ) ) );
	}
	...
	public function test__register_in_customize_preview() {
		...
		$widget = new GC_Widget_Text();
		$widget->_register();
		$this->assertSame( 10, has_action( 'gc_enqueue_scripts', array( $widget, 'enqueue_preview_scripts' ) ) );
	}
	 ...
	 * @covers GC_Widget_Text::enqueue_preview_scripts
	 */
	public function test_enqueue_preview_scripts() {
		...
		$this->assertFalse( gc_script_is( 'gc-playlist' ) );
		...
		$this->assertTrue( gc_script_is( 'gc-playlist' ) );
	}

	/**
	 * Test widget method.
	 *
	 * @covers GC_Widget_Text::widget
	 */
	public function test_widget() {
		...
		$this->assertStringContainsString( gcautop( $expected_instance['text'] . '[filter:widget_text][filter:widget_text_content]' ), $output );
		...
		$this->assertStringContainsString( gcautop( $expected_instance['text'] . '[filter:widget_text]' ), $output );
	...
	/**
	 * Example shortcode content to test for gcautop corruption.
	 *
	 * @var string
	 */
	...
	public function test_widget_shortcodes() {
		...
		// Legacy Text Widget without gcautop().
		...
		$this->assertStringContainsString( $this->example_shortcode_content, $output, 'Shortcode was applied without gcautop corrupting it.' );
		...
		// Legacy Text Widget with gcautop().
		$instance                     = array_merge(
			$base_instance,
			array(
				'filter' => true,
		...
		$this->assertSame( 1, $this->shortcode_render_count );
		$this->assertStringNotContainsString( '[example]', $output, 'Expected shortcode to be processed in legacy widget with plugin adding filter' );
		$this->assertStringContainsString( gcautop( $this->example_shortcode_content ), $output, 'Shortcode was applied *with* gcautop() applying to shortcode output since plugin used legacy filter.' );
		$this->assertNull( $this->post_during_shortcode );
	 ...
	 *
	 * @covers GC_Widget_Text::is_legacy_instance
	 */
	public function test_is_legacy_instance() {
		...
		$this->assertFalse( $widget->is_legacy_instance( $instance ), 'Not legacy when filter is explicitly content (in GC 4.8.0 only).' );
		...
		$this->assertTrue( $widget->is_legacy_instance( $instance ), 'Legacy when not-gcautop and there are line breaks.' );
		...
		$this->assertTrue( $widget->is_legacy_instance( $instance ), 'Legacy when not-gcautop and there are paragraph breaks.' );
		...
		$this->assertFalse( $widget->is_legacy_instance( $instance ), 'Not automatically legacy when gcautop and there are line breaks.' );
		...
		$this->assertFalse( $widget->is_legacy_instance( $instance ), 'Not automatically legacy when gcautop and there are paragraph breaks.' );
		...
		foreach ( $legacy_text_examples as $legacy_text_example ) {
			...
			$this->assertTrue( $widget->is_legacy_instance( $instance ), 'Legacy when gcautop and there is HTML that is not liable to be mutated.' );
			...
			$this->assertTrue( $widget->is_legacy_instance( $instance ), 'Legacy when not-gcautop and there is HTML that is not liable to be mutated.' );
		}
		...
		foreach ( $migratable_text_examples as $migratable_text_example ) {
			...
			$this->assertFalse( $widget->is_legacy_instance( $instance ), 'Legacy when gcautop and there is HTML that is not liable to be mutated.' );
		}
	}
	...
	public function test_update() {
		...
		$instance['text'] = '<script>alert( "Howdy!" );</script>';
		$expected['text'] = gc_kses_post( $instance['text'] );
		$result           = $widget->update( $instance, array() );
	...
	public function test_update_legacy() {
		...
		$this->assertSameSets( $expected, $result, 'Updating a widget that had \'content\' as its filter value persists non-legacy mode. This only existed in GC 4.8.0.' );
		...
		$this->assertSameSets( $expected, $result, 'Updating a pre-existing widget with the filter=content prop in GC 4.8.0 upgrades to filter=true&visual=true.' );
		...
		$this->assertSameSets( $expected, $result, 'Updating a widget with filter=content (from GC 4.8.0) upgrades to filter=true&visual=true.' );
	}
	 ...
	 * Test enqueue_admin_scripts method.
	 *
	 * @covers GC_Widget_Text::enqueue_admin_scripts
	 */
	public function test_enqueue_admin_scripts() {
		...
		$this->assertTrue( gc_script_is( 'text-widgets' ) );
	}

	/**
	 * Test render_control_template_scripts method.
	 *
	 * @covers GC_Widget_Text::render_control_template_scripts
	 */
	public function test_render_control_template_scripts() {
		ob_start();
		GC_Widget_Text::render_control_template_scripts();
		$output = ob_get_clean();
