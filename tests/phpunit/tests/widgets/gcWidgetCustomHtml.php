<?php
/**
 * Unit tests covering GC_Widget_Custom_HTML functionality.
 *
 * @package    GeChiUI
 * @subpackage widgets
 */
...
/**
 * Test gc-includes/widgets/class-gc-widget-custom-html.php
 *
 * @group widgets
 */
class Tests_Widgets_gcWidgetCustomHtml extends GC_UnitTestCase {
	 ...
	 * Test construct.
	 *
	 * @covers GC_Widget_Custom_HTML::__construct
	 */
	public function test_construct() {
		$widget = new GC_Widget_Custom_HTML();
	 ...
	 *
	 * @covers GC_Widget_Custom_HTML::_register
	 */
	public function test__register() {
		...
		$this->assertSame( 10, has_action( 'admin_footer-widgets.php', array( 'GC_Widget_Custom_HTML', 'render_control_template_scripts' ) ) );
		$this->assertSame( 10, has_action( 'admin_head-widgets.php', array( 'GC_Widget_Custom_HTML', 'add_help_text' ) ) );
		$this->assertContains( 'gc.customHtmlWidgets.idBases.push( "custom_html" );', gc_scripts()->registered['custom-html-widgets']->extra['after'] );
	}

	/**
	 * Test widget method.
	 *
	 * @covers GC_Widget_Custom_HTML::widget
	 */
	 ...
	 * @param GC_Widget_Custom_HTML $widget   Current widget instance.
	 ...
	 * Test update method.
	 *
	 * @covers GC_Widget_Custom_HTML::update
	 */
	public function test_update() {
		...
		$instance['content'] = '<script>alert( "Howdy!" );</script>';
		$expected['content'] = gc_kses_post( $instance['content'] );
		$result              = $widget->update( $instance, array() );
		$this->assertSame( $expected, $result );
	 ...
	 *
	 * @covers GC_Widget_Custom_HTML::enqueue_admin_scripts
	 */
	public function test_enqueue_admin_scripts_when_logged_in_and_syntax_highlighting_on() {
		$user = $this->factory()->user->create();
		gc_set_current_user( $user );
		gc_get_current_user()->syntax_highlighting = 'true';
		set_current_screen( 'widgets.php' );
		$widget = new GC_Widget_Custom_HTML();
		$widget->enqueue_admin_scripts();

		$this->assertTrue( gc_script_is( 'custom-html-widgets', 'enqueued' ) );
		$this->assertTrue( gc_script_is( 'code-editor', 'enqueued' ) );
		$this->assertTrue( gc_script_is( 'gc-codemirror', 'enqueued' ) );
		$this->assertTrue( gc_script_is( 'csslint', 'enqueued' ) );
		$this->assertTrue( gc_script_is( 'jshint', 'enqueued' ) );
		$this->assertTrue( gc_script_is( 'htmlhint', 'enqueued' ) );
	}
	...
	public function test_enqueue_admin_scripts_when_logged_in_and_syntax_highlighting_off() {
		...
		$this->assertTrue( gc_script_is( 'custom-html-widgets', 'enqueued' ) );
		$this->assertFalse( gc_script_is( 'code-editor', 'enqueued' ) );
		$this->assertFalse( gc_script_is( 'gc-codemirror', 'enqueued' ) );
		$this->assertFalse( gc_script_is( 'csslint', 'enqueued' ) );
		$this->assertFalse( gc_script_is( 'jshint', 'enqueued' ) );
		$this->assertFalse( gc_script_is( 'htmlhint', 'enqueued' ) );
	}

	/**
	 * Test render_control_template_scripts method.
	 *
	 * @covers GC_Widget_Custom_HTML::render_control_template_scripts
	 */
	public function test_render_control_template_scripts() {
		ob_start();
		GC_Widget_Custom_HTML::render_control_template_scripts();
		$output = ob_get_clean();
	 ...
	 *
	 * @covers GC_Widget_Custom_HTML::add_help_text
	 */
	public function test_add_help_text() {
		set_current_screen( 'widgets.php' );
		GC_Widget_Custom_HTML::add_help_text();
