 ...
 * inclusion of PEAR.
 *
 * The GC_HTTP tests require a class-http.php file of r17550 or later.
 */
abstract class GC_HTTP_UnitTestCase extends GC_UnitTestCase {
	// You can use your own version of data/GCHTTP-testcase-redirection-script.php here.
	public $redirection_script = 'http://api.gechiui.com/core/tests/1.0/redirection.php';
	...
	public function set_up() {
		...
		$class = 'GC_Http_' . ucfirst( $this->transport );
	...
	/**
	 * @covers ::gc_remote_request
	 */
	public function test_redirect_on_301() {
		// 5 : 5 & 301.
		$res = gc_remote_request( $this->redirection_script . '?code=301&rt=' . 5, array( 'redirection' => 5 ) );
	...
	public function test_redirect_on_302() {
		// 5 : 5 & 302.
		$res = gc_remote_request( $this->redirection_script . '?code=302&rt=' . 5, array( 'redirection' => 5 ) );
	...
	public function test_redirect_on_301_no_redirect() {
		// 5 > 0 & 301.
		$res = gc_remote_request( $this->redirection_script . '?code=301&rt=' . 5, array( 'redirection' => 0 ) );
	...
	public function test_redirect_on_302_no_redirect() {
		// 5 > 0 & 302.
		$res = gc_remote_request( $this->redirection_script . '?code=302&rt=' . 5, array( 'redirection' => 0 ) );
	...
	public function test_redirections_equal() {
		// 5 - 5.
		$res = gc_remote_request( $this->redirection_script . '?rt=' . 5, array( 'redirection' => 5 ) );
	...
	public function test_no_head_redirections() {
		...
		$res = gc_remote_request( $this->redirection_script . '?code=302&rt=' . 1, array( 'method' => 'HEAD' ) );
	...
	public function test_redirections_greater() {
		// 10 > 5.
		$res = gc_remote_request( $this->redirection_script . '?rt=' . 10, array( 'redirection' => 5 ) );
	...
	public function test_redirections_greater_edgecase() {
		// 6 > 5 (close edge case).
		$res = gc_remote_request( $this->redirection_script . '?rt=' . 6, array( 'redirection' => 5 ) );
	...
	public function test_redirections_less_edgecase() {
		// 4 < 5 (close edge case).
		$res = gc_remote_request( $this->redirection_script . '?rt=' . 4, array( 'redirection' => 5 ) );
	...
	public function test_location_header_on_201() {
		...
		$res = gc_remote_request( $this->redirection_script . '?201-location=true' );
	 ...
	 *
	 * @covers ::gc_remote_request
	 * @covers ::gc_remote_retrieve_body
	 */
	public function test_no_redirection_on_PUT() {
		$url = 'http://api.gechiui.com/core/tests/1.0/redirection.php?201-location=1';
		...
		$this->skipTestOnTimeout( $res );
		$this->assertSame( 'PASS', gc_remote_retrieve_body( $res ) );
	...
	public function test_send_headers() {
		...
		$res     = gc_remote_request( $this->redirection_script . '?header-check', array( 'headers' => $headers ) );
	 ...
	 * @ticket 17588
	 *
	 * @covers ::gc_remote_post
	 * @covers ::gc_remote_retrieve_body
	 */
	public function test_post_redirect_to_method_300( $response_code, $method ) {
		$url = 'http://api.gechiui.com/core/tests/1.0/redirection.php?post-redirect-to-method=1';

		$res = gc_remote_post( add_query_arg( 'response_code', $response_code, $url ), array( 'timeout' => 30 ) );

		$this->skipTestOnTimeout( $res );
		$this->assertSame( $method, gc_remote_retrieve_body( $res ) );
	}
	...
	public function test_ip_url_with_host_header() {
		$ip   = gethostbyname( 'api.gechiui.com' );
		$url  = 'http://' . $ip . '/core/tests/1.0/redirection.php?print-pass=1';
		$args = array(
			'headers'     => array(
				'Host' => 'api.gechiui.com',
			),
			'timeout'     => 30,
			'redirection' => 0,
		);

		$res = gc_remote_get( $url, $args );
	 ...
	 *
	 * @covers ::gc_remote_head
	 * @covers ::gc_remote_retrieve_header
	 * @covers ::gc_remote_get
	 * @covers ::gc_remote_retrieve_body
	 */
	public function test_multiple_location_headers() {
		$url = 'http://api.gechiui.com/core/tests/1.0/redirection.php?multiple-location-headers=1';
		$res = gc_remote_head( $url, array( 'timeout' => 30 ) );

		$this->skipTestOnTimeout( $res );
		$this->assertIsArray( gc_remote_retrieve_header( $res, 'location' ) );
		$this->assertCount( 2, gc_remote_retrieve_header( $res, 'location' ) );

		$res = gc_remote_get( $url, array( 'timeout' => 30 ) );
	...
	public function test_cookie_handling() {
		$url = 'http://api.gechiui.com/core/tests/1.0/redirection.php?cookie-test=1';

		$res = gc_remote_get( $url );
	...
	public function test_ssl() {
		...
		$res = gc_remote_get( 'https://www.gechiui.com/' );
