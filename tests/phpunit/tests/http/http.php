<?php
/**
 * Non-transport-specific GC_HTTP Tests
 *
 * @group http
 */
class Tests_HTTP_HTTP extends GC_UnitTestCase {
	 ...
	 *
	 * @covers GC_Http::make_absolute_url
	 */
	public function test_make_absolute_url( $relative_url, $absolute_url, $expected ) {
		$actual = GC_Http::make_absolute_url( $relative_url, $absolute_url );
	 ...
	 * @covers ::gc_parse_url
	 */
	public function test_gc_parse_url( $url, $expected ) {
		$actual = gc_parse_url( $url );
		$this->assertSame( $expected, $actual );
	}
	 ...
	 * @covers ::gc_parse_url
	 */
	public function test_gc_parse_url_with_default_component() {
		$actual = gc_parse_url( self::FULL_TEST_URL, -1 );
		$this->assertSame(
			array(
				'scheme'   => 'http',
				'host'     => 'host.name',
	 ...
	 * @covers ::gc_parse_url
	 */
	public function test_gc_parse_url_with_component( $url, $component, $expected ) {
		$actual = gc_parse_url( $url, $component );
		$this->assertSame( $expected, $actual );
	}
	...
	public function test_http_response_code_constants() {
		global $gc_header_to_desc;

		$ref       = new ReflectionClass( 'GC_Http' );
		$constants = $ref->getConstants();

		// This primes the `$gc_header_to_desc` global:
		get_status_header_desc( 200 );

		$this->assertSame( array_keys( $gc_header_to_desc ), array_values( $constants ) );
	 ...
	 * @ticket 37768
	 *
	 * @covers GC_Http::normalize_cookies
	 */
	 ...
	 * @ticket 36356
	 *
	 * @dataProvider gc_translate_php_url_constant_to_key_testcases
	 *
	 * @covers ::_gc_translate_php_url_constant_to_key
	 */
	public function test_gc_translate_php_url_constant_to_key( $input, $expected ) {
		$actual = _gc_translate_php_url_constant_to_key( $input );
		$this->assertSame( $expected, $actual );
	}

	public function gc_translate_php_url_constant_to_key_testcases() {
	...
	/**
	 * Test that gc_http_validate_url validates URLs.
	 *
	 * @ticket 54331
	 *
	 * @dataProvider data_gc_http_validate_url_should_validate
	 *
	 * @covers ::gc_http_validate_url
	 *
	 ...
	 *                                     Default false.
	 */
	public function test_gc_http_validate_url_should_validate( $url, $cb_safe_ports = false, $external_host = false ) {
		if ( $external_host ) {
		...
		$this->assertSame( $url, gc_http_validate_url( $url ) );
	}
	 ...
	 * @return array
	 */
	public function data_gc_http_validate_url_should_validate() {
		return array(
	...
	/**
	 * Tests that gc_http_validate_url validates a url that uses an unsafe port
	 ...
	 * @covers ::gc_http_validate_url
	 */
	public function test_gc_http_validate_url_should_validate_with_an_unsafe_port_when_the_host_and_port_match_the_home_url() {
	...
	/**
	 * Test that gc_http_validate_url does not validate invalid URLs.
	 *
	 * @ticket 54331
	 *
	 * @dataProvider data_gc_http_validate_url_should_not_validate
	 *
	 * @covers ::gc_http_validate_url
	 *
	 * @param string       $url            The URL to validate.
	 ...
	 *                                     Default false.
	 */
	public function test_gc_http_validate_url_should_not_validate( $url, $cb_safe_ports = false, $external_host = false ) {
		if ( $external_host ) {
			add_filter( 'http_request_host_is_external', '__return_true' );
		}
		...
		$this->assertFalse( gc_http_validate_url( $url ) );
	}
	 ...
	 * @return array
	 */
	public function data_gc_http_validate_url_should_not_validate() {
		return array(
			'url as false'                                 => array(
				'url' => false,
			),
