...
class Tests_Query extends GC_UnitTestCase {
	...
	public function test_nested_loop_reset_postdata() {
		...
		$first_query = new GC_Query( array( 'post__in' => array( $post_id ) ) );
		while ( $first_query->have_posts() ) {
			$first_query->the_post();
			$second_query = new GC_Query( array( 'post__in' => array( $nested_post_id ) ) );
	...
	public function test_get_queried_object_should_return_null_when_is_tax_is_true_but_the_taxonomy_args_have_been_removed_in_a_parse_query_callback() {
		...
		gc_set_object_terms( $posts[0], array( $terms[0] ), 'gctests_tax' );
		gc_set_object_terms( $posts[1], array( $terms[1] ), 'gctests_tax' );
	...
	public function filter_parse_query_to_remove_tax( $q ) {
		unset( $q->query_vars['gctests_tax'] );
	}
	...
	public function test_orderby_space_separated() {
		...
		$this->assertStringContainsString( "ORDER BY $gcdb->posts.post_title DESC, $gcdb->posts.post_date DESC", $q->request );
	}
	...
	public function test_cat_querystring_single_term() {
		...
		gc_set_object_terms( $p1, $c1, 'category' );
		gc_set_object_terms( $p2, array( $c1, $c2 ), 'category' );
		gc_set_object_terms( $p3, $c2, 'category' );
		...
		$matching_posts = gc_list_pluck( $GLOBALS['gc_query']->posts, 'ID' );
	...
	public function test_category_querystring_multiple_terms_comma_separated() {
		...
		gc_set_object_terms( $p2, array( $c1, $c2 ), 'category' );
		gc_set_object_terms( $p3, $c2, 'category' );
		gc_set_object_terms( $p4, $c3, 'category' );
	...
	public function test_tag_querystring_single_term() {
		...
		gc_set_object_terms( $p1, $t1->slug, 'post_tag' );
		gc_set_object_terms( $p2, array( $t1->slug, $t2->slug ), 'post_tag' );
		gc_set_object_terms( $p3, $t2->slug, 'post_tag' );
	...
	public function test_tag_querystring_multiple_terms_comma_separated() {
		...
		gc_set_object_terms( $p1, $c1->slug, 'post_tag' );
		gc_set_object_terms( $p2, array( $c1->slug, $c2->slug ), 'post_tag' );
		gc_set_object_terms( $p3, $c2->slug, 'post_tag' );
		gc_set_object_terms( $p4, $c3->slug, 'post_tag' );
	...
	public function test_custom_taxonomy_querystring_single_term() {
		...
		gc_insert_term( 'test1', 'test_tax_cat' );
		gc_insert_term( 'test2', 'test_tax_cat' );
		gc_insert_term( 'test3', 'test_tax_cat' );

		$p1 = self::factory()->post->create();
		$p2 = self::factory()->post->create();
		$p3 = self::factory()->post->create();

		gc_set_object_terms( $p1, 'test1', 'test_tax_cat' );
		gc_set_object_terms( $p2, array( 'test1', 'test2' ), 'test_tax_cat' );
		gc_set_object_terms( $p3, 'test2', 'test_tax_cat' );
		...
		$this->assertSameSets( array( $p1, $p2 ), gc_list_pluck( $GLOBALS['gc_query']->posts, 'ID' ) );
	}
	...
	public function test_custom_taxonomy_querystring_multiple_terms_comma_separated() {
		...
		gc_set_object_terms( $p2, array( 'test1', 'test2' ), 'test_tax_cat' );
		gc_set_object_terms( $p3, 'test2', 'test_tax_cat' );
		gc_set_object_terms( $p4, 'test3', 'test_tax_cat' );
		...
		$this->assertSameSets( array( $p1, $p2, $p3 ), gc_list_pluck( $GLOBALS['gc_query']->posts, 'ID' ) );
	}
	...
	public function test_pages_dont_404_when_queried_post_id_is_modified() {
		...
		$this->assertFalse( $GLOBALS['gc_query']->is_404() );
		$this->assertSame( $post_id, $GLOBALS['gc_query']->post->ID );
	}
	...
	public function test_get_queried_object_should_return_first_of_multiple_terms() {
		...
		gc_set_object_terms( $post_id, 'term1', 'tax1' );
		gc_set_object_terms( $post_id, 'term2', 'tax2' );
