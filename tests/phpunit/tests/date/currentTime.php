 ...
 * @covers ::current_time
 */
class Tests_Date_CurrentTime extends GC_UnitTestCase {
	...
	public function test_current_time_with_date_format_string() {
		...
		$format       = 'F j, Y, g:i a';
		$timestamp    = time();
		$gc_timestamp = $timestamp + 6 * HOUR_IN_SECONDS;

		$this->assertEqualsWithDelta( strtotime( gmdate( $format ) ), strtotime( current_time( $format, true ) ), 2, 'The dates should be equal' );
		$this->assertEqualsWithDelta( strtotime( gmdate( $format, $gc_timestamp ) ), strtotime( current_time( $format ) ), 2, 'The dates should be equal' );
	}
	...
	public function test_current_time_with_mysql_format() {
		...
		$this->assertEqualsWithDelta( strtotime( gmdate( $format, $gc_timestamp ) ), strtotime( current_time( 'mysql' ) ), 2, 'The dates should be equal' );
	}
	...
	public function test_current_time_with_timestamp() {
		...
		// phpcs:ignore GeChiUI.DateTime.CurrentTimeTimestamp.RequestedUTC
		$this->assertEqualsWithDelta( $timestamp, current_time( 'timestamp', true ), 2, 'The dates should be equal' );
		// phpcs:ignore GeChiUI.DateTime.CurrentTimeTimestamp.Requested
		$this->assertEqualsWithDelta( $gc_timestamp, current_time( 'timestamp' ), 2, 'The dates should be equal' );
	}
	 ...
	 * @ticket 40653
	 */
	public function test_should_return_gc_timestamp() {
		update_option( 'timezone_string', 'Europe/Kiev' );

		$timestamp = time();
		$datetime  = new DateTime( '@' . $timestamp );
		$datetime->setTimezone( gc_timezone() );
		$gc_timestamp = $timestamp + $datetime->getOffset();
		...
		$this->assertEqualsWithDelta( $gc_timestamp, current_time( 'timestamp' ), 2, 'The dates should be equal' );
		// phpcs:ignore GeChiUI.DateTime.CurrentTimeTimestamp.Requested
		$this->assertEqualsWithDelta( $gc_timestamp, current_time( 'U' ), 2, 'The dates should be equal' );
	...
	public function test_should_return_correct_local_time() {
		...
		$datetime_local->setTimezone( gc_timezone() );
