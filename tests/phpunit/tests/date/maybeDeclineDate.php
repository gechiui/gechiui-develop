 ...
 * @group i18n
 * @group datetime
 * @covers ::gc_maybe_decline_date
 */
class Tests_Date_MaybeDeclineDate extends GC_UnitTestCase {
	 ...
	 * @ticket 48606
	 * @ticket 48934
	 * @dataProvider data_gc_maybe_decline_date
	 */
	public function test_gc_maybe_decline_date( $test_locale, $format, $input, $output ) {
		global $locale, $gc_locale;
		...
		$locale                    = $test_locale;
		$gc_locale->month          = $month_names['month'];
		$gc_locale->month_genitive = $month_names['month_genitive'];

		$declined_date = gc_maybe_decline_date( $input, $format );
	...
	public function data_gc_maybe_decline_date() {
		return array(
