 ...
 * @covers ::date_i18n
 */
class Tests_Date_DateI18n extends GC_UnitTestCase {
	...
	public function test_should_return_current_time_on_invalid_timestamp() {
		...
		$gc_timestamp = $datetime->getTimestamp() + $datetime->getOffset();

		$this->assertEqualsWithDelta( $gc_timestamp, date_i18n( 'U', 'invalid' ), 5, 'The dates should be equal' );
	}
	...
	public function test_adjusts_format_based_on_locale() {
		$original_locale = $GLOBALS['gc_locale'];
		/* @var GC_Locale $locale */
		$locale = clone $GLOBALS['gc_locale'];
		...
		$GLOBALS['gc_locale'] = $original_locale;
	...
	public function test_should_return_gc_timestamp() {
		...
		$datetime     = new DateTimeImmutable( 'now', gc_timezone() );
		$timestamp    = $datetime->getTimestamp();
		$gc_timestamp = $timestamp + $datetime->getOffset();

		$this->assertEqualsWithDelta( $gc_timestamp, date_i18n( 'U' ), 2, 'The dates should be equal' );
		$this->assertEqualsWithDelta( $timestamp, date_i18n( 'U', false, true ), 2, 'The dates should be equal' );
		$this->assertSame( $gc_timestamp, date_i18n( 'U', $gc_timestamp ) );
	}

	/**
	 * @ticket 43530
	 */
	public function test_swatch_internet_time_with_gc_timestamp() {
		update_option( 'timezone_string', 'America/Regina' );
	...
	public function test_should_handle_dst( $time, $timezone ) {
		...
		$gc_timestamp = strtotime( $time );
		$format       = 'I ' . DATE_RFC3339;

		$this->assertSame( $datetime->format( $format ), date_i18n( $format, $gc_timestamp ) );
	}
