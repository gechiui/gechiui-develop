...
/**
 * Tests for public methods of GC_Date_Query.
 *
 * See query/dateQuery.php for tests that require GC_Query.
 *
 * @group datequery
 * @group date
 * @covers GC_Date_Query
 */
class Tests_Date_Query extends GC_UnitTestCase {
	/**
	 * @var GC_Date_Query $q
	 */
	public $q;
	...
	public function set_up() {
		parent::set_up();
		unset( $this->q );
		$this->q = new GC_Date_Query( array( 'm' => 2 ) );
	}

	public function test_construct_date_query_empty() {
		$q = new GC_Date_Query( array() );
	...
	public function test_validate_column_post_date() {
		...
		$this->assertSame( $gcdb->posts . '.post_date', $q->validate_column( 'post_date' ) );
	}
	...
	public function test_validate_column_post_date_gmt() {
		...
		$this->assertSame( $gcdb->posts . '.post_date_gmt', $q->validate_column( 'post_date_gmt' ) );
	}
	...
	public function test_validate_column_post_modified() {
		...
		$this->assertSame( $gcdb->posts . '.post_modified', $q->validate_column( 'post_modified' ) );
	}
	...
	public function test_validate_column_post_modified_gmt() {
		...
		$this->assertSame( $gcdb->posts . '.post_modified_gmt', $q->validate_column( 'post_modified_gmt' ) );
	}
	...
	public function test_validate_column_comment_date() {
		...
		$this->assertSame( $gcdb->comments . '.comment_date', $q->validate_column( 'comment_date' ) );
	}
	...
	public function test_validate_column_comment_date_gmt() {
		...
		$this->assertSame( $gcdb->comments . '.comment_date_gmt', $q->validate_column( 'comment_date_gmt' ) );
	}
	...
	public function test_validate_column_invalid() {
		...
		$this->assertSame( $gcdb->posts . '.post_date', $q->validate_column( 'foo' ) );
	}
	...
	public function test_build_mysql_datetime_with_relative_date() {
		...
		$yesterday = new DateTimeImmutable( '-1 day', gc_timezone() );
	...
	public function test_build_time_query_should_not_discard_hour_0() {
		...
		$this->assertStringContainsString( '%H', $gcdb->remove_placeholder_escape( $found ) );
	}
	...
	public function test_build_time_query_hour_minute() {
		...
		$this->assertMatchesRegularExpression( "/DATE_FORMAT\( post_date, '%H\.%i' \) = 5\.150*/", $gcdb->remove_placeholder_escape( $found ) );
	}
	...
	public function test_build_time_query_hour_minute_second() {
		...
		// $compare value is floating point - use regex to account for
		// varying precision on different PHP installations.
		$this->assertMatchesRegularExpression( "/DATE_FORMAT\( post_date, '%H\.%i%s' \) = 5\.15350*/", $gcdb->remove_placeholder_escape( $found ) );
	}
	...
	public function test_build_time_query_minute_second() {
		...
		// $compare value is floating point - use regex to account for
		// varying precision on different PHP installations.
		$this->assertMatchesRegularExpression( "/DATE_FORMAT\( post_date, '0\.%i%s' \) = 0\.15350*/", $gcdb->remove_placeholder_escape( $found ) );
	}

	/**
	 * @ticket 25834
	 * @expectedIncorrectUsage GC_Date_Query
	 */
