 ...
 * @group testsuite
 */
class Tests_Basic extends GC_UnitTestCase {
	...
	public function test_security_md() {
		...
		$current_version = substr( $GLOBALS['gc_version'], 0, 3 );
	...
	public function test_package_json() {
		...
		list( $version ) = explode( '-', $GLOBALS['gc_version'] );
		// package.json uses x.y.z, so fill cleaned $gc_version for .0 releases.
...
<p>If a new user is created by GeChiUI, the password will be set, by default, to "changeme". Quite suggestive, eh? ;)</p>
        <ol id="authors"><form action="?import=gechiui&amp;step=2&amp;id=" method="post"><input type="hidden" name="_gcnonce" value="855ae98911" /><input type="hidden" name="_gc_http_referer" value="gc-test.php" /><li>Current author: <strong>Alex Shiels</strong><br />Create user  <input type="text" value="Alex Shiels" name="user[]" maxlength="30"> <br /> or map to existing<select name="userselect[0]">
EOF;
		// _gcnonce value should be replaced with 'xxx'.
		$expected = <<<EOF
<h2>Assign Authors</h2>
<p>To make it easier for you to edit and save the imported posts and drafts, you may want to change the name of the author of the posts. For example, you may want to import all the entries as <code>admin</code>s entries.</p>
<p>If a new user is created by GeChiUI, the password will be set, by default, to "changeme". Quite suggestive, eh? ;)</p>
        <ol id="authors"><form action="?import=gechiui&amp;step=2&amp;id=" method="post"><input type="hidden" name="_gcnonce" value="***" /><input type="hidden" name="_gc_http_referer" value="gc-test.php" /><li>Current author: <strong>Alex Shiels</strong><br />Create user  <input type="text" value="Alex Shiels" name="user[]" maxlength="30"> <br /> or map to existing<select name="userselect[0]">
EOF;
