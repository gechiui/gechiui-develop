 ...
 * @group taxonomy
 */
class Tests_Term extends GC_UnitTestCase {
	 ...
	 * @ticket 29911
	 */
	public function test_gc_delete_term_should_invalidate_cache_for_child_terms() {
		register_taxonomy(
			'gctests_tax',
			'post',
			array(
		...
		// Prime the cache.
		$child_term = get_term( $child, 'gctests_tax' );
		$this->assertSame( $parent, $child_term->parent );

		gc_delete_term( $parent, 'gctests_tax' );
		$child_term = get_term( $child, 'gctests_tax' );
	...
	public function test_is_term_type() {
		// Insert a term.
		$term = 'term_new';
		$t    = gc_insert_term( $term, $this->taxonomy );
		$this->assertIsArray( $t );
		$term_obj = get_term_by( 'name', $term, $this->taxonomy );

		$exists = term_exists( $term_obj->slug );
		// Clean up.
		$deleted = gc_delete_term( $t['term_id'], $this->taxonomy );
	 ...
	 * @ticket 15919
	 */
	public function test_gc_count_terms() {
		$count = gc_count_terms(
			array(
				'hide_empty' => true,
				'taxonomy'   => 'category',
			)
	 ...
	 * @ticket 36399
	 */
	public function test_gc_count_terms_legacy_interoperability() {
		...
		$count        = gc_count_terms( array( 'taxonomy' => 'post_tag' ) );
		$legacy_count = gc_count_terms( 'post_tag' );
	 ...
	 * @ticket 15475
	 */
	public function test_gc_add_remove_object_terms() {
		$posts = self::$post_ids;
		$tags  = self::factory()->tag->create_many( 5 );

		$tt = gc_add_object_terms( $posts[0], $tags[1], 'post_tag' );
		$this->assertCount( 1, $tt );
		$this->assertSame( array( $tags[1] ), gc_get_object_terms( $posts[0], 'post_tag', array( 'fields' => 'ids' ) ) );

		$three_tags = array( $tags[0], $tags[1], $tags[2] );
		$tt         = gc_add_object_terms( $posts[1], $three_tags, 'post_tag' );
		$this->assertCount( 3, $tt );
		$this->assertSame( $three_tags, gc_get_object_terms( $posts[1], 'post_tag', array( 'fields' => 'ids' ) ) );

		$this->assertTrue( gc_remove_object_terms( $posts[0], $tags[1], 'post_tag' ) );
		$this->assertFalse( gc_remove_object_terms( $posts[0], $tags[0], 'post_tag' ) );
		$this->assertInstanceOf( 'GC_Error', gc_remove_object_terms( $posts[0], $tags[1], 'non_existing_taxonomy' ) );
		$this->assertTrue( gc_remove_object_terms( $posts[1], $three_tags, 'post_tag' ) );
		$this->assertCount( 0, gc_get_object_terms( $posts[1], 'post_tag' ) );

		foreach ( $tags as $term_id ) {
			$this->assertTrue( gc_delete_term( $term_id, 'post_tag' ) );
		}

		foreach ( $posts as $post_id ) {
			$this->assertInstanceOf( 'GC_Post', gc_delete_post( $post_id ) );
		}
	}
	...
	public function test_term_is_ancestor_of() {
		...
		$t = gc_insert_term( $term, 'category' );
		$this->assertIsArray( $t );
		$t2 = gc_insert_term( $term, 'category', array( 'parent' => $t['term_id'] ) );
		...
		gc_delete_term( $t['term_id'], 'category' );
		gc_delete_term( $t2['term_id'], 'category' );
	}

	public function test_gc_insert_delete_category() {
		$term = rand_str();
		$this->assertNull( category_exists( $term ) );

		$initial_count = gc_count_terms( array( 'taxonomy' => 'category' ) );

		$t = gc_insert_category( array( 'cat_name' => $term ) );
		$this->assertIsNumeric( $t );
		$this->assertNotWPError( $t );
		$this->assertGreaterThan( 0, $t );
		$this->assertEquals( $initial_count + 1, gc_count_terms( array( 'taxonomy' => 'category' ) ) );
		...
		// Now delete it.
		$this->assertTrue( gc_delete_category( $t ) );
		$this->assertNull( term_exists( $term ) );
		$this->assertNull( term_exists( $t ) );
		$this->assertEquals( $initial_count, gc_count_terms( array( 'taxonomy' => 'category' ) ) );
	}

	/**
	 * @ticket 16550
	 */
	public function test_gc_set_post_categories() {
		...
		$term1 = gc_insert_term( 'Foo', 'category' );
		$term2 = gc_insert_term( 'Bar', 'category' );
		$term3 = gc_insert_term( 'Baz', 'category' );

		gc_set_post_categories( $post_id, array( $term1['term_id'], $term2['term_id'] ) );
		$this->assertCount( 2, $post->post_category );
		$this->assertSame( array( $term2['term_id'], $term1['term_id'] ), $post->post_category );

		gc_set_post_categories( $post_id, $term3['term_id'], true );
		$this->assertSame( array( $term2['term_id'], $term3['term_id'], $term1['term_id'] ), $post->post_category );

		$term4 = gc_insert_term( 'Burrito', 'category' );

		gc_set_post_categories( $post_id, $term4['term_id'] );
		$this->assertSame( array( $term4['term_id'] ), $post->post_category );

		gc_set_post_categories( $post_id, array( $term1['term_id'], $term2['term_id'] ), true );
		$this->assertSame( array( $term2['term_id'], $term4['term_id'], $term1['term_id'] ), $post->post_category );

		gc_set_post_categories( $post_id, array(), true );
		$this->assertCount( 1, $post->post_category );
		$this->assertEquals( get_option( 'default_category' ), $post->post_category[0] );

		gc_set_post_categories( $post_id, array() );
		$this->assertCount( 1, $post->post_category );
		$this->assertEquals( get_option( 'default_category' ), $post->post_category[0] );
	}
	 ...
	 * @ticket 43516
	 */
	public function test_gc_set_post_categories_sets_default_category_for_custom_post_types() {
		...
		$term = gc_insert_term( 'Foo', 'category' );
	...
	public function test_sanitize_term_field() {
		$term = gc_insert_term( 'foo', $this->taxonomy );
