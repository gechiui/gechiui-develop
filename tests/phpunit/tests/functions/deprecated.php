 ...
 * @group deprecated
 */
class Tests_Functions_Deprecated extends GC_UnitTestCase {
	...
	/**
	 * Tests that gc_save_image_file() has a deprecated argument when passed a GD resource.
	 *
	 * @ticket 6821
	 * @expectedDeprecated gc_save_image_file
	 * @requires function imagejpeg
	 *
	 * @covers ::gc_save_image_file
	 */
	public function test_gc_save_image_file_deprecated_with_gd_resource() {
		// Call gc_save_image_file().
		require_once ABSPATH . 'gc-admin/includes/image-edit.php';
		$file = gc_tempnam();
		$img  = imagecreatefromjpeg( DIR_TESTDATA . '/images/canola.jpg' );
		gc_save_image_file( $file, $img, 'image/jpeg', 1 );
		imagedestroy( $img );
		unlink( $file );

		// Check if the arg was deprecated.
		$check = $this->was_deprecated( 'argument', 'gc_save_image_file' );
		$this->assertNotEmpty( $check );
	}

	/**
	 * Tests that gc_save_image_file() doesn't have a deprecated argument when passed a GC_Image_Editor.
	 *
	 * @ticket 6821
	 * @requires function imagejpeg
	 *
	 * @covers ::gc_save_image_file
	 */
	public function test_gc_save_image_file_not_deprecated_with_gc_image_editor() {
		// Call gc_save_image_file().
		require_once ABSPATH . 'gc-admin/includes/image-edit.php';
		$file = gc_tempnam();
