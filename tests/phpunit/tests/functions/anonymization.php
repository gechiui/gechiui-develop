 ...
 * @group functions.php
 * @group privacy
 * @covers ::gc_privacy_anonymize_data
 */
class Tests_Functions_Anonymization extends GC_UnitTestCase {

	/**
	 * Test that gc_privacy_anonymize_ip() properly anonymizes all possible IP address formats.
	 *
	 * @dataProvider data_gc_privacy_anonymize_ip
	 *
	 * @ticket 41083
	 * @ticket 43545
	 *
	 * @covers ::gc_privacy_anonymize_ip
	 *
	 * @param string $raw_ip          Raw IP address.
	 * @param string $expected_result Expected result.
	 */
	public function test_gc_privacy_anonymize_ip( $raw_ip, $expected_result ) {
		$actual_result = gc_privacy_anonymize_data( 'ip', $raw_ip );
	...
	/**
	 * Provide test cases for `test_gc_privacy_anonymize_ip()`.
	 *
	 * @since 4.9.6 Moved from `Test_GC_Community_Events::data_get_unsafe_client_ip_anonymization()`.
	 *
	 * @return array {
	 *     @type array {
	 ...
	 * }
	 */
	public function data_gc_privacy_anonymize_ip() {
		return array(
			// Invalid IP.
			array(
				null,
	 ...
	 * Test that gc_privacy_anonymize_ip() properly anonymizes all possible IP address formats.
	 *
	 * @dataProvider data_gc_privacy_anonymize_ip_with_inet_dependency
	 *
	 * @ticket 41083
	 * @ticket 43545
	 ...
	 * @param string $expected_result Expected result.
	 */
	public function test_gc_privacy_anonymize_ip_with_inet_dependency( $raw_ip, $expected_result ) {
		$this->test_gc_privacy_anonymize_ip( $raw_ip, $expected_result );
	}
	 ...
	 * }
	 */
	public function data_gc_privacy_anonymize_ip_with_inet_dependency() {
		return array(
	...
	/**
	 * Test email anonymization of `gc_privacy_anonymize_data()`.
	 */
	public function test_anonymize_email() {
		$this->assertSame( 'deleted@site.invalid', gc_privacy_anonymize_data( 'email', 'bar@example.com' ) );
	}

	/**
	 * Test url anonymization of `gc_privacy_anonymize_data()`.
	 */
	public function test_anonymize_url() {
		$this->assertSame( 'https://site.invalid', gc_privacy_anonymize_data( 'url', 'https://example.com/author/username' ) );
	}

	/**
	 * Test date anonymization of `gc_privacy_anonymize_data()`.
	 */
	public function test_anonymize_date() {
		$this->assertSame( '0000-00-00 00:00:00', gc_privacy_anonymize_data( 'date', '2003-12-25 12:34:56' ) );
	}

	/**
	 * Test text anonymization of `gc_privacy_anonymize_data()`.
	 */
	public function test_anonymize_text() {
		$text = __( 'Four score and seven years ago' );
		$this->assertSame( '[deleted]', gc_privacy_anonymize_data( 'text', $text ) );
	}

	/**
	 * Test long text anonymization of `gc_privacy_anonymize_data()`.
	 */
	public function test_anonymize_long_text() {
		$text = __( 'Four score and seven years ago' );
		$this->assertSame( 'This content was deleted by the author.', gc_privacy_anonymize_data( 'longtext', $text ) );
	}
	...
	public function test_anonymize_with_filter() {
		add_filter( 'gc_privacy_anonymize_data', array( $this, 'filter_gc_privacy_anonymize_data' ), 10, 3 );
		$actual_url = gc_privacy_anonymize_data( 'url', 'https://example.com/author/username' );
		remove_filter( 'gc_privacy_anonymize_data', array( $this, 'filter_gc_privacy_anonymize_data' ), 10 );
	 ...
	 */
	public function filter_gc_privacy_anonymize_data( $anonymous, $type, $data ) {
