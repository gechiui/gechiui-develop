...
/**
 * Test gc_filter_object_list().
 *
 * @group functions.php
 * @covers ::gc_filter_object_list
 */
class Tests_Functions_gcFilterObjectList extends GC_UnitTestCase {
	...
	public function test_filter_object_list_nested_array_and() {
		$list = gc_filter_object_list( $this->object_list, array( 'field4' => array( 'blue' ) ), 'AND' );
		$this->assertCount( 1, $list );
		$this->assertArrayHasKey( 'baz', $list );
	}

	public function test_filter_object_list_nested_array_not() {
		$list = gc_filter_object_list( $this->object_list, array( 'field4' => array( 'red' ) ), 'NOT' );
		$this->assertCount( 2, $list );
		$this->assertArrayHasKey( 'bar', $list );
		$this->assertArrayHasKey( 'baz', $list );
	}
	...
	public function test_filter_object_list_nested_array_or_singular() {
		$list = gc_filter_object_list( $this->object_list, array( 'field4' => array( 'blue' ) ), 'OR' );
		$this->assertCount( 1, $list );
		$this->assertArrayHasKey( 'baz', $list );
	}

	public function test_filter_object_list_nested_array_and_field() {
		$list = gc_filter_object_list( $this->object_list, array( 'field4' => array( 'blue' ) ), 'AND', 'name' );
		$this->assertSame( array( 'baz' => 'baz' ), $list );
	}

	public function test_filter_object_list_nested_array_not_field() {
		$list = gc_filter_object_list( $this->object_list, array( 'field4' => array( 'green' ) ), 'NOT', 'name' );
		$this->assertSame(
			array(
				'foo' => 'foo',
				'baz' => 'baz',
