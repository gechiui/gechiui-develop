 ...
 * @group phpunit
 */
class Tests_TestHelpers extends GC_UnitTestCase {
	/**
	 * @ticket 30522
	 */
	 ...
	 * @ticket 36166
	 */
	public function test_die_handler_should_handle_gc_error() {
		$this->expectException( 'GCDieException' );

		gc_die( new GC_Error( 'test', 'test' ) );
	}

	/**
	 * @ticket 46813
	 */
	public function test_die_handler_should_not_cause_doing_it_wrong_notice_without_gc_query_set() {
		$this->expectException( 'GCDieException' );
		unset( $GLOBALS['gc_query'] );
	...
	public function test_die_process_input( $input, $expected ) {
		...
		$expected = gc_parse_args(
			$expected,
			$defaults
		);

		list( $message, $title, $args ) = _gc_die_process_input( $input['message'], $input['title'], $input['args'] );
	...
	public function test_setup_postdata_globals_should_be_reset_on_teardown__setup() {
		...
		$GLOBALS['gc_query']->setup_postdata( $post );
