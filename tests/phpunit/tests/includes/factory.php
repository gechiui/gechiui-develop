...
class TestFactoryFor extends GC_UnitTestCase {
	public function set_up() {
		parent::set_up();
		$this->category_factory = new GC_UnitTest_Factory_For_Term( null, 'category' );
	}

	public function test_create_creates_a_category() {
		$id = $this->category_factory->create();
		$this->assertInstanceOf( 'GC_Term', get_term_by( 'id', $id, 'category' ) );
	}

	public function test_get_object_by_id_gets_an_object() {
		$id = $this->category_factory->create();
		$this->assertInstanceOf( 'GC_Term', $this->category_factory->get_object_by_id( $id ) );
	}
	...
	public function test_the_taxonomy_argument_overrules_the_factory_taxonomy() {
		$term_factory = new GC_UnitTest_Factory_For_term( null, 'category' );
