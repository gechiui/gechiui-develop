 ...
 * @group oembed
 */
class Tests_Embed_Template extends GC_UnitTestCase {
	...
	public function test_oembed_output_post() {
		...
		// `print_embed_scripts()` assumes `gc-includes/js/gc-embed-template.js` is present:
		self::touch( ABSPATH . GCINC . '/js/gc-embed-template.js' );

		ob_start();
		require ABSPATH . GCINC . '/theme-compat/embed.php';
		$actual = ob_get_clean();
	...
	public function test_oembed_output_404() {
		...
		$GLOBALS['gc_query']->query_vars['embed'] = true;
	...
	public function test_gc_embed_excerpt_more_no_embed() {
		$GLOBALS['gc_query'] = new GC_Query();

		$this->assertSame( 'foo bar', gc_embed_excerpt_more( 'foo bar' ) );
	}

	public function test_gc_embed_excerpt_more() {
		$post_id = self::factory()->post->create(
			array(
				'post_title'   => 'Foo Bar',
		...
		$this->assertSame( '', gc_embed_excerpt_more( '' ) );

		$this->go_to( get_post_embed_url( $post_id ) );

		$actual = gc_embed_excerpt_more( '' );

		$expected = sprintf(
			' &hellip; <a href="%s" class="gc-embed-more" target="_top">Continue reading <span class="screen-reader-text">Foo Bar</span></a>',
			get_the_permalink()
		);
	...
	public function test_get_post_embed_html() {
		...
		$expected = '<iframe sandbox="allow-scripts" security="restricted" src="' . esc_url( get_post_embed_url( $post_id ) ) . '#?secret=__SECRET__" width="200" height="200" title="' . $title . '" data-secret=__SECRET__ frameborder="0" marginwidth="0" marginheight="0" scrolling="no" class="gc-embedded-content"></iframe>';
		$actual   = get_post_embed_html( 200, 200, $post_id );
		$actual   = preg_replace( '/secret=("?)\w+\1/', 'secret=__SECRET__', $actual );

		$this->assertStringStartsWith( '<blockquote class="gc-embedded-content" data-secret=__SECRET__>', $actual );
		$this->assertStringContainsString( $expected, $actual );
		$this->assertStringEndsWith( '</script>', trim( $actual ) );
	}

	/** @covers ::gc_oembed_add_host_js() */
	public function test_add_host_js() {
		remove_all_filters( 'embed_oembed_html' );

		// This function is now a no-op.
		gc_oembed_add_host_js();

		$this->assertFalse( has_filter( 'embed_oembed_html', 'gc_maybe_enqueue_oembed_host_js' ) );
	}

	/** @covers ::gc_maybe_enqueue_oembed_host_js() */
	public function test_gc_maybe_enqueue_oembed_host_js() {
		$scripts = gc_scripts();

		$this->assertFalse( $scripts->query( 'gc-embed', 'enqueued' ) );

		$post_embed     = '<blockquote class="gc-embedded-content" data-secret="S24AQCJW9i"><a href="https://make.gechiui.com/core/2016/03/11/embeds-changes-in-gechiui-4-5/">Embeds Changes in GeChiUI 4.5</a></blockquote><iframe class="gc-embedded-content" sandbox="allow-scripts" security="restricted" style="position: absolute; clip: rect(1px, 1px, 1px, 1px);" title="&#8220;Embeds Changes in GeChiUI 4.5&#8221; &#8212; Make GeChiUI Core" src="https://make.gechiui.com/core/2016/03/11/embeds-changes-in-gechiui-4-5/embed/#?secret=S24AQCJW9i" data-secret="S24AQCJW9i" width="600" height="338" frameborder="0" marginwidth="0" marginheight="0" scrolling="no"></iframe>';
		...
		gc_maybe_enqueue_oembed_host_js( $non_post_embed );
		$this->assertFalse( $scripts->query( 'gc-embed', 'enqueued' ) );

		gc_maybe_enqueue_oembed_host_js( $post_embed );
		$this->assertTrue( $scripts->query( 'gc-embed', 'enqueued' ) );
	}

	/** @covers ::gc_maybe_enqueue_oembed_host_js() */
	public function test_gc_maybe_enqueue_oembed_host_js_without_gc_head_action() {
		$scripts = gc_scripts();

		remove_action( 'gc_head', 'gc_oembed_add_host_js' );
		$this->assertFalse( $scripts->query( 'gc-embed', 'enqueued' ) );
	...
	/**
	 * Confirms that no ampersands exist in src/gc-includes/js/gc-embed.js.
	 *
	 * See also the `verify:gc-embed` Grunt task for verifying the built file.
	 *
	 * @ticket 34698
	 */
	public function test_js_no_ampersands() {
		$this->assertStringNotContainsString( '&', file_get_contents( ABSPATH . GCINC . '/js/gc-embed.js' ) );
	}
}
