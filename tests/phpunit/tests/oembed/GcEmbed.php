 ...
 * @group oembed
 */
class Tests_GC_Embed extends GC_UnitTestCase {
	/**
	 * @var GC_Embed
	 */
	protected $gc_embed;
	...
	public function test_maybe_run_ajax_cache_should_return_nothing_if_there_is_no_post() {
		...
		$this->gc_embed->maybe_run_ajax_cache();
	}
	...
	public function test_maybe_run_ajax_cache_should_return_javascript() {
		...
		$actual = get_echo( array( $this->gc_embed, 'maybe_run_ajax_cache' ) );
	...
	public function test_gc_maybe_load_embeds() {
		$this->assertSameSets( array( 10, 9999 ), array_keys( $GLOBALS['gc_embed']->handlers ) );
		$this->assertSameSets(
			array(
				'youtube_embed_url',
			),
			array_keys( $GLOBALS['gc_embed']->handlers[10] )
		);
		$this->assertSameSets(
			array(
				'audio',
			...
			array_keys( $GLOBALS['gc_embed']->handlers[9999] )
		);
	}

	public function test_gc_embed_register_handler() {
		$handle   = __FUNCTION__;
		$regex    = '#https?://example\.com/embed/([^/]+)#i';
		$callback = array( $this, '_embed_handler_callback' );

		gc_embed_register_handler( $handle, $regex, $callback );

		$expected = array(
			'regex'    => $regex,
			'callback' => $callback,
		);
		$actual   = $GLOBALS['gc_embed']->handlers[10];

		gc_embed_unregister_handler( $handle );

		$this->assertContains( $expected, $actual );
	}

	public function test_gc_embed_unregister_handler() {
		$this->assertArrayHasKey( 'youtube_embed_url', $GLOBALS['gc_embed']->handlers[10] );

		gc_embed_unregister_handler( 'youtube_embed_url' );

		$handlers = $GLOBALS['gc_embed']->handlers[10];
	...
	public function test_autoembed_should_do_nothing_without_matching_handler() {
		...
		$actual = $this->gc_embed->autoembed( $content );
	...
	public function test_autoembed_should_return_modified_content() {
		...
		$actual = $GLOBALS['gc_embed']->autoembed( $content );
		gc_embed_unregister_handler( $handle );
	...
	public function test_delete_oembed_caches() {
		...
		$this->gc_embed->delete_oembed_caches( $post_id );
	...
	public function test_cache_oembed_invalid_post_type() {
		...
		$this->gc_embed->cache_oembed( $post_id );
		$this->assertNotSame( $post_id, $this->gc_embed->post_ID );
	}
	...
	public function test_cache_oembed_for_post() {
		...
		$key_suffix    = md5( $url . serialize( gc_embed_defaults( $url ) ) );
		...
		$this->assertSame( $post_id, $this->gc_embed->post_ID );
	...
	public function test_shortcode_should_get_cached_data_from_post_meta_for_known_post() {
		...
		add_filter( 'pre_oembed_result', array( $this, '_pre_oembed_result_callback' ) );
		$actual = $this->gc_embed->shortcode( array(), $url );
		remove_filter( 'pre_oembed_result', array( $this, '_pre_oembed_result_callback' ) );

		$actual_2 = $this->gc_embed->shortcode( array(), $url );
	...
	public function test_shortcode_should_cache_data_in_custom_post() {
		...
		$oembed_post_id = $this->gc_embed->find_oembed_post_id( $key_suffix );
		$post_content   = get_post( $oembed_post_id )->post_content;

		// Result should be cached.
		$actual_2 = $this->gc_embed->shortcode( array(), $url );

		gc_delete_post( $oembed_post_id );
	...
	public function test_shortcode_should_update_custom_post() {
		...
		add_filter( 'pre_oembed_result', '__return_empty_string' );
		$this->gc_embed->shortcode( array(), $url );
		remove_filter( 'pre_oembed_result', '__return_empty_string' );
		...
		$previous_usecache        = $this->gc_embed->usecache;
		$this->gc_embed->usecache = false;
		...
		$this->gc_embed->usecache = $previous_usecache;
	...
	public function test_shortcode_should_get_url_from_src_attribute() {
		...
		$actual = $this->gc_embed->shortcode( array( 'src' => $url ) );
	...
	public function test_shortcode_should_return_empty_string_for_missing_url() {
		$this->assertEmpty( $this->gc_embed->shortcode( array() ) );
	}
	...
	public function test_run_shortcode_url_only() {
		$url    = 'http://example.com/embed/foo';
		$actual = $this->gc_embed->run_shortcode( '[embed]' . $url . '[/embed]' );
		$this->assertSame( '<a href="' . esc_url( $url ) . '">' . esc_html( $url ) . '</a>', $actual );
	}

	public function test_maybe_make_link() {
		$url    = 'http://example.com/embed/foo';
		$actual = $this->gc_embed->maybe_make_link( $url );

		$this->assertSame( '<a href="' . esc_url( $url ) . '">' . esc_html( $url ) . '</a>', $actual );
	}

	public function test_maybe_make_link_return_false_on_fail() {
		$this->gc_embed->return_false_on_fail = true;
		$this->assertFalse( $this->gc_embed->maybe_make_link( 'http://example.com/' ) );
	}

	public function test_maybe_make_link_do_not_link_if_unknown() {
		$url = 'http://example.com/';

		$this->gc_embed->linkifunknown = false;
		$this->assertSame( $url, $this->gc_embed->maybe_make_link( $url ) );
	}
}
