 ...
 * @group restapi
 */
class Test_oEmbed_Controller extends GC_UnitTestCase {
	/**
	 * @var GC_REST_Server
	 */
	protected $server;
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		// `get_post_embed_html()` assumes `gc-includes/js/gc-embed.js` is present:
		self::touch( ABSPATH . GCINC . '/js/gc-embed.js' );
	}
	...
	public function test_gc_oembed_ensure_format() {
		$this->assertSame( 'json', gc_oembed_ensure_format( 'json' ) );
		$this->assertSame( 'xml', gc_oembed_ensure_format( 'xml' ) );
		$this->assertSame( 'json', gc_oembed_ensure_format( 123 ) );
		$this->assertSame( 'json', gc_oembed_ensure_format( 'random' ) );
		$this->assertSame( 'json', gc_oembed_ensure_format( array() ) );
	}
	...
	public function test_request_with_wrong_method() {
		$request = new GC_REST_Request( 'POST', '/oembed/1.0/embed' );
	...
	public function test_request_without_url_param() {
		$request = new GC_REST_Request( 'GET', '/oembed/1.0/embed' );
	...
	public function test_rest_pre_serve_request_wrong_method() {
		...
		$request = new GC_REST_Request( 'HEAD', '/oembed/1.0/embed' );
		$request->set_param( 'url', get_permalink( $post->ID ) );
		$request->set_param( 'format', 'xml' );
	...
	public function test_get_oembed_endpoint_url_pretty_permalinks() {
		...
		$this->assertSame( home_url() . '/gc-json/oembed/1.0/embed', get_oembed_endpoint_url() );
		$this->assertSame( home_url() . '/gc-json/oembed/1.0/embed', get_oembed_endpoint_url( '', 'xml' ) );

		$post_id     = $this->factory()->post->create();
		$url         = get_permalink( $post_id );
		$url_encoded = urlencode( $url );

		$this->assertSame( home_url() . '/gc-json/oembed/1.0/embed?url=' . $url_encoded, get_oembed_endpoint_url( $url ) );
		$this->assertSame( home_url() . '/gc-json/oembed/1.0/embed?url=' . $url_encoded . '&format=xml', get_oembed_endpoint_url( $url, 'xml' ) );
	...
	public function test_proxy_without_permission() {
		...
		$request  = new GC_REST_Request( 'GET', '/oembed/1.0/proxy' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_proxy_with_valid_oembed_provider() {
		...
		$request->set_param( '_gcnonce', gc_create_nonce( 'gc_rest' ) );
		$response = rest_get_server()->dispatch( $request );
