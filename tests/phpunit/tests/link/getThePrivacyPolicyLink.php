 ...
 * @since 4.9.6
 */
class Tests_Link_GetThePrivacyPolicyLink extends GC_UnitTestCase {
	/**
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		self::$privacy_policy_page_id = $factory->post->create(
			array(
				'post_type'  => 'page',
				'post_title' => GC_TESTS_DOMAIN . ' Privacy Policy',
			)
		);
	 ...
	 * created and set as the `gc_page_for_privacy_policy`. The post title should
	 * be used as the link text.
	 */
	public function test_get_the_privacy_policy_link_should_return_valid_link_when_privacy_page_set() {
		update_option( 'gc_page_for_privacy_policy', self::$privacy_policy_page_id );
		...
		$this->assertStringEndsWith( '>' . GC_TESTS_DOMAIN . ' Privacy Policy</a>', $actual_link );
	}

	/**
	 * The function should prepend the supplied `$before` markup and append the
	 * supplied `$after` markup when the `gc_page_for_privacy_policy` is configured.
	 */
	 ...
	 * the supplied `$after` markup when the `gc_page_for_privacy_policy` is _not_ configured.
	 */
	...
	public function test_function_should_return_empty_string_when_privacy_page_title_empty() {
		...
		update_option( 'gc_page_for_privacy_policy', $nameless_page_id );

		$this->assertSame( '', get_the_privacy_policy_link( self::$before, self::$after ) );
	}

	/**
	 * The function should return an empty string when `gc_page_for_privacy_policy` is _not_ configured.
	 */
