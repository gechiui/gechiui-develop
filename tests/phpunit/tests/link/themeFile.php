 ...
 * @group link
 */
class Tests_Link_ThemeFile extends GC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		// Copy themes from tests/phpunit/data to gc-content/themes.
		foreach ( $themes as $theme ) {
			$source_dir = DIR_TESTDATA . '/' . $theme;
			$dest_dir   = GC_CONTENT_DIR . '/themes/' . $theme;
	...
	public static function gcTearDownAfterClass() {
		...
		// Remove previously copied themes from gc-content/themes.
		foreach ( $themes as $theme ) {
			$dest_dir = GC_CONTENT_DIR . '/themes/' . $theme;
	...
	public function test_theme_file_path_with_parent_theme( $file, $expected_theme, $existence ) {
		...
		$this->assertSame( GC_CONTENT_DIR . "/themes/theme-file-parent/{$file}", get_theme_file_path( $file ) );
		$this->assertSame( GC_CONTENT_DIR . "/themes/theme-file-parent/{$file}", get_parent_theme_file_path( $file ) );
	}
	...
	public function test_theme_file_path_with_child_theme( $file, $expected_theme, $existence ) {
		...
		$this->assertSame( GC_CONTENT_DIR . "/themes/{$expected_theme}/{$file}", get_theme_file_path( $file ) );
	...
	public function test_theme_file_existance( $file, $expected_theme, $existence ) {
		...
		if ( in_array( 'theme-file-child', $existence, true ) ) {
			$this->assertFileExists( GC_CONTENT_DIR . "/themes/theme-file-child/{$file}" );
		} else {
			$this->assertFileDoesNotExist( GC_CONTENT_DIR . "/themes/theme-file-child/{$file}" );
		}

		if ( in_array( 'theme-file-parent', $existence, true ) ) {
			$this->assertFileExists( GC_CONTENT_DIR . "/themes/theme-file-parent/{$file}" );
		} else {
			$this->assertFileDoesNotExist( GC_CONTENT_DIR . "/themes/theme-file-parent/{$file}" );
		}
