 ...
 * @group link
 * @group canonical
 * @covers ::gc_get_canonical_url
 */
class Tests_Link_gcGetCanonicalUrl extends GC_UnitTestCase {
	...
	public function test_non_existing_post() {
		$this->assertFalse( gc_get_canonical_url( -1 ) );
	}
	...
	public function test_post_status() {
		...
		$this->assertFalse( gc_get_canonical_url( $post_id ) );
	}
	...
	public function test_non_current_page() {
		$this->assertSame( get_permalink( self::$post_id ), gc_get_canonical_url( self::$post_id ) );
	}
	...
	public function test_paged_with_plain_permalink_structure() {
		...
		$this->assertSame( $expected, gc_get_canonical_url( self::$post_id ) );
	}
	...
	public function test_comments_paged_with_pretty_permalink_structure() {
		...
		$expected = user_trailingslashit( trailingslashit( get_permalink( self::$post_id ) ) . $gc_rewrite->comments_pagination_base . '-' . $cpage, 'commentpaged' ) . '#comments';
	...
	public function test_get_canonical_url_filter() {
		...
		$canonical_url = gc_get_canonical_url( self::$post_id );
