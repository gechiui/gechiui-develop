 ...
 */
class Tests_Link_GetAdjacentPost extends GC_UnitTestCase {
	...
	public function test_get_adjacent_post() {
		...
		// Assign some terms.
		gc_set_object_terms( $post_one->ID, 'GeChiUI', 'category', false );
		gc_set_object_terms( $post_three->ID, 'GeChiUI', 'category', false );

		gc_set_object_terms( $post_two->ID, 'plugins', 'post_tag', false );
		gc_set_object_terms( $post_four->ID, 'plugins', 'post_tag', false );
	...
	public function test_get_adjacent_post_excluded_terms() {
		...
		gc_set_post_terms( $p2, array( $t ), 'gctests_tax' );

		// Fake current page.
		$_post           = isset( $GLOBALS['post'] ) ? $GLOBALS['post'] : null;
		$GLOBALS['post'] = get_post( $p1 );

		$found = get_adjacent_post( false, array( $t ), true, 'gctests_tax' );
	...
	public function test_get_adjacent_post_excluded_terms_should_not_require_posts_to_have_terms_in_any_taxonomy() {
		...
		gc_delete_object_term_relationships( $p3, 'category' );
	...
	public function test_get_adjacent_post_excluded_terms_filter() {
		...
		gc_set_post_terms( $p1, array( $terms[0], $terms[1] ), 'gctests_tax' );
		gc_set_post_terms( $p2, array( $terms[1] ), 'gctests_tax' );
		gc_set_post_terms( $p3, array( $terms[0] ), 'gctests_tax' );
		...
		$found = get_adjacent_post( true, array(), true, 'gctests_tax' );
	...
	public function test_get_adjacent_post_excluded_terms_filter_should_apply_to_empty_excluded_terms_parameter() {
		...
		$found = get_adjacent_post( false, array(), true, 'gctests_tax' );
