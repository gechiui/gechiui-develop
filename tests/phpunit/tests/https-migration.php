 ...
 */
class Tests_HTTPS_Migration extends GC_UnitTestCase {

	/**
	 * @ticket 51437
	 */
	public function test_gc_should_replace_insecure_home_url() {
		// Should return false because site is not using HTTPS.
		$this->force_gc_is_using_https( false );
		$this->assertFalse( gc_should_replace_insecure_home_url() );

		// Should still return false because HTTPS migration flag is not set.
		$this->force_gc_is_using_https( true );
		$this->assertFalse( gc_should_replace_insecure_home_url() );
		...
		$this->assertTrue( gc_should_replace_insecure_home_url() );

		// Should be overridable via filter.
		add_filter( 'gc_should_replace_insecure_home_url', '__return_false' );
		$this->assertFalse( gc_should_replace_insecure_home_url() );
	}
	 ...
	 * @ticket 51437
	 */
	public function test_gc_replace_insecure_home_url() {
		$http_url  = home_url( '', 'http' );
		$https_url = home_url( '', 'https' );

		$http_block_data  = array(
			'id'  => 3,
			'url' => $http_url . '/gc-content/uploads/2021/01/image.jpg',
		);
		$https_block_data = array(
			'id'  => 3,
			'url' => $https_url . '/gc-content/uploads/2021/01/image.jpg',
		);
		...
		$http_content  = sprintf( $content, $http_url, gc_json_encode( $http_block_data ), $http_block_data['url'] );
		$https_content = sprintf( $content, $https_url, gc_json_encode( $https_block_data ), $https_block_data['url'] );

		// Replaces URLs, including its encoded variant.
		add_filter( 'gc_should_replace_insecure_home_url', '__return_true' );
		$this->assertSame( $https_content, gc_replace_insecure_home_url( $http_content ) );

		// Does not replace anything if determined as unnecessary.
		add_filter( 'gc_should_replace_insecure_home_url', '__return_false' );
		$this->assertSame( $http_content, gc_replace_insecure_home_url( $http_content ) );
	}

	/**
	 * @ticket 51437
	 */
	public function test_gc_update_urls_to_https() {
		...
		$this->assertTrue( gc_update_urls_to_https() );
		...
		$this->assertFalse( gc_update_urls_to_https() );
	 ...
	 * @ticket 51437
	 */
	public function test_gc_update_https_migration_required() {
		// Changing HTTP to HTTPS on a site with content should result in flag being set, requiring migration.
		update_option( 'fresh_site', '0' );
		gc_update_https_migration_required( 'http://example.org', 'https://example.org' );
		...
		gc_update_https_migration_required( 'http://example.org', 'https://another-example.org' );
		...
		gc_update_https_migration_required( 'https://example.org', 'http://example.org' );
		$this->assertFalse( get_option( 'https_migration_required' ) );
	}
	 ...
	 * @ticket 51437
	 */
	public function test_gc_should_replace_insecure_home_url_integration() {
		...
		update_option( 'siteurl', 'https://gc.example.org' );
		$this->assertFalse( gc_should_replace_insecure_home_url() );
	}

	private function force_gc_is_using_https( $enabled ) {
