 ...
 * @group formatting
 */
class Tests_Post_GetTheExcerpt extends GC_UnitTestCase {
	...
	public function test_inner_blocks_excerpt() {
		$content_1 = '<!-- gc:group -->
<div class="gc-block-group"><!-- gc:columns -->
<div class="gc-block-columns"><!-- gc:column -->
...
<!-- /gc:paragraph -->';
		...
		$content_2 = '<!-- gc:group -->
<div class="gc-block-group"><!-- gc:paragraph -->
...
<!-- /gc:paragraph -->';
		...
		$this->assertSame(
			'Column 1 Column 2',
			get_the_excerpt( ( new GC_Query( array( 'p' => $post_1->ID ) ) )->posts[0] )
		);

		$this->assertSame(
			'Paragraph inside group block',
			get_the_excerpt( ( new GC_Query( array( 'p' => $post_2->ID ) ) )->posts[0] )
		);
	}
}
