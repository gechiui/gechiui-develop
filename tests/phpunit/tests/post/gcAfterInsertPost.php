 ...
 * @group post
 */
class Tests_Post_gcAfterInsertPost extends GC_UnitTestCase {
	...
	public function set_up() {
		parent::set_up();
		add_action( 'gc_after_insert_post', array( $this, 'action_gc_after_insert_post' ), 10, 4 );
	}
	...
	/**
	 * Helper function to obtain data running on the hook `gc_after_insert_post`.
	 *
	 ...
	 */
	public function action_gc_after_insert_post( $post_id, $post, $update, $post_before ) {
		...
		remove_action( 'gc_after_insert_post', array( $this, 'action_gc_after_insert_post' ), 10 );
	}
	 ...
	 * @ticket 45114
	 */
	public function test_update_via_gc_update_post() {
	 ...
	 * @ticket 45114
	 */
	public function test_update_via_gc_publish_post() {
		gc_publish_post( self::$post_id );
	 ...
	 * @ticket 45114
	 */
	public function test_new_post_via_gc_insert_post() {
		gc_insert_post(
			array(
				'post_status'  => 'draft',
	...
	public function test_new_post_via_rest_contoller() {
		...
		$request = new GC_REST_Request( 'POST', sprintf( '/gc/v2/posts' ) );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
	...
	public function test_update_attachment_via_rest_contoller() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/media/%d', $attachment_id ) );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
