 ...
 * @group post
 */
class Tests_Post_GetPageByPath extends GC_UnitTestCase {
	/**
	 * @ticket 15665
	 */
	public function test_get_page_by_path_priority() {
		...
		$gcdb->update( $gcdb->posts, array( 'post_name' => 'some-page' ), array( 'ID' => $page->ID ) );
	...
	public function test_should_obey_post_type() {
		...
		$found = get_page_by_path( 'foo', OBJECT, 'gctests_pt' );
		$this->assertSame( $page, $found->ID );
	}
