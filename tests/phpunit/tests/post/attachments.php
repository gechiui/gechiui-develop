 ...
 * @group upload
 */
class Tests_Post_Attachments extends GC_UnitTestCase {
	...
	public function test_insert_bogus_image() {
		...
		$upload = gc_upload_bits( $filename, null, $contents );
	...
	public function test_insert_image_no_thumb() {
		...
		$this->assertSame( gc_basename( $upload['file'] ), gc_basename( $downsize[0] ) );
	...
	public function test_insert_image_medium_sizes() {
		...
		// image_downsize() should return the correct images and sizes.
		$downsize = image_downsize( $id, 'thumbnail' );
		$this->assertSame( '2007-06-17DSC_4173-150x150.jpg', gc_basename( $downsize[0] ) );
		$this->assertSame( 150, $downsize[1] );
		$this->assertSame( 150, $downsize[2] );

		$downsize = image_downsize( $id, 'medium' );
		$this->assertSame( '2007-06-17DSC_4173-400x602.jpg', gc_basename( $downsize[0] ) );
		$this->assertSame( 400, $downsize[1] );
		$this->assertSame( 602, $downsize[2] );

		$downsize = image_downsize( $id, 'medium_large' );
		$this->assertSame( '2007-06-17DSC_4173-600x904.jpg', gc_basename( $downsize[0] ) );
		$this->assertSame( 600, $downsize[1] );
		$this->assertSame( 904, $downsize[2] );

		$downsize = image_downsize( $id, 'full' );
		$this->assertSame( '2007-06-17DSC_4173.jpg', gc_basename( $downsize[0] ) );
	...
	public function test_insert_image_delete() {
		...
		$meta     = gc_get_attachment_metadata( $id );
		$original = $meta['file'];
	...
	public function test_update_attachment_fields() {
		...
		$attached_file = get_post_meta( $id, '_gc_attached_file', true );
		...
		$this->assertSame( $attached_file, get_post_meta( $id, '_gc_attached_file', true ) );
	}
	...
	public function test_update_orphan_attachment_parent() {
		...
		gc_insert_attachment( $attachment, '', $post_id );
		$attachment = get_post( $attachment_id );
	 ...
	 * @ticket 15928
	 */
	public function test_gc_get_attachment_url_should_not_force_https_when_current_page_is_non_ssl_and_siteurl_is_non_ssl() {
	 ...
	 */
	public function test_gc_get_attachment_url_should_not_force_https_when_current_page_is_non_ssl_and_siteurl_is_ssl() {
		$siteurl = get_option( 'siteurl' );
	 ...
	 */
	public function test_gc_get_attachment_url_should_force_https_with_https_on_same_host_when_siteurl_is_non_ssl_but_ssl_is_available() {
		$siteurl = get_option( 'siteurl' );
		update_option( 'siteurl', set_url_scheme( $siteurl, 'http' ) );
		...
		// Ensure that server host matches the host of gc_upload_dir().
		$upload_dir           = gc_upload_dir();
		$_SERVER['HTTP_HOST'] = parse_url( $upload_dir['baseurl'], PHP_URL_HOST );

		// Test that gc_get_attachemt_url returns with https scheme.
		$url = gc_get_attachment_url( $attachment_id );
	 ...
	 * @ticket 15928
	 */
	public function test_gc_get_attachment_url_with_https_on_same_host_when_siteurl_is_https() {
		$siteurl = get_option( 'siteurl' );
		update_option( 'siteurl', set_url_scheme( $siteurl, 'https' ) );
	 ...
	 * @ticket 15928
	 */
	public function test_gc_get_attachment_url_should_not_force_https_when_administering_over_https_but_siteurl_is_not_https() {
		$siteurl = get_option( 'siteurl' );
		update_option( 'siteurl', set_url_scheme( $siteurl, 'http' ) );
	 ...
	 * @ticket 15928
	 */
	public function test_gc_get_attachment_url_should_force_https_when_administering_over_https_and_siteurl_is_https() {
	...
	public function test_gc_attachment_is() {
		$filename = DIR_TESTDATA . '/images/test-image.jpg';
		$contents = file_get_contents( $filename );
		...
		$this->assertTrue( gc_attachment_is_image( $attachment_id ) );
		$this->assertTrue( gc_attachment_is( 'image', $attachment_id ) );
		$this->assertFalse( gc_attachment_is( 'audio', $attachment_id ) );
		$this->assertFalse( gc_attachment_is( 'video', $attachment_id ) );
	}

	public function test_gc_attachment_is_default() {
		...
		$this->assertFalse( gc_attachment_is_image( $attachment_id ) );
		$this->assertTrue( gc_attachment_is( 'psd', $attachment_id ) );
		$this->assertFalse( gc_attachment_is( 'audio', $attachment_id ) );
		$this->assertFalse( gc_attachment_is( 'video', $attachment_id ) );
	 ...
	 * @ticket 33012
	 */
	public function test_gc_mime_type_icon() {
		$icon = gc_mime_type_icon();
	 ...
	 * @ticket 33012
	 */
	public function test_gc_mime_type_icon_video() {
		$icon = gc_mime_type_icon( 'video/mp4' );
