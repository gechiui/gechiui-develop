 ...
 */
class Tests_Post_GetBodyClass extends GC_UnitTestCase {
	protected $post_id;
	...
	public function test_with_utf8_term_slugs() {
		...
		$term1 = get_term( $term_id1, 'gctests_tax' );
		$term2 = get_term( $term_id2, 'gctests_tax' );
		$term3 = get_term( $term_id3, 'gctests_tax' );

		$this->go_to( home_url( "?gctests_tax={$term1->slug}" ) );
		$this->assertContains( "term-$term_id1", get_body_class() );

		$this->go_to( home_url( "?gctests_tax={$term2->slug}" ) );
		$this->assertContains( "term-$term_id2", get_body_class() );

		$this->go_to( home_url( "?gctests_tax={$term3->slug}" ) );
	...
	public function test_page_template_body_classes() {
		...
		add_post_meta( $post_id, '_gc_page_template', 'templates/cpt.php' );
