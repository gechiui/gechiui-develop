 ...
 * @group post
 */
class Tests_Post_gcInsertPost extends GC_UnitTestCase {
	...
	public function test_trashing_a_post_should_add_trashed_suffix_to_post_name() {
		...
		gc_trash_post( $trashed_about_page_id );
	...
	public function test_trashed_posts_original_post_name_should_be_reassigned_after_untrashing() {
		...
		gc_trash_post( $about_page_id );

		gc_untrash_post( $about_page_id );
	...
	public function test_untrashing_a_post_should_always_restore_it_to_draft_status( $post_status ) {
		...
		gc_trash_post( $page_id );
		gc_untrash_post( $page_id );
	 ...
	 * @dataProvider data_various_post_statuses
	 */
	public function test_gc_untrash_post_status_filter_restores_post_to_correct_status( $post_status ) {
		...
		remove_filter( 'gc_untrash_post_status', 'gc_untrash_post_set_previous_status', 10, 3 );
	...
	public function test_contributor_cannot_set_post_slug( $post_type ) {
		gc_set_current_user( self::$user_ids['contributor'] );
	...
	public function test_administrator_can_set_post_slug( $post_type ) {
		gc_set_current_user( self::$user_ids['administrator'] );
