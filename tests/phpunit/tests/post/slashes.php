 ...
 * @ticket 21767
 */
class Tests_Post_Slashes extends GC_UnitTestCase {
	 ...
	 */
	public function test_gc_insert_post() {
		...
		$this->assertSame( gc_unslash( $this->slash_3 ), $post->post_content );
		...
		$this->assertSame( gc_unslash( $this->slash_4 ), $post->post_content );
	 ...
	 * Tests the model function that expects slashed data.
	 */
	public function test_gc_update_post() {
		$post_id = self::$post_id;
	 ...
	 * @ticket 27550
	 */
	public function test_gc_trash_untrash() {
		$post    = array(
			'post_title'   => $this->slash_1,
			'post_content' => $this->slash_3,
			'post_excerpt' => $this->slash_5,
		);
		$post_id = gc_insert_post( gc_slash( $post ) );

		$trashed = gc_trash_post( $post_id );
		...
		$untrashed = gc_untrash_post( $post_id );
