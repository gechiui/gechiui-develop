 ...
 * @group media
 */
class Tests_Post_Thumbnail_Template extends GC_UnitTestCase {
	...
	public static function tear_down_after_class() {
		gc_delete_post( self::$attachment_id, true );
	...
	public function test_get_the_post_thumbnail() {
		...
		$expected = gc_get_attachment_image(
			self::$attachment_id,
			'post-thumbnail',
			false,
			array(
				'class' => 'attachment-post-thumbnail size-post-thumbnail gc-post-image',
			)
		);
	...
	public function test_get_the_post_thumbnail_url() {
		...
		$this->assertFalse( get_the_post_thumbnail_url() );
		$this->assertSame( gc_get_attachment_url( self::$attachment_id ), get_the_post_thumbnail_url( self::$post ) );

		$GLOBALS['post'] = self::$post;

		$this->assertSame( gc_get_attachment_url( self::$attachment_id ), get_the_post_thumbnail_url() );
	}
	...
	public function test_get_the_post_thumbnail_url_with_invalid_post() {
		...
		$deleted = gc_delete_post( self::$post->ID, true );
	...
	public function test_the_post_thumbnail_url() {
		...
		$this->expectOutputString( gc_get_attachment_url( self::$attachment_id ) );
		the_post_thumbnail_url();
	}
	 ...
	 * @ticket 12922
	 */
	public function test__gc_preview_post_thumbnail_filter() {
		...
		$result = _gc_preview_post_thumbnail_filter( '', self::$post->ID, '_thumbnail_id' );
	 ...
	 * @ticket 37697
	 */
	public function test__gc_preview_post_thumbnail_filter_secondary_post() {
		$old_post = isset( $GLOBALS['post'] ) ? $GLOBALS['post'] : null;
