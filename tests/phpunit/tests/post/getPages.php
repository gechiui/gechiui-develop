...
class Tests_Post_GetPages extends GC_UnitTestCase {
	/**
	 * @ticket 23167
	 */
	public function test_get_pages_cache() {
		...
		foreach ( $pages as $page ) {
			$this->assertInstanceOf( 'GC_Post', $page );
		}
		...
		$this->assertNotEquals( $time1, $time2 = gc_cache_get( 'last_changed', 'posts' ) );
		...
		$pages = get_pages( array( 'number' => 2 ) );
		$this->assertCount( 2, $pages );
		$this->assertSame( $time2, gc_cache_get( 'last_changed', 'posts' ) );
		$this->assertSame( $num_queries + 1, $gcdb->num_queries );
		foreach ( $pages as $page ) {
			$this->assertInstanceOf( 'GC_Post', $page );
		}

		$last_changed = gc_cache_get( 'last_changed', 'posts' );

		// This should bump last_changed.
		gc_delete_post( $pages[0]->ID );
		$old_changed_float = $this->_microtime_to_float( $last_changed );
		$new_changed_float = $this->_microtime_to_float( gc_cache_get( 'last_changed', 'posts' ) );
		$this->assertGreaterThan( $old_changed_float, $new_changed_float );

		$num_queries  = $gcdb->num_queries;
		$last_changed = gc_cache_get( 'last_changed', 'posts' );

		// num_queries should bump after gc_delete_post() bumps last_changed.
		$pages = get_pages();
		$this->assertCount( 2, $pages );
		$this->assertSame( $last_changed, gc_cache_get( 'last_changed', 'posts' ) );
		$this->assertSame( $num_queries + 1, $gcdb->num_queries );
		foreach ( $pages as $page ) {
			$this->assertInstanceOf( 'GC_Post', $page );
		}
	...
	public function test_cache_should_be_invalidated_by_add_post_meta() {
		...
		$cached_ids = gc_list_pluck( $cached, 'ID' );
		...
		$found_ids = gc_list_pluck( $found, 'ID' );
	...
	public function test_get_pages_include_exclude() {
		...
		$inc_result = gc_list_pluck( $include, 'ID' );
		sort( $inc_result );
		$this->assertSame( $inc, $inc_result );

		$exclude    = get_pages( array( 'exclude' => $exc ) );
		$exc_result = gc_list_pluck( $exclude, 'ID' );
		sort( $exc_result );
	...
	public function test_get_pages_parent() {
		...
		$this->assertSameSets( array( $page_id1 ), gc_list_pluck( $pages, 'ID' ) );
		...
		$this->assertSameSets( array( $page_id2, $page_id4 ), gc_list_pluck( $pages, 'ID' ) );
		...
		$this->assertSameSets( array( $page_id2, $page_id3, $page_id4 ), gc_list_pluck( $pages, 'ID' ) );
	 ...
	 * @ticket 22389
	 */
	public function test_gc_dropdown_pages() {
		self::factory()->post->create_many( 5, array( 'post_type' => 'page' ) );

		preg_match_all( '#<option#', gc_dropdown_pages( 'echo=0' ), $matches );
	...
	public function test_get_pages_hierarchical_and_no_parent() {
		...
		$this->assertSameSets( array( $page_1, $page_2, $page_4, $page_3 ), gc_list_pluck( $pages, 'ID' ) );
	}
	...
	public function test_get_pages_hierarchical_empty_child_of() {
		...
		$found_pages = gc_list_filter( $pages, array( 'post_parent' => 0 ) );

		$this->assertSameSets( array( $page_1, $page_2 ), gc_list_pluck( $found_pages, 'ID' ) );
	}
	...
	public function test_get_pages_non_hierarchical_empty_child_of() {
		...
		$this->assertSameSets( array( $page_1, $page_2, $page_3, $page_4 ), gc_list_pluck( $pages, 'ID' ) );
	}
	...
	public function test_get_pages_hierarchical_non_empty_child_of() {
		...
		$this->assertSameSets( array( $page_3, $page_4, $page_5 ), gc_list_pluck( $pages, 'ID' ) );
	}
	...
	public function test_get_pages_non_hierarchical_non_empty_child_of() {
		...
		// How it should work.
		$found_pages = gc_list_filter( $pages, array( 'post_parent' => $page_1 ) );
		$this->assertSameSets( array( $page_3, $page_5 ), gc_list_pluck( $found_pages, 'ID' ) );

	}

	public function test_gc_list_pages_classes() {
		$type = 'taco';
		register_post_type(
			$type,
			array(
