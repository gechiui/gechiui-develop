 ...
 * @group post
 */
class Tests_Post_Query extends GC_UnitTestCase {
	/**
	 * @group taxonomy
	 */
	public function test_category__and_var() {
		...
		gc_set_post_categories( $post_id, $term_id );
		...
		$this->assertSame( array( $post_id ), gc_list_pluck( $posts, 'ID' ) );
	...
	public function test_empty_category__in() {
		...
		$post_id = self::factory()->post->create();
		gc_set_post_categories( $post_id, $cat_id );
		...
		$tag    = gc_insert_term( 'woo', 'post_tag' );
		$tag_id = $tag['term_id'];
		$slug   = get_tag( $tag_id )->slug;
		gc_set_post_tags( $post_id, $slug );
	...
	public function test_post__in_ordering() {
		...
		$this->assertSame( $ordered, gc_list_pluck( $q->posts, 'ID' ) );
	}
	...
	public function test_post__in_attachment_ordering() {
		...
		$this->assertSame( $ordered, gc_list_pluck( $attached->posts, 'ID' ) );
	}
	...
	public function test_post_name__in_ordering() {
		...
		$this->assertSame( $ordered, gc_list_pluck( $q->posts, 'post_name' ) );
	}
	...
	public function test_post_status() {
		...
		$q1 = new GC_Query( array( 'post_status' => 'any' ) );
		$this->assertStringContainsString( "post_status <> 'auto-draft'", $q1->request );

		$q2 = new GC_Query( array( 'post_status' => 'any, auto-draft' ) );
		$this->assertStringNotContainsString( "post_status <> 'auto-draft'", $q2->request );

		$q3 = new GC_Query( array( 'post_status' => array( 'any', 'auto-draft' ) ) );
	...
	public function test_orderby_array() {
		...
		$this->assertStringContainsString(
			"ORDER BY $gcdb->posts.post_type DESC, $gcdb->posts.post_name ASC",
			$q1->request
		);

		$q2 = new GC_Query( array( 'orderby' => array() ) );
		$this->assertStringNotContainsString( 'ORDER BY', $q2->request );
		$this->assertStringNotContainsString( 'ORDER', $q2->request );

		$q3 = new GC_Query( array( 'post_type' => 'post' ) );
		$this->assertStringContainsString(
			"ORDER BY $gcdb->posts.post_date DESC",
			$q3->request
		);

		$q4 = new GC_Query( array( 'post_type' => 'post' ) );
		$this->assertStringContainsString(
			"ORDER BY $gcdb->posts.post_date DESC",
			$q4->request
		);
	...
	public function test_order() {
		...
		$this->assertStringContainsString(
			"ORDER BY $gcdb->posts.post_type DESC",
			$q1->request
		);
		...
		$this->assertStringContainsString(
			"ORDER BY $gcdb->posts.post_title DESC",
			$q2->request
		);
		...
		$this->assertStringContainsString(
			"ORDER BY $gcdb->posts.post_date ASC",
			$q3->request
		);
	}
	...
	public function test_orderby() {
		...
		$q = new GC_Query( array( 'orderby' => 'rand' ) );
		...
		// This isn't allowed.
		$q2 = new GC_Query( array( 'order' => 'rand' ) );
		...
		$q3 = new GC_Query( array( 'orderby' => 'none' ) );
		...
		$q4 = new GC_Query( array( 'orderby' => false ) );
		...
		$q5 = new GC_Query( array( 'orderby' => array() ) );
	...
	/**
	 * Tests the post_name__in attribute of GC_Query.
	 *
	 * @ticket 33065
	 */
	...
	public function test_posts_pre_query_filter_should_respect_set_found_posts() {
		...
		$q = new GC_Query( array( 'suppress_filters' => false ) );
		...
		$this->assertSame( array( $this->post_id ), gc_list_pluck( $q->posts, 'ID' ) );
	...
	public function test_set_found_posts_fields_ids() {
		...
		$posts = self::factory()->post->create_many( 2, array( 'post_type' => 'gctests_pt' ) );
	...
	public function test_set_found_posts_not_posts_as_an_array( $posts, $expected ) {
		...
		$methd = new ReflectionMethod( 'GC_Query', 'set_found_posts' );
