 ...
 * @group menu
 */
class Tests_Post_Nav_Menu extends GC_UnitTestCase {
	/**
	 * @var int
	 */
	public $menu_id;
	...
	public function set_up() {
		...
		$this->menu_id = gc_create_nav_menu( 'foo' );
	}

	/**
	 * @ticket 32464
	 */
	public function test_gc_nav_menu_empty_container() {
	...
	public function test_gc_get_associated_nav_menu_items() {
		$tag_id    = self::factory()->tag->create();
		...
		$tag_insert = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'taxonomy',
		...
		$cat_insert = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'taxonomy',
		...
		$post_insert = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
		...
		$post_2_insert = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'post_type',
		...
		$page_insert = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'post_type',
		...
		$tag_items = gc_get_associated_nav_menu_items( $tag_id, 'taxonomy', 'post_tag' );
		$this->assertSameSets( array( $tag_insert ), $tag_items );
		$cat_items = gc_get_associated_nav_menu_items( $cat_id, 'taxonomy', 'category' );
		$this->assertSameSets( array( $cat_insert ), $cat_items );
		$post_items = gc_get_associated_nav_menu_items( $post_id );
		$this->assertSameSets( array( $post_insert ), $post_items );
		$post_2_items = gc_get_associated_nav_menu_items( $post_2_id );
		$this->assertSameSets( array( $post_2_insert ), $post_2_items );
		$page_items = gc_get_associated_nav_menu_items( $page_id );
		$this->assertSameSets( array( $page_insert ), $page_items );

		gc_delete_term( $tag_id, 'post_tag' );
		$tag_items = gc_get_associated_nav_menu_items( $tag_id, 'taxonomy', 'post_tag' );
		$this->assertSameSets( array(), $tag_items );

		gc_delete_term( $cat_id, 'category' );
		$cat_items = gc_get_associated_nav_menu_items( $cat_id, 'taxonomy', 'category' );
	...
	public function test_orphan_nav_menu_item() {
		...
		$custom_item_id = gc_update_nav_menu_item(
			0,
			0,
			array(
				'menu-item-type'   => 'custom',
				'menu-item-title'  => 'Gechiui.org',
				'menu-item-url'    => 'http://www.gechiui.com',
				'menu-item-status' => 'publish',
			)
		);

		// Confirm it saved properly.
		$custom_item = gc_setup_nav_menu_item( get_post( $custom_item_id ) );
		$this->assertSame( 'Gechiui.org', $custom_item->title );
		...
		$menu_items  = gc_get_nav_menu_items( $this->menu_id );
		$custom_item = gc_filter_object_list( $menu_items, array( 'db_id' => $custom_item_id ) );
		$custom_item = array_pop( $custom_item );
		$this->assertSame( 'www.GeChiUI.com', $custom_item->title );

	}

	public function test_gc_get_nav_menu_items_with_taxonomy_term() {
		register_taxonomy( 'gctests_tax', 'post', array( 'hierarchical' => true ) );
		...
		$term_menu_item = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'taxonomy',
				'menu-item-object'    => 'gctests_tax',
	 ...
	 * @ticket 13910
	 */
	public function test_gc_get_nav_menu_name() {
		// Register a nav menu location.
		register_nav_menu( 'primary', 'Primary Navigation' );

		// Create a menu with a title.
		$menu = gc_create_nav_menu( 'My Menu' );

		// Assign the menu to the `primary` location.
		$locations            = get_nav_menu_locations();
		$menu_obj             = gc_get_nav_menu_object( $menu );
		$locations['primary'] = $menu_obj->term_id;
		set_theme_mod( 'nav_menu_locations', $locations );

		$this->assertSame( 'My Menu', gc_get_nav_menu_name( 'primary' ) );
	}
	...
	public function test_orderby_name_by_default() {
		...
		for ( $i = 0; $i <= $menus_no; $i++ ) {
			gc_create_nav_menu( rand_str() );
		}

		// This is the expected array of menu names.
		$expected_nav_menus_names = gc_list_pluck(
			get_terms(
				'nav_menu',
				array(
					'hide_empty' => false,
		...
		// And this is what we got when calling gc_get_nav_menus().
		$nav_menus_names = gc_list_pluck( gc_get_nav_menus(), 'name' );
	 ...
	 * @ticket 35324
	 */
	public function test_gc_setup_nav_menu_item_for_post_type_archive() {
		...
		$post_type_archive_item_id = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
		...
		$post_type_archive_item    = gc_setup_nav_menu_item( get_post( $post_type_archive_item_id ) );
	 ...
	 * @ticket 35324
	 */
	public function test_gc_setup_nav_menu_item_for_post_type_archive_no_description() {
	 ...
	 * @ticket 35324
	 */
	public function test_gc_setup_nav_menu_item_for_post_type_archive_custom_description() {
	 ...
	 * @ticket 35324
	 */
	public function test_gc_setup_nav_menu_item_for_unknown_post_type_archive_no_description() {
	 ...
	 * @ticket 19038
	 */
	public function test_gc_setup_nav_menu_item_for_trashed_post() {
		$post_id = self::factory()->post->create(
			array(
				'post_status' => 'trash',
			)
		...
		$menu_item_id = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'post_type',
		...
		$menu_item = gc_setup_nav_menu_item( get_post( $menu_item_id ) );
	 ...
	 * @ticket 35206
	 */
	public function test_gc_nav_menu_whitespace_options() {
		...
		$post_insert2 = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'post_type',
		...
		$post_insert3 = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'post_type',
		...
		$post_insert4 = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'post_type',
	...
	/*
	 * Confirm `gc_nav_menu()` and `Walker_Nav_Menu` passes an $args object to filters.
	 *
	 * `gc_nav_menu()` is unique in that it uses an $args object rather than an array.
	 * This has been the case for some time and should be maintained for reasons of
	 * backward compatibility.
	 *
	 * @ticket 24587
	 */
	public function test_gc_nav_menu_filters_are_passed_args_object() {
		$tag_id = self::factory()->tag->create();
		 ...
		 */
		// In function.
		add_filter( 'pre_gc_nav_menu', array( $this, 'confirm_second_param_args_object' ), 10, 2 );
		add_filter( 'gc_nav_menu_objects', array( $this, 'confirm_second_param_args_object' ), 10, 2 );
		add_filter( 'gc_nav_menu_items', array( $this, 'confirm_second_param_args_object' ), 10, 2 );
		 ...
		 */
		// In function.
		remove_filter( 'pre_gc_nav_menu', array( $this, 'confirm_second_param_args_object' ), 10, 2 );
		remove_filter( 'gc_nav_menu_objects', array( $this, 'confirm_second_param_args_object' ), 10, 2 );
		remove_filter( 'gc_nav_menu_items', array( $this, 'confirm_second_param_args_object' ), 10, 2 );
	...
	public function test_class_not_applied_to_taxonomies_with_same_id_as_front_page_item() {
		...
		$gcdb->update( $gcdb->posts, array( 'ID' => $new_id ), array( 'ID' => $page_id ) );
		$gcdb->update( $gcdb->terms, array( 'term_id' => $new_id ), array( 'term_id' => $tag_id ) );
		$gcdb->update( $gcdb->term_taxonomy, array( 'term_id' => $new_id ), array( 'term_id' => $tag_id ) );
	...
	/**
	 * Test _gc_delete_customize_changeset_dependent_auto_drafts.
	 *
	 * @covers ::_gc_delete_customize_changeset_dependent_auto_drafts
	 */
	public function test_gc_delete_customize_changeset_dependent_auto_drafts() {
		...
		gc_delete_post( $gc_customize->changeset_post_id(), true );
	...
	public function test_parent_ancestor_for_post_archive() {
		...
		$first_menu_id = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'post_type',
		...
		$second_menu_id = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'post_type',
	...
	/**
	 * Provides IRI matching data for _gc_menu_item_classes_by_context() test.
	 */
	...
	public function test_class_applied_to_privacy_policy_page_item() {
		...
		update_option( 'gc_page_for_privacy_policy', $page_id );
		...
		delete_option( 'gc_page_for_privacy_policy' );
	...
	public function data_trim_url_for_custom_item() {
		return array(
			array( 'https://www.gechiui.com ', 'https://www.gechiui.com' ),
			array( ' https://www.gechiui.com', 'https://www.gechiui.com' ),
		);
	}

	/**
	 * Tests `gc_update_nav_menu_item()` with special characters in a category name.
	 *
	 ...
	 * @ticket 48011
	 */
	public function test_gc_update_nav_menu_item_with_special_characters_in_category_name() {
		...
		$category_item_id = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
				'menu-item-type'      => 'taxonomy',
	 ...
	 * @ticket 52189
	 */
	public function test_gc_update_nav_menu_item_with_post_date() {
