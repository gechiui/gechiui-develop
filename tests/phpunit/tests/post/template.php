 ...
 * @group template
 */
class Tests_Post_Template extends GC_UnitTestCase {

	public function test_gc_link_pages() {
		...
		$page2     = _gc_link_page( 2 );
		$page3     = _gc_link_page( 3 );
	...
	public function test_gc_dropdown_pages() {
		$none = gc_dropdown_pages( array( 'echo' => 0 ) );
...
LINEAGE;
		...
		$output = gc_dropdown_pages( array( 'echo' => 0 ) );
...
NO;
	 ...
	 * @ticket 12494
	 */
	public function test_gc_dropdown_pages_value_field_should_default_to_ID() {
	 ...
	 * @ticket 12494
	 */
	public function test_gc_dropdown_pages_value_field_ID() {
		$p = self::factory()->post->create(
			array(
				'post_type' => 'page',
			)
	 ...
	 * @ticket 12494
	 */
	public function test_gc_dropdown_pages_value_field_post_name() {
		$p = self::factory()->post->create(
			array(
				'post_type' => 'page',
				'post_name' => 'foo',
	 ...
	 * @ticket 12494
	 */
	public function test_gc_dropdown_pages_value_field_should_fall_back_on_ID_when_an_invalid_value_is_provided() {
		$p = self::factory()->post->create(
			array(
				'post_type' => 'page',
				'post_name' => 'foo',
	 ...
	 * @ticket 30082
	 */
	public function test_gc_dropdown_pages_should_not_contain_class_attribute_when_no_class_is_passed() {
		$p = self::factory()->post->create(
			array(
				'post_type' => 'page',
				'post_name' => 'foo',
	 ...
	 * @ticket 30082
	 */
	public function test_gc_dropdown_pages_should_obey_class_parameter() {
		$p = self::factory()->post->create(
			array(
				'post_type' => 'page',
				'post_name' => 'foo',
	...
	public function test_get_page_template_slug_by_id() {
		...
		update_post_meta( $page_id, '_gc_page_template', 'default' );
		$this->assertSame( '', get_page_template_slug( $page_id ) );

		update_post_meta( $page_id, '_gc_page_template', 'example.php' );
	...
	public function test_get_page_template_slug_non_page() {
		...
		update_post_meta( $post_id, '_gc_page_template', 'example.php' );
	 ...
	 * @ticket 33974
	 */
	public function test_gc_page_menu_gc_nav_menu_fallback() {
		$pages = self::factory()->post->create_many( 3, array( 'post_type' => 'page' ) );

		// No menus + gc_nav_menu() falls back to gc_page_menu().
		$menu = gc_nav_menu( array( 'echo' => false ) );
		...
		// No menus + gc_nav_menu() falls back to gc_page_menu(), this time without a container.
		$menu = gc_nav_menu(
			array(
				'echo'      => false,
				'container' => false,
		...
		// No menus + gc_nav_menu() falls back to gc_page_menu(), this time without white-space.
		$menu = gc_nav_menu(
			array(
				'echo'         => false,
