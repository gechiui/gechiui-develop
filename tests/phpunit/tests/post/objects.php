 ...
 * @group post
 */
class Tests_Post_Objects extends GC_UnitTestCase {
	...
	public function test_get_post_category_property() {
		...
		gc_set_post_categories( $post_id, array( $term1['term_id'], $term2['term_id'], $term3['term_id'] ) );
	...
	public function test_get_tags_input_property() {
		...
		gc_set_post_tags( $post_id, 'Foo, Bar, Baz' );
		$this->assertIsArray( $post->tags_input );
	...
	public function test_get_page_template_property() {
		...
		update_post_meta( $post_id, '_gc_page_template', 'foo.php' );
		$template = get_post_meta( $post->ID, '_gc_page_template', true );
	...
	public function test_get_post_filter() {
		...
		// Pass a js filtered GC_Post to get_post() with the filter set to raw.
	...
	public function test_get_post_cache() {
		...
		$post = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->posts WHERE ID = %d LIMIT 1", $id ) );
		$post = get_post( $post );
		$this->assertEmpty( gc_cache_get( $id, 'posts' ) );

		// get_post( GC_Post ) should not prime the cache.
		get_post( $post );
		$this->assertEmpty( gc_cache_get( $id, 'posts' ) );

		// get_post( ID ) should prime the cache.
		get_post( $post->ID );
		$this->assertNotEmpty( gc_cache_get( $id, 'posts' ) );
	}
}
