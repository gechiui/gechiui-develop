 ...
 * @group post
 */
class Tests_Post_IsPostStatusViewable extends GC_UnitTestCase {
	...
	public static function gcTearDownAfterClass() {
		global $gc_post_statuses;
		unset( $gc_post_statuses['gc_tests_ps'] );
	}
	...
	public function test_custom_post_statuses( $cps_args, $expected ) {
		...
		$this->assertSame( $expected, is_post_status_viewable( 'gc_tests_ps' ) );
		// Test status passed as object.
		$this->assertSame( $expected, is_post_status_viewable( get_post_status_object( 'gc_tests_ps' ) ) );
	}
	...
	public function test_sanitize_key_not_run() {
		...
		$this->assertTrue( is_post_status_viewable( 'gc_tests_ps' ) );
		$this->assertTrue( is_post_status_viewable( get_post_status_object( 'gc_tests_ps' ) ) );

		// Unsanitized key should return false.
		$this->assertFalse( is_post_status_viewable( 'GC_tests_ps' ) );
		$this->assertFalse( is_post_status_viewable( get_post_status_object( 'GC_tests_ps' ) ) );
	}
}
