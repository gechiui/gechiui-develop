...
/**
 * test gc-includes/theme.php
 *
 * @group themes
 */
class Tests_Theme extends GC_UnitTestCase {
	...
	public function set_up() {
		...
		$backup_gc_theme_directories = $gc_theme_directories;
		$gc_theme_directories        = array( GC_CONTENT_DIR . '/themes' );
	...
	public function tear_down() {
		...
		$gc_theme_directories = $this->gc_theme_directories;
	...
	public function test_gc_get_themes_default() {
		$themes = gc_get_themes();
		$this->assertInstanceOf( 'GC_Theme', $themes[ $this->theme_slug ] );
		$this->assertSame( $this->theme_name, $themes[ $this->theme_slug ]->get( 'Name' ) );

		$single_theme = gc_get_theme( $this->theme_slug );
	...
	public function test_get_themes_default() {
		$themes = get_themes();
		$this->assertInstanceOf( 'GC_Theme', $themes[ $this->theme_name ] );
	...
	public function test_get_theme() {
		...
		foreach ( array_keys( $themes ) as $name ) {
			...
			// GC_Theme implements ArrayAccess. Even ArrayObject returns false for is_array().
			$this->assertFalse( is_array( $theme ) );
			$this->assertInstanceOf( 'GC_Theme', $theme );
			$this->assertSame( $theme, $themes[ $name ] );
		}
	}

	public function test_gc_get_theme() {
		$themes = gc_get_themes();
		foreach ( $themes as $theme ) {
			$this->assertInstanceOf( 'GC_Theme', $theme );
			$this->assertFalse( $theme->errors() );
			$_theme = gc_get_theme( $theme->get_stylesheet() );
			// This primes internal GC_Theme caches for the next assertion (headers_sanitized, textdomain_loaded).
	...
	public function test_get_themes_contents() {
		...
		foreach ( $themes as $k => $theme ) {
			...
			if ( empty( $theme['Author'] ) || false === strpos( $theme['Author'], 'GeChiUI' ) ) {
				continue;
			}
			...
			// Make the tests work both for GeChiUI 2.8.5 and GeChiUI 2.9-rare.
			$dir = isset( $theme['Theme Root'] ) ? '' : GC_CONTENT_DIR;
	...
	public function test_gc_get_theme_contents() {
		$theme = gc_get_theme( $this->theme_slug );
		...
		$this->assertSame( GC_CONTENT_DIR . '/themes/' . $this->theme_slug, $theme->get_stylesheet_directory(), 'get_stylesheet_directory' );
		$this->assertSame( GC_CONTENT_DIR . '/themes/' . $this->theme_slug, $theme->get_template_directory(), 'get_template_directory' );
	...
	public function test_default_theme_in_default_theme_list() {
		$latest_default_theme = GC_Theme::get_core_default_theme();
	...
	public function test_default_themes_have_textdomain() {
		foreach ( $this->default_themes as $theme ) {
			if ( gc_get_theme( $theme )->exists() ) {
				$this->assertSame( $theme, gc_get_theme( $theme )->get( 'TextDomain' ) );
			}
		}
	}
	...
	public function test_year_in_readme() {
		...
		foreach ( $this->default_themes as $theme ) {
			$gc_theme = gc_get_theme( $theme );

			$path_to_readme_txt = $gc_theme->get_theme_root() . '/' . $gc_theme->get_stylesheet() . '/readme.txt';
			$this->assertFileExists( $path_to_readme_txt );
			$readme    = file_get_contents( $path_to_readme_txt );
			$this_year = gmdate( 'Y' );

			preg_match( '#Copyright (\d+) www.GeChiUI.com#', $readme, $matches );
			if ( $matches ) {
				$this->assertSame( $this_year, trim( $matches[1] ), "Bundled themes readme.txt's year needs to be updated to $this_year." );
			}

			preg_match( '#Copyright 20\d\d-(\d+) www.GeChiUI.com#', $readme, $matches );
			if ( $matches ) {
				$this->assertSame( $this_year, trim( $matches[1] ), "Bundled themes readme.txt's year needs to be updated to $this_year." );
			}
		}
	...
	public function test_extra_theme_headers() {
		$gc_theme = gc_get_theme( $this->theme_slug );
		$this->assertNotEmpty( $gc_theme->get( 'License' ) );
		$path_to_style_css = $gc_theme->get_theme_root() . '/' . $gc_theme->get_stylesheet() . '/style.css';
		...
		$this->assertSame( $theme_data['License'], $gc_theme->get( 'License' ) );
	}
		...
		for ( $i = 0; $i < 3; $i++ ) {
			foreach ( $themes as $name => $theme ) {
				...
				*/
				...
				// nb: This probably doesn't run because GC_INSTALLING is defined.
	...
	/**
	 * Test _gc_keep_alive_customize_changeset_dependent_auto_drafts.
	 *
	 * @covers ::_gc_keep_alive_customize_changeset_dependent_auto_drafts
	 */
	public function test_gc_keep_alive_customize_changeset_dependent_auto_drafts() {
		...
		// to keep them from from being garbage collected by gc_delete_auto_drafts().
		$gc_customize->save_changeset_post(
			array(
				'data' => $data,
			)
		);
		$this->assertSame( get_post( $gc_customize->changeset_post_id() )->post_date, get_post( $nav_created_post_ids[0] )->post_date );
		$this->assertSame( get_post( $gc_customize->changeset_post_id() )->post_date, get_post( $nav_created_post_ids[1] )->post_date );
		...
		$gc_customize->trash_changeset_post( $gc_customize->changeset_post_id() );
	 ...
	 *
	 * @covers ::_add_default_theme_supports
	 * @covers ::gc_should_load_separate_core_block_assets
	 */
	public function test_block_theme_should_load_separate_core_block_assets_by_default() {
		...
		$this->assertFalse(
			gc_should_load_separate_core_block_assets(),
	...
	private function helper_requires_block_theme() {
		...
		if ( ! gc_get_theme( $block_theme )->exists() ) {
		...
		if ( gc_get_theme()->stylesheet !== $block_theme ) {
