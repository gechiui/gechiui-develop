<?php
/**
 * A set of unit tests for functions in gc-includes/general-template.php
 *
 * @group general
 * @group template
 * @group site_icon
...
class Tests_General_Template extends GC_UnitTestCase {
	protected $gc_site_icon;
	public $site_icon_id;
	...
	public function set_up() {
		...
		$this->gc_site_icon = new GC_Site_Icon();
	}
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_site_icon() {
		$this->expectOutputString( '' );
		gc_site_icon();
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_site_icon_with_filter() {
		$this->expectOutputString( '' );
		gc_site_icon();
	 ...
	 * @covers ::gc_site_icon
	 */
	public function test_customize_preview_gc_site_icon_empty() {
		global $gc_customize;
	 ...
	 * @covers ::gc_site_icon
	 */
	public function test_customize_preview_gc_site_icon_dirty() {
		global $gc_customize;
		gc_set_current_user( $this->factory()->user->create( array( 'role' => 'administrator' ) ) );
		...
		$gc_customize->set_post_value( 'site_icon', $attachment_id );
		$gc_customize->get_setting( 'site_icon' )->preview();
		$output = array(
			sprintf( '<link rel="icon" href="%s" sizes="32x32" />', esc_url( gc_get_attachment_image_url( $attachment_id, 32 ) ) ),
			sprintf( '<link rel="icon" href="%s" sizes="192x192" />', esc_url( gc_get_attachment_image_url( $attachment_id, 192 ) ) ),
			sprintf( '<link rel="apple-touch-icon" href="%s" />', esc_url( gc_get_attachment_image_url( $attachment_id, 180 ) ) ),
			sprintf( '<meta name="msapplication-TileImage" content="%s" />', esc_url( gc_get_attachment_image_url( $attachment_id, 270 ) ) ),
			'',
		);
		$output = implode( "\n", $output );
		$this->expectOutputString( $output );
	...
	private function set_site_icon() {
		if ( ! $this->site_icon_id ) {
			add_filter( 'intermediate_image_sizes_advanced', array( $this->gc_site_icon, 'additional_sizes' ) );
			$this->insert_attachment();
			remove_filter( 'intermediate_image_sizes_advanced', array( $this->gc_site_icon, 'additional_sizes' ) );
		}
	...
	public function test_get_custom_logo_returns_logo_when_called_for_other_site_with_custom_logo_set() {
		...
		$image_alt = get_post_meta( $this->custom_logo_id, '_gc_attachment_image_alt', true );
		if ( empty( $image_alt ) ) {
			$custom_logo_attr['alt'] = get_bloginfo( 'name', 'display' );
		}

		$home_url = get_home_url( $blog_id, '/' );
		$image    = gc_get_attachment_image( $this->custom_logo_id, 'full', false, $custom_logo_attr );
		restore_current_blog();
	...
	public function test_the_custom_logo() {
		...
		$this->expectOutputString( '<a href="http://' . GC_TESTS_DOMAIN . '/" class="custom-logo-link" rel="home">' . $image . '</a>' );
		the_custom_logo();
	}
	...
	public function test_the_custom_logo_with_alt() {
		...
		update_post_meta( $this->custom_logo_id, '_gc_attachment_image_alt', $image_alt );

		$image = gc_get_attachment_image(
			$this->custom_logo_id,
			'full',
			false,
			array(
	...
	public function test_get_site_icon_url_preserves_switched_state() {
		...
		$expected = $GLOBALS['_gc_switched_stack'];

		get_site_icon_url( 512, '', $blog_id );

		$result = $GLOBALS['_gc_switched_stack'];
