 ...
 * @covers ::_mb_strlen
 */
class Tests_Compat_mbStrlen extends GC_UnitTestCase {

	/**
	 * Test that mb_strlen() is always available (either from PHP or GC).
	 */
	...
	public function test_mb_strlen_via_regex( $string, $expected_character_length ) {
		_gc_can_use_pcre_u( false );
		$this->assertSame( $expected_character_length, _mb_strlen( $string, 'UTF-8' ) );
		_gc_can_use_pcre_u( 'reset' );
	}
