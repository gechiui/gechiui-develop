 ...
 *
 * @covers ::gc_read_image_metadata
 */
class Tests_Image_Meta extends GC_UnitTestCase {

	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		require_once DIR_TESTROOT . '/includes/class-gc-test-stream.php';
		stream_wrapper_register( 'testimagemeta', 'GC_Test_Stream' );

		GC_Test_Stream::$data = array(
			'gc_read_image_metadata' => array(
	...
	public function test_exif_d70() {
		...
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/2004-07-22-DSC_0008.jpg' );
	...
	public function test_exif_d70_mf() {
		...
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/2007-06-17DSC_4173.JPG' );
	...
	public function test_exif_d70_iptc() {
		...
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/2004-07-22-DSC_0007.jpg' );
	...
	public function test_exif_fuji() {
		...
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/a2-small.jpg' );
	...
	public function test_exif_error() {
		// https://core.trac.gechiui.com/ticket/6571
		// This triggers a warning mesage when reading the Exif block.
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/waffles.jpg' );
	...
	public function test_exif_no_data() {
		...
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/canola.jpg' );
	...
	public function test_utf8_iptc_tags() {
		...
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/test-image-iptc.jpg' );

		$this->assertSame( 'This is a comment. / Это комментарий. / Βλέπετε ένα σχόλιο.', $out['caption'] );
	}

	/**
	 * gc_read_image_metadata() should return false if the image file doesn't exist.
	 */
	public function test_missing_image_file() {
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/404_image.png' );
	...
	public function test_exif_keywords() {
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/33772.jpg' );
	...
	public function test_stream( $file, $expected ) {
		$actual = gc_read_image_metadata( $file );
	...
	public function data_stream() {
		return array(
			'Orientation only metadata'                => array(
				'file'     => 'testimagemeta://gc_read_image_metadata/image1.jpg',
				'metadata' => array(
					'aperture'          => '0',
					'credit'            => '',
					'camera'            => '',
			...
			'Exif from a Nikon D70 with IPTC data added later' => array(
				'file'     => 'testimagemeta://gc_read_image_metadata/image2.jpg',
				'metadata' => array(
					'aperture'          => '6.3',
					'credit'            => 'IPTC Creator',
					'camera'            => 'NIKON D70',
			...
			'Exif from a DMC-LX2 camera with keywords' => array(
				'file'     => 'testimagemeta://gc_read_image_metadata/image3.jpg',
				'metadata' => array(
					'aperture'          => '8',
	...
	public function test_exif_unexpected_data() {
		...
		$out = gc_read_image_metadata( DIR_TESTDATA . '/images/sugarloaf-mountain.jpg' );
