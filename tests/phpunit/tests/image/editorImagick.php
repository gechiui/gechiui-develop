...
/**
 * Test the GC_Image_Editor_Imagick class
 *
 * @group image
 * @group media
 * @group gc-image-editor-imagick
 */
require_once __DIR__ . '/base.php';

class Tests_Image_Editor_Imagick extends GC_Image_UnitTestCase {
	...
	public function test_supports_mime_type() {
		$imagick_image_editor = new GC_Image_Editor_Imagick( null );
	...
	public function test_resize() {
		...
		$imagick_image_editor = new GC_Image_Editor_Imagick( $file );
	...
	/**
	 * Test the image created with GC_Image_Editor_Imagick preserves alpha when resizing
	 *
	 * @ticket 24871
	 */
	public function test_image_preserves_alpha_on_resize() {
		...
		$editor = new GC_Image_Editor_Imagick( $file );
		$editor->load();
		$editor->resize( 5, 5 );
	...
	/**
	 * Test the image created with GC_Image_Editor_Imagick preserves alpha with no resizing etc
	 *
	 * @ticket 24871
	 */
	...
	public function test_image_preserves_alpha_on_rotate() {
		...
		$image_editor = new GC_Image_Editor_Imagick( $save_to_file );
		$image_editor->load();
	...
	/**
	 * Test GC_Image_Editor_Imagick handles extension-less images
	 *
	 * @ticket 39195
	 */
	public function test_image_non_existent_extension() {
		$image_editor = new GC_Image_Editor_Imagick( DIR_TESTDATA . '/images/test-image-no-extension' );
	...
	public function test_remove_orientation_data_on_rotate() {
		...
		$data = gc_read_image_metadata( $file );
		...
		$temp_file = gc_tempnam( $file );
		$image     = gc_get_image_editor( $file );

		// Test a value that would not lead back to 1, as GC is resetting the value to 1 manually.
		$image->rotate( 90 );
		$ret = $image->save( $temp_file, 'image/jpeg' );

		$data = gc_read_image_metadata( $ret['path'] );
		...
		// Remove both the generated file ending in .tmp and tmp.jpg due to gc_tempnam().
		unlink( $temp_file );
		unlink( $ret['path'] );
	}
	...
	public function test_streams() {
		stream_wrapper_register( 'gctest', 'GC_Test_Stream' );
		...
		$file                 = 'gctest://Tests_Image_Editor_Imagick/read.jpg';
		$imagick_image_editor = new GC_Image_Editor_Imagick( $file );

		$ret = $imagick_image_editor->load();
		$this->assertNotWPError( $ret );

		$temp_file = 'gctest://Tests_Image_Editor_Imagick/write.jpg';
