<?php
/**
 * Tests for the GC_Site_Icon class.
 *
 * @group site_icon
 */
...
class Tests_GC_Site_Icon extends GC_UnitTestCase {
	...
	public function test_intermediate_image_sizes() {
		$image_sizes = $this->gc_site_icon->intermediate_image_sizes( array() );

		$sizes = array();
		foreach ( $this->gc_site_icon->site_icon_sizes as $size ) {
	...
	public function test_intermediate_image_sizes_with_filter() {
		...
		// Remove custom size.
		unset( $this->gc_site_icon->site_icon_sizes[ array_search( 321, $this->gc_site_icon->site_icon_sizes, true ) ] );
	...
	public function test_additional_sizes() {
		$image_sizes = $this->gc_site_icon->additional_sizes( array() );
	...
	public function test_create_attachment_object() {
		...
		$cropped       = str_replace( gc_basename( $parent_url ), 'cropped-test-image.jpg', $parent_url );

		$object = $this->gc_site_icon->create_attachment_object( $cropped, $attachment_id );
	...
	public function test_insert_cropped_attachment() {
		...
		$object     = $this->gc_site_icon->create_attachment_object( $cropped, $attachment_id );
		$cropped_id = $this->gc_site_icon->insert_attachment( $object, $cropped );
	...
	public function test_get_post_metadata() {
		...
		$this->gc_site_icon->get_post_metadata( '', $attachment_id, '_some_post_meta', true );
		$this->assertFalse( has_filter( 'intermediate_image_sizes', array( $this->gc_site_icon, 'intermediate_image_sizes' ) ) );

		$this->gc_site_icon->get_post_metadata( '', $attachment_id, '_gc_attachment_backup_sizes', true );
		$this->assertSame( 10, has_filter( 'intermediate_image_sizes', array( $this->gc_site_icon, 'intermediate_image_sizes' ) ) );
