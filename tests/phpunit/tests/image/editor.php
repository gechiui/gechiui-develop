...
/**
 * Test the GC_Image_Editor base class
 *
 * @group image
 * @group media
 */
class Tests_Image_Editor extends GC_Image_UnitTestCase {
	public $editor_engine = 'GC_Image_Editor_Mock';
	...
	/**
	 * Test gc_get_image_editor() where load returns true
	 *
	 * @ticket 6821
	 */
	public function test_get_editor_load_returns_true() {
		$editor = gc_get_image_editor( DIR_TESTDATA . '/images/canola.jpg' );

		$this->assertInstanceOf( 'GC_Image_Editor_Mock', $editor );
	}

	/**
	 * Test gc_get_image_editor() where load returns false
	 *
	 * @ticket 6821
	 */
	public function test_get_editor_load_returns_false() {
		GC_Image_Editor_Mock::$load_return = new GC_Error();

		$editor = gc_get_image_editor( DIR_TESTDATA . '/images/canola.jpg' );

		$this->assertInstanceOf( 'GC_Error', $editor );

		GC_Image_Editor_Mock::$load_return = true;
	}
	...
	public function test_set_quality() {
		...
		add_filter( 'gc_editor_set_quality', $func_100_percent );
		$this->assertSame( 82, $editor->get_quality() );
		...
		// Ensure gc_editor_set_quality filter applies if it exists before editor instantiation.
		$this->assertSame( 100, $editor->get_quality() );

		// Clean up.
		remove_filter( 'gc_editor_set_quality', $func_100_percent );
	}
	...
	public function test_set_quality_with_image_conversion() {
		$editor = gc_get_image_editor( DIR_TESTDATA . '/images/test-image.png' );
		...
		add_filter( 'gc_editor_set_quality', array( $this, 'image_editor_change_quality' ), 10, 2 );

		// Get a new editor to clear quality state.
		$editor = gc_get_image_editor( DIR_TESTDATA . '/images/test-image.jpg' );
		...
		remove_filter( 'gc_editor_set_quality', array( $this, 'image_editor_change_quality' ) );
	}
	...
	public function test_generate_filename() {
		...
		$this->assertSame( 'canola-100x50.jpg', gc_basename( $editor->generate_filename() ) );

		// Test with a suffix only.
		$this->assertSame( 'canola-new.jpg', gc_basename( $editor->generate_filename( 'new' ) ) );
		...
		// Test with a suffix only.
		$this->assertSame( 'canola-100x50.png', gc_basename( $editor->generate_filename( null, null, 'png' ) ) );
	...
	/**
	 * Test gc_get_webp_info.
	 *
	 * @ticket 35725
	 * @dataProvider _test_gc_get_webp_info
	 *
	 */
	public function test_gc_get_webp_info( $file, $expected ) {
		...
		$file_data = gc_get_webp_info( $file );
		$this->assertSame( $expected, $file_data );
	}

	/**
	 * Data provider for test_gc_get_webp_info().
	 */
	public function _test_gc_get_webp_info() {
		return array(
			// Standard JPEG.
			array(
