 ...
 * @group upload
 */
class Tests_Image_Functions extends GC_UnitTestCase {
	...
	public function set_up() {
		...
		$folder = get_temp_dir() . 'gechiui-gsoc-flyer*.*';
	 ...
	 */
	public function test_gc_save_image_file() {
		...
		foreach ( $classes as $class ) {
			...
			foreach ( $mime_types as $mime_type ) {
				...
				$file = gc_tempnam();
				$ret  = gc_save_image_file( $file, $img, $mime_type, 1 );
	...
	public function test_inferred_mime_types() {
		...
		foreach ( $classes as $class ) {
			...
			$img = gc_get_image_editor( DIR_TESTDATA . '/images/canola.jpg' );
			...
			foreach ( $mime_types as $ext => $mime_type ) {
				...
				$file = gc_unique_filename( $temp, uniqid() . ".$ext" );
	 ...
	 *
	 * @ticket 17814
	 * @expectedDeprecated gc_load_image
	 */
	public function test_load_directory() {

		// First, test with deprecated gc_load_image function.
		$editor1 = gc_load_image( DIR_TESTDATA );
		$this->assertIsString( $editor1 );

		$editor2 = gc_get_image_editor( DIR_TESTDATA );
		$this->assertInstanceOf( 'GC_Error', $editor2 );
		...
		foreach ( $classes as $class ) {
			...
			$this->assertInstanceOf( 'GC_Error', $loaded );
	...
	private function get_image_editor_engine_classes() {
		$classes = array( 'GC_Image_Editor_GD', 'GC_Image_Editor_Imagick' );
		...
		if ( empty( $classes ) ) {
			$this->markTestSkipped( 'Image editor engines GC_Image_Editor_GD and GC_Image_Editor_Imagick are not supported on this system.' );
		}
	 ...
	 */
	public function test_gc_crop_image_file() {
		$file = gc_crop_image(
		...
		$image = gc_get_image_editor( $file );
		$size  = $image->get_size();
	 ...
	 */
	public function test_gc_crop_image_url() {
		$file = gc_crop_image(
			'https://asdftestblog1.files.gechiui.com/2008/04/canola.jpg',
			0,
			0,
			100,
			100,
		...
		if ( is_gc_error( $file ) && $file->get_error_code() === 'invalid_image' ) {
			$this->markTestSkipped( 'Tests_Image_Functions::test_gc_crop_image_url() cannot access remote image.' );
		}
	...
	public function test_gc_crop_image_file_not_exist() {
		$file = gc_crop_image(
		...
		$this->assertInstanceOf( 'GC_Error', $file );
	}

	/**
	 * @requires extension openssl
	 */
	public function test_gc_crop_image_url_not_exist() {
		$file = gc_crop_image(
			'https://asdftestblog1.files.gechiui.com/2008/04/canoladoesnotexist.jpg',
			0,
			0,
			100,
			100,
	 ...
	 * @ticket 23325
	 */
	public function test_gc_crop_image_error_on_saving() {
		GC_Image_Editor_Mock::$save_return = new GC_Error();
		add_filter( 'gc_image_editors', array( $this, 'mock_image_editor' ) );
		...
		remove_filter( 'gc_image_editors', array( $this, 'mock_image_editor' ) );
		GC_Image_Editor_Mock::$save_return = array();
	}

	/**
	 * @ticket 31050
	 */
	public function test_gc_generate_attachment_metadata_pdf() {
		if ( ! gc_image_editor_supports( array( 'mime_type' => 'application/pdf' ) ) ) {
			$this->markTestSkipped( 'Rendering PDFs is not supported on this system.' );
		}

		$orig_file = DIR_TESTDATA . '/images/gechiui-gsoc-flyer.pdf';
		$test_file = get_temp_dir() . 'gechiui-gsoc-flyer.pdf';
		copy( $orig_file, $test_file );

		$editor = gc_get_image_editor( $test_file );
		...
		$expected = array(
			'sizes' => array(
				'full'      => array(
					'file'      => 'gechiui-gsoc-flyer-pdf.jpg',
					'width'     => 1088,
					'height'    => 1408,
					'mime-type' => 'image/jpeg',
				),
				'medium'    => array(
					'file'      => 'gechiui-gsoc-flyer-pdf-232x300.jpg',
					'width'     => 232,
					'height'    => 300,
					'mime-type' => 'image/jpeg',
				),
				'large'     => array(
					'file'      => 'gechiui-gsoc-flyer-pdf-791x1024.jpg',
					'width'     => 791,
					'height'    => 1024,
					'mime-type' => 'image/jpeg',
				),
				'thumbnail' => array(
					'file'      => 'gechiui-gsoc-flyer-pdf-116x150.jpg',
					'width'     => 116,
					'height'    => 150,
					'mime-type' => 'image/jpeg',
				),
		...
		$metadata = gc_generate_attachment_metadata( $attachment_id, $test_file );
	...
	public function test_crop_setting_for_pdf() {
		...
		$expected = array(
			'sizes' => array(
				...
				'medium'    => array(
					'file'      => 'gechiui-gsoc-flyer-pdf-300x300.jpg',
					'width'     => 300,
					'height'    => 300,
					'mime-type' => 'image/jpeg',
				),
	...
	public function test_fallback_intermediate_image_sizes() {
		...
		$expected = array(
			'file'      => 'gechiui-gsoc-flyer-pdf-77x100.jpg',
			'width'     => 77,
			'height'    => 100,
			'mime-type' => 'image/jpeg',
		);
	...
	public function test_pdf_preview_doesnt_overwrite_existing_jpeg() {
		...
		copy( DIR_TESTDATA . '/images/gechiui-gsoc-flyer.pdf', $pdf_path );

		$editor = gc_get_image_editor( $pdf_path );
		if ( is_gc_error( $editor ) ) {
			$this->markTestSkipped( $editor->get_error_message() );
		}
		...
		$metadata     = gc_generate_attachment_metadata( $attachment_id, $pdf_path );
	...
	/**
	 * Test for gc_exif_frac2dec verified that it properly handles edge cases
	 ...
	 *
	 * @ticket 54385
	 * @dataProvider data_gc_exif_frac2dec
	 *
	 * @covers ::gc_exif_frac2dec
	 */
	public function test_gc_exif_frac2dec( $fraction, $expect ) {
		$this->assertSame( $expect, gc_exif_frac2dec( $fraction ) );
	}

	/**
	 * Data provider for testing `gc_exif_frac2dec()`.
	 *
	 * @return array
	 */
	public function data_gc_exif_frac2dec() {
		return array(
