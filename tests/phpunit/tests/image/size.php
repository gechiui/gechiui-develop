 ...
 * @group upload
 */
class Tests_Image_Size extends GC_UnitTestCase {

	public function test_constrain_dims_zero() {
		// No constraint - should have no effect.
		$out = gc_constrain_dimensions( 640, 480, 0, 0 );
		$this->assertSame( array( 640, 480 ), $out );

		$out = gc_constrain_dimensions( 640, 480 );
		$this->assertSame( array( 640, 480 ), $out );

		$out = gc_constrain_dimensions( 0, 0, 0, 0 );
		$this->assertSame( array( 0, 0 ), $out );

		$out = gc_constrain_dimensions( 465, 700, 177, 177 );
	...
	public function test_constrain_dims_smaller() {
		...
		$out = gc_constrain_dimensions( 500, 600, 1024, 768 );
		$this->assertSame( array( 500, 600 ), $out );

		$out = gc_constrain_dimensions( 500, 600, 0, 768 );
		$this->assertSame( array( 500, 600 ), $out );

		$out = gc_constrain_dimensions( 500, 600, 1024, 0 );
		$this->assertSame( array( 500, 600 ), $out );
	}

	public function test_constrain_dims_equal() {
		// Image size is equal to the constraint - no effect.
		$out = gc_constrain_dimensions( 1024, 768, 1024, 768 );
		$this->assertSame( array( 1024, 768 ), $out );

		$out = gc_constrain_dimensions( 1024, 768, 0, 768 );
		$this->assertSame( array( 1024, 768 ), $out );

		$out = gc_constrain_dimensions( 1024, 768, 1024, 0 );
		$this->assertSame( array( 1024, 768 ), $out );
	}

	public function test_constrain_dims_larger() {
		// Image size is larger than the constraint - result should be constrained.
		$out = gc_constrain_dimensions( 1024, 768, 500, 600 );
		$this->assertSame( array( 500, 375 ), $out );

		$out = gc_constrain_dimensions( 1024, 768, 0, 600 );
		$this->assertSame( array( 800, 600 ), $out );

		$out = gc_constrain_dimensions( 1024, 768, 500, 0 );
		$this->assertSame( array( 500, 375 ), $out );

		// Also try a portrait oriented image.
		$out = gc_constrain_dimensions( 300, 800, 500, 600 );
		$this->assertSame( array( 225, 600 ), $out );

		$out = gc_constrain_dimensions( 300, 800, 0, 600 );
		$this->assertSame( array( 225, 600 ), $out );

		$out = gc_constrain_dimensions( 300, 800, 200, 0 );
	...
	public function test_constrain_dims_boundary() {
		...
		$out = gc_constrain_dimensions( 1024, 768, 500, 800 );
		$this->assertSame( array( 500, 375 ), $out );

		$out = gc_constrain_dimensions( 1024, 768, 2000, 700 );
		$this->assertSame( array( 933, 700 ), $out );

		// Portrait.
		$out = gc_constrain_dimensions( 768, 1024, 800, 500 );
		$this->assertSame( array( 375, 500 ), $out );

		$out = gc_constrain_dimensions( 768, 1024, 2000, 700 );
		$this->assertSame( array( 525, 700 ), $out );
	}

	/**
	 * @expectedDeprecated gc_shrink_dimensions
	 */
	public function test_shrink_dimensions_default() {
		$out = gc_shrink_dimensions( 640, 480 );
		$this->assertSame( array( 128, 96 ), $out );

		$out = gc_shrink_dimensions( 480, 640 );
	...
	public function test_shrink_dimensions_smaller() {
		// Image size is smaller than the constraint - no effect.
		$out = gc_shrink_dimensions( 500, 600, 1024, 768 );
		$this->assertSame( array( 500, 600 ), $out );

		$out = gc_shrink_dimensions( 600, 500, 1024, 768 );
	...
	public function test_shrink_dimensions_equal() {
		// Image size is equal to the constraint - no effect.
		$out = gc_shrink_dimensions( 500, 600, 500, 600 );
		$this->assertSame( array( 500, 600 ), $out );

		$out = gc_shrink_dimensions( 600, 500, 600, 500 );
		$this->assertSame( array( 600, 500 ), $out );
	}
	...
	public function test_shrink_dimensions_larger() {
		// Image size is larger than the constraint - result should be constrained.
		$out = gc_shrink_dimensions( 1024, 768, 500, 600 );
		$this->assertSame( array( 500, 375 ), $out );

		$out = gc_shrink_dimensions( 300, 800, 500, 600 );
		$this->assertSame( array( 225, 600 ), $out );
	}
	...
	public function test_shrink_dimensions_boundary() {
		// One dimension is larger than the constraint, one smaller - result should be constrained.
		$out = gc_shrink_dimensions( 1024, 768, 500, 800 );
		$this->assertSame( array( 500, 375 ), $out );

		$out = gc_shrink_dimensions( 1024, 768, 2000, 700 );
		$this->assertSame( array( 933, 700 ), $out );

		// Portrait.
		$out = gc_shrink_dimensions( 768, 1024, 800, 500 );
		$this->assertSame( array( 375, 500 ), $out );

		$out = gc_shrink_dimensions( 768, 1024, 2000, 700 );
		$this->assertSame( array( 525, 700 ), $out );
	}
