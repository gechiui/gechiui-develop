 ...
 * @group cache
 */
class Tests_Cache extends GC_UnitTestCase {
	public $cache = null;
	...
	private function &init_cache() {
		...
		$cache_class = get_class( $gc_object_cache );
	...
	public function test_gc_cache_incr() {
		$key = __FUNCTION__;

		$this->assertFalse( gc_cache_incr( $key ) );

		gc_cache_set( $key, 0 );
		gc_cache_incr( $key );
		$this->assertSame( 1, gc_cache_get( $key ) );

		gc_cache_incr( $key, 2 );
		$this->assertSame( 3, gc_cache_get( $key ) );
	}
	 ...
	 * @ticket 21327
	 */
	public function test_gc_cache_decr() {
		$key = __FUNCTION__;

		$this->assertFalse( gc_cache_decr( $key ) );

		gc_cache_set( $key, 0 );
		gc_cache_decr( $key );
		$this->assertSame( 0, gc_cache_get( $key ) );

		gc_cache_set( $key, 3 );
		gc_cache_decr( $key );
		$this->assertSame( 2, gc_cache_get( $key ) );
	...
	public function test_gc_cache_delete() {
		$key = __FUNCTION__;
		$val = 'val';

		// Verify set.
		$this->assertTrue( gc_cache_set( $key, $val ) );
		$this->assertSame( $val, gc_cache_get( $key ) );

		// Verify successful delete.
		$this->assertTrue( gc_cache_delete( $key ) );
		$this->assertFalse( gc_cache_get( $key ) );

		// gc_cache_delete() does not have a $force method.
		// Delete returns (bool) true when key is not set and $force is true.
		// $this->assertTrue( gc_cache_delete( $key, 'default', true ) );

		$this->assertFalse( gc_cache_delete( $key, 'default' ) );
	}
	...
	public function test_gc_cache_init() {
		$new_blank_cache_object = new GC_Object_Cache();
		gc_cache_init();
		...
		if ( gc_using_ext_object_cache() ) {
			...
			$this->assertInstanceOf( 'GC_Object_Cache', $gc_object_cache );
		} else {
			$this->assertEquals( $gc_object_cache, $new_blank_cache_object );
		}
	}

	public function test_gc_cache_replace() {
		$key  = 'my-key';
		$val1 = 'first-val';
		$val2 = 'second-val';
		...
		gc_cache_set( $key, $val1 );
		$this->assertSame( $val1, gc_cache_get( $key ) );

		// Replace the value and verify.
		gc_cache_replace( $key, $val2 );
		$this->assertSame( $val2, gc_cache_get( $key ) );

		// Non-existent key should fail.
		$this->assertFalse( gc_cache_replace( $fake_key, $val1 ) );

		// Make sure $fake_key is not stored.
		$this->assertFalse( gc_cache_get( $fake_key ) );
	}
	...
	public function test_get_multiple() {
		gc_cache_set( 'foo1', 'bar', 'group1' );
		gc_cache_set( 'foo2', 'bar', 'group1' );
		gc_cache_set( 'foo1', 'bar', 'group2' );

		$found = gc_cache_get_multiple( array( 'foo1', 'foo2', 'foo3' ), 'group1' );
