 ...
 * @group link
 */
class Tests_Link extends GC_UnitTestCase {
	...
	public function test_gc_get_shortlink() {
		$post_id  = self::factory()->post->create();
		$post_id2 = self::factory()->post->create();

		// Basic case.
		$this->assertSame( get_permalink( $post_id ), gc_get_shortlink( $post_id, 'post' ) );

		unset( $GLOBALS['post'] );

		// Global post is not set.
		$this->assertSame( '', gc_get_shortlink( 0, 'post' ) );
		$this->assertSame( '', gc_get_shortlink( 0 ) );
		$this->assertSame( '', gc_get_shortlink() );

		$GLOBALS['post'] = get_post( $post_id );

		// Global post is set.
		$this->assertSame( get_permalink( $post_id ), gc_get_shortlink( 0, 'post' ) );
		$this->assertSame( get_permalink( $post_id ), gc_get_shortlink( 0 ) );
		$this->assertSame( get_permalink( $post_id ), gc_get_shortlink() );

		// Not the global post.
		$this->assertSame( get_permalink( $post_id2 ), gc_get_shortlink( $post_id2, 'post' ) );
		...
		$this->assertNotEquals( get_permalink( $post_id ), gc_get_shortlink( $post_id, 'post' ) );
		$this->assertSame( home_url( '?p=' . $post_id ), gc_get_shortlink( $post_id, 'post' ) );

		// Global post and permalink structure are set.
		$GLOBALS['post'] = get_post( $post_id );
		$this->assertSame( home_url( '?p=' . $post_id ), gc_get_shortlink( 0, 'post' ) );
		$this->assertSame( home_url( '?p=' . $post_id ), gc_get_shortlink( 0 ) );
		$this->assertSame( home_url( '?p=' . $post_id ), gc_get_shortlink() );
	}

	public function test_gc_get_shortlink_with_page() {
	 ...
	 * @ticket 26871
	 */
	public function test_gc_get_shortlink_with_home_page() {
		$post_id = self::factory()->post->create( array( 'post_type' => 'page' ) );
		update_option( 'show_on_front', 'page' );
		update_option( 'page_on_front', $post_id );

		$this->assertSame( home_url( '/' ), gc_get_shortlink( $post_id, 'post' ) );
	...
	public function test_get_permalink_should_not_reveal_post_name_for_cpt_with_post_status_future() {
		...
		register_post_type( 'gctests_pt', array( 'public' => true ) );
	...
	public function test_attachment_attached_to_non_existent_post_type_has_a_pretty_permalink() {
		...
		foreach ( $gc_post_types as $id => $pt ) {
			if ( 'not_a_post_type' === $pt->name ) {
				unset( $gc_post_types[ $id ] );
				break;
			}
		}
