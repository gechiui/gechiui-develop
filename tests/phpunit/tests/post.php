...
/**
 * test gc-includes/post.php
 *
 * @group post
 */
class Tests_Post extends GC_UnitTestCase {
	...
	private function next_schedule_for_post( $hook, $id ) {
		return gc_next_scheduled( 'publish_future_post', array( 0 => (int) $id ) );
	}
	...
	public function test_vb_insert_get_delete() {
		...
		foreach ( $post_types as $post_type ) {
			...
			gc_delete_post( $id, true );
			$this->assertFalse( gc_cache_get( $id, 'posts' ) );
			$this->assertFalse( gc_cache_get( $id, 'post_tag_relationships' ) );
			$this->assertFalse( gc_cache_get( $id, 'ctax_relationships' ) );
		}

		$GLOBALS['gc_taxonomies']['post_tag']->object_type = array( 'post' );
	}
	...
	public function test_vb_insert_invalid_date() {
		...
		// Test both return paths with or without GC_Error.
		$insert_post = gc_insert_post( $post, true );
		$this->assertWPError( $insert_post );
		$this->assertSame( 'invalid_date', $insert_post->get_error_code() );

		$insert_post = gc_insert_post( $post );
	 ...
	 * @ticket 5305
	 */
	public function test_gc_insert_post_should_not_allow_a_bare_numeric_slug_that_might_conflict_with_a_date_archive_when_generating_from_an_empty_post_title() {
	 ...
	 * @ticket 33392
	 */
	public function test_gc_insert_post_should_invalidate_post_cache_before_generating_guid_when_post_name_is_empty_and_is_generated_from_the_post_ID() {
		...
		$this->assertStringContainsString( 'gctests_pt=' . $p, $post->guid );
	}

	/**
	 * @ticket 20451
	 */
	public function test_gc_insert_post_with_meta_input() {
	...
	/**
	 * "When I delete a future post using gc_delete_post( $post->ID ) it does not update the cron correctly."
	 *
	 * @ticket 5364
	 */
	 ...
	 * @ticket 22944
	 */
	public function test_gc_insert_post_and_gc_publish_post_with_future_date() {
	 ...
	 * @ticket 48145
	 */
	public function test_gc_insert_post_should_default_to_publish_if_post_date_is_within_59_seconds_from_current_time() {
	...
	public function test_publish_post_with_content_filtering() {
		...
		$post_id = gc_insert_post( array( 'post_title' => '<script>Test</script>' ) );
		$post    = get_post( $post_id );
	 ...
	 * @ticket 22944
	 */
	public function test_gc_publish_post_and_avoid_content_filtering() {
		kses_remove_filters();
	...
	public function test_update_invalid_post_id() {
		...
		$this->assertSame( 0, gc_insert_post( $post ) );
		$this->assertSame( 0, gc_update_post( $post ) );

		$this->assertInstanceOf( 'GC_Error', gc_insert_post( $post, true ) );
		$this->assertInstanceOf( 'GC_Error', gc_update_post( $post, true ) );
	...
	public function test_insert_programmatic_sanitized() {
		...
		$insert_post_id = gc_insert_post( $post_data, true, true );
	 ...
	 * @ticket 24803
	 */
	public function test_gc_count_posts() {
		...
		$count = gc_count_posts( $post_type, 'readable' );
		$this->assertEquals( 1, $count->publish );
		_unregister_post_type( $post_type );
		$this->assertEquals( new stdClass, gc_count_posts( $post_type, 'readable' ) );
	}

	public function test_gc_count_posts_filtered() {
		$post_type = rand_str( 20 );
		register_post_type( $post_type );
		...
		$count1 = gc_count_posts( $post_type, 'readable' );
		$this->assertEquals( 3, $count1->publish );
		add_filter( 'gc_count_posts', array( $this, 'filter_gc_count_posts' ) );

		$count2 = gc_count_posts( $post_type, 'readable' );
		$this->assertEquals( 2, $count2->publish );

		remove_filter( 'gc_count_posts', array( $this, 'filter_gc_count_posts' ) );
	}

	public function filter_gc_count_posts( $counts ) {
		$counts->publish = 2;
		return $counts;
	}

	public function test_gc_count_posts_insert_invalidation() {
		$post_ids       = self::factory()->post->create_many( 3 );
		$initial_counts = gc_count_posts();

		$key                  = array_rand( $post_ids );
		$_post                = get_post( $post_ids[ $key ], ARRAY_A );
		$_post['post_status'] = 'draft';
		gc_insert_post( $_post );
		$post = get_post( $post_ids[ $key ] );
		$this->assertSame( 'draft', $post->post_status );
		$this->assertNotEquals( 'publish', $post->post_status );

		$after_draft_counts = gc_count_posts();
	...
	public function test_gc_count_posts_trash_invalidation() {
		$post_ids       = self::factory()->post->create_many( 3 );
		$initial_counts = gc_count_posts();

		$key = array_rand( $post_ids );

		gc_trash_post( $post_ids[ $key ] );

		$post = get_post( $post_ids[ $key ] );
		$this->assertSame( 'trash', $post->post_status );
		$this->assertNotEquals( 'publish', $post->post_status );

		$after_trash_counts = gc_count_posts();
	 ...
	 * @ticket 49685
	 */
	public function test_gc_count_posts_status_changes_visible() {
	 ...
	 * @ticket 25566
	 */
	public function test_gc_tag_cloud_link_with_post_type() {
		...
		gc_set_object_terms( $post, 'foo', $tax );

		$gc_tag_cloud = gc_tag_cloud(
			array(
				'post_type' => $post_type,
				'taxonomy'  => $tax,
				'echo'      => false,
		...
		preg_match_all( '|href="([^"]+)"|', $gc_tag_cloud, $matches );
	...
	public function test_utf8mb3_post_saves_with_emoji() {
		...
		if ( 'utf8' !== $gcdb->get_col_charset( $gcdb->posts, 'post_title' ) ) {
	 ...
	 * @ticket 31168
	 */
	public function test_gc_insert_post_default_comment_ping_status_open() {
		$post_id = self::factory()->post->create(
			array(
				'post_author'  => self::$editor_id,
				'post_status'  => 'publish',
	 ...
	 * @ticket 31168
	 */
	public function test_gc_insert_post_page_default_comment_ping_status_closed() {
		$post_id = self::factory()->post->create(
			array(
				'post_author'  => self::$editor_id,
				'post_status'  => 'publish',
	 ...
	 * @ticket 31168
	 */
	public function test_gc_insert_post_cpt_default_comment_ping_status_open() {
		$post_type = rand_str( 20 );
	 ...
	 * @ticket 31168
	 */
	public function test_gc_insert_post_cpt_default_comment_ping_status_closed() {
		$post_type = rand_str( 20 );
		register_post_type( $post_type );
		$post_id = self::factory()->post->create(
			array(
	...
	public function test_user_without_publish_cannot_affect_sticky() {
		gc_set_current_user( self::$grammarian_id );
	 ...
	 * @ticket 32585
	 */
	public function test_gc_insert_post_author_zero() {
	 ...
	 * @ticket 32585
	 */
	public function test_gc_insert_post_author_null() {
	 ...
	 * @ticket 15946
	 */
	public function test_gc_insert_post_should_respect_post_date_gmt() {
		$post = array(
			'post_author'   => self::$editor_id,
			'post_status'   => 'publish',
			'post_content'  => 'content',
	...
	public function test_gc_delete_post_reassign_hierarchical_post_type() {
		...
		gc_delete_post( $parent_page_id, true );
		$this->assertSame( $grandparent_page_id, get_post( $page_id )->post_parent );
		gc_delete_post( $grandparent_page_id, true );
		$this->assertSame( 0, get_post( $page_id )->post_parent );
	}

	/**
	 * Test ensuring that the post_name (UUID) is preserved when gc_insert_post()/gc_update_post() is called.
	 *
	 * @see _gc_customize_changeset_filter_insert_post_data()
	 * @ticket 30937
	 */
	public function test_gc_insert_post_for_customize_changeset_should_not_drop_post_name() {

		$this->assertSame( 10, has_filter( 'gc_insert_post_data', '_gc_customize_changeset_filter_insert_post_data' ) );
		...
		gc_set_current_user( $this->factory()->user->create( array( 'role' => 'contributor' ) ) );
	 ...
	 *
	 * @see gc_unique_post_slug()
	 * @ticket 21112
	 */
	public function test_pre_gc_unique_post_slug_filter() {
		add_filter( 'pre_gc_unique_post_slug', array( $this, 'filter_pre_gc_unique_post_slug' ), 10, 6 );
		...
		remove_filter( 'pre_gc_unique_post_slug', array( $this, 'filter_pre_gc_unique_post_slug' ), 10, 6 );
	}

	public function filter_pre_gc_unique_post_slug( $default, $slug, $post_ID, $post_status, $post_type, $post_parent ) {
	...
	/**
	 * Test ensuring that gc_update_post() does not unintentionally modify post tags
	...
	public function test_update_post_should_only_modify_post_tags_if_different_tags_input_was_provided() {
		$tag_1 = gc_insert_term( 'gc_update_post_tag', 'post_tag', array( 'slug' => 'gc_update_post_tag_1' ) );
		$tag_2 = gc_insert_term( 'gc_update_post_tag', 'post_tag', array( 'slug' => 'gc_update_post_tag_2' ) );
		$tag_3 = gc_insert_term( 'gc_update_post_tag', 'post_tag', array( 'slug' => 'gc_update_post_tag_3' ) );
		...
		$tags = gc_get_post_tags( $post->ID, array( 'fields' => 'ids' ) );
	 ...
	 * @ticket 52187
	 */
	public function test_gc_resolve_post_date() {
		$post_date     = '2020-12-28 11:26:35';
		$post_date_gmt = '2020-12-29 10:11:45';
		$invalid_date  = '2020-12-41 14:15:27';

		$resolved_post_date = gc_resolve_post_date();
		$this->assertEqualsWithDelta( strtotime( gmdate( 'Y-m-d H:i:s' ) ), strtotime( $resolved_post_date ), 2, 'The dates should be equal' );

		$resolved_post_date = gc_resolve_post_date( '', $post_date_gmt );
		$this->assertSame( get_date_from_gmt( $post_date_gmt ), $resolved_post_date );

		$resolved_post_date = gc_resolve_post_date( '', $invalid_date );
		$this->assertSame( '1970-01-01 00:00:00', $resolved_post_date );

		$resolved_post_date = gc_resolve_post_date( $post_date );
		$this->assertSame( $post_date, $resolved_post_date );

		$resolved_post_date = gc_resolve_post_date( $post_date, $post_date_gmt );
		$this->assertSame( $post_date, $resolved_post_date );

		$resolved_post_date = gc_resolve_post_date( $post_date, $invalid_date );
		$this->assertSame( $post_date, $resolved_post_date );

		$resolved_post_date = gc_resolve_post_date( $invalid_date );
		$this->assertFalse( $resolved_post_date );

		$resolved_post_date = gc_resolve_post_date( $invalid_date, $post_date_gmt );
		$this->assertFalse( $resolved_post_date );

		$resolved_post_date = gc_resolve_post_date( $invalid_date, $invalid_date );
		$this->assertFalse( $resolved_post_date );
	}
