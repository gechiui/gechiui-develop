...
/**
 * Tests GC_Customize_Nav_Menu_Setting.
 *
 * @group customize
 */
class Test_GC_Customize_Nav_Menu_Setting extends GC_UnitTestCase {
	...
	public function test_constants() {
		...
		$this->assertTrue( taxonomy_exists( GC_Customize_Nav_Menu_Setting::TAXONOMY ) );
	}

	/**
	 * Test constructor.
	 *
	 * @see GC_Customize_Nav_Menu_Setting::__construct()
	 */
	public function test_construct() {
		do_action( 'customize_register', $this->gc_customize );

		$setting = new GC_Customize_Nav_Menu_Setting( $this->gc_customize, 'nav_menu[123]' );
		...
		$exception = null;
		try {
			$bad_setting = new GC_Customize_Nav_Menu_Setting( $this->gc_customize, 'foo_bar_baz' );
			unset( $bad_setting );
		} catch ( Exception $e ) {
			$exception = $e;
		}
	...
	public function test_construct_empty_menus() {
		...
		$exception = null;
		try {
			$bad_setting = new GC_Customize_Nav_Menu_Setting( $_gc_customize, 'nav_menu_item[123]' );
			unset( $bad_setting );
		} catch ( Exception $e ) {
			$exception = $e;
		}
	...
	public function test_construct_placeholder() {
		...
		$setting = new GC_Customize_Nav_Menu_Setting( $this->gc_customize, 'nav_menu[-5]', compact( 'default' ) );
	 ...
	 * Test value method.
	 *
	 * @see GC_Customize_Nav_Menu_Setting::value()
	 */
	public function test_value() {
		do_action( 'customize_register', $this->gc_customize );

		$menu_name      = 'Test 123 \\o/';
		$parent_menu_id = gc_create_nav_menu( gc_slash( "Parent $menu_name" ) );
		$description    = 'Hello my world \\o/.';
		$menu_id        = gc_update_nav_menu_object(
			0,
			gc_slash(
				array(
					'menu-name'   => $menu_name,
		...
		$setting    = new GC_Customize_Nav_Menu_Setting( $this->gc_customize, $setting_id );
		...
		$new_menu_name = 'Foo';
		gc_update_nav_menu_object( $menu_id, gc_slash( array( 'menu-name' => $new_menu_name ) ) );
	 ...
	 *
	 * @see GC_Customize_Nav_Menu_Setting::preview()
	 */
	public function test_preview_updated() {
		...
		$term = (array) gc_get_nav_menu_object( $menu_id );

		$this->assertSameSets(
			gc_array_slice_assoc( $value, array( 'name', 'description', 'parent' ) ),
			gc_array_slice_assoc( $term, array( 'name', 'description', 'parent' ) )
		);
		...
		$term = (array) gc_get_nav_menu_object( $menu_id );
		$this->assertSameSets( $value, gc_array_slice_assoc( $term, array_keys( $value ) ) );

		$menu_object = gc_get_nav_menu_object( $menu_id );
		...
		$menus_ids = gc_list_pluck( $menus, 'term_id' );
		$i         = array_search( $menu_id, $menus_ids, true );
		$this->assertIsInt( $i, 'Update-previewed menu does not appear in gc_get_nav_menus()' );
	...
	public function test_preview_inserted() {
		...
		$this->assertSameSets( $post_value, gc_array_slice_assoc( $term, array_keys( $value ) ) );
		...
		$menus_ids = gc_list_pluck( $menus, 'term_id' );
		$i         = array_search( $menu_id, $menus_ids, true );
		$this->assertIsInt( $i, 'Insert-previewed menu was not injected into gc_get_nav_menus()' );
		$filtered_menu = $menus[ $i ];
	...
	public function test_preview_deleted() {
		...
		$this->assertIsObject( gc_get_nav_menu_object( $menu_id ) );
		$setting->preview();
		$this->assertFalse( $setting->value() );
		$this->assertFalse( gc_get_nav_menu_object( $menu_id ) );
	 ...
	 *
	 * @see GC_Customize_Nav_Menu_Setting::sanitize()
	 */
	 ...
	 *
	 * @see GC_Customize_Nav_Menu_Setting::update()
	 */
	public function test_save_updated() {
		...
		$this->gc_customize->set_post_value( $setting_id, $new_value );
		$setting->save();
		...
		$this->assertSameSets(
			gc_array_slice_assoc( $new_value, array( 'name', 'description', 'parent' ) ),
			gc_array_slice_assoc( (array) $menu_object, array( 'name', 'description', 'parent' ) )
		);
	...
	public function test_save_inserted() {
		...
		$menu = gc_get_nav_menu_object( $setting->term_id );
		unset( $post_value['auto_add'] );
		$this->assertSameSets( $post_value, gc_array_slice_assoc( (array) $menu, array_keys( $post_value ) ) );
	...
	public function test_save_inserted_conflicted_name() {
		...
		$menu_name = 'Foo';
		gc_update_nav_menu_object( 0, gc_slash( array( 'menu-name' => $menu_name ) ) );

		$menu_id    = -123;
		$setting_id = "nav_menu[$menu_id]";
		$setting    = new GC_Customize_Nav_Menu_Setting( $this->gc_customize, $setting_id );
		$this->gc_customize->set_post_value( $setting->id, array( 'name' => $menu_name ) );
		$setting->save();

		$expected_resolved_menu_name = "$menu_name (2)";
		$new_menu                    = gc_get_nav_menu_object( $setting->term_id );
	...
	public function test_save_deleted() {
		...
		$menu_id                        = gc_create_nav_menu( gc_slash( $menu_name ) );
		$setting_id                     = "nav_menu[$menu_id]";
		$setting                        = new GC_Customize_Nav_Menu_Setting( $this->gc_customize, $setting_id );
		$nav_menu_options               = $this->get_nav_menu_items_option();
		$nav_menu_options['auto_add'][] = $menu_id;
