<?php
/**
 * Tests GC_Customize_Nav_Menu_Item_Setting.
 *
 * @group customize
 */
class Test_GC_Customize_Nav_Menu_Item_Setting extends GC_UnitTestCase {
	...
	public function test_constants() {
		...
		$this->assertTrue( post_type_exists( GC_Customize_Nav_Menu_Item_Setting::POST_TYPE ) );
	}

	/**
	 * Test constructor.
	 *
	 * @see GC_Customize_Nav_Menu_Item_Setting::__construct()
	 */
	public function test_construct() {
		do_action( 'customize_register', $this->gc_customize );

		$setting = new GC_Customize_Nav_Menu_Item_Setting( $this->gc_customize, 'nav_menu_item[123]' );
		...
		$exception = null;
		try {
			$bad_setting = new GC_Customize_Nav_Menu_Item_Setting( $this->gc_customize, 'foo_bar_baz' );
			unset( $bad_setting );
		} catch ( Exception $e ) {
			$exception = $e;
		}
	...
	public function test_construct_empty_menus() {
		do_action( 'customize_register', $this->gc_customize );
		$_gc_customize = $this->gc_customize;
		unset( $_gc_customize->nav_menus );

		$exception = null;
		try {
			$bad_setting = new GC_Customize_Nav_Menu_Item_Setting( $_gc_customize, 'nav_menu_item[123]' );
			unset( $bad_setting );
		} catch ( Exception $e ) {
			$exception = $e;
		}
	...
	public function test_construct_placeholder() {
		...
		$setting = new GC_Customize_Nav_Menu_Item_Setting( $this->gc_customize, 'nav_menu_item[-5]', compact( 'default' ) );
	...
	public function test_value_type_post_type() {
		...
		$menu_id    = gc_create_nav_menu( 'Menu' );
		...
		$menu_item = gc_setup_nav_menu_item( $post );
		$this->assertSame( $item_title, $menu_item->title );

		$setting_id = "nav_menu_item[$item_id]";
		$setting    = new GC_Customize_Nav_Menu_Item_Setting( $this->gc_customize, $setting_id );
		...
		$other_menu_id = gc_create_nav_menu( 'Menu2' );
	...
	public function test_custom_type_label() {
		do_action( 'customize_register', $this->gc_customize );
		add_filter( 'gc_setup_nav_menu_item', array( $this, 'filter_type_label' ) );
	...
	public function test_value_nav_menu_term_id_returns_zero() {
		...
		$menu       = new GC_Customize_Nav_Menu_Setting( $this->gc_customize, $setting_id );

		$this->gc_customize->set_post_value( $menu->id, $post_value );
		$menu->preview();
		$value = $menu->value();
	 ...
	 *
	 * @see GC_Customize_Nav_Menu_Item_Setting::preview()
	 */
	public function test_preview_updated() {
		...
		$primary_menu_id   = gc_create_nav_menu( 'Primary' );
		$secondary_menu_id = gc_create_nav_menu( 'Secondary' );
		$item_title        = 'Greetings';
		$item_id           = gc_update_nav_menu_item(
			$primary_menu_id,
			0,
		...
		$this->assertNotEmpty( gc_get_nav_menu_items( $primary_menu_id, array( 'post_status' => 'publish,draft' ) ) );
		...
		$setting_id = "nav_menu_item[$item_id]";
		$setting    = new GC_Customize_Nav_Menu_Item_Setting( $this->gc_customize, $setting_id );
		$this->gc_customize->set_post_value( $setting_id, $post_value );
		unset( $post_value['nav_menu_term_id'] );
		$setting->preview();

		// Make sure the menu item appears in the new menu.
		$this->assertNotContains( $item_id, gc_list_pluck( gc_get_nav_menu_items( $primary_menu_id ), 'db_id' ) );
		$menu_items = gc_get_nav_menu_items( $secondary_menu_id );
		$db_ids     = gc_list_pluck( $menu_items, 'db_id' );
	...
	public function test_preview_inserted() {
		...
		$current_items = gc_get_nav_menu_items( $menu_id );
		$setting->preview();
		$preview_items = gc_get_nav_menu_items( $menu_id );
	...
	public function test_preview_deleted() {
		...
		$setting        = new GC_Customize_Nav_Menu_Item_Setting( $this->gc_customize, $setting_id );
		$this->gc_customize->set_post_value( $setting_id, false );

		$current_items = gc_get_nav_menu_items( $menu_id );
		$this->assertContains( $delete_item_id, gc_list_pluck( $current_items, 'db_id' ) );
		$setting->preview();
		$preview_items = gc_get_nav_menu_items( $menu_id );
		$this->assertNotEquals( count( $current_items ), count( $preview_items ) );
		$this->assertContains( $delete_item_id, gc_list_pluck( $current_items, 'db_id' ) );
	 ...
	 *
	 * @see GC_Customize_Nav_Menu_Item_Setting::sanitize()
	 */
	public function test_sanitize() {
		...
		$valid_urls = array(
			...
			'irc://irc.freenode.net/gechiui',
			'irc6://irc.freenode.net/gechiui',
			'ircs://irc.freenode.net/gechiui',
			'gopher://example.com',
			...
			'mms://example.com',
			'rtsp://example.com/',
			'svn://develop.svn.gechiui.com/trunk',
			'tel:000-000-000',
			'fax:000-000-000',
		...
		foreach ( $invalid_urls as $invalid_url ) {
			...
			$this->assertInstanceOf( 'GC_Error', $url_setting );
		...
		$nav_menu_item_id = gc_update_nav_menu_item(
			$menu_id,
			0,
			gc_slash(
				array(
		...
		$nav_menu_item = gc_setup_nav_menu_item( clone $post );
	...
	public function test_invalid_nav_menu_item() {
		...
		$value_object = $setting->value_as_gc_post_nav_menu_item();
	...
	/**
	 * Test GC_Customize_Nav_Menu_Item_Setting::value_as_gc_post_nav_menu_item().
	 *
	 * @see GC_Customize_Nav_Menu_Item_Setting::value_as_gc_post_nav_menu_item()
	 */
	public function test_value_as_gc_post_nav_menu_item() {
		$post_id = self::factory()->post->create();

		$setting    = new GC_Customize_Nav_Menu_Item_Setting(
			$this->gc_customize,
			'nav_menu_item[123]'
		);
		$post_value = array(
		...
		$this->gc_customize->set_post_value( $setting->id, $post_value );

		$setting->preview();

		$item_value = $setting->value();
		$this->assertArrayHasKey( 'type_label', $item_value );
		$nav_menu_item = $setting->value_as_gc_post_nav_menu_item();
		...
		$this->assertSame( gc_get_current_user()->ID, $nav_menu_item->post_author );
		$this->assertObjectHasAttribute( 'type_label', $nav_menu_item );
		$expected = apply_filters( 'nav_menu_attr_title', gc_unslash( apply_filters( 'excerpt_save_pre', gc_slash( $post_value['attr_title'] ) ) ) );
	...
	/**
	 * Test GC_Customize_Nav_Menu_Item_Setting::value_as_gc_post_nav_menu_item() to set url for posts, terms, and post type archives.
	 *
	 * @ticket 38945
	 * @covers GC_Customize_Nav_Menu_Item_Setting::value_as_gc_post_nav_menu_item
	 */
	public function test_value_as_gc_post_nav_menu_item_term_urls() {
		...
		$this->gc_customize->set_post_value(
			$setting->id,
			array(
				'type'      => 'taxonomy',
				'object'    => 'category',
	...
	/**
	 * Test GC_Customize_Nav_Menu_Item_Setting::value_as_gc_post_nav_menu_item() for obtaining original title.
	 *
	 * @ticket 38945
	 * @covers GC_Customize_Nav_Menu_Item_Setting::get_original_title
	 */
	public function test_get_original_title() {
		...
		$item = $setting->value_as_gc_post_nav_menu_item();
	...
	/**
	 * Test GC_Customize_Nav_Menu_Item_Setting::value_as_gc_post_nav_menu_item() where title is empty.
	 *
	 * @ticket 38015
	 * @see GC_Customize_Nav_Menu_Item_Setting::value_as_gc_post_nav_menu_item()
	 */
	public function test_value_as_gc_post_nav_menu_item_with_empty_title() {
