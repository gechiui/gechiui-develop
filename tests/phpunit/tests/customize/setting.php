...
/**
 * Tests for the GC_Customize_Setting class.
 *
 * @group customize
 */
class Tests_GC_Customize_Setting extends GC_UnitTestCase {
	...
	public function test_constructor_without_args() {
		$setting = new GC_Customize_Setting( $this->manager, 'foo' );
	 ...
	 * @param GC_Customize_Setting $setting The setting object.
	 */
	...
	public function test_constructor_with_args() {
		...
		$setting = new GC_Customize_Setting( $this->manager, 'bar', $args );
	...
	public function test_preview_standard_types_non_multidimensional() {
		...
		$_POST['customized'] = gc_slash( gc_json_encode( $this->post_data_overrides ) );
		...
		foreach ( $this->standard_type_configs as $type => $type_options ) {
			...
			$setting = new GC_Customize_Setting( $this->manager, $name, compact( 'type', 'default' ) );
	 ...
	 *
	 * @param mixed $default
	 * @param GC_Customize_Setting $setting
	 *
	 * @return mixed|null
	 */
	...
	public function test_preview_custom_type() {
		...
		$_POST['customized'] = gc_slash( gc_json_encode( $post_data_overrides ) );
	 ...
	 *
	 * @see GC_Customize_Setting::save()
	 * @see GC_Customize_Setting::update()
	 */
	public function test_update_custom_type() {
		$type    = 'custom';
		$name    = 'foo';
		$setting = new GC_Customize_Setting( $this->manager, $name, compact( 'type' ) );
	 ...
	 *
	 * @see Tests_GC_Customize_Setting::test_update_custom_type()
	 * @param mixed $value
	 * @param GC_Customize_Setting $setting
	 */
	...
	public function test_option_autoloading() {
		...
		$name    = 'autoloaded1';
		$setting = new GC_Customize_Setting(
			$this->manager,
			$name,
			array(
				'type' => 'option',
		...
		$setting->save();
		$autoload = $gcdb->get_var( $gcdb->prepare( "SELECT autoload FROM $gcdb->options WHERE option_name = %s", $setting->id ) );
		...
		$setting1 = new GC_Customize_Setting(
			$this->manager,
			$id_base . '[foo]',
			array(
				'type' => 'option',
		...
		$setting2 = new GC_Customize_Setting(
			$this->manager,
			$id_base . '[bar]',
			array(
				'type'     => 'option',
		...
		$setting1->save();
		$autoload = $gcdb->get_var( $gcdb->prepare( "SELECT autoload FROM $gcdb->options WHERE option_name = %s", $id_base ) );
	 ...
	 *
	 * @see GC_Customize_Setting::js_value()
	 * @see GC_Customize_Setting::json()
	 */
	public function test_js_value() {
		...
		$setting = new GC_Customize_Setting( $this->manager, 'name', $args );
	 ...
	 * Validate callback.
	 *
	 * @see Tests_GC_Customize_Setting::test_validate()
	 *
	...
	/**
	 * Ensure that GC_Customize_Setting::value() can return a previewed value for aggregated multidimensionals.
	 *
	 * @ticket 37294
	 */
	public function test_multidimensional_value_when_previewed() {
		...
		$setting = new GC_Customize_Setting( $this->manager, $setting_id );
		$this->assertSame( $initial_value, $setting->value() );
