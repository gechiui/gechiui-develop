<?php
/**
 * GC_Customize_Manager tests.
 *
 * @package GeChiUI
 */

/**
 * Tests for the GC_Customize_Manager class.
 *
 * @group customize
 */
class Tests_GC_Customize_Manager extends GC_UnitTestCase {
	...
	private function get_inactive_core_theme() {
		...
		foreach ( gc_get_themes() as $theme ) {
	...
	/**
	 * Instantiate class, set global $gc_customize, and return instance.
	 *
	 * @return GC_Customize_Manager
	 */
	private function instantiate() {
		$GLOBALS['gc_customize'] = new GC_Customize_Manager();
		return $GLOBALS['gc_customize'];
	}

	/**
	 * Test GC_Customize_Manager::__construct().
	 *
	 * @covers GC_Customize_Manager::__construct
	 */
	public function test_constructor() {
		...
		$this->assertSame( $uuid, $gc_customize->changeset_uuid() );
		$this->assertSame( $theme, $gc_customize->get_stylesheet() );
		$this->assertSame( $messenger_channel, $gc_customize->get_messenger_channel() );
		$this->assertFalse( $gc_customize->autosaved() );
		$this->assertTrue( $gc_customize->branching() );
		...
		$this->assertTrue( gc_is_uuid( $gc_customize->changeset_uuid(), 4 ) );

		$theme                                   = 'twentyfourteen';
		$messenger_channel                       = 'preview-456';
		$_REQUEST['theme']                       = $theme;
		$_REQUEST['customize_messenger_channel'] = $messenger_channel;
		$gc_customize                            = new GC_Customize_Manager( array( 'changeset_uuid' => $uuid ) );
		$this->assertSame( $theme, $gc_customize->get_stylesheet() );
		$this->assertSame( $messenger_channel, $gc_customize->get_messenger_channel() );
	 ...
	 *
	 * @ticket 39896
	 * @covers GC_Customize_Manager::establish_loaded_changeset
	 * @covers GC_Customize_Manager::__construct
	 */
	public function test_constructor_deferred_changeset_uuid() {
		...
		$uuid1 = gc_generate_uuid4();
		$this->factory()->post->create(
			array(
				'post_type'     => 'customize_changeset',
				'post_name'     => $uuid1,
				'post_status'   => 'draft',
				'post_content'  => gc_json_encode( $data ),
		...
		$this->assertSame( $uuid2, $gc_customize->changeset_uuid() );
		$this->assertSame( $post_id, $gc_customize->changeset_post_id() );
		...
		$this->assertNotContains( $gc_customize->changeset_uuid(), array( $uuid1, $uuid2 ) );
		$this->assertEmpty( $gc_customize->changeset_post_id() );
	...
	/**
	 * Test GC_Customize_Manager::setup_theme() for admin screen.
	 *
	 * @covers GC_Customize_Manager::setup_theme
	 */
	public function test_setup_theme_in_customize_admin() {
		global $pagenow, $gc_customize;
		...
		$this->assertInstanceOf( 'GCDieException', $exception );
		$this->assertStringContainsString( 'you are not allowed to customize this site', $exception->getMessage() );

		// Bad changeset.
		$exception = null;
		gc_set_current_user( self::$admin_user_id );
		$gc_customize = new GC_Customize_Manager( array( 'changeset_uuid' => 'bad' ) );
		try {
			$gc_customize->setup_theme();
		} catch ( Exception $e ) {
			$exception = $e;
		...
		$gc_customize = new GC_Customize_Manager();
		$gc_customize->setup_theme();
		$this->assertFalse( has_action( 'after_setup_theme', array( $gc_customize, 'import_theme_starter_content' ) ) );

		// Make sure that starter content import gets queued on a fresh site.
		update_option( 'fresh_site', '1' );
		$gc_customize->setup_theme();
		$this->assertSame( 100, has_action( 'after_setup_theme', array( $gc_customize, 'import_theme_starter_content' ) ) );
	}
	...
	public function test_fresh_site_flag_clearing() {
		...
		update_option( 'fresh_site', '1' );
		do_action( 'customize_save_after', $gc_customize );
		...
		$query_count = $gcdb->num_queries;
		do_action( 'customize_save_after', $gc_customize );
		$this->assertSame( $query_count, $gcdb->num_queries );
	}

	/**
	 * Test GC_Customize_Manager::setup_theme() for frontend.
	 *
	 * @covers GC_Customize_Manager::setup_theme
	 */
	public function test_setup_theme_in_frontend() {
		global $gc_customize, $pagenow, $show_admin_bar;
		$pagenow = 'front';
		...
		gc_set_current_user( self::$admin_user_id );
		$gc_customize = new GC_Customize_Manager( array( 'messenger_channel' => 'preview-1' ) );
		$gc_customize->setup_theme();
		$this->assertFalse( $show_admin_bar );

		show_admin_bar( true );
		gc_set_current_user( self::$admin_user_id );
		$gc_customize = new GC_Customize_Manager( array( 'messenger_channel' => null ) );
		$gc_customize->setup_theme();
		$this->assertTrue( $show_admin_bar );
	}

	/**
	 * Test GC_Customize_Manager::settings_previewed().
	 *
	 * @ticket 39221
	 * @covers GC_Customize_Manager::settings_previewed
	 */
	public function test_settings_previewed() {
		$gc_customize = new GC_Customize_Manager( array( 'settings_previewed' => false ) );
		$this->assertFalse( $gc_customize->settings_previewed() );

		$gc_customize = new GC_Customize_Manager();
		$this->assertTrue( $gc_customize->settings_previewed() );
	}

	/**
	 * Test GC_Customize_Manager::autosaved().
	 *
	 * @ticket 39896
	 * @covers GC_Customize_Manager::autosaved
	 */
	public function test_autosaved() {
		$gc_customize = new GC_Customize_Manager();
		$this->assertFalse( $gc_customize->autosaved() );

		$gc_customize = new GC_Customize_Manager( array( 'autosaved' => false ) );
		$this->assertFalse( $gc_customize->autosaved() );

		$gc_customize = new GC_Customize_Manager( array( 'autosaved' => true ) );
		$this->assertTrue( $gc_customize->autosaved() );
	}

	/**
	 * Test GC_Customize_Manager::branching().
	 *
	 * @ticket 39896
	 * @covers GC_Customize_Manager::branching
	 */
	public function test_branching() {
		$gc_customize = new GC_Customize_Manager();
		$this->assertTrue( $gc_customize->branching(), 'Branching should default to true since it is original behavior in 4.7.' );

		$gc_customize = new GC_Customize_Manager( array( 'branching' => false ) );
		$this->assertFalse( $gc_customize->branching() );
		add_filter( 'customize_changeset_branching', '__return_true' );
		$this->assertTrue( $gc_customize->branching() );
		remove_filter( 'customize_changeset_branching', '__return_true' );

		$gc_customize = new GC_Customize_Manager( array( 'branching' => true ) );
		$this->assertTrue( $gc_customize->branching() );
	...
	/**
	 * Test GC_Customize_Manager::changeset_uuid().
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::changeset_uuid
	 */
	...
	/**
	 * Test GC_Customize_Manager::gc_loaded().
	 *
	 * Ensure that post values are previewed even without being in preview.
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::gc_loaded
	 */
	public function test_gc_loaded() {
		gc_set_current_user( self::$admin_user_id );
		$gc_customize = new GC_Customize_Manager();
		$title        = 'Hello World';
		$gc_customize->set_post_value( 'blogname', $title );
		$this->assertNotEquals( $title, get_option( 'blogname' ) );
		$gc_customize->gc_loaded();
		$this->assertFalse( $gc_customize->is_preview() );
		$this->assertSame( $title, $gc_customize->get_setting( 'blogname' )->value() );
		$this->assertSame( $title, get_option( 'blogname' ) );
	}

	/**
	 * Test GC_Customize_Manager::find_changeset_post_id().
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::find_changeset_post_id
	 */
	public function test_find_changeset_post_id() {
		...
		$gc_customize = new GC_Customize_Manager();
		$this->assertNull( $gc_customize->find_changeset_post_id( gc_generate_uuid4() ) );
		$this->assertSame( $post_id, $gc_customize->find_changeset_post_id( $uuid ) );

		// Verify that the found post ID was cached under the given UUID, not the manager's UUID.
		$this->assertNotEquals( $post_id, $gc_customize->find_changeset_post_id( $gc_customize->changeset_uuid() ) );
	}

	/**
	 * Test GC_Customize_Manager::changeset_post_id().
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::changeset_post_id
	 */
	public function test_changeset_post_id() {
		$uuid         = gc_generate_uuid4();
		$gc_customize = new GC_Customize_Manager( array( 'changeset_uuid' => $uuid ) );
		$this->assertNull( $gc_customize->changeset_post_id() );
	...
	/**
	 * Test GC_Customize_Manager::changeset_data().
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::changeset_data
	 */
	public function test_changeset_data() {
		gc_set_current_user( self::$admin_user_id );
		$uuid         = gc_generate_uuid4();
		$gc_customize = new GC_Customize_Manager( array( 'changeset_uuid' => $uuid ) );
		$this->assertSame( array(), $gc_customize->changeset_data() );
		...
		$gc_customize = new GC_Customize_Manager( array( 'changeset_uuid' => $uuid ) );
		$this->assertSame( $data, $gc_customize->changeset_data() );

		// Autosave.
		$gc_customize->set_post_value( 'blogname', 'Hola Mundo' );
		$gc_customize->register_controls(); // That is, settings, so blogname setting is registered.
		...
		$gc_customize->register_controls(); // That is, settings.
		$this->assertFalse( $gc_customize->autosaved() );
		$this->assertSame( $data, $gc_customize->changeset_data() );
		...
		$this->assertTrue( $gc_customize->autosaved() );
		$this->assertNotEquals( $data, $gc_customize->changeset_data() );
		$this->assertSame(
			array_merge(
				gc_list_pluck( $data, 'value' ),
				array( 'blogname' => 'Hola Mundo' )
			),
			gc_list_pluck( $gc_customize->changeset_data(), 'value' )
		);
	...
	/**
	 * Test GC_Customize_Manager::import_theme_starter_content().
	 *
	 * @covers GC_Customize_Manager::import_theme_starter_content
	 * @covers GC_Customize_Manager::_save_starter_content_changeset
	...
	public function test_import_theme_starter_content() {
		...
		$this->assertEmpty( $gc_customize->unsanitized_post_values() );
		$gc_customize->import_theme_starter_content();
		$changeset_values     = $gc_customize->unsanitized_post_values();
		...
		foreach ( array( 'widget_text[2]', 'widget_meta[2]' ) as $setting_id ) {
			...
			$instance_data = $gc_customize->widgets->sanitize_widget_instance( $changeset_values[ $setting_id ] );
		...
		$attachment_metadata = gc_get_attachment_metadata( $posts_by_name['waffles'] );
		...
		$this->assertEmpty( $gc_customize->changeset_data() );
		$this->assertNull( $gc_customize->changeset_post_id() );
		$this->assertSame( 1000, has_action( 'customize_register', array( $gc_customize, '_save_starter_content_changeset' ) ) );
		do_action( 'customize_register', $gc_customize ); // This will trigger the changeset save.
		$this->assertIsInt( $gc_customize->changeset_post_id() );
		$this->assertNotEmpty( $gc_customize->changeset_data() );
		foreach ( $gc_customize->changeset_data() as $setting_id => $setting_params ) {
		...
		$gc_customize->import_theme_starter_content();
		$changeset_data = $gc_customize->changeset_data();
		...
		$r = $gc_customize->save_changeset_post( array( 'status' => 'publish' ) );
	...
	/**
	 * Test GC_Customize_Manager::import_theme_starter_content() with nested arrays.
	 *
	 * @ticket 45484
	 * @covers GC_Customize_Manager::import_theme_starter_content
	 */
	...
	/**
	 * Test GC_Customize_Manager::customize_preview_init().
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::customize_preview_init
	 */
	public function test_customize_preview_init() {
		...
		$gc_customize                      = new GC_Customize_Manager();
		$gc_customize->customize_preview_init();
		$this->assertSame( $did_action_customize_preview_init + 1, did_action( 'customize_preview_init' ) );

		$this->assertSame( 10, has_filter( 'gc_robots', 'gc_robots_no_robots' ) );
		$this->assertSame( 10, has_action( 'gc_head', array( $gc_customize, 'remove_frameless_preview_messenger_channel' ) ) );
		$this->assertSame( 10, has_filter( 'gc_headers', array( $gc_customize, 'filter_iframe_security_headers' ) ) );
		$this->assertSame( 10, has_filter( 'gc_redirect', array( $gc_customize, 'add_state_query_params' ) ) );
		$this->assertTrue( gc_script_is( 'customize-preview', 'enqueued' ) );
		$this->assertSame( 10, has_action( 'gc_head', array( $gc_customize, 'customize_preview_loading_style' ) ) );
		$this->assertSame( 20, has_action( 'gc_footer', array( $gc_customize, 'customize_preview_settings' ) ) );

		// Test unauthorized user outside preview (no messenger_channel).
		gc_set_current_user( self::$subscriber_user_id );
		$gc_customize = new GC_Customize_Manager();
		$gc_customize->register_controls();
		$this->assertNotEmpty( $gc_customize->controls() );
		$gc_customize->customize_preview_init();
		$this->assertEmpty( $gc_customize->controls() );

		// Test unauthorized user inside preview (with messenger_channel).
		gc_set_current_user( self::$subscriber_user_id );
		$gc_customize = new GC_Customize_Manager( array( 'messenger_channel' => 'preview-0' ) );
		$exception    = null;
		try {
			$gc_customize->customize_preview_init();
	...
	/**
	 * Test GC_Customize_Manager::filter_iframe_security_headers().
	 *
	 * @ticket 30937
	 * @ticket 40020
	 * @covers GC_Customize_Manager::filter_iframe_security_headers
	 */
	public function test_filter_iframe_security_headers() {
		$gc_customize = new GC_Customize_Manager();
		$headers      = $gc_customize->filter_iframe_security_headers( array() );
	...
	/**
	 * Test GC_Customize_Manager::add_state_query_params().
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::add_state_query_params
	 */
	public function test_add_state_query_params() {
		...
		$url               = $gc_customize->add_state_query_params( home_url( '/' ) );
		...
		$url          = $gc_customize->add_state_query_params( 'http://not-allowed.example.com/?q=1' );
		$parsed_url   = gc_parse_url( $url );
		parse_str( $parsed_url['query'], $query_params );
		$this->assertArrayNotHasKey( 'customize_messenger_channel', $query_params );
	...
	/**
	 * Test GC_Customize_Manager::save_changeset_post().
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::save_changeset_post
	 */
	public function test_save_changeset_post_without_theme_activation() {
		...
		$manager      = new GC_Customize_Manager(
			array(
				'changeset_uuid' => $uuid,
			)
		);
		$gc_customize = $manager;
		...
		$this->assertSame( $manager->unsanitized_post_values(), gc_list_pluck( $saved_data, 'value' ) );
		...
		$this->assertInstanceOf( 'GC_Error', $r );
		...
		$this->assertInstanceOf( 'GC_Error', $error_data['setting_validities']['foo_unauthorized'] );
		...
		$this->assertInstanceOf( 'GC_Error', $error_data['setting_validities']['bar_unknown'] );
		...
		$this->assertInstanceOf( 'GC_Error', $error_data['setting_validities']['baz_illegal'] );
		...
		$this->assertInstanceOf( 'GC_Error', $r['setting_validities']['bar_unknown'] );
		$saved_data = json_decode( get_post( $post_id )->post_content, true );
		...
		$manager      = new GC_Customize_Manager( array( 'changeset_uuid' => $uuid ) );
		$gc_customize = $manager;
		...
		$original_capabilities                          = gc_list_pluck( $manager->settings(), 'capability' );
		gc_set_current_user( self::$subscriber_user_id );
		$r = $manager->save_changeset_post(
			array(
				'status' => 'publish',
		...
		$this->assertSame( $original_capabilities, gc_list_pluck( $manager->settings(), 'capability' ) );
		...
		gc_set_current_user( self::$admin_user_id );
		$this->assertSame( 'publish', get_post_meta( $post_id, '_gc_trash_meta_status', true ) );
		$this->assertIsNumeric( get_post_meta( $post_id, '_gc_trash_meta_time', true ) );
		...
		$this->assertTrue( gc_revisions_enabled( get_post( $manager->changeset_post_id() ) ) );
		...
		$manager->save_changeset_post( array( 'status' => 'draft' ) );
		$this->assertCount( 3, gc_get_post_revisions( $manager->changeset_post_id() ) );
	}
	...
	public function test_save_changeset_post_without_kses_corrupting_json() {
		...
		$options = array(
			'custom_html_1' => '<script>document.write(" Gechiui 1")</script>',
			'custom_html_2' => '<script>document.write(" Gechiui 2")</script>',
			'custom_html_3' => '<script>document.write(" Gechiui 3")</script>',
		);

		// Populate setting as user who can bypass content_save_pre filter.
		gc_set_current_user( self::$admin_user_id );
		$gc_customize = $this->get_manager_for_testing_json_corruption_protection( $uuid );
		$gc_customize->set_post_value( 'custom_html_1', $options['custom_html_1'] );
		$gc_customize->save_changeset_post(
			array(
				'status' => 'draft',
			)
		...
		gc_set_current_user( $lesser_admin_user_id );
		$gc_customize = $this->get_manager_for_testing_json_corruption_protection( $uuid );
		$gc_customize->set_post_value( 'custom_html_2', $options['custom_html_2'] );
		$gc_customize->save_changeset_post(
			array(
				'autosave' => true,
			)
		 ...
		 */
		$autosave_revision = gc_get_post_autosave( $gc_customize->changeset_post_id(), get_current_user_id() );
		 ...
		 */
		$gc_customize = $this->get_manager_for_testing_json_corruption_protection( $uuid );
		$saved_data   = json_decode( get_post( $gc_customize->changeset_post_id() )->post_content, true );
		$this->assertSame( $options['custom_html_1'], $saved_data['custom_html_1']['value'] );
		$this->assertSame( $options['custom_html_2'], $saved_data['custom_html_2']['value'] );
		 ...
		 * The value is sent through the sanitize function when it is read from the changeset.
		 */
		$revisions  = gc_get_post_revisions( $gc_customize->changeset_post_id() );
		...
		gc_set_current_user( $lesser_admin_user_id );
		$gc_customize = $this->get_manager_for_testing_json_corruption_protection( $uuid );
		$gc_customize->set_post_value( 'custom_html_3', $options['custom_html_3'] );
		$gc_customize->save_changeset_post(
			array(
				'status' => 'publish',
			)
		...
		$this->assertStringContainsString( 'Gechiui', get_option( 'custom_html_1' ) ); // phpcs:ignore GeChiUI.GC.CapitalPDangit.Misspelled
		...
		$this->assertStringContainsString( 'GeChiUI', get_option( 'custom_html_2' ) );
		...
		$this->assertStringContainsString( 'GeChiUI', get_option( 'custom_html_3' ) );
	}
	 ...
	 * @return GC_Customize_Manager Manager.
	 */
	 ...
	 *
	 * @return GC_Error Error.
	 */
	public function return_illegal_error() {
		return new GC_Error( 'illegal' );
	}
	 ...
	 * @ticket 30937
	 * @covers GC_Customize_Manager::save_changeset_post
	 * @covers GC_Customize_Manager::update_stashed_theme_mod_settings
	 */
	...
	public function test_save_changeset_post_with_varying_users() {
		...
		$uuid         = gc_generate_uuid4();
		$gc_customize = $this->create_test_manager( $uuid );
		$r            = $gc_customize->save_changeset_post(
			array(
				'status' => 'auto-draft',
				'data'   => array(
		...
		$post_id = $gc_customize->find_changeset_post_id( $uuid );
		...
		gc_set_current_user( $other_admin_user_id );
		$gc_customize = $this->create_test_manager( $uuid );
		$r            = $gc_customize->save_changeset_post(
			array(
				'status' => 'auto-draft',
		...
		// Ensure the modifying user set as the current user when each is saved, simulating GC Cron envronment.
		gc_set_current_user( 0 );
		$save_counts = array();
		...
		foreach ( $gc_customize->settings() as $setting ) {
	...
	/**
	 * Test that updating an auto-draft changeset bumps its post_date to keep it from getting garbage collected by gc_delete_auto_drafts().
	 *
	 * @ticket 31089
	...
	public function test_save_changeset_post_dumping_auto_draft_date() {
		...
		$uuid              = gc_generate_uuid4();
		$changeset_post_id = gc_insert_post(
			array(
				'post_type'    => 'customize_changeset',
				'post_content' => '{}',
				'post_name'    => $uuid,
	...
	public function test_save_changeset_post_with_unchanged_values() {
		...
		$data = get_post( $gc_customize->changeset_post_id() )->post_content;
		$r    = $gc_customize->save_changeset_post(
			array(
				'data' => array(
		...
		$this->assertInstanceOf( 'GC_Error', $r['setting_validities']['unknownsetting'] );
		$this->assertSame( 'unrecognized', $r['setting_validities']['unknownsetting']->get_error_code() );
		$this->assertInstanceOf( 'GC_Error', $r['setting_validities']['blogterminated'] );
		$this->assertSame( 'unauthorized', $r['setting_validities']['blogterminated']->get_error_code() );
		$this->assertSame( $data, get_post( $gc_customize->changeset_post_id() )->post_content );
		...
		$data = json_decode( get_post( $gc_customize->changeset_post_id() )->post_content, true );
	 ...
	 * @ticket 39896
	 * @covers GC_Customize_Manager::save_changeset_post
	 * @covers GC_Customize_Manager::grant_edit_post_capability_for_changeset
	 */
	public function test_save_changeset_post_with_autosave() {
		...
		$this->assertFalse( gc_get_post_autosave( $changeset_post_id, get_current_user_id() ) );
		...
		$autosave_revision = gc_get_post_autosave( $changeset_post_id, get_current_user_id() );
	...
	public function test_save_changeset_post_with_varying_unfiltered_html_cap() {
		...
		$gc_customize = new GC_Customize_Manager();
		do_action( 'customize_register', $gc_customize );
		$gc_customize->set_post_value( 'scratchpad', 'Unfiltered<script>evil</script>' );
		$gc_customize->save_changeset_post(
			array(
				'status'  => 'auto-draft',
		...
		$gc_customize = new GC_Customize_Manager( array( 'changeset_uuid' => $gc_customize->changeset_uuid() ) );
		do_action( 'customize_register', $gc_customize );
		...
		$changeset_post_id = $gc_customize->changeset_post_id();
		gc_set_current_user( 0 );
		$gc_customize = null;
		unset( $GLOBALS['gc_actions']['customize_register'] );
		$this->assertSame( 'Unfilteredevil', apply_filters( 'content_save_pre', 'Unfiltered<script>evil</script>' ) );
		gc_publish_post( $changeset_post_id ); // @todo If gc_update_post() is used here, then kses will corrupt the post_content.
		$this->assertSame( 'Unfiltered<script>evil</script>', get_option( 'scratchpad' ) );
	}
	 ...
	 *
	 * @ticket 39221
	 * @covers ::_gc_customize_publish_changeset
	 * @see GC_Customize_Widgets::schedule_customize_register()
	 * @see GC_Customize_Widgets::customize_register()
	 */
	public function test_gc_customize_publish_changeset() {
		global $gc_customize;
		$gc_customize = null;

		// Set the admin current screen to cause GC_Customize_Widgets::schedule_customize_register() to do early setting registration.
		...
		// Make sure previewing filters were never added, since GC_Customize_Manager should be constructed with settings_previewed=false.
	...
	/**
	 * Ensure that saving a changeset with a publish status but future date will change the status to future, to align with behavior in gc_insert_post().
	 *
	 * @ticket 41336
	 * @covers GC_Customize_Manager::save_changeset_post
	 */
	public function test_publish_changeset_with_future_status_when_future_date() {
		$gc_customize = $this->create_test_manager( gc_generate_uuid4() );
	 ...
	 * @ticket 41252
	 * @covers GC_Customize_Manager::save_changeset_post
	 * @covers GC_Customize_Manager::get_changeset_post_data
	 */
	 ...
	 *
	 * @covers GC_Customize_Manager::trash_changeset_post
	 */
	 ...
	 *
	 * @param GC_Customize_Manager $gc_customize Manager.
	 */
	public function register_scratchpad_setting( GC_Customize_Manager $gc_customize ) {
		$gc_customize->add_setting(
			'scratchpad',
			array(
				'type'              => 'option',
	...
	/**
	 * Test GC_Customize_Manager::is_cross_domain().
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::is_cross_domain
	 */
	public function test_is_cross_domain() {
		...
		$this->assertFalse( $gc_customize->is_cross_domain() );

		update_option( 'home', 'http://example.com' );
		update_option( 'siteurl', 'https://admin.example.com' );
		$this->assertTrue( $gc_customize->is_cross_domain() );
	}

	/**
	 * Test GC_Customize_Manager::get_allowed_urls().
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::get_allowed_urls
	 */
	public function test_get_allowed_urls() {
		$gc_customize = new GC_Customize_Manager();
		$this->assertFalse( is_ssl() );
		$this->assertFalse( $gc_customize->is_cross_domain() );
		$allowed = $gc_customize->get_allowed_urls();
	...
	/**
	 * Test GC_Customize_Manager::doing_ajax().
	 *
	 * @group ajax
	 */
	...
	public function test_not_doing_ajax() {
		add_filter( 'gc_doing_ajax', '__return_false' );
	...
	/**
	 * Test GC_Customize_Manager::unsanitized_post_values().
	 *
	 * @ticket 30988
	 */
	public function test_unsanitized_post_values_from_input() {
		...
		$_POST['customized'] = gc_slash( gc_json_encode( $customized ) );
		$post_values         = $manager->unsanitized_post_values();
	 ...
	 *
	 * @ticket 30937
	 * @covers GC_Customize_Manager::unsanitized_post_values
	 */
	public function test_unsanitized_post_values_with_changeset_and_stashed_theme_mods() {
		...
		$_POST['customized'] = gc_slash( gc_json_encode( $post_values ) );
		...
		$this->factory()->post->create(
			array(
				...
				'post_status'  => 'auto-draft',
				'post_name'    => $uuid,
				'post_content' => gc_json_encode( $changeset_data ),
			)
		);
	...
	/**
	 * Test the GC_Customize_Manager::post_value() method.
	 *
	 * @ticket 30988
	 */
	public function test_post_value() {
		...
		$_POST['customized'] = gc_slash( gc_json_encode( $posted_settings ) );
	...
	/**
	 * Test the GC_Customize_Manager::post_value() method for a setting value that fails validation.
	 *
	 * @ticket 34893
	 */
	public function test_invalid_post_value() {
		...
		$this->manager->set_post_value( 'foo', 'return_gc_error_in_sanitize' );
		$this->assertSame( $default_value, $this->manager->post_value( $setting, $default_value ) );
		$this->assertSame( $default_value, $setting->post_value( $default_value ) );
	...
	public function filter_customize_sanitize_foo( $value ) {
		if ( 'return_null_in_sanitize' === $value ) {
			...
			if ( false !== stripos( $value, 'return_gc_error_in_sanitize' ) ) {
				$value = new GC_Error( 'invalid_value_in_sanitize', __( 'Invalid value.' ), array( 'source' => 'filter_customize_sanitize_foo' ) );
			}
		}
		return $value;
	}
	 ...
	 * Filter customize_validate callback.
	 *
	 * @param GC_Error $validity Validity.
	 * @param mixed    $value    Value.
	 * @return GC_Error
	 */
	...
	/**
	 * Test the GC_Customize_Manager::post_value() method to make sure that the validation and sanitization are done in the right order.
	 *
	 * @ticket 37247
	 */
	...
	/**
	 * Test GC_Customize_Manager::validate_setting_values().
	 *
	 * @see GC_Customize_Manager::validate_setting_values()
	 */
	public function test_validate_setting_values() {
		...
		$this->assertInstanceOf( 'GC_Error', $invalid_settings[ $setting->id ] );
	 ...
	 *
	 * @ticket 37638
	 * @covers GC_Customize_Manager::validate_setting_values
	 */
	public function test_late_validate_setting_values() {
		...
		$this->assertInstanceOf( 'GC_Error', $setting->validate( '' ) );
		$setting_validities = $this->manager->validate_setting_values( array( $setting->id => '' ) );
		$this->assertInstanceOf( 'GC_Error', $setting_validities[ $setting->id ] );
		...
		$this->assertInstanceOf( 'GC_Error', $validity );
	...
	public function test_validate_setting_values_args() {
		...
		$this->assertInstanceOf( 'GC_Error', $error );
	 ...
	 * @param mixed                $value    Value.
	 * @param GC_Customize_Setting $setting  Setting.
	 * @return GC_Error Validity.
	 */
	public function late_validate_length( $validity, $value, $setting ) {
		$this->assertInstanceOf( 'GC_Customize_Setting', $setting );
		if ( strlen( $value ) < 10 ) {
	...
	/**
	 * Test the GC_Customize_Manager::validate_setting_values() method to make sure that the validation and sanitization are done in the right order.
	 *
	 * @ticket 37247
	 */
	...
	/**
	 * Test GC_Customize_Manager::prepare_setting_validity_for_js().
	 *
	 * @see GC_Customize_Manager::prepare_setting_validity_for_js()
	 */
	...
	/**
	 * Test GC_Customize_Manager::set_post_value().
	 *
	...
	/**
	 * Sanitize a value for Tests_GC_Customize_Manager::test_set_post_value().
	 *
	 * @see Tests_GC_Customize_Manager::test_set_post_value()
	 *
	 * @param mixed $value Value.
	 * @return int Value.
	 */
	 ...
	 *
	 * @see Tests_GC_Customize_Manager::capture_customize_post_value_set_actions()
	 * @var array
	 */
	protected $captured_customize_post_value_set_actions = array();

	/**
	 * Capture the actions fired when calling GC_Customize_Manager::set_post_value().
	 *
	 * @see Tests_GC_Customize_Manager::test_set_post_value()
	 *
	...
	/**
	 * Test the GC_Customize_Manager::add_dynamic_settings() method.
	 *
	 * @ticket 30936
	 */
	...
	/**
	 * Test GC_Customize_Manager::has_published_pages().
	 *
	 * @ticket 38013
	 * @covers GC_Customize_Manager::has_published_pages
	 */
	public function test_has_published_pages() {
		foreach ( get_pages() as $page ) {
			gc_delete_post( $page->ID, true );
		}
	...
	public function test_has_published_pages_when_nav_menus_created_posts() {
		...
		$this->assertInstanceOf( 'GC_Customize_Filter_Setting', $setting );
	...
	/**
	 * Test the GC_Customize_Manager::register_dynamic_settings() method.
	 *
	...
	public function filter_customize_dynamic_setting_class_for_test_dynamic_settings( $setting_class, $setting_id, $setting_args ) {
		$this->assertSame( 'GC_Customize_Setting', $setting_class );
		$this->assertIsString( $setting_id );
	 ...
	 *
	 * @see GC_Customize_Manager::get_document_title_template()
	 */
	 ...
	 *
	 * @see GC_Customize_Manager::get_preview_url()
	 * @see GC_Customize_Manager::set_preview_url()
	 */
	 ...
	 *
	 * @see GC_Customize_Manager::get_return_url()
	 * @see GC_Customize_Manager::set_return_url()
	 */
	public function test_return_url() {
		gc_set_current_user( self::factory()->user->create( array( 'role' => 'author' ) ) );
		...
		$_SERVER['HTTP_REFERER'] = gc_slash( admin_url( 'customize.php' ) );
		$this->assertSame( $preview_url, $this->manager->get_return_url() );

		// See #35355.
		$_SERVER['HTTP_REFERER'] = gc_slash( admin_url( 'gc-login.php' ) );
		$this->assertSame( $preview_url, $this->manager->get_return_url() );

		$url                     = home_url( '/referred/' );
		$_SERVER['HTTP_REFERER'] = gc_slash( $url );
	 ...
	 *
	 * @see GC_Customize_Manager::get_autofocus()
	 * @see GC_Customize_Manager::set_autofocus()
	 */
	 ...
	 *
	 * @see GC_Customize_Manager::get_nonces()
	 */
	public function test_nonces() {
		...
		$this->assertSame( gc_create_nonce( 'foo' ), $nonces['foo'] );
	}
	 ...
	 * @param GC_Customize_Manager $manager Manager.
	 * @return array Nonces.
	 */
	public function filter_customize_refresh_nonces( $nonces, $manager ) {
		$this->assertInstanceOf( 'GC_Customize_Manager', $manager );
		$nonces['foo'] = gc_create_nonce( 'foo' );
		return $nonces;
	}
	 ...
	 *
	 * @see GC_Customize_Manager::customize_pane_settings()
	 */
	public function test_customize_pane_settings() {
		...
		$this->assertStringContainsString( 'var _gcCustomizeSettings =', $content );
		$this->assertStringContainsString( '"blogname"', $content );
		$this->assertStringContainsString( '"type":"option"', $content );
		$this->assertStringContainsString( '_gcCustomizeSettings.controls', $content );
		$this->assertStringContainsString( '_gcCustomizeSettings.settings', $content );
		$this->assertStringContainsString( '</script>', $content );

		$this->assertNotEmpty( preg_match( '#var _gcCustomizeSettings\s*=\s*({.*?});\s*\n#', $content, $matches ) );
		$json = $matches[1];
	 ...
	 *
	 * @ticket 38867
	 * @covers GC_Customize_Manager::remove_frameless_preview_messenger_channel
	 */
	public function test_remove_frameless_preview_messenger_channel() {
		gc_set_current_user( self::$admin_user_id );
		$manager = new GC_Customize_Manager( array( 'messenger_channel' => null ) );
		ob_start();
		$manager->remove_frameless_preview_messenger_channel();
		$output = ob_get_clean();
		$this->assertEmpty( $output );

		$manager = new GC_Customize_Manager( array( 'messenger_channel' => 'preview-0' ) );
		ob_start();
		$manager->remove_frameless_preview_messenger_channel();
		$output = ob_get_clean();
	 ...
	 *
	 * @see GC_Customize_Manager::customize_preview_settings()
	 */
	public function test_customize_preview_settings() {
		...
		$this->assertSame( 1, preg_match( '/var _gcCustomizeSettings = ({.+});/', $content, $matches ) );
	...
	public function test_customize_loaded_components_filter() {
		$manager = new GC_Customize_Manager();
		$this->assertInstanceOf( 'GC_Customize_Widgets', $manager->widgets );
		$this->assertInstanceOf( 'GC_Customize_Nav_Menus', $manager->nav_menus );
	...
	/**
	 * @see Tests_GC_Customize_Manager::test_customize_loaded_components_filter()
	 *
	 * @param array                $components         Components.
	 * @param GC_Customize_Manager $customize_manager  Manager.
	 *
	...
	public function return_array_containing_widgets( $components, $customize_manager ) {
		...
		$this->assertIsArray( $components );
		$this->assertInstanceOf( 'GC_Customize_Manager', $customize_manager );
	...
	public function test_prepare_controls_stable_sorting() {
		...
		$sorted_control_ids = gc_list_pluck( $manager->get_section( $section_id )->controls, 'id' );
	...
	public function test_add_section_return_instance() {
		...
		$this->assertInstanceOf( 'GC_Customize_Section', $result_section );
		$this->assertSame( $section_id, $result_section->id );

		$section        = new GC_Customize_Section(
			$manager,
			$section_id,
			array(
				'title'    => 'Section 2',
	...
	public function test_add_setting_return_instance() {
		...
		$this->assertInstanceOf( 'GC_Customize_Setting', $result_setting );
		$this->assertSame( $setting_id, $result_setting->id );

		$setting        = new GC_Customize_Setting( $manager, $setting_id );
	...
	public function test_add_setting_honoring_dynamic() {
		...
		$this->assertSame( 'GC_Customize_Setting', get_class( $setting ) );
	...
	public function test_add_panel_return_instance() {
		...
		$this->assertInstanceOf( 'GC_Customize_Panel', $result_panel );
		$this->assertSame( $panel_id, $result_panel->id );

		$panel        = new GC_Customize_Panel(
			$manager,
			$panel_id,
			array(
				'title' => 'Test Panel 2',
	...
	public function test_add_control_return_instance() {
		...
		$this->assertInstanceOf( 'GC_Customize_Control', $result_control );
	 ...
	 * @ticket 37128
	 */
	public function test_prepare_controls_gc_list_sort_controls() {
		gc_set_current_user( self::$admin_user_id );
	 ...
	 * @ticket 37128
	 */
	public function test_prepare_controls_gc_list_sort_sections() {
		gc_set_current_user( self::$admin_user_id );
	 ...
	 * @ticket 37128
	 */
	public function test_prepare_controls_gc_list_sort_panels() {
		gc_set_current_user( self::$admin_user_id );
 ...
 * Class Test_Dynamic_Customize_Setting
 *
 * @see Tests_GC_Customize_Manager::test_add_setting_honoring_dynamic()
 */
class Test_Dynamic_Customize_Setting extends GC_Customize_Setting {
	public $type = 'dynamic';
	public $custom;
}
 ...
 *
 * @see Tests_GC_Customize_Manager::test_late_validate_setting_values()
 */
class Test_Setting_Without_Applying_Validate_Filter extends GC_Customize_Setting {
	...
	public function validate( $value ) {
		if ( empty( $value ) ) {
			return new GC_Error( 'empty_value', __( 'You must supply a value' ) );
		}
		return true;
	}
