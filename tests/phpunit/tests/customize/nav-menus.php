...
/**
 * Tests GC_Customize_Nav_Menus.
 *
 * @group customize
 */
class Test_GC_Customize_Nav_Menus extends GC_UnitTestCase {
	...
	public function set_up() {
		...
		global $gc_customize;
		$this->gc_customize = new GC_Customize_Manager();
		$gc_customize       = $this->gc_customize;
	}

	/**
	 * Delete the $gc_customize global when cleaning up scope.
	 */
	 ...
	 * Test constructor.
	 *
	 * @see GC_Customize_Nav_Menus::__construct()
	 */
	public function test_construct() {
		do_action( 'customize_register', $this->gc_customize );
		$menus = new GC_Customize_Nav_Menus( $this->gc_customize );
		$this->assertInstanceOf( 'GC_Customize_Manager', $menus->manager );

		$this->assertTrue( add_filter( 'customize_refresh_nonces', array( $menus, 'filter_nonces' ) ) );
		$this->assertTrue( add_action( 'gc_ajax_load-available-menu-items-customizer', array( $menus, 'ajax_load_available_items' ) ) );
		$this->assertTrue( add_action( 'gc_ajax_search-available-menu-items-customizer', array( $menus, 'ajax_search_available_items' ) ) );
		$this->assertTrue( add_action( 'gc_ajax_customize-nav-menus-insert-auto-draft', array( $menus, 'ajax_insert_auto_draft_post' ) ) );
	...
	/**
	 * Test that the load_available_items_query method returns a GC_Error object.
	 *
	 * @see GC_Customize_Nav_Menus::load_available_items_query()
	 */
	public function test_load_available_items_query_returns_gc_error() {
		$menus = new GC_Customize_Nav_Menus( $this->gc_customize );

		// Invalid post type $obj_name.
		$items = $menus->load_available_items_query( 'post_type', 'invalid' );
		$this->assertInstanceOf( 'GC_Error', $items );
	 ...
	 *
	 * @see GC_Customize_Nav_Menus::search_available_items_query()
	 */
	public function test_search_available_items_query() {
		...
		foreach ( $post_ids as $post_id ) {
			...
			gc_set_object_terms( $post_id, $term_ids, 'category' );
			$search  = $post_id === $post_ids[0] ? 'test & search' : 'other title';
			$s       = sanitize_text_field( gc_unslash( $search ) );
			$results = $menus->search_available_items_query(
				array(
					'pagenum' => 1,
					's'       => $s,
		...
		foreach ( $term_ids as $term_id ) {
			...
			$s        = sanitize_text_field( gc_unslash( $term->name ) );
			$results  = $menus->search_available_items_query(
				array(
					'pagenum' => 1,
					's'       => $s,
	 ...
	 *
	 * @see GC_Customize_Nav_Menus::enqueue_scripts()
	 */
	public function test_enqueue_scripts() {
		do_action( 'customize_register', $this->gc_customize );
		$menus = new GC_Customize_Nav_Menus( $this->gc_customize );
		$menus->enqueue_scripts();
		$this->assertTrue( gc_script_is( 'customize-nav-menus' ) );

		gc_dequeue_style( 'customize-nav-menus' );
		gc_dequeue_script( 'customize-nav-menus' );
	}

	/**
	 * Test the filter_dynamic_setting_args method.
	 *
	 * @see GC_Customize_Nav_Menus::filter_dynamic_setting_args()
	 */
	public function test_filter_dynamic_setting_args() {
		$menus = new GC_Customize_Nav_Menus( $this->gc_customize );

		$expected = array( 'type' => 'nav_menu_item' );
		$results  = $menus->filter_dynamic_setting_args( $this->gc_customize, 'nav_menu_item[123]' );
		$this->assertSame( $expected['type'], $results['type'] );

		$expected = array( 'type' => 'nav_menu' );
		$results  = $menus->filter_dynamic_setting_args( $this->gc_customize, 'nav_menu[123]' );
		$this->assertSame( $expected['type'], $results['type'] );
	}
	 ...
	 *
	 * @see GC_Customize_Nav_Menus::filter_dynamic_setting_class()
	 */
	public function test_filter_dynamic_setting_class() {
		do_action( 'customize_register', $this->gc_customize );
		$menus = new GC_Customize_Nav_Menus( $this->gc_customize );

		$expected = 'GC_Customize_Nav_Menu_Item_Setting';
		$results  = $menus->filter_dynamic_setting_class( 'GC_Customize_Setting', 'nav_menu_item[123]', array( 'type' => 'nav_menu_item' ) );
		$this->assertSame( $expected, $results );

		$expected = 'GC_Customize_Nav_Menu_Setting';
		$results  = $menus->filter_dynamic_setting_class( 'GC_Customize_Setting', 'nav_menu[123]', array( 'type' => 'nav_menu' ) );
		$this->assertSame( $expected, $results );
	}
	 ...
	 *
	 * @see GC_Customize_Nav_Menus::customize_register()
	 */
	public function test_customize_register() {
		do_action( 'customize_register', $this->gc_customize );
		$menu_id = gc_create_nav_menu( 'Primary' );
		$post_id = self::factory()->post->create( array( 'post_title' => 'Hello World' ) );
		$item_id = gc_update_nav_menu_item(
			$menu_id,
			0,
			array(
		...
		do_action( 'customize_register', $this->gc_customize );
		$this->assertInstanceOf( 'GC_Customize_Nav_Menu_Item_Setting', $this->gc_customize->get_setting( "nav_menu_item[$item_id]" ) );
		$this->assertSame( 'Primary', $this->gc_customize->get_section( "nav_menu[$menu_id]" )->title );
		$this->assertSame( 'Hello World', $this->gc_customize->get_control( "nav_menu_item[$item_id]" )->label );
		...
		$this->assertSame( array( $this->gc_customize->nav_menus, 'sanitize_nav_menus_created_posts' ), $nav_menus_created_posts_setting->sanitize_callback );
	}

	/**
	 * Test the intval_base10 method.
	 *
	 * @see GC_Customize_Nav_Menus::intval_base10()
	 */
	 ...
	 *
	 * @see GC_Customize_Nav_Menus::available_item_types()
	 */
	public function test_available_item_types() {
		...
		register_taxonomy( 'gctests_tax', array( 'post' ), array( 'labels' => array( 'name' => 'Foo' ) ) );
		$expected[] = array(
			'title'      => 'Foo',
			'type_label' => 'Foo',
			'type'       => 'taxonomy',
	 ...
	 *
	 * @covers GC_Customize_Nav_Menus::insert_auto_draft_post
	 */
	public function test_insert_auto_draft_post() {
		...
		$this->assertInstanceOf( 'GC_Post', $r );
		$this->assertSame( $this->gc_customize->changeset_uuid(), get_post_meta( $r->ID, '_customize_changeset_uuid', true ) );
	 ...
	 *
	 * @see GC_Customize_Nav_Menus::print_templates()
	 */
	 ...
	 *
	 * @see GC_Customize_Nav_Menus::available_items_template()
	 */
	public function test_available_items_template() {
		...
		$expected = sprintf( 'Customizing &#9656; %s', esc_html( $this->gc_customize->get_panel( 'nav_menus' )->title ) );
	...
	/**
	 * Test GC_Customize_Nav_Menus::customize_dynamic_partial_args().
	 *
	 * @see GC_Customize_Nav_Menus::customize_dynamic_partial_args()
	 */
	public function test_customize_dynamic_partial_args() {
		...
		$this->assertSame( array( $this->gc_customize->nav_menus, 'render_nav_menu_partial' ), $args['render_callback'] );
	 ...
	 *
	 * @see GC_Customize_Nav_Menus::customize_preview_init()
	 */
	public function test_customize_preview_init() {
		...
		$this->assertSame( 10, has_action( 'gc_enqueue_scripts', array( $menus, 'customize_preview_enqueue_deps' ) ) );
		$this->assertSame( 1000, has_filter( 'gc_nav_menu_args', array( $menus, 'filter_gc_nav_menu_args' ) ) );
		$this->assertSame( 10, has_filter( 'gc_nav_menu', array( $menus, 'filter_gc_nav_menu' ) ) );
	}

	/**
	 * Test make_auto_draft_status_previewable.
	 *
	 * @covers GC_Customize_Nav_Menus::make_auto_draft_status_previewable
	 */
	public function test_make_auto_draft_status_previewable() {
		global $gc_post_statuses;
		$menus = new GC_Customize_Nav_Menus( $this->gc_customize );
		$menus->make_auto_draft_status_previewable();
		$this->assertTrue( $gc_post_statuses['auto-draft']->protected );
	}

	/**
	 * Test sanitize_nav_menus_created_posts.
	 *
	 * @covers GC_Customize_Nav_Menus::sanitize_nav_menus_created_posts
	 */
	 ...
	 *
	 * @covers GC_Customize_Nav_Menus::save_nav_menus_created_posts
	 */
	public function test_save_nav_menus_created_posts() {
		...
		$this->gc_customize->set_post_value( $setting_id, array_merge( $post_ids, array( $pre_published_post_id ) ) );
		$setting = $this->gc_customize->get_setting( $setting_id );
		...
		$post_names = gc_list_pluck( $posts, 'post_name' );
		$this->assertSameSets( $post_names, array_unique( $post_names ) );
	}

	/**
	 * Test the filter_gc_nav_menu_args method.
	 *
	 * @see GC_Customize_Nav_Menus::filter_gc_nav_menu_args()
	 */
	public function test_filter_gc_nav_menu_args() {
		do_action( 'customize_register', $this->gc_customize );
		$menus   = $this->gc_customize->nav_menus;
		$menu_id = gc_create_nav_menu( 'Foo' );

		$results = $menus->filter_gc_nav_menu_args(
			array(
				'echo'        => true,
		...
		$this->assertArrayHasKey( 'customize_preview_nav_menus_args', $results );
		$this->assertSame( 'gc_page_menu', $results['fallback_cb'] );

		$nav_menu_term = get_term( gc_create_nav_menu( 'Bar' ) );
		$results       = $menus->filter_gc_nav_menu_args(
			array(
				'echo'        => true,
				'fallback_cb' => 'gc_page_menu',
	...
	/**
	 * Test the filter_gc_nav_menu method.
	 *
	 * @covers GC_Customize_Nav_Menus::filter_gc_nav_menu
	 * @covers GC_Customize_Nav_Menus::filter_gc_nav_menu_args
	 */
	public function test_filter_gc_nav_menu() {
		do_action( 'customize_register', $this->gc_customize );
		$menus = new GC_Customize_Nav_Menus( $this->gc_customize );

		$original_args = array(
			'echo'        => true,
			'menu'        => gc_create_nav_menu( 'Foo' ),
			'fallback_cb' => 'gc_page_menu',
			'walker'      => '',
			'items_wrap'  => '<ul id="%1$s" class="%2$s">%3$s</ul>',
		);
		...
		$args = $menus->filter_gc_nav_menu_args( $original_args );

		ob_start();
		gc_nav_menu( $args );
		$nav_menu_content = ob_get_clean();

		$result = $menus->filter_gc_nav_menu( $nav_menu_content, (object) $args );
	 ...
	 *
	 * @see GC_Customize_Nav_Menus::customize_preview_enqueue_deps()
	 */
	public function test_customize_preview_enqueue_deps() {
		...
		$this->assertTrue( gc_script_is( 'customize-preview-nav-menus' ) );
	}

	/**
	 * Test GC_Customize_Nav_Menus::export_preview_data() method.
	 *
	 * @see GC_Customize_Nav_Menus::export_preview_data()
	 */
	public function test_export_preview_data() {
		ob_start();
		$this->gc_customize->nav_menus->export_preview_data();
		$html = ob_get_clean();
		$this->assertTrue( (bool) preg_match( '/_gcCustomizePreviewNavMenusExports = ({.+})/s', $html, $matches ) );
		$exported_data = json_decode( $matches[1], true );
		$this->assertArrayHasKey( 'navMenuInstanceArgs', $exported_data );
	}

	/**
	 * Test GC_Customize_Nav_Menus::render_nav_menu_partial() method.
	 *
	 * @see GC_Customize_Nav_Menus::render_nav_menu_partial()
	 */
	public function test_render_nav_menu_partial() {
		$this->gc_customize->nav_menus->customize_preview_init();

		$menu = gc_create_nav_menu( 'Foo' );
		gc_update_nav_menu_item(
			$menu,
			0,
			array(
				'menu-item-type'   => 'custom',
				'menu-item-title'  => 'www.GeChiUI.com',
				'menu-item-url'    => 'https://www.gechiui.com',
				'menu-item-status' => 'publish',
			)
		);

		$nav_menu_args = $this->gc_customize->nav_menus->filter_gc_nav_menu_args(
			array(
				'echo'        => true,
				'menu'        => $menu,
				'fallback_cb' => 'gc_page_menu',
		...
		$partials   = $this->gc_customize->selective_refresh->add_dynamic_partials( array( $partial_id ) );
		...
		$this->assertStringContainsString( 'www.GeChiUI.com', $rendered );
	}
}
