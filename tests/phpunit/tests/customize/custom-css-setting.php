<?php
/**
 * Test Test_GC_Customize_Custom_CSS_Setting.
 *
 * Tests GC_Customize_Custom_CSS_Setting.
 *
 * @group customize
 */
class Test_GC_Customize_Custom_CSS_Setting extends GC_UnitTestCase {
	 ...
	 *
	 * @var GC_Customize_Custom_CSS_Setting
	 */
	public $setting;
	...
	public function set_up() {
		...
		$this->setting = new GC_Customize_Custom_CSS_Setting( $this->gc_customize, 'custom_css[' . get_stylesheet() . ']' );
		$this->gc_customize->add_setting( $this->setting );
	}
	 ...
	 *
	 * @covers GC_Customize_Custom_CSS_Setting::__construct
	 */
	public function test_construct() {
		...
		$exception = null;
		try {
			$x = new GC_Customize_Custom_CSS_Setting( $this->gc_customize, 'bad' );
			unset( $x );
		} catch ( Exception $e ) {
			$exception = $e;
		}
		...
		$exception = null;
		try {
			$x = new GC_Customize_Custom_CSS_Setting( $this->gc_customize, 'custom_css' );
			unset( $x );
		} catch ( Exception $e ) {
			$exception = $e;
		}
	...
	/**
	 * Test crud methods on GC_Customize_Custom_CSS_Setting.
	 *
	 * @covers ::gc_get_custom_css
	 * @covers GC_Customize_Custom_CSS_Setting::value
	 * @covers GC_Customize_Custom_CSS_Setting::preview
	 * @covers GC_Customize_Custom_CSS_Setting::update
	 */
	public function test_crud() {
		...
		$this->assertNull( gc_get_custom_css_post() );
		$this->assertNull( gc_get_custom_css_post( $this->setting->stylesheet ) );
		$this->assertNull( gc_get_custom_css_post( 'twentyten' ) );
		...
		$twentyten_setting = new GC_Customize_Custom_CSS_Setting( $this->gc_customize, 'custom_css[twentyten]' );

		remove_theme_mod( 'custom_css_post_id' );

		$this->assertSame( $post_id, gc_get_custom_css_post()->ID );
		$this->assertSame( $post_id, gc_get_custom_css_post( $this->setting->stylesheet )->ID );
		$this->assertSame( $twentyten_post_id, gc_get_custom_css_post( 'twentyten' )->ID );

		$this->assertSame( $original_css, gc_get_custom_css( $this->setting->stylesheet ) );
		$this->assertSame( $original_css, $this->setting->value() );
		$this->assertSame( $twentyten_css, gc_get_custom_css( 'twentyten' ) );
		$this->assertSame( $twentyten_css, $twentyten_setting->value() );

		$updated_css = 'body { color: blue; }';
		$this->gc_customize->set_post_value( $this->setting->id, $updated_css );
		$saved = $this->setting->save();

		$this->assertNotFalse( $saved );
		$this->assertSame( $updated_css, $this->setting->value() );
		$this->assertSame( $updated_css, gc_get_custom_css( $this->setting->stylesheet ) );
		$this->assertSame( $updated_css, get_post( $post_id )->post_content );

		$previewed_css = 'body { color: red; }';
		$this->gc_customize->set_post_value( $this->setting->id, $previewed_css );
		$this->setting->preview();
		$this->assertSame( $previewed_css, $this->setting->value() );
		$this->assertSame( $previewed_css, gc_get_custom_css( $this->setting->stylesheet ) );

		// Make sure that gc_update_custom_css_post() works as expected for updates.
		...
		$r = gc_update_custom_css_post( 'body { content: "\o/"; }' );
		$this->assertSame( $this->gc_customize->get_stylesheet(), get_post( $r )->post_name );
		$this->assertSame( 'body { content: "\o/"; }', get_post( $r )->post_content );
		$this->assertSame( '', get_post( $r )->post_content_filtered );

		// Make sure that gc_update_custom_css_post() works as expected for insertion.
		$r = gc_update_custom_css_post(
		...
		$this->assertNull( gc_get_custom_css_post() );
		$this->assertNull( gc_get_custom_css_post( get_stylesheet() ) );
		$this->assertSame( $previewed_css, gc_get_custom_css( get_stylesheet() ), 'Previewed value remains in spite of deleted post.' );
		gc_delete_post( $twentyten_post_id );
		$this->assertNull( gc_get_custom_css_post( 'twentyten' ) );
		$this->assertSame( '', gc_get_custom_css( 'twentyten' ) );
	}
	...
	public function test_custom_css_revision_saved() {
		...
		$post = gc_update_custom_css_post(
			$inserted_css,
			array(
		...
		$revisions = array_values( gc_get_post_revisions( $post ) );
		$this->assertCount( 1, $revisions );
		$this->assertSame( $inserted_css, $revisions[0]->post_content );

		gc_update_custom_css_post(
			$updated_css,
			array(
				'stylesheet' => 'testtheme',
			)
	...
	/**
	 * Test that gc_get_custom_css_post() doesn't query for a post after caching a failed lookup.
	 *
	 * @ticket 39259
	 */
	public function test_get_custom_css_post_queries_after_failed_lookup() {
		...
		gc_get_custom_css_post();
		$this->assertSame( get_num_queries(), $queries_before );
	}

	/**
	 * Test that gc_update_custom_css_post() updates the 'custom_css_post_id' theme mod.
	 *
	 * @ticket 39259
	 */
	public function test_update_custom_css_updates_theme_mod() {
		set_theme_mod( 'custom_css_post_id', -1 );
		$post = gc_update_custom_css_post( 'body { background: blue; }' );
	...
	public function test_value_filter() {
		...
		$this->gc_customize->set_post_value( $this->setting->id, '/*overridden*/' );
		$this->setting->preview();
	...
	public function filter_value( $value, $setting ) {
		$this->assertInstanceOf( 'GC_Customize_Custom_CSS_Setting', $setting );
		$value .= '/*filtered*/';
		return $value;
	}

	/**
	 * Test update filter on GC_Customize_Custom_CSS_Setting.
	 *
	 * @covers GC_Customize_Custom_CSS_Setting::update
	 */
	public function test_update_filter() {
		...
		$this->gc_customize->set_post_value( $this->setting->id, $overridden_css );
	 ...
	 *
	 * Note that the $validity \GC_Error object must be reset each time
	 * as it picks up the Errors and passes them to the next assertion.
	 *
	 * @covers GC_Customize_Custom_CSS_Setting::validate
	 */
