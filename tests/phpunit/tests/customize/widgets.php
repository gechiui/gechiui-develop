...
/**
 * Tests for the GC_Customize_Widgets class.
 *
 * @group customize
 */
class Tests_GC_Customize_Widgets extends GC_UnitTestCase {
	...
	public function set_up() {
		...
		$this->manager           = $GLOBALS['gc_customize'];

		unset( $GLOBALS['_gc_sidebars_widgets'] ); // Clear out cache set by gc_get_sidebars_widgets().
		$sidebars_widgets = gc_get_sidebars_widgets();
		$this->assertSameSets( array( 'gc_inactive_widgets', 'sidebar-1', 'sidebar-2' ), array_keys( gc_get_sidebars_widgets() ) );
		...
		$this->backup_registered_sidebars = $GLOBALS['gc_registered_sidebars'];

		// Reset protected static var on class.
		GC_Customize_Setting::reset_aggregated_multidimensionals();
	}
	...
	public function tear_down() {
		...
		$GLOBALS['gc_registered_sidebars'] = $this->backup_registered_sidebars;
		parent::tear_down();
	}
	...
	private function do_customize_boot_actions() {
		...
		$_REQUEST['nonce']         = gc_create_nonce( 'preview-customize_' . $this->manager->theme()->get_stylesheet() );
	...
	/**
	 * Test GC_Customize_Widgets::__construct()
	 */
	public function test_construct() {
		$this->assertInstanceOf( 'GC_Customize_Widgets', $this->manager->widgets );
	 ...
	 * @see GC_Customize_Widgets::preview_sidebars_widgets()
	 * @ticket 36660
	 */
	public function test_customize_register_with_deleted_sidebars() {
		...
		$this->assertSame( array_fill_keys( array( 'gc_inactive_widgets', $sidebar_id ), array() ), gc_get_sidebars_widgets() );
	}
	 ...
	 * @covers       GC_Customize_Widgets::customize_register
	 */
	...
	/**
	 * Tests GC_Customize_Widgets::get_selective_refreshable_widgets().
	 *
	 * @see GC_Customize_Widgets::get_selective_refreshable_widgets()
	 */
	public function test_get_selective_refreshable_widgets_when_theme_supports() {
		...
		$this->assertSame( count( $gc_widget_factory->widgets ), count( $selective_refreshable_widgets ) );
	 ...
	 *
	 * @see Tests_GC_Customize_Widgets::test_get_selective_refreshable_widgets_when_theme_supports()
	 * @see Tests_GC_Customize_Widgets::test_get_selective_refreshable_widgets_when_no_theme_supports()
	 */
	public function override_search_widget_customize_selective_refresh() {
		global $gc_widget_factory;
		$gc_widget_factory->widgets['GC_Widget_Search']->widget_options['customize_selective_refresh'] = false;
	}

	/**
	 * Tests GC_Customize_Widgets::is_widget_selective_refreshable().
	 *
	 * @see GC_Customize_Widgets::is_widget_selective_refreshable()
	 */
	...
	/**
	 * Test GC_Customize_Widgets::register_settings() with selective refresh enabled.
	 *
	 * @ticket 30988
	 * @ticket 36389
	 */
	public function test_register_settings() {
		...
		$widget_categories = get_option( 'widget_categories' );
		$this->assertSame( $raw_widget_customized['widget_categories[2]'], $widget_categories[2], 'Expected $gc_customize->get_setting(widget_categories[2])->preview() to have been called.' );
	}
	...
	/**
	 * Test GC_Customize_Widgets::get_setting_args()
	 */
	...
	/**
	 * Test GC_Customize_Widgets::sanitize_widget_js_instance() and GC_Customize_Widgets::sanitize_widget_instance()
	 */
	...
	public function test_sanitize_widget_instance_raw_instance() {
		...
		$block_instance = array(
			'content' => '<!-- gc:paragraph --><p>Hello</p><!-- /gc:paragraph -->',
		);
	...
	public function test_sanitize_widget_instance_with_no_show_instance_in_rest() {
		...
		$widget_object = $gc_widget_factory->get_widget_object( 'block' );
	...
	/**
	 * @see GC_Customize_Widgets::get_widget_control()
	 */
	...
	/**
	 * @see GC_Customize_Widgets::get_widget_control_parts()
	 */
	...
	/**
	 * @see GC_Widget_Form_Customize_Control::json()
	 */
	public function test_gc_widget_form_customize_control_json() {
		$this->do_customize_boot_actions();
	...
	/**
	 * @see GC_Customize_Widgets::is_panel_active()
	 */
	public function test_is_panel_active() {
		global $gc_registered_sidebars;
		$this->do_customize_boot_actions();

		$this->assertNotEmpty( $gc_registered_sidebars );
	...
	/**
	 * @ticket 34738
	 * @see GC_Customize_Widgets::call_widget_update()
	 */
	public function test_call_widget_update() {
		...
		$_POST = gc_slash(
			array(
				...
				'gc_customize'  => 'on',
				'nonce'         => gc_create_nonce( 'update-widget' ),
	...
	/**
	 * Test GC_Customize_Widgets::customize_dynamic_partial_args().
	 *
	 * @see GC_Customize_Widgets::customize_dynamic_partial_args()
	 */
	...
	/**
	 * Test GC_Customize_Widgets::selective_refresh_init().
	 *
	 * @see GC_Customize_Widgets::selective_refresh_init()
	 */
	public function test_selective_refresh_init_with_theme_support() {
		...
		$this->assertSame( 10, has_filter( 'gc_kses_allowed_html', array( $this->manager->widgets, 'filter_gc_kses_allowed_data_attributes' ) ) );
	}
	...
	public function test_selective_refresh_init_without_theme_support() {
		...
		$this->assertFalse( has_filter( 'gc_kses_allowed_html', array( $this->manager->widgets, 'filter_gc_kses_allowed_data_attributes' ) ) );
	}

	/**
	 * Test GC_Customize_Widgets::customize_preview_enqueue().
	 *
	 * @see GC_Customize_Widgets::customize_preview_enqueue()
	 */
	public function test_customize_preview_enqueue() {
		$this->manager->widgets->customize_preview_enqueue();
		$this->assertTrue( gc_script_is( 'customize-preview-widgets', 'enqueued' ) );
		$this->assertTrue( gc_style_is( 'customize-preview', 'enqueued' ) );
		$script = gc_scripts()->registered['customize-preview-widgets'];
	 ...
	 * @see GC_Customize_Widgets::start_dynamic_sidebar()
	 * @see GC_Customize_Widgets::end_dynamic_sidebar()
	 */
	public function test_filter_dynamic_sidebar_params() {
		...
		$output = gc_kses_post( $filtered_params[0]['before_widget'] );
	...
	/**
	 * Test GC_Customize_Widgets::render_widget_partial() method.
	 *
	 * @see GC_Customize_Widgets::render_widget_partial()
	 */
	...
	public function test_deprecated_methods() {
		$this->setExpectedDeprecated( 'GC_Customize_Widgets::setup_widget_addition_previews' );
		$this->setExpectedDeprecated( 'GC_Customize_Widgets::prepreview_added_sidebars_widgets' );
		$this->setExpectedDeprecated( 'GC_Customize_Widgets::prepreview_added_widget_instance' );
		$this->setExpectedDeprecated( 'GC_Customize_Widgets::remove_prepreview_filters' );
