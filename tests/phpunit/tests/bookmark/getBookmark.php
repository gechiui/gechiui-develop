 ...
 * @covers ::get_bookmark
 */
class Tests_Bookmark_GetBookmark extends GC_UnitTestCase {
	/**
	 ...
	 *
	 * @param GC_UnitTest_Factory $factory
	 */
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		gc_cache_delete( self::$bookmark->link_id, 'bookmark' );
	}
	...
	public function test_should_return_global_link_in_requested_output_format( $args ) {
		...
		$this->assertFalse( gc_cache_get( self::$bookmark->link_id, 'bookmark' ) );
	}
	...
	public function test_should_cache_bookmark_when_given_instance( $args ) {
		...
		$this->assertFalse( gc_cache_get( $bookmark->link_id, 'bookmark' ) );
		...
		$actual_cache = gc_cache_get( $bookmark->link_id, 'bookmark' );
	...
	public function test_should_return_cached_bookmark_when_given_existing_link_id( $args ) {
		...
		gc_cache_add( self::$bookmark->link_id, self::$bookmark, 'bookmark' );
		$args     = $this->init_func_args( $args, self::$bookmark->link_id );
		...
		// Check the bookmark was cached.
		$actual_cache = gc_cache_get( self::$bookmark->link_id, 'bookmark' );
	...
	public function test_should_return_null_when_bookmark_not_in_database( $args ) {
		...
		$this->assertFalse( gc_cache_get( $bookmark_link_id, 'bookmark' ) );
		$this->assertArrayNotHasKey( 'link', $GLOBALS );
		global $gcdb;
		$db_actual = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->links WHERE link_id = %d LIMIT 1", $bookmark_link_id ) );
