 ...
 * @group bookmark
 */
class Tests_Bookmark_GetBookmarks extends GC_UnitTestCase {
	...
	public function test_adding_bookmark_should_bust_get_bookmarks_cache() {
		...
		$this->assertEqualSets( $bookmarks, gc_list_pluck( $found2, 'link_id' ) );
		$this->assertGreaterThan( $num_queries, $gcdb->num_queries );
	}
	...
	public function test_category_param_properly_gets_parsed_as_list() {
		...
		$add = gc_add_object_terms( $bookmarks[0], $categories[0], 'link_category' );
		$add = gc_add_object_terms( $bookmarks[1], $categories[1], 'link_category' );
		$add = gc_add_object_terms( $bookmarks[2], $categories[2], 'link_category' );
