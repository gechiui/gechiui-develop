 ...
 *
 * In the process of doing so, it also tests GC, GC_Rewrite and GC_Query, A fail here may show a bug in any one of these areas.
 *
 * @group canonical
 * @group rewrite
 * @group query
 */
class Tests_Canonical extends GC_Canonical_UnitTestCase {
	...
	public function data_canonical() {
		...
		return array(
			...
			*/
			...
			array( '/2012/13/', '/2012/' ),
			array( '/2012/11/51/', '/2012/11/', 0, array( 'GC_Date_Query' ) ),
	...
	public function test_pre_redirect_guess_404_permalink() {
		...
		$this->go_to( '/child-page-1' );
		$this->assertSame( 'gc', redirect_guess_404_permalink() );
	}
