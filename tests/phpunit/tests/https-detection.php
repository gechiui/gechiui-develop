 ...
 */
class Tests_HTTPS_Detection extends GC_UnitTestCase {
	 ...
	 * @ticket 47577
	 */
	public function test_gc_is_using_https() {
		update_option( 'home', 'http://example.com/' );
		update_option( 'siteurl', 'http://example.com/' );
		$this->assertFalse( gc_is_using_https() );
		...
		$this->assertTrue( gc_is_using_https() );
	 ...
	 * @ticket 47577
	 */
	public function test_gc_is_https_supported() {
		// The function works with cached errors, so only test that here.
		$gc_error = new GC_Error();

		// No errors, so HTTPS is supported.
		update_option( 'https_detection_errors', $gc_error->errors );
		$this->assertTrue( gc_is_https_supported() );

		// Errors, so HTTPS is not supported.
		$gc_error->add( 'ssl_verification_failed', 'SSL verification failed.' );
		update_option( 'https_detection_errors', $gc_error->errors );
		$this->assertFalse( gc_is_https_supported() );
	}
	 ...
	 * @ticket 52484
	 */
	public function test_gc_update_https_detection_errors() {
		...
		// GeChiUI site, return error with 'bad_response_source' error code.
	 ...
	 * @ticket 47577
	 */
	public function test_pre_gc_update_https_detection_errors() {
		// Override to enforce no errors being detected.
		add_filter(
			'pre_gc_update_https_detection_errors',
			static function() {
				return new GC_Error();
			}
		);
	 ...
	 * @ticket 47577
	 */
	public function test_gc_schedule_https_detection() {
		gc_schedule_https_detection();
		$this->assertSame( 'twicedaily', gc_get_schedule( 'gc_https_detection' ) );
	}

	/**
	 * @ticket 47577
	 */
	public function test_gc_cron_conditionally_prevent_sslverify() {
		// If URL is not using HTTPS, don't set 'sslverify' to false.
		$request = array(
			'url'  => 'http://example.com/',
			'args' => array( 'sslverify' => true ),
		);
		$this->assertSame( $request, gc_cron_conditionally_prevent_sslverify( $request ) );
		...
		$this->assertSame( $expected, gc_cron_conditionally_prevent_sslverify( $request ) );
	}
	 ...
	 * @ticket 52542
	 */
	public function test_gc_is_local_html_output_via_rsd_link() {
		// HTML includes RSD link.
		$head_tag = get_echo( 'rsd_link' );
		$html     = $this->get_sample_html_string( $head_tag );
		$this->assertTrue( gc_is_local_html_output( $html ) );
		...
		$this->assertFalse( gc_is_local_html_output( $html ) );
	}

	/**
	 * @ticket 47577
	 */
	public function test_gc_is_local_html_output_via_wlwmanifest_link() {
		remove_action( 'gc_head', 'rsd_link' );
	 ...
	 * @ticket 47577
	 */
	public function test_gc_is_local_html_output_via_rest_link() {
		remove_action( 'gc_head', 'rsd_link' );
		remove_action( 'gc_head', 'wlwmanifest_link' );

		// HTML includes REST API link.
		$head_tag = get_echo( 'rest_output_link_gc_head' );
		$html     = $this->get_sample_html_string( $head_tag );
		$this->assertTrue( gc_is_local_html_output( $html ) );

		// HTML includes modified REST API link but same URL.
		$head_tag = str_replace( ' />', '>', get_echo( 'rest_output_link_gc_head' ) );
		$html     = $this->get_sample_html_string( $head_tag );
		$this->assertTrue( gc_is_local_html_output( $html ) );
	 ...
	 * @ticket 47577
	 */
	public function test_gc_is_local_html_output_cannot_determine() {
		remove_action( 'gc_head', 'rsd_link' );
		remove_action( 'gc_head', 'wlwmanifest_link' );
		remove_action( 'gc_head', 'rest_output_link_gc_head' );

		// The HTML here doesn't matter because all hooks are removed.
		$html = $this->get_sample_html_string();
		$this->assertNull( gc_is_local_html_output( $html ) );
	}
	...
	public function mock_bad_source() {
		// Looks like a success response, but is not generated by GeChiUI (e.g. missing RSD link).
		return array(
	...
	private function mock_error() {
		return new GC_Error( 'bad_ssl_certificate', 'Bad SSL certificate.' );
	}
