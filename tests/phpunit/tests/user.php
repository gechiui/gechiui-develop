...
/**
 * Test functions in gc-includes/user.php
 *
 * @group user
 */
class Tests_User extends GC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		self::$contrib_id = $factory->user->create(
			array(
				...
				'role'          => 'contributor',
				'nickname'      => 'Johnny',
				'description'   => 'I am a GeChiUI user that cares about privacy.',
			)
		);
	...
	public function test_user_properties() {
		$user = new GC_User( self::$author_id );
		...
		$this->assertSame( 'foo', $user->data->$key );  // This will fail with GC < 3.3.
	...
	public function test_user_unset() {
		// phpcs:disable GeChiUI.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		$user = new GC_User( self::$author_id );
	...
	public function test_construction() {
		...
		$user2 = new GC_User( 0, $user->user_login );
		$this->assertInstanceOf( 'GC_User', $user2 );
		$this->assertSame( self::$author_id, $user2->ID );
		$this->assertSame( $user->user_login, $user2->user_login );

		$user3 = new GC_User();
		$this->assertInstanceOf( 'GC_User', $user3 );
	...
	public function test_update_user() {
		...
		gc_update_user( (object) $user_data );
	...
	public function test_global_userdata() {
		global $userdata, $gcdb;

		gc_set_current_user( self::$sub_id );

		$this->assertNotEmpty( $userdata );
		$this->assertInstanceOf( 'GC_User', $userdata );
	...
	public function test_global_authordata() {
		...
		$post_id = gc_insert_post( $post );
		$this->assertIsNumeric( $post_id );

		setup_postdata( get_post( $post_id ) );

		$this->assertNotEmpty( $authordata );
		$this->assertInstanceOf( 'GC_User', $authordata );
	...
	public function test_user_get_data_by_id() {
		$user = GC_User::get_data_by( 'id', self::$author_id );
		$this->assertInstanceOf( 'stdClass', $user );
		$this->assertEquals( self::$author_id, $user->ID );

		// @ticket 23480
		$user1 = GC_User::get_data_by( 'id', -1 );
		$this->assertFalse( $user1 );

		$user2 = GC_User::get_data_by( 'id', 0 );
		$this->assertFalse( $user2 );

		$user3 = GC_User::get_data_by( 'id', null );
		$this->assertFalse( $user3 );

		$user4 = GC_User::get_data_by( 'id', '' );
		$this->assertFalse( $user4 );

		$user5 = GC_User::get_data_by( 'id', false );
		$this->assertFalse( $user5 );

		$user6 = GC_User::get_data_by( 'id', $user->user_nicename );
		$this->assertFalse( $user6 );

		$user7 = GC_User::get_data_by( 'id', 99999 );
	 ...
	 * @ticket 22858
	 */
	public function test_gc_update_user_on_nonexistent_users() {
		$user_id = 1;
		...
		$this->assertInstanceOf( 'GC_Error', gc_update_user( array( 'ID' => $user_id ) ) );
	}

	/**
	 * @ticket 28435
	 */
	public function test_gc_update_user_should_not_change_password_when_passed_GC_User_instance() {
		$testuserid = 1;
	 ...
	 * @group ms-excluded
	 */
	public function test_gc_update_user_should_not_mark_user_as_spam_on_single_site() {
		$u = gc_update_user(
			array(
				'ID'   => self::$contrib_id,
				'spam' => '0',
	 ...
	 * @dataProvider data_illegal_user_logins
	 */
	public function test_illegal_user_logins_single_gc_create_user( $user_login ) {
	...
	public function test_illegal_user_logins_multisite() {
		...
		$response = gcmu_validate_user_signup( $user_data['user_login'], $user_data['user_email'] );
		$this->assertInstanceOf( 'GC_Error', $response['errors'] );
	 ...
	 * @ticket 29880
	 */
	public function test_gc_insert_user_should_not_wipe_existing_password() {
		$user_details = array(
			'user_login' => 'jonsnow',
			'user_pass'  => 'password',
		...
		$user_id = gc_insert_user( $user_details );
		...
		$user_id = gc_insert_user( $user_details );
		$user    = GC_User::get_data_by( 'id', $user_id );
	 ...
	 * @ticket 29696
	 */
	public function test_gc_insert_user_should_sanitize_user_nicename_parameter() {
		$user = $this->author;

		$userdata                  = $user->to_array();
		$userdata['user_nicename'] = str_replace( '-', '.', $user->user_nicename );
		gc_insert_user( $userdata );

		$updated_user = new GC_User( $user->ID );
	 ...
	 * @ticket 33793
	 */
	public function test_gc_insert_user_should_accept_user_login_with_60_characters() {
	 ...
	 * @ticket 33793
	 */
	public function test_gc_insert_user_should_reject_user_login_over_60_characters() {
	 ...
	 * @ticket 33793
	 */
	public function test_gc_insert_user_should_reject_user_nicename_over_50_characters() {
	 ...
	 * @ticket 33793
	 */
	public function test_gc_insert_user_should_not_generate_user_nicename_longer_than_50_chars() {
	 ...
	 * @ticket 33793
	 */
	public function test_gc_insert_user_should_not_truncate_to_a_duplicate_user_nicename() {
	 ...
	 * @ticket 33793
	 */
	public function test_gc_insert_user_should_not_truncate_to_a_duplicate_user_nicename_when_suffix_has_more_than_one_character() {
	 ...
	 * @ticket 44107
	 */
	public function test_gc_insert_user_should_reject_user_url_over_100_characters() {
	 ...
	 * @ticket 28004
	 */
	public function test_gc_insert_user_with_invalid_user_id() {
		global $gcdb;
		$max_user = $gcdb->get_var( "SELECT MAX(ID) FROM $gcdb->users" );
	 ...
	 * @ticket 47902
	 */
	public function test_gc_insert_user_with_empty_data() {
		add_filter( 'gc_pre_insert_user_data', '__return_empty_array' );

		$u = self::factory()->user->create();

		remove_filter( 'gc_pre_insert_user_data', '__return_empty_array' );
	 ...
	 * @ticket 35750
	 */
	public function test_gc_update_user_should_delete_userslugs_cache() {
		$u    = self::factory()->user->create();
		$user = get_userdata( $u );
		...
		$this->assertFalse( gc_cache_get( $user->user_nicename, 'userslugs' ) );
		$this->assertEquals( $u, gc_cache_get( $updated_user->user_nicename, 'userslugs' ) );
	}
	...
	public function test_changing_email_invalidates_password_reset_key() {
		...
		$user = $this->author;
		$gcdb->update( $gcdb->users, array( 'user_activation_key' => 'key' ), array( 'ID' => $user->ID ) );
	...
	public function test_email_case() {
		...
		$update   = gc_update_user( $userdata );
	...
	/**
	 * Testing gc_new_user_notification email statuses.
	 *
	 * @dataProvider data_gc_new_user_notifications
	 * @ticket 33654
	 * @ticket 36009
	 */
	public function test_gc_new_user_notification( $notify, $admin_email_sent_expected, $user_email_sent_expected ) {
		reset_phpmailer_instance();

		$was_admin_email_sent = false;
		$was_user_email_sent  = false;

		gc_new_user_notification( self::$contrib_id, null, $notify );
		...
		if ( $first_recipient ) {
			$was_admin_email_sent = GC_TESTS_EMAIL === $first_recipient->address;
		...
		$this->assertSame( $admin_email_sent_expected, $was_admin_email_sent, 'Admin email result was not as expected in test_gc_new_user_notification' );
		$this->assertSame( $user_email_sent_expected, $was_user_email_sent, 'User email result was not as expected in test_gc_new_user_notification' );
	}

	/**
	 * Data provider for test_gc_new_user_notification().
	 *
	 ...
	 * }
	 */
	public function data_gc_new_user_notifications() {
		return array(
			array(
				'',
				true,
	 ...
	 * function signature `gc_new_user_notification( $user, 'plaintext_password' );`.
	 *
	 * @ticket 33654
	 * @expectedDeprecated gc_new_user_notification
	 */
	public function test_gc_new_user_notification_old_signature_throws_deprecated_warning_but_sends() {
		reset_phpmailer_instance();

		$was_admin_email_sent = false;
		$was_user_email_sent  = false;
		gc_new_user_notification( self::$contrib_id, 'this_is_a_test_password' );
		...
		if ( ! empty( $GLOBALS['phpmailer']->mock_sent ) ) {
			$was_admin_email_sent = ( isset( $GLOBALS['phpmailer']->mock_sent[0] ) && GC_TESTS_EMAIL === $GLOBALS['phpmailer']->mock_sent[0]['to'][0][0] );
	...
	/**
	 * Set up a user and try sending a notification using `gc_new_user_notification( $user );`.
	 *
	 * @ticket 34377
	 */
	public function test_gc_new_user_notification_old_signature_no_password() {
		reset_phpmailer_instance();

		$was_admin_email_sent = false;
		$was_user_email_sent  = false;
		gc_new_user_notification( self::$contrib_id );
	...
	public function test_profile_email_confirmation_not_sent_invalid_email( $email, $message ) {
		...
		gc_set_current_user( $old_current );
	}
	...
	public function test_edit_user_role_update() {
		...
		gc_set_current_user( $administrator );
	...
	/**
	 * Testing the `gc_user_personal_data_exporter()` function when no user exists.
	 *
	 * @ticket 43547
	 */
	public function test_gc_user_personal_data_exporter_no_user() {
		$actual = gc_user_personal_data_exporter( 'not-a-user-email@test.com' );
	...
	/**
	 * Testing the `gc_user_personal_data_exporter()` function when the requested
	 * user exists.
	 *
	 * @ticket 43547
	 */
	public function test_gc_user_personal_data_exporter() {
		$test_user = new GC_User( self::$contrib_id );

		$actual = gc_user_personal_data_exporter( $test_user->user_email );
	...
	/**
	 * Testing the `gc_user_personal_data_exporter()` function
	 * with Community Events Location IP data.
	 *
	 * @ticket 43921
	 */
	public function test_gc_community_events_location_ip_personal_data_exporter() {
		$test_user = new GC_User( self::$contrib_id );
	 ...
	 * @ticket 43921
	 */
	public function test_gc_community_events_location_city_personal_data_exporter() {
		$test_user = new GC_User( self::$contrib_id );
	 ...
	 * @ticket 45889
	 */
	public function test_gc_session_tokens_personal_data_exporter() {
		$test_user = new GC_User( self::$contrib_id );
	...
	/**
	 * Test `$user_data['meta_input']` args in `gc_insert_user( $user_data )`.
	 *
	 * @ticket 41950
	 */
	public function test_gc_insert_user_with_meta() {
		$user_data   = array(
		...
		$create_user = gc_insert_user( $user_data );
		...
		// Update the user meta thru gc_insert_user.
		$update_data = array(
			'ID'         => $create_user,
			'user_login' => 'test_user',
			'meta_input' => array(
		...
		$update_user = gc_insert_user( $update_data );
		...
		$new_user = gc_insert_user( $new_user_data );
	 ...
	 * This hook is used in `test_gc_insert_user_with_meta()`.
	 */
	...
	/**
	 * Testing the `gc_privacy_additional_user_profile_data` filter works.
	 *
	 * @since 5.4.0
	 *
	 * @ticket 47509
	 */
	public function test_filter_gc_privacy_additional_user_profile_data() {
		$test_user = new GC_User( self::$contrib_id );

		add_filter( 'gc_privacy_additional_user_profile_data', array( $this, 'export_additional_user_profile_data' ) );

		$actual = gc_user_personal_data_exporter( $test_user->user_email );

		remove_filter( 'gc_privacy_additional_user_profile_data', array( $this, 'export_additional_user_profile_data' ) );
		...
		$this->setExpectedIncorrectUsage( 'gc_user_personal_data_exporter' );
		add_filter( 'gc_privacy_additional_user_profile_data', array( $this, 'export_additional_user_profile_data_with_dup_name' ) );
	...
	public function export_additional_user_profile_data_with_dup_name() {
		$additional_profile_data = array(
			// This item should be stripped out by gc_user_personal_data_exporter()
