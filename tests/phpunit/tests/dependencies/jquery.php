 ...
 * @group scripts
 */
class Tests_Dependencies_jQuery extends GC_UnitTestCase {

	/**
	 * @covers GC_Scripts::query
	 */
	public function test_location_of_jquery() {
		$scripts = new GC_Scripts;
		gc_default_scripts( $scripts );

		if ( SCRIPT_DEBUG ) {
			$jquery_scripts = array(
				'jquery-core'    => '/gc-includes/js/jquery/jquery.js',
				'jquery-migrate' => '/gc-includes/js/jquery/jquery-migrate.js',
			);
		} else {
			$jquery_scripts = array(
				'jquery-core'    => '/gc-includes/js/jquery/jquery.min.js',
				'jquery-migrate' => '/gc-includes/js/jquery/jquery-migrate.min.js',
			);
		}

		$object = $scripts->query( 'jquery', 'registered' );
		$this->assertInstanceOf( '_GC_Dependency', $object );

		// The following test is disabled in GC 5.5 as jQuery 1.12.4 is loaded without jQuery Migrate 1.4.1,
	 ...
	 * @ticket 22896
	 *
	 * @expectedIncorrectUsage gc_deregister_script
	 *
	 * @covers ::gc_script_is
	 */
	public function test_dont_allow_deregister_core_scripts_in_admin() {
		...
		foreach ( $libraries as $library ) {
			...
			gc_deregister_script( $library );
			$this->assertTrue( gc_script_is( $library, 'registered' ) );
		}
	}
	 ...
	 * @covers ::gc_script_is
	 */
	public function test_gc_script_is_dep_enqueued() {
		gc_enqueue_script( 'jquery-ui-accordion' );

		$this->assertTrue( gc_script_is( 'jquery', 'enqueued' ) );
		$this->assertFalse( gc_script_is( 'underscore', 'enqueued' ) );
