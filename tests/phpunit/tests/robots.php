 ...
 * @group robots
 */
class Tests_Robots extends GC_UnitTestCase {

	public function set_up() {
		parent::set_up();

		remove_all_filters( 'gc_robots' );
	}

	/**
	 * @ticket 51511
	 */
	public function test_gc_robots_renders_when_relevant() {
		// Do not render robots meta tag when there are no directives.
		$output = get_echo( 'gc_robots' );
		$this->assertEmpty( $output );

		// Render robots meta tag with noindex.
		add_filter( 'gc_robots', array( $this, 'add_noindex_directive' ) );
		$output = get_echo( 'gc_robots' );
		$this->assertSame( "<meta name='robots' content='noindex' />\n", $output );

		// Do not render robots meta tag when there are only false-y directives.
		add_filter( 'gc_robots', array( $this, 'remove_noindex_directive' ), 11 );
		$output = get_echo( 'gc_robots' );
		$this->assertEmpty( $output );
	}
	 ...
	 * @ticket 51511
	 */
	public function test_gc_robots_parses_directives_correctly() {
		add_filter(
			'gc_robots',
	 ...
	 * @ticket 51511
	 */
	public function test_gc_robots_noindex() {
	 ...
	 * @ticket 51511
	 */
	public function test_gc_robots_no_robots() {
	 ...
	 * @ticket 51511
	 */
	public function test_gc_robots_sensitive_page() {
	 ...
	 * @ticket 51511
	 */
	public function test_gc_robots_max_image_preview_large() {
	 ...
	 * @ticket 52457
	 */
	public function test_gc_robots_search_page() {
		add_filter( 'gc_robots', 'gc_robots_noindex_search' );
		$this->go_to( home_url( '?s=ticket+52457+core.trac.gechiui.com' ) );
	 ...
	 * @ticket 52457
	 */
	public function test_gc_robots_non_search_page() {
		add_filter( 'gc_robots', 'gc_robots_noindex_search' );
		$this->go_to( home_url() );
