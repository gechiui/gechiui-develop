 ...
 * @group sitemaps
 */
class Tests_Sitemaps_gcSitemapsTaxonomies extends GC_UnitTestCase {
	 ...
	 * GC_Sitemaps_Taxonomies::get_url_list().
	 */
	public function test_get_url_list_taxonomies() {
		...
		$tax_provider = new GC_Sitemaps_Taxonomies();
	...
	public function test_get_sitemap_entries_custom_taxonomies() {
		...
		$tax_provider = new GC_Sitemaps_Taxonomies();
		$entries      = gc_list_pluck( $tax_provider->get_sitemap_entries(), 'loc' );
		...
		$this->assertContains( 'http://' . GC_TESTS_DOMAIN . '/?sitemap=taxonomies&sitemap-subtype=public_taxonomy&paged=1', $entries, 'Public Taxonomies are not in the index.' );
		$this->assertNotContains( 'http://' . GC_TESTS_DOMAIN . '/?sitemap=taxonomies&sitemap-subtype=non_queryable_taxonomy&paged=1', $entries, 'Private Taxonomies are visible in the index.' );
		$this->assertNotContains( 'http://' . GC_TESTS_DOMAIN . '/?sitemap=taxonomies&sitemap-subtype=private_taxonomy&paged=1', $entries, 'Private Taxonomies are visible in the index.' );
	}
	...
	public function test_filter_sitemaps_taxonomies() {
		$taxonomies_provider = new GC_Sitemaps_Taxonomies();

		// Return an empty array to show that the list of subtypes is filterable.
		add_filter( 'gc_sitemaps_taxonomies', '__return_empty_array' );
