 ...
 * @group sitemaps
 */
class Tests_Sitemaps_gcSitemapsPosts extends GC_UnitTestCase {
	...
	public function test_get_sitemap_entries_homepage() {
		...
		$posts_provider = new GC_Sitemaps_Posts();
	...
	public function test_filter_sitemaps_post_types() {
		...
		add_filter( 'gc_sitemaps_post_types', '__return_empty_array' );
	...
	/**
	 * Test `gc_sitemaps_posts_show_on_front_entry` filter.
	 */
	public function test_posts_show_on_front_entry() {
		$posts_provider = new GC_Sitemaps_Posts();
		update_option( 'show_on_front', 'page' );

		add_filter( 'gc_sitemaps_posts_show_on_front_entry', array( $this, '_show_on_front_entry' ) );
	...
	/**
	 * Callback for 'gc_sitemaps_posts_show_on_front_entry' filter.
	 */
	public function _show_on_front_entry( $sitemap_entry ) {
		$sitemap_entry['lastmod'] = gc_date( DATE_W3C, time() );
