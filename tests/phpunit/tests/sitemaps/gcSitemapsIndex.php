 ...
 * @group sitemaps
 */
class Tests_Sitemaps_gcSitemapsIndex extends GC_UnitTestCase {
	...
	public function test_get_sitemap_list() {
		 ...
		 * Hence, 3*4*2=24.
		 */
		$registry->add_provider( 'foo', new GC_Sitemaps_Test_Provider( 'foo' ) );
		$registry->add_provider( 'bar', new GC_Sitemaps_Test_Provider( 'bar' ) );

		$sitemap_index = new GC_Sitemaps_Index( $registry );
	...
	public function test_get_sitemap_list_limit() {
		...
		$registry->add_provider( 'provider_1', new GC_Sitemaps_Large_Test_Provider( 25000 ) );
		$registry->add_provider( 'provider_2', new GC_Sitemaps_Large_Test_Provider( 25000 ) );
		$registry->add_provider( 'provider_3', new GC_Sitemaps_Large_Test_Provider( 25000 ) );
	...
	public function test_get_sitemap_list_no_entries() {
		...
		$registry->add_provider( 'foo', new GC_Sitemaps_Empty_Test_Provider( 'foo' ) );
	...
	public function test_get_index_url() {
		$sitemap_index = new GC_Sitemaps_Index( new GC_Sitemaps_Registry() );
	...
	public function test_get_index_url_pretty_permalinks() {
		...
		$this->assertStringEndsWith( '/gc-sitemap.xml', $index_url );
	}
}
