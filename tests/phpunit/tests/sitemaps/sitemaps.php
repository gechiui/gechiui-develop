 ...
 * @link      https://github.com/GoogleChromeLabs/gc-sitemaps
 */
 ...
 * @group sitemaps
 */
class Tests_Sitemaps_Sitemaps extends GC_UnitTestCase {
	 ...
	 * Set up fixtures.
	 *
	 * @param GC_UnitTest_Factory $factory A GC_UnitTest_Factory object.
	 */
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$test_provider = new GC_Sitemaps_Test_Provider();
	}
	...
	public function _get_sitemap_entries() {
		...
		$providers = gc_get_sitemap_providers();
	...
	public function test_get_sitemap_entries() {
		...
		$expected = array(
			array(
				'loc' => 'http://' . GC_TESTS_DOMAIN . '/?sitemap=posts&sitemap-subtype=post&paged=1',
			),
			array(
				'loc' => 'http://' . GC_TESTS_DOMAIN . '/?sitemap=posts&sitemap-subtype=page&paged=1',
			),
			array(
				'loc' => 'http://' . GC_TESTS_DOMAIN . '/?sitemap=taxonomies&sitemap-subtype=category&paged=1',
			),
			array(
				'loc' => 'http://' . GC_TESTS_DOMAIN . '/?sitemap=taxonomies&sitemap-subtype=post_tag&paged=1',
			),
			array(
				'loc' => 'http://' . GC_TESTS_DOMAIN . '/?sitemap=users&paged=1',
			),
		);
	...
	public function test_get_sitemap_entries_custom_post_types() {
		...
		$entries = gc_list_pluck( $this->_get_sitemap_entries(), 'loc' );

		// Clean up.
		unregister_post_type( 'public_cpt' );
		unregister_post_type( 'private_cpt' );

		$this->assertContains( 'http://' . GC_TESTS_DOMAIN . '/?sitemap=posts&sitemap-subtype=public_cpt&paged=1', $entries, 'Public CPTs are not in the index.' );
		$this->assertNotContains( 'http://' . GC_TESTS_DOMAIN . '/?sitemap=posts&sitemap-subtype=private_cpt&paged=1', $entries, 'Private CPTs are visible in the index.' );
	}
	...
	public function test_get_sitemap_entries_not_publicly_queryable_post_types() {
		...
		$this->assertNotContains( 'http://' . GC_TESTS_DOMAIN . '/?sitemap=posts&sitemap-subtype=non_queryable_cpt&paged=1', $entries, 'Non-publicly queryable CPTs are visible in the index.' );
	}
	...
	public function test_register_sitemap_provider() {
		gc_register_sitemap_provider( 'test_sitemap', self::$test_provider );

		$sitemaps = gc_get_sitemap_providers();
	...
	public function test_robots_text() {
		...
		$sitemap_string = 'Sitemap: http://' . GC_TESTS_DOMAIN . '/?sitemap=index';
	...
	public function test_robots_text_with_permalinks() {
		...
		// Get the text added to the default robots text output.
		$robots_text    = apply_filters( 'robots_txt', '', true );
		$sitemap_string = 'Sitemap: http://' . GC_TESTS_DOMAIN . '/gc-sitemap.xml';
	...
	public function test_sitemaps_enabled() {
		$before = gc_sitemaps_get_server()->sitemaps_enabled();
		add_filter( 'gc_sitemaps_enabled', '__return_false' );
		$after = gc_sitemaps_get_server()->sitemaps_enabled();
		remove_filter( 'gc_sitemaps_enabled', '__return_false' );
	...
	public function test_disable_sitemap_should_return_404() {
		...
		gc_sitemaps_get_server()->render_sitemaps();
	...
	public function test_empty_url_list_should_return_404() {
		gc_register_sitemap_provider( 'foo', new GC_Sitemaps_Empty_Test_Provider( 'foo' ) );
