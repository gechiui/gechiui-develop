 ...
 * @group sitemaps
 */
class Tests_Sitemaps_Functions extends GC_UnitTestCase {

	/**
	 * Test getting the correct number of URLs for a sitemap.
	 */
	public function test_gc_sitemaps_get_max_urls() {
		// Apply a filter to test filterable values.
		add_filter( 'gc_sitemaps_max_urls', array( $this, '_filter_max_url_value' ), 10, 2 );

		$expected_posts      = gc_sitemaps_get_max_urls( 'post' );
		$expected_taxonomies = gc_sitemaps_get_max_urls( 'term' );
		$expected_users      = gc_sitemaps_get_max_urls( 'user' );
	...
	/**
	 * Test gc_get_sitemap_providers default functionality.
	 */
	public function test_gc_get_sitemap_providers() {
		$sitemaps = gc_get_sitemap_providers();

		$expected = array(
			'posts'      => 'GC_Sitemaps_Posts',
			'taxonomies' => 'GC_Sitemaps_Taxonomies',
			'users'      => 'GC_Sitemaps_Users',
		);
	...
	public function pretty_permalinks_provider() {
		return array(
			array( 'posts', 'post', 1, home_url( '/gc-sitemap-posts-post-1.xml' ) ),
			array( 'posts', 'post', 0, home_url( '/gc-sitemap-posts-post-1.xml' ) ),
			array( 'posts', 'page', 1, home_url( '/gc-sitemap-posts-page-1.xml' ) ),
			array( 'posts', 'page', 5, home_url( '/gc-sitemap-posts-page-5.xml' ) ),
			// Post type doesn't exist.
			array( 'posts', 'foo', 5, false ),
			array( 'taxonomies', 'category', 1, home_url( '/gc-sitemap-taxonomies-category-1.xml' ) ),
			array( 'taxonomies', 'post_tag', 1, home_url( '/gc-sitemap-taxonomies-post_tag-1.xml' ) ),
			// Negative paged, gets converted to its absolute value.
			array( 'taxonomies', 'post_tag', -1, home_url( '/gc-sitemap-taxonomies-post_tag-1.xml' ) ),
			array( 'users', '', 4, home_url( '/gc-sitemap-users-4.xml' ) ),
			// Users provider doesn't allow subtypes.
			array( 'users', 'foo', 4, false ),
			// Provider doesn't exist.
			array( 'foo', '', 4, false ),
