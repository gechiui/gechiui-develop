 ...
 * @group privacy
 *
 * @covers ::gc_ajax_gc_privacy_export_personal_data
 */
class Tests_Ajax_PrivacyExportPersonalData extends GC_Ajax_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$request_id             = gc_create_user_request( self::$request_email, 'export_personal_data' );
		self::$action                 = 'gc-privacy-export-personal-data';
		self::$exporter               = 1;
	...
	public function set_up() {
		...
		remove_all_filters( 'gc_privacy_personal_data_export_page' );

		// Only use our custom privacy personal data exporter.
		remove_all_filters( 'gc_privacy_personal_data_exporters' );
		add_filter( 'gc_privacy_personal_data_exporters', array( $this, 'filter_register_custom_personal_data_exporter' ) );
	...
	public function tear_down() {
		remove_filter( 'gc_privacy_personal_data_exporters', array( $this, 'filter_register_custom_personal_data_exporter' ) );
	...
	protected function _set_exporter_callback( $callback ) {
		...
		add_filter( 'gc_privacy_personal_data_exporters', array( $this, 'filter_exporter_callback_value' ), 20 );
	}
	...
	protected function _unset_exporter_key( $key ) {
		...
		add_filter( 'gc_privacy_personal_data_exporters', array( $this, 'filter_unset_exporter_key' ), 20 );
	}
	...
	public function test_error_when_incorrect_request_type() {
		$request_id = gc_create_user_request(
		...
		$this->_make_ajax_call(
			array(
				'security' => gc_create_nonce( 'gc-privacy-export-personal-data-' . $request_id ),
				'id'       => $request_id,
			)
		);
	...
	/**
	 * The function should send an error when an exporter has improperly used the `gc_privacy_personal_data_exporters` filter.
	 *
	 * @since 5.2.0
	 */
	public function test_error_when_exporter_has_improperly_used_exporters_filter() {
		...
		add_filter( 'gc_privacy_personal_data_exporters', '__return_false', 999 );
		$this->_make_ajax_call();
	...
	/**
	 * When an exporter callback returns a GC_Error, it should be passed as the error.
	 *
	 * @since 5.2.0
	 */
	public function test_error_when_exporter_callback_returns_gc_error() {
		$this->_set_exporter_callback( array( $this, 'callback_return_gc_error' ) );
		$this->_make_ajax_call();

		$this->assertFalse( $this->_last_response_parsed['success'] );
		$this->assertSame( 'passed_message', $this->_last_response_parsed['data'][0]['code'] );
		$this->assertSame( 'This is a GC_Error message.', $this->_last_response_parsed['data'][0]['message'] );
	}
	 ...
	 * @return GC_Error GC_Error instance.
	 */
	public function callback_return_gc_error( $email_address, $page = 1 ) {
		return new GC_Error( 'passed_message', 'This is a GC_Error message.' );
	}
	...
	/**
	 * The function's output should be filterable with the `gc_privacy_personal_data_export_page` filter.
	 *
	 * @since 5.2.0
	 */
	public function test_output_should_be_filterable() {
		add_filter( 'gc_privacy_personal_data_export_page', array( $this, 'filter_exporter_data_response' ), 20, 7 );
		$this->_make_ajax_call();
	...
	protected function _make_ajax_call( $args = array() ) {
		...
		$defaults = array(
			'action'      => self::$action,
			'security'    => gc_create_nonce( self::$action . '-' . self::$request_id ),
		...
		$_POST = gc_parse_args( $args, $defaults );
