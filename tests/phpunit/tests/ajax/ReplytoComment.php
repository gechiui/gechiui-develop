 ...
 * @group      ajax
 */
class Tests_Ajax_ReplytoComment extends GC_Ajax_UnitTestCase {
	...
	public function test_blocked_comment() {
		...
		try {
			...
			$this->fail();
		} catch ( GCAjaxDieStopException $e ) {
			$gcdb->suppress_errors( false );
	...
	public function _block_comments( $sql ) {
		global $gcdb;
		if ( false !== strpos( $sql, $gcdb->comments ) && 0 === stripos( trim( $sql ), 'INSERT INTO' ) ) {
			return '';
		}
		return $sql;
	}
	...
	/**
	 * Blocks comments from being saved on 'pre_comment_approved', by returning GC_Error.
	 */
	public function _pre_comment_approved_filter( $approved, $commentdata ) {
		return new GC_Error( 'comment_wrong', 'pre_comment_approved filter fails for new comment.', 403 );
	}
}
