 ...
 * @group privacy
 *
 * @covers ::gc_ajax_gc_privacy_erase_personal_data
 */
class Tests_Ajax_PrivacyErasePersonalData extends GC_Ajax_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$request_id           = gc_create_user_request( self::$request_email, 'remove_personal_data' );
		self::$action               = 'gc-privacy-erase-personal-data';
		self::$eraser               = 1;
	...
	public function set_up() {
		...
		remove_all_filters( 'gc_privacy_personal_data_erasure_page' );
		remove_all_actions( 'gc_privacy_personal_data_erased' );

		// Only use our custom privacy personal data eraser.
		remove_all_filters( 'gc_privacy_personal_data_erasers' );
		add_filter( 'gc_privacy_personal_data_erasers', array( $this, 'register_custom_personal_data_eraser' ) );
	...
	public function tear_down() {
		remove_filter( 'gc_privacy_personal_data_erasers', array( $this, 'register_custom_personal_data_eraser' ) );
	...
	protected function _set_eraser_callback( $callback ) {
		...
		add_filter( 'gc_privacy_personal_data_erasers', array( $this, 'filter_eraser_callback_value' ), 20 );
	}
	...
	protected function _unset_eraser_key( $key ) {
		...
		add_filter( 'gc_privacy_personal_data_erasers', array( $this, 'filter_unset_eraser_index' ), 20 );
	}
	...
	public function test_error_when_incorrect_request_type() {
		...
		$this->_make_ajax_call(
			array(
				'security' => gc_create_nonce( 'gc-privacy-erase-personal-data-' . $request_id ),
				'id'       => $request_id,
			)
		);
	...
	/**
	 * Test that the function's output should be filterable with the `gc_privacy_personal_data_erasure_page` filter.
	 *
	 * @since 5.2.0
	 */
	public function test_output_should_be_filterable() {
		add_filter( 'gc_privacy_personal_data_erasure_page', array( $this, 'filter_eraser_data_response' ), 20, 6 );
		$this->_make_ajax_call();
