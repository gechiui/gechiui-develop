 ...
 * @group      ajax
 */
class Tests_Ajax_DeleteComment extends GC_Ajax_UnitTestCase {
	...
	public function _test_as_admin( $comment, $action ) {
		...
		$_POST['_ajax_nonce'] = gc_create_nonce( 'delete-comment_' . $comment->comment_ID );
		$_POST[ $action ]     = 1;
	...
	public function _test_with_bad_id( $comment, $action ) {
		...
		// Set up the $_POST request.
		$_POST['id']          = 12346789;
		$_POST['_ajax_nonce'] = gc_create_nonce( 'delete-comment_12346789' );
		$_POST[ $action ]     = 1;
		$_POST['_total']      = count( self::$comments );
		$_POST['_per_page']   = 100;
		$_POST['_page']       = 1;
		...
		try {
			$this->_handleAjax( 'delete-comment' );
			$this->fail( 'Expected exception: GCAjaxDieStopException' );
	...
	public function _test_double_action( $comment, $action ) {
		...
		if ( 'delete' === $action ) {
			gc_delete_comment( $comment->comment_ID, true );
		}
