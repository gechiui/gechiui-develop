 ...
 * @group      ajax
 */
class Tests_Ajax_DimComment extends GC_Ajax_UnitTestCase {
	...
	public function _test_as_admin( $comment ) {
		...
		$_POST['_ajax_nonce'] = gc_create_nonce( 'approve-comment_' . $comment->comment_ID );
		...
		$prev_status = gc_get_comment_status( $comment->comment_ID );
		...
		// Check the status.
		$current = gc_get_comment_status( $comment->comment_ID );
		...
		$comment_count = gc_count_comments( 0 );
	...
	public function _test_with_bad_nonce( $comment ) {
		...
		// Set up the $_POST request.
		$_POST['id']          = $comment->comment_ID;
		$_POST['_ajax_nonce'] = gc_create_nonce( uniqid() );
		$_POST['_total']      = count( $this->_comments );
		$_POST['_per_page']   = 100;
		$_POST['_page']       = 1;
		$_POST['_url']        = admin_url( 'edit-comments.php' );
	...
	public function test_with_bad_id() {
		...
		// Set up the $_POST request.
		$_POST['id']          = 12346789;
		$_POST['_ajax_nonce'] = gc_create_nonce( 'dim-comment_12346789' );
		$_POST['_total']      = count( $this->_comments );
		$_POST['_per_page']   = 100;
		$_POST['_page']       = 1;
		$_POST['_url']        = admin_url( 'edit-comments.php' );
		...
		try {
			$this->_handleAjax( 'dim-comment' );
			$this->fail( 'Expected exception: GCAjaxDieContinueException' );
		} catch ( GCAjaxDieContinueException $e ) {
