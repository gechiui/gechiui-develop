 ...
 * @group      ajax
 */
class Tests_Ajax_EditComment extends GC_Ajax_UnitTestCase {
	...
	public function test_as_admin() {
		...
		$_POST['_ajax_nonce-replyto-comment'] = gc_create_nonce( 'replyto-comment' );
	...
	public function test_editor_can_edit_orphan_comments() {
		...
		// Manually update the comment_post_ID, because gc_update_comment() will prevent it..
		$gcdb->update( $gcdb->comments, array( 'comment_post_ID' => 0 ), array( 'comment_ID' => $comment->comment_ID ) );
	...
	public function test_bad_nonce() {
		...
		// Set up a default request.
		$_POST['_ajax_nonce-replyto-comment'] = gc_create_nonce( uniqid() );
		$_POST['comment_ID']                  = $comment->comment_ID;
		$_POST['content']                     = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit.';
	 ...
	 * @ticket 39732
	 */
	public function test_gc_update_comment_data_is_gc_error() {
		// Become an administrator.
		$this->_setRole( 'administrator' );
		...
		// Make the request.
		$this->expectException( 'GCAjaxDieStopException' );
		$this->expectExceptionMessage( 'gc_update_comment_data filter fails for this comment.' );
		$this->_handleAjax( 'edit-comment' );
	}
