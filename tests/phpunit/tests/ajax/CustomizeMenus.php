 ...
 * @group      ajax
 */
class Tests_Ajax_CustomizeMenus extends GC_Ajax_UnitTestCase {
	...
	public function test_ajax_load_available_items_cap_check( $role, $expected_results ) {
		...
		if ( 'administrator' !== $role ) {
			// If we're not an admin, we should get a gc_die( -1 ).
			$this->expectException( 'GCAjaxDieStopException' );
			$this->expectExceptionMessage( '-1' );
		}

		gc_set_current_user( self::factory()->user->create( array( 'role' => $role ) ) );

		$_POST = array(
			'action'                => 'load-available-menu-items-customizer',
			'customize-menus-nonce' => gc_create_nonce( 'customize-menus' ),
		);
	...
	public function test2_ajax_load_available_items_structure( $post_args ) {
		...
		$auto_draft_post = $this->gc_customize->nav_menus->insert_auto_draft_post(
			array(
				'post_title' => 'Test Auto Draft',
				'post_type'  => 'post',
			)
		);
		$this->gc_customize->set_post_value( 'nav_menus_created_posts', array( $auto_draft_post->ID ) );
		$this->gc_customize->get_setting( 'nav_menus_created_posts' )->preview();
		...
		if ( 'page' === $test_item['object'] ) {
			...
			$item_ids = gc_list_pluck( $response['data']['items']['post_type:post'], 'id' );
	...
	public function test_ajax_search_available_items_results( $post_args, $expected_results ) {
		...
		$included_auto_draft_post = $this->gc_customize->nav_menus->insert_auto_draft_post(
			array(
				'post_title' => 'Test Included Auto Draft',
				'post_type'  => 'post',
			)
		);
		$excluded_auto_draft_post = $this->gc_customize->nav_menus->insert_auto_draft_post(
			array(
				'post_title' => 'Excluded Auto Draft',
				'post_type'  => 'post',
			)
		);
		$this->gc_customize->set_post_value( 'nav_menus_created_posts', array( $included_auto_draft_post->ID, $excluded_auto_draft_post->ID ) );
		$this->gc_customize->get_setting( 'nav_menus_created_posts' )->preview();
		...
		if ( isset( $post_args['search'] ) && 'test' === $post_args['search'] ) {
			...
			$item_ids = gc_list_pluck( $response['data']['items'], 'id' );
	 ...
	 *
	 * @covers GC_Customize_Nav_Menus::ajax_insert_auto_draft_post
	 */
	public function test_ajax_insert_auto_draft_post_success() {
		...
		$this->assertSame( $this->gc_customize->changeset_uuid(), get_post_meta( $post->ID, '_customize_changeset_uuid', true ) );
	}
	...
	public function test_ajax_insert_auto_draft_failures() {
		...
		// Bad nonce.
		gc_set_current_user( $this->factory()->user->create( array( 'role' => 'subscriber' ) ) );
		$_POST                = gc_slash(
			array(
				'customize-menus-nonce' => gc_create_nonce( 'customize-menus' ),
			)
