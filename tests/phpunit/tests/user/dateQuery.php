 ...
 * @group datequery
 */
class Tests_User_DateQuery extends GC_UnitTestCase {
	/**
	 * @ticket 27283
	 */
	public function test_user_registered() {
		...
		$this->assertSameSets( array( $u1 ), gc_list_pluck( $uq->results, 'ID' ) );
	}
	...
	public function test_user_registered_relation_or() {
		...
		$this->assertSameSets( array( $u1, $u2 ), gc_list_pluck( $uq->results, 'ID' ) );
	}
}
