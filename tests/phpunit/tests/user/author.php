...
/**
 * Test functions in gc-includes/author-template.php
 *
 * @group author
 * @group user
 */
class Tests_User_Author_Template extends GC_UnitTestCase {
	...
	public function test_get_the_author_posts_link_no_permalinks() {
		...
		$url = sprintf( 'http://%1$s/?author=%2$s', GC_TESTS_DOMAIN, $author->ID );
	...
	public function test_get_the_author_posts_link_with_permalinks() {
		...
		$url = sprintf( 'http://%1$s/author/%2$s/', GC_TESTS_DOMAIN, $author->user_nicename );
