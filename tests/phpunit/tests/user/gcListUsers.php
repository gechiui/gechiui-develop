 ...
 * @group user
 *
 * @covers ::gc_list_users
 */
class Tests_Functions_gcListUsers extends GC_UnitTestCase {
	...
	/**
	 * Test that gc_list_users() creates the expected list of users.
	 *
	...
	/**
	 * Tests that gc_list_users() does not create a user list.
	 *
	...
	public function test_should_not_create_a_user_list( $args ) {
		...
		if ( null === $actual ) {
			$this->expectOutputString( '', 'gc_list_users() did not output an empty string.' );
		} else {
			$this->assertSame( $actual, 'gc_list_users() did not return an empty string.' );
		}
	}
