...
if ( is_multisite() ) :
	 ...
	 * @group multisite
	 */
	class Tests_Multisite_User extends GC_UnitTestCase {
		...
		public function test_is_user_member_of_blog() {
			...
			gcmu_delete_user( $user1_id );
			$user = new GC_User( $user1_id );
		...
		public function test_revoked_super_admin_can_be_deleted() {
			...
			$this->assertTrue( gcmu_delete_user( $user_id ) );
		...
		public function test_revoked_super_admin_is_deleted() {
			...
			$this->assertFalse( $user->exists(), 'GC_User->exists' );
		...
		public function test_super_admin_cannot_be_deleted() {
			...
			$this->assertFalse( gcmu_delete_user( $user_id ) );
		...
		public function test_numeric_string_user_id() {
			...
			$u_string = (string) $u;
			$this->assertTrue( gcmu_delete_user( $u_string ) );
		...
		public function test_should_return_false_for_non_numeric_string_user_id() {
			$this->assertFalse( gcmu_delete_user( 'abcde' ) );
		}
		...
		public function test_should_return_false_for_object_user_id() {
			...
			$this->assertFalse( gcmu_delete_user( $u_obj ) );
		 ...
		 * @ticket 23016
		 */
		public function test_gc_roles_global_is_reset() {
			global $gc_roles;
			$role      = 'test_global_is_reset';
			$role_name = 'Test Global Is Reset';
			$blog_id   = self::factory()->blog->create();

			$gc_roles->add_role( $role, $role_name, array() );

			$this->assertNotEmpty( $gc_roles->get_role( $role ) );

			switch_to_blog( $blog_id );

			$this->assertEmpty( $gc_roles->get_role( $role ) );
