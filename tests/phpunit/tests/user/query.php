<?php
/**
 * Test GC_User Query, in gc-includes/user.php
 *
 * @group user
 */
class Tests_User_Query extends GC_UnitTestCase {
	...
	public function test_get_all() {
		$users = new GC_User_Query( array( 'blog_id' => get_current_blog_id() ) );
	...
	public function test_orderby_include() {
		...
		$expected_orderby = 'ORDER BY FIELD( ' . $gcdb->users . '.ID, ' . self::$author_ids[1] . ',' . self::$author_ids[0] . ',' . self::$author_ids[3] . ' )';
	...
	public function test_who_authors() {
		...
		$found = gc_list_pluck( $q->get_results(), 'ID' );
	...
	public function test_has_published_posts_with_value_true_should_show_authors_of_posts_in_public_post_types() {
		register_post_type( 'gctests_pt_public', array( 'public' => true ) );
		register_post_type( 'gctests_pt_private', array( 'public' => false ) );

		self::factory()->post->create(
			array(
				'post_author' => self::$author_ids[0],
				'post_status' => 'publish',
				'post_type'   => 'gctests_pt_public',
			)
		);
		self::factory()->post->create(
			array(
	...
	public function test_has_published_posts_should_obey_post_types() {
		...
		$q = new GC_User_Query(
			array(
				'has_published_posts' => array( 'gctests_pt_private', 'post' ),
			)
		);
	...
	public function test_has_published_posts_should_ignore_non_published_posts() {
		...
		$q = new GC_User_Query(
			array(
				'has_published_posts' => array( 'gctests_pt_public', 'gctests_pt_private', 'post' ),
			)
		);
	...
	public function test_query_vars_should_be_filled_in_after_pre_get_users() {
		...
		$q = new GC_User_Query( array_fill_keys( $query_vars, '1' ) );
	...
	public function test_get_single_role_by_user_query() {
		$gc_user_search = new GC_User_Query( array( 'role' => 'subscriber' ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_get_multiple_roles_by_user_query() {
		$gc_user_search = new GC_User_Query( array( 'role__in' => array( 'subscriber', 'editor' ) ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_get_multiple_roles_should_only_match_users_who_have_each_role() {
		$users = new GC_User_Query( array( 'role' => array( 'subscriber', 'editor' ) ) );
		$users = $users->get_results();
	...
	public function test_get_multiple_roles_or() {
		$users = new GC_User_Query( array( 'role__in' => array( 'subscriber', 'editor', 'administrator' ) ) );
		$users = $users->get_results();
	...
	public function test_get_single_capability_by_string() {
		$gc_user_search = new GC_User_Query( array( 'capability' => 'install_plugins' ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_get_single_capability_by_string_multisite() {
		$gc_user_search = new GC_User_Query( array( 'capability' => array( 'install_plugins' ) ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_get_single_capability_invalid() {
		$gc_user_search = new GC_User_Query( array( 'capability' => 'foo_bar' ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_get_single_capability_added_to_user() {
		...
		$gc_user_search = new GC_User_Query( array( 'capability' => 'custom_cap' ) );
		$users          = $gc_user_search->get_results();

		$this->assertCount( 2, $users );
		$this->assertEqualSets( self::$sub_ids, gc_list_pluck( $users, 'ID' ) );
	...
	public function test_get_multiple_capabilities_should_only_match_users_who_have_each_capability_test() {
		gc_roles()->add_role( 'role_1', 'Role 1', array( 'role_1_cap' => true ) );
		gc_roles()->add_role( 'role_2', 'Role 2', array( 'role_2_cap' => true ) );
		...
		$gc_user_search = new GC_User_Query( array( 'capability' => array( 'role_1_cap', 'role_2_cap' ) ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_get_multiple_capabilities_should_only_match_users_who_have_each_capability_added_to_user() {
		...
		$gc_user_search = new GC_User_Query( array( 'capability' => array( 'manage_options', 'custom_cap' ) ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_get_multiple_capabilities_or() {
		$gc_user_search = new GC_User_Query( array( 'capability__in' => array( 'publish_posts', 'edit_posts' ) ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_get_multiple_capabilities_or_added_to_user() {
		...
		$gc_user_search = new GC_User_Query( array( 'capability__in' => array( 'publish_posts', 'custom_cap' ) ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_capability_exclusion() {
		$gc_user_search = new GC_User_Query( array( 'capability__not_in' => array( 'publish_posts', 'edit_posts' ) ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_capability_exclusion_added_to_user() {
		...
		$gc_user_search = new GC_User_Query( array( 'capability__not_in' => array( 'publish_posts', 'custom_cap' ) ) );
		$users          = $gc_user_search->get_results();
	...
	public function test_capability__in_capability__not_in_combined() {
		$gc_user_search = new GC_User_Query(
			array(
	...
	public function test_get_single_capability_multisite_blog_id() {
		...
		$found = gc_list_pluck( $gc_user_search->get_results(), 'ID' );
