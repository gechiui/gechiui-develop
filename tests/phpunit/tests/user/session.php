<?php
/**
 * Test GC_Session_Tokens and GC_User_Meta_Session_Tokens, in gc-includes/session.php
 *
 * @group user
 * @group session
 */
class Tests_User_Session extends GC_UnitTestCase {

	public function set_up() {
		parent::set_up();
		remove_all_filters( 'session_token_manager' );
		$user_id       = self::factory()->user->create();
		$this->manager = GC_Session_Tokens::get_instance( $user_id );
		$this->assertInstanceOf( 'GC_Session_Tokens', $this->manager );
		$this->assertInstanceOf( 'GC_User_Meta_Session_Tokens', $this->manager );
	}
