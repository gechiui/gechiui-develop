...
/**
 * Test roles and capabilities via the GC_User class.
 *
 * @group user
 * @group capabilities
 */
class Tests_User_Capabilities extends GC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$block_id = $factory->post->create(
			array(
				...
				'post_content' => '<!-- gc:core/paragraph --><p>Hello world!</p><!-- /gc:core/paragraph -->',
			)
		);
	}
	...
	public static function gcTearDownAfterClass() {
		gc_delete_post( self::$block_id, true );
	}
	...
	public function testMetaCapsTestsAreCorrect() {
		...
		$file   = file_get_contents( ABSPATH . GCINC . '/capabilities.php' );
	...
	public function test_other_caps_for_all_roles( $role ) {
		...
		$this->assertFalse( $user->has_cap( 'start_a_fire' ), "User with the {$role} role should not have a custom capability (test via GC_User->has_cap() method)." );
		...
		$this->assertFalse( $user->has_cap( 'do_not_allow' ), "User with the {$role} role should not have the do_not_allow capability (test via GC_User->has_cap() method)." );
		...
		$this->assertTrue( $user->has_cap( 'exist' ), "User with the {$role} role should have the exist capability (test via GC_User->has_cap() method)." );
	...
	public function test_add_empty_role() {
		...
		$this->flush_roles();
		$this->assertTrue( $gc_roles->is_role( $role_name ) );
		...
		$this->flush_roles();
		$this->assertFalse( $gc_roles->is_role( $role_name ) );
	}
	...
	public function test_role_add_cap() {
		...
		$gc_roles->add_cap( $role_name, 'sweep_floor' );
		$this->flush_roles();
	...
	public function test_role_remove_cap() {
		...
		$gc_roles->remove_cap( $role_name, 'polish_doorknobs' );
		$this->flush_roles();
	...
	public function test_post_meta_caps() {
		...
		$author_2    = new GC_User( self::factory()->user->create( array( 'role' => 'author' ) ) );
	...
	public function nullify_current_user() {
		...
		$function_stack = gc_debug_backtrace_summary( null, 0, false );
	...
	public function test_require_edit_others_posts_if_post_type_doesnt_exist() {
		...
		foreach ( array( 'delete_post', 'edit_post', 'read_post', 'publish_post' ) as $cap ) {
			gc_set_current_user( $subscriber_id );
	 ...
	 * @ticket 35488
	 */
	public function test_gc_logout_should_clear_current_user() {
	...
	protected $_role_test_gc_roles_role;
	/**
	 * @ticket 23016
	 */
	public function test_gc_roles_init_action() {
		$this->_role_test_gc_roles_init = array(
			'role' => 'test_gc_roles_init',
			'info' => array(
				'name'         => 'Test GC Roles Init',
				'capabilities' => array( 'testing_magic' => true ),
			),
		);
		add_action( 'gc_roles_init', array( $this, '_hook_gc_roles_init' ), 10, 1 );

		$gc_roles = new GC_Roles();

		remove_action( 'gc_roles_init', array( $this, '_hook_gc_roles_init' ) );

		$expected = new GC_Role( $this->_role_test_gc_roles_init['role'], $this->_role_test_gc_roles_init['info']['capabilities'] );

		$role = $gc_roles->get_role( $this->_role_test_gc_roles_init['role'] );

		$this->assertEquals( $expected, $role );
		$this->assertContains( $this->_role_test_gc_roles_init['info']['name'], $gc_roles->role_names );
	}

	public function _hook_gc_roles_init( $gc_roles ) {
		$gc_roles->add_role( $this->_role_test_gc_roles_init['role'], $this->_role_test_gc_roles_init['info']['name'], $this->_role_test_gc_roles_init['info']['capabilities'] );
	}

	/**
	 * @ticket 23016
	 * @expectedDeprecated GC_Roles::reinit
	 */
	public function test_gc_roles_reinit_deprecated() {
		$gc_roles = new GC_Roles();
		$gc_roles->reinit();
	}
	...
	public function test_no_one_can_edit_user_meta_for_non_existent_term() {
		gc_set_current_user( self::$super_admin->ID );
	...
	public function test_user_can_edit_user_meta() {
		gc_set_current_user( self::$users['administrator']->ID );
		if ( is_multisite() ) {
	...
	public function test_user_cannot_edit_user_meta() {
		gc_set_current_user( self::$users['editor']->ID );
	...
	public function test_init_user_caps_for_different_site() {
		...
		$user = new GC_User( self::$users['subscriber']->ID, '', $site_id );
	...
	public function test_init_user_caps_for_different_site_by_user_switch() {
		...
		$user = new GC_User( self::$users['subscriber']->ID );
	...
	public function test_get_caps_data() {
		...
		// Test `GC_User::get_caps_data()` by manually setting capabilities metadata.
		update_user_meta( self::$users['subscriber']->ID, $gcdb->get_blog_prefix( get_current_blog_id() ) . 'capabilities', $custom_caps );
	...
	public function test_user_get_site_id() {
		...
		$suppress = $gcdb->suppress_errors();
		$user     = new GC_User( self::$users['subscriber']->ID, '', 333 );
		$gcdb->suppress_errors( $suppress );
	...
	public function test_init_roles_for_different_site() {
		...
		$gc_roles = gc_roles();
		$gc_roles->for_site( $site_id );

		$this->assertArrayHasKey( $role_name, $gc_roles->role_objects );
	}
	...
	public function test_get_roles_data() {
		...
		// Test `GC_Roles::get_roles_data()` by manually setting the roles option.
		update_option( $gcdb->get_blog_prefix( get_current_blog_id() ) . 'user_roles', $custom_roles );
