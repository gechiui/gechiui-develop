...
/**
 * A set of unit tests for functions in gc-includes/rewrite.php
 *
 * @group rewrite
 */
class Tests_Rewrite extends GC_UnitTestCase {
	private $home_url;
	...
	public function test_add_rule() {
		...
		$gc_rewrite->add_rule( $pattern, $redirect );

		$gc_rewrite->flush_rules();

		$rewrite_rules = $gc_rewrite->rewrite_rules();
	...
	public function test_add_rule_top() {
		...
		$gc_rewrite->add_rule( $pattern, $redirect, 'top' );

		$gc_rewrite->flush_rules();

		$extra_rules_top = $gc_rewrite->extra_rules_top;
	...
	public function test_parse_request_home_path() {
		...
		$this->go_to( $home_url );
		$this->assertSame( array(), $GLOBALS['gc']->query_vars );
	...
	/**
	 * Reveals bug introduced in GC 3.0
	 */
