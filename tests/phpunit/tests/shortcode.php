 ...
 * @group shortcode
 */
class Tests_Shortcode extends GC_UnitTestCase {
	...
	public function test_positional_atts_mixed() {
		$out = do_shortcode( '[test-shortcode-tag 123 https://www.gechiui.com/ 0 "foo" bar]' );
		$this->assertSame( '', $out );
		$this->assertSame(
			array(
				0 => '123',
				1 => 'https://www.gechiui.com/',
				2 => '0',
				3 => 'foo',
				4 => 'bar',
			),
	...
	public function test_positional_and_named_atts() {
		$out = do_shortcode( '[test-shortcode-tag 123 url=https://www.gechiui.com/ foo bar="baz"]' );
		$this->assertSame( '', $out );
		$this->assertSame(
			array(
				0     => '123',
	...
	public function test_nested_tags() {
		$out      = do_shortcode( '[baztag][dumptag abc="foo" def=123 https://www.gechiui.com/][/baztag]' );
		$expected = "content = abc = foo\ndef = 123\n0 = https://www.gechiui.com\n";
...
EOF;
	...
	public function test_shortcode_unautop() {
		...
		$test_string = "[footag]\n";
		$this->assertSame( $test_string, shortcode_unautop( gcautop( $test_string ) ) );
	}
	...
	public function data_escaping() {
		return array(
			...
			array(
				...
				'<div style="width: 12px" class="gc-caption alignnone"><div>hello</div><p class="gc-caption-text">test</p></div>',
			),
			array(
				'<div [gallery]>',
				'<div >',
			...
			array(
				'<a href="[url]plugins/">',
				'<a href="http://www.gechiui.com/plugins/">',
			),
			array(
				'<a href="bad[url]">',
				'<a href="//www.gechiui.com/">',
			),
			array(
				'<a onclick="bad[url]">',
				'<a onclick="bad[url]">',
	...
	public function test_php_and_js_shortcode_attribute_regexes_match() {
		...
		$file_src   = ABSPATH . 'js/_enqueues/gc/shortcode.js';
		$file_build = ABSPATH . 'gc-includes/js/shortcode.js';
	...
	public function test_unnamed_attribute() {
		$out      = do_shortcode( '[dumptag=https://www.gechiui.com/]' );
		$expected = "0 = =https://www.gechiui.com\n";
		$this->assertSame( $expected, $out );
	}
	...
	public function test_pre_do_shortcode_tag() {
		...
		$this->assertSame( gc_json_encode( $arr ), $result_atts );
