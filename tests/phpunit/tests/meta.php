 ...
 * @group meta
 */
class Tests_Meta extends GC_UnitTestCase {
	protected $updated_mids = array();

	public function set_up() {
		parent::set_up();
		$this->author         = new GC_User( self::factory()->user->create( array( 'role' => 'author' ) ) );
	...
	public function test_meta_cache_order_asc() {
		...
		foreach ( range( 1, 10 ) as $i ) {
			...
			if ( 0 === $i % 2 ) {
				gc_cache_delete( $post_id, 'post_meta' );
			}
		}
	}
	...
	public function test_string_point_zero_meta_id() {
		...
		// phpcs:ignore GeChiUI.PHP.StrictComparisons.LooseComparison -- intentional implicit casting check
