 ...
 * @group comment
 */
class Tests_Comment_Submission extends GC_UnitTestCase {
	...
	public static function gcTearDownAfterClass() {
		gc_delete_post( self::$post->ID, true );
	...
	public function test_submitting_comment_to_invalid_post_returns_error() {
		...
		$comment = gc_handle_comment_submission( $data );
	...
	public function test_submitting_comment_to_password_protected_post_succeeds() {
		...
		$_COOKIE[ 'gc-postpass_' . COOKIEHASH ] = $hasher->HashPassword( $password );
		...
		unset( $_COOKIE[ 'gc-postpass_' . COOKIEHASH ] );

		$this->assertNotWPError( $comment );
		$this->assertInstanceOf( 'GC_Comment', $comment );
	...
	/**
	 * gc_handle_comment_submission() expects un-slashed data.
	 *
	 * @group slashes
	 */
	...
	public function test_unprivileged_user_cannot_comment_unfiltered_html_even_with_valid_nonce() {
		...
		$nonce  = gc_create_nonce( $action );

		$this->assertNotEmpty( gc_verify_nonce( $nonce, $action ) );

		$data    = array(
			'comment_post_ID'             => self::$post->ID,
			'comment'                     => 'Comment <script>alert(document.cookie);</script>',
			'_gc_unfiltered_html_comment' => $nonce,
		);
		$comment = gc_handle_comment_submission( $data );
	...
	public function test_inserting_comment_with_empty_type_results_in_correct_type() {
		...
		$comment_id = gc_insert_comment( $data );
	...
	public function test_submitting_duplicate_comments() {
		...
		$first_comment  = gc_handle_comment_submission( $data );
		$second_comment = gc_handle_comment_submission( $data );
