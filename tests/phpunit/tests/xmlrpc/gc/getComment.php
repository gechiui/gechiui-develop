 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_gc_getComment extends GC_XMLRPC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$parent_comment_id   = gc_insert_comment( self::$parent_comment_data );
		...
		self::$child_comment_id   = gc_insert_comment( self::$child_comment_data );
	}

	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_getComment( array( 1, 'username', 'password', self::$parent_comment_id ) );
	...
	public function test_incapable_user() {
		...
		$result = $this->myxmlrpcserver->gc_getComment( array( 1, 'contributor', 'contributor', self::$parent_comment_id ) );
		$this->assertIXRError( $result );
		$this->assertSame( 403, $result->code );
	}
	...
	public function test_valid_comment() {
		...
		$result = $this->myxmlrpcserver->gc_getComment( array( 1, 'editor', 'editor', self::$parent_comment_id ) );
	...
	public function test_valid_child_comment() {
		...
		$result = $this->myxmlrpcserver->gc_getComment( array( 1, 'editor', 'editor', self::$child_comment_id ) );
		$this->assertNotIXRError( $result );
	...
	public function test_invalid_id() {
		...
		$result = $this->myxmlrpcserver->gc_getComment( array( 1, 'editor', 'editor', 123456789 ) );
		$this->assertIXRError( $result );
