 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_gc_getTerms extends GC_XMLRPC_UnitTestCase {

	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_getTerms( array( 1, 'username', 'password', 'category' ) );
	...
	public function test_empty_taxonomy() {
		...
		$result = $this->myxmlrpcserver->gc_getTerms( array( 1, 'editor', 'editor', '' ) );
		$this->assertIXRError( $result );
		$this->assertSame( 403, $result->code );
	...
	public function test_invalid_taxonomy() {
		...
		$result = $this->myxmlrpcserver->gc_getTerms( array( 1, 'editor', 'editor', 'not_existing' ) );
		$this->assertIXRError( $result );
		$this->assertSame( 403, $result->code );
		$this->assertSame( __( 'Invalid taxonomy.' ), $result->message );
	}
	...
	public function test_incapable_user() {
		...
		$result = $this->myxmlrpcserver->gc_getTerms( array( 1, 'subscriber', 'subscriber', 'category' ) );
		$this->assertIXRError( $result );
	...
	public function test_valid_terms() {
		...
		$cat = gc_insert_term( 'term_' . __FUNCTION__, 'category' );

		$results = $this->myxmlrpcserver->gc_getTerms( array( 1, 'editor', 'editor', 'category' ) );
	...
	public function test_custom_taxonomy() {
		...
		$tax_name  = 'gc_getTerms_custom_taxonomy';
		$num_terms = 12;
		register_taxonomy( $tax_name, 'post' );
		for ( $i = 0; $i < $num_terms; $i++ ) {
			gc_insert_term( "term_{$i}", $tax_name );
		}

		// Test fetching all terms.
		$results = $this->myxmlrpcserver->gc_getTerms( array( 1, 'editor', 'editor', $tax_name ) );
		$this->assertNotIXRError( $results );
		...
		$results2 = $this->myxmlrpcserver->gc_getTerms( array( 1, 'editor', 'editor', $tax_name, $filter ) );
		$this->assertNotIXRError( $results );
		$this->assertCount( 5, $results2 );
		$this->assertSame( $results[1]['term_id'], $results2[1]['term_id'] ); // Check one of the terms.

		$filter['offset'] = 10;
		$results3         = $this->myxmlrpcserver->gc_getTerms( array( 1, 'editor', 'editor', $tax_name, $filter ) );
		...
		$results4 = $this->myxmlrpcserver->gc_getTerms( array( 1, 'editor', 'editor', $tax_name, $filter ) );
		$this->assertNotIXRError( $results4 );
		$this->assertCount( 0, $results4 );

		unset( $GLOBALS['gc_taxonomies'][ $tax_name ] );
	}

	public function test_term_ordering() {
		$this->make_user_by_role( 'editor' );

		$cat1 = gc_create_category( 'gc.getTerms_' . __FUNCTION__ . '_1' );
		$cat2 = gc_create_category( 'gc.getTerms_' . __FUNCTION__ . '_2' );
		...
		$results = $this->myxmlrpcserver->gc_getTerms( array( 1, 'editor', 'editor', 'category', $filter ) );
		$this->assertNotIXRError( $results );
	...
	public function test_terms_search() {
		...
		$name    = __FUNCTION__;
		$name_id = gc_create_category( $name );
		...
		$results2 = $this->myxmlrpcserver->gc_getTerms( array( 1, 'editor', 'editor', 'category', $filter ) );
