 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_gc_getTerm extends GC_XMLRPC_UnitTestCase {
	...
	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_getTerm( array( 1, 'username', 'password', 'category', 1 ) );
	...
	public function test_empty_taxonomy() {
		...
		$result = $this->myxmlrpcserver->gc_getTerm( array( 1, 'editor', 'editor', '', 0 ) );
		$this->assertIXRError( $result );
		$this->assertSame( 403, $result->code );
	...
	public function test_invalid_taxonomy() {
		...
		$result = $this->myxmlrpcserver->gc_getTerm( array( 1, 'editor', 'editor', 'not_existing', 0 ) );
		$this->assertIXRError( $result );
		$this->assertSame( 403, $result->code );
		$this->assertSame( __( 'Invalid taxonomy.' ), $result->message );
	}
	...
	public function test_incapable_user() {
		...
		$result = $this->myxmlrpcserver->gc_getTerm( array( 1, 'subscriber', 'subscriber', 'category', self::$term_id ) );
		$this->assertIXRError( $result );
	...
	public function test_empty_term() {
		...
		$result = $this->myxmlrpcserver->gc_getTerm( array( 1, 'editor', 'editor', 'category', '' ) );
		$this->assertIXRError( $result );
	...
	public function test_invalid_term() {
		...
		$result = $this->myxmlrpcserver->gc_getTerm( array( 1, 'editor', 'editor', 'category', 9999 ) );
		$this->assertIXRError( $result );
	...
	public function test_valid_term() {
		...
		$result = $this->myxmlrpcserver->gc_getTerm( array( 1, 'editor', 'editor', 'category', self::$term_id ) );
	...
	public function test_get_term_meta() {
		...
		// Add term meta to test gc.getTerm.
		add_term_meta( self::$term_id, 'foo', 'bar' );

		$term = get_term( self::$term_id, 'category', ARRAY_A );

		$result = $this->myxmlrpcserver->gc_getTerm(
			array(
				1,
				'editor',
				'editor',
