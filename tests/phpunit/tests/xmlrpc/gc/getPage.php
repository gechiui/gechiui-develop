 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_gc_getPage extends GC_XMLRPC_UnitTestCase {
	...
	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_getPage( array( 1, self::$post_id, 'username', 'password' ) );
	...
	public function test_invalid_pageid() {
		...
		$result = $this->myxmlrpcserver->gc_getPage( array( 1, 9999, 'editor', 'editor' ) );
		$this->assertIXRError( $result );
	...
	public function test_valid_page() {
		...
		$result = $this->myxmlrpcserver->gc_getPage( array( 1, self::$post_id, 'editor', 'editor' ) );
		...
		$this->assertIsString( $result['gc_slug'] );
		$this->assertIsString( $result['gc_password'] );
		$this->assertIsString( $result['gc_author'] );
		$this->assertIsInt( $result['gc_page_parent_id'] );
		$this->assertIsString( $result['gc_page_parent_title'] );
		$this->assertIsInt( $result['gc_page_order'] );
		$this->assertIsString( $result['gc_author_id'] );
		$this->assertIsString( $result['gc_author_display_name'] );
		$this->assertIsArray( $result['custom_fields'] );
		$this->assertIsString( $result['gc_page_template'] );
		...
		$this->assertStringMatchesFormat( '%d', $result['gc_author_id'] );
	}
