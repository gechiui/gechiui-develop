 ...
 * @group user
 */
class Tests_XMLRPC_gc_editProfile extends GC_XMLRPC_UnitTestCase {

	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_editProfile( array( 1, 'username', 'password', array() ) );
	...
	public function test_subscriber_profile() {
		...
		$result   = $this->myxmlrpcserver->gc_editProfile( array( 1, 'subscriber', 'subscriber', $new_data ) );
	...
	public function test_ignore_password_change() {
		...
		$result = $this->myxmlrpcserver->gc_editProfile( array( 1, 'author', 'author', $new_data ) );
		$this->assertNotIXRError( $result );
		$this->assertTrue( $result );

		$auth_old = gc_authenticate( 'author', 'author' );
		$auth_new = gc_authenticate( 'author', $new_pass );
		$this->assertInstanceOf( 'GC_User', $auth_old );
	...
	public function test_ignore_email_change() {
		...
		$result = $this->myxmlrpcserver->gc_editProfile( array( 1, 'editor', 'editor', $new_data ) );
		$this->assertNotIXRError( $result );
		$this->assertTrue( $result );
