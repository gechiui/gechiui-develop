 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_gc_getPosts extends GC_XMLRPC_UnitTestCase {

	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_getPosts( array( 1, 'username', 'password' ) );
	...
	public function test_incapable_user() {
		...
		$result = $this->myxmlrpcserver->gc_getPosts( array( 1, 'subscriber', 'subscriber' ) );
		$this->assertIXRError( $result );
		$this->assertSame( 401, $result->code );

		$filter = array( 'post_type' => 'page' );
		$result = $this->myxmlrpcserver->gc_getPosts( array( 1, 'subscriber', 'subscriber', $filter ) );
		$this->assertIXRError( $result );
		$this->assertSame( 401, $result->code );
	}
	...
	public function test_capable_user() {
		...
		$result = $this->myxmlrpcserver->gc_getPosts( array( 1, 'editor', 'editor' ) );
	...
	public function test_invalid_post_type() {
		...
		$result = $this->myxmlrpcserver->gc_getPosts( array( 1, 'editor', 'editor', $filter ) );
		$this->assertIXRError( $result );
	}
	...
	public function test_filters() {
		...
		$results = $this->myxmlrpcserver->gc_getPosts( array( 1, 'editor', 'editor', $filter ) );
		...
		$filter['offset'] = 0;
		do {
			$presults          = $this->myxmlrpcserver->gc_getPosts( array( 1, 'editor', 'editor', $filter ) );
			$posts_found       = array_merge( $posts_found, gc_list_pluck( $presults, 'post_id' ) );
		...
		$results2 = $this->myxmlrpcserver->gc_getPosts( array( 1, 'editor', 'editor', $filter2 ) );
		...
		$results3 = $this->myxmlrpcserver->gc_getPosts( array( 1, 'editor', 'editor', $filter3 ) );
	...
	public function test_fields() {
		...
		$results = $this->myxmlrpcserver->gc_getPosts( array( 1, 'editor', 'editor' ) );
		$this->assertNotIXRError( $results );
		...
		$results2 = $this->myxmlrpcserver->gc_getPosts( array( 1, 'editor', 'editor', $filter, $fields ) );
		$this->assertNotIXRError( $results2 );
