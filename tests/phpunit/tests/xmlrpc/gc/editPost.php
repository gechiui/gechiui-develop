 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_gc_editPost extends GC_XMLRPC_UnitTestCase {

	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_editPost( array( 1, 'username', 'password', 0, array() ) );
	...
	public function test_edit_own_post() {
		...
		$result    = $this->myxmlrpcserver->gc_editPost( array( 1, 'contributor', 'contributor', $post_id, $post2 ) );
	...
	public function test_capable_edit_others_post() {
		...
		$new_title = 'Post test (updated)';
		$post2     = array( 'post_title' => $new_title );
		$result    = $this->myxmlrpcserver->gc_editPost( array( 1, 'editor', 'editor', $post_id, $post2 ) );
		$this->assertNotIXRError( $result );
		$this->assertTrue( $result );
	...
	public function test_post_thumbnail() {
		...
		$result = $this->myxmlrpcserver->gc_editPost( array( 1, 'author', 'author', $post_id, $post2 ) );
		$this->assertNotIXRError( $result );
		$this->assertEquals( $attachment_id, get_post_meta( $post_id, '_thumbnail_id', true ) );

		// Fetch the post to verify that it appears.
		$result = $this->myxmlrpcserver->gc_getPost( array( 1, 'author', 'author', $post_id ) );
		$this->assertNotIXRError( $result );
		...
		$result = $this->myxmlrpcserver->gc_editPost( array( 1, 'author', 'author', $post_id, $post3 ) );
		$this->assertNotIXRError( $result );
		$this->assertEquals( $attachment_id, get_post_meta( $post_id, '_thumbnail_id', true ) );
		...
		$result = $this->myxmlrpcserver->gc_editPost( array( 1, 'author', 'author', $post_id, $post4 ) );
		$this->assertNotIXRError( $result );
		...
		$result = $this->myxmlrpcserver->gc_editPost( array( 1, 'author', 'author', $post_id, $post5 ) );
		$this->assertNotIXRError( $result );
		$this->assertSame( '', get_post_meta( $post_id, '_thumbnail_id', true ) );

		// Use invalid ID.
		$post6  = array( 'post_thumbnail' => 398420983409 );
		$result = $this->myxmlrpcserver->gc_editPost( array( 1, 'author', 'author', $post_id, $post6 ) );
		$this->assertIXRError( $result );
	...
	public function test_if_not_modified_since() {
		...
		$result = $this->myxmlrpcserver->gc_editPost( array( 1, 'editor', 'editor', $post_id, $struct ) );
		$this->assertNotIXRError( $result );
	...
	public function test_loss_of_categories_on_edit() {
		...
		$term_ids = gc_list_pluck( get_the_category( $post_id ), 'term_id' );
		$this->assertContains( $term_id, $term_ids );

		$result = $this->myxmlrpcserver->gc_editPost(
			array(
				1,
				'editor',
				'editor',
	...
	public function test_clear_categories_on_edit() {
		...
		$result           = $this->myxmlrpcserver->gc_editPost( array( 1, 'editor', 'editor', $post_id, $new_post_content ) );
		$this->assertNotIXRError( $result );
		$this->assertSame( 'Updated', get_post( $post_id )->post_title );
	...
	public function test_draft_not_prematurely_published() {
		...
		/**
		 * We have to use gc_newPost method, rather than the factory
		...
		$post_id = $this->myxmlrpcserver->gc_newPost( array( 1, 'editor', 'editor', $post ) );
		...
		$this->myxmlrpcserver->gc_editPost( array( 1, 'editor', 'editor', $post_id, $new_post_content ) );
