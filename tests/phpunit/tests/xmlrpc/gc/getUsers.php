 ...
 * @group user
 */
class Tests_XMLRPC_gc_getUsers extends GC_XMLRPC_UnitTestCase {

	public function test_invalid_username_password() {
		$results = $this->myxmlrpcserver->gc_getUsers( array( 1, 'username', 'password' ) );
	...
	public function test_incapable_user() {
		...
		$results = $this->myxmlrpcserver->gc_getUsers( array( 1, 'subscriber', 'subscriber' ) );
		$this->assertIXRError( $results );
	...
	public function test_capable_user() {
		...
		$result = $this->myxmlrpcserver->gc_getUsers( array( 1, 'administrator', 'administrator' ) );
	...
	public function test_invalid_role() {
		...
		$results = $this->myxmlrpcserver->gc_getUsers( array( 1, 'administrator', 'administrator', $filter ) );
		$this->assertIXRError( $results );
		$this->assertSame( 403, $results->code );
	}
	...
	public function test_role_filter() {
		...
		$results2 = $this->myxmlrpcserver->gc_getUsers( array( 1, 'administrator', 'administrator', $filter2 ) );
		$this->assertNotIXRError( $results2 );
		$this->assertCount( 3, array_intersect( array( $author_id, $editor_id, $administrator_id ), gc_list_pluck( $results2, 'user_id' ) ) );
	}
	...
	public function test_paging_filters() {
		...
		do {
			$presults = $this->myxmlrpcserver->gc_getUsers( array( 1, 'administrator', 'administrator', $filter ) );
