 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_gc_newPost extends GC_XMLRPC_UnitTestCase {

	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_newPost( array( 1, 'username', 'password', array() ) );
	...
	public function test_incapable_user() {
		...
		$result = $this->myxmlrpcserver->gc_newPost( array( 1, 'subscriber', 'subscriber', array() ) );
		$this->assertIXRError( $result );
	...
	public function test_no_content() {
		...
		$result = $this->myxmlrpcserver->gc_newPost( array( 1, 'author', 'author', array() ) );
		$this->assertIXRError( $result );
	...
	public function test_basic_content() {
		...
		$result = $this->myxmlrpcserver->gc_newPost( array( 1, 'author', 'author', $post ) );
	...
	public function test_incapable_publish() {
		...
		$result = $this->myxmlrpcserver->gc_newPost( array( 1, 'contributor', 'contributor', $post ) );
		$this->assertIXRError( $result );
		$this->assertSame( 401, $result->code );
	}
	...
	public function test_capable_private() {
		...
		$result = $this->myxmlrpcserver->gc_newPost( array( 1, 'editor', 'editor', $post ) );
		$this->assertNotIXRError( $result );
	}
	...
	public function test_invalid_taxonomy() {
		...
		$result2 = $this->myxmlrpcserver->gc_newPost( array( 1, 'editor', 'editor', $post2 ) );
	...
	public function test_terms() {
		...
		$tag1 = gc_create_tag( 'tag1' );
		$this->assertIsArray( $tag1 );
		$tag2 = gc_create_tag( 'tag2' );
		$this->assertIsArray( $tag2 );
		$tag3 = gc_create_tag( 'tag3' );
		...
		$post_tags = gc_get_object_terms( $result, 'post_tag', array( 'fields' => 'ids' ) );
	...
	public function test_terms_names() {
		...
		$ambiguous_name = 'foo';
		$parent_cat     = gc_create_category( $ambiguous_name );
		$child_cat      = gc_create_category( $ambiguous_name, $parent_cat );

		$cat1_name = 'cat1';
		$cat1      = gc_create_category( $cat1_name, $parent_cat );
		$cat2_name = 'cat2';
		...
		$post_cats = gc_get_object_terms( $result, 'category', array( 'fields' => 'ids' ) );
