 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_gc_getPostType extends GC_XMLRPC_UnitTestCase {
	public $cpt_name;
	public $cpt_args;
	...
	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_getPostType( array( 1, 'username', 'password', 'post' ) );
	...
	public function test_invalid_post_type_name() {
		...
		$result = $this->myxmlrpcserver->gc_getPostType( array( 1, 'editor', 'editor', 'foobar' ) );
		$this->assertIXRError( $result );
		$this->assertSame( 403, $result->code );
	}
	...
	public function test_valid_post_type_name() {
		...
		$result = $this->myxmlrpcserver->gc_getPostType( array( 1, 'editor', 'editor', 'post' ) );
	...
	public function test_incapable_user() {
		...
		$result = $this->myxmlrpcserver->gc_getPostType( array( 1, 'subscriber', 'subscriber', 'post' ) );
		$this->assertIXRError( $result );
	...
	public function test_valid_type() {
		...
		$result = $this->myxmlrpcserver->gc_getPostType( array( 1, 'editor', 'editor', $this->cpt_name, array( 'labels', 'cap', 'menu', 'taxonomies' ) ) );
		$this->assertNotIXRError( $result );
