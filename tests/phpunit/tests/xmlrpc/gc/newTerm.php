 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_gc_newTerm extends GC_XMLRPC_UnitTestCase {
	...
	public function test_invalid_username_password() {
		$result = $this->myxmlrpcserver->gc_newTerm( array( 1, 'username', 'password', array() ) );
	...
	public function test_empty_taxonomy() {
		...
		$result = $this->myxmlrpcserver->gc_newTerm( array( 1, 'editor', 'editor', array( 'taxonomy' => '' ) ) );
		$this->assertIXRError( $result );
		$this->assertSame( 403, $result->code );
	...
	public function test_invalid_taxonomy() {
		...
		$result = $this->myxmlrpcserver->gc_newTerm( array( 1, 'editor', 'editor', array( 'taxonomy' => 'not_existing' ) ) );
		$this->assertIXRError( $result );
		$this->assertSame( 403, $result->code );
		$this->assertSame( __( 'Invalid taxonomy.' ), $result->message );
	}
	...
	public function test_incapable_user() {
		...
		$result = $this->myxmlrpcserver->gc_newTerm( array( 1, 'subscriber', 'subscriber', array( 'taxonomy' => 'category' ) ) );
		$this->assertIXRError( $result );
	...
	public function test_empty_term() {
		...
		$result = $this->myxmlrpcserver->gc_newTerm(
			array(
				1,
				'editor',
				'editor',
	...
	public function test_add_term_with_all() {
		...
		$result   = $this->myxmlrpcserver->gc_newTerm( array( 1, 'editor', 'editor', $taxonomy ) );
		$this->assertNotIXRError( $result );
		$this->assertStringMatchesFormat( '%d', $result );
	}
