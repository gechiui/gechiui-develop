 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_mw_getRecentPosts extends GC_XMLRPC_UnitTestCase {
	...
	public function test_valid_post() {
		...
		foreach ( $results as $result ) {
			...
			$this->assertIsString( $result['gc_more_text'] );
			...
			$this->assertIsString( $result['gc_post_format'] );
			...
			$this->assertSame( '', $result['gc_post_thumbnail'] );
		}
	...
	public function test_post_thumbnail() {
		...
		foreach ( $results as $result ) {
			$this->assertIsString( $result['gc_post_thumbnail'] );
			$this->assertStringMatchesFormat( '%d', $result['gc_post_thumbnail'] );

			if ( ! empty( $result['gc_post_thumbnail'] ) || $result['postid'] === self::$post_id ) {
				$attachment_id = get_post_meta( $result['postid'], '_thumbnail_id', true );

				$this->assertSame( $attachment_id, $result['gc_post_thumbnail'] );
			}
		}
