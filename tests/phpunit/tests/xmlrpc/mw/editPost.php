 ...
 * @group xmlrpc
 */
class Tests_XMLRPC_mw_editPost extends GC_XMLRPC_UnitTestCase {
	...
	public function test_capable_reassign_author() {
		...
		$post2  = array( 'gc_author_id' => $author_id );
		$result = $this->myxmlrpcserver->mw_editPost( array( $post_id, 'editor', 'editor', $post2 ) );
		$this->assertNotIXRError( $result );
		$this->assertTrue( $result );
	...
	public function test_capable_reassign_author_to_self() {
		...
		$post2  = array( 'gc_author_id' => $editor_id );
		$result = $this->myxmlrpcserver->mw_editPost( array( $post_id, 'editor', 'editor', $post2 ) );
		$this->assertNotIXRError( $result );
		$this->assertTrue( $result );
	...
	public function test_post_thumbnail() {
		...
		$post2  = array( 'gc_post_thumbnail' => $attachment_id );
		...
		$post4  = array( 'gc_post_thumbnail' => $attachment2_id );
