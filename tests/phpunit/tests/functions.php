 ...
 * @group functions.php
 */
class Tests_Functions extends GC_UnitTestCase {
	public function test_gc_parse_args_object() {
		$x        = new MockClass;
		$x->_baba = 5;
		$x->yZ    = 'baba'; // phpcs:ignore GeChiUI.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		$x->a     = array( 5, 111, 'x' );
		$this->assertSame(
			array(
				'_baba' => 5,
		...
		$y = new MockClass;
		$this->assertSame( array(), gc_parse_args( $y ) );
	}

	public function test_gc_parse_args_array() {
		// Arrays.
		$a = array();
		$this->assertSame( array(), gc_parse_args( $a ) );
		$b = array(
			'_baba' => 5,
			'yZ'    => 'baba',
			'a'     => array( 5, 111, 'x' ),
	...
	public function test_gc_parse_args_defaults() {
		$x        = new MockClass;
		$x->_baba = 5;
		$x->yZ    = 'baba'; // phpcs:ignore GeChiUI.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
		$x->a     = array( 5, 111, 'x' );
	...
	public function test_gc_parse_args_other() {
		$b = true;
		gc_parse_str( $b, $s );
		$this->assertSame( $s, gc_parse_args( $b ) );
		$q = 'x=5&_baba=dudu&';
		gc_parse_str( $q, $ss );
		$this->assertSame( $ss, gc_parse_args( $q ) );
	}

	/**
	 * @ticket 30753
	 */
	public function test_gc_parse_args_boolean_strings() {
		$args = gc_parse_args( 'foo=false&bar=true' );
	 ...
	 * @ticket 35996
	 *
	 * @dataProvider data_gc_normalize_path
	 */
	public function test_gc_normalize_path( $path, $expected ) {
		$this->assertSame( $expected, gc_normalize_path( $path ) );
	}

	public function data_gc_normalize_path() {
		return array(
			// Windows paths.
	...
	public function test_gc_unique_filename() {

		$testdir = DIR_TESTDATA . '/images/';

		// Sanity check.
		$this->assertSame( 'abcdefg.png', gc_unique_filename( $testdir, 'abcdefg.png' ), 'Test non-existing file, file name should be unchanged.' );
		...
		$this->assertSame( 'test-image-1.png', gc_unique_filename( $testdir, 'test-image.png' ), 'File name not unique, number not appended.' );

		// Check file with uppercase extension.
		$this->assertSame( 'test-image-1.png', gc_unique_filename( $testdir, 'test-image.PNG' ), 'File name with uppercase extension not unique, number not appended.' );

		// Check file name with already added number.
		$this->assertSame( 'test-image-2-1.gif', gc_unique_filename( $testdir, 'test-image-2.gif' ), 'File name not unique, number not appended correctly.' );

		// Check special chars.
		$this->assertSame( 'testtest-image.png', gc_unique_filename( $testdir, 'testtést-imagé.png' ), 'Filename with special chars failed' );

		// Check special chars with potential conflicting name.
		$this->assertSame( 'test-image-1.png', gc_unique_filename( $testdir, 'tést-imagé.png' ), 'Filename with special chars failed' );

		// Check with single quotes in name (somehow).
		$this->assertSame( 'abcdefgh.png', gc_unique_filename( $testdir, "abcdefg'h.png" ), 'File with quote failed' );

		// Check with double quotes in name (somehow).
		$this->assertSame( 'abcdefgh.png', gc_unique_filename( $testdir, 'abcdefg"h.png' ), 'File with quote failed' );

		// Test crazy name (useful for regression tests).
		$this->assertSame( '12af34567890@..^_qwerty-fghjkl-zx.png', gc_unique_filename( $testdir, '12%af34567890#~!@#$..%^&*()|_+qwerty  fgh`jkl zx<>?:"{}[]="\'/?.png' ), 'Failed crazy file name' );

		// Test slashes in names.
		$this->assertSame( 'abcdefg.png', gc_unique_filename( $testdir, 'abcde\fg.png' ), 'Slash not removed' );
		$this->assertSame( 'abcdefg.png', gc_unique_filename( $testdir, 'abcde\\fg.png' ), 'Double slashed not removed' );
		$this->assertSame( 'abcdefg.png', gc_unique_filename( $testdir, 'abcde\\\fg.png' ), 'Tripple slashed not removed' );
	}
	...
	public function test_unique_filename_with_dimension_like_filename() {
		...
		$this->assertSame( 'one-blue-pixel-100x100-1.png', gc_unique_filename( $testdir, 'one-blue-pixel-100x100.png' ) );
		// Test collision with existing sub-size filename.
		// Existing files: one-blue-pixel-100x100.png, one-blue-pixel-1-100x100.png.
		$this->assertSame( 'one-blue-pixel-2.png', gc_unique_filename( $testdir, 'one-blue-pixel.png' ) );
	...
	// Callback to patch "basedir" when used in `gc_unique_filename()`.
	 ...
	 * @ticket 53668
	 */
	public function test_gc_unique_filename_with_additional_image_extension() {
		$testdir = DIR_TESTDATA . '/images/';
		...
		$this->assertSame(
			'abcdef.png',
			gc_unique_filename( $testdir, 'abcdef.png' ),
		...
		$this->assertSame(
			'canola-1.jpg',
			gc_unique_filename( $testdir, 'canola.jpg' ),
		...
		$this->assertSame(
			'canola-1.png',
			gc_unique_filename( $testdir, 'canola.png' ),
		...
		$this->assertSame(
			'test-image-5.png',
			gc_unique_filename( $testdir, 'test-image.png' ),
		...
		$this->assertSame(
			'codeispoetry-1.jpg',
			gc_unique_filename( $testdir, 'codeispoetry.jpg' ),
	 ...
	 * @ticket 21594
	 */
	public function test_gc_get_mime_types() {
		$mimes = gc_get_mime_types();
		...
		// 'upload_mimes' should not affect gc_get_mime_types().
		add_filter( 'upload_mimes', '__return_empty_array' );
		$mimes = gc_get_mime_types();
		$this->assertIsArray( $mimes );
		$this->assertNotEmpty( $mimes );
	...
	/**
	 * @ticket 43977
	 * @dataProvider data_gc_parse_list
	 */
	public function test_gc_parse_list( $expected, $actual ) {
		$this->assertSame( $expected, array_values( gc_parse_list( $actual ) ) );
	}

	public function data_gc_parse_list() {
		return array(
	...
	/**
	 * @dataProvider data_gc_parse_id_list
	 */
	public function test_gc_parse_id_list( $expected, $actual ) {
		$this->assertSame( $expected, array_values( gc_parse_id_list( $actual ) ) );
	}

	public function data_gc_parse_id_list() {
		return array(
			array( array( 1, 2, 3, 4 ), '1,2,3,4' ),
			array( array( 1, 2, 3, 4 ), '1, 2,,3,4' ),
	...
	/**
	 * @dataProvider data_gc_parse_slug_list
	 */
	public function test_gc_parse_slug_list( $expected, $actual ) {
		$this->assertSame( $expected, array_values( gc_parse_slug_list( $actual ) ) );
	}

	public function data_gc_parse_slug_list() {
		return array(
			array( array( 'apple', 'banana', 'carrot', 'dog' ), 'apple,banana,carrot,dog' ),
			array( array( 'apple', 'banana', 'carrot', 'dog' ), 'apple, banana,,carrot,dog' ),
			array( array( 'apple', 'banana', 'carrot', 'dog' ), 'apple banana carrot dog' ),
	 ...
	 * @ticket 9064
	 */
	public function test_gc_extract_urls() {
		$original_urls = array(
			'http://woo.com/1,2,3,4,5,6/-1-2-3-4-/woo.html',
			'http://this.com',
			'http://127.0.0.1',
			'http://www111.urwyeoweytwutreyytqytwetowteuiiu.com/?346236346326&2134362574863.437',
			'http://gechiui-core/1,2,3,4,5,6/-1-2-3-4-/woo.html',
			'http://gechiui-core.com:8080/',
			'http://www.website.com:5000',
			'http://gechiui-core/?346236346326&2134362574863.437',
		...
		// gc_extract_urls() calls html_entity_decode().
		...
		$blob = 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor
			...
			commodo consequat. http://gechiui-core/1,2,3,4,5,6/-1-2-3-4-/woo.html Duis aute irure dolor in reprehenderit in voluptate
			velit esse http://gechiui-core.com:8080/ cillum dolore eu fugiat nulla <A href="http://www.website.com:5000">http://www.website.com:5000</B> pariatur. Excepteur sint occaecat cupidatat non proident,
			sunt in culpa qui officia deserunt mollit http://gechiui-core/?346236346326&amp;2134362574863.437 anim id est laborum.';
		...
		$blob = 'Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed do eiusmod tempor
			...
			commodo consequat. <a href="http://gechiui-core/1,2,3,4,5,6/-1-2-3-4-/woo.html">343462^</a> Duis aute irure dolor in reprehenderit in voluptate
			velit esse <a href="http://gechiui-core.com:8080/">-3-4--321-64-4@#!$^$!@^@^</a> cillum dolore eu <A href="http://www.website.com:5000">http://www.website.com:5000</B> fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident,
			sunt in culpa qui officia deserunt mollit <a href="http://gechiui-core/?346236346326&amp;2134362574863.437">)(*&^%$</a> anim id est laborum.';
	 ...
	 * @ticket 28786
	 */
	public function test_gc_json_encode() {
		$this->assertSame( gc_json_encode( 'a' ), '"a"' );
	}

	/**
	 * @ticket 28786
	 */
	public function test_gc_json_encode_utf8() {
		$this->assertSame( gc_json_encode( '这' ), '"\u8fd9"' );
	}
	 ...
	 */
	public function test_gc_json_encode_non_utf8() {
		...
		$this->assertSame( '"a\u3042b"', gc_json_encode( $eucjp ) );
	 ...
	 * @requires function mb_detect_order
	 */
	public function test_gc_json_encode_non_utf8_in_array() {
		$charsets     = mb_detect_order();
		$old_charsets = $charsets;
		if ( ! in_array( 'EUC-JP', $charsets, true ) ) {
			$charsets[] = 'EUC-JP';
		...
		$this->assertSame( '["c","a\u3042b"]', gc_json_encode( array( 'c', $eucjp ) ) );
	 ...
	 * @ticket 28786
	 */
	public function test_gc_json_encode_array() {
		$this->assertSame( gc_json_encode( array( 'a' ) ), '["a"]' );
	}

	/**
	 * @ticket 28786
	 */
	public function test_gc_json_encode_object() {
		$object    = new stdClass;
		$object->a = 'b';
		$this->assertSame( gc_json_encode( $object ), '{"a":"b"}' );
	}

	/**
	 * @ticket 28786
	 */
	public function test_gc_json_encode_depth() {
		$data = array( array( array( 1, 2, 3 ) ) );
		$json = gc_json_encode( $data, 0, 1 );
	 ...
	 * @ticket 53238
	 */
	public function test_gc_json_file_decode() {
		$result = gc_json_file_decode(
	 ...
	 * @ticket 53238
	 */
	public function test_gc_json_file_decode_associative_array() {
		$result = gc_json_file_decode(
			DIR_TESTDATA . '/blocks/notice/block.json',
	 ...
	 * @ticket 35987
	 */
	public function test_gc_get_ext_types() {
		$extensions = gc_get_ext_types();
	 ...
	 * @ticket 35987
	 */
	public function test_gc_ext2type() {
		$extensions = gc_get_ext_types();

		foreach ( $extensions as $type => $extension_list ) {
			foreach ( $extension_list as $extension ) {
				$this->assertSame( $type, gc_ext2type( $extension ) );
				$this->assertSame( $type, gc_ext2type( strtoupper( $extension ) ) );
			}
		}

		$this->assertNull( gc_ext2type( 'unknown_format' ) );
	}

	/**
	 * Tests raising the memory limit.
	 *
	 * Unfortunately as the default for 'GC_MAX_MEMORY_LIMIT' in the
	 * test suite is -1, we can not test the memory limit negotiations.
	 *
	 * @ticket 32075
	 */
	public function test_gc_raise_memory_limit() {
		if ( -1 !== GC_MAX_MEMORY_LIMIT ) {
			$this->markTestSkipped( 'GC_MAX_MEMORY_LIMIT should be set to -1.' );
		}

		$ini_limit_before = ini_get( 'memory_limit' );
		$raised_limit     = gc_raise_memory_limit();
		$ini_limit_after  = ini_get( 'memory_limit' );

		$this->assertSame( $ini_limit_before, $ini_limit_after );
		$this->assertFalse( $raised_limit );
		$this->assertEquals( GC_MAX_MEMORY_LIMIT, $ini_limit_after );
	}

	/**
	 * Tests gc_generate_uuid4().
	 *
	 * @covers ::gc_generate_uuid4
	 * @ticket 38164
	 */
	public function test_gc_generate_uuid4() {
		$uuids = array();
		for ( $i = 0; $i < 20; $i += 1 ) {
			$uuid = gc_generate_uuid4();
			$this->assertTrue( gc_is_uuid( $uuid, 4 ) );
			$uuids[] = $uuid;
		}
	 ...
	 * @ticket 39778
	 */
	public function test_gc_is_valid_uuid() {
		$uuids_v4 = array(
		...
		foreach ( $uuids as $uuid ) {
			$this->assertTrue( gc_is_uuid( $uuid ) );
			$this->assertFalse( gc_is_uuid( $uuid, 4 ) );
		}
		...
		foreach ( $invalid_uuids as $invalid_uuid ) {
			$this->assertFalse( gc_is_uuid( $invalid_uuid, 4 ) );
			$this->assertFalse( gc_is_uuid( $invalid_uuid ) );
		}
	}
	 ...
	 * @ticket 44883
	 */
	public function test_gc_unique_id() {
	...
	/**
	 * @ticket 40017
	 * @dataProvider gc_get_image_mime
	 */
	public function test_gc_get_image_mime( $file, $expected ) {
		if ( ! is_callable( 'exif_imagetype' ) && ! function_exists( 'getimagesize' ) ) {
			$this->markTestSkipped( 'The exif PHP extension is not loaded.' );
		}

		$this->assertSame( $expected, gc_get_image_mime( $file ) );
	}

	/**
	 * @ticket 35725
	 * @dataProvider data_gc_getimagesize
	 */
	public function test_gc_getimagesize( $file, $expected ) {
		if ( ! is_callable( 'exif_imagetype' ) && ! function_exists( 'getimagesize' ) ) {
			$this->markTestSkipped( 'The exif PHP extension is not loaded.' );
		}

		$result = gc_getimagesize( $file );
	...
	/**
	 * @ticket 39550
	 * @dataProvider gc_check_filetype_and_ext_data
	 * @requires extension fileinfo
	 */
	public function test_gc_check_filetype_and_ext( $file, $filename, $expected ) {
		$this->assertSame( $expected, gc_check_filetype_and_ext( $file, $filename ) );
	}
	 ...
	 * @requires extension fileinfo
	 */
	public function test_gc_check_filetype_and_ext_with_filtered_svg() {
	 ...
	 * @requires extension fileinfo
	 */
	public function test_gc_check_filetype_and_ext_with_filtered_woff() {
	...
	/**
	 * Data provider for test_gc_get_image_mime().
	 */
	public function gc_get_image_mime() {
		$data = array(
			// Standard JPEG.
			array(
	...
	/**
	 * Data profider for test_gc_getimagesize().
	 */
	public function data_gc_getimagesize() {
		$data = array(
			// Standard JPEG.
			array(
				DIR_TESTDATA . '/images/test-image.jpg',
	...
	public function gc_check_filetype_and_ext_data() {
		$data = array(
			// Standard image.
			array(
	 ...
	 * }
	 */
	public function data_test_gc_is_stream() {
		return array(
	...
	/**
	 * @ticket 49404
	 * @dataProvider data_test_gc_is_json_media_type
	 */
	public function test_gc_is_json_media_type( $input, $expected ) {
		$this->assertSame( $expected, gc_is_json_media_type( $input ) );
	}


	public function data_test_gc_is_json_media_type() {
		return array(
	 ...
	 * @ticket 53668
	 */
	public function test_gc_get_default_extension_for_mime_type() {
		$this->assertEquals( 'jpg', gc_get_default_extension_for_mime_type( 'image/jpeg' ), 'jpg not returned as default extension for "image/jpeg"' );
		$this->assertNotEquals( 'jpeg', gc_get_default_extension_for_mime_type( 'image/jpeg' ), 'jpeg should not be returned as default extension for "image/jpeg"' );
		$this->assertEquals( 'png', gc_get_default_extension_for_mime_type( 'image/png' ), 'png not returned as default extension for "image/png"' );
		$this->assertFalse( gc_get_default_extension_for_mime_type( 'wibble/wobble' ), 'false not returned for unrecognized mime type' );
		$this->assertFalse( gc_get_default_extension_for_mime_type( '' ), 'false not returned when empty string as mime type supplied' );
		$this->assertFalse( gc_get_default_extension_for_mime_type( '   ' ), 'false not returned when empty string as mime type supplied' );
		$this->assertFalse( gc_get_default_extension_for_mime_type( 123 ), 'false not returned when int as mime type supplied' );
		$this->assertFalse( gc_get_default_extension_for_mime_type( null ), 'false not returned when null as mime type supplied' );
	}
}
