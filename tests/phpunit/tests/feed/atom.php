 ...
 * @group feed
 */
class Tests_Feed_Atom extends GC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		foreach ( self::$posts as $post ) {
			gc_set_object_terms( $post, self::$category->slug, 'category' );
		}
	...
	private function do_atom() {
		...
		try {
			...
			@require ABSPATH . 'gc-includes/feed-atom.php';
			$out = ob_get_clean();
		} catch ( Exception $e ) {
			$out = ob_get_clean();
			throw( $e );
	...
	public function test_entry_elements() {
		...
		foreach ( $entries as $key => $entry ) {
			...
			$user   = new GC_User( $post->post_author );
	...
	public function test_atom_enclosure_with_extended_url_length_type_parsing() {
		$enclosures = array(
			array(
				// URL, length, type.
				'actual'   => "https://gechiui.dev/gc-content/uploads/2017/09/movie.mp4\n318465\nvideo/mp4",
				'expected' => array(
					'href'   => 'https://gechiui.dev/gc-content/uploads/2017/09/movie.mp4',
					'length' => 318465,
					'type'   => 'video/mp4',
				),
			),
			...
			array(
				// URL, type.
				'actual'   => "https://gechiui.dev/gc-content/uploads/2017/01/audio.mp3\n\naudio/mpeg",
				'expected' => array(
					'href'   => 'https://gechiui.dev/gc-content/uploads/2017/01/audio.mp3',
					'length' => 0,
					'type'   => 'audio/mpeg',
				),
			),
			array(
				// URL.
				'actual'   => 'https://gechiui.dev/gc-content/uploads/2016/01/test.mp4',
				'expected' => array(
					'href'   => 'https://gechiui.dev/gc-content/uploads/2016/01/test.mp4',
					'length' => 0,
					'type'   => '',
				),
			),
