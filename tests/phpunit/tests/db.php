 ...
 * @group gcdb
 */
class Tests_DB extends GC_UnitTestCase {
	 ...
	 * @var resource
	 */
	protected static $_gcdb;

	public static function set_up_before_class() {
		parent::set_up_before_class();
		self::$_gcdb = new GcdbExposedMethodsForTesting();
	}
	...
	public function set_up() {
		...
		self::$_gcdb->last_error     = null;
		$GLOBALS['gcdb']->last_error = null;
	}
	...
	/**
	 * Test that GCDB will reconnect when the DB link dies
	 *
	 * @ticket 5932
	 */
	public function test_db_reconnect() {
		...
		$var = $gcdb->get_var( "SELECT ID FROM $gcdb->users LIMIT 1" );
		...
		$gcdb->db_connect();
		self::$_gcdb->db_connect();

		$this->assertGreaterThan( 0, $var );
	}

	/**
	 * Test that floats formatted as "0,700" get sanitized properly by gcdb
	 *
	 * @global mixed $gcdb
	 *
	 * @ticket 19861
	...
	public function test_locale_floats() {
		...
		$gcdb->suppress_errors( false );

		// Ensure the float isn't 0,700.
		$this->assertStringContainsString( '0.700', array_pop( $this->_queries ) );

		// Try a prepare.
		$sql = $gcdb->prepare( 'UPDATE test_table SET float_column = %f AND meta_id = %d', 0.7, 5 );
	...
	public function test_esc_like() {
		...
		foreach ( $inputs as $key => $input ) {
			$this->assertSame( $expected[ $key ], $gcdb->esc_like( $input ) );
		}
	}
	 ...
	 * Test LIKE Queries
	 *
	 * Make sure $gcdb is fully compatible with esc_like() by testing the identity of various strings.
	...
	public function test_like_query( $data, $like, $result ) {
		global $gcdb;
		return $this->assertSame( $result, $gcdb->get_var( $gcdb->prepare( 'SELECT %s LIKE %s', $data, $gcdb->esc_like( $like ) ) ) );
	}
	 ...
	 * @ticket 18510
	 */
	public function test_gcdb_supposedly_protected_properties() {
		global $gcdb;

		$this->assertNotEmpty( $gcdb->dbh );
		$dbh = $gcdb->dbh;
		$this->assertNotEmpty( $dbh );
		$this->assertTrue( isset( $gcdb->dbh ) ); // Test __isset().
		unset( $gcdb->dbh );
		$this->assertTrue( empty( $gcdb->dbh ) );
		$gcdb->dbh = $dbh;
		$this->assertNotEmpty( $gcdb->dbh );
	}
	 ...
	 * @ticket 21212
	 */
	public function test_gcdb_actually_protected_properties() {
		global $gcdb;

		$new_meta = "HAHA I HOPE THIS DOESN'T WORK";

		$col_meta       = $gcdb->col_meta;
		$gcdb->col_meta = $new_meta;

		$this->assertNotEquals( $col_meta, $new_meta );
		$this->assertSame( $col_meta, $gcdb->col_meta );
	}

	/**
	 * @ticket 18510
	 */
	public function test_gcdb_nonexistent_properties() {
		global $gcdb;

		$this->assertTrue( empty( $gcdb->nonexistent_property ) );
		$gcdb->nonexistent_property = true;
		$this->assertTrue( $gcdb->nonexistent_property );
		$this->assertTrue( isset( $gcdb->nonexistent_property ) );
		unset( $gcdb->nonexistent_property );
		$this->assertTrue( empty( $gcdb->nonexistent_property ) );
	}
	...
	public function test_double_escaped_placeholders() {
		global $gcdb;
		$sql = $gcdb->prepare( "UPDATE test_table SET string_column = '%%f is a float, %%d is an int %d, %%s is a string', field = %s", 3, '4' );
		$this->assertStringContainsString( $gcdb->placeholder_escape(), $sql );
	...
	public function test_set_sql_mode() {
		...
		$current_modes = $gcdb->get_var( 'SELECT @@SESSION.sql_mode;' );

		$new_modes = array( 'IGNORE_SPACE', 'NO_AUTO_VALUE_ON_ZERO' );

		$gcdb->set_sql_mode( $new_modes );

		$check_new_modes = $gcdb->get_var( 'SELECT @@SESSION.sql_mode;' );
		$this->assertSameSets( $new_modes, explode( ',', $check_new_modes ) );

		$gcdb->set_sql_mode( explode( ',', $current_modes ) );
	}
	...
	/**
	 * @ticket 25604
	 * @expectedIncorrectUsage gcdb::prepare
	 */
	public function test_prepare_without_arguments() {
		...
		$prepared = $gcdb->prepare( "SELECT * FROM $gcdb->users WHERE id = $id", $id );
		$this->assertSame( "SELECT * FROM $gcdb->users WHERE id = 0", $prepared );
	}

	public function test_prepare_sprintf() {
		global $gcdb;

		$prepared = $gcdb->prepare( "SELECT * FROM $gcdb->users WHERE id = %d AND user_login = %s", 1, 'admin' );
		$this->assertSame( "SELECT * FROM $gcdb->users WHERE id = 1 AND user_login = 'admin'", $prepared );
	}
	...
	public function test_prepare_sprintf_invalid_args() {
		...
		$prepared = @$gcdb->prepare( "SELECT * FROM $gcdb->users WHERE id = %d AND user_login = %s", 1, array( 'admin' ) );
		$this->assertSame( "SELECT * FROM $gcdb->users WHERE id = 1 AND user_login = ''", $prepared );

		// phpcs:ignore GeChiUI.PHP.NoSilencedErrors.Discouraged
		$prepared = @$gcdb->prepare( "SELECT * FROM $gcdb->users WHERE id = %d AND user_login = %s", array( 1 ), 'admin' );
		$this->assertSame( "SELECT * FROM $gcdb->users WHERE id = 0 AND user_login = 'admin'", $prepared );
	}
	...
	public function test_prepare_vsprintf_invalid_args() {
		...
		// phpcs:ignore GeChiUI.PHP.NoSilencedErrors.Discouraged
		$prepared = @$gcdb->prepare( "SELECT * FROM $gcdb->users WHERE id = %d AND user_login = %s", array( 1, array( 'admin' ) ) );
		$this->assertSame( "SELECT * FROM $gcdb->users WHERE id = 1 AND user_login = ''", $prepared );

		// phpcs:ignore GeChiUI.PHP.NoSilencedErrors.Discouraged
		$prepared = @$gcdb->prepare( "SELECT * FROM $gcdb->users WHERE id = %d AND user_login = %s", array( array( 1 ), 'admin' ) );
		$this->assertSame( "SELECT * FROM $gcdb->users WHERE id = 0 AND user_login = 'admin'", $prepared );
	}
	...
	public function test_prepare_incorrect_arg_count( $query, $args, $expected ) {
		...
		// phpcs:ignore GeChiUI.PHP.NoSilencedErrors.Discouraged,GeChiUI.DB.PreparedSQL
		$prepared = @$gcdb->prepare( $query, ...$args );
	...
	public function data_prepare_incorrect_arg_count() {
		...
		return array(
			array(
				"SELECT * FROM $gcdb->users WHERE id = %d AND user_login = %s",     // Query.
				array( 1, 'admin', 'extra-arg' ),                                   // ::prepare() args, to be passed via call_user_func_array().
				"SELECT * FROM $gcdb->users WHERE id = 1 AND user_login = 'admin'", // Expected output.
			),
			array(
				"SELECT * FROM $gcdb->users WHERE id = %%%d AND user_login = %s",
				array( 1 ),
				'',
			),
			array(
				...
				"SELECT * FROM $gcdb->users WHERE id = 1 AND user_login = 'admin'",
			),
			array(
				"SELECT * FROM $gcdb->users WHERE id = %d AND %% AND user_login = %s",
				array( 1, 'admin', 'extra-arg' ),
				"SELECT * FROM $gcdb->users WHERE id = 1 AND {$gcdb->placeholder_escape()} AND user_login = 'admin'",
			),
			array(
				"SELECT * FROM $gcdb->users WHERE id = %%%d AND %F AND %f AND user_login = %s",
				array( 1, 2.3, '4.5', 'admin', 'extra-arg' ),
				"SELECT * FROM $gcdb->users WHERE id = {$gcdb->placeholder_escape()}1 AND 2.300000 AND 4.500000 AND user_login = 'admin'",
			),
			array(
				"SELECT * FROM $gcdb->users WHERE id = %d AND user_login = %s",
				array( array( 1 ), 'admin', 'extra-arg' ),
				"SELECT * FROM $gcdb->users WHERE id = 0 AND user_login = 'admin'",
			),
			array(
				"SELECT * FROM $gcdb->users WHERE id = %d and user_nicename = %s and user_status = %d and user_login = %s",
				array( 1, 'admin', 0 ),
				'',
			),
			array(
			...
			array(
				"SELECT * FROM $gcdb->users WHERE id = %d and %% and user_login = %s and user_status = %d and user_login = %s",
				array( 1, 'admin', 'extra-arg' ),
				'',
			),
		);
	...
	public function test_db_version() {
		...
		$this->assertTrue( version_compare( $gcdb->db_version(), '5.0', '>=' ) );
	}
	...
	public function test_has_cap() {
		global $gcdb;
		$this->assertTrue( $gcdb->has_cap( 'collation' ) );
		$this->assertTrue( $gcdb->has_cap( 'group_concat' ) );
		$this->assertTrue( $gcdb->has_cap( 'subqueries' ) );
		$this->assertTrue( $gcdb->has_cap( 'COLLATION' ) );
		$this->assertTrue( $gcdb->has_cap( 'GROUP_CONCAT' ) );
		$this->assertTrue( $gcdb->has_cap( 'SUBQUERIES' ) );
		$this->assertSame(
			version_compare( $gcdb->db_version(), '5.0.7', '>=' ),
			$gcdb->has_cap( 'set_charset' )
		);
		$this->assertSame(
			version_compare( $gcdb->db_version(), '5.0.7', '>=' ),
			$gcdb->has_cap( 'SET_CHARSET' )
	...
	public function test_supports_collation() {
		global $gcdb;
		$this->assertTrue( $gcdb->supports_collation() );
	}

	public function test_check_database_version() {
		global $gcdb;
		$this->assertEmpty( $gcdb->check_database_version() );
	}
	...
	public function test_timers() {
		...
		$gcdb->timer_start();
		usleep( 5 );
		$stop = $gcdb->timer_stop();

		$this->assertNotEquals( $gcdb->time_start, $stop );
		$this->assertGreaterThan( $stop, $gcdb->time_start );
	}

	public function test_get_col_info() {
		global $gcdb;

		$gcdb->get_results( "SELECT ID FROM $gcdb->users" );

		$this->assertSame( array( 'ID' ), $gcdb->get_col_info() );
		$this->assertSame( array( $gcdb->users ), $gcdb->get_col_info( 'table' ) );
		$this->assertSame( $gcdb->users, $gcdb->get_col_info( 'table', 0 ) );
	}

	public function test_query_and_delete() {
		global $gcdb;
		$rows = $gcdb->query( "INSERT INTO $gcdb->users (display_name) VALUES ('Walter Sobchak')" );
		$this->assertSame( 1, $rows );
		$this->assertNotEmpty( $gcdb->insert_id );
		$d_rows = $gcdb->delete( $gcdb->users, array( 'ID' => $gcdb->insert_id ) );
	...
	public function test_get_row() {
		...
		$row = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->users WHERE ID = %d", $gcdb->insert_id ) );
	 ...
	 * @param arrray|string|null $last_result The value to assign to `$gcdb->last_result`.
	...
	public function test_get_col( $query, $expected, $last_result, $column ) {
		...
		$gcdb->last_result = $last_result;

		// phpcs:ignore GeChiUI.DB.PreparedSQL.NotPrepared
		$result = $gcdb->get_col( $query, $column );

		if ( $query ) {
			$this->assertSame( $query, $gcdb->last_query );
		}
	 ...
	 *     @type arrray|string|null $last_result The value to assign to `$gcdb->last_result`.
	...
	public function data_test_get_col() {
		...
		return array(
			array(
				"SELECT display_name FROM $gcdb->users",
				'admin',
				array(),
				0,
			),
			array(
				"SELECT user_login, user_email FROM $gcdb->users",
				'admin',
				array(),
				0,
			),
	...
	public function test_replace() {
		global $gcdb;
		$rows1 = $gcdb->insert( $gcdb->users, array( 'display_name' => 'Walter Sobchak' ) );
		...
		$this->assertSame( $last, $gcdb->insert_id );

		$row = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->users WHERE ID = %d", $last ) );
		$this->assertSame( 'Walter Replace Sobchak', $row->display_name );
	}

	/**
	 * gcdb::update() requires a WHERE condition.
	 *
	 * @ticket 26106
	 */
	public function test_empty_where_on_update() {
		global $gcdb;
		$suppress = $gcdb->suppress_errors( true );
		$gcdb->update( $gcdb->posts, array( 'post_name' => 'burrito' ), array() );

		$expected1 = "UPDATE `{$gcdb->posts}` SET `post_name` = 'burrito' WHERE ";
		$this->assertNotEmpty( $gcdb->last_error );
		$this->assertSame( $expected1, $gcdb->last_query );

		$gcdb->update( $gcdb->posts, array( 'post_name' => 'burrito' ), array( 'post_status' => 'taco' ) );

		$expected2 = "UPDATE `{$gcdb->posts}` SET `post_name` = 'burrito' WHERE `post_status` = 'taco'";
		$this->assertEmpty( $gcdb->last_error );
		$this->assertSame( $expected2, $gcdb->last_query );
	...
	public function test_mysqli_flush_sync() {
		...
		$gcdb->query( 'DROP PROCEDURE IF EXISTS `test_mysqli_flush_sync_procedure`' );
		$gcdb->query(
			'CREATE PROCEDURE `test_mysqli_flush_sync_procedure`() BEGIN
			SELECT ID FROM `' . $gcdb->posts . '` LIMIT 1;
		END'
		);

		if ( count( $gcdb->get_results( 'SHOW CREATE PROCEDURE `test_mysqli_flush_sync_procedure`' ) ) < 1 ) {
			$gcdb->suppress_errors( $suppress );
		...
		$this->assertNotEmpty( $gcdb->get_results( 'CALL `test_mysqli_flush_sync_procedure`' ) );
		$this->assertNotEmpty( $gcdb->get_results( "SELECT ID FROM `{$gcdb->posts}` LIMIT 1" ) );
	...
	public function data_get_table_from_query() {
		...
		$queries = array(
			...
			"LOAD DATA INFILE 'gc.txt' INTO TABLE $table",
			"LOAD DATA LOW_PRIORITY INFILE 'gc.txt' INTO TABLE $table",
			"LOAD DATA CONCURRENT INFILE 'gc.txt' INTO TABLE $table",
			"LOAD DATA LOW_PRIORITY LOCAL INFILE 'gc.txt' INTO TABLE $table",
			"LOAD DATA INFILE 'gc.txt' REPLACE INTO TABLE $table",
			"LOAD DATA INFILE 'gc.txt' IGNORE INTO TABLE $table",
	...
	public function test_get_table_from_query( $query, $table ) {
		$this->assertSame( $table, self::$_gcdb->get_table_from_query( $query ) );
	}
	...
	public function test_get_table_from_query_false( $query ) {
		$this->assertFalse( self::$_gcdb->get_table_from_query( $query ) );
	}
	...
	public function test_process_field_formats( $data, $format, $expected, $message ) {
		$actual = self::$_gcdb->process_field_formats( $data, $format );
	...
	public function test_process_fields() {
		...
		if ( $gcdb->charset ) {
			$expected_charset = $gcdb->charset;
		} else {
			$expected_charset = $gcdb->get_col_charset( $gcdb->posts, 'post_content' );
		}
		...
		$expected = array(
			'post_content' => array(
				...
				'length'  => $gcdb->get_col_length( $gcdb->posts, 'post_content' ),
			),
		);

		$this->assertSame( $expected, self::$_gcdb->process_fields( $gcdb->posts, $data, null ) );
	}
	...
	public function test_process_fields_on_nonexistent_table( $data ) {
		self::$_gcdb->suppress_errors( true );
		$data = array( 'post_content' => '¡foo foo foo!' );
		$this->assertFalse( self::$_gcdb->process_fields( 'nonexistent_table', $data, null ) );
		self::$_gcdb->suppress_errors( false );
	}
	...
	public function test_pre_get_table_charset_filter() {
		...
		$charset = self::$_gcdb->get_table_charset( 'some_table' );
	...
	public function test_pre_get_col_charset_filter() {
		...
		$charset = self::$_gcdb->get_col_charset( 'some_table', 'some_col' );
	...
	public function test_process_fields_value_too_long_for_field( array $data, $errored_fields ) {
		...
		$this->assertFalse( self::$_gcdb->process_fields( $gcdb->posts, $data, null ) );
		$this->assertSame( $this->get_db_error_value_too_long( $errored_fields ), self::$_gcdb->last_error );
	}
	...
	public function test_insert_value_too_long_for_field( array $data, $errored_fields ) {
		...
		$this->assertFalse( $gcdb->insert( $gcdb->posts, $data ) );
		$this->assertSame( $this->get_db_error_value_too_long( $errored_fields ), $gcdb->last_error );
	}
	...
	public function test_replace_value_too_long_for_field( array $data, $errored_fields ) {
		...
		$this->assertFalse( $gcdb->replace( $gcdb->posts, $data ) );
		$this->assertSame( $this->get_db_error_value_too_long( $errored_fields ), $gcdb->last_error );
	}
	...
	public function test_update_value_too_long_for_field( array $data, $errored_fields ) {
		...
		$this->assertFalse( $gcdb->update( $gcdb->posts, $data, array() ) );
		$this->assertSame( $this->get_db_error_value_too_long( $errored_fields ), $gcdb->last_error );
	}
	...
	public function test_delete_value_too_long_for_field( array $data, $errored_fields ) {
		...
		$this->assertFalse( $gcdb->delete( $gcdb->posts, $data, array() ) );
		$this->assertSame( $this->get_db_error_value_too_long( $errored_fields ), $gcdb->last_error );
	}
	...
	private function get_db_error_value_too_long( $errored_fields ) {
		if ( str_contains( $errored_fields, ', ' ) ) {
			return sprintf(
				'GeChiUI database error: Processing the values for the following fields failed: %s. ' .
				'The supplied values may be too long or contain invalid data.',
				$errored_fields
			);
		}
		return sprintf(
			'GeChiUI database error: Processing the value for the following field failed: %s. ' .
	...
	public function test_query_value_contains_invalid_chars() {
		...
		$this->assertFalse(
			$gcdb->query( "INSERT INTO {$gcdb->posts} (post_status) VALUES ('\xF5')" )
		);

		$this->assertSame(
			'GeChiUI database error: Could not perform query because it contains invalid data.',
			$gcdb->last_error
		);
	}
	...
	public function test_null_insert() {
		...
		$row = $gcdb->get_row( $gcdb->prepare( "SELECT * FROM $gcdb->postmeta WHERE meta_key=%s", $key ) );
	...
	public function test_close() {
		...
		$this->assertTrue( $gcdb->close() );
		$this->assertFalse( $gcdb->close() );

		$this->assertFalse( $gcdb->ready );
		$this->assertFalse( $gcdb->has_connected );
	...
	public function test_charset_not_determined_when_disconnected() {
		...
		$result = $gcdb->determine_charset( $charset, $collate );
	...
	public function test_prepare_with_placeholders_and_individual_args( $sql, $values, $incorrect_usage, $expected ) {
		...
		if ( $incorrect_usage ) {
			$this->setExpectedIncorrectUsage( 'gcdb::prepare' );
		}
		...
		// phpcs:ignore GeChiUI.DB.PreparedSQL
		$sql = $gcdb->prepare( $sql, ...$values );
	...
	public function data_prepare_with_placeholders() {
		...
		return array(
			...
			array(
				...
				1,
				true,
				"1 {$gcdb->placeholder_escape()}{$gcdb->placeholder_escape()} {$gcdb->placeholder_escape()} {$gcdb->placeholder_escape()}1\$d{$gcdb->placeholder_escape()} {$gcdb->placeholder_escape()}1{$gcdb->placeholder_escape()}",
			),
			array(
				'%-5s',
				'foo',
			...
			array(
				...
				' %1$s ',
				false,
				" {$gcdb->placeholder_escape()}1\$s ",
			),
			array(
				'%d %1$d %%% %',
				1,
				true,
				"1 1 {$gcdb->placeholder_escape()}{$gcdb->placeholder_escape()} {$gcdb->placeholder_escape()}",
			),
			array(
				'%d %2$s',
				array( 1, 'hello' ),
			...
			array(
				...
				'hello',
				false,
				"{$gcdb->placeholder_escape()}s {$gcdb->placeholder_escape()}'hello'",
			),
			array(
				'%%s %%"%1$s"',
				'hello',
			...
			array(
				...
				3,
				false,
				"{$gcdb->placeholder_escape()}f {$gcdb->placeholder_escape()}\"3.000000\"",
			),
			array(
			...
			array(
				...
				'hello',
				true,
				"'{$gcdb->placeholder_escape()}'{$gcdb->placeholder_escape()}s",
			),
			array(
				"'%'%%s%s",
				'hello',
				false,
				"'{$gcdb->placeholder_escape()}'{$gcdb->placeholder_escape()}s'hello'",
			),
			array(
				"'%'%%s %s",
				'hello',
	...
	public function test_escape_and_prepare( $escape, $sql, $values, $incorrect_usage, $expected ) {
		...
		// phpcs:ignore GeChiUI.DB.PreparedSQL.NotPrepared
		$actual = $gcdb->prepare( $sql, $values );
	...
	public function data_escape_and_prepare() {
		...
		return array(
			array(
				...
				" {$gcdb->placeholder_escape()}s ",    // Expected output.
			),
			array(
				'foo%sbar',
	...
	public function test_double_prepare() {
		...
		$part = $gcdb->prepare( ' AND meta_value = %s', ' %s ' );
		$this->assertStringNotContainsString( '%s', $part );
		// phpcs:ignore GeChiUI.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
		$query = $gcdb->prepare( 'SELECT * FROM {$gcdb->postmeta} WHERE meta_key = %s $part', array( 'foo', 'bar' ) );
	...
	public function test_prepare_numeric_placeholders_float_args() {
		...
		$actual = $gcdb->prepare(
			// phpcs:ignore GeChiUI.DB.PreparedSQLPlaceholders.UnquotedComplexPlaceholder
	...
	public function test_query_unescapes_placeholders() {
		...
		$gcdb->query( "CREATE TABLE {$gcdb->prefix}test_placeholder( a VARCHAR(100) );" );
		$sql = $gcdb->prepare( "INSERT INTO {$gcdb->prefix}test_placeholder VALUES(%s)", $value );

		// phpcs:ignore GeChiUI.DB.PreparedSQL.NotPrepared
		$gcdb->query( $sql );

		$actual = $gcdb->get_var( "SELECT a FROM {$gcdb->prefix}test_placeholder" );

		$gcdb->query( "DROP TABLE {$gcdb->prefix}test_placeholder" );
	...
	public function test_esc_sql_with_unsupported_placeholder_type() {
		...
		$sql = $gcdb->prepare( ' %s %1$c ', 'foo' );
		// phpcs:ignore GeChiUI.DB.PreparedSQL.InterpolatedNotPrepared
		$sql = $gcdb->prepare( " $sql %s ", 'foo' );

		$this->assertSame( "  'foo' {$gcdb->placeholder_escape()}1\$c  'foo' ", $sql );
	}
	...
	public function test_parse_db_host( $host_string, $expect_bail, $host, $port, $socket, $is_ipv6 ) {
		global $gcdb;
		$data = $gcdb->parse_db_host( $host_string );
		if ( $expect_bail ) {
