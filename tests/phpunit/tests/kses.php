 ...
 * @group kses
 */
class Tests_Kses extends GC_UnitTestCase {

	/**
	 * @dataProvider data_gc_filter_post_kses_address
	 * @ticket 20210
	 *
	 * @param string $string        Test string for kses.
	 * @param string $expect_string Expected result after passing through kses.
	 */
	public function test_gc_filter_post_kses_address( $string, $expect_string ) {
		global $allowedposttags;

		$this->assertSame( $expect_string, gc_kses( $string, $allowedposttags ) );
	}

	/**
	 * Data provider for test_gc_filter_post_kses_address.
	 *
	 ...
	 * }
	 */
	public function data_gc_filter_post_kses_address() {
		$attributes = array(
			'class' => 'classname',
			'id'    => 'id',
			'style' => array(
		...
		foreach ( $attributes as $name => $values ) {
			foreach ( (array) $values as $value ) {
				$string        = "<address $name='$value'>1 GeChiUI Avenue, The Internet.</address>";
				$expect_string = "<address $name='" . str_replace( '; ', ';', trim( $value, ';' ) ) . "'>1 GeChiUI Avenue, The Internet.</address>";
	...
	/**
	 * @dataProvider data_gc_filter_post_kses_a
	 * @ticket 20210
	 *
	 * @param string $string        Test string for kses.
	 * @param string $expect_string Expected result after passing through kses.
	 * @return void
	 */
	public function test_gc_filter_post_kses_a( $string, $expect_string ) {
		global $allowedposttags;
	...
	/**
	 * Data provider for test_gc_filter_post_kses_a.
	 *
	 * @return array[] Arguments {
	 *     @type string $string        Test string for kses.
	 *     @type string $expect_string Expected result after passing through kses.
	 * }
	 */
	public function data_gc_filter_post_kses_a() {
		$attributes = array(
			'class'    => 'classname',
			'id'       => 'id',
			'style'    => 'color: red;',
	 ...
	 * @ticket 50167
	 * @ticket 29826
	 * @dataProvider data_gc_kses_video
	 *
	 ...
	 */
	public function test_gc_kses_video( $source, $context, $expected ) {
		$actual = gc_kses( $source, $context );
		$this->assertSame( $expected, $actual );
	}

	/**
	 * Data provider for test_gc_kses_video
	 *
	 ...
	 * }
	 */
	public function data_gc_kses_video() {
		return array(
			...
			array(
				'<video onload="alert(1);" src="https://videos.files.gechiui.com/DZEMDKxc/video-0f9c363010.mp4" />',
				'post',
				'<video src="https://videos.files.gechiui.com/DZEMDKxc/video-0f9c363010.mp4" />',
			),
		);
	}

	/**
	 * @dataProvider data_gc_filter_post_kses_abbr
	 * @ticket 20210
	 *
	 * @param string $string        Test string for kses.
	 * @param string $expect_string Expected result after passing through kses.
	 * @return void
	 */
	public function test_gc_filter_post_kses_abbr( $string, $expect_string ) {
		global $allowedposttags;
	...
	/**
	 * Data provider for data_gc_filter_post_kses_abbr.
	 *
	 * @return array[] Arguments {
	 *     @type string $string        Test string for kses.
	 *     @type string $expect_string Expected result after passing through kses.
	 * }
	 */
	public function data_gc_filter_post_kses_abbr() {
		$attributes = array(
			'class' => 'classname',
			'id'    => 'id',
			'style' => 'color: red;',
		...
		foreach ( $attributes as $name => $value ) {
			$string        = "<abbr $name='$value'>GC</abbr>";
			$expect_string = "<abbr $name='" . trim( $value, ';' ) . "'>GC</abbr>";
			$data[]        = array( $string, $expect_string );
		}
...
EOF;
		...
		$this->assertSame( $expected, gc_kses( $content, $allowedposttags ) );
	}

	public function test_gc_kses_bad_protocol() {
		$bad = array(
			'dummy:alert(1)',
			'javascript:alert(1)',
			'JaVaScRiPt:alert(1)',
		...
		foreach ( $bad as $k => $x ) {
			$result = gc_kses_bad_protocol( gc_kses_normalize_entities( $x ), gc_allowed_protocols() );
			if ( ! empty( $result ) && 'alert(1);' !== $result && 'alert(1)' !== $result ) {
				switch ( $k ) {
					...
					case 27:
						...
						break;
					default:
						$this->fail( "gc_kses_bad_protocol failed on $k, $x. Result: $result" );
				}
			}
		}
		...
		foreach ( $bad_not_normalized as $k => $x ) {
			$result = gc_kses_bad_protocol( $x, gc_allowed_protocols() );
			if ( ! empty( $result ) && 'alert(1);' !== $result && 'alert(1)' !== $result ) {
				$this->fail( "gc_kses_bad_protocol failed on $k, $x. Result: $result" );
			}
		}
		...
		$safe = array(
			...
			'http&#x3A;//example.org/',
			'https://example.org',
			'http://example.org/gc-admin/post.php?post=2&amp;action=edit',
			'http://example.org/index.php?test=&#039;blah&#039;',
		);
		foreach ( $safe as $x ) {
			$result = gc_kses_bad_protocol( gc_kses_normalize_entities( $x ), array( 'http', 'https', 'dummy' ) );
			if ( $result !== $x && 'http://example.org/' !== $result ) {
				$this->fail( "gc_kses_bad_protocol incorrectly blocked $x" );
			}
		}
	}
	...
	public function gc_kses_allowed_html_filter( $html, $context ) {
		if ( 'post' === $context ) {
	 ...
	 * @ticket 20210
	 */
	public function test_gc_kses_allowed_html() {
		global $allowedposttags, $allowedtags, $allowedentitynames;

		$this->assertSame( $allowedposttags, gc_kses_allowed_html( 'post' ) );

		$tags = gc_kses_allowed_html( 'post' );
		...
		$this->assertSame( $allowedtags, gc_kses_allowed_html( 'data' ) );
		$this->assertSame( $allowedtags, gc_kses_allowed_html( '' ) );
		$this->assertSame( $allowedtags, gc_kses_allowed_html() );

		$tags = gc_kses_allowed_html( 'user_description' );
		$this->assertTrue( $tags['a']['rel'] );

		$tags = gc_kses_allowed_html();
		$this->assertArrayNotHasKey( 'rel', $tags['a'] );

		$this->assertSame( array(), gc_kses_allowed_html( 'strip' ) );
		...
		$this->assertSame( $custom_tags, gc_kses_allowed_html( $custom_tags ) );

		add_filter( 'gc_kses_allowed_html', array( $this, 'gc_kses_allowed_html_filter' ), 10, 2 );

		$this->assertSame( array( 'a' => array( 'href' => true ) ), gc_kses_allowed_html( 'post' ) );
		$this->assertSame( array( 'a' => array( 'href' => false ) ), gc_kses_allowed_html( 'data' ) );

		remove_filter( 'gc_kses_allowed_html', array( $this, 'gc_kses_allowed_html_filter' ) );
		$this->assertSame( $allowedposttags, gc_kses_allowed_html( 'post' ) );
		$this->assertSame( $allowedtags, gc_kses_allowed_html( 'data' ) );
	}
	...
	public function test_hyphenated_tag() {
		...
		$this->assertSame( $expect_stripped_string, gc_kses_post( $string ) );
		$this->assertSame( $expect_valid_string, gc_kses( $string, $custom_tags ) );
	}

	/**
	 * @ticket 26290
	 */
	public function test_gc_kses_normalize_entities() {
		$this->assertSame( '&spades;', gc_kses_normalize_entities( '&spades;' ) );

		$this->assertSame( '&sup1;', gc_kses_normalize_entities( '&sup1;' ) );
		$this->assertSame( '&sup2;', gc_kses_normalize_entities( '&sup2;' ) );
		$this->assertSame( '&sup3;', gc_kses_normalize_entities( '&sup3;' ) );
		$this->assertSame( '&frac14;', gc_kses_normalize_entities( '&frac14;' ) );
		$this->assertSame( '&frac12;', gc_kses_normalize_entities( '&frac12;' ) );
		$this->assertSame( '&frac34;', gc_kses_normalize_entities( '&frac34;' ) );
		$this->assertSame( '&there4;', gc_kses_normalize_entities( '&there4;' ) );
	}
	...
	public function test_ctrl_removal( $input, $output ) {
		...
		return $this->assertSame( $output, gc_kses( $input, $allowedposttags ) );
	}
	...
	/**
	 * Test new function gc_kses_hair_parse().
	 *
	 * @dataProvider data_hair_parse
	 */
	public function test_hair_parse( $input, $output ) {
		return $this->assertSame( $output, gc_kses_hair_parse( $input ) );
	}
	...
	/**
	 * Test new function gc_kses_attr_parse().
	 *
	 * @dataProvider data_attr_parse
	 */
	public function test_attr_parse( $input, $output ) {
		return $this->assertSame( $output, gc_kses_attr_parse( $input ) );
	}
	...
	/**
	 * Test new function gc_kses_one_attr().
	 *
	 * @dataProvider data_one_attr
	 */
	public function test_one_attr( $element, $input, $output ) {
		return $this->assertSame( $output, gc_kses_one_attr( $input, $element ) );
	}
	...
	public function test_bdo() {
		...
		$this->assertSame( $input, gc_kses( $input, $allowedposttags ) );
	}
	 ...
	 * @ticket 40680
	 */
	public function test_gc_kses_attr_no_attributes_allowed_with_empty_array() {
		$element   = 'foo';
		$attribute = 'title="foo" class="bar"';

		$this->assertSame( "<{$element}>", gc_kses_attr( $element, $attribute, array( 'foo' => array() ), array() ) );
	}

	/**
	 * @ticket 40680
	 */
	public function test_gc_kses_attr_no_attributes_allowed_with_true() {
		$element   = 'foo';
		$attribute = 'title="foo" class="bar"';

		$this->assertSame( "<{$element}>", gc_kses_attr( $element, $attribute, array( 'foo' => true ), array() ) );
	}

	/**
	 * @ticket 40680
	 */
	public function test_gc_kses_attr_single_attribute_is_allowed() {
		$element   = 'foo';
		$attribute = 'title="foo" class="bar"';

		$this->assertSame( "<{$element} title=\"foo\">", gc_kses_attr( $element, $attribute, array( 'foo' => array( 'title' => true ) ), array() ) );
	}

	/**
	 * @ticket 43312
	 */
	public function test_gc_kses_attr_no_attributes_allowed_with_false() {
		$element   = 'foo';
		$attribute = 'title="foo" class="bar"';

		$this->assertSame( "<{$element}>", gc_kses_attr( $element, $attribute, array( 'foo' => false ), array() ) );
	}
	 ...
	 * @ticket 33121
	 */
	public function test_gc_kses_attr_data_attribute_is_allowed() {
		...
		$this->assertSame( $expected, gc_kses_post( $test ) );
	}
	...
	public function test_wildcard_requires_hyphen_after_prefix() {
		...
		$actual = gc_kses( $string, $allowed_html );
	...
	public function test_wildcard_allows_two_hyphens() {
		...
		$string   = '<div data-gc-id="pens-and-pencils">Well formed attribute</div>';
		$expected = '<div data-gc-id="pens-and-pencils">Well formed attribute</div>';
	...
	public function test_wildcard_attribute_prefixes( $wildcard_attribute, $expected ) {
		...
		$actual = gc_kses_attr_check( $name, $value, $whole, 'n', 'div', $allowed_html );
	 ...
	 *               [1] The outcome of `gc_kses_attr_check` for the prefix.
	 */
	...
	public function data_kses_style_attr_with_url() {
		return array(
			...
			array(
				"background: url('../gc-content/uploads/2018/10/valid.gif');",
				"background: url('../gc-content/uploads/2018/10/valid.gif')",
			),
			...
			// CSS variable.
			array(
				'padding: var(--gc-var1) var(--gc-var2)',
				'padding: var(--gc-var1) var(--gc-var2)',
			),

			// CSS calc() with var().
			array(
				'margin-top: calc(var(--gc-var1) * 3 + 2em)',
				'margin-top: calc(var(--gc-var1) * 3 + 2em)',
			),
	 ...
	 * @ticket 50731
	 */
	public function test_gc_kses_img_tag_standard_attributes() {
		$html = array(
			'<img',
			'loading="lazy"',
		...
		$this->assertSame( $html, gc_kses_post( $html ) );
	}
	 ...
	 * @ticket 53156
	 */
	public function test_gc_kses_main_tag_standard_attributes() {
		$test = array(
			'<main',
			'class="gc-group-block"',
			'style="padding:10px"',
			'/>',
		);
	 ...
	 * @ticket 54261
	 *
	 * @dataProvider data_gc_kses_object_tag_allowed
	 *
	 * @param string $html     A string of HTML to test.
	 * @param string $expected The expected result from KSES.
	 */
	function test_gc_kses_object_tag_allowed( $html, $expected ) {
		$this->assertSame( $expected, gc_kses_post( $html ) );
	}

	/**
	 * Data provider for test_gc_kses_object_tag_allowed().
	 */
	function data_gc_kses_object_tag_allowed() {
		return array(
	 ...
	 * @ticket 54261
	 *
	 * @dataProvider data_gc_kses_object_data_url_with_port_number_allowed
	 *
	 * @param string $html     A string of HTML to test.
	 * @param string $expected The expected result from KSES.
	 */
	function test_gc_kses_object_data_url_with_port_number_allowed( $html, $expected ) {
		add_filter( 'upload_dir', array( $this, 'gc_kses_upload_dir_filter' ), 10, 2 );
		$this->assertSame( $expected, gc_kses_post( $html ) );
	}

	/**
	 * Data provider for test_gc_kses_object_data_url_with_port_number_allowed().
	 */
	function data_gc_kses_object_data_url_with_port_number_allowed() {
		return array(
	 ...
	 *
	 * @param  array $param See gc_upload_dir()
	 * @return array        $param with a modified `url`.
	 */
	public function gc_kses_upload_dir_filter( $param ) {
	 ...
	 * 'gc_kses_allowed_html' filter.
	 *
	 * @ticket 54261
	 */
	function test_gc_kses_object_added_in_html_filter() {
		$html = <<<HTML
<object type="application/pdf" data="https://www.gechiui.com/foo.pdf" />
<object type="application/x-shockwave-flash" data="https://www.gechiui.com/foo.swf">
	<param name="foo" value="bar" />
</object>
HTML;

		add_filter( 'gc_kses_allowed_html', array( $this, 'filter_gc_kses_object_added_in_html_filter' ), 10, 2 );

		$filtered_html = gc_kses_post( $html );

		remove_filter( 'gc_kses_allowed_html', array( $this, 'filter_gc_kses_object_added_in_html_filter' ) );

		$this->assertSame( $html, $filtered_html );
	}

	function filter_gc_kses_object_added_in_html_filter( $tags, $context ) {
		if ( 'post' === $context ) {
			$tags['object'] = array(
				'type' => true,
				'data' => true,
	 ...
	 * @ticket 54261
	 *
	 * @dataProvider data_gc_kses_allowed_values_list
	 *
	 * @param string $html         A string of HTML to test.
	 * @param string $expected     The expected result from KSES.
	 * @param array  $allowed_html The allowed HTML to pass to KSES.
	 */
	function test_gc_kses_allowed_values_list( $html, $expected, $allowed_html ) {
		$this->assertSame( $expected, gc_kses( $html, $allowed_html ) );
	}

	/**
	 * Data provider for test_gc_kses_allowed_values_list().
	 */
	function data_gc_kses_allowed_values_list() {
		$data = array(
	 ...
	 * @ticket 54261
	 *
	 * @dataProvider data_gc_kses_required_attribute
	 *
	 * @param string $html         A string of HTML to test.
	 * @param string $expected     The expected result from KSES.
	 * @param array  $allowed_html The allowed HTML to pass to KSES.
	 */
	function test_gc_kses_required_attribute( $html, $expected, $allowed_html ) {
		$this->assertSame( $expected, gc_kses( $html, $allowed_html ) );
	}

	/**
	 * Data provider for test_gc_kses_required_attribute().
	 */
	function data_gc_kses_required_attribute() {
		$data = array(
			'valid dir attribute value'             => array(
	 ...
	 *
	 * @dataProvider data_gc_kses_xml_named_entities
	 *
	 * @ticket 54060
	 * @covers ::gc_kses_xml_named_entities
	 *
	 * @param array  $input    The input to gc_kses_xml_named_entities().
	 * @param string $expected The expected output.
	 */
	public function test_gc_kses_xml_named_entities( $input, $expected ) {
		$this->assertSame( $expected, gc_kses_xml_named_entities( $input ) );
	}

	/**
	 * Data provider for test_gc_kses_xml_named_entities().
	 *
	 * @return array Nested array of input, expected pairs.
	 */
	public function data_gc_kses_xml_named_entities() {
		return array(
