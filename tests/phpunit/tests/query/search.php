 ...
 * @group search
 */
class Tests_Query_Search extends GC_UnitTestCase {
	protected $q;
	protected $post_type;
	...
	public function test_search_terms_query_var() {
		...
		$query = new GC_Query( array( 's' => 'This is a search term' ) );
	...
	public function test_filter_stopwords() {
		$terms = 'This is a search term';
		add_filter( 'gc_search_stopwords', array( $this, 'filter_gc_search_stopwords' ) );
		$query = new GC_Query( array( 's' => $terms ) );
		remove_filter( 'gc_search_stopwords', array( $this, 'filter_gc_search_stopwords' ) );
	...
	public function filter_gc_search_stopwords() {
		return array();
	}
	...
	public function test_disable_search_exclusion_prefix() {
		...
		add_filter( 'gc_query_search_exclusion_prefix', '__return_false' );
		$result = $this->get_search_results( $title );
		$post   = array_pop( $result );
		$this->assertSame( $post->ID, $post_id );
		remove_filter( 'gc_query_search_exclusion_prefix', '__return_false' );
	}
	...
	public function test_change_search_exclusion_prefix() {
		...
		add_filter( 'gc_query_search_exclusion_prefix', array( $this, 'filter_search_exclusion_prefix_octothorpe' ) );
		$found = $this->get_search_results( $title );
		remove_filter( 'gc_query_search_exclusion_prefix', array( $this, 'filter_search_exclusion_prefix_octothorpe' ) );
	...
	public function test_exclude_file_names_in_attachment_search_by_default() {
		...
		add_post_meta( $attachment, '_gc_attached_file', 'some-image2.png', true );
	...
	public function test_include_file_names_in_attachment_search_as_string() {
		...
		add_post_meta( $attachment, '_gc_attached_file', 'some-image1.png', true );
	...
	public function test_exclude_attachment_file_names_in_general_searches() {
		...
		add_post_meta( $attachment, '_gc_attached_file', 'some-image3.png', true );
	...
	public function test_include_file_names_in_attachment_search_with_meta_query() {
		...
		add_post_meta( $attachment, '_gc_attached_file', 'some-image4.png', true );
	...
	public function test_include_file_names_in_attachment_search_with_tax_query() {
		...
		gc_set_post_terms( $attachment, 'test', 'post_tag' );

		add_post_meta( $attachment, '_gc_attached_file', 'some-image5.png', true );
		add_filter( 'posts_clauses', '_filter_query_attachment_filenames' );
