...
/**
 * Test various query vars and make sure the GC_Query class selects the correct posts.
 * We're testing against a known data set, so we can check that specific posts are included in the output.
 *
 * @group query
 */
class Tests_Query_Results extends GC_UnitTestCase {
	protected $q;
	...
	public function test_query_default() {
		...
		$this->assertSame( $expected, gc_list_pluck( $posts, 'post_name' ) );
	}
	...
	public function test_query_post_parent__in() {
		...
		$this->assertSame(
			...
			gc_list_pluck( $posts, 'post_title' )
		);
		...
		$this->assertSame( array(), gc_list_pluck( $posts, 'post_title' ) );
	}
	...
	public function test_query_orderby_post_parent__in_with_order_desc() {
		...
		$this->assertSame( $expected_returned_array, gc_list_pluck( $posts, 'post_title' ) );
	}
	...
	public function test_query_orderby_post_name__in_with_order_asc() {
		...
		$this->assertSame( $post_name__in_array, array_unique( gc_list_pluck( $q->posts, 'post_title' ) ) );
	}
	...
	public function test_exclude_from_search_empty() {
		...
		foreach ( array_keys( $gc_post_types ) as $slug ) {
			$gc_post_types[ $slug ]->exclude_from_search = true;
		}
		...
		foreach ( array_keys( $gc_post_types ) as $slug ) {
			$gc_post_types[ $slug ]->exclude_from_search = false;
		}
	...
	public function test_query_author_vars() {
		...
		$author_ids = array_unique( gc_list_pluck( $posts, 'post_author' ) );
	...
	public function test_perm_with_status_array() {
		...
		$this->assertStringContainsString(
			"(({$gcdb->posts}.post_status = 'publish') OR ({$gcdb->posts}.post_author = 0 AND ({$gcdb->posts}.post_status = 'private')))",
			$this->q->request
		);
		$this->assertStringNotContainsString( "({$gcdb->posts}.post_status = 'publish') AND", $this->q->request );
	}
	...
	public function test_duplicate_slug_in_hierarchical_post_type() {
		...
		$post_2 = self::factory()->post->create(
			array(
				'post_title' => 'Contributing to the GeChiUI Codex',
				'post_type'  => 'handbook',
			)
		);
		$post_3 = self::factory()->post->create(
	...
	public function test_child_post_in_hierarchical_post_type_with_default_permalinks() {
		...
		$this->assertStringContainsString( 'contributing-to-the-gechiui-codex/getting-started', get_permalink( $post_2 ) );

		$result = $this->q->query(
			array(
				'handbook'  => 'contributing-to-the-gechiui-codex/getting-started',
				'post_type' => 'handbook',
			)
		);
		$this->assertCount( 1, $result );
