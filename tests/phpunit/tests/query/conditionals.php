 ...
 * then we test the effects of each url on the gc_query object.
 *
 * @group query
 * @group rewrite
 */
class Tests_Query_Conditionals extends GC_UnitTestCase {
	...
	public function test_page_trackback() {
		...
		foreach ( $page_ids as $page_id ) {
			...
			// Make sure the correct GC_Query flags are set.
			$this->assertQueryTrue( 'is_page', 'is_singular', 'is_trackback' );

			// Make sure the correct page was fetched.
			global $gc_query;
			$this->assertSame( $page_id, $gc_query->get_queried_object()->ID );
		}
	}
	...
	public function test_post_type_archive_with_tax_query() {
		...
		gc_set_object_terms( $post_id, $tag_id, 'post_tag' );
	...
	public function test_is_page_with_page_id_zero_and_random_page_slug() {
		...
		$_id                           = $GLOBALS['gc_query']->post->ID;
		$GLOBALS['gc_query']->post->ID = 0;
		...
		// Revert $gc_query global change.
		$GLOBALS['gc_query']->post->ID = $_id;
	}
	...
	public function test_is_page_template_not_singular() {
		...
		if ( ! get_post( $user_id ) ) {
			$post_id = self::factory()->post->create( array( 'post_type' => 'post' ) );
			$gcdb->update( $gcdb->posts, array( 'ID' => $user_id ), array( 'ID' => $post_id ), array( '%d' ) );
		}

		update_post_meta( $user_id, '_gc_page_template', 'example.php' );

		// Verify that the post correctly reports having a template.
		$this->go_to( get_post_permalink( $user_id ) );
		$this->assertInstanceOf( 'GC_Post', get_queried_object() );
		$this->assertTrue( is_page_template( 'example.php' ) );

		// Verify that the non-post with a matching ID does not report having a template.
		$this->go_to( get_author_posts_url( $user_id ) );
		$this->assertInstanceOf( 'GC_User', get_queried_object() );
