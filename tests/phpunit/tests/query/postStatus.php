 ...
 * @group query
 */
class Tests_Query_PostStatus extends GC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$post_ids['gctests_pt1_p1'] = $factory->post->create(
			array(
				'post_type'   => 'gctests_pt1',
				'post_status' => 'private',
				'post_author' => self::$editor_user_id,
			)
		);

		self::$post_ids['gctests_pt1_p2'] = $factory->post->create(
			array(
				'post_type'   => 'gctests_pt1',
				'post_status' => 'publish',
				'post_author' => self::$editor_user_id,
		...
		self::$post_ids['gctests_pt2_p1'] = $factory->post->create(
			array(
				'post_type'   => 'gctests_pt2',
				'post_status' => 'private',
				'post_author' => self::$editor_user_id,
			)
		);

		self::$post_ids['gctests_pt2_p2'] = $factory->post->create(
			array(
				'post_type'   => 'gctests_pt2',
				'post_status' => 'publish',
				'post_author' => self::$editor_user_id,
	 ...
	 * in both the gcSetUpBeforeClass() and setUp() methods.
	 */
	...
	public function test_private_should_be_included_if_perm_is_false() {
		...
		$this->assertSameSets( $expected, gc_list_pluck( $q->posts, 'ID' ) );
	}
	...
	public function test_private_statuses_should_be_included_when_current_user_can_read_private_posts() {
		...
		$this->assertContains( self::$post_ids['author_privatefoo_post'], gc_list_pluck( $q->posts, 'ID' ) );
		$this->assertContains( self::$post_ids['editor_privatefoo_post'], gc_list_pluck( $q->posts, 'ID' ) );
	}
	...
	public function test_private_statuses_should_not_be_included_when_current_user_cannot_read_private_posts() {
		...
		$this->assertContains( self::$post_ids['author_privatefoo_post'], gc_list_pluck( $q->posts, 'ID' ) );
		$this->assertNotContains( self::$post_ids['editor_privatefoo_post'], gc_list_pluck( $q->posts, 'ID' ) );
	}
	...
	public function test_single_post_with_nonpublic_and_protected_status_should_be_shown_for_user_who_can_edit_others_posts() {
		...
		$this->assertSame( array( $p ), gc_list_pluck( $q->posts, 'ID' ) );
	}
	...
	public function test_specific_post_should_be_returned_if_trash_is_one_of_the_requested_post_statuses() {
		...
		$this->assertContains( $p1, gc_list_pluck( $q->posts, 'ID' ) );
	}
	...
	public function test_non_singular_queries_using_post_type_any_should_respect_post_type_read_private_posts_cap() {
		...
		$this->assertSameSets( array( $post_ids['gctests_pt1_p2'], $post_ids['gctests_pt2_p2'] ), gc_list_pluck( $q->posts, 'ID' ) );
		...
		$this->assertSameSets( array( $post_ids['gctests_pt1_p1'], $post_ids['gctests_pt1_p2'], $post_ids['gctests_pt2_p2'] ), gc_list_pluck( $q->posts, 'ID' ) );
	}
	...
	public function test_non_singular_queries_using_multiple_post_type_should_respect_post_type_read_private_posts_cap() {
		...
		$q = new GC_Query(
			array(
				'post_type'      => array( 'gctests_pt1', 'gctests_pt2' ),
				'posts_per_page' => -1,
			)
		);
