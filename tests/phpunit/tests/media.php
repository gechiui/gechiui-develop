 ...
 * @group shortcode
 */
class Tests_Media extends GC_UnitTestCase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		self::$_sizes                          = gc_get_additional_image_sizes();
		$GLOBALS['_gc_additional_image_sizes'] = array();
		...
		gc_trash_post( self::$post_ids['trash'] );
	}

	public static function gcTearDownAfterClass() {
		$GLOBALS['_gc_additional_image_sizes'] = self::$_sizes;
	}

	public static function tear_down_after_class() {
		gc_delete_post( self::$large_id, true );
...
CAP;
		...
		$this->img_url           = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . $this->img_name;
	...
	public function test_img_caption_shortcode_added() {
		...
		$this->assertSame( 'img_caption_shortcode', $shortcode_tags['gc_caption'] );
	}
	...
	public function test_img_caption_shortcode_with_old_format() {
		...
		$this->assertSame( 2, preg_match_all( '/gc-caption/', $result, $_r ) );
	...
	public function test_img_caption_shortcode_with_old_format_id_and_align() {
		...
		$this->assertSame( 1, preg_match_all( '/gc-caption &amp;myAlignment/', $result, $_r ) );
	...
	public function test_img_caption_shortcode_with_old_format_and_class() {
		...
		$this->assertSame( 1, preg_match_all( '/gc-caption alignnone some-class another-class/', $result, $_r ) );
	...
	public function test_new_img_caption_shortcode_new_format() {
		...
		$this->assertSame( 1, preg_match_all( "~{$img_preg}.*gc-caption-text~", $result, $_r ) );
		$this->assertSame( 1, preg_match_all( "~gc-caption-text.*{$content_preg}~", $result, $_r ) );
	}
	...
	public function test_add_remove_oembed_provider() {
		gc_oembed_add_provider( 'http://foo.bar/*', 'http://foo.bar/oembed' );
		$this->assertTrue( gc_oembed_remove_provider( 'http://foo.bar/*' ) );
		$this->assertFalse( gc_oembed_remove_provider( 'http://foo.bar/*' ) );
	}
	...
	public function test_autoembed_empty() {
		...
		$result = $gc_embed->autoembed( $content );
...
https://w.org</a>',
	...
	public function test_autoembed( $content, $result = null ) {
		$gc_embed = new Test_Autoembed;

		$this->assertSame( $gc_embed->autoembed( $content ), $result ? $result : $content );
	}

	public function test_gc_prepare_attachment_for_js() {
		// Attachment without media.
		$id   = gc_insert_attachment(
			array(
				'post_status'           => 'publish',
				'post_title'            => 'Prepare',
		...
		$prepped = gc_prepare_attachment_for_js( $post );
		...
		$this->assertSame( site_url( 'gc-includes/images/media/default.png' ), $prepped['icon'] );
		...
		// Fake a mime without a slash. See #GC22532.
	 ...
	 * @ticket 38965
	 */
	public function test_gc_prepare_attachment_for_js_without_image_sizes() {
		...
		$id = gc_insert_attachment(
			array(
				...
				'guid'           => 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/test-image.jpg',
			)
		);

		// Add attachment metadata without sizes.
		gc_update_attachment_metadata(
			$id,
			array(
				'width'  => 50,
				'height' => 50,
		...
		$prepped = gc_prepare_attachment_for_js( get_post( $id ) );
	...
	/**
	 * @ticket 19067
	 * @expectedDeprecated gc_convert_bytes_to_hr
	 */
	public function test_gc_convert_bytes_to_hr() {
		$kb = 1024;
		$mb = $kb * 1024;
		$gb = $mb * 1024;
		$tb = $gb * 1024;
		...
		$this->assertSame( '1TB', gc_convert_bytes_to_hr( $tb ) );
		$this->assertSame( '1GB', gc_convert_bytes_to_hr( $gb ) );
		$this->assertSame( '1MB', gc_convert_bytes_to_hr( $mb ) );
		$this->assertSame( '1KB', gc_convert_bytes_to_hr( $kb ) );
	...
	public function test_post_galleries_images() {
		...
		foreach ( range( 1, 6 ) as $i ) {
			...
			$ids1[]      = $attachment_id;
			$ids1_srcs[] = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . "image$i.jpg";
		}
		...
		foreach ( range( 4, 6 ) as $i ) {
			...
			gc_update_attachment_metadata( $attachment_id, $metadata );
			$ids2[]      = $attachment_id;
			$ids2_srcs[] = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . "image$i.jpg";
		}
...
BLOB;
	...
	public function test_block_post_gallery_images() {
		...
		foreach ( range( 1, 6 ) as $i ) {
			...
			gc_update_attachment_metadata( $attachment_id, $metadata );
			$ids[]      = $attachment_id;
			$url        = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . "image$i.jpg";
			$ids_srcs[] = $url;
...
BLOB;
	...
	public function test_block_post_gallery_images_json() {
		...
		$blob    = <<<BLOB
<!-- gc:gallery {"ids":[$ids1_joined]} -->
<!-- /gc:gallery -->

<!-- gc:gallery {"ids":[$ids2_joined]} -->
<!-- /gc:gallery -->
BLOB;
		$post_id = self::factory()->post->create( array( 'post_content' => $blob ) );
		$srcs    = get_post_gallery_images( $post_id );
...
BLOB;
	...
	public function test_block_post_gallery_innerblock_images() {
		...
		foreach ( range( 1, 3 ) as $i ) {
			...
			$url        = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . "image$i.jpg";
			$ids_srcs[] = $url;
			$imgs[]     = '<!-- gc:image {"id":' . $attachment_id . ',"sizeSlug":"large","linkDestination":"none"} --><figure class="gc-block-image size-large"><img src="' . $url . '" /></figure><!-- /gc:image -->';
		...
		$blob    = <<<BLOB
<!-- gc:gallery -->
<figure class="gc-block-gallery has-nested-images columns-default is-cropped">
$imgs_joined
</figure>
<!-- /gc:gallery -->
BLOB;
...
VIDEO;
	 ...
	 * @ticket 35367
	 */
	public function test_gc_audio_shortcode_with_empty_params() {
		$this->assertNull( gc_audio_shortcode( array() ) );
	}

	/**
	 * @ticket 35367
	 */
	public function test_gc_audio_shortcode_with_bad_attr() {
		$this->assertSame(
			'<a class="gc-embedded-audio" href="https://example.com/foo.php">https://example.com/foo.php</a>',
			gc_audio_shortcode(
				array(
	 ...
	 * @ticket 35367
	 */
	public function test_gc_audio_shortcode_attributes() {
		$actual = gc_audio_shortcode(
			array(
		...
		$this->assertStringContainsString( 'class="gc-audio-shortcode"', $actual );
	...
	public function test_video_shortcode_body() {
		...
		$video = <<<VIDEO
[video width="$width" height="480" mp4="http://domain.tld/gc-content/uploads/2013/12/xyz.mp4"]
...
VIDEO;
		...
		$expected = '<div style="width: ' . $width . 'px;" class="gc-video">' .
			"<!--[if lt IE 9]><script>document.createElement('video');</script><![endif]-->\n" .
			'<video class="gc-video-shortcode" id="video-' . $post_id . '-1" width="' . $width . '" height="' . $h . '" preload="metadata" controls="controls">' .
			'<source type="video/mp4" src="http://domain.tld/gc-content/uploads/2013/12/xyz.mp4?_=1" />' .
			...
			'<a href="http://domain.tld/gc-content/uploads/2013/12/xyz.mp4">' .
			"http://domain.tld/gc-content/uploads/2013/12/xyz.mp4</a></video></div>\n";
	 ...
	 */
	public function test_gc_video_shortcode_with_empty_params() {
		$this->assertNull( gc_video_shortcode( array() ) );
	}
	 ...
	 * @depends test_video_shortcode_body
	 */
	public function test_gc_video_shortcode_with_bad_attr() {
		$this->assertSame(
			'<a class="gc-embedded-video" href="https://example.com/foo.php">https://example.com/foo.php</a>',
			gc_video_shortcode(
				array(
					'src' => 'https://example.com/foo.php',
				)
	 ...
	 * @depends test_video_shortcode_body
	 */
	public function test_gc_video_shortcode_attributes() {
		$actual = gc_video_shortcode(
			array(
		...
		$this->assertStringContainsString( 'class="gc-video-shortcode"', $actual );
	 ...
	 * @depends test_video_shortcode_body
	 */
	public function test_gc_video_shortcode_youtube_remove_feature() {
		$actual = gc_video_shortcode(
			array(
	 ...
	 * @depends test_video_shortcode_body
	 */
	public function test_gc_video_shortcode_youtube_force_ssl() {
		$actual = gc_video_shortcode(
			array(
	 ...
	 * @depends test_video_shortcode_body
	 */
	public function test_gc_video_shortcode_vimeo_force_ssl_remove_query_args() {
		$actual = gc_video_shortcode(
			array(
	 ...
	 * @depends test_video_shortcode_body
	 */
	public function test_gc_video_shortcode_vimeo_adds_loop() {
		$actual = gc_video_shortcode(
			array(
	 ...
	 * @depends test_video_shortcode_body
	 */
	public function test_gc_video_shortcode_vimeo_force_adds_loop_true() {
		$actual = gc_video_shortcode(
			array(
				'src'  => 'http://vimeo.com/76979871',
				'loop' => true,
	...
	public function test_add_image_size() {
		...
		$this->assertArrayNotHasKey( 'test-size', $_gc_additional_image_sizes );
		add_image_size( 'test-size', 200, 600 );

		$sizes = gc_get_additional_image_sizes();
	...
	public function test_attachment_url_to_postid() {
		...
		$image_url = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . $image_path;
	...
	public function test_attachment_url_to_postid_with_different_scheme() {
		...
		$image_url = 'https://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . $image_path;
		$this->assertSame( $attachment_id, attachment_url_to_postid( $image_url ) );
	}
	...
	public function test_attachment_url_to_postid_should_be_case_sensitive() {
		...
		$image_url = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . $image_path_upper_case;
	...
	public function test_attachment_url_to_postid_filtered() {
		...
		$image_url = 'http://192.168.1.20.com/gc-content/uploads/' . $image_path;
		$this->assertSame( $attachment_id, attachment_url_to_postid( $image_url ) );
		remove_filter( 'upload_dir', array( $this, 'upload_dir' ) );
	}

	public function upload_dir( $dir ) {
		$dir['baseurl'] = 'http://192.168.1.20.com/gc-content/uploads';
		return $dir;
	}
...
EOF;
	 ...
	 * @ticket 33016
	 */
	public function filter_gc_embed_shortcode_custom( $content, $url ) {
	...
	public function test_oembed_explicit_media_link() {
		global $gc_embed;
		add_filter( 'embed_maybe_make_link', array( $this, 'filter_gc_embed_shortcode_custom' ), 10, 2 );
...
EOF;
		...
		remove_filter( 'embed_maybe_make_link', array( $this, 'filter_gc_embed_shortcode_custom' ), 10 );
	}

	/**
	 * Tests the default output of `gc_get_attachment_image()`.
	 *
	 * @ticket 34635
	 */
	public function test_gc_get_attachment_image_defaults() {
		...
		$this->assertSame( $expected, gc_get_attachment_image( self::$large_id ) );
	}

	/**
	 * @ticket 50801
	 */
	public function test_gc_get_attachment_image_filter_output() {
		$image    = image_downsize( self::$large_id, 'thumbnail' );
		$expected = 'Override gc_get_attachment_image';

		add_filter( 'gc_get_attachment_image', array( $this, 'filter_gc_get_attachment_image' ) );
		$output = gc_get_attachment_image( self::$large_id );
		remove_filter( 'gc_get_attachment_image', array( $this, 'filter_gc_get_attachment_image' ) );

		$this->assertSame( $expected, $output );
	}

	public function filter_gc_get_attachment_image() {
		return 'Override gc_get_attachment_image';
	}

	/**
	 * Test that `gc_get_attachment_image()` returns a proper alt value.
	 *
	 * @ticket 34635
	 */
	public function test_gc_get_attachment_image_with_alt() {
		// Add test alt metadata.
		update_post_meta( self::$large_id, '_gc_attachment_image_alt', 'Some very clever alt text', true );
		...
		// Cleanup.
		update_post_meta( self::$large_id, '_gc_attachment_image_alt', '', true );
	}

	/**
	 * @ticket 33878
	 */
	public function test_gc_get_attachment_image_url() {
		$this->assertFalse( gc_get_attachment_image_url( 0 ) );
		...
		$image = gc_get_attachment_image_src( $attachment_id, 'thumbnail', false );

		$this->assertSame( $image[0], gc_get_attachment_image_url( $attachment_id ) );
	}

	/**
	 * @ticket 12235
	 */
	public function test_gc_get_attachment_caption() {
		$this->assertFalse( gc_get_attachment_caption( 0 ) );
		...
		$this->assertFalse( gc_get_attachment_caption( $post_id ) );

		$this->assertSame( $caption, gc_get_attachment_caption( $attachment_id ) );
	}

	/**
	 * @ticket 12235
	 */
	public function test_gc_get_attachment_caption_empty() {
		$post_id       = self::factory()->post->create();
		$attachment_id = self::factory()->attachment->create_object(
			$this->img_name,
			$post_id,
		...
		$this->assertSame( '', gc_get_attachment_caption( $attachment_id ) );
	}
	 ...
	 */
	public function test_gc_calculate_image_srcset() {
		$_gc_additional_image_sizes = gc_get_additional_image_sizes();

		$year_month      = gmdate( 'Y/m' );
		$image_meta      = gc_get_attachment_metadata( self::$large_id );
		$uploads_dir_url = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/';
		...
		foreach ( $_gc_additional_image_sizes as $name => $additional_size ) {
			if ( ! $_gc_additional_image_sizes[ $name ]['crop'] || 0 === $_gc_additional_image_sizes[ $name ]['height'] ) {
				$intermediates[] = $name;
			}
		}
		...
		foreach ( $intermediates as $int ) {
			$image_url  = gc_get_attachment_image_url( self::$large_id, $int );
			...
			$this->assertSame( $expected_srcset, gc_calculate_image_srcset( $size_array, $image_url, $image_meta ) );
		}
	}
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_calculate_image_srcset_no_date_uploads() {
		$_gc_additional_image_sizes = gc_get_additional_image_sizes();
		...
		$image_meta      = gc_get_attachment_metadata( $id );
		$uploads_dir_url = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/';
		...
		foreach ( $intermediates as $int ) {
			$size_array = $this->get_image_size_array_from_meta( $image_meta, $int );
			$image_url  = gc_get_attachment_image_url( $id, $int );
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_calculate_image_srcset_with_edits() {
		// For this test we're going to mock metadata changes from an edit.
		// Start by getting the attachment metadata.
		$image_meta = gc_get_attachment_metadata( self::$large_id );
		$image_url  = gc_get_attachment_image_url( self::$large_id, 'medium' );
		$size_array = $this->get_image_size_array_from_meta( $image_meta, 'medium' );

		// Copy hash generation method used in gc_save_image().
		$hash = 'e' . time() . rand( 100, 999 );

		$filename_base = gc_basename( self::$large_filename, '.jpg' );
		...
		$sizes = explode( ', ', gc_calculate_image_srcset( $size_array, $image_url, $image_meta ) );
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_calculate_image_srcset_with_absolute_path_in_meta() {
		$_gc_additional_image_sizes = gc_get_additional_image_sizes();
		...
		$image_meta['file'] = 'H:\home\gechiui\trunk/gc-content/uploads/' . $image_meta['file'];
	 ...
	 * @ticket 33641
	 */
	public function test_gc_calculate_image_srcset_false() {
		$sizes = gc_calculate_image_srcset( array( 400, 300 ), 'file.png', array() );
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_calculate_image_srcset_no_width() {
		$file       = get_attached_file( self::$large_id );
		$image_url  = gc_get_attachment_image_url( self::$large_id, 'medium' );
		$image_meta = gc_generate_attachment_metadata( self::$large_id, $file );

		$size_array = array( 0, 0 );

		$srcset = gc_calculate_image_srcset( $size_array, $image_url, $image_meta );
	 ...
	 * @ticket 33641
	 */
	public function test_gc_calculate_image_srcset_ratio_variance() {
		// Mock data for this test.
		$size_array = array( 218, 300 );
		$image_src  = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/2015/12/test-768x1055-218x300.png';
		$image_meta = array(
			'width'  => 768,
			'height' => 1055,
		...
		$uploads_url = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/2015/12/';
		...
		$this->assertSame( $expected_srcset, gc_calculate_image_srcset( $size_array, $image_src, $image_meta ) );
	}
	 ...
	 * @ticket 33641
	 */
	public function test_gc_calculate_image_srcset_include_src() {
		// Mock data for this test.
		$size_array = array( 2000, 1000 );
		$image_src  = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/2015/12/test.png';
		$image_meta = array(
			'width'  => 2000,
			'height' => 1000,
	 ...
	 * @ticket 35480
	 */
	public function test_gc_calculate_image_srcset_corrupted_image_meta() {
		$size_array = array( 300, 150 );
		$image_src  = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/2015/12/test-300x150.png';
		$image_meta = array(
			'width'  => 1600,
			'height' => 800,
			'file'   => '2015/12/test.png',
		...
		$srcset = array(
			300  => 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/2015/12/test-300x150.png 300w',
			768  => 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/2015/12/test-768x384.png 768w',
			1024 => 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/2015/12/test-1024x512.png 1024w',
			1600 => 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/2015/12/test.png 1600w',
		);

		// No sizes array.
		$image_meta1 = $image_meta;
		unset( $image_meta1['sizes'] );
		$this->assertFalse( gc_calculate_image_srcset( $size_array, $image_src, $image_meta1 ) );

		// Sizes is string instead of array; only full size available means no srcset.
		$image_meta2          = $image_meta;
		$image_meta2['sizes'] = '';
		$this->assertFalse( gc_calculate_image_srcset( $size_array, $image_src, $image_meta2 ) );

		// File name is incorrect.
		$image_meta3         = $image_meta;
		$image_meta3['file'] = '/';
		$this->assertFalse( gc_calculate_image_srcset( $size_array, $image_src, $image_meta3 ) );

		// File name is incorrect.
		$image_meta4 = $image_meta;
		unset( $image_meta4['file'] );
		$this->assertFalse( gc_calculate_image_srcset( $size_array, $image_src, $image_meta4 ) );
		...
		$this->assertSame( $expected_srcset, gc_calculate_image_srcset( $size_array, $image_src, $image_meta5 ) );
	}
	 ...
	 * @ticket 33641
	 */
	public function test_gc_calculate_image_srcset_with_spaces_in_filenames() {
		// Mock data for this test.
		$image_src  = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/2015/12/test image-300x150.png';
		$image_meta = array(
			'width'  => 3000,
			'height' => 1500,
		...
		$this->assertSame( $expected_srcset, gc_calculate_image_srcset( array( 300, 150 ), $image_src, $image_meta ) );
	}
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_get_attachment_image_srcset() {
		$_gc_additional_image_sizes = gc_get_additional_image_sizes();

		$image_meta = gc_get_attachment_metadata( self::$large_id );
		$size_array = array( $image_meta['width'], $image_meta['height'] ); // Full size.

		$srcset = gc_get_attachment_image_srcset( self::$large_id, $size_array, $image_meta );

		$year_month  = gmdate( 'Y/m' );
		$uploads_dir = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/';
	 ...
	 * @ticket 33641
	 */
	public function test_gc_get_attachment_image_srcset_single_srcset() {
		$image_meta = gc_get_attachment_metadata( self::$large_id );
		 ...
		 */
		$sizes = gc_get_attachment_image_srcset( self::$large_id, $size_array, $image_meta );
	 ...
	 * @ticket 33641
	 */
	public function test_gc_get_attachment_image_srcset_invalidsize() {
		$image_meta    = gc_get_attachment_metadata( self::$large_id );
		$invalid_size  = 'nailthumb';
		$original_size = array( 1600, 1200 );

		$srcset = gc_get_attachment_image_srcset( self::$large_id, $invalid_size, $image_meta );

		// Expect a srcset for the original full size image to be returned.
		$expected = gc_get_attachment_image_srcset( self::$large_id, $original_size, $image_meta );
	 ...
	 * @ticket 33641
	 */
	public function test_gc_get_attachment_image_sizes() {
		// Test sizes against the default GC sizes.
		$intermediates = array( 'thumbnail', 'medium', 'medium_large', 'large' );

		// Make sure themes aren't filtering the sizes array.
		remove_all_filters( 'gc_calculate_image_sizes' );

		foreach ( $intermediates as $int_size ) {
			$image = gc_get_attachment_image_src( self::$large_id, $int_size );

			$expected = '(max-width: ' . $image[1] . 'px) 100vw, ' . $image[1] . 'px';
			$sizes    = gc_get_attachment_image_sizes( self::$large_id, $int_size );
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_calculate_image_sizes() {
		// Test sizes against the default GC sizes.
		$intermediates = array( 'thumbnail', 'medium', 'medium_large', 'large' );
		$image_meta    = gc_get_attachment_metadata( self::$large_id );
		...
		foreach ( $intermediates as $int_size ) {
			...
			$sizes    = gc_calculate_image_sizes( $size_array, $image_src, $image_meta );
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_filter_content_tags_srcset_sizes() {
		$image_meta = gc_get_attachment_metadata( self::$large_id );
		$size_array = $this->get_image_size_array_from_meta( $image_meta, 'medium' );

		$srcset = sprintf( 'srcset="%s"', gc_get_attachment_image_srcset( self::$large_id, $size_array, $image_meta ) );
		$sizes  = sprintf( 'sizes="%s"', gc_get_attachment_image_sizes( self::$large_id, $size_array, $image_meta ) );
		...
		$img_no_size_id       = str_replace( 'gc-image-', 'id-', $img );
		...
		add_filter( 'gc_img_tag_add_width_and_height_attr', '__return_false' );
		add_filter( 'gc_img_tag_add_loading_attr', '__return_false' );

		$this->assertSame( $content_filtered, gc_filter_content_tags( $content_unfiltered ) );

		remove_filter( 'gc_img_tag_add_width_and_height_attr', '__return_false' );
		remove_filter( 'gc_img_tag_add_loading_attr', '__return_false' );
	}

	/**
	 * When rendering attributes for responsive images,
	 * we rely on the 'gc-image-*' class to find the image by ID.
	 ...
	 * @ticket 33641
	 */
	public function test_gc_filter_content_tags_srcset_sizes_wrong() {
		$img = get_image_tag( self::$large_id, '', '', '', 'medium' );
		$img = gc_img_tag_add_loading_attr( $img, 'test' );

		// Replace the src URL.
		$image_wrong_src = preg_replace( '|src="[^"]+"|', 'src="http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/foo.jpg"', $img );

		$this->assertSame( $image_wrong_src, gc_filter_content_tags( $image_wrong_src ) );
	}

	/**
	 * @ticket 33641
	 */
	public function test_gc_filter_content_tags_srcset_sizes_with_preexisting_srcset() {
		...
		$this->assertSame( $img, gc_filter_content_tags( $img ) );
	}
	 ...
	 * @ticket 34528
	 */
	public function test_gc_calculate_image_srcset_animated_gifs() {
		...
		$full_src  = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . $image_meta['file'];
		$large_src = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . $image_meta['sizes']['large']['file'];

		// Test with soft resized size array.
		$size_array = array( 900, 450 );

		// Full size GIFs should not return a srcset.
		$this->assertFalse( gc_calculate_image_srcset( $size_array, $full_src, $image_meta ) );
		// Intermediate sized GIFs should not include the full size in the srcset.
		$this->assertStringNotContainsString( $full_src, gc_calculate_image_srcset( $size_array, $large_src, $image_meta ) );
	}
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_filter_content_tags_schemes() {
		$image_meta = gc_get_attachment_metadata( self::$large_id );
		$size_array = $this->get_image_size_array_from_meta( $image_meta, 'medium' );
		...
		$actual = gc_filter_content_tags( $unfiltered );
	 ...
	 * @ticket 33641
	 */
	public function test_gc_get_attachment_image_with_https_on() {
		...
		$image_url  = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . $image_meta['sizes']['large']['file'];

		$_SERVER['HTTPS'] = 'on';

		$uploads_url = 'https://' . GC_TESTS_DOMAIN . '/gc-content/uploads/';

		$expected = $uploads_url . 'test-1024x512.jpg 1024w, ' .
			$uploads_url . 'test-300x150.jpg 300w, ' .
			$uploads_url . 'test.jpg 1200w';

		$actual = gc_calculate_image_srcset( $size_array, $image_url, $image_meta );
	...
	public function test_get_image_send_to_editor_defaults() {
		...
		$attachment = gc_get_attachment_image_src( $id, 'medium' );

		$html     = '<img src="%1$s" alt="" width="%2$d" height="%3$d" class="align%4$s size-medium gc-image-%5$d" />';
	...
	public function test_get_image_send_to_editor_defaults_with_optional_params() {
		...
		$attachment = gc_get_attachment_image_src( $id, $size );

		$html = '<a href="%1$s" rel="%2$s"><img src="%3$s" alt="%4$s" width="%5$d" height="%6$d" class="size-%8$s gc-image-%9$d" /></a>';
		$html = '[caption id="attachment_%9$d" align="align%7$s" width="%5$d"]' . $html . ' %10$s[/caption]';

		$expected = sprintf( $html, $url, 'attachment gc-att-' . $id, $attachment[0], $alt, $attachment[1], $attachment[2], $align, $size, $id, $caption );
	...
	public function test_get_image_send_to_editor_defaults_no_caption_no_rel() {
		...
		$html = '<a href="%1$s"><img src="%2$s" alt="%3$s" width="%4$d" height="%5$d" class="align%6$s size-%7$s gc-image-%8$d" /></a>';
	...
	/**
	 * Tests if gc_get_attachment_image() uses gc_get_attachment_metadata().
	 *
	 * In this way, the meta data can be filtered using the filter
	 * `gc_get_attachment_metadata`.
	 *
	 * The test checks if the image size that is added in the filter is
	 * used in the output of `gc_get_attachment_image()`.
	 *
	 * @ticket 36246
	 * @requires function imagejpeg
	 */
	public function test_gc_get_attachment_image_should_use_gc_get_attachment_metadata() {
		add_filter( 'gc_get_attachment_metadata', array( $this, 'filter_36246' ), 10, 2 );

		remove_all_filters( 'gc_calculate_image_sizes' );

		$basename    = gc_basename( self::$large_filename, '.jpg' );
		$year_month  = gmdate( 'Y/m' );
		$uploads_url = 'http://' . GC_TESTS_DOMAIN . '/gc-content/uploads/' . $year_month . '/';
		...
		$actual = gc_get_attachment_image( self::$large_id, 'testsize' );

		remove_filter( 'gc_get_attachment_metadata', array( $this, 'filter_36246' ) );
	 ...
	 * @ticket 50679
	 */
	public function test_gc_get_attachment_metadata_should_return_false_if_no_attachment() {
		$post_id = self::factory()->post->create();
		$data    = gc_get_attachment_metadata( $post_id );
	 ...
	 * @ticket 35218
	 */
	public function test_gc_get_media_creation_timestamp_video_asf() {
		$metadata = array(
			'fileformat' => 'asf',
			'asf'        => array(
		...
		$this->assertSame( 123, gc_get_media_creation_timestamp( $metadata ) );
	}

	/**
	 * @ticket 35218
	 */
	public function test_gc_get_media_creation_timestamp_video_matroska() {
		$metadata = array(
			'fileformat' => 'matroska',
			'matroska'   => array(
				'comments' => array(
		...
		$this->assertSame( 1450978809, gc_get_media_creation_timestamp( $metadata ) );
	}

	/**
	 * @ticket 35218
	 */
	public function test_gc_get_media_creation_timestamp_video_quicktime() {
		$metadata = array(
		...
		$this->assertSame( 1450978805, gc_get_media_creation_timestamp( $metadata ) );
	}

	/**
	 * @ticket 35218
	 */
	public function test_gc_get_media_creation_timestamp_video_webm() {
		$metadata = array(
			'fileformat' => 'webm',
			'matroska'   => array(
				'info' => array(
		...
		$this->assertSame( 1265680539, gc_get_media_creation_timestamp( $metadata ) );
	}
	 ...
	 *`gc_read_audio_metadata()`.
	 *
	 * @ticket 42017
	 */
	public function test_gc_read_audio_metadata_adds_creation_date_with_mp4() {
		$video    = DIR_TESTDATA . '/uploads/small-video.mp4';
		$metadata = gc_read_audio_metadata( $video );
	 ...
	 * @ticket 35218
	 */
	public function test_gc_read_video_metadata_adds_creation_date_with_quicktime() {
		$video    = DIR_TESTDATA . '/uploads/small-video.mov';
		$metadata = gc_read_video_metadata( $video );
	 ...
	 * @ticket 35218
	 */
	public function test_gc_read_video_metadata_adds_creation_date_with_mp4() {
		$video    = DIR_TESTDATA . '/uploads/small-video.mp4';
		$metadata = gc_read_video_metadata( $video );
	 ...
	 * @ticket 35218
	 */
	public function test_gc_read_video_metadata_adds_creation_date_with_mkv() {
	 ...
	 * @ticket 35218
	 */
	public function test_gc_read_video_metadata_adds_creation_date_with_webm() {
	...
	public function test_media_handle_upload_uses_post_parent_for_directory_date() {
		...
		$url = gc_get_attachment_url( $post_id );

		$uploads_dir = gc_upload_dir( '2010/01' );

		$expected = $uploads_dir['url'] . '/test-image-iptc.jpg';

		// Clean up.
		gc_delete_attachment( $post_id );
		gc_delete_post( $parent_id );
	...
	public function test_media_handle_upload_ignores_page_parent_for_directory_date() {
		...
		$uploads_dir = gc_upload_dir( current_time( 'mysql' ) );
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_filter_content_tags_width_height() {
		$image_meta = gc_get_attachment_metadata( self::$large_id );
		$size_array = $this->get_image_size_array_from_meta( $image_meta, 'medium' );
		...
		add_filter( 'gc_img_tag_add_loading_attr', '__return_false' );
		add_filter( 'gc_img_tag_add_srcset_and_sizes_attr', '__return_false' );

		$this->assertSame( $content_filtered, gc_filter_content_tags( $content_unfiltered ) );

		remove_filter( 'gc_img_tag_add_loading_attr', '__return_false' );
		remove_filter( 'gc_img_tag_add_srcset_and_sizes_attr', '__return_false' );
	}
	 ...
	 * @requires function imagejpeg
	 */
	public function test_gc_filter_content_tags_loading_lazy() {
		$image_meta = gc_get_attachment_metadata( self::$large_id );
		$size_array = $this->get_image_size_array_from_meta( $image_meta, 'medium' );
		...
		$lazy_img       = gc_img_tag_add_loading_attr( $img, 'test' );
		$lazy_img_xhtml = gc_img_tag_add_loading_attr( $img_xhtml, 'test' );
		$lazy_img_html5 = gc_img_tag_add_loading_attr( $img_html5, 'test' );
		$lazy_iframe    = gc_iframe_tag_add_loading_attr( $iframe, 'test' );
	 ...
	 * @ticket 50756
	 */
	public function test_gc_filter_content_tags_loading_lazy_opted_in() {
		$img         = get_image_tag( self::$large_id, '', '', '', 'medium' );
		$lazy_img    = gc_img_tag_add_loading_attr( $img, 'test' );
		$iframe      = '<iframe src="https://www.example.com" width="640" height="360"></iframe>';
		$lazy_iframe = gc_iframe_tag_add_loading_attr( $iframe, 'test' );
		...
		add_filter( 'gc_lazy_loading_enabled', '__return_true' );

		$this->assertSame( $content_filtered, gc_filter_content_tags( $content_unfiltered ) );
		remove_filter( 'gc_lazy_loading_enabled', '__return_true' );
		remove_filter( 'gc_img_tag_add_srcset_and_sizes_attr', '__return_false' );
	}
	 ...
	 * @ticket 50756
	 */
	public function test_gc_filter_content_tags_loading_lazy_opted_out() {
		$img    = get_image_tag( self::$large_id, '', '', '', 'medium' );
		$iframe = '<iframe src="https://www.example.com" width="640" height="360"></iframe>';
		...
		add_filter( 'gc_lazy_loading_enabled', '__return_false' );

		$this->assertSame( $content, gc_filter_content_tags( $content ) );
		remove_filter( 'gc_lazy_loading_enabled', '__return_false' );
		remove_filter( 'gc_img_tag_add_srcset_and_sizes_attr', '__return_false' );
	}
	 ...
	 * @ticket 50367
	 */
	public function test_gc_img_tag_add_loading_attr() {
	 ...
	 * @ticket 50367
	 */
	public function test_gc_img_tag_add_loading_attr_without_src() {
	 ...
	 * @ticket 50367
	 */
	public function test_gc_img_tag_add_loading_attr_with_single_quotes() {
		$img = "<img src='example.png' alt=' width='300' height='225' />";
		$img = gc_img_tag_add_loading_attr( $img, 'test' );
	 ...
	 * @ticket 50425
	 */
	public function test_gc_img_tag_add_loading_attr_opt_out() {
		$img = '<img src="example.png" alt=" width="300" height="225" />';
		add_filter( 'gc_img_tag_add_loading_attr', '__return_false' );
	 ...
	 * @ticket 50756
	 */
	public function test_gc_iframe_tag_add_loading_attr() {
		$iframe = '<iframe src="https://www.example.com" width="640" height="360"></iframe>';
		$iframe = gc_iframe_tag_add_loading_attr( $iframe, 'test' );
	 ...
	 * @ticket 50756
	 */
	public function test_gc_iframe_tag_add_loading_attr_without_src() {
	 ...
	 * @ticket 50756
	 */
	public function test_gc_iframe_tag_add_loading_attr_with_single_quotes() {
		$iframe = "<iframe src='https://www.example.com' width='640' height='360'></iframe>";
		$iframe = gc_iframe_tag_add_loading_attr( $iframe, 'test' );
	 ...
	 * @ticket 50756
	 */
	public function test_gc_iframe_tag_add_loading_attr_opt_out() {
		$iframe = '<iframe src="https://www.example.com" width="640" height="360"></iframe>';
		add_filter( 'gc_iframe_tag_add_loading_attr', '__return_false' );
		$iframe = gc_iframe_tag_add_loading_attr( $iframe, 'test' );
	 ...
	 * @ticket 52768
	 */
	public function test_gc_iframe_tag_add_loading_attr_skip_gc_embed() {
		$iframe   = '<iframe src="https://www.example.com" width="640" height="360"></iframe>';
		$fallback = '<blockquote>Fallback content.</blockquote>';
		$iframe   = gc_filter_oembed_result( $fallback . $iframe, (object) array( 'type' => 'rich' ), 'https://www.example.com' );
		$iframe   = gc_iframe_tag_add_loading_attr( $iframe, 'test' );
	 ...
	 * @ticket 50425
	 */
	public function test_gc_get_attachment_image_loading() {
		$img = gc_get_attachment_image( self::$large_id );
	 ...
	 * @ticket 50425
	 */
	public function test_gc_get_attachment_image_loading_opt_out() {
		add_filter( 'gc_lazy_loading_enabled', '__return_false' );
		$img = gc_get_attachment_image( self::$large_id );
	 ...
	 * @ticket 50425
	 */
	public function test_gc_get_attachment_image_loading_opt_out_individual() {
		// The default is already tested above, the filter below ensures that
		// lazy-loading is definitely enabled globally for images.
		add_filter( 'gc_lazy_loading_enabled', '__return_true' );

		$img = gc_get_attachment_image( self::$large_id, 'thumbnail', false, array( 'loading' => false ) );
	 ...
	 * @ticket 50425
	 * @ticket 50756
	 * @dataProvider data_gc_lazy_loading_enabled_tag_name_defaults
	 *
	 * @param string $tag_name Tag name.
	 * @param bool   $expected Expected return value.
	 */
	public function test_gc_lazy_loading_enabled_tag_name_defaults( $tag_name, $expected ) {
		if ( $expected ) {
			$this->assertTrue( gc_lazy_loading_enabled( $tag_name, 'the_content' ) );
		} else {
			$this->assertFalse( gc_lazy_loading_enabled( $tag_name, 'the_content' ) );
		}
	}

	public function data_gc_lazy_loading_enabled_tag_name_defaults() {
		return array(
	 ...
	 * @ticket 53463
	 * @ticket 53675
	 * @dataProvider data_gc_lazy_loading_enabled_context_defaults
	 *
	 * @param string $context  Function context.
	 * @param bool   $expected Expected return value.
	 */
	public function test_gc_lazy_loading_enabled_context_defaults( $context, $expected ) {
		if ( $expected ) {
			$this->assertTrue( gc_lazy_loading_enabled( 'img', $context ) );
		} else {
			$this->assertFalse( gc_lazy_loading_enabled( 'img', $context ) );
		}
	}

	public function data_gc_lazy_loading_enabled_context_defaults() {
		return array(
			'gc_get_attachment_image => true' => array( 'gc_get_attachment_image', true ),
	 ...
	 * @ticket 50543
	 */
	public function test_gc_image_file_matches_image_meta() {
		$image_meta       = gc_get_attachment_metadata( self::$large_id );
		$image_src_full   = gc_get_attachment_image_url( self::$large_id, 'full' );
		$image_src_medium = gc_get_attachment_image_url( self::$large_id, 'medium' );

		$this->assertTrue( gc_image_file_matches_image_meta( $image_src_full, $image_meta ) );
		$this->assertTrue( gc_image_file_matches_image_meta( $image_src_medium, $image_meta ) );
	}

	/**
	 * @ticket 50543
	 */
	public function test_gc_image_file_matches_image_meta_no_subsizes() {
		$image_meta = gc_get_attachment_metadata( self::$large_id );
		$image_src  = gc_get_attachment_image_url( self::$large_id, 'full' );

		$image_meta['sizes'] = array();

		$this->assertTrue( gc_image_file_matches_image_meta( $image_src, $image_meta ) );
	}

	/**
	 * @ticket 50543
	 */
	public function test_gc_image_file_matches_image_meta_invalid_meta() {
		$image_meta = ''; // Attachment is not an image.
		$image_src  = $this->img_url;

		$this->assertFalse( gc_image_file_matches_image_meta( $image_src, $image_meta ) );
	}

	/**
	 * @ticket 50543
	 */
	public function test_gc_image_file_matches_image_meta_different_meta() {
		$image_meta = gc_get_attachment_metadata( self::$large_id );
	 ...
	 * @ticket 50543
	 */
	public function test_gc_image_file_matches_image_meta_original_image() {
		$image_meta = gc_get_attachment_metadata( self::$large_id );
		$image_src  = gc_get_original_image_url( self::$large_id );
	...
	/**
	 * @ticket 53675
	 * @dataProvider data_gc_get_loading_attr_default
	 *
	 * @param string $context
	 */
	function test_gc_get_loading_attr_default( $context ) {
		global $gc_query, $gc_the_query;

		// Return 'lazy' by default.
		$this->assertSame( 'lazy', gc_get_loading_attr_default( 'test' ) );
		$this->assertSame( 'lazy', gc_get_loading_attr_default( 'gc_get_attachment_image' ) );

		// Return 'lazy' if not in the loop or the main query.
		$this->assertSame( 'lazy', gc_get_loading_attr_default( $context ) );

		$gc_query = new GC_Query( array( 'post__in' => array( self::$post_ids['publish'] ) ) );
		...
		while ( have_posts() ) {
			...
			$this->assertFalse( gc_get_loading_attr_default( $context ) );
	...
	function data_gc_get_loading_attr_default() {
		return array(
			array( 'the_content' ),
	 ...
	 * @ticket 53675
	 */
	function test_gc_omit_loading_attr_threshold_filter() {
		global $gc_query, $gc_the_query;
		...
		add_filter(
			'gc_omit_loading_attr_threshold',
			function() {
				return 3;
			}
		);
		...
		while ( have_posts() ) {
			...
			for ( $i = 0; $i < 3; $i++ ) {
				$this->assertFalse( gc_get_loading_attr_default( 'the_content' ) );
			}

			// For following elements, lazy-load them again.
			$this->assertSame( 'lazy', gc_get_loading_attr_default( 'the_content' ) );
		}
	}
	 ...
	 * @ticket 53675
	 */
	function test_gc_filter_content_tags_with_gc_get_loading_attr_default() {
		global $gc_query, $gc_the_query;
		...
		$iframe2      = '<iframe src="https://www.gechiui.com" width="640" height="360"></iframe>';
		$lazy_img2    = gc_img_tag_add_loading_attr( $img2, 'the_content' );
		$lazy_img3    = gc_img_tag_add_loading_attr( $img3, 'the_content' );
		$lazy_iframe2 = gc_iframe_tag_add_loading_attr( $iframe2, 'the_content' );
		...
		while ( have_posts() ) {
			...
			add_filter( 'gc_img_tag_add_srcset_and_sizes_attr', '__return_false' );
			$content_filtered = gc_filter_content_tags( $content_unfiltered, 'the_content' );
			remove_filter( 'gc_img_tag_add_srcset_and_sizes_attr', '__return_false' );
		}
	 ...
	 * @ticket 53675
	 */
	public function test_gc_omit_loading_attr_threshold() {
		$this->reset_omit_loading_attr_filter();

		// Apply filter, ensure default value of 1.
		$omit_threshold = gc_omit_loading_attr_threshold();
 ...
 */
class Test_Autoembed extends GC_Embed {
