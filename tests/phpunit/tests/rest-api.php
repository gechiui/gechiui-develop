...
require_once ABSPATH . GCINC . '/rest-api.php';
require_once __DIR__ . '/../includes/class-jsonserializable-object.php';

/**
 * @group restapi
 */
class Tests_REST_API extends GC_UnitTestCase {
	public function set_up() {
		parent::set_up();

		// Override the normal server with our spying server.
		$GLOBALS['gc_rest_server'] = new Spy_REST_Server();
		do_action( 'rest_api_init', $GLOBALS['gc_rest_server'] );
	}

	public function tear_down() {
		remove_filter( 'gc_rest_server_class', array( $this, 'filter_gc_rest_server_class' ) );
		parent::tear_down();
	}
	...
	public function test_rest_api_active() {
		$this->assertTrue( class_exists( 'GC_REST_Server' ) );
		$this->assertTrue( class_exists( 'GC_REST_Request' ) );
		$this->assertTrue( class_exists( 'GC_REST_Response' ) );
		$this->assertTrue( class_exists( 'GC_REST_Posts_Controller' ) );
	}
	...
	public function test_add_extra_api_taxonomy_arguments() {
		...
		$this->assertSame( 'GC_REST_Terms_Controller', $taxonomy->rest_controller_class );
	...
	public function test_add_extra_api_post_type_arguments() {
		...
		$this->assertSame( 'GC_REST_Posts_Controller', $post_type->rest_controller_class );
		...
		$this->assertSame( 'GC_REST_Attachments_Controller', $post_type->rest_controller_class );
	}
	...
	public function test_route_canonicalized() {
		...
		$endpoints = $GLOBALS['gc_rest_server']->get_raw_endpoint_data();
		...
		$filtered_endpoints = $GLOBALS['gc_rest_server']->get_routes();
	...
	public function test_route_merge() {
		...
		$endpoints = $GLOBALS['gc_rest_server']->get_routes();
		$endpoint  = $endpoints['/test-ns/test'];
		$this->assertCount( 2, $endpoint );
	}
	...
	public function test_rest_route_query_var() {
		rest_api_init();
		$this->assertContains( 'rest_route', $GLOBALS['gc']->public_query_vars );
	}
	...
	public function test_route_method() {
		...
		$routes = $GLOBALS['gc_rest_server']->get_routes();
	...
	public function test_options_request() {
		...
		$request  = new GC_REST_Request( 'OPTIONS', '/test-ns/test' );
		$response = rest_handle_options_request( null, $GLOBALS['gc_rest_server'], $request );
		$response = rest_send_allow_header( $response, $GLOBALS['gc_rest_server'], $request );
	...
	public function test_options_request_not_options() {
		...
		$request  = new GC_REST_Request( 'GET', '/test-ns/test' );
		$response = rest_handle_options_request( null, $GLOBALS['gc_rest_server'], $request );
	...
	public function test_rest_url_generation() {
		// In pretty permalinks case, we expect a path of gc-json/ with no query.
		$this->set_permalink_structure( '/%year%/%monthnum%/%day%/%postname%/' );
		$this->assertSame( 'http://' . GC_TESTS_DOMAIN . '/gc-json/', get_rest_url() );

		// In index permalinks case, we expect a path of index.php/gc-json/ with no query.
		$this->set_permalink_structure( '/index.php/%year%/%monthnum%/%day%/%postname%/' );
		$this->assertSame( 'http://' . GC_TESTS_DOMAIN . '/index.php/gc-json/', get_rest_url() );

		// In non-pretty case, we get a query string to invoke the rest router.
		$this->set_permalink_structure( '' );
		$this->assertSame( 'http://' . GC_TESTS_DOMAIN . '/index.php?rest_route=/', get_rest_url() );
	}
	...
	public function test_always_prepend_path_with_slash_in_rest_url_filter() {
		...
		get_rest_url( null, 'gc/media/' );
		$args = $filter->get_args();
		$this->assertSame( '/gc/media/', $args[0][1] );
		$filter->reset();
	...
	public function test_jsonp_callback_check( $callback, $valid ) {
		$this->assertSame( $valid, gc_check_jsonp_callback( $callback ) );
	}
	...
	public function filter_gc_rest_server_class( $class_name ) {
		return 'Spy_REST_Server';
	}

	public function test_register_rest_route_without_server() {
		$GLOBALS['gc_rest_server'] = null;
		add_filter( 'gc_rest_server_class', array( $this, 'filter_gc_rest_server_class' ) );
	...
	public function test_rest_preload_api_request_with_method() {
		$rest_server               = $GLOBALS['gc_rest_server'];
		$GLOBALS['gc_rest_server'] = null;
		...
		$this->assertSame( array_keys( $preload_data ), array( '/gc/v2/types', 'OPTIONS' ) );
		$this->assertArrayHasKey( '/gc/v2/media', $preload_data['OPTIONS'] );

		$GLOBALS['gc_rest_server'] = $rest_server;
	}
	...
	public function test_rest_ensure_request_accepts_path_string() {
		$request = rest_ensure_request( '/gc/v2/posts' );
		$this->assertInstanceOf( 'GC_REST_Request', $request );
		$this->assertSame( '/gc/v2/posts', $request->get_route() );
	...
	public function data_rest_parse_embed_param() {
		return array(
			...
			array( array( 'author', 'replies' ), 'author,replies ' ),
			array( array( 'gc:term' ), 'gc:term' ),
			array( array( 'gc:term', 'gc:attachment' ), 'gc:term,gc:attachment' ),
	...
	public function test_rest_ensure_response_accepts_gc_error_and_returns_gc_error() {
		$response = rest_ensure_response( new GC_Error() );
	 ...
	 */
	public function test_rest_ensure_response_returns_instance_of_gc_rest_response( $response, $expected_data ) {
		$response_object = rest_ensure_response( $response );
		$this->assertInstanceOf( 'GC_REST_Response', $response_object );
		$this->assertSame( $expected_data, $response_object->get_data() );
	}

	/**
	 * Data provider for test_rest_ensure_response_returns_instance_of_gc_rest_response().
	 *
	 * @return array
	 */
	public function rest_ensure_response_data_provider() {
		return array(
			...
			array( new GC_HTTP_Response( 'http' ), 'http' ),
			array( new GC_REST_Response( 'rest' ), 'rest' ),
		);
	}
	...
	public function test_rest_get_route_for_post_custom_namespace() {
		register_post_type(
			...
			array(
				'show_in_rest'   => true,
				'rest_base'      => 'cpt',
				'rest_namespace' => 'gechiui/v1',
			)
		);
		$post = self::factory()->post->create_and_get( array( 'post_type' => 'cpt' ) );

		$this->assertSame( '/gechiui/v1/cpt/' . $post->ID, rest_get_route_for_post( $post ) );
	...
	public function test_rest_get_route_for_post_type_items() {
		$this->assertSame( '/gc/v2/posts', rest_get_route_for_post_type_items( 'post' ) );
	}
	...
	public function test_rest_get_route_for_post_type_items_custom_namespace() {
		...
		$this->assertSame( '/gechiui/v1/cpt', rest_get_route_for_post_type_items( 'cpt' ) );
		unregister_post_type( 'cpt' );
	}
	...
	public function test_rest_get_route_for_post_custom_controller() {
		$post = self::factory()->post->create_and_get( array( 'post_type' => 'gc_block' ) );
		$this->assertSame( '/gc/v2/blocks/' . $post->ID, rest_get_route_for_post( $post ) );
	}
	...
	public function test_rest_get_route_for_post() {
		...
		$this->assertSame( '/gc/v2/posts/' . $post->ID, rest_get_route_for_post( $post ) );
	}
	...
	public function test_rest_get_route_for_media() {
		...
		$this->assertSame( '/gc/v2/media/' . $post->ID, rest_get_route_for_post( $post ) );
	}
	...
	public function test_rest_get_route_for_post_id() {
		$post = self::factory()->post->create_and_get();
		$this->assertSame( '/gc/v2/posts/' . $post->ID, rest_get_route_for_post( $post->ID ) );
	}
	...
	public function test_rest_get_route_for_term() {
		...
		$this->assertSame( '/gc/v2/tags/' . $term->term_id, rest_get_route_for_term( $term ) );
	}
	...
	public function test_rest_get_route_for_category() {
		...
		$this->assertSame( '/gc/v2/categories/' . $term->term_id, rest_get_route_for_term( $term ) );
	}
	...
	public function test_rest_get_route_for_term_id() {
		$term = self::factory()->term->create_and_get();
		$this->assertSame( '/gc/v2/tags/' . $term->term_id, rest_get_route_for_term( $term->term_id ) );
	}
	...
	public function test_rest_get_route_for_taxonomy_custom_namespace() {
		...
		$this->assertSame( '/gechiui/v1/ct/' . $term->term_id, rest_get_route_for_term( $term ) );
	...
	public function test_rest_get_route_for_taxonomy_items() {
		$this->assertSame( '/gc/v2/categories', rest_get_route_for_taxonomy_items( 'category' ) );
	}
	...
	public function test_rest_get_route_for_taxonomy_items_custom_namespace() {
		...
		$this->assertSame( '/gechiui/v1/ct', rest_get_route_for_taxonomy_items( 'ct' ) );
	...
	public function test_rest_preload_api_request_embeds_links( $embed, $expected, $not_expected ) {
		...
		$url           = sprintf( '/gc/v2/posts/%d?%s', $post_id, $embed );
	...
	public function data_rest_preload_api_request_embeds_links() {
		return array(
			array( '_embed=gc:term,author', array( 'gc:term', 'author' ), array( 'replies' ) ),
			array( '_embed[]=gc:term&_embed[]=author', array( 'gc:term', 'author' ), array( 'replies' ) ),
			array( '_embed', array( 'gc:term', 'author', 'replies' ), array() ),
			array( '_embed=1', array( 'gc:term', 'author', 'replies' ), array() ),
			array( '_embed=true', array( 'gc:term', 'author', 'replies' ), array() ),
			array( '', array(), array() ),
		);
	}
}
