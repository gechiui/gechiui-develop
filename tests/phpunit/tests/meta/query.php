...
class Tests_Meta_Query extends GC_UnitTestCase {
	...
	public function test_single_inner_join_for_keys_only() {
		...
		$sql = $query->get_sql( 'post', $gcdb->posts, 'ID' );
	...
	/**
	 * GC_Query-style query must be at index 0 for order_by=meta_value to work.
	 */
	...
	/**
	 * GC_Query sets meta_value to '' by default. It should be removed by parse_query_vars().
	 */
	...
	/**
	 * Test the conversion between "GC_Query" style meta args (meta_value=x&meta_key=y)
	...
	public function test_empty_value_sql() {
		...
		$sql = $query->get_sql( 'post', $gcdb->posts, 'ID', $this );
	...
	public function test_null_value_sql() {
		...
		$this->assertSame( 1, substr_count( $sql['where'], "$gcdb->postmeta.meta_value = ''" ) );
	}
	...
	public function test_get_sql_key_only_queries() {
		...
		$sql = $query1->get_sql( 'post', $gcdb->posts, 'ID', $this );

		// 'foo' and 'bar' should be queried against the non-aliased table.
		$this->assertSame( 1, substr_count( $sql['where'], "$gcdb->postmeta.meta_key = 'foo'" ) );
		$this->assertSame( 1, substr_count( $sql['where'], "$gcdb->postmeta.meta_key = 'bar'" ) );

		// NOT EXISTS compare queries are not key-only so should not be non-aliased.
		$this->assertSame( 0, substr_count( $sql['where'], "$gcdb->postmeta.meta_key = 'baz'" ) );

		// 'AND' queries don't have key-only queries.
		$query2 = new GC_Meta_Query(
			array(
				// Empty 'compare'.
				array(
					'key' => 'foo',
		...
		$sql = $query2->get_sql( 'post', $gcdb->posts, 'ID', $this );

		// Only 'foo' should be queried against the non-aliased table.
		$this->assertSame( 1, substr_count( $sql['where'], "$gcdb->postmeta.meta_key = 'foo'" ) );
		$this->assertSame( 0, substr_count( $sql['where'], "$gcdb->postmeta.meta_key = 'bar'" ) );
	}
	...
	public function test_get_sql_empty_meta_compare_with_array_value() {
		...
		$this->assertSame( 1, substr_count( $sql['where'], "$gcdb->postmeta.meta_value IN" ) );
	}
	...
	public function test_get_sql_trim_string_value() {
		...
		$this->assertSame( 1, substr_count( $sql['where'], "$gcdb->postmeta.meta_value = 'bar'" ) );
	}
	...
	public function test_not_exists() {
		...
		$sql = $query->get_sql( 'post', $gcdb->posts, 'ID', $this );
		$this->assertStringNotContainsString( "{$gcdb->postmeta}.meta_key = 'exclude'\nOR", $sql['where'] );
		$this->assertStringContainsString( "{$gcdb->postmeta}.post_id IS NULL", $sql['where'] );
	}
	...
	public function test_empty_compare() {
		...
		$this->assertMatchesRegularExpression( "/{$gcdb->postmeta}\.meta_key = \'exclude\'\s+OR/", $sql['where'] );
		$this->assertStringNotContainsString( "{$gcdb->postmeta}.post_id IS NULL", $sql['where'] );
	}
