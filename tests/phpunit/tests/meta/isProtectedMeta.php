 ...
 */
class Tests_Meta_isProtectedMeta extends GC_UnitTestCase {
	...
	public function protected_data() {
		$protected_keys = array(
			array( '_gc_attachment' ),
		);
		for ( $i = 0, $max = 31; $i < $max; $i ++ ) {
			$protected_keys[] = array( chr( $i ) . '_gc_attachment' );
		}
		for ( $i = 127, $max = 159; $i <= $max; $i ++ ) {
			$protected_keys[] = array( chr( $i ) . '_gc_attachment' );
		}
		$protected_keys[] = array( chr( 95 ) . '_gc_attachment' );
	...
	public function unprotected_data() {
		...
		for ( $i = 32, $max = 94; $i <= $max; $i ++ ) {
			$unprotected_keys[] = array( chr( $i ) . '_gc_attachment' );
		}
