 ...
 * @group file
 */
class Tests_File extends GC_UnitTestCase {
	...
	public function test_get_file_data() {
		...
		$expected = array(
			'Name'        => 'GeChiUI Default',
			'ThemeURI'    => 'http://www.gechiui.com/',
			'Description' => 'The default GeChiUI theme based on the famous <a href="http://binarybonsai.com/kubrick/">Kubrick</a>.',
			'Version'     => '1.6',
	...
	public function test_unique_filename_is_valid() {
		...
		$filename = gc_unique_filename( $this->dir, __FUNCTION__ . '.txt' );
	...
	public function test_unique_filename_is_unique() {
		...
		$filename1 = gc_unique_filename( $this->dir, $name . '.txt' );
		$this->assertTrue( $this->is_unique_writable_file( $this->dir, $filename1 ) );
		$filename2 = gc_unique_filename( $this->dir, $name . '.txt' );
	...
	public function test_unique_filename_is_sanitized() {
		$name     = __FUNCTION__;
		$filename = gc_unique_filename( $this->dir, $name . $this->badchars . '.txt' );
	...
	public function test_unique_filename_with_slashes() {
		$name = __FUNCTION__;
		// "foo/foo.txt"
		$filename = gc_unique_filename( $this->dir, $name . '/' . $name . '.txt' );
	...
	public function test_unique_filename_multiple_ext() {
		$name     = __FUNCTION__;
		$filename = gc_unique_filename( $this->dir, $name . '.php.txt' );
	...
	public function test_unique_filename_no_ext() {
		$name     = __FUNCTION__;
		$filename = gc_unique_filename( $this->dir, $name );
	...
	/**
	 * @dataProvider data_gc_tempnam_filenames
	 */
	public function test_gc_tempnam( $case ) {
		$file = gc_tempnam( $case );
		unlink( $file );

		$this->assertNotEmpty( basename( basename( $file, '.tmp' ), '.zip' ) );
	}
	public function data_gc_tempnam_filenames() {
		return array(
			array( '0.zip' ),
			array( '0.1.2.3.zip' ),
			array( 'filename.zip' ),
	...
	public function test_file_signature_functions_as_expected() {
		$file = gc_tempnam();
		file_put_contents( $file, 'GeChiUI' );

		// The signature of 'GeChiUI' after SHA384 hashing, for verification against the key within self::filter_trust_plus85Tq_key().
		$expected_signature = 'PmNv0b1ziwJAsVhjdpjd4+PQZidZWSlBm5b+GbbwE9m9HVKDFhEyvyRTHkRYOLypB8P2YvbW7CoOMZqGh8mEAA==';

		add_filter( 'gc_trusted_keys', array( $this, 'filter_trust_plus85Tq_key' ) );

		// Measure how long the call takes.
		$timer_start = microtime( 1 );
		$verify      = verify_file_signature( $file, $expected_signature, 'GeChiUI' );
		$timer_end   = microtime( 1 );
		$time_taken  = ( $timer_end - $timer_start );

		unlink( $file );
		remove_filter( 'gc_trusted_keys', array( $this, 'filter_trust_plus85Tq_key' ) );
		...
		if ( is_gc_error( $verify ) && 'signature_verification_unsupported' === $verify->get_error_code() ) {
