<?php
/**
 * Unit tests covering GC_REST_Posts_Statuses_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Post_Statuses_Controller extends GC_Test_REST_Controller_Testcase {

	public function test_register_routes() {
		$routes = rest_get_server()->get_routes();
		$this->assertArrayHasKey( '/gc/v2/statuses', $routes );
		$this->assertArrayHasKey( '/gc/v2/statuses/(?P<status>[\w-]+)', $routes );
	}

	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/statuses' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSameSets( array( 'embed', 'view', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/statuses/publish' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSameSets( array( 'embed', 'view', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/statuses' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item() {
		$user_id = $this->factory->user->create( array( 'role' => 'author' ) );
		gc_set_current_user( $user_id );
		$request = new GC_REST_Request( 'GET', '/gc/v2/statuses/publish' );
		$request->set_param( 'context', 'edit' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_invalid_status() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/statuses/invalid' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_invalid_access() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/statuses/draft' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_invalid_internal() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/statuses/inherit' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_create_item() {
		...
		$request  = new GC_REST_Request( 'POST', '/gc/v2/statuses' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_update_item() {
		...
		$request  = new GC_REST_Request( 'POST', '/gc/v2/statuses/draft' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 404, $response->get_status() );
	}
	...
	public function test_delete_item() {
		...
		$request  = new GC_REST_Request( 'DELETE', '/gc/v2/statuses/draft' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 404, $response->get_status() );
	}
	...
	public function test_prepare_item() {
		...
		$endpoint = new GC_REST_Post_Statuses_Controller;
