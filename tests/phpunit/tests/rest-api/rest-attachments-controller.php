<?php
/**
 * Unit tests covering GC_REST_Attachments_Controller functionality
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Attachments_Controller extends GC_Test_REST_Post_Type_Controller_Testcase {
	...
	public function tear_down() {
		...
		if ( class_exists( GC_Image_Editor_Mock::class ) ) {
			GC_Image_Editor_Mock::$spy         = array();
			GC_Image_Editor_Mock::$edit_return = array();
			GC_Image_Editor_Mock::$size_return = null;
		}
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/media', $routes );
		$this->assertCount( 2, $routes['/gc/v2/media'] );
		$this->assertArrayHasKey( '/gc/v2/media/(?P<id>[\d]+)', $routes );
		$this->assertCount( 3, $routes['/gc/v2/media/(?P<id>[\d]+)'] );
	}
	...
	public function test_parse_disposition( $header, $expected ) {
		...
		$parsed      = GC_REST_Attachments_Controller::get_filename_from_disposition( $header_list );
		$this->assertSame( $expected, $parsed );
	}

	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/media' );
		...
		$request       = new GC_REST_Request( 'OPTIONS', '/gc/v2/media/' . $attachment_id );
		$response      = rest_get_server()->dispatch( $request );
		$data          = $response->get_data();
		$this->assertArrayNotHasKey( 'allow_batch', $data['endpoints'][0] );
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
	...
	public function test_registered_get_item_params() {
		...
		$request  = new GC_REST_Request( 'OPTIONS', sprintf( '/gc/v2/media/%d', $id1 ) );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$keys     = array_keys( $data['endpoints'][0]['args'] );
		sort( $keys );
	...
	public function test_get_items() {
		...
		$request        = new GC_REST_Request( 'GET', '/gc/v2/media' );
		$response       = rest_get_server()->dispatch( $request );
		$data           = $response->get_data();
	...
	public function test_get_item() {
		...
		update_post_meta( $attachment_id, '_gc_attachment_image_alt', 'Sample alt text' );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/media/' . $attachment_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_sizes() {
		...
		gc_update_attachment_metadata( $attachment_id, gc_generate_attachment_metadata( $attachment_id, $this->test_file ) );

		$request            = new GC_REST_Request( 'GET', '/gc/v2/media/' . $attachment_id );
		$response           = rest_get_server()->dispatch( $request );
		$data               = $response->get_data();
		$image_src          = gc_get_attachment_image_src( $attachment_id, 'rest-api-test' );
		$original_image_src = gc_get_attachment_image_src( $attachment_id, 'full' );
	...
	public function test_get_item_sizes_with_no_url() {
		...
		add_filter( 'gc_get_attachment_image_src', '__return_false' );

		$request  = new GC_REST_Request( 'GET', '/gc/v2/media/' . $attachment_id );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		remove_filter( 'gc_get_attachment_image_src', '__return_false' );
		remove_image_size( 'rest-api-test' );
	...
	public function test_get_item_private_post_not_authenticated() {
		...
		$request    = new GC_REST_Request( 'GET', '/gc/v2/media/' . $id1 );
		$response   = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_inherit_status_with_invalid_parent() {
		...
		$request       = new GC_REST_Request( 'GET', sprintf( '/gc/v2/media/%d', $attachment_id ) );
		$response      = rest_get_server()->dispatch( $request );
		$data          = $response->get_data();
	...
	public function test_create_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/media' );
		...
		$this->assertSame( 'Alt text is stored outside post schema.', get_post_meta( $attachment->ID, '_gc_attachment_image_alt', true ) );
	}
	...
	public function test_create_item_with_upload_files_role() {
		gc_set_current_user( self::$uploader_id );
		$request = new GC_REST_Request( 'POST', '/gc/v2/media' );
		$request->set_file_params(
			array(
				'file' => array(
	...
	public function test_create_item_ensure_relative_path() {
		...
		$response   = rest_get_server()->dispatch( $request );
		$attachment = $response->get_data();
		$this->assertStringNotContainsString( ABSPATH, get_post_meta( $attachment['id'], '_gc_attached_file', true ) );
	}
	...
	public function test_update_item() {
		...
		$request       = new GC_REST_Request( 'POST', '/gc/v2/media/' . $attachment_id );
		$request->set_param( 'title', 'My title is very cool' );
		$request->set_param( 'caption', 'This is a better caption.' );
		$request->set_param( 'description', 'Without a description, my attachment is descriptionless.' );
		$request->set_param( 'alt_text', 'Alt text is stored outside post schema.' );
	...
	public function verify_attachment_roundtrip( $input = array(), $expected_output = array() ) {
		...
		// See https://core.trac.gechiui.com/ticket/38679
		...
		// Update the post.
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/media/%d', $actual_output['id'] ) );
		foreach ( $input as $name => $value ) {
			$request->set_param( $name, $value );
		}
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_delete_item() {
		...
		$request          = new GC_REST_Request( 'DELETE', '/gc/v2/media/' . $attachment_id );
		$request['force'] = true;
		$response         = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	}
	...
	public function test_prepare_item_limit_fields() {
		...
		gc_set_current_user( self::$editor_id );
		$endpoint = new GC_REST_Attachments_Controller( 'post' );
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/media/%d', $attachment_id ) );
		$request->set_param( 'context', 'edit' );
	...
	public function test_additional_field_update_errors() {
		...
		$request = new GC_REST_Request( 'POST', sprintf( '/gc/v2/media/%d', $attachment_id ) );
	...
	public function test_search_item_by_filename() {
		...
		$filename = gc_basename( $this->test_file2 );
	...
	public function test_links_exist() {
		...
		$request = new GC_REST_Request( 'GET', "/gc/v2/media/{$post}" );
	...
	public function test_publish_action_ldo_not_registered() {
		...
		$response = rest_get_server()->dispatch( new GC_REST_Request( 'OPTIONS', '/gc/v2/media' ) );
		$data     = $response->get_data();
		$schema   = $data['schema'];
	...
	protected function check_post_data( $attachment, $data, $context = 'view', $links = array() ) {
		...
		$this->assertSame( get_post_meta( $attachment->ID, '_gc_attachment_image_alt', true ), $data['alt_text'] );
		if ( 'edit' === $context ) {
		...
		$this->assertSame( gc_get_attachment_url( $attachment->ID ), $data['source_url'] );
	...
	public function test_edit_image_returns_error_if_logged_out() {
		...
		$request = new GC_REST_Request( 'POST', "/gc/v2/media/{$attachment}/edit" );
		$request->set_body_params( array( 'src' => gc_get_attachment_image_url( $attachment, 'full' ) ) );
	...
	public function test_edit_image_returns_error_if_no_attachment() {
		...
		$request = new GC_REST_Request( 'POST', "/gc/v2/media/{$attachment}/edit" );
		$request->set_body_params( array( 'src' => '/gc-content/uploads/2020/07/canola.jpg' ) );
		$response = rest_do_request( $request );
	...
	public function test_edit_image_rotate() {
		...
		GC_Image_Editor_Mock::$edit_return['rotate'] = new GC_Error();

		$params = array(
			'rotation' => 60,
			'src'      => gc_get_attachment_image_url( $attachment, 'full' ),
		);
		...
		$this->assertCount( 1, GC_Image_Editor_Mock::$spy['rotate'] );
		$this->assertSame( array( -60 ), GC_Image_Editor_Mock::$spy['rotate'][0] );
	}
	...
	public function test_edit_image_crop() {
		...
		$this->setup_mock_editor();
		GC_Image_Editor_Mock::$size_return = array(
			'width'  => 640,
			'height' => 480,
		);

		GC_Image_Editor_Mock::$edit_return['crop'] = new GC_Error();
		...
		$this->assertCount( 1, GC_Image_Editor_Mock::$spy['crop'] );
		$this->assertSame(
			array( 320.0, 48.0, 64.0, 24.0 ),
			GC_Image_Editor_Mock::$spy['crop'][0]
		);
	}
	...
	public function test_edit_image() {
		...
		$this->assertSame( 201, $response->get_status() );
		$this->assertSame( rest_url( '/gc/v2/media/' . $item['id'] ), $response->get_headers()['Location'] );
	...
	public function test_edit_image_returns_error_if_mismatched_src() {
		...
		$image_src = gc_get_attachment_image_url( $attachment_id_image1, 'large' );

		// Test: attachment ID points to a different, non-image attachment.
		$request_1 = new GC_REST_Request( 'POST', "/gc/v2/media/{$attachment_id_file}/edit" );
		...
		$request_2 = new GC_REST_Request( 'POST', "/gc/v2/media/{$attachment_id_image2}/edit" );
		...
		$request_3 = new GC_REST_Request( 'POST', "/gc/v2/media/{$attachment_id_image1}/edit" );
		$request_3->set_body_params( array( 'src' => gc_get_attachment_image_url( $attachment_id_image1, 'medium' ) ) );
	...
	protected function setup_mock_editor() {
		...
		add_filter(
			'gc_image_editors',
			static function () {
				return array( 'GC_Image_Editor_Mock' );
			}
		);
	}
}
