<?php
/**
 * Unit tests covering GC_REST_Sidebars_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST_API
 * @since 5.8.0
 ...
 * @group restapi
 * @group widgets
 * @covers GC_REST_Sidebars_Controller
 */
class GC_Test_REST_Sidebars_Controller extends GC_Test_REST_Controller_Testcase {
	...
	public static function gcTearDownAfterClass() {
		gc_delete_user( self::$admin_id );
		gc_delete_user( self::$author_id );
	}
	...
	public function set_up() {
		...
		global $gc_registered_sidebars, $_gc_sidebars_widgets;
	...
	private function setup_sidebar( $id, $attrs = array(), $widgets = array() ) {
		...
		foreach ( $gc_registered_widgets as $gc_registered_widget ) {
			if ( is_array( $gc_registered_widget['callback'] ) ) {
				$gc_registered_widget['callback'][0]->_register();
			}
		}
	}
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/sidebars', $routes );
		$this->assertArrayHasKey( '/gc/v2/sidebars/(?P<id>[\w-]+)', $routes );
	}
	...
	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/sidebars' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/sidebars/sidebar-1' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/sidebars' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	public function test_get_item() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/sidebars/sidebar-1' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$data     = $this->remove_links( $data );
		$this->assertSame(
	...
	public function test_update_item_removes_widget_from_existing_sidebar() {
		...
		$request = new GC_REST_Request( 'PUT', '/gc/v2/sidebars/sidebar-2' );
		$request->set_body_params(
			array(
				'widgets' => array(
					'text-1',
		...
		$this->assertNotContains( 'text-1', rest_do_request( '/gc/v2/sidebars/sidebar-1' )->get_data()['widgets'] );
	}
	...
	public function test_batch_remove_widgets_from_existing_sidebar() {
		...
		$this->assertSame(
			...
			rest_do_request( '/gc/v2/sidebars/sidebar-1' )->get_data()['widgets']
		);
	}
	...
	public function test_update_item_moves_omitted_widget_to_inactive_sidebar() {
		...
		$this->assertContains( 'text-1', rest_do_request( '/gc/v2/sidebars/gc_inactive_widgets' )->get_data()['widgets'] );
	}
	...
	public function test_get_items_inactive_widgets() {
		...
		update_option(
			...
			array_merge(
				...
				array(
					'gc_inactive_widgets' => array( 'rss-1', 'rss' ),
				)
			)
		);
	...
	public function test_update_item_no_permission() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/sidebars/sidebar-1' );
		$request->set_body_params(
			array(
				'widgets' => array(),
			)
