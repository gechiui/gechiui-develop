<?php
/**
 * Unit tests covering GC_REST_Terms_Controller functionality, used for Tags.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Tags_Controller extends GC_Test_REST_Controller_Testcase {
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/tags', $routes );
		$this->assertArrayHasKey( '/gc/v2/tags/(?P<id>[\d]+)', $routes );
	}

	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/tags' );
		...
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/tags/' . $tag1 );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( array( 'v1' => true ), $data['endpoints'][0]['allow_batch'] );
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
	...
	public function test_get_items() {
		...
		$request = new GC_REST_Request( 'GET', '/gc/v2/tags' );
	...
	public function test_get_items_hide_empty_arg() {
		...
		gc_set_object_terms( $post_id, array( $tag1, $tag2 ), 'post_tag' );
	...
	public function test_get_terms_post_args_paging() {
		...
		gc_set_object_terms( $post_id, self::$tag_ids, 'post_tag' );
	...
	public function test_get_terms_pagination_headers() {
		...
		$this->assertSame( $total_tags, $headers['X-GC-Total'] );
	...
	public function test_get_item() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/tags/' . $id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_term_invalid_term() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/tags/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_incorrect_taxonomy() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/tags/' . $term1 );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_term_invalid', $response, 404 );
	}
	...
	public function test_create_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/tags' );
		...
		$headers = $response->get_headers();
		$data    = $response->get_data();
		$this->assertStringContainsString( '/gc/v2/tags/' . $data['id'], $headers['Location'] );
	...
	public function test_update_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/tags/' . $term->term_id );
	...
	public function test_update_item_no_change() {
		...
		$request  = new GC_REST_Request( 'PUT', '/gc/v2/tags/' . $term->term_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	...
	public function test_update_item_invalid_term() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/tags/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
	...
	public function verify_tag_roundtrip( $input = array(), $expected_output = array() ) {
		...
		// Update the tag.
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/tags/%d', $actual_output['id'] ) );
		foreach ( $input as $name => $value ) {
			$request->set_param( $name, $value );
		}
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_delete_item() {
		...
		$request = new GC_REST_Request( 'DELETE', '/gc/v2/tags/' . $term->term_id );
	...
	public function test_delete_item_invalid_term() {
		...
		$request  = new GC_REST_Request( 'DELETE', '/gc/v2/tags/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_term_invalid', $response, 404 );
	}
	...
	public function test_prepare_item() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/tags/' . $term->term_id );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	public function test_prepare_item_limit_fields() {
		...
		$endpoint = new GC_REST_Terms_Controller( 'post_tag' );
	...
	public function test_get_additional_field_registration() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/tags/' . $tag_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_additional_field_update_errors() {
		...
		$request = new GC_REST_Request( 'POST', sprintf( '/gc/v2/tags/%d', $tag_id ) );
	...
	public function test_object_term_queries_are_cached() {
		...
		gc_set_object_terms( $p, $tags[0], 'post_tag' );

		$request = new GC_REST_Request( 'GET', '/gc/v2/tags' );
		$request->set_param( 'post', $p );
		$response = rest_get_server()->dispatch( $request );
		$found_1  = gc_list_pluck( $response->data, 'id' );
		...
		$request->set_param( 'post', $p );
		$response = rest_get_server()->dispatch( $request );
		$found_2  = gc_list_pluck( $response->data, 'id' );
	...
	public function test_editable_response_uses_edit_context() {
		...
		$create = new GC_REST_Request( 'POST', '/gc/v2/tags' );
		...
		$update = new GC_REST_Request( 'PUT', '/gc/v2/tags/' . $data['id'] );
	...
	protected function check_taxonomy_term( $term, $data, $links ) {
		...
		$this->assertSame( add_query_arg( 'tags', $term->term_id, rest_url( 'gc/v2/posts' ) ), $links['https://api.w.org/post_type'][0]['href'] );
	}
