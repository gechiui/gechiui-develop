<?php
/**
 * Unit tests covering GC_REST_Posts_Controller functionality, used for
 * Pages
 *
 * @package GeChiUI
 * @subpackage REST API
 ...
 * @group restapi
 */
class GC_Test_REST_Pages_Controller extends GC_Test_REST_Post_Type_Controller_Testcase {
	...
	public function set_up() {
		...
		$controller                                     = new GC_REST_Posts_Controller( 'page' );
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/pages', $routes );
		$this->assertCount( 2, $routes['/gc/v2/pages'] );
		$this->assertArrayHasKey( '/gc/v2/pages/(?P<id>[\d]+)', $routes );
		$this->assertCount( 3, $routes['/gc/v2/pages/(?P<id>[\d]+)'] );
	}

	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/pages' );
		...
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/pages/' . $page_id );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items_parents_query() {
		...
		$data     = $response->get_data();
		$this->assertCount( 2, $data );
		$this->assertSameSets( array( $id2, $id4 ), gc_list_pluck( $data, 'id' ) );
	}
	...
	public function test_get_items_menu_order_query() {
		...
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSameSets( array( $id1, $id2, $id3, $id4 ), gc_list_pluck( $data, 'id' ) );

		// Filter to 'menu_order'.
		$request->set_param( 'menu_order', 1 );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSameSets( array( $id4 ), gc_list_pluck( $data, 'id' ) );
	...
	public function test_get_items_min_max_pages_query() {
		...
		$this->assertErrorResponse( 'rest_invalid_param', $response, 400 );
		$data = $response->get_data();
		// Safe format for 4.4 and 4.5. See https://core.trac.gechiui.com/ticket/35028
	...
	public function test_get_item_invalid_post_type() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/pages/' . $post_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_create_item_with_template() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/pages' );
	...
	public function test_delete_item() {
		...
		$request = new GC_REST_Request( 'DELETE', sprintf( '/gc/v2/pages/%d', $page_id ) );
	...
	public function test_prepare_item_limit_fields() {
		...
		$endpoint = new GC_REST_Posts_Controller( 'page' );
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/pages/%d', $page_id ) );
	...
	public function test_get_pages_params() {
		...
		$this->assertSame( 8, $headers['X-GC-Total'] );
		$this->assertSame( 2, $headers['X-GC-TotalPages'] );
	...
	public function test_update_page_menu_order() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/pages/%d', $page_id ) );
	...
	public function test_update_page_parent_non_zero() {
		...
		gc_set_current_user( self::$editor_id );
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/pages/%d', $page_id2 ) );
		$request->set_body_params(
			array(
				'parent' => $page_id1,
			)
	...
	public function test_get_page_with_password_using_password() {
		...
		$data = $response->get_data();
		$this->assertSame( gcautop( $page->post_content ), $data['content']['rendered'] );
		$this->assertTrue( $data['content']['protected'] );
		$this->assertSame( gcautop( $page->post_excerpt ), $data['excerpt']['rendered'] );
		$this->assertTrue( $data['excerpt']['protected'] );
	}
