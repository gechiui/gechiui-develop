<?php
/**
 * Unit tests covering GC_REST_Users_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Users_Controller extends GC_Test_REST_Controller_Testcase {
	...
	public function set_up() {
		parent::set_up();
		$this->endpoint = new GC_REST_Users_Controller();
	}

	public function test_register_routes() {
		$routes = rest_get_server()->get_routes();

		$this->assertArrayHasKey( '/gc/v2/users', $routes );
		$this->assertCount( 2, $routes['/gc/v2/users'] );
		$this->assertArrayHasKey( '/gc/v2/users/(?P<id>[\d]+)', $routes );
		$this->assertCount( 3, $routes['/gc/v2/users/(?P<id>[\d]+)'] );
		$this->assertArrayHasKey( '/gc/v2/users/me', $routes );
	}
	...
	public function test_context_param() {
		...
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/users/' . self::$user );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		gc_set_current_user( self::$user );

		$request = new GC_REST_Request( 'GET', '/gc/v2/users' );
	...
	public function test_get_items_with_edit_context_without_permission() {
		...
		gc_set_current_user( self::$editor );
	...
	public function test_get_items_unauthenticated_includes_authors_of_post_types_shown_in_rest() {
		...
		$user_ids = gc_list_pluck( $users, 'id' );
	...
	public function test_get_items_pagination_headers() {
		...
		$this->assertSame( $total_users, $headers['X-GC-Total'] );
		$this->assertSame( $total_pages, $headers['X-GC-TotalPages'] );
		$next_link = add_query_arg(
			array(
				'page' => 2,
			),
			rest_url( 'gc/v2/users' )
		);
	...
	/**
	 * @expectedDeprecated GC_User_Query
	 */
	public function test_get_items_who_author_query() {
		gc_set_current_user( self::$superadmin );
	...
	public function test_get_item() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/users/%d', $user_id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_user_avatar_urls() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/users/%d', self::$editor ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_user_invalid_id() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/users/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_user_empty_capabilities() {
		...
		$request = new GC_REST_Request( 'GET', '/gc/v2/users/' . $lolz );
		$request->set_param( 'context', 'edit' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_cannot_get_item_without_permission() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/users/%d', self::$user ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_user_cannot_view', $response, 403 );
	}

	public function test_can_get_item_author_of_rest_true_public_true_unauthenticated() {
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/users/%d', self::$authors['r_true_p_true'] ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	}
	...
	public function test_can_get_item_author_of_rest_true_public_false() {
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/users/%d', self::$authors['r_true_p_false'] ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	}

	public function test_cannot_get_item_author_of_rest_false_public_true_unauthenticated() {
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/users/%d', self::$authors['r_false_p_true'] ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_user_cannot_view', $response, 401 );
	}
	...
	public function test_cannot_get_item_author_of_rest_false_public_false() {
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/users/%d', self::$authors['r_false_p_false'] ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_user_cannot_view', $response, 401 );
	}
	...
	public function test_cannot_get_item_author_of_draft() {
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/users/%d', self::$draft_editor ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_user_cannot_view', $response, 401 );
	}
	...
	public function test_get_item_published_author_post() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/users/%d', $this->author_id ) );
		$response = rest_get_server()->dispatch( $request );
		$this->check_get_user_response( $response, 'embed' );
	}
	...
	public function test_get_current_user() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/users/me' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
		...
		$this->assertSame( rest_url( 'gc/v2/users/' . self::$user ), $links['self'][0]['href'] );
	}
	...
	public function test_create_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/users' );
	...
	public function test_create_item_invalid_username() {
		...
		// Username rules are different (more strict) for multisite; see `gcmu_validate_user_signup`.
		if ( is_multisite() ) {
		...
		foreach ( $errors as $error ) {
			...
			if ( 'user_name' === $error['code'] ) {
				...
				$expected = '<strong>Error:</strong> This email address is already registered. ' .
							'<a href="http://rest.gechiui.com/gc-login.php">Log in</a> with ' .
	...
	public function test_update_item() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/users/%d', $user_id ) );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
		...
		// as per https://core.trac.gechiui.com/ticket/21429
	...
	public function test_update_item_no_change() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/users/%d', self::$editor ) );
	...
	public function test_update_item_existing_email() {
		...
		$request = new GC_REST_Request( 'PUT', '/gc/v2/users/' . $user2 );
		$request->set_param( 'email', 'testjson@example.com' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertInstanceOf( 'GC_Error', $response->as_error() );
	...
	public function test_update_item_invalid_locale() {
		...
		$request = new GC_REST_Request( 'PUT', '/gc/v2/users/' . $user1 );
	...
	public function test_update_item_en_US_locale() {
		...
		$request = new GC_REST_Request( 'PUT', '/gc/v2/users/' . $user_id );
	...
	public function test_update_user_role_invalid_privilege_escalation() {
		...
		$request = new GC_REST_Request( 'PUT', '/gc/v2/users/me' );
		$request->set_param( 'roles', array( 'administrator' ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_update_user_without_permission() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/users/%d', self::$user ) );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
		$request->set_body_params( $params );
		$response = rest_get_server()->dispatch( $request );
	...
	public function verify_user_roundtrip( $input = array(), $expected_output = array() ) {
		if ( isset( $input['id'] ) ) {
			...
			$this->assertTrue( gc_check_password( addslashes( $expected_output['password'] ), $user->user_pass ) );
	...
	public function test_delete_item() {
		...
		$request  = new GC_REST_Request( 'DELETE', sprintf( '/gc/v2/users/%d', $user_id ) );
	...
	public function test_delete_current_item() {
		...
		$request          = new GC_REST_Request( 'DELETE', '/gc/v2/users/me' );
		$request['force'] = true;
		$request->set_param( 'reassign', false );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_delete_user_invalid_id() {
		...
		$request          = new GC_REST_Request( 'DELETE', '/gc/v2/users/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$request['force'] = true;
		$request->set_param( 'reassign', false );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_additional_field_registration() {
		...
		if ( is_multisite() ) {
			$current_user = gc_get_current_user( 1 );
			update_site_option( 'site_admins', array( $current_user->user_login ) );
		}

		$request  = new GC_REST_Request( 'GET', '/gc/v2/users/1' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertArrayHasKey( 'my_custom_int', $response->data );

		$request = new GC_REST_Request( 'POST', '/gc/v2/users/1' );
	...
	public function test_additional_field_update_errors() {
		...
		$request = new GC_REST_Request( 'POST', sprintf( '/gc/v2/users/%d', self::$user ) );
		$request->set_body_params(
			array(
