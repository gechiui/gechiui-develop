<?php
/**
 * Unit tests covering GC_REST_Server functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class Tests_REST_Server extends GC_Test_REST_TestCase {
	...
	public function set_up() {
		...
		$GLOBALS['gc_rest_server'] = rest_get_server();
	...
	public function test_envelope() {
		...
		$response = new GC_REST_Response( $data, $status );
		...
		$this->assertInstanceOf( 'GC_REST_Response', $envelope_response );
	...
	public function test_optional_param() {
		...
		$request = new GC_REST_Request( 'GET', '/optional/test' );
		$request->set_query_params( array() );
		$response = rest_get_server()->dispatch( $request );
		$this->assertInstanceOf( 'GC_REST_Response', $response );
	...
	public function test_no_zero_param() {
		...
		$request = new GC_REST_Request( 'GET', '/no-zero/test' );
	...
	public function test_head_request_handled_by_get() {
		...
		$request  = new GC_REST_Request( 'HEAD', '/head-request/test' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	}
	...
	public function test_url_params_no_numeric_keys() {
		...
		$request = new GC_REST_Request( 'GET', '/test/some-value' );
		rest_get_server()->dispatch( $request );
	...
	public function test_rest_route_capability_authorization_fails() {
		...
		$request = new GC_REST_Request( 'GET', '/test-ns/test', array() );
	...
	public function permission_denied() {
		return new GC_Error( 'forbidden', 'You are not allowed to do this', array( 'status' => 403 ) );
	}

	public function test_error_to_response() {
		$code    = 'gc-api-test-error';
		$message = 'Test error message for the API';
		$error   = new GC_Error( $code, $message );
	...
	public function test_error_to_response_with_status() {
		$code    = 'gc-api-test-error';
		$message = 'Test error message for the API';
		$error   = new GC_Error( $code, $message, array( 'status' => 400 ) );
	...
	public function test_error_to_response_with_additional_data() {
		$error = new GC_Error( 'test', 'test', array( 'status' => 400 ) );
	...
	public function test_rest_error() {
		...
		$expected = gc_json_encode( $data );
		$response = rest_get_server()->json_error( 'gc-api-test-error', 'Message text' );
	...
	public function test_json_error_with_status() {
		...
		$response = $stub->json_error( 'gc-api-test-error', 'Message text', 400 );
	...
	public function test_link_curies() {
		...
		$this->assertArrayHasKey( 'gc:term', $links );
	...
	public function test_link_embedding_error() {
		...
		$up_data = $up[0];
		$this->assertSame( 'gc-api-test-error', $up_data['code'] );
	...
	public function test_link_embedding_clears_cache() {
		...
		$response = new GC_REST_Response();
		$response->add_link( 'post', rest_url( 'gc/v2/posts/' . $post_id ), array( 'embeddable' => true ) );
	...
	public function test_link_embedding_cache_collection() {
		...
		$embeds = gc_list_pluck( $data, '_embedded' );
	...
	public function embedded_response_callback( $request ) {
		...
		if ( isset( $params['error'] ) ) {
			return new GC_Error( 'gc-api-test-error', 'Test message', array( 'status' => 403 ) );
		}
	...
	public function test_response_to_data_embedding( $expected, $embed ) {
		...
		$response->add_link( 'https://www.gechiui.com', rest_url( '404' ), array( 'embeddable' => true ) );
	...
	public function _dp_response_to_data_embedding() {
		return array(
			array(
				array( 'author', 'gc:term', 'https://www.gechiui.com' ),
				true,
			),
			array(
				array( 'author', 'gc:term', 'https://www.gechiui.com' ),
			...
			array(
				array( 'https://www.gechiui.com' ),
				array( 'https://www.gechiui.com' ),
			),
			array(
				array( 'author', 'gc:term' ),
				array( 'author', 'gc:term' ),
			),
			array(
				false,
	...
	public function test_get_index() {
		...
		$request = new GC_REST_Request( 'GET', '/' );
		...
		$this->assertArrayNotHasKey( 'gc:active-theme', $index->get_links() );
	...
	public function test_get_data_for_non_variable_route_includes_links() {
		$expected = array(
			'self' => array(
				array( 'href' => rest_url( 'gc/v2/posts' ) ),
			),
		);
	...
	public function test_x_robot_tag_header_on_requests() {
		$request = new GC_REST_Request( 'GET', '/', array() );
	...
	public function test_nocache_headers_on_authenticated_requests() {
		...
		foreach ( gc_get_nocache_headers() as $header => $value ) {
			if ( empty( $value ) ) {
				continue;
			}
		...
		// Last-Modified should be unset as per #GC23021.
	...
	public function test_serve_request_query_params_are_unslashed() {
		...
		// GeChiUI internally will slash the superglobals on bootstrap.
		$_GET = gc_slash(
			array(
	...
	public function test_serve_request_headers_are_unslashed() {
		...
		// GeChiUI internally will slash the superglobals on bootstrap.
		$_SERVER['HTTP_X_MY_HEADER'] = gc_slash( 'data\\with\\slashes' );

		$result = rest_get_server()->serve_request( '/test/data\\with\\slashes' );
		$this->assertSame( 'data\\with\\slashes', rest_get_server()->last_request->get_header( 'x_my_header' ) );
	}

	public function filter_gc_rest_server_class() {
	...
	public function test_rest_send_refreshed_nonce( $has_logged_in_user, $has_nonce ) {
		...
		if ( $has_nonce ) {
			$_REQUEST['_gcnonce'] = gc_create_nonce( 'gc_rest' );
		}

		$headers = $this->helper_make_request_and_return_headers_for_rest_send_refreshed_nonce_tests();

		if ( $has_nonce ) {
			$this->assertArrayHasKey( 'X-GC-Nonce', $headers );
		} else {
			$this->assertArrayNotHasKey( 'X-GC-Nonce', $headers );
		}
	}
	...
	public function test_does_not_echo_body_for_responses_with_204_status() {
		register_rest_route(
			...
			array(
				'methods'             => array( 'GET' ),
				'callback'            => static function () {
					return new GC_REST_Response( 'data', 204 );
				},
				'permission_callback' => '__return_true',
			)
		);
	...
	public function test_get_routes_no_namespace_overriding() {
		...
		$request  = new GC_REST_Request( 'GET', '/test-ns/v1/test' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_batch_v1_optin( $allow_batch, $allowed ) {
		$args = array(
			'methods'             => 'POST',
			'callback'            => static function () {
				return new GC_REST_Response( 'data' );
			},
			'permission_callback' => '__return_true',
		);
		...
		register_rest_route(
			...
			array(
				...
				'callback'            => static function ( $request ) {
					...
					return new GC_REST_Response( $project );
				},
				'permission_callback' => '__return_true',
				'allow_batch'         => array( 'v1' => true ),
				'args'                => array(
	...
	public function test_batch_v1_pre_validation_all_successful() {
		register_rest_route(
			...
			array(
				'methods'             => 'POST',
				'callback'            => static function ( $request ) {
					return new GC_REST_Response( $request['project'] );
				},
				'permission_callback' => '__return_true',
				'allow_batch'         => array( 'v1' => true ),
				'args'                => array(
		...
		$this->assertSame( 'GeChiUI', $data['responses'][1]['body'] );
	}
	...
	public function test_batch_v1() {
		register_rest_route(
			...
			array(
				...
				'callback'            => function ( GC_REST_Request $request ) {
					...
					return new GC_REST_Response( 'test' );
				},
				'permission_callback' => '__return_true',
				'allow_batch'         => array( 'v1' => true ),
			)
	...
	public function test_batch_v1_max_requests() {
		...
		register_rest_route(
			...
			array(
				'methods'             => array( 'POST', 'DELETE' ),
				'callback'            => static function ( GC_REST_Request $request ) {
					return new GC_REST_Response( 'test' );
				},
				'permission_callback' => '__return_true',
				'allow_batch'         => array( 'v1' => true ),
	...
	public function test_get_data_for_route_includes_permitted_schema_keywords() {
		...
		$response = rest_do_request( new GC_REST_Request( 'OPTIONS', '/test-ns/v1/test' ) );
		...
		register_rest_route(
			...
			array(
				array(
					'methods'             => \GC_REST_Server::READABLE,
					'callback'            => function() {
						return new \GC_REST_Response( INF );
					},
					'permission_callback' => '__return_true',
					'args'                => array(),
				),
	...
	public function _validate_as_integer_123( $value, $request, $key ) {
		if ( ! is_int( $value ) ) {
			return new GC_Error( 'some-error', 'This is not valid!' );
		}
