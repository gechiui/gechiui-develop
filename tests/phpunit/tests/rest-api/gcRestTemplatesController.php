 ...
 *
 * @covers GC_REST_Templates_Controller
 *
 * @group restapi
 */
class Tests_REST_GcRestTemplatesController extends GC_Test_REST_Controller_Testcase {
	/**
	 * @var int
	 */
	...
	public static function gcSetupBeforeClass( $factory ) {
		...
		gc_set_post_terms( self::$post->ID, get_stylesheet(), 'gc_theme' );
	}
	...
	/**
	 * @covers GC_REST_Templates_Controller::register_routes
	 * @ticket 54596
	 */
	...
	/**
	 * @covers GC_REST_Templates_Controller::get_context_param
	 */
	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/templates' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/templates/default//my_template' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	/**
	 * @covers GC_REST_Templates_Controller::get_items
	 */
	public function test_get_items() {
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/templates' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	/**
	 * @covers GC_REST_Templates_Controller::get_item
	 */
	public function test_get_item() {
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/templates/default//my_template' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		unset( $data['content'] );
		unset( $data['_links'] );
	...
	public function test_get_item_works_with_a_single_slash( $endpoint_url ) {
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', $endpoint_url );
		$response = rest_get_server()->dispatch( $request );
	...
	public function get_template_endpoint_urls() {
		return array(
			array( '/gc/v2/templates/default/my_template' ),
			array( '/gc/v2/templates/default//my_template' ),
		);
	}
	 ...
	 * @param array  $args      Arguments to create the 'gc_template" post.
	 */
	public function test_get_item_with_valid_theme_dirname( $theme_dir, $template, array $args ) {
		...
		gc_set_post_terms( $post->ID, get_stylesheet(), 'gc_theme' );

		$request  = new GC_REST_Request( 'GET', "/gc/v2/templates/{$theme_dir}//{$template}" );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		unset( $data['content'] );
		unset( $data['_links'] );
	...
	public function test_sanitize_template_id( $input_id, $sanitized_id ) {
		$endpoint = new GC_REST_Templates_Controller( 'gc_template' );
		$this->assertEquals(
			$sanitized_id,
	...
	/**
	 * @ticket 54422
	 * @covers GC_REST_Templates_Controller::create_item
	 */
	public function test_create_item() {
		gc_set_current_user( self::$admin_id );
		$request = new GC_REST_Request( 'POST', '/gc/v2/templates' );
	 ...
	 * @ticket 54680
	 * @covers GC_REST_Templates_Controller::create_item
	 * @covers GC_REST_Templates_Controller::get_item_schema
	 */
	...
	/**
	 * @covers GC_REST_Templates_Controller::update_item
	 */
	public function test_update_item() {
		gc_set_current_user( self::$admin_id );
		$request = new GC_REST_Request( 'PUT', '/gc/v2/templates/default//my_template' );
		$request->set_body_params(
			array(
	...
	/**
	 * @covers GC_REST_Templates_Controller::delete_item
	 */
	public function test_delete_item() {
		...
		gc_set_current_user( self::$admin_id );
		$request = new GC_REST_Request( 'DELETE', '/gc/v2/templates/default//my_test_template' );
	...
	public function test_delete_item_fail() {
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'DELETE', '/gc/v2/templates/justrandom//template' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_prepare_item_limit_fields() {
		...
		$obj      = get_block_template( 'default//my_template', 'gc_template' );
	...
	public function test_get_item_schema() {
		...
		$this->assertArrayHasKey( 'gc_id', $properties );
