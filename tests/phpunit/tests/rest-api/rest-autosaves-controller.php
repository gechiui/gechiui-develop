<?php
/**
 * Unit tests covering GC_REST_Autosaves_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Autosaves_Controller extends GC_Test_REST_Post_Type_Controller_Testcase {
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		// Create an autosave.
		self::$autosave_post_id = gc_create_post_autosave(
			array(
		...
		self::$autosave_page_id = gc_create_post_autosave(
			array(
				'post_content' => 'This content is better.',
	...
	public function set_up() {
		...
		$this->post_autosave = gc_get_post_autosave( self::$post_id );
	}

	public function test_register_routes() {
		$routes = rest_get_server()->get_routes();
		$this->assertArrayHasKey( '/gc/v2/posts/(?P<id>[\d]+)/autosaves', $routes );
		$this->assertArrayHasKey( '/gc/v2/posts/(?P<parent>[\d]+)/autosaves/(?P<id>[\d]+)', $routes );
		$this->assertArrayHasKey( '/gc/v2/pages/(?P<id>[\d]+)/autosaves', $routes );
		$this->assertArrayHasKey( '/gc/v2/pages/(?P<parent>[\d]+)/autosaves/(?P<id>[\d]+)', $routes );
	}

	public function test_context_param() {

		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/posts/' . self::$post_id . '/autosaves' );
		...
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/posts/' . self::$post_id . '/autosaves/' . self::$autosave_post_id );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSameSets( array( 'view', 'edit', 'embed' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . self::$post_id . '/autosaves' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	public function test_get_items_missing_parent() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER . '/autosaves' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_items_invalid_parent_post_type() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . self::$page_id . '/autosaves' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_post_invalid_parent', $response, 404 );
	}
	...
	public function test_get_item() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . self::$post_id . '/autosaves/' . self::$autosave_post_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
		$data = $response->get_data();
	...
	public function test_get_item_missing_parent() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER . '/autosaves/' . self::$autosave_post_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_post_invalid_parent', $response, 404 );
	...
	public function test_create_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/posts/' . self::$post_id . '/autosaves' );
	...
	public function test_rest_autosave_published_post() {
		...
		$request->set_body( gc_json_encode( $autosave_data ) );
		$response = rest_get_server()->dispatch( $request );
		...
		$autosave_post = gc_get_post_autosave( self::$post_id );
	...
	public function test_rest_autosave_draft_post_different_author() {
		...
		$autosave_post = gc_get_post_autosave( $post_id );
	...
	protected function check_get_autosave_response( $response, $autosave ) {
		...
		$this->assertSame( rest_url( '/gc/v2/' . $parent_base . '/' . $autosave->post_parent ), $links['parent'][0]['href'] );
	}
	...
	public function test_update_item_draft_page_with_parent() {
		gc_set_current_user( self::$editor_id );
		$request = new GC_REST_Request( 'POST', '/gc/v2/pages/' . self::$child_draft_page_id . '/autosaves' );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
	...
	public function test_schema_validation_is_applied() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/pages/' . self::$draft_page_id . '/autosaves' );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
