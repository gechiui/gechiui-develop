<?php
/**
 * GC_REST_Menu_Items_Controller tests
 *
 * @package GeChiUI
 * @subpackage REST_API
 * @since 5.9.0
 ...
 * @group restapi
 *
 * @coversDefaultClass GC_REST_Menu_Items_Controller
 */
class Tests_REST_GcRestMenuItemsController extends GC_Test_REST_Post_Type_Controller_Testcase {
	/**
	 * @var int
	 */
	protected $menu_id;
	...
	public function set_up() {
		...
		$this->menu_id = gc_create_nav_menu( rand_str() );

		$this->menu_item_id = gc_update_nav_menu_item(
			$this->menu_id,
			0,
			array(
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/menu-items', $routes );
		$this->assertCount( 2, $routes['/gc/v2/menu-items'] );
		$this->assertArrayHasKey( '/gc/v2/menu-items/(?P<id>[\d]+)', $routes );
		$this->assertCount( 3, $routes['/gc/v2/menu-items/(?P<id>[\d]+)'] );
	}
	...
	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/menu-items' );
		...
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/menu-items/' . $this->menu_item_id );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_registered_get_item_params() {
		$request  = new GC_REST_Request( 'OPTIONS', sprintf( '/gc/v2/menu-items/%d', $this->menu_item_id ) );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	public function test_get_items() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/menu-items' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item() {
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/menu-items/%d', $this->menu_item_id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_term_links() {
		...
		$request      = new GC_REST_Request( 'GET', sprintf( '/gc/v2/menu-items/%d', $menu_item_id ) );
		$request->set_param( 'context', 'edit' );
		$response = rest_get_server()->dispatch( $request );
	 ...
	 *
	 * @see https://github.com/GeChiUI/gutenberg/pull/24673
	 *
	 * @ticket 40878
	 * @covers ::get_item
	 */
	public function test_get_item_escapes_title() {
		...
		if ( ! is_multisite() ) {
			...
			// post_title is ran through gc_kses before being saved in the
	...
	public function test_create_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/menu-items' );
	...
	public function test_create_item_change_position() {
		gc_set_current_user( self::$admin_id );
		$new_menu_id = gc_create_nav_menu( rand_str() );
		$expected    = array();
		$actual      = array();
		for ( $i = 1; $i < 5; $i ++ ) {
			$request = new GC_REST_Request( 'POST', '/gc/v2/menu-items' );
	...
	public function test_update_item() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/menu-items/%d', $this->menu_item_id ) );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
		$params = $this->set_menu_item_data(
			array(
	...
	public function test_update_item_invalid() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/menu-items/%d', $post_id ) );
		$request->add_header( 'content-type', 'application/x-www-form-urlencoded' );
		$params = $this->set_menu_item_data();
		$request->set_body_params( $params );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_delete_item() {
		gc_set_current_user( self::$admin_id );
		$request = new GC_REST_Request( 'DELETE', sprintf( '/gc/v2/menu-items/%d', $this->menu_item_id ) );
	...
	public function test_delete_item_invalid() {
		gc_set_current_user( self::$admin_id );
		$request = new GC_REST_Request( 'DELETE', '/gc/v2/menu-items/9999' );
		$request->set_param( 'force', true );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_post_invalid_id', $response, 404 );
	}
	...
	public function test_prepare_item() {
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/menu-items/' . $this->menu_item_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	...
	/**
	 * @param GC_REST_Response $response Response Class.
	...
	protected function check_menu_item_data( $post, $data, $context, $links ) {
		...
		$this->assertSame( gcautop( $post->post_content ), $data['description'] );
		...
		if ( $post->post_parent ) {
			if ( is_int( $data['parent'] ) ) {
				...
				$menu_item = gc_setup_nav_menu_item( get_post( $data['parent']['id'] ) );
		...
		$taxonomies = gc_list_filter( get_object_taxonomies( self::POST_TYPE, 'objects' ), array( 'show_in_rest' => true ) );
		foreach ( $taxonomies as $taxonomy ) {
			$this->assertTrue( isset( $data[ $taxonomy->rest_base ] ) );
			$terms = gc_get_object_terms( $post->ID, $taxonomy->name, array( 'fields' => 'ids' ) );
			sort( $terms );
		...
		if ( $links ) {
			...
			$this->assertSame( $links['self'][0]['href'], rest_url( 'gc/v2/' . $post_type_obj->rest_base . '/' . $data['id'] ) );
			$this->assertSame( $links['collection'][0]['href'], rest_url( 'gc/v2/' . $post_type_obj->rest_base ) );
			$this->assertSame( $links['about'][0]['href'], rest_url( 'gc/v2/types/' . self::POST_TYPE ) );

			$num = 0;
			foreach ( $taxonomies as $taxonomy ) {
				$this->assertSame( $taxonomy->name, $links['https://api.w.org/term'][ $num ]['attributes']['taxonomy'] );
				$this->assertSame( add_query_arg( 'post', $data['id'], rest_url( 'gc/v2/' . $taxonomy->rest_base ) ), $links['https://api.w.org/term'][ $num ]['href'] );
				$num ++;
			}
