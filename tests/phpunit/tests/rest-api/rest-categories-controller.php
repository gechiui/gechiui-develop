<?php
/**
 * Unit tests covering GC_REST_Terms_Controller functionality, used for
 * Categories.
 *
 * @package GeChiUI
 * @subpackage REST API
 ...
 * @group restapi
 */
class GC_Test_REST_Categories_Controller extends GC_Test_REST_Controller_Testcase {
	...
	public static function gcTearDownAfterClass() {
		...
		foreach ( self::$category_ids as $category_id ) {
			gc_delete_term( $category_id, 'category' );
		}
	}
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/categories', $routes );
		$this->assertArrayHasKey( '/gc/v2/categories/(?P<id>[\d]+)', $routes );
	}

	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/categories' );
		...
		$request   = new GC_REST_Request( 'OPTIONS', '/gc/v2/categories/' . $category1 );
		$response  = rest_get_server()->dispatch( $request );
		$data      = $response->get_data();
		$this->assertSame( array( 'v1' => true ), $data['endpoints'][0]['allow_batch'] );
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
	...
	public function test_get_items() {
		$request = new GC_REST_Request( 'GET', '/gc/v2/categories' );
	...
	public function test_get_items_hide_empty_arg() {
		...
		gc_set_object_terms( $post_id, array( $category1, $category2 ), 'category' );
	...
	protected function post_with_categories() {
		...
		gc_set_object_terms( $post_id, array( $category1, $category2, $category3 ), 'category' );
	...
	public function test_get_items_custom_tax_post_args() {
		...
		$controller = new GC_REST_Terms_Controller( 'batman' );
		...
		$post_id = $this->factory->post->create();
		gc_set_object_terms( $post_id, array( $term1, $term2 ), 'batman' );

		$request = new GC_REST_Request( 'GET', '/gc/v2/batman' );
		$request->set_param( 'post', $post_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	...
	public function test_get_terms_private_taxonomy() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/terms/robin' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}

	public function test_get_terms_invalid_taxonomy() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/invalid-taxonomy' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}
	...
	public function test_get_terms_pagination_headers() {
		...
		$this->assertSame( $total_categories, $headers['X-GC-Total'] );
	...
	public function test_get_items_per_page_exceeds_number_of_items() {
		...
		$response = rest_get_server()->dispatch( $request );
		$headers  = $response->get_headers();
		$this->assertSame( self::$total_categories, $headers['X-GC-Total'] );
		$this->assertSame( 1, $headers['X-GC-TotalPages'] );
	...
	public function test_get_item() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/categories/1' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_term_invalid_taxonomy() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/invalid-taxonomy/1' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}

	public function test_get_term_invalid_term() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/categories/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_term_private_taxonomy() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/terms/robin/' . $term1 );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}
	...
	public function test_get_item_incorrect_taxonomy() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/categories/' . $term1 );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_term_invalid', $response, 404 );
	}

	public function test_create_item() {
		gc_set_current_user( self::$administrator );

		$request = new GC_REST_Request( 'POST', '/gc/v2/categories' );
		...
		$headers = $response->get_headers();
		$data    = $response->get_data();
		$this->assertStringContainsString( '/gc/v2/categories/' . $data['id'], $headers['Location'] );
	...
	public function test_create_item_term_already_exists() {
		...
		gc_delete_term( $existing_id, 'category' );
	}

	public function test_create_item_invalid_taxonomy() {
		gc_set_current_user( self::$administrator );

		$request = new GC_REST_Request( 'POST', '/gc/v2/invalid-taxonomy' );
	...
	public function test_create_item_incorrect_permissions_contributor() {
		gc_set_current_user( self::$contributor );
	...
	public function test_create_item_with_parent() {
		...
		$parent = gc_insert_term( 'test-category', 'category' );
	...
	public function test_create_item_invalid_parent() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/categories/' . $term->term_id );
		$request->set_param( 'name', 'My Awesome Term' );
	...
	public function test_update_item_invalid_taxonomy() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/invalid-taxonomy/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$request->set_param( 'name', 'Invalid Taxonomy' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}
	...
	public function test_update_item_invalid_term() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/categories/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
	...
	public function test_delete_item() {
		...
		$request = new GC_REST_Request( 'DELETE', '/gc/v2/categories/' . $term->term_id );
	...
	public function test_delete_item_invalid_taxonomy() {
		...
		$request  = new GC_REST_Request( 'DELETE', '/gc/v2/invalid-taxonomy/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}
	...
	public function test_delete_item_invalid_term() {
		...
		$request  = new GC_REST_Request( 'DELETE', '/gc/v2/categories/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_term_invalid', $response, 404 );
	}
	...
	public function test_prepare_item_limit_fields() {
		...
		$endpoint = new GC_REST_Terms_Controller( 'category' );
	...
	public function test_prepare_taxonomy_term_child() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/categories/' . $child );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		...
		$this->assertSame( rest_url( 'gc/v2/categories/1' ), $links['up'][0]['href'] );
	}
	...
	public function test_get_additional_field_registration() {
		...
		$request     = new GC_REST_Request( 'GET', '/gc/v2/categories/' . $category_id );
	...
	protected function check_taxonomy_term( $term, $data, $links ) {
		...
		$this->assertStringContainsString( 'gc/v2/taxonomies/' . $term->taxonomy, $links['about'][0]['href'] );
		$this->assertSame( add_query_arg( 'categories', $term->term_id, rest_url( 'gc/v2/posts' ) ), $links['https://api.w.org/post_type'][0]['href'] );
	}
