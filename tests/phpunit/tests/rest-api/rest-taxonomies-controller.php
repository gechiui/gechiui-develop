<?php
/**
 * Unit tests covering GC_REST_Taxonomies_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Taxonomies_Controller extends GC_Test_REST_Controller_Testcase {
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/taxonomies', $routes );
		$this->assertArrayHasKey( '/gc/v2/taxonomies/(?P<taxonomy>[\w-]+)', $routes );
	}

	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/taxonomies' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSameSets( array( 'view', 'edit', 'embed' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/taxonomies/post_tag' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSameSets( array( 'view', 'edit', 'embed' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		$request    = new GC_REST_Request( 'GET', '/gc/v2/taxonomies' );
		$response   = rest_get_server()->dispatch( $request );
		$data       = $response->get_data();
	...
	public function test_get_item() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/taxonomies/category' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_invalid_taxonomy() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/taxonomies/invalid' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_non_public_taxonomy_not_authenticated() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/taxonomies/api-private' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_create_item() {
		...
		$request  = new GC_REST_Request( 'POST', '/gc/v2/taxonomies' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_update_item() {
		...
		$request  = new GC_REST_Request( 'POST', '/gc/v2/taxonomies/category' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 404, $response->get_status() );
	}
	...
	public function test_delete_item() {
		...
		$request  = new GC_REST_Request( 'DELETE', '/gc/v2/taxonomies/category' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 404, $response->get_status() );
	}
	...
	public function test_prepare_item() {
		...
		$endpoint = new GC_REST_Taxonomies_Controller;
	...
	protected function check_taxonomy_object( $context, $tax_obj, $data, $links ) {
		...
		$this->assertSame( rest_url( 'gc/v2/taxonomies' ), $links['collection'][0]['href'] );
	...
	public function test_get_for_taxonomy_returns_terms_controller_if_custom_class_not_specified() {
		...
		$this->assertInstanceOf(
			GC_REST_Terms_Controller::class,
