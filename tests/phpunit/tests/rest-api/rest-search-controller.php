<?php
/**
 * GC_REST_Search_Controller tests
 *
 * @package GeChiUI
 * @subpackage REST_API
 */
...
/**
 * Tests for GC_REST_Search_Controller.
 *
 * @group restapi
 */
class GC_Test_REST_Search_Controller extends GC_Test_REST_Controller_Testcase {
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/search', $routes );
		$this->assertCount( 1, $routes['/gc/v2/search'] );
	}
	...
	public function test_get_items() {
		...
		$this->assertSameSets(
			...
			gc_list_pluck( $response->get_data(), 'id' )
		);
	}
	...
	public function test_get_item() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/search' . self::$my_title_post_ids[0] );
	...
	public function test_create_item() {
		...
		$request  = new GC_REST_Request( 'POST', '/gc/v2/search' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 404, $response->get_status() );
	}
	...
	public function test_update_item() {
		...
		$request  = new GC_REST_Request( 'POST', '/gc/v2/search' . self::$my_title_post_ids[0] );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 404, $response->get_status() );
	}
	...
	public function test_delete_item() {
		...
		$request  = new GC_REST_Request( 'DELETE', '/gc/v2/search' . self::$my_title_post_ids[0] );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 404, $response->get_status() );
	}
	...
	public function test_prepare_item_limit_fields() {
		if ( ! method_exists( 'GC_REST_Controller', 'get_fields_for_response' ) ) {
			$this->markTestSkipped( 'Limiting fields requires the GC_REST_Controller::get_fields_for_response() method.' );
		}
	...
	public function test_get_item_schema() {
		$request    = new GC_REST_Request( 'OPTIONS', '/gc/v2/search' );
		$response   = rest_get_server()->dispatch( $request );
		$data       = $response->get_data();
	...
	public function test_custom_search_handler_get_items() {
		$controller = new GC_REST_Search_Controller( array( new GC_REST_Test_Search_Handler( 10 ) ) );
		...
		$request  = $this->get_request(
			array(
				...
				'per_page' => 10,
				'type'     => 'test',
				'subtype'  => array( GC_REST_Search_Controller::TYPE_ANY ),
			)
		);
		$response = $controller->get_items( $request );
		$this->assertSameSets( range( 1, 10 ), gc_list_pluck( $response->get_data(), 'id' ) );
		...
		$response = $controller->get_items( $request );
		$this->assertSameSets( range( 1, 5 ), gc_list_pluck( $response->get_data(), 'id' ) );
	}
	...
	public function test_custom_search_handler_get_collection_params() {
		...
		$this->assertSame( 'test', $params[ GC_REST_Search_Controller::PROP_TYPE ]['default'] );
		$this->assertSameSets( array( 'test' ), $params[ GC_REST_Search_Controller::PROP_TYPE ]['enum'] );
		$this->assertSameSets( array( 'test_first_type', 'test_second_type', GC_REST_Search_Controller::TYPE_ANY ), $params[ GC_REST_Search_Controller::PROP_SUBTYPE ]['items']['enum'] );
	}
	...
	public function test_search_result_links_are_embedded() {
		...
		$this->assertArrayHasKey( GC_REST_Search_Controller::PROP_ID, $data['_embedded']['self'][0] );
		$this->assertSame( $data[ GC_REST_Search_Controller::PROP_ID ], $data['_embedded']['self'][0][ GC_REST_Search_Controller::PROP_ID ] );
	}
	...
	public function test_get_items_search_type_post_format() {
		...
		$this->assertContains(
			'Aside',
			gc_list_pluck( $response->get_data(), 'title' )
		);
	}
	...
	private function get_request( $params = array(), $method = 'GET' ) {
		$request = new GC_REST_Request( $method, '/gc/v2/search' );
