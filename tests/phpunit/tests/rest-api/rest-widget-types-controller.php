<?php
/**
 * Unit tests covering GC_Test_REST_Widget_Types_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST_API
 * @since 5.8.0
...
/**
 * Tests for GC_REST_Widget_Types_Controller.
 *
 * @since 5.8.0
 *
 * @see GC_TEST_REST_Controller_Testcase
 * @group restapi
 * @covers GC_REST_Widget_Types_Controller
 */
class GC_Test_REST_Widget_Types_Controller extends GC_Test_REST_Controller_Testcase {
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/widget-types', $routes );
		$this->assertCount( 1, $routes['/gc/v2/widget-types'] );
		$this->assertArrayHasKey( '/gc/v2/widget-types/(?P<id>[a-zA-Z0-9_-]+)', $routes );
		$this->assertCount( 1, $routes['/gc/v2/widget-types/(?P<id>[a-zA-Z0-9_-]+)'] );
		$this->assertArrayHasKey( '/gc/v2/widget-types/(?P<id>[a-zA-Z0-9_-]+)/encode', $routes );
		$this->assertCount( 1, $routes['/gc/v2/widget-types/(?P<id>[a-zA-Z0-9_-]+)/encode'] );
	}
	...
	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/widget-types' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/widget-types/calendar' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/widget-types' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertGreaterThan( 1, count( $data ) );
		$endpoint = new GC_REST_Widget_Types_Controller;
	...
	public function test_get_item() {
		...
		gc_set_current_user( self::$admin_id );
		$request     = new GC_REST_Request( 'GET', '/gc/v2/widget-types/' . $widget_name );
		$response    = rest_get_server()->dispatch( $request );
		$endpoint    = new GC_REST_Widget_Types_Controller;
	...
	public function test_get_widget_legacy() {
		...
		gc_set_current_user( self::$admin_id );
		$request     = new GC_REST_Request( 'GET', '/gc/v2/widget-types/' . $widget_id );
		$response    = rest_get_server()->dispatch( $request );
		$endpoint    = new GC_REST_Widget_Types_Controller;
	...
	public function test_get_widget_invalid_name() {
		$widget_type = 'fake';
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/widget-types/' . $widget_type );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_widgets_decodes_html_entities() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/widget-types/archives' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	public function test_get_item_wrong_permission() {
		gc_set_current_user( self::$subscriber_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/widget-types/calendar' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_cannot_manage_widgets', $response, 403 );
	}
	...
	protected function check_widget_type_object( $widget_type, $data, $links ) {
		...
		// Test links.
		$this->assertSame( rest_url( 'gc/v2/widget-types' ), $links['collection'][0]['href'] );
	}
	...
	public function test_encode_form_data_with_no_input() {
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'POST', '/gc/v2/widget-types/search/encode' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	public function test_encode_form_data_with_instance() {
		...
		$request->set_param(
			...
			array(
				...
				'hash'    => gc_hash( serialize( array( 'title' => 'Test title' ) ) ),
			)
		);
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	public function test_encode_form_data_with_form_data() {
		...
		$this->assertSameSets(
			array(
				...
				'hash'    => gc_hash( serialize( array( 'title' => 'Updated title' ) ) ),
	...
	public function test_encode_form_data_no_raw() {
		global $gc_widget_factory;
		gc_set_current_user( self::$admin_id );
		$gc_widget_factory->widgets['GC_Widget_Search']->widget_options['show_instance_in_rest'] = false;
		$request = new GC_REST_Request( 'POST', '/gc/v2/widget-types/search/encode' );
		$request->set_param(
			'instance',
			array(
		...
		$gc_widget_factory->widgets['GC_Widget_Search']->widget_options['show_instance_in_rest'] = true;
	}
