<?php
/**
 * Unit tests covering GC_REST_Posts meta functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Post_Meta_Fields extends GC_Test_REST_TestCase {
	protected static $gc_meta_keys_saved;
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$gc_meta_keys_saved = isset( $GLOBALS['gc_meta_keys'] ) ? $GLOBALS['gc_meta_keys'] : array();
		self::$post_id            = $factory->post->create();
		self::$cpt_post_id        = $factory->post->create( array( 'post_type' => 'cpt' ) );
	}

	public static function gcTearDownAfterClass() {
		$GLOBALS['gc_meta_keys'] = self::$gc_meta_keys_saved;
		gc_delete_post( self::$post_id, true );
		gc_delete_post( self::$cpt_post_id, true );
	...
	public function test_get_subtype_meta_value( $post_type, $meta_key, $single, $in_post_type ) {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/%s/%d', $endpoint, $post_id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_set_subtype_meta_value( $post_type, $meta_key, $single, $in_post_type, $can_write ) {
		...
		$request = new GC_REST_Request( 'POST', sprintf( '/gc/v2/%s/%d', $endpoint, $post_id ) );
	...
	public function test_object_single() {
		...
		$this->assertSame( 'GeChiUI', $data['meta']['object']['project'] );

		$meta = get_post_meta( self::$post_id, 'object', true );
		$this->assertArrayHasKey( 'project', $meta );
		$this->assertSame( 'GeChiUI', $meta['project'] );
	}
	...
	public function test_object_multiple() {
		...
		$this->assertSame( 'GeChiUI', $data['meta']['object'][0]['project'] );
		...
		$this->assertSame( 'GeChiUI', $meta[0]['project'] );
	...
	public function test_array_single() {
		...
		$this->assertSame( array( 'GeChiUI', 'bbPress' ), $data['meta']['list'] );

		$meta = get_post_meta( self::$post_id, 'list', true );
		$this->assertSame( array( 'GeChiUI', 'bbPress' ), $meta );
	}
	...
	public function test_php_jsonserializable_object_returns_value() {
		...
		update_post_meta( self::$post_id, 'object', new JsonSerializable_Object( array( 'project' => 'GeChiUI' ) ) );
		...
		$this->assertArrayHasKey( 'object', $data['meta'] );
		$this->assertSame( array( 'project' => 'GeChiUI' ), $data['meta']['object'] );
	}
	...
	public function test_update_multi_meta_value_object() {
		...
		$this->assertCount( 2, $data['meta']['object'] );
		$this->assertSame( array( 'project' => 'GeChiUI' ), $data['meta']['object'][0] );
		$this->assertSame( array( 'project' => 'BuddyPress' ), $data['meta']['object'][1] );

		$meta = get_post_meta( self::$post_id, 'object' );
		$this->assertCount( 2, $meta );
		$this->assertSame( array( 'project' => 'GeChiUI' ), $meta[0] );
	...
	public function test_update_multi_meta_value_array() {
		...
		add_post_meta( self::$post_id, 'list', array( 'GeChiUI', 'bbPress' ) );
		...
		$this->assertSame( array( 'GeChiUI', 'bbPress' ), $data['meta']['list'][0] );
		$this->assertSame( array( 'BuddyPress' ), $data['meta']['list'][1] );

		$meta = get_post_meta( self::$post_id, 'list' );
		$this->assertCount( 2, $meta );
		$this->assertSame( array( 'GeChiUI', 'bbPress' ), $meta[0] );
	...
	public function test_update_array_of_ints_meta_stored_strings_are_updated() {
		...
		$response = rest_get_server()->dispatch( new GC_REST_Request( 'GET', sprintf( '/gc/v2/posts/%d', self::$post_id ) ) );
	...
	public function test_set_default_in_schema() {
		...
		$response = rest_do_request( '/gc/v2/posts/' . self::$post_id );
