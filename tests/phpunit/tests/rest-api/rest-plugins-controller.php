<?php
/**
 * Unit tests covering GC_REST_Plugins_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_REST_Plugins_Controller_Test extends GC_Test_REST_Controller_Testcase {

	const BASE        = '/gc/v2/plugins';
	...
	public function tear_down() {
		if ( file_exists( GC_PLUGIN_DIR . '/test-plugin/test-plugin.php' ) ) {
			$this->rmdir( GC_PLUGIN_DIR . '/test-plugin' );
		}
	...
	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', self::BASE );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', self::BASE . '/' . self::PLUGIN );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		...
		gc_set_current_user( self::$super_admin );

		$response = rest_do_request( self::BASE );
		$this->assertSame( 200, $response->get_status() );

		$items = gc_list_filter( $response->get_data(), array( 'plugin' => self::PLUGIN ) );
	...
	public function test_get_items_search() {
		...
		$request = new GC_REST_Request( 'GET', self::BASE );
		...
		$response = rest_do_request( $request );
		$this->assertCount( 1, gc_list_filter( $response->get_data(), array( 'plugin' => self::PLUGIN ) ) );
	}
	...
	public function test_get_items_status() {
		...
		$response = rest_do_request( $request );
		$this->assertCount( 0, gc_list_filter( $response->get_data(), array( 'plugin' => self::PLUGIN ) ) );
	}
	...
	public function test_get_items_status_multiple() {
		...
		$this->assertGreaterThan( 0, count( gc_list_filter( $response->get_data(), array( 'plugin' => self::PLUGIN ), 'NOT' ) ) );
		$this->assertCount( 1, gc_list_filter( $response->get_data(), array( 'plugin' => self::PLUGIN ) ) );
	}
	...
	public function test_cannot_get_item_if_plugins_menu_not_available() {
		...
		$request  = new GC_REST_Request( 'GET', self::BASE . '/' . self::PLUGIN );
		$response = rest_do_request( $request );
	...
	public function test_create_item() {
		...
		$request = new GC_REST_Request( 'POST', self::BASE );
	...
	public function test_create_item_wdotorg_unreachable() {
		...
		$this->prevent_requests_to_host( 'api.gechiui.com' );
	...
	public function test_create_item_unknown_plugin() {
		...
		add_filter(
			...
			static function() {
				 ...
				 * https://api.gechiui.com/plugins/info/1.2/?action=plugin_information&request%5Bslug%5D=alex-says-this-block-definitely-doesnt-exist&request%5Bfields%5D%5Bsections%5D=0&request%5Bfields%5D%5Blanguage_packs%5D=1&request%5Blocale%5D=en_US&request%5Bgc_version%5D=5.9
				 */
				return array(
					'headers'  => array(),
					'response' => array(
	...
	public function test_update_item() {
		...
		$request  = new GC_REST_Request( 'PUT', self::BASE . '/' . self::PLUGIN );
		$response = rest_do_request( $request );
	...
	public function test_delete_item() {
		...
		$request  = new GC_REST_Request( 'DELETE', self::BASE . '/' . self::PLUGIN );
		$response = rest_do_request( $request );
		...
		$this->assertFileDoesNotExist( GC_PLUGIN_DIR . '/' . self::PLUGIN_FILE );
	}
	...
	public function test_prepare_item() {
		...
		$endpoint = new GC_REST_Plugins_Controller();
		$response = $endpoint->prepare_item_for_response( $item, new GC_REST_Request( 'GET', self::BASE . '/' . self::PLUGIN ) );
	...
	protected function check_get_plugin_data( $data, $network_only = false ) {
		...
		$this->assertSame( 'https://www.gechiui.com/plugins/test-plugin/', $data['plugin_uri'] );
		$this->assertSame( 'www.GeChiUI.com', $data['author'] );
		$this->assertSame( 'https://www.gechiui.com/', $data['author_uri'] );
		$this->assertSame( "My 'Cool' Plugin", $data['description']['raw'] );
		$this->assertSame( 'My &#8216;Cool&#8217; Plugin <cite>By <a href="https://www.gechiui.com/">www.GeChiUI.com</a>.</cite>', $data['description']['rendered'] );
		$this->assertSame( $network_only, $data['network_only'] );
		$this->assertSame( '5.6.0', $data['requires_php'] );
		$this->assertSame( '5.4.0', $data['requires_gc'] );
 ...
 * Plugin URI: https://www.gechiui.com/plugins/test-plugin/
 * Description: My 'Cool' Plugin
 * Version: 1.5.4
 * Author: www.GeChiUI.com
 ...
 */
PHP;
		gc_mkdir_p( GC_PLUGIN_DIR . '/test-plugin' );
		file_put_contents( GC_PLUGIN_DIR . '/test-plugin/test-plugin.php', $php );
	}
	 ...
	 */
	private function prevent_requests_to_host( $blocked_host = 'api.gechiui.com' ) {
		add_filter(
			'pre_http_request',
			static function ( $return, $args, $url ) use ( $blocked_host ) {
				if ( @parse_url( $url, PHP_URL_HOST ) === $blocked_host ) {
					return new GC_Error( 'plugins_api_failed', "An expected error occurred connecting to $blocked_host because of a unit test", "cURL error 7: Failed to connect to $blocked_host port 80: Connection refused" );
