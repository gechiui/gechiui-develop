<?php
/**
 * GC_REST_Block_Types_Controller tests.
 *
 * @package GeChiUI
 * @subpackage REST_API
 * @since 5.5.0
...
/**
 * Tests for GC_REST_Block_Types_Controller.
 *
 * @since 5.5.0
 *
 * @covers GC_REST_Block_Types_Controller
 *
 * @group restapi-blocks
 * @group restapi
 */
class REST_Block_Type_Controller_Test extends GC_Test_REST_Controller_Testcase {
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/block-types', $routes );
		$this->assertCount( 1, $routes['/gc/v2/block-types'] );
		$this->assertArrayHasKey( '/gc/v2/block-types/(?P<namespace>[a-zA-Z0-9_-]+)', $routes );
		$this->assertCount( 1, $routes['/gc/v2/block-types/(?P<namespace>[a-zA-Z0-9_-]+)'] );
		$this->assertArrayHasKey( '/gc/v2/block-types/(?P<namespace>[a-zA-Z0-9_-]+)/(?P<name>[a-zA-Z0-9_-]+)', $routes );
		$this->assertCount( 1, $routes['/gc/v2/block-types/(?P<namespace>[a-zA-Z0-9_-]+)/(?P<name>[a-zA-Z0-9_-]+)'] );
	}
	...
	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/block-types' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/block-types/fake/test' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/block-types/fake' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertCount( 1, $data );
		$block_type = GC_Block_Type_Registry::get_instance()->get_registered( $block_name );
	...
	public function test_get_item() {
		$block_name = 'fake/test';
		gc_set_current_user( self::$admin_id );
		$request    = new GC_REST_Request( 'GET', '/gc/v2/block-types/' . $block_name );
		$response   = rest_get_server()->dispatch( $request );
		$block_type = GC_Block_Type_Registry::get_instance()->get_registered( $block_name );
	...
	public function test_get_block_invalid_name() {
		...
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/block-types/' . $block_type );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_items_wrong_permission() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/block-types' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_wrong_permission() {
		gc_set_current_user( self::$subscriber_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/block-types/fake/test' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_block_type_cannot_view', $response, 403 );
	}
	...
	public function test_prepare_item() {
		$registry = new GC_Block_Type_Registry;
		$settings = array(
			'icon'            => 'text',
		...
		$endpoint   = new GC_REST_Block_Types_Controller;
	 ...
	 * @since 5.5.0
	 *
	 * @param GC_Block_Type $block_type Sample block type.
	...
	protected function check_block_type_object( $block_type, $data, $links ) {
		...
		// Test links.
		$this->assertSame( rest_url( 'gc/v2/block-types' ), $links['collection'][0]['href'] );
		$this->assertSame( rest_url( 'gc/v2/block-types/' . $block_type->name ), $links['self'][0]['href'] );
