<?php
/**
 * GC_REST_Menus_Controller tests
 *
 * @package GeChiUI
 * @subpackage REST_API
 * @since 5.9.0
 ...
 * @group restapi
 *
 * @coversDefaultClass GC_REST_Menus_Controller
 */
class Tests_REST_GcRestMenusController extends GC_Test_REST_Controller_Testcase {
	/**
	 * @var int
	 */
	public $menu_id;
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/menus', $routes );
		$this->assertArrayHasKey( '/gc/v2/menus/(?P<id>[\d]+)', $routes );
	}
	...
	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/menus' );
		...
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/menus/' . $tag1 );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSameSets( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		...
		$request = new GC_REST_Request( 'GET', '/gc/v2/menus' );
	...
	public function test_get_item() {
		gc_set_current_user( self::$admin_id );
		$nav_menu_id = gc_update_nav_menu_object(
			0,
			array(
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/menus/' . $nav_menu_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_create_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/menus' );
		...
		$data    = $response->get_data();
		$this->assertStringContainsString( '/gc/v2/menus/' . $data['id'], $headers['Location'] );
	...
	public function test_update_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/menus/' . $this->menu_id );
	...
	public function test_delete_item() {
		...
		$request = new GC_REST_Request( 'DELETE', '/gc/v2/menus/' . $term->term_id );
	...
	public function test_prepare_item() {
		...
		$term = get_term_by( 'id', $nav_menu_id, self::TAXONOMY );
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/menus/' . $term->term_id );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	public function test_get_item_links() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/menus/%d', $nav_menu_id ) );
		$response = rest_get_server()->dispatch( $request );

		$links = $response->get_links();
		$this->assertArrayHasKey( 'https://api.w.org/menu-location', $links );

		$location_url = rest_url( '/gc/v2/menu-locations/foo' );
	...
	public function test_get_item_no_permission() {
		gc_set_current_user( 0 );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/menus/' . $this->menu_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_cannot_view', $response, 401 );
	}
	...
	public function test_it_allows_batch_requests_when_updating_menus() {
		...
		$route_options = $rest_server->get_route_options( '/gc/v2/menus/(?P<id>[\d]+)' );
	...
	/**
	 * @param GC_Term $term GC_Term object.
	...
	protected function check_taxonomy_term( $term, $data, $links ) {
		...
		$this->assertSame( add_query_arg( 'menus', $term->term_id, rest_url( 'gc/v2/menu-items' ) ), $links['https://api.w.org/post_type'][0]['href'] );
	}
}
