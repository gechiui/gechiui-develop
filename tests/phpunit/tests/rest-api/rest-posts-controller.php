<?php
/**
 * Unit tests covering GC_REST_Posts_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Posts_Controller extends GC_Test_REST_Post_Type_Controller_Testcase {
	...
	public static function gcTearDownAfterClass() {
		...
		gc_delete_post( self::$post_id, true );
	...
	public function set_up() {
		...
		add_filter( 'rest_pre_dispatch', array( $this, 'gcSetUpBeforeRequest' ), 10, 3 );
		add_filter( 'posts_clauses', array( $this, 'save_posts_clauses' ), 10, 2 );
	}

	public function gcSetUpBeforeRequest( $result, $server, $request ) {
	...
	public function assertPostsClause( $clause, $pattern ) {
		global $gcdb;
		$expected_clause = str_replace( '{posts}', $gcdb->posts, $pattern );
		$this->assertCount( 1, $this->posts_clauses );
		$this->assertSame( $expected_clause, $gcdb->remove_placeholder_escape( $this->posts_clauses[0][ $clause ] ) );
	}
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/posts', $routes );
		$this->assertCount( 2, $routes['/gc/v2/posts'] );
		$this->assertArrayHasKey( '/gc/v2/posts/(?P<id>[\d]+)', $routes );
		$this->assertCount( 3, $routes['/gc/v2/posts/(?P<id>[\d]+)'] );
	}

	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/posts' );
		...
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/posts/' . self::$post_id );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( array( 'v1' => true ), $data['endpoints'][0]['allow_batch'] );
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
	...
	public function test_registered_get_item_params() {
		$request  = new GC_REST_Request( 'OPTIONS', sprintf( '/gc/v2/posts/%d', self::$post_id ) );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$keys     = array_keys( $data['endpoints'][0]['args'] );
		sort( $keys );
	...
	public function test_allow_header_sent_on_options_request() {
		...
		gc_set_current_user( self::$editor_id );
	...
	public function test_get_items_author_query() {
		...
		$this->assertSameSets( array( self::$editor_id, self::$author_id ), gc_list_pluck( $data, 'author' ) );
	...
	public function test_get_items_orderby_parent_query() {
		...
		$request = new GC_REST_Request( 'GET', '/gc/v2/pages' );
		$request->set_param( 'include', array( $id1, $id2, $id3 ) );
	...
	public function test_get_items_multiple_statuses_custom_role_one_invalid_query() {
		...
		gc_set_current_user( self::$private_reader_id );
	...
	public function test_get_items_tags_query() {
		$id1 = self::$post_id;
		$tag = gc_insert_term( 'My Tag', 'post_tag' );

		gc_set_object_terms( $id1, array( $tag['term_id'] ), 'post_tag' );
	...
	public function test_get_items_tags_and_categories_query() {
		...
		$id2      = $this->factory->post->create( array( 'post_status' => 'publish' ) );
		$tag      = gc_insert_term( 'My Tag', 'post_tag' );
		$category = gc_insert_term( 'My Category', 'category' );

		gc_set_object_terms( $id1, array( $tag['term_id'] ), 'post_tag' );
		gc_set_object_terms( $id2, array( $tag['term_id'] ), 'post_tag' );
		gc_set_object_terms( $id1, array( $category['term_id'] ), 'category' );
	...
	public function test_get_items_tags_or_categories_query() {
		...
		gc_set_object_terms( $id1, array( $tag['term_id'] ), 'post_tag' );
		gc_set_object_terms( $id2, array( $category['term_id'] ), 'category' );
	...
	public function test_get_items_tags_or_categories_exclude_query() {
		...
		gc_set_object_terms( $id2, array( $tag['term_id'] ), 'post_tag' );
		gc_set_object_terms( $id2, array( $category['term_id'] ), 'category' );
		gc_set_object_terms( $id3, array( $category['term_id'] ), 'category' );
	...
	public function test_get_items_pagination_headers() {
		...
		$response = rest_get_server()->dispatch( $request );
		$headers  = $response->get_headers();
		$this->assertSame( $total_posts, $headers['X-GC-Total'] );
		$this->assertSame( $total_pages, $headers['X-GC-TotalPages'] );
		$next_link = add_query_arg(
			array(
				'page' => 2,
			),
			rest_url( '/gc/v2/posts' )
		);
	...
	public function test_get_item() {
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/posts/%d', self::$post_id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_links() {
		...
		$this->assertSame( rest_url( '/gc/v2/posts/' . self::$post_id ), $links['self'][0]['href'] );
		$this->assertSame( rest_url( '/gc/v2/posts' ), $links['collection'][0]['href'] );
		$this->assertArrayNotHasKey( 'embeddable', $links['self'][0]['attributes'] );

		$this->assertSame( rest_url( '/gc/v2/types/' . get_post_type( self::$post_id ) ), $links['about'][0]['href'] );
		...
		$this->assertSame( rest_url( '/gc/v2/posts/' . self::$post_id . '/revisions' ), $links['version-history'][0]['href'] );
		$this->assertSame( 0, $links['version-history'][0]['attributes']['count'] );
		$this->assertArrayNotHasKey( 'predecessor-version', $links );

		$attachments_url = rest_url( '/gc/v2/media' );
		...
		$tags_url = add_query_arg( 'post', self::$post_id, rest_url( '/gc/v2/tags' ) );
		$this->assertSame( $tags_url, $tag_link['href'] );

		$category_url = add_query_arg( 'post', self::$post_id, rest_url( '/gc/v2/categories' ) );
	...
	public function test_get_item_links_predecessor() {
		...
		$revisions  = gc_get_post_revisions( self::$post_id );
		...
		$this->assertSame( rest_url( '/gc/v2/posts/' . self::$post_id . '/revisions/' . $revision_1->ID ), $links['predecessor-version'][0]['href'] );
	...
	public function test_get_item_links_no_author() {
		...
		$response = rest_get_server()->dispatch( $request );
		$links    = $response->get_links();
		$this->assertSame( rest_url( '/gc/v2/users/' . self::$author_id ), $links['author'][0]['href'] );
	}
	...
	public function test_get_post_draft_status_not_authenticated() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/posts/%d', $draft_id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_post_draft_edit_context() {
		...
		$draft_id = $this->factory->post->create(
			array(
				...
				'post_content' => '<!-- gc:latest-posts {"displayPostContent":true} /--> <!-- gc:latest-posts {"displayPostContent":true,"displayPostContentRadio":"full_post"} /-->',
			)
		);
	...
	public function test_get_post_invalid_id() {
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_post_with_password() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/posts/%d', $post_id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_post_with_password_using_password() {
		...
		$data = $response->get_data();
		$this->assertSame( gcautop( $post->post_content ), $data['content']['rendered'] );
		$this->assertTrue( $data['content']['protected'] );
		$this->assertSame( gcautop( $post->post_excerpt ), $data['excerpt']['rendered'] );
		$this->assertTrue( $data['excerpt']['protected'] );
	}
	...
	public function test_get_post_should_not_have_block_version_when_context_view() {
		$post_id = $this->factory->post->create(
			array(
				'post_content' => '<!-- gc:core/separator -->',
			)
		);
	...
	public function test_prepare_item_limit_fields() {
		...
		$endpoint = new GC_REST_Posts_Controller( 'post' );
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/posts/%d', self::$post_id ) );
		$request->set_param( 'context', 'edit' );
	...
	public function test_create_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/posts' );
	...
	public function test_create_item_with_template() {
		...
		$GLOBALS['gc_rest_server']->override_by_default = true;
		$controller                                     = new GC_REST_Posts_Controller( 'post' );
		$controller->register_routes();
		$GLOBALS['gc_rest_server']->override_by_default = false;
	...
	public function test_create_item_with_template_none() {
		...
		add_filter( 'theme_post_templates', array( $this, 'filter_theme_post_templates' ) );
		update_post_meta( self::$post_id, '_gc_page_template', 'post-my-test-template.php' );

		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/posts/%d', self::$post_id ) );
		$params  = $this->set_post_data(
			array(
				'template' => '',
			)
	...
	public function test_rest_create_item() {
		...
		$params = $this->set_post_data();
		$request->set_body( gc_json_encode( $params ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_create_post_private_without_permission() {
		...
		// Flush capabilities, https://core.trac.gechiui.com/ticket/28374
		$user->get_role_caps();
	 ...
	 *
	 * https://core.trac.gechiui.com/ticket/38610
	 */
	...
	public function test_create_update_post_with_featured_media() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/posts/' . $new_post->ID );
		$params  = $this->set_post_data(
			array(
				'featured_media' => 0,
			)
	...
	public function test_create_post_with_categories() {
		...
		$category = gc_insert_term( 'Test Category', 'category' );
	...
	public function test_create_post_with_categories_as_csv() {
		...
		$category  = gc_insert_term( 'Chicken', 'category' );
		$category2 = gc_insert_term( 'Ribs', 'category' );
	...
	public function test_rest_update_post_with_empty_date() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/posts/%d', $post_id ) );
		$request->add_header( 'content-type', 'application/json' );
		$params = $this->set_post_data(
			array(
	...
	public function test_update_post_invalid_id() {
		...
		$request  = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/posts/%d', REST_TESTS_IMPOSSIBLY_HIGH_NUMBER ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_update_post_invalid_route() {
		...
		$request  = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/pages/%d', self::$post_id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_empty_post_date_gmt_shimmed_using_post_date() {
		...
		// Need to set dates using gcdb directly because `gc_update_post` and
		// `gc_insert_post` have additional validation on dates.
		$post_id = $this->factory->post->create();
		$gcdb->update(
			$gcdb->posts,
			array(
	...
	public function test_update_post_with_categories() {
		...
		$request = new GC_REST_Request( 'GET', $args['rest_route'] );
	...
	public function test_update_post_with_empty_categories() {
		...
		$category = gc_insert_term( 'Test Category', 'category' );
		gc_set_object_terms( self::$post_id, $category['term_id'], 'category' );
	 ...
	 *
	 * @covers GC_REST_Posts_Controller::check_template
	 * @ticket 39996
	 */
	public function test_update_item_with_same_template_that_no_longer_exists() {
		gc_set_current_user( self::$editor_id );

		update_post_meta( self::$post_id, '_gc_page_template', 'post-my-invalid-template.php' );
	...
	public function verify_post_roundtrip( $input = array(), $expected_output = array() ) {
		...
		// Compare expected API output to GC internal values.
		...
		// Update the post.
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/posts/%d', $actual_output['id'] ) );
		foreach ( $input as $name => $value ) {
			$request->set_param( $name, $value );
		}
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_delete_item() {
		...
		$request = new GC_REST_Request( 'DELETE', sprintf( '/gc/v2/posts/%d', $post_id ) );
	...
	public function test_delete_post_invalid_id() {
		...
		$request  = new GC_REST_Request( 'DELETE', '/gc/v2/posts/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_delete_post_invalid_post_type() {
		...
		$request  = new GC_REST_Request( 'DELETE', '/gc/v2/posts/' . $page_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_delete_post_without_permission() {
		...
		$request  = new GC_REST_Request( 'DELETE', sprintf( '/gc/v2/posts/%d', self::$post_id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_register_post_type_invalid_controller() {
		...
		$routes = rest_get_server()->get_routes();
		$this->assertArrayNotHasKey( '/gc/v2/invalid-controller', $routes );
	...
	public function test_status_array_enum_args() {
		$request         = new GC_REST_Request( 'GET', '/gc/v2' );
		$response        = rest_get_server()->dispatch( $request );
		$data            = $response->get_data();
		$list_posts_args = $data['routes']['/gc/v2/posts']['endpoints'][0]['args'];
	...
	public function test_get_additional_field_registration() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . $post_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertArrayHasKey( 'my_custom_int', $response->data );

		$request = new GC_REST_Request( 'POST', '/gc/v2/posts/' . $post_id );
		$request->set_body_params(
			array(
				'my_custom_int' => 123,
			)
		...
		$gc_rest_additional_fields = array();
	}
	...
	public function test_additional_field_update_errors() {
		...
		$request = new GC_REST_Request( 'POST', sprintf( '/gc/v2/posts/%d', self::$post_id ) );
		$request->set_body_params(
			array(
	...
	public function additional_field_update_callback( $value, $post ) {
		if ( 'returnError' === $value ) {
			return new GC_Error( 'rest_invalid_param', 'Testing an error.', array( 'status' => 400 ) );
		}
		update_post_meta( $post->ID, 'my_custom_int', $value );
	}

	public function test_publish_action_ldo_registered() {
		$response = rest_get_server()->dispatch( new GC_REST_Request( 'OPTIONS', '/gc/v2/posts' ) );
		$data     = $response->get_data();
		$schema   = $data['schema'];

		$this->assertArrayHasKey( 'links', $schema );
		$publish = gc_list_filter( $schema['links'], array( 'rel' => 'https://api.w.org/action-publish' ) );
	...
	public function test_sticky_action_ldo_registered_for_posts() {
		...
		$this->assertArrayHasKey( 'links', $schema );
		$publish = gc_list_filter( $schema['links'], array( 'rel' => 'https://api.w.org/action-sticky' ) );

		$this->assertCount( 1, $publish, 'LDO found on schema.' );
	}

	public function test_sticky_action_ldo_not_registered_for_non_posts() {
		$response = rest_get_server()->dispatch( new GC_REST_Request( 'OPTIONS', '/gc/v2/pages' ) );
		$data     = $response->get_data();
		$schema   = $data['schema'];
	...
	public function test_author_action_ldo_registered_for_post_types_with_author_support() {
		...
		$this->assertArrayHasKey( 'links', $schema );
		$publish = gc_list_filter( $schema['links'], array( 'rel' => 'https://api.w.org/action-assign-author' ) );
	...
	public function test_term_action_ldos_registered() {
		...
		$this->assertArrayHasKey( 'links', $schema );
		$rels = array_flip( gc_list_pluck( $schema['links'], 'rel' ) );
	...
	public function test_action_links_only_available_in_edit_context() {
		...
		$request = new GC_REST_Request( 'GET', "/gc/v2/posts/{$post}" );
	...
	public function test_generated_permalink_template_generated_slug_for_non_viewable_posts() {
		...
		$request = new GC_REST_Request( 'GET', '/gc/v2/private-post/' . $post_id );
		$request->set_param( 'context', 'edit' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 200, $response->get_status() );
	...
	public function test_putting_same_publish_date_does_not_remove_floating_date() {
		...
		$get = new GC_REST_Request( 'GET', "/gc/v2/posts/{$post->ID}" );
		$get->set_query_params( array( 'context' => 'edit' ) );

		$get      = rest_get_server()->dispatch( $get );
		$get_body = $get->get_data();

		$put = new GC_REST_Request( 'PUT', "/gc/v2/posts/{$post->ID}" );
	...
	public function test_get_for_post_type_returns_posts_controller_if_custom_class_not_specified() {
		...
		$this->assertInstanceOf(
			GC_REST_Posts_Controller::class,
	...
	public function test_get_for_post_type_returns_provided_controller_class() {
		$this->assertInstanceOf(
			GC_REST_Blocks_Controller::class,
			get_post_type_object( 'gc_block' )->get_rest_controller()
		);
	}
	...
	public function test_get_for_post_type_returns_null_for_controller_class_mismatch() {
		register_post_type(
			...
			array(
				'show_in_rest'          => true,
				'rest_controller_class' => GC_REST_Posts_Controller::class,
				'rest_controller'       => new GC_REST_Terms_Controller( 'category' ),
			)
		);
	...
	public function test_rest_post_type_item_schema_filter_add_property_triggers_doing_it_wrong() {
		...
		$this->setExpectedIncorrectUsage( 'GC_REST_Posts_Controller::get_item_schema' );
	 ...
	 * will trigger a gcdb error for testing purposes.
	 */
