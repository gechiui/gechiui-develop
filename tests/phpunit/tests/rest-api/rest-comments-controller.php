<?php
/**
 * Unit tests covering GC_REST_Comments_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Comments_Controller extends GC_Test_REST_Controller_Testcase {
	...
	public static function gcTearDownAfterClass() {
		...
		gc_delete_post( self::$private_id, true );
		gc_delete_post( self::$password_id, true );
		gc_delete_post( self::$draft_id, true );
		gc_delete_post( self::$trash_id, true );
		gc_delete_post( self::$approved_id, true );
		gc_delete_post( self::$hold_id, true );

		// Remove comments for pagination tests.
		foreach ( self::$comment_ids as $comment_id ) {
			gc_delete_comment( $comment_id, true );
		}
	}

	public function set_up() {
		parent::set_up();
		$this->endpoint = new GC_REST_Comments_Controller;
		if ( is_multisite() ) {
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/comments', $routes );
		$this->assertCount( 2, $routes['/gc/v2/comments'] );
		$this->assertArrayHasKey( '/gc/v2/comments/(?P<id>[\d]+)', $routes );
		$this->assertCount( 3, $routes['/gc/v2/comments/(?P<id>[\d]+)'] );
	}

	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/comments' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/comments/' . self::$approved_id );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSame( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		$request = new GC_REST_Request( 'GET', '/gc/v2/comments' );
	...
	public function test_get_items_with_password() {
		...
		$this->assertContains( $password_comment, gc_list_pluck( $collection_data, 'id' ) );
	}
	...
	public function test_get_items_with_password_without_post() {
		...
		$collection_data = $response->get_data();
		$this->assertNotContains( $password_comment, gc_list_pluck( $collection_data, 'id' ) );
	}
	...
	public function test_get_items_without_private_post_permission() {
		...
		$collection_data = $response->get_data();
		$this->assertNotContains( $private_comment, gc_list_pluck( $collection_data, 'id' ) );
	}
	...
	public function test_get_items_with_private_post_permission() {
		...
		$collection_data = $response->get_data();
		$this->assertContains( $private_comment, gc_list_pluck( $collection_data, 'id' ) );
	}
	...
	public function test_get_items_with_invalid_post() {
		...
		$collection_data = $response->get_data();
		$this->assertNotContains( $comment_id, gc_list_pluck( $collection_data, 'id' ) );
	...
	public function test_get_items_with_invalid_post_permission() {
		...
		$collection_data = $response->get_data();
		$this->assertContains( $comment_id, gc_list_pluck( $collection_data, 'id' ) );
	...
	public function test_get_comments_pagination_headers() {
		...
		$this->assertSame( $total_comments, $headers['X-GC-Total'] );
		$this->assertSame( $total_pages, $headers['X-GC-TotalPages'] );
		$next_link = add_query_arg(
			array(
				'page' => 2,
			),
			rest_url( '/gc/v2/comments' )
		);
		...
		$headers  = $response->get_headers();
		$this->assertSame( $total_comments, $headers['X-GC-Total'] );
		$this->assertEquals( $total_pages, $headers['X-GC-TotalPages'] );
		$prev_link = add_query_arg(
			array(
				'page' => $total_pages,
			),
	...
	public function test_get_item() {
		$request = new GC_REST_Request( 'GET', sprintf( '/gc/v2/comments/%d', self::$approved_id ) );
	...
	public function test_prepare_item_limit_fields() {
		...
		$endpoint = new GC_REST_Comments_Controller;
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/comments/%d', self::$approved_id ) );
		$request->set_param( 'context', 'edit' );
	...
	public function test_get_comment_invalid_id() {
		$request = new GC_REST_Request( 'GET', '/gc/v2/comments/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
	...
	public function test_get_comment_invalid_context() {
		...
		$request = new GC_REST_Request( 'GET', sprintf( '/gc/v2/comments/%s', self::$approved_id ) );
		$request->set_param( 'context', 'edit' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_forbidden_context', $response, 401 );
	}
	...
	public function test_get_comment_invalid_post_id() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/comments/' . $comment_id );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_comment_not_approved() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/comments/%d', self::$hold_id ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_cannot_read', $response, 401 );
	}
	...
	public function test_get_comment_with_children_link() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/comments/%s', $comment_id_1 ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	...
	public function test_get_comment_with_password_without_edit_post_permission() {
		...
		$request  = new GC_REST_Request( 'GET', sprintf( '/gc/v2/comments/%s', $password_comment ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_create_item() {
		...
		$request = new GC_REST_Request( 'POST', '/gc/v2/comments' );
	...
	public function return_premade_error() {
		return new GC_Error( 'test_rest_premade_error', "I'm sorry, I thought he was a party robot.", array( 'status' => 418 ) );
	}
	...
	public function test_create_comment_without_type() {
		...
		$collection = new GC_REST_Request( 'GET', '/gc/v2/comments' );
	...
	public function test_create_item_pull_author_info() {
		...
		$author = new GC_User( self::$author_id );
		$params = array(
			'post'    => self::$post_id,
	...
	public function test_update_item() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/comments/%d', self::$approved_id ) );
		$request->add_header( 'content-type', 'application/json' );
		$request->set_body( gc_json_encode( $params ) );
	...
	public function test_update_comment_date( $params, $results ) {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/comments/%d', $comment_id ) );
		if ( isset( $params['date'] ) ) {
			$request->set_param( 'date', $params['date'] );
		}
		if ( isset( $params['date_gmt'] ) ) {
	...
	public function test_update_comment_invalid_id() {
		...
		$request = new GC_REST_Request( 'PUT', '/gc/v2/comments/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$request->add_header( 'content-type', 'application/json' );
		$request->set_body( gc_json_encode( $params ) );
	...
	public function test_update_comment_invalid_permission() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/comments/%d', self::$hold_id ) );
		$request->add_header( 'content-type', 'application/json' );
		$request->set_body( gc_json_encode( $params ) );
	...
	public function test_update_comment_when_can_moderate_comments() {
		gc_set_current_user( self::$moderator_id );
	...
	public function test_update_comment_private_post_invalid_permission() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/comments/%d', $private_comment_id ) );
		$request->add_header( 'content-type', 'application/json' );
		$request->set_body( gc_json_encode( $params ) );
	...
	public function test_update_comment_with_children_link() {
		...
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/comments/%s', $child_comment ) );
	 ...
	 * @ticket 39732
	 */
	public function test_update_comment_is_gc_error() {
		gc_set_current_user( self::$admin_id );
		...
		add_filter( 'gc_update_comment_data', array( $this, '_gc_update_comment_data_filter' ), 10, 3 );
		...
		remove_filter( 'gc_update_comment_data', array( $this, '_gc_update_comment_data_filter' ), 10, 3 );
	 ...
	 */
	public function _gc_update_comment_data_filter( $data, $comment, $commentarr ) {
		return new GC_Error( 'comment_wrong', 'gc_update_comment_data filter fails for this comment.', array( 'status' => 500 ) );
	}
	...
	public function verify_comment_roundtrip( $input = array(), $expected_output = array() ) {
		...
		// Update the comment.
		$request = new GC_REST_Request( 'PUT', sprintf( '/gc/v2/comments/%d', $actual_output['id'] ) );
		foreach ( $input as $name => $value ) {
			$request->set_param( $name, $value );
		}
		// FIXME At least one value must change, or update fails.
		// See https://core.trac.gechiui.com/ticket/38700
	...
	public function test_delete_item() {
		...
		$request = new GC_REST_Request( 'DELETE', sprintf( '/gc/v2/comments/%d', $comment_id ) );
	...
	public function test_delete_comment_invalid_id() {
		...
		$request  = new GC_REST_Request( 'DELETE', sprintf( '/gc/v2/comments/%d', REST_TESTS_IMPOSSIBLY_HIGH_NUMBER ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_comment_invalid_id', $response, 404 );
	}
	...
	public function test_delete_comment_without_permission() {
		...
		$request  = new GC_REST_Request( 'DELETE', sprintf( '/gc/v2/comments/%d', self::$approved_id ) );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_delete_child_comment_link() {
		...
		$request  = new GC_REST_Request( 'DELETE', sprintf( '/gc/v2/comments/%s', $child_comment ) );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	...
	public function test_get_item_schema_show_avatar() {
		...
		$request    = new GC_REST_Request( 'OPTIONS', '/gc/v2/users' );
		$response   = rest_get_server()->dispatch( $request );
		$data       = $response->get_data();
		$properties = $data['schema']['properties'];
	...
	public function test_get_additional_field_registration() {
		...
		$request  = new GC_REST_Request( 'GET', '/gc/v2/comments/' . self::$approved_id );
		$response = rest_get_server()->dispatch( $request );
		$this->assertArrayHasKey( 'my_custom_int', $response->data );

		$request = new GC_REST_Request( 'POST', '/gc/v2/comments/' . self::$approved_id );
	...
	public function test_additional_field_update_errors() {
		...
		$request = new GC_REST_Request( 'POST', sprintf( '/gc/v2/comments/%d', self::$approved_id ) );
		$request->set_body_params(
			array(
	...
	protected function check_comment_data( $data, $context, $links ) {
		...
		$this->assertSame( gcautop( $comment->comment_content ), $data['content']['rendered'] );
		...
		if ( $comment->comment_post_ID ) {
			$this->assertSame( rest_url( '/gc/v2/posts/' . $comment->comment_post_ID ), $links['up'][0]['href'] );
		}
