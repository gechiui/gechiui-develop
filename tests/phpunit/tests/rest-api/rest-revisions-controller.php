<?php
/**
 * Unit tests covering GC_REST_Revisions_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Revisions_Controller extends GC_Test_REST_Controller_Testcase {
	...
	public static function gcTearDownAfterClass() {
		...
		gc_delete_post( self::$page_id, true );
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/gc/v2/posts/(?P<parent>[\d]+)/revisions', $routes );
		$this->assertArrayHasKey( '/gc/v2/posts/(?P<parent>[\d]+)/revisions/(?P<id>[\d]+)', $routes );
		$this->assertArrayHasKey( '/gc/v2/pages/(?P<parent>[\d]+)/revisions', $routes );
		$this->assertArrayHasKey( '/gc/v2/pages/(?P<parent>[\d]+)/revisions/(?P<id>[\d]+)', $routes );
	}

	public function test_context_param() {
		// Collection.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/posts/' . self::$post_id . '/revisions' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSameSets( array( 'view', 'edit', 'embed' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single.
		$request  = new GC_REST_Request( 'OPTIONS', '/gc/v2/posts/' . self::$post_id . '/revisions/' . $this->revision_1->ID );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
		$this->assertSame( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertSameSets( array( 'view', 'edit', 'embed' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . self::$post_id . '/revisions' );
		$response = rest_get_server()->dispatch( $request );
		$data     = $response->get_data();
	...
	public function test_get_items_missing_parent() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER . '/revisions' );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_items_invalid_parent_post_type() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . self::$page_id . '/revisions' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_post_invalid_parent', $response, 404 );
	}
	...
	public function test_get_item() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . self::$post_id . '/revisions/' . $this->revision_id1 );
		$response = rest_get_server()->dispatch( $request );
		$this->assertSame( 200, $response->get_status() );
	...
	public function test_get_item_missing_parent() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER . '/revisions/' . $this->revision_id1 );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_post_invalid_parent', $response, 404 );
	}
	...
	public function test_get_item_invalid_parent_post_type() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . self::$page_id . '/revisions/' . $this->revision_id1 );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_post_invalid_parent', $response, 404 );
	}
	...
	public function test_delete_item() {
		gc_set_current_user( self::$editor_id );
		$request = new GC_REST_Request( 'DELETE', '/gc/v2/posts/' . self::$post_id . '/revisions/' . $this->revision_id1 );
	...
	public function test_prepare_item_limit_fields() {
		gc_set_current_user( self::$editor_id );
		$request  = new GC_REST_Request( 'GET', '/gc/v2/posts/' . self::$post_id . '/revisions/' . $this->revision_id1 );
		$endpoint = new GC_REST_Revisions_Controller( 'post' );
	...
	public function test_create_item() {
		$request  = new GC_REST_Request( 'POST', '/gc/v2/posts/' . self::$post_id . '/revisions' );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}

	public function test_update_item() {
		$request  = new GC_REST_Request( 'POST', '/gc/v2/posts/' . self::$post_id . '/revisions/' . $this->revision_id1 );
		$response = rest_get_server()->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}
	...
	protected function check_get_revision_response( $response, $revision ) {
		...
		$parent_controller = new GC_REST_Posts_Controller( $parent->post_type );
		...
		$this->assertSame( rest_url( '/gc/v2/' . $parent_base . '/' . $revision->post_parent ), $links['parent'][0]['href'] );
	}
	...
	public function test_get_item_sets_up_postdata() {
		...
		$post           = get_post();
		$parent_post_id = gc_is_post_revision( $post->ID );
	...
	public function test_get_items_pagination_header_of_the_first_page() {
		...
		$rest_route  = '/gc/v2/posts/' . self::$post_id . '/revisions';
		$per_page    = 2;
		$total_pages = (int) ceil( $this->total_revisions / $per_page );
		$page        = 1;  // First page.

		$request = new GC_REST_Request( 'GET', $rest_route );
		...
		$this->assertSame( $this->total_revisions, $headers['X-GC-Total'] );
