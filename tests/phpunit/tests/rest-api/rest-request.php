<?php
/**
 * Unit tests covering GC_REST_Request functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class Tests_REST_Request extends GC_UnitTestCase {
	public $request;

	public function set_up() {
		parent::set_up();

		$this->request = new GC_REST_Request();
	}
	...
	public function test_header() {
		$value = 'application/x-gc-example';
	...
	public function test_header_multiple() {
		$value1 = 'application/x-gc-example-1';
		$value2 = 'application/x-gc-example-2';
	...
	public static function content_type_provider() {
		return array(
			// Check basic parsing.
			array( 'application/x-gc-example', 'application/x-gc-example', 'application', 'x-gc-example', '' ),
			array( 'application/x-gc-example; charset=utf-8', 'application/x-gc-example', 'application', 'x-gc-example', 'charset=utf-8' ),
	 ...
	 * other methods `GC_REST_Request` needs to parse the body for us.
	 *
	...
	public function test_parameters_for_json_put() {
		...
		$this->request->set_body( gc_json_encode( $data ) );
		...
		$this->request->set_attributes(
			array(
				'args' => array(
					...
					'failparam'    => array(
						'sanitize_callback' => array( $this, '_return_gc_error_on_validate_callback' ),
					),
				),
			)
		);
				...
				'args' => array(
					'failparam' => array(
						'sanitize_callback' => static function () {
							$error = new GC_Error( 'invalid', 'Invalid.' );
		...
		$this->request->set_attributes(
			array(
				'args' => array(
					'failparam' => array(
						'sanitize_callback' => static function () {
							return new GC_Error( 'invalid', 'Invalid.', 'mydata' );
						},
					),
				),
			)
			...
			array(
				'args' => array(
					...
					'someotherparams' => array(
						'validate_callback' => array( $this, '_return_gc_error_on_validate_callback' ),
					),
				),
			)
		);
	...
	public function _return_gc_error_on_validate_callback() {
	...
	public function data_from_url() {
		return array(
			array(
				...
				'original_url'        => 'http://' . GC_TESTS_DOMAIN . '/gc-json/gc/v2/posts/1?foo=bar',
			),
			array(
				'permalink_structure' => '',
				'original_url'        => 'http://' . GC_TESTS_DOMAIN . '/index.php?rest_route=%2Fgc%2Fv2%2Fposts%2F1&foo=bar',
			),
		);
	}
	...
	public function test_from_url( $permalink_structure, $original_url ) {
		...
		$url = add_query_arg( 'foo', 'bar', rest_url( '/gc/v2/posts/1' ) );
		$this->assertSame( $original_url, $url );
		$request = GC_REST_Request::from_url( $url );
		$this->assertInstanceOf( 'GC_REST_Request', $request );
		$this->assertSame( '/gc/v2/posts/1', $request->get_route() );
		$this->assertSameSets(
			array(
				'foo' => 'bar',
			),
	...
	public function test_from_url_invalid( $permalink_structure ) {
		update_option( 'permalink_structure', $permalink_structure );
		$using_site = site_url( '/gc/v2/posts/1' );
		$request    = GC_REST_Request::from_url( $using_site );
		$this->assertFalse( $request );

		$using_home = home_url( '/gc/v2/posts/1' );
		$request    = GC_REST_Request::from_url( $using_home );
		$this->assertFalse( $request );
	}
	...
	public function test_route_level_validate_callback() {
		...
		$error    = new GC_Error( 'error_code', __( 'Error Message' ), array( 'status' => 400 ) );
