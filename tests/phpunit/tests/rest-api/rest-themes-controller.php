<?php
/**
 * Unit tests covering GC_REST_Themes_Controller functionality.
 *
 * @package GeChiUI
 * @subpackage REST API
 */
 ...
 * @group restapi
 */
class GC_Test_REST_Themes_Controller extends GC_Test_REST_Controller_Testcase {
	/**
	 * Subscriber user ID.
	 *
	 * @since 5.0.0
	 ...
	 */
	protected static $themes_route = '/gc/v2/themes';
	 ...
	 * @return GC_REST_Response The request's response.
	 */
	protected function perform_active_theme_request( $method = 'GET' ) {
		$request = new GC_REST_Request( $method, self::$themes_route );
	 ...
	 * @since 5.0.0
	 *
	 * @param GC_REST_Response $response Current REST API response.
	 */
	protected function check_get_theme_response( $response ) {
		...
		$this->assertArrayHasKey( 'X-GC-Total', $headers );
		$this->assertSame( 1, $headers['X-GC-Total'] );
		$this->assertArrayHasKey( 'X-GC-TotalPages', $headers );
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$current_theme  = gc_get_theme();
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey(
			sprintf( '%s/(?P<stylesheet>%s)', self::$themes_route, GC_REST_Themes_Controller::PATTERN ),
			$routes
		);
	}
	...
	public function test_get_items_inactive() {
		...
		$request = new GC_REST_Request( 'GET', self::$themes_route );
		...
		$this->assertContains( 'twentytwenty', gc_list_pluck( $data, 'stylesheet' ) );
		$this->assertNotContains( get_stylesheet(), gc_list_pluck( $data, 'stylesheet' ) );
	}
	...
	public function test_get_items_active_and_inactive() {
		...
		$this->assertContains( 'twentytwenty', gc_list_pluck( $data, 'stylesheet' ) );
		$this->assertContains( get_stylesheet(), gc_list_pluck( $data, 'stylesheet' ) );
	}
	...
	public function test_get_item_schema() {
		...
		$this->assertArrayHasKey( 'requires_gc', $properties );
		...
		$this->assertArrayHasKey( 'title-tag', $theme_supports );
		$this->assertArrayHasKey( 'gc-block-styles', $theme_supports );
	 ...
	 * @ticket 49906
	 */
	public function test_theme_requires_gc() {
		$response = self::perform_active_theme_request();
		$result   = $response->get_data();
		$this->assertArrayHasKey( 'requires_gc', $result[0] );
		$this->assertSame( '5.3', $result[0]['requires_gc'] );
	}
	...
	public function test_theme_theme_uri() {
		...
		$this->assertSame( 'http://www.gechiui.com/?search=1&term=2', $result[0]['theme_uri']['raw'] );
		$this->assertSame( 'http://www.gechiui.com/?search=1&#038;term=2', $result[0]['theme_uri']['rendered'] );
	}
	...
	public function test_theme_supports_editor_color_palette_array() {
		...
		add_theme_support( 'editor-color-palette', array( $gechiui_blue ) );
		$response = self::perform_active_theme_request();
		$result   = $response->get_data();
		$this->assertArrayHasKey( 'theme_supports', $result[0] );
		$this->assertSame( array( $gechiui_blue ), $result[0]['theme_supports']['editor-color-palette'] );
	}
	...
	public function test_theme_supports_custom_logo() {
		...
		add_theme_support( 'custom-logo', $gechiui_logo );
		$response = self::perform_active_theme_request();
		$result   = $response->get_data();
		$this->assertArrayHasKey( 'theme_supports', $result[0] );
		$this->assertSame( $gechiui_logo, $result[0]['theme_supports']['custom-logo'] );
	}
	...
	public function test_theme_supports_custom_header() {
		...
		add_theme_support( 'custom-header', $gechiui_header );
		$response = self::perform_active_theme_request();
		$result   = $response->get_data();
		$this->assertArrayHasKey( 'theme_supports', $result[0] );

		$expected = array_diff_key( $gechiui_header, array_flip( $excluded ) );
	...
	public function test_theme_can_manage_title_tag() {
		...
		$_gc_theme_features['title-tag'] = true;
		$response                        = self::perform_active_theme_request();
		$result                          = $response->get_data();
		$this->assertArrayHasKey( 'theme_supports', $result[0] );
	 ...
	 * @ticket 49037
	 */
	public function test_theme_no_gc_block_styles() {
		remove_theme_support( 'gc-block-styles' );
		$response = self::perform_active_theme_request();
		$result   = $response->get_data();
		$this->assertArrayHasKey( 'theme_supports', $result[0] );
		$this->assertArrayHasKey( 'gc-block-styles', $result[0]['theme_supports'] );
		$this->assertFalse( $result[0]['theme_supports']['gc-block-styles'] );
	}

	/**
	 * @ticket 49037
	 */
	public function test_theme_gc_block_styles_optin() {
		remove_theme_support( 'gc-block-styles' );
		add_theme_support( 'gc-block-styles' );
		$response = self::perform_active_theme_request();
		$result   = $response->get_data();
		$this->assertArrayHasKey( 'theme_supports', $result[0] );
		$this->assertTrue( $result[0]['theme_supports']['gc-block-styles'] );
	}
	...
	public function test_get_item() {
		gc_set_current_user( self::$admin_id );
		$route    = sprintf( '%s/%s', self::$themes_route, GC_DEFAULT_THEME );
		$request  = new GC_REST_Request( 'GET', $route );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_no_permission() {
		gc_set_current_user( self::$subscriber_id );
		$request  = new GC_REST_Request( 'GET', self::$themes_route . '/' . GC_DEFAULT_THEME );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_active_item_no_permission() {
		gc_set_current_user( self::$subscriber_id );
		$request  = new GC_REST_Request( 'GET', self::$themes_route . '/' . get_stylesheet() );
		$response = rest_get_server()->dispatch( $request );
	...
	public function test_get_item_invalid() {
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', self::$themes_route . '/invalid' );
		$response = rest_get_server()->dispatch( $request );
	 ...
	 * @ticket 54596
	 * @covers GC_REST_Themes_Controller::get_item
	 *
	...
	public function test_get_item_non_subdir_theme( $theme_dir, $expected_name ) {
		gc_set_current_user( self::$admin_id );
		$request  = new GC_REST_Request( 'GET', self::$themes_route . $theme_dir );
	...
	public function test_can_support_further_routes() {
		register_rest_route(
			'gc/v2',
			sprintf( '/themes/(?P<stylesheet>%s)//test', GC_REST_Themes_Controller::PATTERN ),
			array(
