<?php
/**
 * Test cases for the `gc_privacy_send_personal_data_export_email()` function.
 *
 * @package GeChiUI
...
/**
 * Tests_Privacy_gcPrivacySendPersonalDataExportEmail class.
 *
 * @group privacy
 * @covers ::gc_privacy_send_personal_data_export_email
 *
 * @since 4.9.6
 */
class Tests_Privacy_gcPrivacySendPersonalDataExportEmail extends GC_UnitTestCase {
	/**
	 * Request ID.
	 *
	 * @since 4.9.6
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		_gc_privacy_account_request_confirmed( self::$request_id );
	}
	...
	public function test_function_should_send_export_link_to_requester() {
		...
		$export_file_name = 'gc-personal-data-file-Wv0RfMnGIkl4CFEDEEkSeIdfLmaUrLsl.zip';
		$export_file_url  = $exports_url . $export_file_name;
		update_post_meta( self::$request_id, '_export_file_name', $export_file_name );

		$email_sent = gc_privacy_send_personal_data_export_email( self::$request_id );
	...
	public function test_function_should_error_when_request_id_invalid() {
		$request_id = 0;
		$email_sent = gc_privacy_send_personal_data_export_email( $request_id );
	 ...
	 * @since 4.9.6
	 */
	public function test_return_gc_error_when_send_fails() {
		add_filter( 'gc_mail_from', '__return_empty_string' ); // Cause `gc_mail()` to return false.
		$email_sent = gc_privacy_send_personal_data_export_email( self::$request_id );
	...
	public function test_export_expiration_should_be_filterable() {
		add_filter( 'gc_privacy_export_expiration', array( $this, 'modify_export_expiration' ) );
		gc_privacy_send_personal_data_export_email( self::$request_id );
	...
	public function test_email_address_of_recipient_should_be_filterable() {
		add_filter( 'gc_privacy_personal_data_email_to', array( $this, 'filter_email_address' ) );
		gc_privacy_send_personal_data_export_email( self::$request_id );
	...
	public function test_email_subject_should_be_filterable() {
		add_filter( 'gc_privacy_personal_data_email_subject', array( $this, 'filter_email_subject' ) );
		gc_privacy_send_personal_data_export_email( self::$request_id );
	...
	public function test_email_content_should_be_filterable() {
		add_filter( 'gc_privacy_personal_data_email_content', array( $this, 'modify_email_content' ), 10, 2 );
		gc_privacy_send_personal_data_export_email( self::$request_id );
	...
	public function test_email_headers_should_be_filterable() {
		add_filter( 'gc_privacy_personal_data_email_headers', array( $this, 'modify_email_headers' ) );
		gc_privacy_send_personal_data_export_email( self::$request_id );
	...
	public function test_email_content_should_be_filterable_using_email_data() {
		add_filter( 'gc_privacy_personal_data_email_content', array( $this, 'modify_email_content_with_email_data' ), 10, 3 );
		gc_privacy_send_personal_data_export_email( self::$request_id );
	...
	public function test_should_send_personal_data_export_email_in_user_locale_when_site_is_not_en_us() {
		update_option( 'GCLANG', 'es_ES' );
		switch_to_locale( 'es_ES' );

		update_user_meta( self::$request_user->ID, 'locale', 'de_DE' );
		gc_set_current_user( self::$admin_user->ID );
	...
	public function test_should_send_personal_data_export_email_in_site_locale() {
		...
		$request_id = gc_create_user_request( 'export-user-not-registered@example.com', 'export_personal_data' );

		_gc_privacy_account_request_confirmed( self::$request_id );
		gc_privacy_send_personal_data_export_email( $request_id );
