<?php
/**
 * Test cases for the `gc_privacy_process_personal_data_export_page()` function.
 *
 * @package GeChiUI
...
/**
 * Tests_Privacy_gcPrivacyProcessPersonalDataExportPage class.
 *
 * @group privacy
 * @covers ::gc_privacy_process_personal_data_export_page
 *
 * @since 5.2.0
 */
class Tests_Privacy_gcPrivacyProcessPersonalDataExportPage extends GC_UnitTestCase {
	/**
	 * Request ID.
	 *
	 * @since 5.2.0
	...
	/**
	 * Export data stored on the `gc_privacy_personal_data_export_file` action hook.
	 *
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$exports_url          = gc_privacy_exports_url();
		self::$export_file_name     = 'gc-personal-data-file-Wv0RfMnGIkl4CFEDEEkSeIdfLmaUrLsl.zip';
		self::$export_file_url      = self::$exports_url . self::$export_file_name;
		self::$request_id           = gc_create_user_request( self::$requester_email, 'export_personal_data' );
	...
	public function set_up() {
		...
		// Avoid writing export files to disk. Using `GC_Filesystem_MockFS` is blocked by #44204.
		remove_action( 'gc_privacy_personal_data_export_file', 'gc_privacy_generate_personal_data_export_file', 10 );

		// Register our custom data exporters, very late, so we can override other unrelated exporters.
		add_filter( 'gc_privacy_personal_data_exporters', array( $this, 'filter_register_custom_personal_data_exporters' ), 9999 );

		// Set Ajax context for `gc_send_json()` and `gc_die()`.
		add_filter( 'gc_doing_ajax', '__return_true' );

		// Set up a `gc_die()` ajax handler that throws an exception, to be able to get
		// the error message from `gc_send_json_error( 'some message here' )`,
		// called by `gc_privacy_process_personal_data_export_page()`.
	 ...
	 * @ticket 44233
	 *
	 * @dataProvider data_gc_privacy_process_personal_data_export_page
	 *
	 * @param string|array $expected_response The response from the personal data exporter for the given test.
	 */
	public function test_gc_privacy_process_personal_data_export_page( $expected_response ) {
		$actual_response = gc_privacy_process_personal_data_export_page(
			$expected_response,
	...
	/**
	 * Provide test cases for `test_gc_privacy_process_personal_data_export_page()`.
	 *
	 * @since 5.2.0
	 *
	 * @return array {
	 ...
	 * }
	 */
	public function data_gc_privacy_process_personal_data_export_page() {
		return array(
	...
	public function test_send_error_when_invalid_request_id( $send_as_email ) {
		...
		gc_privacy_process_personal_data_export_page(
			$response,
			self::$exporter_index_last,
			self::$requester_email,
			self::$page_index_last,
	...
	public function test_send_error_when_invalid_request_action_name( $send_as_email ) {
		...
		$request_id = gc_create_user_request( self::$requester_email, 'remove_personal_data' );
	...
	public function test_add_post_meta_with_groups_data_only_available_when_export_file_generated( $send_as_email ) {
		...
		add_action( 'gc_privacy_personal_data_export_file', array( $this, 'action_callback_to_get_export_groups_data' ) );
	...
	public function test_send_error_on_last_page_of_last_exporter_when_mail_delivery_fails() {
		// Cause `gc_mail()` to return false, to simulate mail delivery failure. Filter removed in tearDown.
		add_filter( 'gc_mail_from', '__return_empty_string' );
	...
	/**
	 * A callback for the `gc_privacy_personal_data_export_file` action that stores the
