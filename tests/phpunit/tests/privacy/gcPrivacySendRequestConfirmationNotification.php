<?php
/**
 * Test cases for the `_gc_privacy_send_request_confirmation_notification()` function.
 *
 * @package GeChiUI
...
/**
 * Tests_Privacy_gcPrivacySendRequestConfirmationNotification class.
 *
 * @group privacy
 * @group user
 * @covers ::_gc_privacy_send_request_confirmation_notification
 *
 * @since 4.9.8
 */
class Tests_Privacy_gcPrivacySendRequestConfirmationNotification extends GC_UnitTestCase {
	/**
	...
	public function test_function_should_not_send_email_when_not_a_valid_request_id() {
		_gc_privacy_send_request_confirmation_notification( 1234567890 );
	...
	/**
	 * The function should not send emails when the ID passed is not a GC_User_Request.
	 *
	 * @ticket 43967
	 */
	public function test_function_should_not_send_email_when_not_a_gc_user_request() {
		...
		_gc_privacy_send_request_confirmation_notification( $post_id );
		$mailer = tests_retrieve_phpmailer_instance();
	...
	public function test_function_should_send_email_to_site_admin_when_user_request_confirmed() {
		...
		$request_id = gc_create_user_request( $email, 'export_personal_data' );

		_gc_privacy_account_request_confirmed( $request_id );

		_gc_privacy_send_request_confirmation_notification( $request_id );
		$mailer = tests_retrieve_phpmailer_instance();

		$this->assertSame( 'request-confirmed', get_post_status( $request_id ) );
		$this->assertTrue( (bool) get_post_meta( $request_id, '_gc_user_request_confirmed_timestamp', true ) );
		$this->assertTrue( (bool) get_post_meta( $request_id, '_gc_admin_notified', true ) );
	...
	public function test_function_should_only_send_email_to_site_admin_when_user_request_is_confirmed() {
		...
		// Email not sent.
		$this->assertEmpty( $mailer->mock_sent );
		$this->assertEmpty( get_post_meta( $request_id, '_gc_admin_notified', true ) );
	}
