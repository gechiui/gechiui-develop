<?php
/**
 * Test cases for the `_gc_privacy_send_erasure_fulfillment_notification()` function.
 *
 * @package GeChiUI
...
/**
 * Tests_Privacy_gcPrivacySendErasureFulfillmentNotification class.
 *
 * @group privacy
 * @covers ::_gc_privacy_send_erasure_fulfillment_notification
 *
 * @since 5.1.0
 */
class Tests_Privacy_gcPrivacySendErasureFulfillmentNotification extends GC_UnitTestCase {
	/**
	 * Request ID.
	 *
	 * @since 5.1.0
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		self::$request_id = gc_create_user_request( self::$requester_email, 'remove_personal_data' );
		gc_update_post(
			array(
				'ID'          => self::$request_id,
	...
	public function test_should_send_email_no_privacy_policy() {
		...
		_gc_privacy_send_erasure_fulfillment_notification( self::$request_id );
		...
		$this->assertStringContainsString( gc_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), $mailer->get_sent()->body );
		...
		$this->assertTrue( (bool) get_post_meta( self::$request_id, '_gc_user_notified', true ) );
	}
	...
	public function test_should_send_email_with_privacy_policy() {
		...
		update_option( 'gc_page_for_privacy_policy', $privacy_policy );
	...
	public function test_should_send_email_only_once() {
		...
		$this->assertTrue( metadata_exists( 'post', self::$request_id, '_gc_user_notified' ) );
	}
	...
	public function test_should_not_send_email_when_passed_invalid_request_id() {
		_gc_privacy_send_erasure_fulfillment_notification( 1234567890 );
	...
	public function test_should_not_send_email_when_not_user_request() {
		...
		_gc_privacy_send_erasure_fulfillment_notification( $post_id );
		$mailer = tests_retrieve_phpmailer_instance();
	...
	public function test_should_not_send_email_when_request_not_completed() {
		...
		$this->assertEmpty( $mailer->mock_sent );
		$this->assertFalse( metadata_exists( 'post', self::$request_id, '_gc_user_notified' ) );
	}
	...
	public function test_should_send_fulfillment_email_in_site_locale() {
		...
		$request_id = gc_create_user_request( 'erase-user-not-registered@example.com', 'remove_personal_data' );
		gc_update_post(
			array(
				'ID'          => $request_id,
				'post_status' => 'request-completed',
		...
		_gc_privacy_send_erasure_fulfillment_notification( $request_id );
		$mailer = tests_retrieve_phpmailer_instance();
