<?php
/**
 * Define a class to test `gc_privacy_generate_personal_data_export_file()`.
 *
 * @package GeChiUI
...
/**
 * Test cases for `gc_privacy_generate_personal_data_export_file()`.
 *
 * @group privacy
 * @covers ::gc_privacy_generate_personal_data_export_file
 *
 * @since 5.2.0
 */
class Tests_Privacy_gcPrivacyGeneratePersonalDataExportFile extends GC_UnitTestCase {
	/**
	 ...
	 * @since 5.2.0
	 *
	 * @param GC_UnitTest_Factory $factory The base factory object.
	 */
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		self::$export_request_id = gc_create_user_request( 'export-requester@example.com', 'export_personal_data' );
		update_post_meta( self::$export_request_id, '_export_data_grouped', array() );
		self::$exports_dir = gc_privacy_exports_dir();
	}

	/**
	 * Set up the test fixture.
	 *
	 * Override `gc_die()`, pretend to be Ajax, and suppress `E_WARNING`s.
	 *
	 * @since 5.2.0
	 */
	public function set_up() {
		...
		add_action( 'gc_privacy_personal_data_export_file_created', array( $this, 'action_gc_privacy_personal_data_export_file_created' ) );
	 ...
	 *
	 * Remove the `gc_die()` override, restore error reporting.
	 *
	 * @since 5.2.0
	 */
	 ...
	 */
	public function action_gc_privacy_personal_data_export_file_created( $archive_name ) {
	...
	private function remove_exports_dir() {
		 ...
		 * if an incorrect value is returned to the `gc_privacy_exports_dir` filter.
		 */
		if ( is_file( untrailingslashit( self::$exports_dir ) ) ) {
			gc_delete_file( untrailingslashit( self::$exports_dir ) );
	...
	public function test_rejects_remove_requests() {
		$request_id = gc_create_user_request( 'removal-requester@example.com', 'remove_personal_data' );
		...
		gc_privacy_generate_personal_data_export_file( $request_id );
	}
	...
	public function test_invalid_request_id() {
		$this->expectException( 'GCDieException' );
		$this->expectOutputString( '{"success":false,"data":"Invalid request ID when generating personal data export file."}' );
		gc_privacy_generate_personal_data_export_file( 123456789 );
	}
	...
	public function test_rejects_requests_with_bad_email_addresses() {
		$request_id = gc_create_user_request( 'bad-email-requester@example.com', 'export_personal_data' );
	...
	public function test_detect_cannot_create_folder() {
		...
		gc_privacy_generate_personal_data_export_file( self::$export_request_id );
	}
	...
	public function test_doing_it_wrong_for_export_data_grouped_invalid_type( $groups ) {
		...
		$this->setExpectedIncorrectUsage( 'gc_privacy_generate_personal_data_export_file' );
	...
	public function data_export_data_grouped_invalid_type() {
		return array(
			...
			array( false ),
			array( new stdClass() ),
			array( serialize( array( 10, 'GeChiUI', null, true, false ) ) ),
			array(
				json_encode(
					array(
						'user' => array(
	...
	public function test_html_contents( $groups, array $expected_content = array() ) {
		...
		$request    = gc_get_user_request( self::$export_request_id );
	 ...
	 * - Run `gc_privacy_generate_personal_data_export_file()`.
	...
	public function test_should_generate_json_error_when_json_encoding_fails() {
		...
		$this->assertFalse( gc_json_encode( $metadata ) );
