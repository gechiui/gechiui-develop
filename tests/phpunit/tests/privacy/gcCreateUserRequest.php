<?php
/**
 * Test the `gc_create_user_request()` function.
 *
 * @package GeChiUI
...
/**
 * Tests_Privacy_gcCreateUserRequest class.
 *
 * @group privacy
 * @covers ::gc_create_user_request
 *
 * @since 5.2.0
 */
class Tests_Privacy_gcCreateUserRequest extends GC_UnitTestCase {
	/**
	 * Request ID.
	 *
	 * @since 5.2.0
	...
	/**
	 * Ensure a GC_Error is returned when an invalid email is passed.
	 *
	 * @ticket 44707
	 */
	public function test_invalid_email() {
		$actual = gc_create_user_request( 'not-a-valid-email', 'export_personal_data' );
	...
	/**
	 * Ensure a GC_Error is returned when no action is passed.
	 *
	 * @ticket 46536
	 */
	public function test_missing_action() {
		$actual = gc_create_user_request( self::$registered_user_email, false );
	...
	/**
	 * Ensure a GC_Error is returned when an invalid action is passed.
	 *
	 * @ticket 44707
	 * @ticket 46536
	 */
	public function test_invalid_action() {
		$actual = gc_create_user_request( self::$registered_user_email, 'invalid_action_name' );
	...
	/**
	 * When there are incomplete requests for a registered user, a GC_Error should be returned.
	 *
	 * @ticket 44707
	 */
	public function test_failure_due_to_incomplete_registered_user() {
		// Second request (duplicated).
		$actual = gc_create_user_request( self::$registered_user_email, 'export_personal_data' );
	...
	/**
	 * When there are incomplete requests for an non-registered user, a GC_Error should be returned.
	 *
	 * @ticket 44707
	 */
	public function test_failure_due_to_incomplete_unregistered_user() {
		...
		// Second request (duplicated).
		$actual = gc_create_user_request( self::$non_registered_user_email, 'export_personal_data' );
	...
	public function test_sanitized_email() {
		$actual = gc_create_user_request( 'some(email<withinvalid\characters@local.test', 'export_personal_data' );
	...
	public function test_sanitized_action_name() {
		$actual = gc_create_user_request( self::$non_registered_user_email, 'export[_person*al_\data' );
	...
	public function test_create_request_registered_user() {
		gc_delete_post( self::$request_id, true );
		...
		$actual = gc_create_user_request( self::$registered_user_email, 'export_personal_data', $test_data );
		...
		$this->assertSame( gc_json_encode( $test_data ), $post->post_content );
	}
	...
	public function test_create_request_unregistered_user() {
		...
		$actual = gc_create_user_request( self::$non_registered_user_email, 'export_personal_data', $test_data );
	...
	/**
	 * Test that an error from `gc_insert_post()` is returned.
	 *
	 * @ticket 44707
	 */
	public function test_gc_error_returned_from_gc_insert_post() {
		gc_delete_post( self::$request_id, true );

		add_filter( 'gc_insert_post_empty_content', '__return_true' );
		$actual = gc_create_user_request( self::$registered_user_email, 'export_personal_data' );
	 ...
	 * @ticket 43890
	 */
	public function test_gc_create_user_request_default_pending_status() {
		$actual = gc_create_user_request( self::$non_registered_user_email, 'export_personal_data' );
		$post   = get_post( $actual );
	 ...
	 * @ticket 43890
	 */
	public function test_gc_create_user_request_pending_status() {
		$actual = gc_create_user_request( self::$non_registered_user_email, 'export_personal_data', array(), 'pending' );
		$post   = get_post( $actual );
	 ...
	 * @ticket 43890
	 */
	public function test_gc_create_user_request_confirmed_status() {
		$actual = gc_create_user_request( self::$non_registered_user_email, 'export_personal_data', array(), 'confirmed' );
		$post   = get_post( $actual );
	...
	/**
	 * Test that the request returns a GC_Error if $status isn't 'pending' or 'confirmed'.
	 *
	 * @ticket 43890
	 */
	public function test_gc_create_user_request_gc_error_status() {
		$actual = gc_create_user_request( self::$non_registered_user_email, 'export_personal_data', array(), 'wrong-status' );
