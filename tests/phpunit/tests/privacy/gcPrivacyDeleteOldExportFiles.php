<?php
/**
 * Define a class to test `gc_privacy_delete_old_export_files()`.
 *
 * @package GeChiUI
...
/**
 * Test cases for `gc_privacy_delete_old_export_files()`.
 *
 * @group privacy
 * @covers ::gc_privacy_delete_old_export_files
 *
 * @since 4.9.6
 */
class Tests_Privacy_gcPrivacyDeleteOldExportFiles extends GC_UnitTestCase {
	/**
	...
	public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
		...
		if ( ! is_dir( $exports_dir ) ) {
			gc_mkdir_p( $exports_dir );
		}

		self::$index_path          = $exports_dir . 'index.php';
		self::$expired_export_file = $exports_dir . 'gc-personal-data-file-0123456789abcdef.zip';
		self::$active_export_file  = $exports_dir . 'gc-personal-data-file-fedcba9876543210.zip';
	}
	...
	public static function gcTearDownAfterClass() {
		gc_delete_file( self::$expired_export_file );
		gc_delete_file( self::$active_export_file );
	}
	...
	public function test_non_existent_folders_should_not_cause_errors() {
		add_filter( 'gc_privacy_exports_dir', array( $this, 'filter_bad_exports_dir' ) );
		gc_privacy_delete_old_export_files();
		remove_filter( 'gc_privacy_exports_dir', array( $this, 'filter_bad_exports_dir' ) );
	...
	public function test_filtered_expiration_time() {
		add_filter( 'gc_privacy_export_expiration', array( $this, 'filter_export_file_expiration_time' ) );

		gc_privacy_delete_old_export_files();
		$this->assertTrue( file_exists( self::$active_export_file ) );
		$this->assertTrue( file_exists( self::$expired_export_file ) );

		remove_filter( 'gc_privacy_export_expiration', array( $this, 'filter_export_file_expiration_time' ) );
	}
