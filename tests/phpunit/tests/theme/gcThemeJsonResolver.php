...
/**
 * Test GC_Theme_JSON_Resolver class.
 *
 * @package GeChiUI
 * @subpackage Theme
 *
 ...
 * @group themes
 */
class Tests_Theme_gcThemeJsonResolver extends GC_UnitTestCase {
	...
	function filter_db_query( $query ) {
		if ( preg_match( '#post_type = \'gc_global_styles\'#', $query ) ) {
			$this->queries[] = $query;
		}
		return $query;
	}
	...
	public function test_translations_are_applied() {
		...
		$actual = GC_Theme_JSON_Resolver::get_theme_data();

		unload_textdomain( 'block-theme' );
		remove_filter( 'locale', array( $this, 'filter_set_locale_to_polish' ) );

		$this->assertSame( gc_get_theme()->get( 'TextDomain' ), 'block-theme' );
		$this->assertSame(
			array(
				'color'      => array(
					'custom'         => false,
	...
	public function test_switching_themes_recalculates_data() {
		...
		$default = GC_Theme_JSON_Resolver::theme_has_support();

		// Switch to a theme that does have support.
		switch_theme( 'block-theme' );
		$has_theme_json_support = GC_Theme_JSON_Resolver::theme_has_support();
	...
	function test_add_theme_supports_are_loaded_for_themes_without_theme_json() {
		...
		$settings = GC_Theme_JSON_Resolver::get_theme_data()->get_settings();

		remove_theme_support( 'custom-line-height' );
		remove_theme_support( 'editor-color-palette' );

		$this->assertFalse( GC_Theme_JSON_Resolver::theme_has_support() );
	...
	function test_merges_child_theme_json_into_parent_theme_json() {
		...
		$actual_settings   = GC_Theme_JSON_Resolver::get_theme_data()->get_settings();
		...
		$this->assertSame(
			GC_Theme_JSON_Resolver::get_theme_data()->get_custom_templates(),
			array(
				'page-home' => array(
					'title'     => 'Homepage',
					'postTypes' => array( 'page' ),
	...
	function test_get_user_data_from_gc_global_styles_does_not_use_uncached_queries() {
		add_filter( 'query', array( $this, 'filter_db_query' ) );
		$query_count = count( $this->queries );
		for ( $i = 0; $i < 3; $i++ ) {
			GC_Theme_JSON_Resolver::get_user_data_from_gc_global_styles( gc_get_theme() );
			GC_Theme_JSON_Resolver::clean_cached_data();
		}
		$query_count = count( $this->queries ) - $query_count;
		$this->assertEquals( 1, $query_count, 'Only one SQL query should be peformed for multiple invocations of GC_Theme_JSON_Resolver::get_global_styles_from_post()' );

		$user_cpt = GC_Theme_JSON_Resolver::get_user_data_from_gc_global_styles( gc_get_theme() );
		$this->assertEmpty( $user_cpt );

		$user_cpt = GC_Theme_JSON_Resolver::get_user_data_from_gc_global_styles( gc_get_theme(), true );
		...
		$query_count = count( $this->queries ) - $query_count;
		$this->assertEquals( 0, $query_count, 'Unexpected SQL queries detected for the gc_global_style post type' );
