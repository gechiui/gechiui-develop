 ...
 * @group themes
 */
class Tests_Theme_ThemeDir extends GC_UnitTestCase {
	...
	public function test_theme_default() {
		$themes = get_themes();
		$theme  = get_theme( 'GeChiUI Default' );
		$this->assertSame( $themes['GeChiUI Default'], $theme );

		$this->assertNotEmpty( $theme );

		// echo gen_tests_array( 'theme', $theme );

		$this->assertSame( 'GeChiUI Default', $theme['Name'] );
		$this->assertSame( 'GeChiUI Default', $theme['Title'] );
		$this->assertSame( 'The default GeChiUI theme based on the famous <a href="http://binarybonsai.com/kubrick/">Kubrick</a>.', $theme['Description'] );
	...
	public function test_theme_sandbox() {
		...
		$this->assertSame( '<a href="http://andy.gechiui.com/">Andy Skelton</a> &amp; <a href="http://www.plaintxt.org/">Scott Allan Wallick</a>', $theme['Author'] );
		$this->assertSame( '0.6.1-gccom', $theme['Version'] );
	...
	public function test_gc_get_theme_with_non_default_theme_root() {
		$this->assertFalse( gc_get_theme( 'sandbox', $this->theme_root )->errors() );
		$this->assertFalse( gc_get_theme( 'sandbox' )->errors() );
	}
	...
	public function test_get_theme_data_subdir() {
		...
		$this->assertSame( '<a href="http://www.gechiui.com/">Mr. GeChiUI</a>', $theme_data['Author'] );
		$this->assertSame( 'http://www.gechiui.com/', $theme_data['AuthorURI'] );
		$this->assertSame( '0.1', $theme_data['Version'] );
		$this->assertSame( '', $theme_data['Template'] );
		$this->assertSame( 'publish', $theme_data['Status'] );
		$this->assertSame( array(), $theme_data['Tags'] );
		$this->assertSame( 'My Subdir Theme', $theme_data['Title'] );
		$this->assertSame( 'Mr. GeChiUI', $theme_data['AuthorName'] );
	}
	...
	public function test_theme_dir_slashes() {
		$size = count( $GLOBALS['gc_theme_directories'] );

		@mkdir( GC_CONTENT_DIR . '/themes/foo' );
		@mkdir( GC_CONTENT_DIR . '/themes/foo-themes' );

		$this->assertFileExists( GC_CONTENT_DIR . '/themes/foo' );
		$this->assertFileExists( GC_CONTENT_DIR . '/themes/foo-themes' );

		register_theme_directory( '/' );

		$this->assertCount( $size, $GLOBALS['gc_theme_directories'] );
		...
		$this->assertCount( $size + 1, $GLOBALS['gc_theme_directories'] );

		register_theme_directory( GC_CONTENT_DIR . '/foo-themes/' );

		$this->assertCount( $size + 1, $GLOBALS['gc_theme_directories'] );

		foreach ( $GLOBALS['gc_theme_directories'] as $dir ) {
			$this->assertNotEquals( '/', substr( $dir, -1 ) );
		}

		rmdir( GC_CONTENT_DIR . '/themes/foo' );
		rmdir( GC_CONTENT_DIR . '/themes/foo-themes' );
	}
}
