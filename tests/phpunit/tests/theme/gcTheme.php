 ...
 * @group themes
 */
class Tests_Theme_gcTheme extends GC_UnitTestCase {
	...
	public function set_up() {
		...
		$GLOBALS['gc_theme_directories'] = array( $this->theme_root );
	...
	public function test_new_GC_Theme_top_level() {
		$theme = new GC_Theme( 'theme1', $this->theme_root );
	...
	public function test_new_GC_Theme_subdir() {
		$theme = new GC_Theme( 'subdir/theme2', $this->theme_root );

		// Meta.
		$this->assertSame( 'My Subdir Theme', $theme->get( 'Name' ) );
		$this->assertSame( 'http://example.org/', $theme->get( 'ThemeURI' ) );
		$this->assertSame( 'An example theme in a sub directory', $theme->get( 'Description' ) );
		$this->assertSame( 'Mr. GeChiUI', $theme->get( 'Author' ) );
		$this->assertSame( 'http://www.gechiui.com/', $theme->get( 'AuthorURI' ) );
	 ...
	 * @ticket 20313
	 */
	public function test_new_GC_Theme_subdir_bad_root() {
		// This is what get_theme_data() does when you pass it a style.css file for a theme in a subdirectory.
		$theme = new GC_Theme( 'theme2', $this->theme_root . '/subdir' );
	 ...
	 * @ticket 21749
	 */
	public function test_gc_theme_uris_with_spaces() {
		$theme = new GC_Theme( 'theme with spaces', $this->theme_root . '/subdir' );
		...
		// Check that gc_customize_url() uses URL encoding, as it is a query arg (spaces become +).
		$this->assertSame( admin_url( 'customize.php?theme=theme+with+spaces' ), gc_customize_url( 'theme with spaces' ) );
	}
	...
	public function test_display_method_on_get_method_failure() {
		$theme = new GC_Theme( 'nonexistent', $this->theme_root );
	...
	public function test_child_theme_with_itself_as_parent_should_appear_as_broken() {
		$theme  = new GC_Theme( 'child-parent-itself', $this->theme_root );
		$errors = $theme->errors();
	 ...
	 * @group ms-required
	 */
	public function test_gc_theme_network_enable_single_theme() {
		$theme                  = 'testtheme-1';
		$current_allowed_themes = get_site_option( 'allowedthemes' );
		GC_Theme::network_enable_theme( $theme );
	 ...
	 * @group ms-required
	 */
	public function test_gc_theme_network_enable_multiple_themes() {
		$themes                 = array( 'testtheme-2', 'testtheme-3' );
		$current_allowed_themes = get_site_option( 'allowedthemes' );
		GC_Theme::network_enable_theme( $themes );
		$new_allowed_themes = get_site_option( 'allowedthemes' );
		update_site_option( 'allowedthemes', $current_allowed_themes ); // Reset previous value.
	...
	public function test_network_disable_single_theme() {
		...
		GC_Theme::network_disable_theme( $disable_theme );
		$new_allowed_themes = get_site_option( 'allowedthemes' );
		update_site_option( 'allowedthemes', $current_allowed_themes ); // Reset previous value.
	...
	public function test_network_disable_multiple_themes() {
		...
		GC_Theme::network_disable_theme( $disable_themes );
		$new_allowed_themes = get_site_option( 'allowedthemes' );
		update_site_option( 'allowedthemes', $current_allowed_themes ); // Reset previous value.
