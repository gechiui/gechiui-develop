...
if ( is_multisite() ) :
	/**
	 * Tests specific to the filtering of `GC_Theme::get_allowed()` and related functions.
	 *
	 * @group themes
	 * @group multisite
	 */
	class Tests_Theme_gcThemeGetAllowedFilters extends GC_UnitTestCase {
		/**
		 ...
		 */
		public function test_gc_theme_get_allowed_with_allowed_themes_filter() {
			$blog_id = 1;

			$this->default_allowed = GC_Theme::get_allowed( $blog_id );

			add_filter( 'allowed_themes', array( $this, 'filter_allowed_themes' ), 10 );
			$allowed = GC_Theme::get_allowed( $blog_id );
		 ...
		 */
		public function test_gc_theme_get_allowed_with_network_allowed_themes_filter() {
			$blog_id = 1;
		 ...
		 */
		public function test_gc_theme_get_allowed_with_site_allowed_themes_filter() {
			$blog_id = 1;
