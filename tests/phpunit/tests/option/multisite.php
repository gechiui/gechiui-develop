...
if ( is_multisite() ) :
	 ...
	 * @group multisite
	 */
	class Tests_Multisite_Option extends GC_UnitTestCase {
		...
		public function test_site_notoptions() {
			...
			$_notoptions = gc_cache_get( 'notoptions', 'site-options' );
			$this->assertEmpty( $_notoptions );
			$_notoptions1 = gc_cache_get( $notoptions_key, 'site-options' );
			$this->assertEmpty( $_notoptions1 );

			get_site_option( 'burrito' );

			$notoptions = gc_cache_get( 'notoptions', 'site-options' );
			$this->assertEmpty( $notoptions );
			$notoptions1 = gc_cache_get( $notoptions_key, 'site-options' );
