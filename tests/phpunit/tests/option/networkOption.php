 ...
 * @group multisite
 */
class Tests_Option_NetworkOption extends GC_UnitTestCase {
	...
	public function test_get_network_option_sets_notoptions_if_option_found() {
		...
		$original_cache = gc_cache_get( $notoptions_key, 'site-options' );
		if ( false !== $original_cache ) {
			gc_cache_delete( $notoptions_key, 'site-options' );
		}

		// Retrieve any existing option.
		get_network_option( $network_id, 'site_name' );

		$cache = gc_cache_get( $notoptions_key, 'site-options' );
		if ( false !== $original_cache ) {
			gc_cache_set( $notoptions_key, $original_cache, 'site-options' );
		}
	...
	public function test_update_network_option_array_with_object() {
		$array_w_object = array(
			'url'       => 'http://src.gechiui-develop.dev/gc-content/uploads/2016/10/cropped-Blurry-Lights.jpg',
