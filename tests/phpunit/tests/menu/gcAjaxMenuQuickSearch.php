 ...
 * @group menu
 */
class Tests_Menu_GcAjaxMenuQuickSeach extends GC_UnitTestCase {
	...
	public function test_search_returns_results_for_pages() {
		...
		$output = get_echo( '_gc_ajax_menu_quick_search', array( $request ) );
	...
	public function test_search_returns_results_for_published_posts() {
		...
		// This will make sure that GC_Query sets is_admin to true.
	...
	public function test_search_should_return_unassigned_term_items() {
		...
		$request = array(
			'type' => 'quick-search-taxonomy-gctests_tax',
			'q'    => 'foobar',
		);
		$output  = get_echo( '_gc_ajax_menu_quick_search', array( $request ) );
