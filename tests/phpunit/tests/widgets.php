 ...
 * @group widgets
 */
class Tests_Widgets extends GC_UnitTestCase {
	public $sidebar_index;
	public $valid_sidebar;

	public function clean_up_global_scope() {
		global $gc_widget_factory, $gc_registered_sidebars, $gc_registered_widgets, $gc_registered_widget_controls, $gc_registered_widget_updates;
	...
	public function test_register_and_unregister_widget_core_widget() {
		...
		$widget_class = 'GC_Widget_Search';

		register_widget( $widget_class );
		$this->assertArrayHasKey( $widget_class, $gc_widget_factory->widgets );

		unregister_widget( $widget_class );
		$this->assertArrayNotHasKey( $widget_class, $gc_widget_factory->widgets );
	}
	...
	public function test_register_and_unregister_widget_instance() {
		...
		$this->assertEmpty( $gc_widget_factory->widgets );
		$this->assertEmpty( $gc_registered_widgets );
		...
		$this->assertArrayHasKey( 'GC_Widget_Search', $gc_widget_factory->widgets );

		$widget_better_search                              = new GC_Widget_Search();
		...
		$this->assertContains( $widget_better_search, $gc_widget_factory->widgets );

		$widget_best_search                              = new GC_Widget_Search();
		...
		$this->assertContains( $widget_best_search, $gc_widget_factory->widgets );

		$this->assertCount( 3, $gc_widget_factory->widgets );
		$this->assertArrayHasKey( 'GC_Widget_Search', $gc_widget_factory->widgets );
		$this->assertContains( $widget_better_search, $gc_widget_factory->widgets );
		$this->assertContains( $widget_best_search, $gc_widget_factory->widgets );

		$gc_widget_factory->_register_widgets();

		$this->assertArrayHasKey( 'search-2', $gc_registered_widgets );
		$this->assertArrayHasKey( 'better_search-3', $gc_registered_widgets );
		$this->assertArrayHasKey( 'best_search-4', $gc_registered_widgets );
		$this->assertInstanceOf( 'GC_Widget_Search', $gc_registered_widgets['search-2']['callback'][0] );
		$this->assertSame( $widget_better_search, $gc_registered_widgets['better_search-3']['callback'][0] );
		$this->assertSame( $widget_best_search, $gc_registered_widgets['best_search-4']['callback'][0] );

		$this->assertContains( $widget_better_search, $gc_widget_factory->widgets );
		$this->assertContains( $widget_best_search, $gc_widget_factory->widgets );
		$this->assertArrayHasKey( 'GC_Widget_Search', $gc_widget_factory->widgets );
		unregister_widget( 'GC_Widget_Search' );
		unregister_widget( $widget_better_search );
		unregister_widget( $widget_best_search );
		$this->assertNotContains( $widget_better_search, $gc_widget_factory->widgets );
		$this->assertNotContains( $widget_best_search, $gc_widget_factory->widgets );
		$this->assertArrayNotHasKey( 'GC_Widget_Search', $gc_widget_factory->widgets );
	}
	...
	public function test_register_sidebars_single() {
		...
		register_sidebars( 1, array( 'id' => 'gc-unit-test' ) );

		$this->assertArrayHasKey( 'gc-unit-test', $gc_registered_sidebars );
	...
	public function test_register_sidebars_multiple() {
		...
		$result  = array();
		$num     = 3;
		$id_base = 'GC Unit Test';
		register_sidebars( $num, array( 'name' => $id_base . ' %d' ) );

		$names = gc_list_pluck( $gc_registered_sidebars, 'name' );
		for ( $i = 1; $i <= $num; $i++ ) {
	...
	public function test_register_sidebar_with_no_id() {
		...
		$this->assertArrayHasKey( $derived_sidebar_id, $gc_registered_sidebars );
	}
	...
	public function test_unregister_sidebar_registered_with_no_id() {
		...
		$this->assertArrayNotHasKey( $derived_sidebar_id, $gc_registered_sidebars );
	}
	...
	public function test_register_sidebar_with_string_id() {
		...
		$sidebar_id = 'gc-unit-test';
		register_sidebar( array( 'id' => $sidebar_id ) );

		$this->assertArrayHasKey( $sidebar_id, $gc_registered_sidebars );
	}
	...
	public function test_unregister_sidebar_with_string_id() {
		...
		$sidebar_id = 'gc-unit-tests';
		register_sidebar( array( 'id' => $sidebar_id ) );

		unregister_sidebar( $sidebar_id );
		$this->assertArrayNotHasKey( $sidebar_id, $gc_registered_sidebars );
	}
	...
	public function test_register_sidebar_with_after_and_before_sidebar() {
		...
		$this->assertArrayHasKey( $sidebar_id, $gc_registered_sidebars );
		$this->assertStringContainsString( '<div id="%1$s" class="before-sidebar %2$s">', $gc_registered_sidebars[ $sidebar_id ]['before_sidebar'] );
		$this->assertStringContainsString( '</div> <!-- .before-sidebar -->', $gc_registered_sidebars[ $sidebar_id ]['after_sidebar'] );
	...
	public function test_register_sidebar_without_after_and_before_sidebar() {
		...
		$this->assertArrayHasKey( $sidebar_id, $gc_registered_sidebars );
		$this->assertEmpty( $gc_registered_sidebars[ $sidebar_id ]['before_sidebar'] );
		$this->assertEmpty( $gc_registered_sidebars[ $sidebar_id ]['after_sidebar'] );
	...
	public function test_dynamic_sidebar_using_invalid_sidebar_id() {
		register_sidebar( array( 'id' => 'gc-unit-text' ) );
	...
	/**
	 * @see GC_Widget_Search::form()
	 */
	public function test_gc_widget_search_form() {
		$widget = new GC_Widget_Search( 'foo', 'Foo' );
		ob_start();
		$args     = array(
	 ...
	 * @see GC_Widget::form()
	 */
	public function test_gc_widget_form() {
		$widget = new GC_Widget( 'foo', 'Foo' );
		ob_start();
	 ...
	 */
	public function test_gc_widget_constructor() {
		$id_base    = 'foo';
		$name       = 'Foo';
		$foo_widget = new GC_Widget( $id_base, $name );
		...
		$bar_widget      = new GC_Widget( $id_base, $name, $widget_options, $control_options );
	 ...
	 * @ticket 44098
	 * @see GC_Widget::__construct()
	 * @dataProvider data_gc_widget_classname
	 */
	public function test_gc_widget_classname( $expected, $widget_class ) {
	 ...
	 * }
	 */
	public function data_gc_widget_classname() {
		return array(
			array(
				'widget_search',
				'GC_Widget_Search',
	...
	/**
	 * @see GC_Widget::get_field_name()
	 * @dataProvider data_gc_widget_get_field_name
	 */
	public function test_gc_widget_get_field_name( $expected, $value_to_test ) {
		$widget = new GC_Widget( 'foo', 'Foo' );
		$widget->_set( 2 );
	 ...
	 * }
	 */
	public function data_gc_widget_get_field_name() {
		return array(
			array(
				'widget-foo[2][title]',
				'title',
	...
	/**
	 * @see GC_Widget::get_field_id()
	 * @dataProvider data_gc_widget_get_field_id
	 */
	public function test_gc_widget_get_field_id( $expected, $value_to_test ) {
		$widget = new GC_Widget( 'foo', 'Foo' );
		$widget->_set( 2 );
	 ...
	 * }
	 */
	public function data_gc_widget_get_field_id() {
		return array(
			array(
				'widget-foo-2-title',
				'title',
	...
	/**
	 * @see GC_Widget::_register()
	 */
	public function test_gc_widget__register() {
		...
		foreach ( $settings as $widget_number => $instance ) {
			...
			$this->assertArrayHasKey( $widget_id, $gc_registered_widgets );
		}
	}

	// @todo Test GC_Widget::display_callback().

	/**
	 * @ticket 52728
	 */
	function test_widget_display_callback_handles_arrayobject() {
		$widget = new GC_Widget_Text();
	...
	/**
	 * @see GC_Widget::is_preview()
	 */
	public function test_gc_widget_is_preview() {
		global $gc_customize;

		$widget = new GC_Widget( 'foo', 'Foo' );

		$this->assertEmpty( $gc_customize );
		...
		$gc_customize->start_previewing_theme();

		$this->assertTrue( $widget->is_preview() );
	}

	// @todo Test GC_Widget::update_callback().
	// @todo Test GC_Widget::form_callback().
	// @todo Test GC_Widget::_register_one().

	/**
	 * @see GC_Widget::get_settings()
	 */
	public function test_gc_widget_get_settings() {
		global $gc_registered_widgets;
		...
		gc_widgets_init();
		$gc_widget_block = $gc_registered_widgets['block-2']['callback'][0];

		$settings = $gc_widget_block->get_settings();
	...
	/**
	 * @see GC_Widget::save_settings()
	 */
	public function test_gc_widget_save_settings() {
		global $gc_registered_widgets;
		...
		$gc_widget_block->save_settings( $settings );

		$option_value = get_option( $gc_widget_block->option_name );
		$this->assertArrayHasKey( '_multiwidget', $option_value );
	 ...
	 * @see GC_Widget::save_settings()
	 */
	public function test_gc_widget_save_settings_delete() {
		global $gc_registered_widgets;
	...
	/**
	 * @see gc_widget_control()
	 */
	public function test_gc_widget_control() {
		global $gc_registered_widgets;
		...
		$sidebar_args = gc_list_widget_controls_dynamic_sidebar( $control_args );

		ob_start();
		gc_widget_control( ...$sidebar_args );
	...
	public function test_the_widget_custom_before_title_arg() {
		register_widget( 'GC_Widget_Text' );
		...
		unregister_widget( 'GC_Widget_Text' );
	...
	public function test_retrieve_widgets_with_theme_mod() {
		global $sidebars_widgets, $_gc_sidebars_widgets;

		gc_widgets_init();
		$this->register_sidebars( array( 'sidebar-1', 'sidebar-2', 'sidebar-3', 'sidebar-4', 'gc_inactive_widgets' ) );
		...
		$this->assertCount( 5, $sidebars_widgets['gc_inactive_widgets'] );

		$this->assertContains( 'block-2', $sidebars_widgets['gc_inactive_widgets'] );
		$this->assertContains( 'block-3', $sidebars_widgets['gc_inactive_widgets'] );
		$this->assertContains( 'block-4', $sidebars_widgets['gc_inactive_widgets'] );
		$this->assertContains( 'block-5', $sidebars_widgets['gc_inactive_widgets'] );
		$this->assertContains( 'block-6', $sidebars_widgets['gc_inactive_widgets'] );

		// Sidebar_widgets option was updated.
		$this->assertSame( $sidebars_widgets, gc_get_sidebars_widgets() );
	}
	...
	public function test_retrieve_widgets_with_sidebars_widgets_matching_registered_sidebars() {
		...
		gc_widgets_init();
		$this->register_sidebars( array( 'sidebar-1', 'sidebar-2', 'sidebar-3', 'gc_inactive_widgets' ) );
		...
		$this->assertEmpty( $sidebars_widgets['gc_inactive_widgets'] );
	}
	...
	public function test_retrieve_widgets_for_customizer() {
		...
		$this->assertNotEquals( $sidebars_widgets, gc_get_sidebars_widgets() );
	}
	...
	public function test_retrieve_widgets_with_single_widget() {
		...
		gc_register_sidebar_widget( 'single', 'Single', '__return_false', array(), array() );
		gc_register_widget_control( 'single', 'Single', '__return_false', array(), array() );

		$this->register_sidebars( array( 'sidebar-1', 'sidebar-2', 'gc_inactive_widgets' ) );
		...
		$this->assertContains( 'single', $result['gc_inactive_widgets'] );
	}
	...
	public function test_retrieve_widgets_move_orphaned_widgets_to_inactive() {
		...
		$sidebars_widgets = array(
			'sidebar-1'           => array( 'tag_cloud-1' ),
			'sidebar-2'           => array( 'text-1' ),
			'gc_inactive_widgets' => array( 'search-2', 'archives-2' ),
		...
		$this->assertCount( 6, $sidebars_widgets['gc_inactive_widgets'] );
		$this->assertContains( 'calendar-1', $sidebars_widgets['gc_inactive_widgets'] );
		$this->assertArrayNotHasKey( 'orphaned_widgets_1', $sidebars_widgets );
	...
	/**
	 * Test _gc_remove_unregistered_widgets.
	 *
	 * @covers ::_gc_remove_unregistered_widgets
	 */
	public function test__gc_remove_unregistered_widgets() {
		$widgets = array(
			'sidebar-1'           => array( 'tag_cloud-1' ),
			'sidebar-2'           => array( 'text-1' ),
			'fantasy'             => array( 'archives-2' ),
		...
		$filtered_widgets = _gc_remove_unregistered_widgets( $widgets, $allowed_widgets );
	...
	public function test_one_sidebar_each() {
		...
		$new_next_theme_sidebars = gc_map_sidebars_widgets( $prev_theme_sidebars );
