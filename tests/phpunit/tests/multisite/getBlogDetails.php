...
if ( is_multisite() ) :
	 ...
	 * @group multisite
	 */
	class Tests_Multisite_GetBlogDetails extends GC_UnitTestCase {
		protected static $network_ids;
		protected static $site_ids;

		public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
			self::$site_ids = array(
				GC_TESTS_DOMAIN . '/foo/'      => array(
					'domain' => GC_TESTS_DOMAIN,
					'path'   => '/foo/',
				),
				'foo.' . GC_TESTS_DOMAIN . '/' => array(
					'domain' => 'foo.' . GC_TESTS_DOMAIN,
					'path'   => '/',
				),
		...
		public function test_get_blog_details_with_site_name_string_subdirectory() {
			...
			$this->assertEquals( self::$site_ids[ GC_TESTS_DOMAIN . '/foo/' ], $site->blog_id );
		}
		...
		public function test_get_blog_details_with_site_name_string_subdomain() {
			...
			$site = get_blog_details( 'foo' );
			$this->assertEquals( self::$site_ids[ 'foo.' . GC_TESTS_DOMAIN . '/' ], $site->blog_id );
		}
		...
		public function test_get_blog_details_with_site_id_int() {
			$site = get_blog_details( self::$site_ids['www.gechiui.com/'] );
			$this->assertEquals( self::$site_ids['www.gechiui.com/'], $site->blog_id );
		}
		...
		public function test_get_blog_details_with_blog_id_in_fields() {
			$site = get_blog_details( array( 'blog_id' => self::$site_ids['www.gechiui.com/'] ) );
			$this->assertEquals( self::$site_ids['www.gechiui.com/'], $site->blog_id );
		}
		...
		public function test_get_blog_details_with_only_domain_in_fields_subdomain() {
			...
			$site = get_blog_details( array( 'domain' => 'www.gechiui.com' ) );
			$this->assertSame( self::$site_ids['www.gechiui.com/'], $site->blog_id );
		}
