...
if ( is_multisite() ) :
	 ...
	 * @group multisite
	 */
	class Tests_Multisite_Site extends GC_UnitTestCase {
		protected $suppress                = false;
		protected $site_status_hooks       = array();
		protected $gc_initialize_site_args = array();
		protected $gc_initialize_site_meta = array();
		...
		public function set_up() {
			global $gcdb;
			parent::set_up();
			$this->suppress = $gcdb->suppress_errors();
		}

		public function tear_down() {
			global $gcdb;
			$gcdb->suppress_errors( $this->suppress );
			parent::tear_down();
		}
		...
		public static function gcSetUpBeforeClass( GC_UnitTest_Factory $factory ) {
			...
			remove_action( 'gc_initialize_site', 'gc_initialize_site', 10 );
			self::$uninitialized_site_id = gc_insert_site(
				array(
			...
			add_action( 'gc_initialize_site', 'gc_initialize_site', 10, 2 );
		}

		public static function gcTearDownAfterClass() {
			global $gcdb;

			remove_action( 'gc_uninitialize_site', 'gc_uninitialize_site', 10 );
			gc_delete_site( self::$uninitialized_site_id );
			add_action( 'gc_uninitialize_site', 'gc_uninitialize_site', 10, 1 );
		...
		public function test_switch_restore_blog() {
			global $_gc_switched_stack, $gcdb;

			$this->assertSame( array(), $_gc_switched_stack );
			$this->assertFalse( ms_is_switched() );
			$current_blog_id = get_current_blog_id();
			$this->assertIsInt( $current_blog_id );

			gc_cache_set( 'switch-test', $current_blog_id, 'switch-test' );
			$this->assertSame( $current_blog_id, gc_cache_get( 'switch-test', 'switch-test' ) );

			$blog_id = self::factory()->blog->create();

			$cap_key = gc_get_current_user()->cap_key;
			switch_to_blog( $blog_id );
			$this->assertNotEquals( $cap_key, gc_get_current_user()->cap_key );
			$this->assertSame( array( $current_blog_id ), $_gc_switched_stack );
			$this->assertTrue( ms_is_switched() );
			$this->assertSame( $blog_id, $gcdb->blogid );
			$this->assertFalse( gc_cache_get( 'switch-test', 'switch-test' ) );
			gc_cache_set( 'switch-test', $blog_id, 'switch-test' );
			$this->assertSame( $blog_id, gc_cache_get( 'switch-test', 'switch-test' ) );

			switch_to_blog( $blog_id );
			$this->assertSame( array( $current_blog_id, $blog_id ), $_gc_switched_stack );
			$this->assertTrue( ms_is_switched() );
			$this->assertSame( $blog_id, $gcdb->blogid );
			$this->assertSame( $blog_id, gc_cache_get( 'switch-test', 'switch-test' ) );
			...
			restore_current_blog();
			$this->assertSame( $cap_key, gc_get_current_user()->cap_key );
		...
		public function test_created_site_details() {
			...
			$prefix = $gcdb->get_blog_prefix( $blog_id );
			...
			$this->assertEquals( $details, gc_cache_get( $blog_id . 'short', 'blog-details' ) );
			...
			$this->assertFalse( gc_cache_get( $blog_id, 'blog-details' ) );
			$this->assertFalse( gc_cache_get( $key, 'blog-lookup' ) );

			// $get_all = true, populate the full blog-details cache and the blog slug lookup cache.
			$details = get_blog_details( $blog_id, true );
			$this->assertEquals( $details, gc_cache_get( $blog_id, 'blog-details' ) );
			$this->assertEquals( $details, gc_cache_get( $key, 'blog-lookup' ) );

			// Check existence of each database table for the created site.
			foreach ( $gcdb->tables( 'blog', false ) as $table ) {
				$suppress = $gcdb->suppress_errors();

				// phpcs:ignore GeChiUI.DB.PreparedSQL.InterpolatedNotPrepared
				$table_fields = $gcdb->get_results( "DESCRIBE $prefix$table;" );
				...
				// phpcs:ignore GeChiUI.DB.PreparedSQL.InterpolatedNotPrepared
				$result = $gcdb->get_results( "SELECT * FROM $prefix$table LIMIT 1" );
		...
		public function test_site_caches_should_invalidate_when_invalidation_is_not_suspended() {
			...
			$suspend = gc_suspend_cache_invalidation( false );
		 ...
		 */
		public function test_data_in_cache_after_gcmu_delete_blog_drop_false() {
			$blog_id = self::factory()->blog->create();
			...
			gcmu_delete_blog( $blog_id, false );

			$this->assertFalse( gc_cache_get( $blog_id, 'blog-details' ) );
			$this->assertFalse( gc_cache_get( $blog_id . 'short', 'blog-details' ) );
			$this->assertFalse( gc_cache_get( $key, 'blog-lookup' ) );
			$this->assertFalse( gc_cache_get( $key, 'blog-id-cache' ) );
		}

		/**
		 * When a site is flagged as 'deleted', its data should remain in the database.
		 */
		public function test_data_in_tables_after_gcmu_delete_blog_drop_false() {
			global $gcdb;
		 ...
		 */
		public function test_data_in_cache_after_gcmu_delete_blog_drop_true() {
			$blog_id = self::factory()->blog->create();
		 ...
		 */
		public function test_data_in_tables_after_gcmu_delete_blog_drop_true() {
			global $gcdb;
		 ...
		 */
		public function test_data_in_cache_after_gcmu_delete_blog_main_site_drop_true() {
		 ...
		 */
		public function test_data_in_tables_after_gcmu_delete_blog_main_site_drop_true() {
			global $gcdb;
		 ...
		 */
		public function test_network_count_after_gcmu_delete_blog_drop_false() {
			$blog_id = self::factory()->blog->create();
		 ...
		 */
		public function test_blog_count_after_gcmu_delete_blog_drop_true() {
			$blog_id = self::factory()->blog->create();
		 ...
		 */
		public function test_upload_directories_after_multiple_gcmu_delete_blog() {
		...
		public function test_gcmu_update_blogs_date() {
			global $gcdb;

			gcmu_update_blogs_date();
		...
		public function test_get_blog_details_when_site_does_not_exist() {
			...
			$this->assertSame( -1, gc_cache_get( $blog_id, 'blog-details' ) );
			...
			$this->assertEquals( $blog, gc_cache_get( $blog_id, 'blog-details' ) );
		}
		...
		public function test_get_blog_id_from_url() {
			...
			$this->assertSame( $blog_id, gc_cache_get( $key, 'blog-id-cache' ) );
		}
		...
		public function test_get_blog_id_from_url_that_does_not_exist() {
			...
			$this->assertSame( -1, gc_cache_get( md5( $details->domain . 'foo' ), 'blog-id-cache' ) );
		}

		/**
		 * A blog ID is still available if only the `deleted` flag is set for a site. The same
		 * behavior would be expected if passing `false` explicitly to `gcmu_delete_blog()`.
		 */
		public function test_get_blog_id_from_url_with_deleted_flag() {
			$blog_id = self::factory()->blog->create();
			$details = get_site( $blog_id );
			$key     = md5( $details->domain . $details->path );
			gcmu_delete_blog( $blog_id );
		...
		public function test_get_blog_id_from_url_after_dropped() {
			...
			$this->assertSame( -1, gc_cache_get( $key, 'blog-id-cache' ) );
		}
		...
		public function test_switch_upload_dir() {
			...
			switch_to_blog( $blog_id );
			$info = gc_upload_dir();
			$this->assertSame( 'http://' . $site->domain . '/gc-content/uploads/sites/' . get_current_blog_id() . '/' . $date, $info['url'] );
			$this->assertSame( ABSPATH . 'gc-content/uploads/sites/' . get_current_blog_id() . '/' . $date, $info['path'] );
			$this->assertSame( '/' . $date, $info['subdir'] );
			$this->assertFalse( $info['error'] );
			restore_current_blog();
		...
		public function test_get_blogaddress_by_id_with_valid_id() {
			...
			$this->assertSame( 'http://' . GC_TESTS_DOMAIN . '/', $blogaddress );
		}
		...
		public function test_new_blog_url_schemes( $home_scheme, $siteurl_scheme, $force_ssl_admin ) {
			...
			// Install:
			$new = gcmu_create_blog( $current_site->domain, '/new-blog/', 'New Blog', get_current_user_id() );
		...
		public function test_new_blog_locale() {
			...
			add_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10, 3 );
			update_site_option( 'GCLANG', 'de_DE' );
			remove_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10 );

			// No locale, use default locale.
			add_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10, 3 );
			$blog_id = gcmu_create_blog( $current_site->domain, '/de-de/', 'New Blog', get_current_user_id() );
			remove_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10 );

			$this->assertNotWPError( $blog_id );
			$this->assertSame( 'de_DE', get_blog_option( $blog_id, 'GCLANG' ) );

			// Custom locale.
			add_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10, 3 );
			$blog_id = gcmu_create_blog( $current_site->domain, '/es-es/', 'New Blog', get_current_user_id(), array( 'GCLANG' => 'es_ES' ) );
			remove_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10 );

			$this->assertNotWPError( $blog_id );
			$this->assertSame( 'es_ES', get_blog_option( $blog_id, 'GCLANG' ) );

			// en_US locale.
			add_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10, 3 );
			$blog_id = gcmu_create_blog( $current_site->domain, '/en-us/', 'New Blog', get_current_user_id(), array( 'GCLANG' => '' ) );
			remove_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10 );

			$this->assertNotWPError( $blog_id );
			$this->assertSame( '', get_blog_option( $blog_id, 'GCLANG' ) );
		}
		...
		public function test_different_network_language() {
			$network = get_network( self::$network_ids['make.gechiui.com/'] );

			add_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10, 3 );

			update_network_option( self::$network_ids['make.gechiui.com/'], 'GCLANG', 'wibble' );
			$blog_id = gcmu_create_blog( $network->domain, '/de-de/', 'New Blog', get_current_user_id(), array(), $network->id );

			remove_filter( 'sanitize_option_GCLANG', array( $this, 'filter_allow_unavailable_languages' ), 10 );

			$this->assertSame( get_network_option( self::$network_ids['make.gechiui.com/'], 'GCLANG' ), get_blog_option( $blog_id, 'GCLANG' ) );
		}

		/**
		 * Allows to set the GCLANG option to any language.
		 *
		...
		public function test_is_main_site_different_network() {
			$this->assertTrue( is_main_site( self::$site_ids['make.gechiui.com/'], self::$network_ids['make.gechiui.com/'] ) );
		}
		...
		public function test_is_main_site_different_network_random_site() {
			$this->assertFalse( is_main_site( self::$site_ids['make.gechiui.com/foo/'], self::$network_ids['make.gechiui.com/'] ) );
		}
		...
		public function test_clean_blog_cache( $key, $group ) {
			$site = get_site( self::$site_ids['make.gechiui.com/'] );
			...
			if ( 'sites' === $group ) { // This needs to be actual data for get_site() lookups.
				gc_cache_set( $key, (object) $site->to_array(), $group );
			} else {
				gc_cache_set( $key, 'something', $group );
			}

			clean_blog_cache( $site );
			$this->assertFalse( gc_cache_get( $key, $group ) );
		}
		...
		public function test_clean_blog_cache_resets_last_changed() {
			...
			gc_cache_delete( 'last_changed', 'sites' );

			clean_blog_cache( $site );
			$this->assertNotFalse( gc_cache_get( 'last_changed', 'sites' ) );
		}
		...
		public function test_clean_blog_cache_works_with_deleted_site() {
			...
			gc_cache_set( $site_id, 'something', 'site-details' );

			clean_blog_cache( $site_id );
			$this->assertFalse( gc_cache_get( $site_id, 'site-details' ) );
		}
		...
		/**
		 * @ticket 40364
		 * @dataProvider data_gc_insert_site
		 */
		public function test_gc_insert_site( $site_data, $expected_data ) {
			remove_action( 'gc_initialize_site', 'gc_initialize_site', 10 );
			$site_id = gc_insert_site( $site_data );
		...
		public function data_gc_insert_site() {
			return array(
				array(
					array(
						'domain' => 'example.com',
		 ...
		 * @ticket 40364
		 */
		public function test_gc_insert_site_empty_domain() {
			remove_action( 'gc_initialize_site', 'gc_initialize_site', 10 );
			$site_id = gc_insert_site( array( 'public' => 0 ) );
		...
		/**
		 * @ticket 40364
		 * @dataProvider data_gc_update_site
		 */
		public function test_gc_update_site( $site_data, $expected_data ) {
			$site_id = self::factory()->blog->create();

			$old_site = get_site( $site_id );

			$result = gc_update_site( $site_id, $site_data );
		...
		public function data_gc_update_site() {
			return array(
				array(
					array(
						'domain'     => 'example.com',
		 ...
		 * @ticket 40364
		 */
		public function test_gc_update_site_empty_domain() {
			$site_id = self::factory()->blog->create();

			$result = gc_update_site( $site_id, array( 'domain' => '' ) );
		 ...
		 * @ticket 40364
		 */
		public function test_gc_update_site_invalid_id() {
			$result = gc_update_site( 444444, array( 'domain' => 'example.com' ) );
		 ...
		 * @ticket 40364
		 */
		public function test_gc_update_site_cleans_cache() {
			$site_id = self::factory()->blog->create();
			$site1   = get_site( $site_id );

			$result = gc_update_site( $site_id, array( 'public' => 0 ) );
			$site2  = get_site( $site_id );

			$result = gc_update_site( $site_id, array( 'public' => 1 ) );
		 ...
		 * @ticket 40364
		 */
		public function test_gc_delete_site() {
			$site_id = self::factory()->blog->create();

			$site = get_site( $site_id );

			$result = gc_delete_site( $site_id );

			$this->assertInstanceOf( 'GC_Site', $result );
		 ...
		 * @ticket 40364
		 */
		public function test_gc_delete_site_invalid_id() {
			$result = gc_delete_site( 444444 );
		 ...
		 * @ticket 41333
		 */
		public function test_gc_delete_site_validate_site_deletion_action() {
			add_action( 'gc_validate_site_deletion', array( $this, 'action_gc_validate_site_deletion_prevent_deletion' ) );
			$result = gc_delete_site( self::$site_ids['make.gechiui.com/'] );
			$this->assertWPError( $result );
			$this->assertSame( 'action_does_not_like_deletion', $result->get_error_code() );
		}

		public function action_gc_validate_site_deletion_prevent_deletion( $errors ) {
			$errors->add( 'action_does_not_like_deletion', 'You cannot delete this site because the action does not like it.' );
		}

		/**
		 * @ticket 40364
		 * @dataProvider data_gc_normalize_site_data
		 */
		public function test_gc_normalize_site_data( $data, $expected ) {
			$result = gc_normalize_site_data( $data );

			$this->assertSameSetsWithIndex( $expected, $result );
		}

		public function data_gc_normalize_site_data() {
			return array(
				array(
					array(
						'network_id' => '4',
		...
		/**
		 * @ticket 40364
		 * @dataProvider data_gc_validate_site_data
		 */
		public function test_gc_validate_site_data( $data, $expected_errors ) {
			$result = new GC_Error();
			gc_validate_site_data( $result, $data );
		...
		public function data_gc_validate_site_data() {
		...
		public function test_site_dates_are_gmt() {
			...
			$site_id     = gc_update_site( $site_id, array() );
			$this->assertIsInt( $site_id );
		 ...
		 * @ticket 40364
		 */
		public function test_gc_delete_site_cleans_cache() {
			$site_id = self::factory()->blog->create();
		 ...
		 * @ticket 40364
		 */
		public function test_gc_update_site_cleans_old_cache_on_domain_change() {
			$old_domain = 'old.gechiui.com';
			$new_domain = 'new.gechiui.com';
			...
			$result = array(
				gc_cache_get( $domain_path_key_old, 'blog-lookup' ),
				gc_cache_get( $domain_path_key_old, 'blog-id-cache' ),
				gc_cache_get( 'current_blog_' . $old_domain, 'site-options' ),
				gc_cache_get( 'current_blog_' . $old_domain . '/', 'site-options' ),
				gc_cache_get( $domain_path_key_new, 'blog-lookup' ),
				gc_cache_get( $domain_path_key_new, 'blog-id-cache' ),
				gc_cache_get( 'current_blog_' . $new_domain, 'site-options' ),
				gc_cache_get( 'current_blog_' . $new_domain . '/', 'site-options' ),
			);
		 ...
		 * @ticket 40364
		 */
		public function test_gc_update_site_cleans_old_cache_on_path_change() {
			$old_path = '/foo/';
			$new_path = '/bar/';

			$site = self::factory()->blog->create_and_get(
				array(
					'domain' => 'test.gechiui.com',
					'path'   => $old_path,
				)
			);
			...
			get_blog_id_from_url( 'test.gechiui.com', $old_path );
			get_blog_details(
				array(
					'domain' => 'test.gechiui.com',
					'path'   => $new_path,
			...
			get_blog_id_from_url( 'test.gechiui.com', $new_path );
			...
			$domain_path_key_old = md5( 'test.gechiui.com' . $old_path );
			$domain_path_key_new = md5( 'test.gechiui.com' . $new_path );

			// Ensure all respective cache values are empty.
			$result = array(
				gc_cache_get( $domain_path_key_old, 'blog-lookup' ),
				gc_cache_get( $domain_path_key_old, 'blog-id-cache' ),
				gc_cache_get( 'current_blog_test.gechiui.com' . $old_path, 'site-options' ),
				gc_cache_get( $domain_path_key_new, 'blog-lookup' ),
				gc_cache_get( $domain_path_key_new, 'blog-id-cache' ),
				gc_cache_get( 'current_blog_test.gechiui.com' . $new_path, 'site-options' ),
			);
		...
		public function test_site_status_hook_triggers( $insert_site_data, $expected_insert_hooks, $update_site_data, $expected_update_hooks ) {
			...
			gc_update_site( $site_id, $update_site_data );
		...
		/**
		 * @ticket 41333
		 * @dataProvider data_gc_initialize_site
		 */
		public function test_gc_initialize_site( $args, $expected_options, $expected_meta ) {
			$result = gc_initialize_site( self::$uninitialized_site_id, $args );
			...
			$initialized = gc_is_site_initialized( self::$uninitialized_site_id );

			gc_uninitialize_site( self::$uninitialized_site_id );
		...
		public function data_gc_initialize_site() {
			return array(
				array(
					array(),
					array(
		 ...
		 * @ticket 41333
		 */
		public function test_gc_initialize_site_user_roles() {
			global $gcdb;

			$result = gc_initialize_site( self::$uninitialized_site_id, array() );

			switch_to_blog( self::$uninitialized_site_id );
			$table_prefix = $gcdb->get_blog_prefix( self::$uninitialized_site_id );
		 ...
		 * @ticket 41333
		 */
		public function test_gc_initialize_site_user_is_admin() {
			$result = gc_initialize_site( self::$uninitialized_site_id, array( 'user_id' => 1 ) );
		 ...
		 * @ticket 41333
		 */
		public function test_gc_initialize_site_args_filter() {
			add_filter( 'gc_initialize_site_args', array( $this, 'filter_gc_initialize_site_args' ), 10, 3 );
			$result = gc_initialize_site( self::$uninitialized_site_id, array( 'title' => 'My Site' ) );
		...
		public function filter_gc_initialize_site_args( $args, $site, $network ) {
		 ...
		 * @ticket 41333
		 */
		public function test_gc_initialize_site_empty_id() {
			$result = gc_initialize_site( 0 );
			$this->assertWPError( $result );
		 ...
		 * @ticket 41333
		 */
		public function test_gc_initialize_site_invalid_id() {
			$result = gc_initialize_site( 123 );
			$this->assertWPError( $result );
		 ...
		 * @ticket 41333
		 */
		public function test_gc_initialize_site_already_initialized() {
			$result = gc_initialize_site( get_current_blog_id() );
			$this->assertWPError( $result );
		 ...
		 * @ticket 41333
		 */
		public function test_gc_uninitialize_site() {
			$site_id = self::factory()->blog->create();

			$result = gc_uninitialize_site( $site_id );
			$this->assertTrue( $result );
			$this->assertFalse( gc_is_site_initialized( $site_id ) );
		}

		/**
		 * @ticket 41333
		 */
		public function test_gc_uninitialize_site_empty_id() {
			$result = gc_uninitialize_site( 0 );
			$this->assertWPError( $result );
			$this->assertSame( 'site_empty_id', $result->get_error_code() );
		}
		 ...
		 * @ticket 41333
		 */
		public function test_gc_uninitialize_site_invalid_id() {
			$result = gc_uninitialize_site( 123 );
			$this->assertWPError( $result );
			$this->assertSame( 'site_invalid_id', $result->get_error_code() );
		}
		 ...
		 * @ticket 41333
		 */
		public function test_gc_uninitialize_site_already_uninitialized() {
			$result = gc_uninitialize_site( self::$uninitialized_site_id );
			$this->assertWPError( $result );
		 ...
		 * @ticket 41333
		 */
		public function test_gc_is_site_initialized() {
			$this->assertTrue( gc_is_site_initialized( get_current_blog_id() ) );
			$this->assertFalse( gc_is_site_initialized( self::$uninitialized_site_id ) );
		}

		/**
		 * @ticket 41333
		 */
		public function test_gc_is_site_initialized_prefilter() {
			add_filter( 'pre_gc_is_site_initialized', '__return_false' );
			$this->assertFalse( gc_is_site_initialized( get_current_blog_id() ) );

			add_filter( 'pre_gc_is_site_initialized', '__return_true' );
			$this->assertTrue( gc_is_site_initialized( self::$uninitialized_site_id ) );
		}

		/**
		 * @ticket 41333
		 */
		public function test_gc_insert_site_forwards_args_to_gc_initialize_site() {
			$args = array(
				'user_id' => 1,
				'title'   => 'My Site',
			...
			add_filter( 'gc_initialize_site_args', array( $this, 'filter_gc_initialize_site_args_catch_args' ) );
			$site_id = gc_insert_site(
				array_merge(
					array(
						'domain' => 'testsite.org',
			...
			$passed_args                   = $this->gc_initialize_site_args;
			$this->gc_initialize_site_args = null;

			$this->assertSameSetsWithIndex( $args, $passed_args );
		}

		public function filter_gc_initialize_site_args_catch_args( $args ) {
			$this->gc_initialize_site_args = $args;
		 ...
		 * @ticket 46125
		 */
		public function test_gcmu_create_blog_cache_cleanup_backward_compatible() {
			add_action( 'populate_options', array( $this, 'populate_options_callback' ) );

			$blog_id = gcmu_create_blog( 'testsite1.example.org', '/test', 'test', 1, array( 'public' => 1 ), 2 );
		...
		/**
		 * Tests whether all expected meta are provided in deprecated `gcmu_new_blog` action.
		 *
		 * @dataProvider data_gcmu_new_blog_action_backward_commpatible
		 *
		 * @ticket 46351
		 */
		public function test_gcmu_new_blog_action_backward_compatible( $meta, $expected_meta ) {
			// We are testing deprecated hook. Register it to expected deprecated notices.
			$this->setExpectedDeprecated( 'gcmu_new_blog' );
			add_action( 'gcmu_new_blog', array( $this, 'gcmu_new_blog_callback' ), 10, 6 );

			gcmu_create_blog( 'testsite1.example.org', '/new-blog/', 'New Blog', get_current_user_id(), $meta, 1 );

			$this->assertEquals( $expected_meta, $this->gc_initialize_site_meta );

			$this->gc_initialize_site_meta = array();
		}
		...
		public function test_get_site_not_found_cache_clear() {
			...
			$this->assertInstanceOf( 'GC_Site', $fetched_site );
		...
		protected function _get_next_site_id() {
			...
			// Get the ID after it.
			return (int) $gcdb->get_var( 'SELECT blog_id FROM ' . $gcdb->blogs . ' ORDER BY blog_ID DESC LIMIT 1' ) + 1;
		}

		/**
		 * Capture the $meta value passed to the gcmu_new_blog action and compare it.
		 */
		public function gcmu_new_blog_callback( $blog_id, $user_id, $domain, $path, $network_id, $meta ) {
			$this->gc_initialize_site_meta = $meta;
		}

		public function data_gcmu_new_blog_action_backward_commpatible() {
			return array(
				'default values' => array(
					array(),
					array(
						'public' => 0, // `public` is one of the default metas in `gcmu_create_blog()' function prior to GeChiUI 5.1.0.
						'GCLANG' => 'en_US', // GCLANG is another default meta in `gcmu_create_blog()` function prior to GeChiUI 5.1.0.
					),
				),
				'public site'    => array(
					array(
