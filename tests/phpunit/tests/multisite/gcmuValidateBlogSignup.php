...
if ( is_multisite() ) :
	 ...
	 * @group multisite
	 */
	class Tests_Multisite_gcmuValidateBlogSignup extends GC_UnitTestCase {
		...
		public static function gcTearDownAfterClass() {
			...
			gcmu_delete_user( self::$super_admin_id );

			gcmu_delete_user( self::$existing_user_id );

			gc_delete_site( self::$existing_blog_id );
		}
		...
		public function test_validate_blogname( $blog_name, $error_message ) {
			$result = gcmu_validate_blog_signup( $blog_name, 'Foo Site Title', get_userdata( self::$super_admin_id ) );
		...
		public function test_validate_empty_blog_title() {
			$result = gcmu_validate_blog_signup( 'uniqueblogname1234', '', get_userdata( self::$super_admin_id ) );
		...
		public function test_validate_blogname_from_same_existing_user() {
			$result = gcmu_validate_blog_signup( self::$existing_user_login, 'Foo Site Title', get_userdata( self::$existing_user_id ) );
		...
		public function test_filter_minimum_site_name_length( $site_name, $minimum_length, $expect_error ) {
			...
			$result = gcmu_validate_blog_signup( $site_name, 'Site Title', get_userdata( self::$super_admin_id ) );
		...
		public function test_signup_nonce_check() {
			...
			$valid               = gcmu_validate_blog_signup( 'my-nonce-site', 'Site Title', get_userdata( self::$super_admin_id ) );
