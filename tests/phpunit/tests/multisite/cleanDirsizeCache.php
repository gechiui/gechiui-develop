...
if ( is_multisite() ) :
	 ...
	 * @group multisite
	 */
	class Tests_Multisite_CleanDirsizeCache extends GC_UnitTestCase {
		...
		public function test_get_dirsize_cache_in_recurse_dirsize_upload() {
			...
			// Upload a file to the new site using gc_upload_bits().
			$filename = __FUNCTION__ . '.jpg';
			$contents = __FUNCTION__ . '_contents';
			$file     = gc_upload_bits( $filename, null, $contents );
		...
		public function test_5_5_transient_structure_compat() {
			 ...
			 */
			gc_mkdir_p( $upload_dir['basedir'] . '/2/1' );
		...
		private function get_mock_5_5_dirsize_cache( $site_id ) {
			$prefix = untrailingslashit( gc_upload_dir()['basedir'] );
